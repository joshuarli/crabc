...
impl<'main> MainHeapThreadProcessPageExitFullSingletonOrRegularPagesRoute<'main> {
    /// Routes one exact client free through the bounded mixed
    /// singleton/regular aggregate after the originating later Theap/TLD has
    /// torn down.
    ///
    /// # Safety
    ///
    ...
    /// It must not be freed, transferred, or concurrently used through
    /// another route. This consuming API serializes one source failed-reclaim
    /// decision at a time; it does not make the aggregate an allocation-time,
    /// reclaim/requeue, producer, or concurrent-free interface.
    pub(crate) unsafe fn remote_free_after_thread_exit(
        self,
        block: NonNull<u8>,
    ) -> Result<
        MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeResult<'main>,
        MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeFailure<'main>,
    > {
        let Self {
            mut parts,
            page_map_access,
        ...
        let free = page_map_access.with_page_map(|page_map| {
            // SAFETY: this boundary carries the caller's exact client-block
            // proof through source-class selection, lookup, low-owner claim,
            // optional mapped regular publication, and terminal span release.
            unsafe { parts.remote_free_after_thread_exit(page_map, block) }
        });
        match free {
            Ok(Ok(ThreadExitFullSingletonOrRegularPagesPostExitFreeOutcome::StillLive)) => Ok(
                MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeResult::StillLive(
                    Self {
                        parts,
                        page_map_access,
                    },
            ...
            Ok(Ok(ThreadExitFullSingletonOrRegularPagesPostExitFreeOutcome::ReleasedPage)) => Ok(
                MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeResult::ReleasedPage(
                    Self {
                        parts,
                        page_map_access,
                    },
            ...
            Ok(Ok(ThreadExitFullSingletonOrRegularPagesPostExitFreeOutcome::ReleasedAll)) => {
                // The selected source tail has removed the final PageMap span,
                // its bitmap/count state where applicable, ordinary arena bit,
                // metadata, and exact backing span. Drop the empty composed
                // registry before reopening its process map.
                drop(parts);
                // SAFETY: `ReleasedAll` is emitted only after the aggregate
                // terminal count reached zero following the final physical
                // source-member release.
                match unsafe { page_map_access.finish_after_all_pages_released() } {
                    Ok(()) => Ok(
                        MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeResult::ReleasedAll,
                    ),
                    Err(error) => Err(
                        MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeFailure::ReleasedAllPageMapPoisoned {
                            error,
                        },
                    ),
                }
            ...
            Ok(Err(error)) => {
                ...
                let error = MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeError::Route(error);
                if matches!(
                    error,
                    MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeError::Route(
                        ThreadExitFullSingletonOrRegularPagesPostExitFreeError::Unmapped
                            | ThreadExitFullSingletonOrRegularPagesPostExitFreeError::InvalidBlock
                            | ThreadExitFullSingletonOrRegularPagesPostExitFreeError::Singleton(
                                ThreadExitFullSingletonPagesPostExitFreeError::Unmapped
                                    | ThreadExitFullSingletonPagesPostExitFreeError::InvalidBlock
                            )
                            | ThreadExitFullSingletonOrRegularPagesPostExitFreeError::Regular(
                                ThreadExitFullMediumPagesPostExitFreeError::Unmapped
                            )
                    )
                ) {
                    Err(
                        MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeFailure::Rejected {
                            route,
                            error,
                        },
                    )
                    ...
                    Err(
                        MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeFailure::Terminal {
                            route,
                            error,
                        },
                    )
            ...
            Err(error) => Err(
                MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeFailure::Terminal {
                    ...
                    error: MainHeapThreadProcessPageExitFullSingletonOrRegularPagesFreeError::PageMap(error),
                },
            ),
        }
    }
    ...
    pub(crate) fn test_regular_abandoned_count_for_bin(&self, bin: usize) -> Option<usize> {
        self.parts.test_regular_abandoned_count_for_bin(bin)
    }
}
...
impl<'main> MainHeapThreadProcessPageExitFullNonDirectSmallPagesRoute<'main> {
    ...
    /// It must not be freed, transferred, or concurrently used through
    /// another route. This consuming API serializes one source normal-collector
    /// failed-reclaim decision at a time; it does not make the aggregate a
    /// direct-small, allocation-time, reclaim/requeue, or concurrent-free
    /// interface.
    pub(crate) unsafe fn remote_free_after_thread_exit(
        self,
        block: NonNull<u8>,
    ) -> Result<
        MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeResult<'main>,
        MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeFailure<'main>,
    > {
        let Self {
            mut parts,
            page_map_access,
        ...
        let free = page_map_access.with_page_map(|page_map| {
            // SAFETY: this boundary carries the caller's exact client-block
            // proof through lookup, low-owner claim, an optional
            // unmapped-to-mapped publication, and any terminal one-slice
            // release.
            unsafe { parts.remote_free_after_thread_exit(page_map, block) }
        });
        match free {
            Ok(Ok(ThreadExitFullNonDirectSmallPagesPostExitFreeOutcome::StillLive)) => Ok(
                MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeResult::StillLive(Self {
                    parts,
                    page_map_access,
                }),
            ),
            Ok(Ok(ThreadExitFullNonDirectSmallPagesPostExitFreeOutcome::ReleasedPage)) => Ok(
                MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeResult::ReleasedPage(Self {
                    parts,
                    page_map_access,
                }),
            ),
            Ok(Ok(ThreadExitFullNonDirectSmallPagesPostExitFreeOutcome::ReleasedAll)) => {
                // The source terminal release has removed the last member's
                // PageMap span, any mapped bitmap/count state, ordinary arena
                // bit, metadata, and slice. Drop the empty registry before
                // reopening its process map.
                drop(parts);
                // SAFETY: `ReleasedAll` is emitted only after the exact route
                // count reached zero following the final terminal release.
                match unsafe { page_map_access.finish_after_all_pages_released() } {
                    Ok(()) => Ok(
                        MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeResult::ReleasedAll,
                    ),
                    Err(error) => Err(
                        MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeFailure::ReleasedAllPageMapPoisoned {
                            error,
                        },
                    ),
                }
            ...
            Ok(Err(error)) => {
                ...
                let error = MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeError::Route(error);
                if matches!(
                    error,
                    MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeError::Route(
                        ThreadExitFullNonDirectSmallPagesPostExitFreeError::Unmapped
                    )
                ) {
                    Err(
                        MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeFailure::Rejected {
                            route,
                            error,
                        },
                    )
                    ...
                    Err(
                        MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeFailure::Terminal {
                            route,
                            error,
                        },
                    )
            ...
            Err(error) => Err(
                MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeFailure::Terminal {
                    ...
                    error: MainHeapThreadProcessPageExitFullNonDirectSmallPagesFreeError::PageMap(error),
                },
            ),
        }
    }
...
impl<'main> MainHeapThreadProcessPageExitFullDirectSmallPagesRoute<'main> {
    /// Routes one exact client free through the bounded homogeneous full
    /// direct-small aggregate after the originating later Theap/TLD has torn
    /// down.
    ///
    /// # Safety
    ///
    ...
    /// It must not be freed, transferred, or concurrently used through
    /// another route. This consuming API serializes one source partial-
    /// collector failed-reclaim decision at a time; it does not make the
    /// aggregate an allocation-time, reclaim/requeue, or concurrent-free
    /// interface.
    pub(crate) unsafe fn remote_free_after_thread_exit(
        self,
        block: NonNull<u8>,
    ) -> Result<
        MainHeapThreadProcessPageExitFullDirectSmallPagesFreeResult<'main>,
        MainHeapThreadProcessPageExitFullDirectSmallPagesFreeFailure<'main>,
    > {
        let Self {
            mut parts,
            page_map_access,
        ...
        match free {
            Ok(Ok(ThreadExitFullDirectSmallPagesPostExitFreeOutcome::StillLive)) => Ok(
                MainHeapThreadProcessPageExitFullDirectSmallPagesFreeResult::StillLive(Self {
                    parts,
                    page_map_access,
                }),
            ),
            Ok(Ok(ThreadExitFullDirectSmallPagesPostExitFreeOutcome::ReleasedPage)) => Ok(
                MainHeapThreadProcessPageExitFullDirectSmallPagesFreeResult::ReleasedPage(Self {
                    parts,
                    page_map_access,
                }),
            ),
            Ok(Ok(ThreadExitFullDirectSmallPagesPostExitFreeOutcome::ReleasedAll)) => {
                // The source terminal release has removed the last member's
                // PageMap span, any mapped bitmap/count state, ordinary arena
                // bit, metadata, and slice. Drop the empty registry before
                // reopening its process map.
                ...
                match unsafe { page_map_access.finish_after_all_pages_released() } {
                    Ok(()) => Ok(
                        MainHeapThreadProcessPageExitFullDirectSmallPagesFreeResult::ReleasedAll,
                    ),
                    Err(error) => Err(
                        MainHeapThreadProcessPageExitFullDirectSmallPagesFreeFailure::ReleasedAllPageMapPoisoned {
                            error,
                        },
                    ),
                }
            ...
            Ok(Err(error)) => {
                ...
                let error = MainHeapThreadProcessPageExitFullDirectSmallPagesFreeError::Route(error);
                if matches!(
                    error,
                    MainHeapThreadProcessPageExitFullDirectSmallPagesFreeError::Route(
                        ThreadExitFullDirectSmallPagesPostExitFreeError::Unmapped
                    )
                ) {
                    Err(
                        MainHeapThreadProcessPageExitFullDirectSmallPagesFreeFailure::Rejected {
                            route,
                            error,
                        },
                    )
                    ...
                    Err(
                        MainHeapThreadProcessPageExitFullDirectSmallPagesFreeFailure::Terminal {
                            route,
                            error,
                        },
                    )
            ...
            Err(error) => Err(
                MainHeapThreadProcessPageExitFullDirectSmallPagesFreeFailure::Terminal {
                    ...
                    error: MainHeapThreadProcessPageExitFullDirectSmallPagesFreeError::PageMap(error),
                },
            ),
        }
    }
...
impl<'main> MainHeapThreadProcessPageExitFullLargePagesRoute<'main> {
    ...
    pub(crate) unsafe fn remote_free_after_thread_exit(
        ...
        MainHeapThreadProcessPageExitFullLargePagesFreeResult<'main>,
        MainHeapThreadProcessPageExitFullLargePagesFreeFailure<'main>,
    > {
        let Self {
            mut parts,
            page_map_access,
        ...
        let free = page_map_access.with_page_map(|page_map| {
            // SAFETY: this boundary carries the caller's exact client-block
            // proof through lookup, low-owner claim, an optional
            // unmapped-to-mapped publication, and any terminal large-span
            // release.
            unsafe { parts.remote_free_after_thread_exit(page_map, block) }
        });
        match free {
            Ok(Ok(ThreadExitFullLargePagesPostExitFreeOutcome::StillLive)) => Ok(
                MainHeapThreadProcessPageExitFullLargePagesFreeResult::StillLive(Self {
                    parts,
                    page_map_access,
                }),
            ),
            Ok(Ok(ThreadExitFullLargePagesPostExitFreeOutcome::ReleasedPage)) => Ok(
                MainHeapThreadProcessPageExitFullLargePagesFreeResult::ReleasedPage(Self {
                    parts,
                    page_map_access,
                }),
            ),
            Ok(Ok(ThreadExitFullLargePagesPostExitFreeOutcome::ReleasedAll)) => {
                // The source terminal release has removed the last member's
                // PageMap span, any mapped bitmap/count state, ordinary arena
                // bit, metadata, and all large backing slices. Drop the empty
                // registry before reopening its process map.
                drop(parts);
                // SAFETY: `ReleasedAll` is emitted only after the exact route
                // count reached zero following the final terminal release.
                match unsafe { page_map_access.finish_after_all_pages_released() } {
                    Ok(()) => Ok(MainHeapThreadProcessPageExitFullLargePagesFreeResult::ReleasedAll),
                    Err(error) => Err(
                        MainHeapThreadProcessPageExitFullLargePagesFreeFailure::ReleasedAllPageMapPoisoned {
                            error,
                        },
                    ),
                }
            ...
            Ok(Err(error)) => {
                ...
                let error = MainHeapThreadProcessPageExitFullLargePagesFreeError::Route(error);
                if matches!(
                    error,
                    MainHeapThreadProcessPageExitFullLargePagesFreeError::Route(
                        ThreadExitFullLargePagesPostExitFreeError::Unmapped
                    )
                ) {
                    Err(
                        MainHeapThreadProcessPageExitFullLargePagesFreeFailure::Rejected {
                            route,
                            error,
                        },
                    )
                    ...
                    Err(
                        MainHeapThreadProcessPageExitFullLargePagesFreeFailure::Terminal {
                            route,
                            error,
                        },
                    )
            ...
            Err(error) => Err(
                MainHeapThreadProcessPageExitFullLargePagesFreeFailure::Terminal {
                    ...
                    error: MainHeapThreadProcessPageExitFullLargePagesFreeError::PageMap(error),
                },
            ),
        }
    }
...
mod tests {
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the singleton aggregate fixture");
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the singleton aggregate collection-failure fixture");
                    let request = LARGE_MAX_OBJ_SIZE + 1;
                    let first = allocator
                        .allocate(request, false)
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the singleton aggregate refusal fixture");
                    let block = allocator
                        .allocate(LARGE_MAX_OBJ_SIZE + 1, false)
    ...
    fn later_thread_exit_full_singleton_or_regular_pages_route_releases_each_source_tail() {
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the mixed singleton/regular fixture");
                    ...
                    assert_eq!(
                        ...
                        Some(true),
                        "the final regular free clears its ordinary main-arena bit"
                    );
                    assert_eq!(
                        page_map.begin_page_lifecycle().unwrap().finish(),
                        Ok(()),
                ...
                worker
                    .join()
                    .expect("mixed singleton/regular aggregate route remains current-thread local");
            });
        ...
        .join()
        .expect("mixed singleton/regular aggregate route fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the large post-exit route");
                    ...
                    assert_eq!(
                        unsafe { arena.pages() }.unwrap().is_clear_range(slice, slice_count),
                        Some(true),
                        "the final release leaves the complete large range clear before slice return"
                    );
                    assert_eq!(
                        page_map.begin_page_lifecycle().unwrap().finish(),
                        Ok(()),
                ...
                worker
                    .join()
                    .expect("post-exit large client frees remain local to the later owner fixture");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("post-exit mapped two-block large route fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the large refusal fixture");
                    let block = allocator
                    ...
                    let drain = match unsafe {
                        ...
                        Err(MainHeapThreadProcessPageExitMappedRegularRouteBeginFailure::PageMap {
                            ...
                            core::mem::forget(parts);
                            panic!("the two-block shape check runs before PageMap-route transfer: {error:?}");
                        }
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("one live large block must not create a process route");
                        }
                    };
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(block.as_ptr()) },
                        page.as_ptr(),
                        "the pre-detach refusal preserves PageMap publication"
                    );
                    assert_eq!(unsafe { page.as_ref().used() }, 1);
                    assert_eq!(
                ...
                let worker = scope.spawn(move || {
                    ...
                    if matches!(
                        ...
                        if let Some((capacity_before, extend)) = direct_extension {
                            ...
                            assert!(
                                !page_ref.free_list_head().is_null(),
                                "the direct-small scalar extension publishes an immediate local block"
                            );
                        }
                        {
                            let mut heap = main_heap
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the full-medium aggregate fixture");
                ...
                worker
                    .join()
                    .expect("the full-medium aggregate route remains local to its later owner fixture");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-medium aggregate route fixture remains current-thread local");
    }
    ...
    fn later_thread_exit_full_non_direct_small_pages_route_reabandons_each_distinct_bin_page_then_releases() {
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    for index in 0..PAGES_DIRECT {
                        assert_eq!(
                            allocator.test_direct_page(index),
                            Some(crate::types::EMPTY_PAGE.as_ptr()),
                            "the non-direct aggregate has no source direct-cache image"
                        );
                    }
                    let first_slice = first_ref
                        .memid()
                        .arena_memory()
                        .expect("the first non-direct small page belongs to the paired arena")
                        .slice_index as usize;
                    let second_slice = second_ref
                        .memid()
                        .arena_memory()
                        .expect("the second non-direct small page belongs to the paired arena")
                        .slice_index as usize;
                ...
                worker
                    .join()
                    .expect("the full non-direct-small aggregate route remains local to its later owner fixture");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full non-direct-small aggregate route fixture remains current-thread local");
    }
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let first_slice = first_ref
                        .memid()
                        .arena_memory()
                        .expect("the first direct-small page belongs to the paired arena")
                        .slice_index as usize;
                    let second_slice = second_ref
                        .memid()
                        .arena_memory()
                        .expect("the second direct-small page belongs to the paired arena")
                        .slice_index as usize;
                ...
                worker
                    .join()
                    .expect("the full direct-small aggregate route remains local to its later owner fixture");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full direct-small aggregate route fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the sole full-direct-small refusal fixture");
                    let first = allocator
                        .allocate(SMALL_SIZE_MAX, false)
                    ...
                    let drain = match unsafe {
                        ...
                        Err(
                            ...
                            core::mem::forget(parts);
                            panic!("a sole full direct-small page rejects before PageMap-route transfer: {error:?}");
                        }
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("a sole full direct-small page cannot enter the aggregate route");
                        }
                    };
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(first.as_ptr()) },
                        page.as_ptr(),
                        "the rejected aggregate preflight leaves PageMap publication untouched"
                    );
                    assert_eq!(unsafe { page.as_ref().used() }, capacity);
                    assert_eq!(
                        drain.test_queue_count(bin),
                    ...
                    assert_eq!(
                        owner.finish_after_page_drain(),
                        Err(MainHeapThreadAttachmentError::Poisoned),
                        "dropping a rejected drain cannot imitate process-route teardown"
                    );
                    core::mem::forget(owner);
                });
                worker
                    .join()
                    .expect("the full direct-small aggregate refusal remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full direct-small aggregate refusal fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the stale direct-cache fixture");
                    let first = allocator
                        .allocate(SMALL_SIZE_MAX, false)
                        .expect("the fixture creates its first direct-small page");
                    let first_page = NonNull::new(unsafe { allocator.test_page_for_block(first) })
                    ...
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(first.as_ptr()) },
                        first_page.as_ptr(),
                        "the rejected preflight leaves the first PageMap member published"
                    );
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(second.as_ptr()) },
                        second_page.as_ptr(),
                        "the rejected preflight leaves the second PageMap member published"
                    );
                ...
                worker
                    .join()
                    .expect("the stale direct-cache aggregate refusal remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("stale direct-cache aggregate refusal fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the direct aggregate collection-failure fixture");
                    let first = allocator
                        .allocate(SMALL_SIZE_MAX, false)
                        .expect("the fixture creates its first direct-small page");
                    let first_page = NonNull::new(unsafe { allocator.test_page_for_block(first) })
                    ...
                    let drain = match unsafe {
                        ...
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("the injected collection failure cannot create an aggregate route");
                        }
                    };
                    assert!(
                        drain.engine.test_has_collection_poison(),
                    ...
                    for index in 0..PAGES_DIRECT {
                        ...
                        assert_eq!(
                            ...
                            Some(expected),
                            "the failed force collection leaves the exact direct-cache image intact"
                        );
                    }
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(first.as_ptr()) },
                        first_page.as_ptr(),
                        "the injected failure leaves the first PageMap member published"
                    );
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(second.as_ptr()) },
                        second_page.as_ptr(),
                        "the injected failure leaves the second PageMap member published"
                    );
                ...
                worker
                    .join()
                    .expect("the direct aggregate collection-failure fixture remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full direct-small aggregate collection-failure fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the sole-full refusal fixture");
                    let request = SMALL_SIZE_MAX + WORD_SIZE;
                    let first = allocator
                        .allocate(request, false)
                    ...
                    while blocks.len() < capacity {
                        ...
                        assert_eq!(
                            unsafe { allocator.test_page_for_block(block) },
                            page.as_ptr(),
                            "the refusal fixture must not create an aggregate member"
                        );
                        blocks.push(block);
                    }
                    let page_ref = unsafe { page.as_ref() };
                    ...
                    let drain = match unsafe {
                        ...
                        Err(
                            ...
                            core::mem::forget(parts);
                            panic!("a sole full non-direct-small page rejects before PageMap-route transfer: {error:?}");
                        }
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("a sole full non-direct-small page cannot enter the aggregate route");
                        }
                    };
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(first.as_ptr()) },
                    ...
                    assert!(
                        !crate::types::page_queue::page_is_in_full(unsafe { page.as_ref() }),
                        "the rejected aggregate preflight leaves the ordinary-bin full flag unchanged"
                    );
                ...
                worker
                    .join()
                    .expect("the full non-direct-small aggregate refusal remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full non-direct-small aggregate refusal fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the aggregate collection-failure fixture");
                    let request = SMALL_SIZE_MAX + WORD_SIZE;
                ...
                worker
                    .join()
                    .expect("the aggregate collection-failure fixture remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full non-direct-small aggregate collection-failure fixture remains current-thread local");
    }
    ...
    fn later_thread_exit_full_large_pages_route_reabandons_each_distinct_bin_page_then_releases() {
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    while index < second_unmapped_frees {
                        route = match unsafe { route.remote_free_after_thread_exit(second_blocks[index]) } {
                            ...
                            Err(_) => panic!("a mostly-used second large page stays source-unmapped"),
                        };
                        index += 1;
                    }
                    route = match unsafe { route.remote_free_after_thread_exit(second_blocks[index]) } {
                    ...
                    while index + 1 < second_capacity {
                        route = match unsafe { route.remote_free_after_thread_exit(second_blocks[index]) } {
                            Ok(MainHeapThreadProcessPageExitFullLargePagesFreeResult::StillLive(route)) => route,
                            Ok(_) => panic!("a nonfinal mapped second large-page free remains live"),
                            Err(_) => panic!("a mapped second large-page free remains in the aggregate route"),
                        };
                        index += 1;
                    }
                    match unsafe { route.remote_free_after_thread_exit(second_blocks[index]) } {
                ...
                worker
                    .join()
                    .expect("the full-large aggregate route remains local to its later owner fixture");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-large aggregate route fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the sole-full-large refusal fixture");
                    ...
                    assert!(
                        crate::types::page_queue::page_is_in_full(unsafe { page.as_ref() }),
                        "the one large page enters the source full queue"
                    );
                    ...
                    let drain = match unsafe { drain.abandon_full_large_pages_to_process_route() } {
                        ...
                        Err(MainHeapThreadProcessPageExitFullLargePagesRouteBeginFailure::PageMap {
                            ...
                            core::mem::forget(parts);
                            panic!("a sole full large page rejects before PageMap-route transfer: {error:?}");
                        }
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("a sole full large page cannot enter the aggregate route");
                        }
                    };
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(first.as_ptr()) },
                    ...
                    assert_eq!(
                        drain.test_queue_count(crate::config::BIN_FULL),
                        Some(1),
                        "the rejected aggregate preflight leaves the sole full page attached"
                    );
                    assert!(
                        crate::types::page_queue::page_is_in_full(unsafe { page.as_ref() }),
                        "the rejected aggregate preflight leaves the full flag unchanged"
                    );
                ...
                worker
                    .join()
                    .expect("the full-large aggregate refusal remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-large aggregate refusal fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the mixed-full refusal fixture");
                    ...
                    let drain = match unsafe { drain.abandon_full_large_pages_to_process_route() } {
                        ...
                        Err(MainHeapThreadProcessPageExitFullLargePagesRouteBeginFailure::PageMap {
                            ...
                            core::mem::forget(parts);
                            panic!("mixed full classes reject before PageMap-route transfer: {error:?}");
                        }
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("mixed full classes cannot enter the large aggregate route");
                        }
                    };
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(medium.as_ptr()) },
                        medium_page.as_ptr(),
                        "the refusal leaves the medium PageMap member published"
                    );
                    assert_eq!(
                    ...
                    assert!(
                        ...
                        "the rejected preflight leaves both full flags unchanged"
                    );
                ...
                worker
                    .join()
                    .expect("the mixed full-class refusal remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("mixed full-class aggregate refusal fixture remains current-thread local");
    }
    ...
    fn later_thread_exit_full_large_pages_route_retains_a_collection_failure() {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    for _ in 1..first_capacity {
                        let block = allocator
                            .allocate(request, false)
                            .expect("the fixture fills exactly its first large page");
                        assert_eq!(unsafe { allocator.test_page_for_block(block) }, first_page.as_ptr());
                    }

                    let second = allocator
                        .allocate(request, false)
                        .expect("the fixture creates its second large page");
                    let second_page = NonNull::new(unsafe { allocator.test_page_for_block(second) })
                        .expect("the second large page remains PageMap-published");
                    let second_capacity = unsafe { second_page.as_ref().reserved() as usize };
                    for _ in 1..second_capacity {
                        let block = allocator
                            .allocate(request, false)
                            .expect("the fixture fills exactly its second large page");
                        assert_eq!(unsafe { allocator.test_page_for_block(block) }, second_page.as_ptr());
                    }
                    assert!(
                        crate::types::page_queue::page_is_in_full(unsafe { first_page.as_ref() })
                    ...
                    assert_eq!(
                        drain.test_queue_count(crate::config::BIN_FULL),
                        Some(2),
                        "the injected collection failure leaves both full source members linked"
                    );
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(first.as_ptr()) },
                        first_page.as_ptr(),
        ...
        .join()
        .expect("full-large aggregate collection-failure fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    assert!(
                        crate::types::page_queue::page_is_in_full(unsafe { page.as_ref() }),
                        "the one medium page enters the source full queue"
                    );
                    ...
                    let drain = match unsafe { drain.abandon_full_medium_pages_to_process_route() } {
                        ...
                        Err(MainHeapThreadProcessPageExitFullMediumPagesRouteBeginFailure::PageMap {
                            ...
                            core::mem::forget(parts);
                            panic!("a sole full page rejects before PageMap-route transfer: {error:?}");
                        }
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("a sole full medium page cannot enter the aggregate route");
                        }
                    };
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(first.as_ptr()) },
                ...
                worker
                    .join()
                    .expect("the full-medium aggregate refusal remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-medium aggregate refusal fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    assert!(
                        capacity >= 16,
                        "the chosen medium geometry exposes a distinct full queue state"
                    );
                    let mut blocks = std::vec::Vec::with_capacity(capacity);
                    blocks.push(first);
                    while blocks.len() < capacity {
                    ...
                    assert_eq!(
                        crate::size_class::page_kind_for_block_size(page_ref.block_size()),
                        Some(crate::types::PageKind::Medium),
                        "the source transition starts from one medium page"
                    );
                    ...
                    let route = match route.adopt_into_later_main(&mut target, pair) {
                        ...
                        Err(
                            ...
                            core::mem::forget(adoption);
                            panic!(
                                "the force-collected full origin cannot claim or mutate a target: {error:?}"
                            );
                        }
                        Err(
                            MainHeapThreadProcessPageExitMappedRegularAdoptFailure::Reabandoned {
                            ...
                            core::mem::forget(adoption);
                            panic!(
                                "the force-collected full origin cannot enter the target commit/reabandon branch: {error:?}"
                            );
                        }
                        Ok(allocator) => {
                            core::mem::forget(allocator);
                            panic!(
                                "the force-collected full origin cannot become a normal allocation owner"
                            );
                        }
                    };
                    assert_eq!(
                        target.finish_after_user_destructors(),
                        Ok(()),
                        "the pre-transfer full-origin refusal leaves the fresh target attachment empty"
                    );
                    assert_eq!(
                    ...
                    for offset in 0..slice_count {
                        ...
                        assert!(
                            unsafe { page_map.page_map().unwrap().checked_lookup(address) }.is_null(),
                            "the terminal release unregisters every medium PageMap slice"
                        );
                    }
                    assert_eq!(
                        unsafe { arena.pages() }.unwrap().is_clear_range(slice, 1),
                        Some(true),
                        "the final mapped release clears the ordinary main-arena bit"
                    );
                    assert_eq!(
                        page_map.begin_page_lifecycle().unwrap().finish(),
                        Ok(()),
                        "the completed route reopens an empty process map"
                    );
                });
                worker
                    .join()
                    .expect("the force-collected medium route remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-medium force-collection process route fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the preflight fixture");
                    let block = allocator
                        .allocate(SMALL_MAX_OBJ_SIZE + 1, false)
                        .expect("the fixture creates one nonfull medium page");
                    let page = NonNull::new(unsafe { allocator.test_page_for_block(block) })
                        .expect("the regular medium page stays PageMap-published");
                    let bin = crate::size_class::bin(unsafe { page.as_ref().block_size() })
                    ...
                    let drain = match unsafe {
                        ...
                        Err(MainHeapThreadProcessPageExitMappedRegularRouteBeginFailure::PageMap {
                            ...
                            core::mem::forget(parts);
                            panic!("the full-medium shape rejects before PageMap-route transfer: {error:?}");
                        }
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("a regular medium page cannot cross into the full-to-nonfull route");
                        }
                    };
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(block.as_ptr()) },
                        page.as_ptr(),
                        "the rejected full-medium preflight leaves PageMap publication untouched"
                    );
                    assert_eq!(unsafe { page.as_ref().used() }, 1);
                    assert_eq!(
                        drain.test_queue_count(bin),
                        Some(1),
                        "the rejected preflight leaves the ordinary medium queue member attached"
                    );
                    assert_eq!(drain.test_queue_count(crate::config::BIN_FULL), Some(0));
                    assert!(
                        !crate::types::page_queue::page_is_in_full(unsafe { page.as_ref() }),
                        "the rejected preflight leaves the regular queue's full flag unchanged"
                    );
                ...
                worker
                    .join()
                    .expect("the full-medium preflight refusal remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-medium preflight refusal fixture remains current-thread local");
    }
    ...
    fn later_thread_exit_full_medium_force_collect_route_retains_a_collection_failure() {
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                ...
                worker
                    .join()
                    .expect("the full-medium collection-failure fixture remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-medium collection-failure fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the full non-direct remote fixture");
                    let request = SMALL_SIZE_MAX + WORD_SIZE;
                    let first = allocator
                        .allocate(request, false)
                        .expect("the fixture creates one non-direct small regular page");
                    let page = NonNull::new(unsafe { allocator.test_page_for_block(first) })
                        .expect("the full non-direct small page stays PageMap-published");
                    let capacity = unsafe { page.as_ref().reserved() as usize };
                    assert!(capacity > 1, "the source transition requires more than one block");
                    let mut blocks = std::vec::Vec::with_capacity(capacity);
                    blocks.push(first);
                    while blocks.len() < capacity {
                        let block = allocator
                        ...
                        assert_eq!(
                            unsafe { allocator.test_page_for_block(block) },
                            page.as_ptr(),
                            "the fixture does not allocate a second non-direct small page"
                        );
                        blocks.push(block);
                    }
                    let page_ref = unsafe { page.as_ref() };
                    ...
                    match unsafe {
                        ...
                        Ok(MainHeapThreadProcessPageExitMappedRegularFreeResult::StillLive(route)) => {
                            core::mem::forget(route);
                            panic!("the final mapped client free releases the non-direct small page")
                        }
                        Err(_) => panic!("the final mapped client free releases the non-direct small page"),
                    }
                    assert!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(first.as_ptr()) }.is_null(),
                        "the final route release unregisters the small PageMap span"
                ...
                worker
                    .join()
                    .expect("the force-collected non-direct small route remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full non-direct-small force-collection process route fixture remains current-thread local");
    }
    ...
    fn later_thread_exit_full_non_direct_small_force_collect_route_rejects_direct_small_before_mutation(
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                    ...
                    let drain = match unsafe {
                        ...
                        Err(
                            ...
                            assert_eq!(
                                ...
                                "a direct-small page rejects before source collection or detachment"
                            );
                            drain
                        }
                        Err(
                            ...
                            core::mem::forget(drain);
                            panic!("the direct-small class rejects before a source transition: {error:?}");
                        }
                        Err(MainHeapThreadProcessPageExitMappedRegularRouteBeginFailure::Teardown {
                            terminal,
                            ..
                            ...
                            core::mem::forget(terminal);
                            panic!("the direct-small class rejects before Theap/TLD teardown");
                        }
                        Err(MainHeapThreadProcessPageExitMappedRegularRouteBeginFailure::PageMap {
                            parts,
                            error,
                            ...
                            core::mem::forget(parts);
                            panic!("the direct-small class rejects before PageMap-route transfer: {error:?}");
                        }
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("a direct-small page cannot enter the non-direct mapped route");
                        }
                    };
                    assert_eq!(
                        unsafe { page.as_ref().used() },
                        capacity,
                        "the rejected direct-small class leaves the source object count untouched"
                    );
                    assert_eq!(
                        drain.test_queue_count(bin),
                        Some(1),
                        "the rejected direct-small class leaves its source queue member attached"
                    );
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(first.as_ptr()) },
                        page.as_ptr(),
                        "the rejected direct-small class leaves PageMap publication untouched"
                    );
                ...
                worker
                    .join()
                    .expect("the non-direct force-route class refusal remains local to its later owner");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full non-direct-small force-collection class refusal fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the non-direct collection-failure fixture");
                    let request = SMALL_SIZE_MAX + WORD_SIZE;
                    let first = allocator
                        .allocate(request, false)
                        .expect("the fixture creates one non-direct small regular page");
                    ...
                    assert!(
                        unsafe { page.as_ref().block_size() } > SMALL_SIZE_MAX
                            && !crate::types::page_queue::page_is_in_full(unsafe { page.as_ref() }),
                        "the failure fixture starts in the full non-direct-small queue shape"
                    );
                    ...
                    assert_eq!(
                        drain.test_queue_count(bin),
                        Some(1),
                        "the injected failure leaves the regular source queue linked"
                    );
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(blocks[1].as_ptr()) },
                        page.as_ptr(),
                ...
                worker
                    .join()
                    .expect("the non-direct collection-failure fixture remains local to its later owner");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full non-direct-small force-collection failure fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the large force-collection fixture");
                    let request = MEDIUM_MAX_OBJ_SIZE + 1;
                    let first = allocator
                        .allocate(request, false)
                        .expect("the fixture creates one large regular page");
                    let page = NonNull::new(unsafe { allocator.test_page_for_block(first) })
                        .expect("the full large page stays PageMap-published");
                    let capacity = unsafe { page.as_ref().reserved() as usize };
                    assert!(capacity > 1, "the large source page has a joined-free predecessor");
                    let mut blocks = std::vec::Vec::with_capacity(capacity);
                    blocks.push(first);
                    while blocks.len() < capacity {
                        let block = allocator
                    ...
                    assert_eq!(
                        crate::size_class::page_kind_for_block_size(page_ref.block_size()),
                        Some(crate::types::PageKind::Large),
                        "the source transition starts from one large page"
                    );
                    assert_eq!(page_ref.used(), capacity);
                    assert!(
                        crate::types::page_queue::page_is_in_full(page_ref),
                    ...
                    match unsafe {
                        ...
                        Ok(MainHeapThreadProcessPageExitMappedRegularFreeResult::StillLive(route)) => {
                            core::mem::forget(route);
                            panic!("the final mapped client free releases the large page")
                        }
                        Err(_) => panic!("the final mapped client free releases the large page"),
                    }
                    for offset in 0..slice_count {
                        let address = unsafe {
                            slice_start.add(offset * crate::config::ARENA_SLICE_SIZE)
                ...
                worker
                    .join()
                    .expect("the force-collected large route remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-large force-collection process route fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the large preflight fixture");
                    let block = allocator
                    ...
                    let drain = match unsafe {
                        ...
                        Err(
                            ...
                            assert_eq!(
                                ...
                                "a regular large page cannot enter the full-to-nonfull source branch"
                            );
                            drain
                        }
                        Err(
                            ...
                            core::mem::forget(drain);
                            panic!("the full-large shape rejects before source collection: {error:?}");
                        }
                        Err(MainHeapThreadProcessPageExitMappedRegularRouteBeginFailure::Teardown {
                            terminal,
                            ..
                            ...
                            core::mem::forget(terminal);
                            panic!("the full-large shape rejects before Theap/TLD teardown");
                        }
                        Err(MainHeapThreadProcessPageExitMappedRegularRouteBeginFailure::PageMap {
                            parts,
                            error,
                            ...
                            core::mem::forget(parts);
                            panic!("the full-large shape rejects before PageMap-route transfer: {error:?}");
                        }
                        Ok(route) => {
                            core::mem::forget(route);
                            panic!("a regular large page cannot cross into the full-to-nonfull route");
                        }
                    };
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(block.as_ptr()) },
                        page.as_ptr(),
                        "the rejected full-large preflight leaves PageMap publication untouched"
                    );
                    assert_eq!(unsafe { page.as_ref().used() }, 1);
                    assert_eq!(
                        drain.test_queue_count(bin),
                        Some(1),
                        "the rejected preflight leaves the ordinary large queue member attached"
                    );
                    assert_eq!(drain.test_queue_count(crate::config::BIN_FULL), Some(0));
                    assert!(
                        !crate::types::page_queue::page_is_in_full(unsafe { page.as_ref() }),
                ...
                worker
                    .join()
                    .expect("the full-large preflight refusal remains current-thread local");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-large preflight refusal fixture remains current-thread local");
    }
        ...
        thread::spawn(|| {
            ...
            thread::scope(|scope| {
                let worker = scope.spawn(move || {
                    ...
                    let mut allocator = MainHeapThreadProcessPageAllocator::begin(&mut owner, pair)
                        .expect("the matched process pair admits the large collection-failure fixture");
                    let request = MEDIUM_MAX_OBJ_SIZE + 1;
                    let first = allocator
                        .allocate(request, false)
                        .expect("the fixture creates one large regular page");
                    ...
                    assert!(
                        crate::types::page_queue::page_is_in_full(unsafe { page.as_ref() }),
                        "the failure fixture starts in the full large queue shape"
                    );
                    ...
                    assert!(
                        crate::types::page_queue::page_is_in_full(unsafe { page.as_ref() }),
                        "the injected failure leaves the full-queue flag unchanged"
                    );
                    assert_eq!(
                        unsafe { page_map.page_map().unwrap().checked_lookup(blocks[1].as_ptr()) },
                        page.as_ptr(),
                ...
                worker
                    .join()
                    .expect("the full-large collection-failure fixture remains local to its later owner");
            });
            core::mem::forget(main);
        })
        .join()
        .expect("full-large force-collection failure fixture remains current-thread local");
    }
