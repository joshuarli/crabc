...
The active x86 milestone queue is in §26. The linked AArch64 handoff and
chronologies are paused historical records. This scheduling section
...
The following architecture-convergence packages are candidates only after
their native x86 milestone prerequisites are met. Schedule from §26 and the
M0–M11 dependency order, not from this older audited list. Re-evaluate paths
against current head and skip work already completed:
...
worktrees as dependencies land. Do not fill slots with prerequisite-blocked
implementation or mistake a useful review for a completed implementation.
...
# 21. Milestone completion contracts
...
The audited checkpoints and chronologies linked below are historical
...
[`plan.md`](plan.md). Resume from the first unmet x86 gate, not the archived
AArch64 continuation prompt.

## Paused AArch64 record

The exact-revision handoff, milestone table, and chronological evidence are
preserved in [the AArch64 archive](docs/history/native-mimalloc-aarch64.md).
M0/M1 were complete as bounded AArch64 contracts; M2 remained partial, with
PageMap its sole complete component. These are not native x86 milestone passes.
Resume that architecture only after explicit user direction.
