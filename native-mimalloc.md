...
| M2 — memory substrate | partial (current executable gate) | `compat/allocator/m2-memory-substrate-v3.5.0.json` fixes eight categories. The 37-, 38-, 39-, 40-, and 41-check clean detached reports at earlier commits are historical. At `b09b1fd98cec6b811f52cf7e972e9dbda2127872`, clean detached native `./scripts/dev.sh allocator --quick` and `./scripts/dev.sh allocator-m1` exited 0; `./scripts/dev.sh allocator-m2` executed all 42 selected checks and exited 3 as defined because exactly seven components remain unmet: `vm-primitives`, `metadata`, `bitmaps`, `arenas`, `initialization`, `fault-injection`, and `allocator-recursion`. The current counts are eight VM primitives, eleven metadata, four pinned-C/Rust bitmap differentials, ten PageMap, three arenas, three initialization, one native protect/unprotect fault-injection regression, and two same-thread metadata-recursion regressions. PageMap alone is complete. The selected metadata records include a typed `ArenaSliceClaim` subprocess-identity gate, a requested-parent pre-init reservation, a synthetic no-page Arena-prefix lifecycle, the bounded lock-free metadata-page identity query, the selected direct-allocation `theap_meta_lock` phase, and a selected typed Malloc free pre-claim recursion boundary; none closes the metadata category. The protection test uses test-only pre-syscall `NOMEM`, retains the exact base/length, proves a failed protect remains writable, and retries after disabling injection; it is not live-kernel or C failure parity. The ten PageMap checks cover source-private C/Rust success and failed-first-init differentials, bootstrap/lazy/release ownership failures, private-lock publication, and the process-owner terminal boundary. The metadata checks do not claim Rust's bounded private direct-OS backing matches C's normal `_mi_meta_zalloc` backing route, generic `_mi_meta_free` dispatch, or full `mi_tld_init`/`mi_tld_free` list and lock behavior; `MetaRelease::RegularOs` remains only a standalone retry witness, not a C metadata caller. |
...
The working selected set contains 42 native checks: eight VM-primitives
...
protect/unprotect fault-injection check, and two same-thread metadata-recursion
checks. The 37-, 38-, 39-, 40-, and 41-check reports are historical evidence
for prior contracts. At `b09b1fd98cec6b811f52cf7e972e9dbda2127872`, a clean
detached native M2 run executed this 42-check shape, left its source clean
before and after and unchanged during execution, and exited 3 as the
partial-gate contract defines. `page-map` is complete within this M2 contract;
the other seven required components remain partial under their explicit
...
the current closure gate. Its current clean native evidence has 42 selected
...
fault-injection check, and two same-thread metadata-recursion checks); it exits
