...
def check_x86_libc_static_c_abi_boundary(errors: list[str]) -> None:
    ...
    res_init_text = res_init_source.read_text(errors="replace")
    for required in (
        "pinned musl 1.2.6 release commit",
        "9fa28ece75d8a2191de7c5bb53bed224c5947417",
        ...
        if required not in res_init_text:
            errors.append(
                "libc/src/c_abi/x86_64/res_init.rs: selected static legacy "
                f"resolver-initializer boundary is missing {required!r}"
            )
    res_init_exports = set(
        re.findall(
            r'(?m)^pub\s+(?:unsafe\s+)?extern\s+"C"\s+fn\s+(\w+)\s*\(',
            res_init_text,
        )
    )
    if res_init_exports != {"res_init"}:
        errors.append(
            "libc/src/c_abi/x86_64/res_init.rs: selected static legacy "
            "resolver-initializer artifact must export only res_init"
        )
    for forbidden in (
        "raw_syscall::",
        "errno::",
    ...
    posix_spawnattr_getflags_source = (
        ROOT
        / "libc"
        / "src"
        / "c_abi"
    ...
    posix_spawnattr_getflags_text = posix_spawnattr_getflags_source.read_text(
        errors="replace"
    )
    for required in (
        "pinned musl 1.2.6 release commit",
        ...
        if required not in posix_spawnattr_getflags_text:
            errors.append(
                "libc/src/c_abi/x86_64/posix_spawnattr_getflags.rs: selected "
                f"static spawn-attribute flag-readback boundary is missing {required!r}"
            )
    posix_spawnattr_getflags_exports = set(
        re.findall(
            r'(?m)^pub\s+(?:unsafe\s+)?extern\s+"C"\s+fn\s+(\w+)\s*\(',
            posix_spawnattr_getflags_text,
        )
    )
    if posix_spawnattr_getflags_exports != {"posix_spawnattr_getflags"}:
        errors.append(
            "libc/src/c_abi/x86_64/posix_spawnattr_getflags.rs: selected "
            "static artifact must export only posix_spawnattr_getflags"
        )
    for forbidden in (
        "raw_syscall::",
        "errno::",
        ...
        if forbidden in posix_spawnattr_getflags_text:
            errors.append(
                "libc/src/c_abi/x86_64/posix_spawnattr_getflags.rs: selected "
    ...
    posix_spawnattr_setschedpolicy_source = (
        ROOT
        / "libc"
        / "src"
        / "c_abi"
    ...
    posix_spawnattr_setschedpolicy_text = (
        posix_spawnattr_setschedpolicy_source.read_text(errors="replace")
    )
    for required in (
        "pinned musl 1.2.6 release commit",
        "9fa28ece75d8a2191de7c5bb53bed224c5947417",
        ...
        if required not in posix_spawnattr_setschedpolicy_text:
            errors.append(
                "libc/src/c_abi/x86_64/posix_spawnattr_setschedpolicy.rs: selected "
                f"static spawn-attribute scheduler-policy boundary is missing {required!r}"
            )
    posix_spawnattr_setschedpolicy_exports = set(
        re.findall(
            r'(?m)^pub\s+(?:unsafe\s+)?extern\s+"C"\s+fn\s+(\w+)\s*\(',
            posix_spawnattr_setschedpolicy_text,
        )
    )
    if posix_spawnattr_setschedpolicy_exports != {
        "posix_spawnattr_setschedpolicy"
    }:
        errors.append(
            "libc/src/c_abi/x86_64/posix_spawnattr_setschedpolicy.rs: selected "
            "static artifact must export only posix_spawnattr_setschedpolicy"
        )
    for forbidden in (
        "raw_syscall::",
        "errno::",
        ...
        if forbidden in posix_spawnattr_setschedpolicy_text:
            errors.append(
                "libc/src/c_abi/x86_64/posix_spawnattr_setschedpolicy.rs: selected "
                f"static spawn-attribute scheduler-policy boundary must not select {forbidden!r}"
            )
    ...
    for source_name, source_text in (
        ...
        ("posix_spawnattr_setschedpolicy.rs", posix_spawnattr_setschedpolicy_text),
