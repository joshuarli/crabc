...
# descriptor creation, nanosleep, sleep, and clock_nanosleep, descriptor entry, selected
...
def check_x86_libc_static_c_abi_boundary(errors: list[str]) -> None:
    ...
    sched_get_priority_max_source = (
        ROOT / "libc" / "src" / "c_abi" / "x86_64" / "sched_get_priority_max.rs"
    )
    sched_get_priority_max_text = sched_get_priority_max_source.read_text(errors="replace")
    for required in (
        "Selected static Linux/x86-64 C `sched_get_priority_max` boundary",
        "musl 1.2.6 release commit",
        "src/sched/sched_get_priority_max.c::sched_get_priority_max",
        "sched_get_priority_max=146",
        "raw_syscall::SYS_SCHED_GET_PRIORITY_MAX",
        "raw_syscall::syscall1",
        "c_status(result)",
        ...
        if required not in sched_get_priority_max_text:
            errors.append(
                "libc/src/c_abi/x86_64/sched_get_priority_max.rs: selected static "
                f"scheduler priority-maximum boundary is missing {required!r}"
            )
    sched_get_priority_max_exports = set(
        re.findall(
            r'(?m)^pub\s+(?:unsafe\s+)?extern\s+"C"\s+fn\s+(\w+)\s*\(',
            sched_get_priority_max_text,
        )
    )
    if sched_get_priority_max_exports != {"sched_get_priority_max"}:
        errors.append(
            "libc/src/c_abi/x86_64/sched_get_priority_max.rs: selected static "
            "artifact must export only sched_get_priority_max"
        )
    for forbidden in (
        ...
        if forbidden in sched_get_priority_max_text:
            errors.append(
                "libc/src/c_abi/x86_64/sched_get_priority_max.rs: selected static "
