...
def check_x86_libc_static_c_abi_boundary(errors: list[str]) -> None:
    ...
    sched_getparam_source = (
        ROOT / "libc" / "src" / "c_abi" / "x86_64" / "sched_getparam.rs"
    )
    sched_getparam_text = sched_getparam_source.read_text(errors="replace")
    for required in (
        ...
        if required not in sched_getparam_text:
            errors.append(
                "libc/src/c_abi/x86_64/sched_getparam.rs: selected static "
                f"musl-ENOSYS scheduler boundary is missing {required!r}"
            )
    sched_getparam_exports = set(
        re.findall(
            r'(?m)^pub\s+(?:unsafe\s+)?extern\s+"C"\s+fn\s+(\w+)\s*\(',
            sched_getparam_text,
        )
    )
    if sched_getparam_exports != {"sched_getparam"}:
        errors.append(
            "libc/src/c_abi/x86_64/sched_getparam.rs: selected static "
            "artifact must export only sched_getparam"
        )
    for forbidden in (
        "raw_syscall::",
        "SYS_SCHED_GETPARAM",
        ...
        if forbidden in sched_getparam_text:
            errors.append(
                "libc/src/c_abi/x86_64/sched_getparam.rs: selected static "
                f"musl-ENOSYS scheduler boundary must not select {forbidden!r}"
            )

    sched_setparam_source = (
        ROOT / "libc" / "src" / "c_abi" / "x86_64" / "sched_setparam.rs"
    )
    sched_setparam_text = sched_setparam_source.read_text(errors="replace")
    for required in (
        ...
        if required not in sched_setparam_text:
            errors.append(
                "libc/src/c_abi/x86_64/sched_setparam.rs: selected static "
                f"musl-ENOSYS scheduler boundary is missing {required!r}"
            )
    sched_setparam_exports = set(
        re.findall(
            r'(?m)^pub\s+(?:unsafe\s+)?extern\s+"C"\s+fn\s+(\w+)\s*\(',
            sched_setparam_text,
        )
    )
    if sched_setparam_exports != {"sched_setparam"}:
        errors.append(
            "libc/src/c_abi/x86_64/sched_setparam.rs: selected static "
            "artifact must export only sched_setparam"
        )
    for forbidden in (
        "raw_syscall::",
        "SYS_SCHED_SETPARAM",
        ...
        if forbidden in sched_setparam_text:
            errors.append(
                "libc/src/c_abi/x86_64/sched_setparam.rs: selected static "
                f"musl-ENOSYS scheduler boundary must not select {forbidden!r}"
            )
    ...
    if exports != expected_exports:
        errors.append(
            ...
            "fixed-C-locale ctype, integer-arithmetic, integer-parsing, intmax-arithmetic, one-symbol filesystem-credential setfsuid, credential-observation, and "
