...
X86_RUNTIME_FOUNDATION_LIBC_SOURCES = {
    ...
    Path("libc/src/c_abi/x86_64/unlinkat.rs"),
...
def check_x86_libc_static_c_abi_boundary(errors: list[str]) -> None:
    ...
    for required in (
        ...
        if required not in sync_text:
            errors.append(
                "libc/src/c_abi/x86_64/sync.rs: selected direct void writeback "
                f"boundary is missing {required!r}"
            )
    sync_exports = set(
        re.findall(
    ...
    if sync_exports != {"sync"}:
        errors.append(
            "libc/src/c_abi/x86_64/sync.rs: selected static artifact must export only sync"
        )
    for forbidden in (
        "fn syncfs(",
        "fn fsync(",
    ...
    if "pub(crate) const SYS_SYNC: i64 = 162;" not in raw_syscall_text:
        errors.append(
            "libc/src/c_abi/x86_64/syscall.rs: selected static sync boundary "
    ...
    for required in (
        ...
        "AT_REMOVEDIR",
        "AT_FDCWD",
        "pathname, CWD, namespace",
        "public x86 support",
    ):
        if required not in unlinkat_text:
            errors.append(
                "libc/src/c_abi/x86_64/unlinkat.rs: selected direct pathname "
                f"removal boundary is missing {required!r}"
            )
    unlinkat_exports = set(
        re.findall(
            r'(?m)^pub\s+(?:unsafe\s+)?extern\s+"C"\s+fn\s+(\w+)\s*\(',
            unlinkat_text,
        )
    )
    if unlinkat_exports != {"unlinkat"}:
        errors.append(
            "libc/src/c_abi/x86_64/unlinkat.rs: selected static artifact must "
            "export only unlinkat"
        )
    for forbidden in (
        "const AT_FDCWD",
        "fn unlink(",
        ...
        if forbidden in unlinkat_text:
            errors.append(
                "libc/src/c_abi/x86_64/unlinkat.rs: selected direct pathname "
                f"removal boundary must not select {forbidden!r}"
            )
    if "pub(crate) const SYS_UNLINKAT: i64 = 263;" not in raw_syscall_text:
        errors.append(
            "libc/src/c_abi/x86_64/syscall.rs: selected static unlinkat boundary "
            "requires SYS_UNLINKAT=263"
        )
