...
def check_x86_libc_static_c_abi_boundary(errors: list[str]) -> None:
    ...
    if "inet_makeaddr inet_lnaof inet_netof inet_network" not in inet_address_runner:
        errors.append(
            "compat/x86_64/run_libc_inet_address.sh: numeric candidate must "
    ...
    inet_network_probe_source = (
        ROOT / "compat" / "x86_64" / "libc_inet_network_probe.c"
    )
    inet_network_start_source = (
        ROOT / "compat" / "x86_64" / "libc_inet_network_start.S"
    )
    inet_network_runner_source = (
        ROOT / "compat" / "x86_64" / "run_libc_inet_network.sh"
    )
    for path in (
        inet_network_probe_source,
        inet_network_start_source,
        ...
        if not path.is_file():
            errors.append(
                f"x86 static inet_network artifact is missing {path.relative_to(ROOT)}"
            )
            return
    ...
    inet_network_text = inet_network_source.read_text(errors="replace")
    inet_network_probe = inet_network_probe_source.read_text(errors="replace")
    inet_network_start = inet_network_start_source.read_text(errors="replace")
    inet_network_runner = inet_network_runner_source.read_text(errors="replace")
    for required in (
        "9fa28ece75d8a2191de7c5bb53bed224c5947417",
        "src/network/inet_legacy.c",
        ...
        if required not in inet_network_text:
            errors.append(
                "libc/src/c_abi/x86_64/inet_network.rs: selected static "
                f"inet_network boundary is missing {required!r}"
            )
    for forbidden in (
        "raw_syscall",
        "getaddrinfo",
        ...
        if forbidden in inet_network_text:
            errors.append(
                "libc/src/c_abi/x86_64/inet_network.rs: selected static "
                f"inet_network boundary must not select {forbidden!r}"
            )
    inet_network_exports = set(
        re.findall(
            r'(?m)^pub\s+(?:unsafe\s+)?extern\s+"C"\s+fn\s+(\w+)\s*\(',
            inet_network_text,
        )
    )
    if inet_network_exports != {"inet_network"}:
        errors.append(
            "libc/src/c_abi/x86_64/inet_network.rs: selected static artifact "
            "must export only inet_network"
        )
    for required in (
        ...
        if required not in inet_network_probe:
            errors.append(
                "compat/x86_64/libc_inet_network_probe.c: static inet_network "
                f"regression is missing {required!r}"
            )
    for required in (
        "__crabc_x86_static_tls_bootstrap",
        "crabc_x86_64_inet_network_probe",
        "mov $231, %eax",
    ):
        if required not in inet_network_start:
            errors.append(
                "compat/x86_64/libc_inet_network_start.S: static inet_network "
                f"TLS shim is missing {required!r}"
            )
    for required in (
        "inet_legacy.lo",
        ...
        if required not in inet_network_runner:
            errors.append(
                "compat/x86_64/run_libc_inet_network.sh: true static "
                f"inet_network evidence is missing {required!r}"
            )
    if "archive-free candidate" in inet_network_runner:
        errors.append(
            "compat/x86_64/run_libc_inet_network.sh: parser-dependent final "
            "candidate must not claim to be archive-free"
        )
    if "inet_makeaddr inet_lnaof inet_netof inet_network" not in inet_address_runner:
        errors.append(
            "compat/x86_64/run_libc_inet_address.sh: numeric candidate must "
            "continue excluding the separate inet_network leaf"
        )
    ...
    ns_get16_start_source = ROOT / "compat" / "x86_64" / "libc_ns_get16_start.S"
    ns_get16_runner_source = ROOT / "compat" / "x86_64" / "run_libc_ns_get16.sh"
    for path in (ns_get16_probe_source, ns_get16_start_source, ns_get16_runner_source):
        if not path.is_file():
            errors.append(
                f"x86 static ns_get16 artifact is missing {path.relative_to(ROOT)}"
            )
            return

    ns_get16_source = ROOT / "libc" / "src" / "c_abi" / "x86_64" / "ns_get16.rs"
    ns_get16_text = ns_get16_source.read_text(errors="replace")
    ns_get16_probe = ns_get16_probe_source.read_text(errors="replace")
    ns_get16_start = ns_get16_start_source.read_text(errors="replace")
    ns_get16_runner = ns_get16_runner_source.read_text(errors="replace")
    for required in (
        "9fa28ece75d8a2191de7c5bb53bed224c5947417",
        "src/network/ns_parse.c",
        "core::ptr::read",
        ...
        if required not in ns_get16_text:
            errors.append(
                "libc/src/c_abi/x86_64/ns_get16.rs: selected static "
    ...
    for forbidden in (
        ...
        if forbidden in ns_get16_text:
            errors.append(
                "libc/src/c_abi/x86_64/ns_get16.rs: selected static "
                f"wire-read boundary must not select {forbidden!r}"
            )
    ns_get16_exports = set(
        re.findall(
            r'(?m)^pub\s+unsafe\s+extern\s+"C"\s+fn\s+(\w+)\s*\(',
            ns_get16_text,
        )
    )
    if ns_get16_exports != {"ns_get16"}:
        errors.append(
            "libc/src/c_abi/x86_64/ns_get16.rs: selected static artifact "
            "must export only ns_get16"
        )
    for required in (
        "#include <resolv.h>",
        "ns_get16_signature",
        ...
        if required not in ns_get16_probe:
            errors.append(
                "compat/x86_64/libc_ns_get16_probe.c: static wire-read "
                f"regression is missing {required!r}"
            )
    for required in ("crabc_x86_64_ns_get16_probe", "mov $60, %eax"):
        if required not in ns_get16_start:
            errors.append(
                "compat/x86_64/libc_ns_get16_start.S: static wire-read "
                f"entry is missing {required!r}"
            )
    if "ARCH_SET_FS" in ns_get16_start:
        errors.append(
            "compat/x86_64/libc_ns_get16_start.S: wire-read entry must not "
            "bootstrap TLS"
        )
    for required in (
        "ns_parse.lo",
        ...
        if required not in ns_get16_runner:
            errors.append(
                "compat/x86_64/run_libc_ns_get16.sh: archive-free static "
    ...
    ns_put16_probe_source = ROOT / "compat" / "x86_64" / "libc_ns_put16_probe.c"
    ns_put16_start_source = ROOT / "compat" / "x86_64" / "libc_ns_put16_start.S"
    ns_put16_runner_source = ROOT / "compat" / "x86_64" / "run_libc_ns_put16.sh"
    for path in (ns_put16_probe_source, ns_put16_start_source, ns_put16_runner_source):
        if not path.is_file():
            errors.append(
                f"x86 static ns_put16 artifact is missing {path.relative_to(ROOT)}"
            )
            return

    ns_put16_source = ROOT / "libc" / "src" / "c_abi" / "x86_64" / "ns_put16.rs"
    ns_put16_text = ns_put16_source.read_text(errors="replace")
    ns_put16_probe = ns_put16_probe_source.read_text(errors="replace")
    ns_put16_start = ns_put16_start_source.read_text(errors="replace")
    ns_put16_runner = ns_put16_runner_source.read_text(errors="replace")
    for required in (
        "9fa28ece75d8a2191de7c5bb53bed224c5947417",
        "src/network/ns_parse.c",
        "core::ptr::write",
        ...
        if required not in ns_put16_text:
            errors.append(
                "libc/src/c_abi/x86_64/ns_put16.rs: selected static "
                f"wire-write boundary is missing {required!r}"
            )
    for forbidden in (
        "static mut",
        "raw_syscall",
        ...
        if forbidden in ns_put16_text:
            errors.append(
                "libc/src/c_abi/x86_64/ns_put16.rs: selected static "
                f"wire-write boundary must not select {forbidden!r}"
            )
    ns_put16_exports = set(
        re.findall(
            r'(?m)^pub\s+unsafe\s+extern\s+"C"\s+fn\s+(\w+)\s*\(',
            ns_put16_text,
        )
    )
    if ns_put16_exports != {"ns_put16"}:
        errors.append(
            "libc/src/c_abi/x86_64/ns_put16.rs: selected static artifact "
            "must export only ns_put16"
        )
    for required in (
        "#include <resolv.h>",
        "ns_put16_signature",
        ...
        if required not in ns_put16_probe:
            errors.append(
                "compat/x86_64/libc_ns_put16_probe.c: static wire-write "
                f"regression is missing {required!r}"
            )
    for required in ("crabc_x86_64_ns_put16_probe", "mov $60, %eax"):
        if required not in ns_put16_start:
            errors.append(
                "compat/x86_64/libc_ns_put16_start.S: static wire-write "
                f"entry is missing {required!r}"
            )
    if "ARCH_SET_FS" in ns_put16_start:
        errors.append(
            "compat/x86_64/libc_ns_put16_start.S: wire-write entry must not "
            "bootstrap TLS"
        )
    for required in (
        "ns_parse.lo",
        ...
        if required not in ns_put16_runner:
            errors.append(
                "compat/x86_64/run_libc_ns_put16.sh: archive-free static "
                f"wire-write evidence is missing {required!r}"
            )
    if '"$archive" -o "$candidate"' in ns_put16_runner:
        errors.append(
            "compat/x86_64/run_libc_ns_put16.sh: final wire-write candidate "
            "must not link libc.a"
        )

    ns_put32_probe_source = ROOT / "compat" / "x86_64" / "libc_ns_put32_probe.c"
    ns_put32_start_source = ROOT / "compat" / "x86_64" / "libc_ns_put32_start.S"
    ns_put32_runner_source = ROOT / "compat" / "x86_64" / "run_libc_ns_put32.sh"
    for path in (ns_put32_probe_source, ns_put32_start_source, ns_put32_runner_source):
        if not path.is_file():
            errors.append(
                f"x86 static ns_put32 artifact is missing {path.relative_to(ROOT)}"
            )
            return

    ns_put32_source = ROOT / "libc" / "src" / "c_abi" / "x86_64" / "ns_put32.rs"
    ns_put32_text = ns_put32_source.read_text(errors="replace")
    ns_put32_probe = ns_put32_probe_source.read_text(errors="replace")
    ns_put32_start = ns_put32_start_source.read_text(errors="replace")
    ns_put32_runner = ns_put32_runner_source.read_text(errors="replace")
    for required in (
        "9fa28ece75d8a2191de7c5bb53bed224c5947417",
        "src/network/ns_parse.c",
        "core::ptr::write",
        ...
        if required not in ns_put32_text:
            errors.append(
                "libc/src/c_abi/x86_64/ns_put32.rs: selected static "
                f"wire-write boundary is missing {required!r}"
            )
    for forbidden in (
        "static mut",
        ...
        if forbidden in ns_put32_text:
            errors.append(
                "libc/src/c_abi/x86_64/ns_put32.rs: selected static "
                f"wire-write boundary must not select {forbidden!r}"
            )
    ns_put32_exports = set(
        re.findall(
            r'(?m)^pub\s+unsafe\s+extern\s+"C"\s+fn\s+(\w+)\s*\(',
            ns_put32_text,
        )
    )
    if ns_put32_exports != {"ns_put32"}:
        errors.append(
            "libc/src/c_abi/x86_64/ns_put32.rs: selected static artifact "
            "must export only ns_put32"
        )
    for required in (
        "#include <resolv.h>",
        "ns_put32_signature",
        ...
        if required not in ns_put32_probe:
            errors.append(
                "compat/x86_64/libc_ns_put32_probe.c: static wire-write "
                f"regression is missing {required!r}"
            )
    for required in ("crabc_x86_64_ns_put32_probe", "mov $60, %eax"):
        if required not in ns_put32_start:
            errors.append(
                "compat/x86_64/libc_ns_put32_start.S: static wire-write "
                f"entry is missing {required!r}"
            )
    if "ARCH_SET_FS" in ns_put32_start:
        errors.append(
            "compat/x86_64/libc_ns_put32_start.S: wire-write entry must not "
            "bootstrap TLS"
        )
    for required in (
        "ns_parse.lo",
        ...
        if required not in ns_put32_runner:
            errors.append(
                "compat/x86_64/run_libc_ns_put32.sh: archive-free static "
                f"wire-write evidence is missing {required!r}"
            )
    if '"$archive" -o "$candidate"' in ns_put32_runner:
        errors.append(
            "compat/x86_64/run_libc_ns_put32.sh: final wire-write candidate "
            "must not link libc.a"
        )
