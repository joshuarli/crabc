...
`libc-pthread-mutexattr-robust-query` is a separate static project-header
...
`pthread_mutexattr_getrobust` raw bit-2 projection: it returns the installed
`PTHREAD_MUTEX_STALLED`/`PTHREAD_MUTEX_ROBUST` `0`/`1` vocabulary without
changing caller-owned storage. The fixture deliberately constructs raw record
words and does not call `pthread_mutexattr_setrobust`, an attribute lifecycle
...
support.
`libc-pthread-mutexattr-pshared-query` is a separate static project-header
fixture that first runs through pinned musl, then links only the selected
archive. It selects only the four-byte public `pthread_mutexattr_t` record's
`pthread_mutexattr_getpshared` raw bit-7 projection: it returns the installed
`PTHREAD_PROCESS_PRIVATE`/`PTHREAD_PROCESS_SHARED` `0`/`1` vocabulary without
changing caller-owned storage. The fixture deliberately constructs raw record
words and does not call `pthread_mutexattr_setpshared`, an attribute lifecycle
function, or any mutex entry. It does not select setter/record-construction/
...
mutex operation, threads, TLS, synchronization, cancellation, CRT, loader,
sysroot, pthread-family completion, or public x86 support.
`libc-pthread-mutexattr-type-setter` is a separate static project-header
fixture that first runs through pinned musl, then links only the selected
archive. It selects only `pthread_mutexattr_settype`'s input-first raw-record
branch: `0`, `1`, and `2` replace the public four-byte
`pthread_mutexattr_t` word's low two bits while retaining all higher
caller-owned bits; every other `int` returns `EINVAL` without touching even a
null record pointer. It does not select getter, record construction or
lifecycle, mutex initialization/operation, recursive or error-checking mutex
behavior, threads, TLS, synchronization, cancellation, CRT, loader, sysroot,
pthread-family completion, or public x86 support.
