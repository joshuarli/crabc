...
`libc-sched-setparam` is a separate one-entry POSIX scheduler-parameter
compatibility-failure boundary. Pinned musl deliberately returns `-1` with
`ENOSYS` for every `sched_setparam(pid_t, const struct sched_param *)` input,
including a null record pointer, instead of exposing Linux's thread-scoped raw
syscall 142 as a process API. The common reference/candidate fixture calls raw
142 only with impossible `INT_MAX` and a valid zero parameter to observe
non-mutating `ESRCH`, then proves musl's ENOSYS result for current, invalid,
missing, and null inputs while retaining a byte-for-byte untouched 48-byte
record. It does not select scheduler policy/mutation or parameter records,
priority bounds, affinity, lifecycle, pthread scheduling attributes, dynamic
libc, or application startup.
`libc-sched-getaffinity` is a separate one-entry GNU scheduler-affinity
observation boundary. Pinned musl's `src/sched/affinity.c` sends raw syscall
204, converts its positive initialized-prefix result to zero, and clears only
the remaining caller-owned tail. The common reference/candidate fixture proves
...
data, process/session control, scheduler state, pthread lifecycle, dynamic
libc, or application startup.
`libc-personality` is a separate one-entry Linux process-personality boundary.
Pinned musl's `src/linux/personality.c` forwards syscall 135 and preserves the
current personality returned by Linux's all-ones non-mutating query instead of
inventing a zero-or-error status. The common reference/candidate fixture checks
raw/C query agreement and stale `errno` on ordinary returns. It does not select
personality policy, executable transition, prctl/capability/namespace controls,
...
case "$command" in
    ...
    posix-exit-header-abi|posix-spawnattr-init-header-abi|posix-spawnattr-getpgroup-header-abi) ;;
    ffs-header-abi) ;;
    ...
    libc-posix-exit|libc-posix-spawnattr-init|libc-posix-spawnattr-getpgroup) ;;
