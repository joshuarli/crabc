...
Native Linux/x86-64 staged-foundation evidence commands:
  ...
  math-pow-header-abi verify x86 pow/powf C++ ABI/linkage
  ...
  libc-math-pow run the static x86 pow/powf scalar power slice
...
`libc-math-log` is the separate selected binary32/binary64 natural-logarithm
slice for `log` and `logf`. It compares parenthesized C calls and default-SSE/
`-mfpmath=387` C++ declarations with pinned musl, then runs one freestanding
static candidate. The checked GCC 15.2.0 translation of musl 1.2.6
`log.c`/`logf.c` carries private binary64/binary32 Arm tables and local domain
helpers rather than sharing `math.special` state. Its 224 raw 32-byte records
cover signed zero divide-by-zero, negative-domain invalid, raw subnormal and
normal bounds, close-to-one directed-zero behavior, finite extrema,
infinities, quiet/signaling NaNs, IEEE flags, and requested versus observed
direction in all four MXCSR modes. It excludes `logl`, adjacent
...
special/complex/binary80 math, family completion, promotion, and public x86
support.
`libc-math-sin` is the separate selected binary32/binary64 trigonometric
slice for `sin` and `sinf`. It compares parenthesized C calls and default-SSE/
`-mfpmath=387` C++ declarations with pinned musl, then runs one freestanding
static candidate. The checked GCC 15.2.0 translation of musl 1.2.6
`sin.c`/`sinf.c` localizes its generic kernel, fixed `2/pi` argument-reduction,
and floor/scalbn providers rather than sharing an existing math artifact. Its
256 raw 32-byte records cover signed zero, tiny/subnormal and normal bounds,
quadrant boundaries, moderate and huge reductions, finite extrema, infinities,
quiet/signaling NaNs, IEEE flags, and requested versus observed direction in
...
functions, fenv API/policy, special/complex/binary80 math, family completion,
promotion, and public x86 support.
`libc-math-tanh` is the separate selected binary32/binary64 hyperbolic slice
for `tanh` and `tanhf`. It compares parenthesized C calls and default-SSE/
`-mfpmath=387` C++ declarations with pinned musl, then runs one freestanding
static candidate. The checked GCC 15.2.0 translation of musl 1.2.6
`tanh.c`/`tanhf.c` localizes only their matching `expm1.c`/`expm1f.c`
providers instead of calling the separately selected public entry points or
ambient libm. Its 256 raw 32-byte records cover signed zero, subnormal force
evaluation, both source threshold neighborhoods, saturation boundaries, finite
extrema, infinities, quiet/signaling NaNs, IEEE flags, and requested versus
observed direction in all four MXCSR modes. It excludes `tanhl`, tan/sin/cos,
complex/special/inverse-hyperbolic math, fenv API/policy, family completion,
promotion, and public x86 support.
`libc-math-atanh` is the separate selected binary32/binary64 inverse-
hyperbolic slice for `atanh` and `atanhf`. It compares parenthesized C calls
and default-SSE/`-mfpmath=387` C++ declarations with pinned musl, then runs one
freestanding static candidate. The checked GCC 15.2.0 translation of musl
1.2.6 `atanh.c`/`atanhf.c` localizes only their matching
`log1p.c`/`log1pf.c` providers instead of calling a separately selected public
entry point or ambient libm. Its 256 raw 32-byte records cover signed zero,
subnormal force evaluation, tiny-input and half-branch neighborhoods,
pole/domain boundaries, finite extrema, infinities, quiet/signaling NaNs, IEEE
flags, and requested versus observed direction in all four MXCSR modes. It
excludes `atanhl`, atan/tanh/sin/cos/asinh/acosh/hyperbolic math,
fenv API/policy, family completion, promotion, and public x86 support.
`libc-math-acosh` is the separate selected binary32/binary64 inverse-
hyperbolic slice for `acosh` and `acoshf`. It compares parenthesized C calls
and default-SSE/`-mfpmath=387` C++ declarations with pinned musl, then runs one
freestanding static candidate. The checked GCC 15.2.0 translation of musl
1.2.6 `acosh.c`/`acoshf.c` localizes their log/log1p/sqrt providers, fixed
logarithm/reciprocal-square-root tables, and typed domain helpers instead of
calling a selected public sibling or ambient libm. Its 256 raw 32-byte records
cover signed zero, raw below-one subnormal/normal values, near-one/two/large
cutoff boundaries, finite extrema, infinities, quiet/signaling NaNs, IEEE
flags, and requested versus observed direction in all four MXCSR modes. It
...
public x86 support.
`libc-math-pow` is the separate selected binary32/binary64 scalar power slice
for `pow` and `powf`. It compares parenthesized C calls and default-SSE/
`-mfpmath=387` C++ declarations with pinned musl, then runs one freestanding
static candidate. The checked GCC 15.2.0 translation of musl 1.2.6
`pow.c`/`powf.c` localizes their fixed logarithm/exponential tables and typed
exception helpers instead of calling public exp/log/exp2/fabs siblings or
ambient libm. Its 256 raw 40-byte records cover signed-zero parity, signed
integral exponents, pole/domain behavior, finite exponent/logarithm paths,
overflow/underflow, infinities, quiet/signaling NaNs, IEEE flags, and requested
versus observed direction in all four MXCSR modes. It excludes `powl`, public
exp/log/exp2/fabs, fenv API/policy, family completion, promotion, and public
x86 support.
