...
Native Linux/x86-64 staged-foundation evidence commands:
  ...
  project-header-extension-policy [--static-sysroot STATIC_SYSROOT] [DYNAMIC_SYSROOT]  prove reviewed direct project-header C/C++ types and owned-product linkage
  ...
  math-scalar-corrections  run exact native fmaf/fmal/powf regressions with retained oracle defects
...
prepare_owned_text_locale_numeric_component_arguments() {
    ...
    static_product="$(translate_owned_posix_product "$2")" || exit 2
    dynamic_product="$(translate_owned_posix_product "$3")" || exit 2
    TEXT_LOCALE_NUMERIC_COMPONENT_ARGUMENTS=(--static-sysroot "$static_product" "$dynamic_product")
}
...
prepare_owned_bsd_random_arguments() {
    local expected='usage: ./scripts/dev-x86_64.sh owned-bsd-random [--extracted] --static-sysroot STATIC_SYSROOT DYNAMIC_SYSROOT'
    local extracted=0
    if [ "${1:-}" = --extracted ]; then
        extracted=1
        shift
    ...
    [ "$#" -eq 3 ] && [ "$1" = --static-sysroot ] && [ -n "$2" ] && [ -n "$3" ] \
        && [[ "$2" != -* ]] && [[ "$3" != -* ]] || fail "$expected"
    prepare_owned_posix_replay_arguments owned-bsd-random "$@"
    BSD_RANDOM_ARGUMENTS=()
    if [ "$extracted" -eq 1 ]; then
        BSD_RANDOM_ARGUMENTS+=(--extracted)
    fi
    BSD_RANDOM_ARGUMENTS+=("${POSIX_REPLAY_ARGUMENTS[@]}")
}
...
run_project_header_extension_policy() {
...
run_owned_resolver_cancellation_probe() {
    if [ "$#" -eq 1 ]; then
        run_in_resolver_network_container bash /workspace/compat/x86_64/run_owned_resolver_cancellation.sh "$1"
        return
    fi
    prepare_work_dir
    local state container_state
    state="$(mktemp -d "$TMP_DIR/owned-resolver-cancellation.XXXXXX")"
    container_state="/workspace/.work/x86_64/tmp/${state##*/}"
    run_in_container bash /workspace/compat/x86_64/run_owned_resolver_cancellation.sh --prepare "$container_state"
    run_in_resolver_network_container bash /workspace/compat/x86_64/run_owned_resolver_cancellation.sh --prepared "$container_state"
}
...
case "$command" in
    ...
    owned-resolver-cancellation)
        [ "$#" -le 1 ] || fail "owned-resolver-cancellation takes at most one dynamic sysroot"
        ensure_image
        run_owned_resolver_cancellation_probe "$@"
        ;;
    ...
    owned-utmpx)
        ensure_image
        run_in_chroot_cap_container bash /workspace/compat/x86_64/run_owned_utmpx.sh "$@"
        ;;
    owned-fmtmsg)
        ensure_image
    ...
    owned-underscore-fork)
        ...
        ;;
    owned-native-allocator-fork)
        ensure_image
        run_in_chroot_cap_container bash /workspace/compat/x86_64/run_owned_native_allocator_fork.sh "$@"
        ;;
    owned-aio)
        ensure_image
    ...
    owned-locale)
        ensure_image
        run_in_chroot_cap_container bash /workspace/compat/x86_64/run_owned_locale.sh "$@"
        ;;
    owned-wordexp)
        ensure_image
...
PY
    ...
    owned-strfmon)
        ensure_image
        run_in_chroot_cap_container bash /workspace/compat/x86_64/run_owned_strfmon.sh "$@"
        ;;
    owned-wcsftime)
        ensure_image
    ...
    unwinder-dynamic-bounds)
        ...
        ;;
    unwinder-indirect-personality-bounds)
        [ "$#" -eq 0 ] || fail "unwinder-indirect-personality-bounds takes no arguments"
        ensure_image
        run_in_container python3 -B /workspace/unwinder/indirect_personality_bounds.py
        ;;
    unwinder-metadata-target-bounds)
    ...
    libc-fenv)
        ...
        ;;
    math-scalar-corrections)
        [ "$#" -eq 0 ] || fail "math-scalar-corrections takes no arguments"
        ensure_image
        run_in_container bash /workspace/compat/x86_64/run_math_scalar_corrections.sh
        ;;
    libc-owned-scalar-math)
