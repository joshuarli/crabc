#!/usr/bin/env bash
# Native Linux/x86-64 proof for the private __ashrti3 helper ABI.
#
# A pinned-musl C reference reconstructs the selected Rust source branch with
# defined unsigned-word operations: a negative value sign-fills, including for
# negative and >=128 raw counts. The candidate is a fresh Rust-only archive
# linked to an otherwise freestanding C object that directly calls
# __ashrti3. The archive-free link must fail; the archive-backed image must be
# static, closed, retain and transfer control to the helper, and execute the
# same bounded cases.
set -euo pipefail
...
readonly PROBE="${ROOT_DIR}/builtins/fixtures/x86_64_ashrti3_probe.c"
readonly START="${ROOT_DIR}/builtins/fixtures/x86_64_ashrti3_start.S"

fail() {
    printf 'ERROR: private x86 ashrti3 builtins proof: %s\n' "$*" >&2
    exit 1
}
...
if "__ashrti3" not in set(archive.get("defined_symbols", [])):
    raise SystemExit("archive lacks __ashrti3")
...
grep -Eq '[[:space:]]__ashrti3$' "$symbols" || {
    fail "native C object did not require __ashrti3"
}
if grep -Evq '[[:space:]]__ashrti3$' "$symbols"; then
...
if oracle_cc \
    ...
    fail "freestanding ashrti3 link unexpectedly succeeded without the bounded archive"
fi
grep -Fq '__ashrti3' "$without_archive_log" || {
    fail "archive-free link failure did not name __ashrti3"
}
...
grep -Eq '[[:space:]]__ashrti3$' "$candidate_symbols" || {
    fail "candidate did not retain __ashrti3"
}
objdump --disassemble "$candidate" >"$disassembly"
grep -Eq '(call[a-z]*|jmp[a-z]*)[[:space:]].*<__ashrti3>' "$disassembly" || {
    fail "candidate code does not transfer control to __ashrti3"
}
...
printf 'private x86 __ashrti3 compiler-helper ABI: PASS\n'
