#!/usr/bin/env bash
# Native Linux/x86-64 proof for the private __divmodti4 helper ABI.
#
# A pinned-musl C reference uses ordinary defined signed division and remainder.
# The candidate is a fresh Rust-only archive linked to an otherwise freestanding
# C object that directly calls __divmodti4 and receives its remainder through a
# signed __int128 pointer. The archive-free link must fail; the archive-backed
# image must be static, closed, retain and transfer control to the helper, and
# execute the same defined nonzero-divisor cases.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly BUILDER="${ROOT_DIR}/builtins/build_x86_64.py"
readonly PROBE="${ROOT_DIR}/builtins/fixtures/x86_64_divmodti4_probe.c"
readonly START="${ROOT_DIR}/builtins/fixtures/x86_64_divmodti4_start.S"

fail() {
    printf 'ERROR: private x86 divmodti4 builtins proof: %s\n' "$*" >&2
    exit 1
}
...
[ -f "$PROBE" ] || fail "missing divmodti4 C fixture"
[ -f "$START" ] || fail "missing freestanding x86 start fixture"

bash "${ROOT_DIR}/compat/x86_64/run_musl_oracle.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-divmodti4.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
archive="${work_dir}/libcrabc-builtins.a"
provenance="${work_dir}/libcrabc-builtins.a.provenance.json"
object="${work_dir}/divmodti4.o"
start_object="${work_dir}/divmodti4-start.o"
...
if "__divmodti4" not in set(archive.get("defined_symbols", [])):
    raise SystemExit("archive lacks __divmodti4")
...
grep -Eq '[[:space:]]__divmodti4$' "$symbols" || {
    fail "native C object did not require __divmodti4"
}
if grep -Evq '[[:space:]]__divmodti4$' "$symbols"; then
...
if oracle_cc \
    ...
    fail "freestanding divmodti4 link unexpectedly succeeded without the bounded archive"
fi
grep -Fq '__divmodti4' "$without_archive_log" || {
    fail "archive-free link failure did not name __divmodti4"
}
...
grep -Eq '[[:space:]]__divmodti4$' "$candidate_symbols" || {
    fail "candidate did not retain __divmodti4"
}
objdump --disassemble "$candidate" >"$disassembly"
grep -Eq '(call[a-z]*|jmp[a-z]*)[[:space:]].*<__divmodti4>' "$disassembly" || {
    fail "candidate code does not transfer control to __divmodti4"
}
...
printf 'private x86 __divmodti4 compiler-helper ABI: PASS\n'
