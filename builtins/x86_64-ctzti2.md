# Private x86-64 `__ctzti2` helper proof

`builtins/run_x86_64_ctzti2.sh` records one narrow native C ABI boundary for
the existing Rust-only archive: a two-word `Uint128` trailing-zero count through
`__ctzti2`. It is private evidence for the bounded archive only. It is not a
complete compiler runtime, installed public sysroot, libc capability, CRT
startup, or dynamic-loader support.

## Source closure

`builtins/src/lib.rs::__ctzti2` calls `u64::trailing_zeros` on a nonzero low
word. When that word is zero, it returns `64 +` the high-word count instead.
...
name `__ctzti2` while retaining its existing private x86 scope. This is
...
trailing-zero builtin or relying on another compiler runtime.

## Native boundary

The candidate C object declares and calls `__ctzti2` directly. Its static link
...
`__ctzti2` (a direct call or optimized tail jump).

The cases cover the all-zero result, the low-word trailing positions that
produce 0, 1, and 63, and the high-word boundary that produces 64, 65, and
127. They are raw helper cases derived from `__ctzti2`, not a general public C
trailing-zero contract. The leading-zero, first-set-bit, population-count, and
...
```sh
bash /workspace/builtins/run_x86_64_ctzti2.sh
```
