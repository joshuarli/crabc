#!/usr/bin/env bash
# Native Linux/x86-64 proof for the private __suboti4 helper ABI.
#
# A pinned-musl C reference uses checked signed subtraction. The candidate is
# a fresh Rust-only archive linked to an otherwise freestanding C object that
# directly calls __suboti4 and receives its overflow result through an int
# pointer. The archive-free link must fail; the archive-backed image must be
...
readonly PROBE="${ROOT_DIR}/builtins/fixtures/x86_64_suboti4_probe.c"
readonly START="${ROOT_DIR}/builtins/fixtures/x86_64_suboti4_start.S"

fail() {
    printf 'ERROR: private x86 suboti4 builtins proof: %s\n' "$*" >&2
    exit 1
}
...
if "__suboti4" not in set(archive.get("defined_symbols", [])):
    raise SystemExit("archive lacks __suboti4")
...
grep -Eq '[[:space:]]__suboti4$' "$symbols" || {
    fail "native C object did not require __suboti4"
}
...
if oracle_cc \
    ...
    fail "freestanding suboti4 link unexpectedly succeeded without the bounded archive"
fi
grep -Fq '__suboti4' "$without_archive_log" || {
    fail "archive-free link failure did not name __suboti4"
}
...
grep -Eq '[[:space:]]__suboti4$' "$candidate_symbols" || {
    fail "candidate did not retain __suboti4"
}
objdump --disassemble "$candidate" >"$disassembly"
grep -Eq '(call[a-z]*|jmp[a-z]*)[[:space:]].*<__suboti4>' "$disassembly" || {
    fail "candidate code does not transfer control to __suboti4"
}
...
printf 'private x86 __suboti4 compiler-helper ABI: PASS\n'
