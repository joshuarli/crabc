#!/usr/bin/env bash
# Native Linux/x86-64 proof for the private __clzti2 helper ABI.
#
# A pinned-musl C reference reconstructs only the selected Rust source branch:
# count the leading zeros in the high word, or add 64 to the low-word count
# when the high word is zero. The candidate is a fresh Rust-only archive linked
# to an otherwise freestanding C object that directly calls __clzti2. The
...
readonly PROBE="${ROOT_DIR}/builtins/fixtures/x86_64_clzti2_probe.c"
readonly START="${ROOT_DIR}/builtins/fixtures/x86_64_clzti2_start.S"

fail() {
    printf 'ERROR: private x86 clzti2 builtins proof: %s\n' "$*" >&2
    exit 1
}
...
if "__clzti2" not in set(archive.get("defined_symbols", [])):
    raise SystemExit("archive lacks __clzti2")
...
grep -Eq '[[:space:]]__clzti2$' "$symbols" || {
    fail "native C object did not require __clzti2"
}
if grep -Evq '[[:space:]]__clzti2$' "$symbols"; then
...
if oracle_cc \
    ...
    fail "freestanding clzti2 link unexpectedly succeeded without the bounded archive"
fi
grep -Fq '__clzti2' "$without_archive_log" || {
    fail "archive-free link failure did not name __clzti2"
}
...
grep -Eq '[[:space:]]__clzti2$' "$candidate_symbols" || {
    fail "candidate did not retain __clzti2"
}
objdump --disassemble "$candidate" >"$disassembly"
grep -Eq '(call[a-z]*|jmp[a-z]*)[[:space:]].*<__clzti2>' "$disassembly" || {
    fail "candidate code does not transfer control to __clzti2"
}
...
printf 'private x86 __clzti2 compiler-helper ABI: PASS\n'
