#!/usr/bin/env bash
# Native Linux/x86-64 proof for the private __addoti4 helper ABI.
#
# A pinned-musl C reference uses checked signed addition.  The candidate is a
# fresh Rust-only archive linked to an otherwise freestanding C object that
# directly calls __addoti4 and receives its overflow result through an int
...
readonly PROBE="${ROOT_DIR}/builtins/fixtures/x86_64_addoti4_probe.c"
readonly START="${ROOT_DIR}/builtins/fixtures/x86_64_addoti4_start.S"

fail() {
    printf 'ERROR: private x86 addoti4 builtins proof: %s\n' "$*" >&2
    exit 1
}
...
if "__addoti4" not in set(archive.get("defined_symbols", [])):
    raise SystemExit("archive lacks __addoti4")
...
grep -Eq '[[:space:]]__addoti4$' "$symbols" || {
    fail "native C object did not require __addoti4"
}
...
if oracle_cc \
    ...
    fail "freestanding addoti4 link unexpectedly succeeded without the bounded archive"
fi
grep -Fq '__addoti4' "$without_archive_log" || {
    fail "archive-free link failure did not name __addoti4"
}
...
grep -Eq '[[:space:]]__addoti4$' "$candidate_symbols" || {
    fail "candidate did not retain __addoti4"
}
objdump --disassemble "$candidate" >"$disassembly"
grep -Eq '(call[a-z]*|jmp[a-z]*)[[:space:]].*<__addoti4>' "$disassembly" || {
    fail "candidate code does not transfer control to __addoti4"
}
...
printf 'private x86 __addoti4 compiler-helper ABI: PASS\n'
