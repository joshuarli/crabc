#!/usr/bin/env bash
# Native Linux/x86-64 proof for the private __ffsti2 helper ABI.
#
# A pinned-musl C reference reconstructs only the selected Rust source branch:
# zero returns zero; otherwise the first set bit is the two-word trailing-zero
# count plus one. The candidate is a fresh Rust-only archive linked to an
# otherwise freestanding C object that directly calls __ffsti2. The archive-free
# link must fail; the archive-backed image must be static, closed, retain and
# transfer control to the helper, and execute the same bounded cases.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly BUILDER="${ROOT_DIR}/builtins/build_x86_64.py"
readonly PROBE="${ROOT_DIR}/builtins/fixtures/x86_64_ffsti2_probe.c"
readonly START="${ROOT_DIR}/builtins/fixtures/x86_64_ffsti2_start.S"

fail() {
    printf 'ERROR: private x86 ffsti2 builtins proof: %s\n' "$*" >&2
    exit 1
}
...
if "__ffsti2" not in set(archive.get("defined_symbols", [])):
    raise SystemExit("archive lacks __ffsti2")
...
grep -Eq '[[:space:]]__ffsti2$' "$symbols" || {
    fail "native C object did not require __ffsti2"
}
if grep -Evq '[[:space:]]__ffsti2$' "$symbols"; then
...
if oracle_cc \
    ...
    fail "freestanding ffsti2 link unexpectedly succeeded without the bounded archive"
fi
grep -Fq '__ffsti2' "$without_archive_log" || {
    fail "archive-free link failure did not name __ffsti2"
}
...
grep -Eq '[[:space:]]__ffsti2$' "$candidate_symbols" || {
    fail "candidate did not retain __ffsti2"
}
grep -Eq '[[:space:]]__ctzti2$' "$candidate_symbols" || {
    fail "candidate did not retain the source-owned __ctzti2 dependency"
}
objdump --disassemble "$candidate" >"$disassembly"
grep -Eq '(call[a-z]*|jmp[a-z]*)[[:space:]].*<__ffsti2>' "$disassembly" || {
    fail "candidate code does not transfer control to __ffsti2"
}
...
printf 'private x86 __ffsti2 compiler-helper ABI: PASS\n'
