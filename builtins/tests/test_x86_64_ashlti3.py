"""Static contract checks for the private x86 __ashlti3 proof."""
...
FIXTURE = ROOT / "fixtures" / "x86_64_ashlti3_probe.c"
START = ROOT / "fixtures" / "x86_64_ashlti3_start.S"
RUNNER = ROOT / "run_x86_64_ashlti3.sh"
NOTE = ROOT / "x86_64-ashlti3.md"


class Ashlti3ProofTests(unittest.TestCase):
    def test_fixture_preserves_the_raw_count_boundary(self) -> None:
        source = FIXTURE.read_text(encoding="utf-8")
        self.assertIn("extern unsigned_int128 __ashlti3", source)
        self.assertIn("if (shift < 0 || shift >= 128)", source)
        self.assertIn("return value << (unsigned int)shift;", source)
        self.assertIn("return __ashlti3(value, shift);", source)
        self.assertIn("        63,", source)
        self.assertIn("        64,", source)
        self.assertIn("check_case(input, 65", source)
        self.assertIn("check_case(input, 127", source)
    ...
    def test_freestanding_start_returns_probe_status_via_exit_syscall(self) -> None:
        ...
        self.assertIn("call crabc_x86_64_ashlti3_probe", source)
    ...
    def test_runner_closes_the_fresh_archive_link_boundary(self) -> None:
        ...
        self.assertIn('"__ashlti3" not in set', source)
        self.assertIn("freestanding ashlti3 link unexpectedly succeeded", source)
        ...
        self.assertIn("candidate did not retain __ashlti3", source)
        self.assertIn("candidate code does not transfer control to __ashlti3", source)
    ...
    def test_note_keeps_the_proof_private_and_source_closed(self) -> None:
        ...
        self.assertIn("`builtins/src/lib.rs::__ashlti3`", source)
        self.assertIn("`Uint128::shl`", source)
        self.assertIn("not ordinary C\nshift semantics", source)
