#!/usr/bin/env bash
# Native Linux/x86-64 proof for the private __muloti4 helper ABI.
#
# A pinned-musl C reference uses checked signed multiplication. The candidate
# is a fresh Rust-only archive linked to an otherwise freestanding C object
# that directly calls __muloti4 and receives its overflow result through an
# int pointer. The archive-free link must fail; the archive-backed image must
# be static, closed, retain and transfer control to the helper, and execute
# the same cases.
set -euo pipefail
...
readonly PROBE="${ROOT_DIR}/builtins/fixtures/x86_64_muloti4_probe.c"
readonly START="${ROOT_DIR}/builtins/fixtures/x86_64_muloti4_start.S"

fail() {
    printf 'ERROR: private x86 muloti4 builtins proof: %s\n' "$*" >&2
    exit 1
}
...
if "__muloti4" not in set(archive.get("defined_symbols", [])):
    raise SystemExit("archive lacks __muloti4")
...
grep -Eq '[[:space:]]__muloti4$' "$symbols" || {
    fail "native C object did not require __muloti4"
}
...
if oracle_cc \
    ...
    fail "freestanding muloti4 link unexpectedly succeeded without the bounded archive"
fi
grep -Fq '__muloti4' "$without_archive_log" || {
    fail "archive-free link failure did not name __muloti4"
}
...
grep -Eq '[[:space:]]__muloti4$' "$candidate_symbols" || {
    fail "candidate did not retain __muloti4"
}
objdump --disassemble "$candidate" >"$disassembly"
grep -Eq '(call[a-z]*|jmp[a-z]*)[[:space:]].*<__muloti4>' "$disassembly" || {
    fail "candidate code does not transfer control to __muloti4"
}
...
printf 'private x86 __muloti4 compiler-helper ABI: PASS\n'
