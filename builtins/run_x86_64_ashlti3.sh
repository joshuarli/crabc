#!/usr/bin/env bash
# Native Linux/x86-64 proof for the private __ashlti3 helper ABI.
#
...
# in-range unsigned shifts execute normally. The candidate is a fresh Rust-only
# archive linked to an otherwise freestanding C object that directly calls
# __ashlti3. The archive-free link must fail; the archive-backed image must be
...
readonly PROBE="${ROOT_DIR}/builtins/fixtures/x86_64_ashlti3_probe.c"
readonly START="${ROOT_DIR}/builtins/fixtures/x86_64_ashlti3_start.S"

fail() {
    printf 'ERROR: private x86 ashlti3 builtins proof: %s\n' "$*" >&2
    exit 1
}
...
if "__ashlti3" not in set(archive.get("defined_symbols", [])):
    raise SystemExit("archive lacks __ashlti3")
...
grep -Eq '[[:space:]]__ashlti3$' "$symbols" || {
    fail "native C object did not require __ashlti3"
}
if grep -Evq '[[:space:]]__ashlti3$' "$symbols"; then
...
if oracle_cc \
    ...
    fail "freestanding ashlti3 link unexpectedly succeeded without the bounded archive"
fi
grep -Fq '__ashlti3' "$without_archive_log" || {
    fail "archive-free link failure did not name __ashlti3"
}
...
grep -Eq '[[:space:]]__ashlti3$' "$candidate_symbols" || {
    fail "candidate did not retain __ashlti3"
}
objdump --disassemble "$candidate" >"$disassembly"
grep -Eq '(call[a-z]*|jmp[a-z]*)[[:space:]].*<__ashlti3>' "$disassembly" || {
    fail "candidate code does not transfer control to __ashlti3"
}
...
printf 'private x86 __ashlti3 compiler-helper ABI: PASS\n'
