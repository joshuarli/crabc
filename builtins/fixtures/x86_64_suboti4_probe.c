/*
 * Private x86-64 __suboti4 compiler-helper ABI probe.
 *
 ...
 * native C ABI. The reference path uses GCC's checked-subtract builtin and
...
static signed_int128 call_suboti4(
    signed_int128 left,
    signed_int128 right,
    int *overflow
) {
...
#else
extern signed_int128 __suboti4(signed_int128 left, signed_int128 right, int *overflow);
...
static signed_int128 call_suboti4(
    ...
    return __suboti4(left, right, overflow);
}
#endif
...
static int check_case(
    ...
    int overflow = -77;
    signed_int128 result = call_suboti4(left, right, &overflow);
...
int crabc_x86_64_suboti4_probe(void) {
    ...
    result = check_case(minimum, maximum, 1, 1, 4);
    if (result != 0) {
        return result;
    }
    result = check_case(minimum, minimum, 0, 0, 5);
    if (result != 0) {
        return result;
    }
...
int main(void) {
    return crabc_x86_64_suboti4_probe();
}
#endif
