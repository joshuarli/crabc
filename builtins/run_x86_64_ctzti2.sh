#!/usr/bin/env bash
# Native Linux/x86-64 proof for the private __ctzti2 helper ABI.
#
# A pinned-musl C reference reconstructs only the selected Rust source branch:
# count the trailing zeros in the low word, or add 64 to the high-word count
# when the low word is zero. The candidate is a fresh Rust-only archive linked
# to an otherwise freestanding C object that directly calls __ctzti2. The
# archive-free link must fail; the archive-backed image must be static, closed,
# retain and transfer control to the helper, and execute the same bounded cases.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly BUILDER="${ROOT_DIR}/builtins/build_x86_64.py"
readonly PROBE="${ROOT_DIR}/builtins/fixtures/x86_64_ctzti2_probe.c"
readonly START="${ROOT_DIR}/builtins/fixtures/x86_64_ctzti2_start.S"

fail() {
    printf 'ERROR: private x86 ctzti2 builtins proof: %s\n' "$*" >&2
    exit 1
}
...
if "__ctzti2" not in set(archive.get("defined_symbols", [])):
    raise SystemExit("archive lacks __ctzti2")
...
grep -Eq '[[:space:]]__ctzti2$' "$symbols" || {
    fail "native C object did not require __ctzti2"
}
if grep -Evq '[[:space:]]__ctzti2$' "$symbols"; then
...
if oracle_cc \
    ...
    fail "freestanding ctzti2 link unexpectedly succeeded without the bounded archive"
fi
grep -Fq '__ctzti2' "$without_archive_log" || {
    fail "archive-free link failure did not name __ctzti2"
}
...
grep -Eq '[[:space:]]__ctzti2$' "$candidate_symbols" || {
    fail "candidate did not retain __ctzti2"
}
objdump --disassemble "$candidate" >"$disassembly"
grep -Eq '(call[a-z]*|jmp[a-z]*)[[:space:]].*<__ctzti2>' "$disassembly" || {
    fail "candidate code does not transfer control to __ctzti2"
}
...
printf 'private x86 __ctzti2 compiler-helper ABI: PASS\n'
