...
| C `pthread_mutex_cond_ping_pong` ×10,000 | 1.0130×–1.0144× one-sided CPU upper bounds across three 31-sample `pthread-cond-relaxed-waiter-*-31` reports; 6.0009–6.0029 crabc vs 6.0014–6.0026 musl marked calls/op | two protected increments and condition handoffs per round prove deterministic contention; the sequence and waiter increments use the inline AArch64 acquire/release fetch-add exclusive loop rather than LLVM's outlined LSE capability dispatcher. Signal and broadcast read their advisory waiter hint with an ordinary AArch64 `ldr`, matching musl; mutex release/acquire and the sequence futex retain the synchronization edge. The futex boundary and 10-iteration pthread stress remain unchanged, but the result stays above the 0.90× gate | P1 |
