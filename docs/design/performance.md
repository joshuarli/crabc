...
| C `stdio_file_4k` ×100 | 0.9865×–1.0168× one-sided CPU upper bounds across three 31-sample `stdio-current-cancel-{31,repeat-31,repeat2-31}` reports; 5.03 crabc vs 8.00 musl marked calls/op | `fdopen` clears the complete observable `FILE` state without zeroing its trailing buffer, which read/write paths initialize before consumption. `__stdio_read` checks its already-published current pthread slot directly; a caller without such a slot cannot have a crabc cancellation request, while public `pthread_testcancel` retains lazy foreign-thread registration. The direct musl lifecycle regression and ten-iteration pthread stress preserve `O_CLOEXEC`, descriptor ownership across `fclose`, write/flush/seek, buffered reads, `ungetc`, and deferred/asynchronous stdio cancellation. The syscall gate passes, but CPU remains red | P1 |
| C `stdio_format_parse` ×1,000 | 1.0328×–1.0585× one-sided CPU upper bounds across five `stdio-direct-local-lookahead-{matrix,repeat,repeat2,final,final2}-31` reports; 6.003 crabc vs 6.211 musl marked calls/op | lane-private formatted records prove `fprintf`/flush/rewind/`fscanf`, bounded memory formatting/scanning, and ordered unread-tail preservation. The ordinary no-length scalar grammar parses directly from `FILE`, retaining only one local delimiter before restoring it through `ungetc`; its 0.9882×–1.0161× median range removes the staged parser's EOF probe. The direct differential exercises that path with one-byte FILE storage, separately forces staged `%n` seek-back, and proves an unbuffered read invalidates the cache before another scan. The marked syscall gate passes; CPU remains red | P1 |
| C `pthread_create_join_tls` ×1,000 | 0.8911×–0.9171× one-sided CPU upper bounds across three 31-sample `pthread-tls-single-tcb-init-{matrix,repeat,repeat2}-31` reports; 6.000 crabc vs 11.966 musl marked calls/op | one page-aligned mapping places the TLS block above the downward-growing stack, matching musl at one marked `mmap` and `munmap` per operation. The TLS bridge materializes fresh variable images before copying the parent ABI TCB, then writes allocation and logical-thread metadata once rather than initializing and immediately overwriting a fresh TCB. The next `pthread_join` iteration already checks cancellation after a futex return, so one redundant adjacent `pthread_testcancel` call is removed. The pinned-musl stress regression cancels a blocked joiner and proves a later join still returns its target result. Normal exit still omits the unused `detach_state` futex wake, while `pthread_timedjoin_np` permanently marks interest in that word before waiting: exit either wakes the marked waiter or changes its expected word first. The 513-lifetime direct musl differential, dynamic-TLS cases, ten-iteration pthread stress, and loader cases pass. CPU remains red | P1 |
...
| C `pthread_mutex_cond_ping_pong` ×10,000 | 0.9963×–1.0043× one-sided CPU upper bounds across three 31-sample `pthread-cond-current-cancel-{,repeat,repeat2}` reports; 6.0007–6.0021 crabc vs 6.0022–6.0052 musl marked calls/op | two protected increments and condition handoffs per round prove deterministic contention; the sequence and waiter increments use the inline AArch64 acquire/release fetch-add exclusive loop rather than LLVM's outlined LSE capability dispatcher. `pthread_cond_wait` uses private exact mutex lock/unlock helpers after its existing private timed-wait route, matching musl's internal binding and removing the two public mutex PLT calls per wait. Its cancellation points inspect the already-published current pthread slot directly: an unregistered caller cannot have a crabc cancellation request, while pthread-created threads publish the slot before user code. The public `pthread_testcancel` ABI, weak/default-visible C ABI, sequence futex, advisory waiter hint, and 10-iteration pthread stress remain unchanged. The result stays above the 0.90× gate | P1 |
...
4. `fd_file_4k`, `stdio_file_4k`, and `stdio_format_parse` give
   ...
   inputs. `__stdio_read` follows musl's two-iovec lookahead shape and checks
   only its published current pthread slot at its cancellation points; the
   public cancellation API retains lazy foreign-thread registration. The
   formatted row retains
