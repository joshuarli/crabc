...
| C `memset` scalar matrix | 1.1723×/1.1689×; 1.1720×/1.8702×; 1.0956×/1.5741× | all CPU bounds fail; unaligned medium/large fills remain the largest gap | P1 |
...
3. `strlen` and `memchr` use bounded word scans, `strstr` is a page-safe
   ...
   unaligned. `memset` remains at 1.1889×/1.2951×, while `strlen`, `memchr`, `strstr`, and
