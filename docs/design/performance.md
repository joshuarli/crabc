...
| C `stdio_file_4k` ×100 | 0.9875×–1.0031× one-sided CPU upper bounds across three 31-sample `fdopen-malloc-{matrix,repeat,repeat2}-31` reports; 5.03 crabc vs 8.00 musl marked calls/op | `fdopen` clears the complete observable `FILE` state without zeroing its trailing buffer, which read/write paths initialize before consumption. The direct musl lifecycle regression proves `O_CLOEXEC`, descriptor ownership across `fclose`, write/flush/seek, buffered reads, and `ungetc`; broad stdio tests pass. The syscall gate passes, but CPU remains red | P1 |
