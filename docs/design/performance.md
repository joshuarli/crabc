...
| C `pthread_create_join_tls` ×1,000 | 0.9139×–0.9381× one-sided CPU upper bounds across three 31-sample `pthread-conditional-exit-wake-{matrix,repeat,repeat2}-31` reports; 6.000 crabc vs 11.966 musl marked calls/op | one page-aligned mapping places the TLS block above the downward-growing stack, matching musl at one marked `mmap` and `munmap` per operation. Normal `pthread_join` waits on `CLONE_CHILD_CLEARTID`, so ordinary exit omits its unused `detach_state` futex wake. `pthread_timedjoin_np` permanently marks interest in that same futex word before waiting: exit either wakes the marked waiter or changes its expected word first. The extension regression proves timeout then successful timed join; the 513-lifetime direct musl differential, dynamic-TLS cases, ten-iteration pthread stress, and loader cases pass. CPU remains red | P1 |
