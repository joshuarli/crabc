...
The workspace now contains the `crabc-mimalloc` crate with source-mapped pure
configuration, arithmetic, types, provenance, atomic operations, size classes,
bitmap kernels and a caller-owned dynamic bitmap view, page-map arithmetic, a
private regular-mapping primitive
boundary, a private futex lock boundary, and bounded nonallocating support
kernels. The recursive allocation-free once protocol is present as a
...
claiming live page initialization. A fixed-capacity `cfg(miri)` mapping model exercises the current VM
transitions without broadening production support; the pinned toolchain does
not currently install Miri itself. The crate exposes no allocator or lifecycle operation, adds no libc
integration or backend selection, and makes no allocator-readiness claim.
...
The random module remains absent until an exact focused RustCrypto dependency
is explicitly approved and its 64-bit counter/nonce layout, output ordering,
context splitting, `no_std` behavior, transitive graph, build code, allocation,
global state, and LTO effects are documented and differentially tested. This
is a dependency decision, not permission to reduce secure-mode parity or port
cryptographic code by hand.

The audited candidate, still pending approval, is
`chacha20 = "=0.10.1"` with default features disabled and only `legacy` plus
`zeroize` enabled. The selected Linux/AArch64 graph is nine packages total:
...
either package in the fixed Linux/AArch64 graph. Any approval must therefore
make the structure check judge the selected production target graph and still
reject a compiled libc dependency, rather than mistaking lockfile presence for
an active allocator edge.
...
buffer. The pure-Rust generic implementation introduces no native call
boundary and remains eligible for fat LTO; its AArch64 code size, selected
NEON path, and throughput are explicitly unqualified until measured. Approval
would require updating the structure dependency rule and checked-in lockfile;
this paragraph does not grant that approval.
