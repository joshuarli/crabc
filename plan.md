...
| Allocator component qualification / implementation | Clean `8e00164c` M2 now produces a partial receipt rather than crashing. Its six remaining components retain their actual source-transition, failure-ownership and native evidence requirements; partial status alone does not prove that an implementation is absent. | Close each receipt's `remaining_conditions` through its real production owner and pinned-C evidence. Current work targets later-thread Theap metadata failure cleanup and traces the explicitly refused detached-page free/reallocation state. Keep C selected until all promotion requirements pass. | Ordinary-page lifecycle, metadata, failure and concurrency work; no dependency on native huge-page success for these paths. |
