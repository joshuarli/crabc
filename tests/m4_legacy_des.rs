...
fn legacy_des_bit_array_apis_under_libc_so() {
    ...
    let source = root.join("tests/fixtures/m4_legacy_des_test.c");
    let binary = test_support::TempArtifact::new("crabc-m4-legacy-des");
    ...
    let status = Command::new("musl-gcc")
        ...
        .status()
        .expect("failed to run musl-gcc for m4_legacy_des_test");
    assert!(status.success(), "m4_legacy_des_test compilation failed");

    let output = Command::new(&binary)
        .env("LD_LIBRARY_PATH", &target)
        .output()
        .expect("failed to run m4_legacy_des_test");
    assert!(
        output.status.success(),
        "m4_legacy_des_test exited with {:?}, stdout: {}, stderr: {}",
        output.status.code(),
