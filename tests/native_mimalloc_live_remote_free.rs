...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_live_remote_free_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc live remote-free fixture");
    assert!(status.success(), "native mimalloc live remote-free fixture compilation failed");
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc live remote-free fixture")
}

#[test]
fn native_mimalloc_live_remote_free_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new("native-mimalloc-live-remote-free-reference");
    let candidate = test_support::TempArtifact::new("native-mimalloc-live-remote-free-candidate");
    ...
    assert_eq!(reference_output.stdout, b"native mimalloc live remote free ok\n");
    assert_eq!(
