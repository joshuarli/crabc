...
fn syscall_filesystem_exports_under_libc_so() {
    ...
    let src = fixtures.join("m4_syscall_exports_test.c");
    ...
    let bin = test_support::TempArtifact::new("crabc-m4-syscall-exports");
    let status = Command::new("musl-gcc")
        ...
        .status()
        .expect("failed to run musl-gcc for m4_syscall_exports_test");
    assert!(
        status.success(),
        "musl-gcc m4_syscall_exports_test compilation failed"
    );

    let output = Command::new(&bin)
        .env("LD_LIBRARY_PATH", &target)
        .output()
        .expect("failed to run m4_syscall_exports_test");
    let _ = std::fs::remove_file(&bin);

    assert!(
        output.status.success(),
        "m4_syscall_exports_test exited with {:?}, stdout: {}, stderr: {}",
        output.status.code(),
