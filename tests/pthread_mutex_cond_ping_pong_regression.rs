...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/pthread_mutex_cond_ping_pong_regression_test.c");
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the pthread mutex condition ping-pong regression fixture")
}

#[test]
fn pthread_mutex_condition_ping_pong_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new("pthread-mutex-condition-ping-pong-reference");
    let candidate = test_support::TempArtifact::new("pthread-mutex-condition-ping-pong-candidate");
