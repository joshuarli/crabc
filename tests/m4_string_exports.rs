...
fn string_and_memory_exports_under_libc_so() {
    ...
    let source = root.join("tests/fixtures/m4_string_exports_test.c");
    let binary = test_support::TempArtifact::new("crabc-m4-string");
    let status = Command::new("musl-gcc")
        ...
        .status()
        .expect("failed to compile M4 string fixture");
    assert!(status.success());
    let output = Command::new(&binary)
        .env("LD_LIBRARY_PATH", &target)
        .output()
        .expect("failed to run M4 string fixture");
    ...
    assert_eq!(String::from_utf8_lossy(&output.stdout), "m4 string exports ok\n");
}
