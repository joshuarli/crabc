...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_post_exit_source_published_successor_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc post-exit source-published successor fixture");
    assert!(
        status.success(),
        "native mimalloc post-exit source-published successor fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc post-exit source-published successor fixture")
}

#[test]
fn native_mimalloc_post_exit_source_published_successor_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new(
        "native-mimalloc-post-exit-source-published-successor-reference",
    );
    let candidate = test_support::TempArtifact::new(
        "native-mimalloc-post-exit-source-published-successor-candidate",
    );
    ...
    assert_eq!(
        ...
        b"native mimalloc post exit source published successor ok\n"
    );
    assert_eq!(
