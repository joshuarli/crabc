...
const NATIVE_CONCURRENT_SESSION_START_EPOCHS: usize = 128;

fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    let root = std::path::Path::new(test_support::REPOSITORY_ROOT);
    let fixture = root.join("tests/fixtures/native_mimalloc_concurrent_session_start_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc concurrent-session-start fixture");
    assert!(
        status.success(),
        "native mimalloc concurrent-session-start fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc concurrent-session-start fixture")
}

#[test]
fn native_mimalloc_concurrent_session_start_matches_pinned_musl() {
    let reference =
        test_support::TempArtifact::new("native-mimalloc-concurrent-session-start-reference");
    let candidate =
        test_support::TempArtifact::new("native-mimalloc-concurrent-session-start-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc concurrent session start ok\n"
    );
    for epoch in 0..NATIVE_CONCURRENT_SESSION_START_EPOCHS {
