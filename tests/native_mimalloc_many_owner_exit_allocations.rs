...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_many_owner_exit_allocations_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc many-owner-exit fixture");
    assert!(
        status.success(),
        "native mimalloc many-owner-exit fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc many-owner-exit fixture")
}

#[test]
fn native_mimalloc_many_owner_exit_allocations_match_pinned_musl() {
    let reference = test_support::TempArtifact::new("native-mimalloc-many-owner-exit-reference");
    let candidate = test_support::TempArtifact::new("native-mimalloc-many-owner-exit-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc many owner-exit allocations ok\n"
    );
    assert_eq!(
