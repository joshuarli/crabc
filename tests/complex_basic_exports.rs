...
fn complex_basic_exports_under_libc_so() {
    ...
    let source = root.join("tests/fixtures/complex_basic_exports_test.c");
    let binary = test_support::TempArtifact::new("crabc-c-abi-complex-basic");
    ...
    assert!(status.success(), "complex-basic fixture compilation failed");

    let output = Command::new(&binary)
        .env("LD_LIBRARY_PATH", &target)
        .output()
        .expect("failed to run complex-basic fixture");
    let _ = std::fs::remove_file(&binary);
    assert!(
        output.status.success(),
        "complex-basic fixture exited with {:?}, stdout: {}, stderr: {}",
        output.status.code(),
    ...
    assert_eq!(
        String::from_utf8_lossy(&output.stdout),
        "c-abi complex basic exports ok\n"
    );
}
