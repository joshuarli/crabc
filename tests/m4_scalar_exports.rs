...
#[test]
fn scalar_math_and_bit_exports_under_libc_so() {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let target = root.join("target/debug");
    let source = root.join("tests/fixtures/m4_scalar_exports_test.c");
    let binary = test_support::TempArtifact::new("crabc-m4-scalar");
    let status = Command::new("musl-gcc")
        ...
        .status()
        .expect("failed to compile M4 scalar fixture");
    assert!(status.success());
    let output = Command::new(&binary)
        .env("LD_LIBRARY_PATH", &target)
        .output()
        .expect("failed to run M4 scalar fixture");
    ...
    assert_eq!(String::from_utf8_lossy(&output.stdout), "m4 scalar exports ok\n");
}
