...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/pthread_create_join_tls_regression_test.c");
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the pthread create/join TLS regression fixture")
}

#[test]
fn pthread_create_join_tls_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new("pthread-create-join-tls-reference");
    let candidate = test_support::TempArtifact::new("pthread-create-join-tls-candidate");
