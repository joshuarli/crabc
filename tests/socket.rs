...
#[test]
fn socket_functions_under_libc_so() {
    ...
    let status = Command::new(test_support::crabc_cc())
        ...
        .status()
        .expect("failed to run crabc-cc for socket_test");
    assert!(status.success(), "crabc-cc socket_test compilation failed");
    ...
    let output = Command::new(&bin)
        ...
        .output()
        .expect("failed to run socket_test");

    assert!(
        output.status.success(),
        "socket_test exited with {:?}, stderr: {}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
    assert_eq!(String::from_utf8_lossy(&output.stdout), "socket ok\n");
}
