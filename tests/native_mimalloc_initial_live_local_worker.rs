...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_initial_live_local_worker_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc initial-live-local-worker fixture");
    assert!(
        status.success(),
        "native mimalloc initial-live-local-worker fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc initial-live-local-worker fixture")
}

#[test]
fn native_mimalloc_initial_live_local_worker_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new("native-mimalloc-initial-live-local-reference");
    let candidate = test_support::TempArtifact::new("native-mimalloc-initial-live-local-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc initial live local worker ok\n"
    );
    assert_eq!(
