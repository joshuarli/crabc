...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_post_exit_split_releaser_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc post-exit-split-releaser fixture");
    assert!(
        status.success(),
        "native mimalloc post-exit-split-releaser fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc post-exit-split-releaser fixture")
}

#[test]
fn native_mimalloc_post_exit_split_releaser_matches_pinned_musl() {
    let reference =
        test_support::TempArtifact::new("native-mimalloc-post-exit-split-releaser-reference");
    let candidate =
        test_support::TempArtifact::new("native-mimalloc-post-exit-split-releaser-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc post exit split releaser ok\n"
    );
    assert_eq!(
