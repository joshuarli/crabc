...
fn complex_power_exports_under_libc_so() {
    ...
    let source = root.join("tests/fixtures/m4_complex_powers_exports_test.c");
    let binary = test_support::TempArtifact::new("crabc-m4-complex-powers");
    ...
    let status = Command::new("musl-gcc")
        .args(&args)
        .status()
        .expect("failed to compile M4 complex-powers fixture");
    assert!(
        status.success(),
        "M4 complex-powers fixture compilation failed"
    );

    let output = Command::new(&binary)
        .env("LD_LIBRARY_PATH", &target)
        .output()
        .expect("failed to run M4 complex-powers fixture");
    assert!(
        output.status.success(),
        "M4 complex-powers fixture exited with {:?}, stdout: {}, stderr: {}",
        output.status.code(),
