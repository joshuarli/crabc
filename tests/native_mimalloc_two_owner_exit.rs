...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_two_owner_exit_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc two-owner-exit fixture");
    assert!(
        status.success(),
        "native mimalloc two-owner-exit fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc two-owner-exit fixture")
}

#[test]
fn native_mimalloc_two_owner_exit_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new("native-mimalloc-two-owner-exit-reference");
    let candidate = test_support::TempArtifact::new("native-mimalloc-two-owner-exit-candidate");
    ...
    assert_eq!(reference_output.stdout, b"native mimalloc two owner exit ok\n");
    assert_eq!(
