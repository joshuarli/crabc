...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_source_published_live_owner_exit_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc source-published live-owner-exit fixture");
    assert!(
        status.success(),
        "native mimalloc source-published live-owner-exit fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc source-published live-owner-exit fixture")
}

#[test]
fn native_mimalloc_source_published_live_owner_exit_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new(
        "native-mimalloc-source-published-live-owner-exit-reference",
    );
    let candidate = test_support::TempArtifact::new(
        "native-mimalloc-source-published-live-owner-exit-candidate",
    );
    ...
    assert_eq!(
        ...
        b"native mimalloc source published live owner exit ok\n"
    );
    assert_eq!(
