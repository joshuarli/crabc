...
#[test]
fn fnmatch_posix_classes_under_libldso() {
    ...
    let src = fixtures.join("fnmatch_test.c");
    let bin = test_support::TempArtifact::new("fnmatch_test");
    let status = Command::new(test_support::crabc_cc())
        ...
        .status()
        .expect("failed to run crabc-cc for fnmatch_test");
    assert!(status.success(), "crabc-cc fnmatch_test compilation failed");
    ...
    let output = Command::new(&bin)
        ...
        .output()
        .expect("failed to run fnmatch_test");

    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        output.status.success(),
        "fnmatch_test exited with {:?}, stderr: {}",
        output.status.code(),
        stderr
    );
