...
#[test]
fn net_functions_under_libc_so() {
    ...
    let src = fixtures.join("net_test.c");
    let bin = test_support::TempArtifact::new("net_test");
    let status = Command::new(test_support::crabc_cc())
        ...
        .status()
        .expect("failed to run crabc-cc for net_test");
    assert!(status.success(), "crabc-cc net_test compilation failed");
