...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_two_live_remote_owners_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the two-live-owner native mimalloc fixture");
    assert!(
        status.success(),
        "two-live-owner native mimalloc fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the two-live-owner native mimalloc fixture")
}

#[test]
fn native_mimalloc_two_live_remote_owners_match_pinned_musl() {
    let reference = test_support::TempArtifact::new("native-mimalloc-two-live-owners-reference");
    let candidate = test_support::TempArtifact::new("native-mimalloc-two-live-owners-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc two live remote owners ok\n"
    );
    assert_eq!(
