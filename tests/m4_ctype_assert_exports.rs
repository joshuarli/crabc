...
fn ctype_assert_exports_under_libc_so() {
    ...
    let source = root.join("tests/fixtures/m4_ctype_assert_exports_test.c");
    let binary = test_support::TempArtifact::new("crabc-m4-ctype-assert");
    ...
    let status = Command::new("musl-gcc")
        .args(&args)
        .status()
        .expect("failed to compile m4_ctype_assert_exports_test");
    assert!(
        status.success(),
        "m4_ctype_assert_exports_test compilation failed"
    );

    let output = Command::new(&binary)
        .env("LD_LIBRARY_PATH", &target)
        .output()
        .expect("failed to run m4_ctype_assert_exports_test");
    let _ = std::fs::remove_file(&binary);
    assert!(
        output.status.success(),
        "m4_ctype_assert_exports_test exited with {:?}, stdout: {}, stderr: {}",
        output.status.code(),
    ...
    assert_eq!(
        String::from_utf8_lossy(&output.stdout),
        "m4 ctype assert exports ok\n"
    );
    assert_eq!(
        String::from_utf8_lossy(&output.stderr),
        "Assertion failed: value != 0 (m4_ctype_assert_exports_test.c: main: 77)\n"
    );
}
