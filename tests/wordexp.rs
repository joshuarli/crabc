...
#[test]
fn shell_word_expansion_under_libc_so() {
    ...
    let src = fixtures.join("wordexp_test.c");
    let bin = test_support::TempArtifact::new("wordexp_test");
    let status = Command::new(test_support::crabc_cc())
        ...
        .status()
        .expect("failed to run crabc-cc for wordexp_test");
    assert!(status.success(), "crabc-cc wordexp_test compilation failed");
    ...
    assert!(
        ...
        "wordexp_test exited with {:?}, stderr: {}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
    assert_eq!(String::from_utf8_lossy(&output.stdout), "wordexp ok\n");
}
