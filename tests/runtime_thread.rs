...
fn native_runtime_thread_uses_libc_owned_state() {
    ...
    let archive = target
        .join("release/examples/libruntime_thread_probe.a");
    ...
    assert!(probe_build.success(), "native thread probe archive did not build");
    ...
    assert!(status.success(), "native thread runtime fixture did not compile");
