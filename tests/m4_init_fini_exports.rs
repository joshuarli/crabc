...
fn weak_init_fini_exports_preserve_startup_and_finalization_order() {
    ...
    let source = root.join("tests/fixtures/m4_init_fini_exports_test.c");
    let binary = test_support::TempArtifact::new("crabc-m4-init-fini");
    ...
    let status = Command::new("musl-gcc")
        ...
        .status()
        .expect("failed to run musl-gcc for m4_init_fini_exports_test");
    assert!(
        status.success(),
        "musl-gcc m4_init_fini_exports_test compilation failed"
    );
    ...
    let output = Command::new(&binary)
        ...
        .output()
        .expect("failed to run m4_init_fini_exports_test");
    assert_eq!(
        output.status.code(),
        Some(37),
        "m4_init_fini_exports_test exited with {:?}, stdout: {}, stderr: {}",
        output.status.code(),
