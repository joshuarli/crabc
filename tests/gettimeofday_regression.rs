...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/gettimeofday_regression_test.c");
...
#[test]
fn gettimeofday_matches_pinned_musl_for_null_and_canonical_microseconds() {
    let reference = test_support::TempArtifact::new("gettimeofday-reference");
    let candidate = test_support::TempArtifact::new("gettimeofday-candidate");
    ...
    assert_eq!(reference_output.stdout, b"gettimeofday contract ok\n");
    assert_eq!(
