...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_many_local_allocations_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc many-local-allocations fixture");
    assert!(
        status.success(),
        "native mimalloc many-local-allocations fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc many-local-allocations fixture")
}

#[test]
fn native_mimalloc_many_local_allocations_match_pinned_musl() {
    let reference = test_support::TempArtifact::new("native-mimalloc-many-local-reference");
    let candidate = test_support::TempArtifact::new("native-mimalloc-many-local-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc many local allocations ok\n"
    );
    assert_eq!(
