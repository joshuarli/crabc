...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_initial_remote_free_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc initial remote-free fixture");
    assert!(
        status.success(),
        "native mimalloc initial remote-free fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc initial remote-free fixture")
}

#[test]
fn native_mimalloc_initial_remote_free_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new("native-mimalloc-initial-remote-free-reference");
    let candidate = test_support::TempArtifact::new("native-mimalloc-initial-remote-free-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc initial remote free ok\n"
    );
    assert_eq!(
