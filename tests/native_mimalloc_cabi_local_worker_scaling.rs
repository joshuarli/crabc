...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_cabi_local_worker_scaling_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc C ABI local-worker scaling fixture");
    assert!(
        status.success(),
        "native mimalloc C ABI local-worker scaling fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc C ABI local-worker scaling fixture")
}

#[test]
fn native_mimalloc_cabi_local_worker_scaling_matches_pinned_musl() {
    let reference =
        test_support::TempArtifact::new("native-mimalloc-cabi-local-worker-scaling-reference");
    let candidate =
        test_support::TempArtifact::new("native-mimalloc-cabi-local-worker-scaling-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc C ABI local worker scaling ok\n"
    );
    assert_eq!(reference_output.stderr, b"");
    assert_eq!(
        candidate_output.status,
        reference_output.status,
        "selected native shadow differs; stderr: {}",
