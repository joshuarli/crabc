...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_initial_live_parallel_workers_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc initial-live-parallel-workers fixture");
    assert!(
        status.success(),
        "native mimalloc initial-live-parallel-workers fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc initial-live-parallel-workers fixture")
}

#[test]
fn native_mimalloc_initial_live_parallel_workers_match_pinned_musl() {
    let reference =
        test_support::TempArtifact::new("native-mimalloc-initial-live-parallel-workers-reference");
    let candidate =
        test_support::TempArtifact::new("native-mimalloc-initial-live-parallel-workers-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc initial live parallel workers ok\n"
    );
    assert_eq!(
