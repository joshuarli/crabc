...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/native_mimalloc_aggregate_reclaim_test.c");
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc aggregate-reclaim fixture");
    assert!(
        status.success(),
        "native mimalloc aggregate-reclaim fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc aggregate-reclaim fixture")
}

#[test]
fn native_mimalloc_aggregate_reclaim_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new("native-mimalloc-aggregate-reclaim-reference");
    let candidate = test_support::TempArtifact::new("native-mimalloc-aggregate-reclaim-candidate");
    ...
    assert_eq!(
        ...
        b"native mimalloc aggregate reclaim ok\n"
    );
    assert_eq!(
