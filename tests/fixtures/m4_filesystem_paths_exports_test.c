...
int main(void)
{
    char file_name[] = "/tmp/crabc-m4-paths-file-XXXXXX";
    char dir_name[256] = { 0 };
    ...
    CHECK(eaccess(file_name, F_OK) == 0 && euidaccess(file_name, F_OK) == 0,
          "effective access");
    errno = 0;
    CHECK(faccessat(AT_FDCWD, "/tmp/crabc-m4-paths-missing", F_OK, 0) == -1 &&
              errno == ENOENT,
          "faccessat errno");
    errno = 0;
    CHECK(eaccess("/tmp/crabc-m4-paths-missing", F_OK) == -1 && errno == ENOENT,
