...
int main(void)
{
    char template[] = "/tmp/crabc-c-abi-traversal-XXXXXX";
    char pattern[256];
    char nested[256];
    char file_a[256];
    char file_b[256];
    ...
    free(selected);
    selected = NULL;
    if (scandir("/tmp/crabc-c-abi-traversal-no-such-directory", &entries, NULL, NULL) != -1)
        return 9;
    ...
    glob_errors = 0;
    if (glob("/tmp/crabc-c-abi-traversal-no-such-directory/*", GLOB_ERR,
    ...
    puts("c-abi filesystem traversal ok");
    return 0;
}
