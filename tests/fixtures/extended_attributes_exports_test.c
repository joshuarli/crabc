...
int main(void)
{
    static const char path[] = "/tmp/crabc-c-abi-xattr-test";
    static const char path_name[] = "user.crabc_xattr_path";
    static const char link_name[] = "user.crabc_xattr_link";
    static const char fd_name[] = "user.crabc_xattr_fd";
    ...
    if (setxattr(path, path_name, path_value, sizeof(path_value) - 1,
        ...
        if (xattr_unavailable()) {
            printf("c-abi xattr unavailable errno=%d\n", errno);
            close(fd);
            unlink(path);
            return 77;
        }
    ...
    puts("c-abi extended attributes exports ok");
    return 0;
}
