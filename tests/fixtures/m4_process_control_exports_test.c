...
static int waitid_case(void)
{
    ...
    CHECK(waitid(P_PID, (id_t)child, &info, WEXITED | WNOWAIT) == 0,
          "waitid");
    /* siginfo_t member offsets are verified as part of the M5 ABI milestone.
     * This M4 runtime case proves the native waitid/ WNOWAIT state transition
...
int main(void)
{
    ...
    puts("m4 process control exports ok");
    return 0;
}
