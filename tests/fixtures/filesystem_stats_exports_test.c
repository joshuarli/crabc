...
int main(void)
{
    char file_name[] = "/tmp/crabc-c-abi-stat-XXXXXX";
    ...
    errno = 0;
    CHECK(statfs("/tmp/crabc-c-abi-stat-missing", &path_fs) == -1 &&
              errno == ENOENT,
          "statfs errno");
    ...
    /* `struct stat` layout verification belongs to . This fixture
    ...
    result = 0;
    puts("c-abi filesystem stats exports ok");
