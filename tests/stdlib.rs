...
fn stdlib_functions_under_libc_so() {
    ...
    let bin = test_support::TempArtifact::new("stdlib_test");
    let status = Command::new(test_support::crabc_cc())
        ...
        .status()
        .expect("failed to run crabc-cc for stdlib_test");
    assert!(status.success(), "crabc-cc stdlib_test compilation failed");
