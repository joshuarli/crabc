...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc owner-exit fixture");
    assert!(status.success(), "native mimalloc owner-exit fixture compilation failed");
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc owner-exit fixture")
}
...
fn native_mimalloc_owner_exit_matches_pinned_musl() {
    ...
    assert_eq!(reference_output.stdout, b"native mimalloc owner exit ok\n");
    assert_eq!(
