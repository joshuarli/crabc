...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let fixture = root.join("tests/fixtures/stdio_format_parse_regression_test.c");
...
fn run(binary: &std::path::Path, path: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the stdio format/parse regression fixture")
}

#[test]
fn stdio_format_parse_round_trip_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new("stdio-format-parse-reference");
    let candidate = test_support::TempArtifact::new("stdio-format-parse-candidate");
    let reference_file = test_support::TempArtifact::new("stdio-format-parse-reference-file");
    let candidate_file = test_support::TempArtifact::new("stdio-format-parse-candidate-file");
    ...
    assert_eq!(reference_output.stdout, b"stdio format/parse contract ok\n");
    assert_eq!(
