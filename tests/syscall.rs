...
#[test]
fn syscall_sign_extend_under_libldso() {
    ...
    let status = Command::new("musl-gcc")
        ...
        .status()
        .expect("failed to run musl-gcc for syscall_test");
    assert!(status.success(), "musl-gcc syscall_test compilation failed");
    ...
    let output = Command::new(&bin)
        ...
        .output()
        .expect("failed to run syscall_test");

    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        output.status.success(),
        "syscall_test exited with {:?}, stderr: {}",
        output.status.code(),
        stderr
    );
