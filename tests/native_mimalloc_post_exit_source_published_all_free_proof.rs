...
fn compile_fixture(binary: &std::path::Path, candidate: bool) {
    ...
    let status = command
        .status()
        .expect("failed to compile the native mimalloc post-exit source-published all-free proof fixture");
    assert!(
        status.success(),
        "native mimalloc post-exit source-published all-free proof fixture compilation failed"
    );
}
...
fn run(binary: &std::path::Path, candidate: bool) -> Output {
    ...
    command
        .output()
        .expect("failed to run the native mimalloc post-exit source-published all-free proof fixture")
}

#[test]
fn native_mimalloc_post_exit_source_published_all_free_proof_matches_pinned_musl() {
    let reference = test_support::TempArtifact::new(
        "native-mimalloc-post-exit-source-published-all-free-proof-reference",
    );
    let candidate = test_support::TempArtifact::new(
        "native-mimalloc-post-exit-source-published-all-free-proof-candidate",
    );
    ...
    assert_eq!(
        ...
        b"native mimalloc post exit source published all free proof ok\n"
    );
    assert_eq!(
