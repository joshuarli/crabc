...
//!
//! This staged module admits `uname(2)` and `sysinfo(2)` values with fixed
//! x86-64 kernel layouts, plus a separately evidenced inotify descriptor and
//! event-record boundary. Other system/runtime facilities remain outside the
//! x86 facade until they have independent evidence.
...
pub mod inotify {
    ...
    mod tests {
        ...
        #[test]
        fn event_batch_retains_unknown_bits_and_descriptor_wide_records() {
            let mut bytes = [0u8; EVENT_HEADER_SIZE];
            bytes[0..4].copy_from_slice(&(-1_i32).to_ne_bytes());
            bytes[4..8].copy_from_slice(&(0x0001_0000_u32).to_ne_bytes());
            let mut events = Events {
                bytes: &bytes,
                offset: 0,
                malformed: false,
            ...
            let event = events.next().expect("one event").expect("valid event");
            assert_eq!(event.watch(), None);
            assert_eq!(event.mask().bits(), 0x0001_0000);
