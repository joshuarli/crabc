...
impl ScopedIpAddress {
    fn new(family: u8, bytes: &[u8], index: u32) -> Result<Self> {
        let address = match family {
            AF_INET if bytes.len() == 4 => crate::net::IpAddress::V4([
...
mod tests {
    ...
    fn ip_parser_uses_local_as_primary_regardless_of_attribute_order() {
        ...
        assert_eq!(ip.address().address(), crate::net::IpAddress::V4([10, 0, 0, 1]));
        assert_eq!(ip.destination().expect("P2P destination").address(), crate::net::IpAddress::V4([10, 0, 0, 2]));
...
mod tests {
    use super::{
        align4, parse_netlink_packet, write_u16, write_u32, IFA_LABEL, IFLA_IFNAME,
        InterfaceNameIndex, NetlinkDump, NLMSG_HEADER_LEN, RTM_NEWADDR, RTM_NEWLINK,
    };
