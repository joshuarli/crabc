//! Link-free no-std proof for the native `preadv2`/`pwritev2` seam.
//!
...
#[no_mangle]
pub extern "C" fn crabc_rs_preadv2_direct_probe() -> i32 {
    let path = &b"/tmp/crabc-rs-native-preadv2-probe"[..];
    ...
    if io::pwritev2(&file, &current_writes, u64::MAX, ReadWriteFlags::empty()).ok() != Some(4)
    ...
    if io::pwritev2(&file, &high_writes, high_offset, ReadWriteFlags::empty()).ok() != Some(4) {
        return 4;
    }
    ...
    if io::preadv2(&file, &mut reads, high_offset, ReadWriteFlags::empty()).ok() != Some(4)
        || first != *b"hi"
        || second != *b"GH"
    {
        return 5;
