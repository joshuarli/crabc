//! Runtime proof for 's owned native buffered memory-stream boundary.
//!
...
#[no_mangle]
pub extern "C" fn crabc_rs_cfile_direct_probe(buffer: *mut u8, length: usize) -> i32 {
    ...
    for iteration in 0..1024 {
        ...
        let mut read = [0; 2];
        if stream.read(&mut read) != Ok(2) || read != *b"cf" {
            return 4;
        }
        let mut eof = [0; 1];
