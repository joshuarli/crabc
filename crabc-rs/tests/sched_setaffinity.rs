...
fn sched_setaffinity_reapplies_observed_current_mask() {
    let mask = thread::sched_getaffinity(None)
        .expect("read the calling task's CPU-affinity mask");
    assert!(!mask.is_empty());

    thread::sched_setaffinity(None, &mask)
        .expect("reapply the calling task's CPU-affinity mask");

    let observed = thread::sched_getaffinity(None)
        .expect("read the re-applied CPU-affinity mask");
...
fn sched_setaffinity_preserves_missing_task_errors() {
    let mask = thread::sched_getaffinity(None)
        .expect("read a valid mask for the syscall argument");
