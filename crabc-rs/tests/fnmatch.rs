...
fn native_matcher_handles_wildcards_classes_and_non_utf8_names() {
    assert!(fnmatch(cstr(b"*.rs\0"), cstr(b"lib.rs\0"), FnmatchFlags::empty()));
    assert!(fnmatch(cstr(b"[[:alpha:]]*[[:digit:]]\0"), cstr(b"crate7\0"), FnmatchFlags::empty()));
    assert!(!fnmatch(cstr(b"[[:digit:]]\0"), cstr(b"x\0"), FnmatchFlags::empty()));
    assert!(!fnmatch(cstr(b"[A]\0"), cstr(b"a\0"), FnmatchFlags::empty()));
    assert!(fnmatch(cstr(b"[A]\0"), cstr(b"a\0"), FnmatchFlags::CASEFOLD));
...
fn native_matcher_preserves_path_period_escape_and_casefold_flags() {
    ...
    assert!(fnmatch(cstr(b"*.rs\0"), cstr(b"src/lib.rs\0"), FnmatchFlags::empty()));

    assert!(!fnmatch(cstr(b"*.rs\0"), cstr(b".lib.rs\0"), FnmatchFlags::PERIOD));
    assert!(fnmatch(cstr(b".*\0"), cstr(b".lib.rs\0"), FnmatchFlags::PERIOD));
    assert!(fnmatch(cstr(b"\\*.rs\0"), cstr(b"*.rs\0"), FnmatchFlags::empty()));
    assert!(!fnmatch(
        cstr(b"\\*.rs\0"),
        cstr(b"*.rs\0"),
        FnmatchFlags::NOESCAPE,
    ));
    assert!(fnmatch(cstr(b"*.RS\0"), cstr(b"lib.rs\0"), FnmatchFlags::CASEFOLD));
}
