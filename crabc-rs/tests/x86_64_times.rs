...
#[test]
fn x86_64_times_reads_validated_tick_accounting_without_a_clock_rate() {
    let first = process::times().expect("Linux times syscall");
    assert_structurally_sound(first);

    // Work may consume zero or more accounting ticks, so only monotonicity is
    // a stable observation. This also keeps the syscall result independent of
    // any assumed `CLK_TCK` value.
