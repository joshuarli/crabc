...
fn fixture_root() -> PathBuf {
    let root = PathBuf::from(format!("/tmp/crabc-x86-glob-{}", std::process::id()));
...
#[test]
fn x86_glob_returns_sorted_owned_paths_and_preserves_non_utf8_names() {
...
#[test]
fn x86_glob_has_explicit_root_and_no_match_policy() {
    let root = fixture_root();
    let root_bytes = root.as_os_str().as_bytes();
    ...
    assert!(glob(root_bytes, b"missing/*.txt")
        ...
        .is_empty());
    assert_eq!(
        glob(&b"/tmp/crabc-x86-glob-no-such-root"[..], b"*"),
        Err(Errno::NOENT)
    );
