...
fn named_tempfile_is_exclusive_private_cloexec_and_drop_unlinks() {
    let path = {
        let file = native_fs::create_temp_file("/tmp", "crabc-rs-m10-named-")
            .expect("create named temporary file");
        assert!(file.name().starts_with(b"crabc-rs-m10-named-"));
...
fn named_tempfile_parent_descriptor_survives_cwd_changes_and_remove_is_explicit() {
    ...
    let file = native_fs::create_temp_file_at(&parent, "crabc-rs-m10-at-named-")
...
fn named_tempfile_persist_transfers_fd_without_unlinking() {
    let file = native_fs::create_temp_file("/tmp", "crabc-rs-m10-persist-")
...
fn named_tempfile_rejects_ambient_cwd_token_and_invalid_prefixes() {
    assert!(matches!(
        native_fs::create_temp_file_at(native_fs::CWD, "crabc-rs-m10-cwd-"),
        Err(Errno::BADF)
    ));
    assert!(matches!(
