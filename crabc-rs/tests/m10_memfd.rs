...
fn memfd_owns_a_cloexec_descriptor_and_preserves_content() {
    ...
    assert_eq!(io::write(&file, b"m10-memfd").expect("write memfd content"), 9);
    assert_eq!(
        fs::seek(&file, SeekFrom::Start(0)).expect("rewind memfd"),
        0,
    );
    let mut content = [0_u8; 9];
    assert_eq!(io::read(&file, &mut content).expect("read memfd content"), 9);
    assert_eq!(&content, b"m10-memfd");
}
