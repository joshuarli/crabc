...
fn with_scratch_directory<T>(run: impl FnOnce(&str) -> Result<T>) -> T {
    ...
    match fs::rmdir(&root) {
        Ok(()) | Err(Errno::NOENT) => {}
        Err(error) => panic!("remove stale M2 directory: {error}"),
    }
    fs::mkdir(&root, Mode::RWXU).expect("create M2 directory through the direct kernel seam");
    let result = run(&root).expect("M2 filesystem operation");
    fs::rmdir(&root).expect("remove empty M2 directory through the direct kernel seam");
    result
}
...
fn statat_fstat_and_unlinkat_share_direct_metadata_contract() {
    with_scratch_directory(|root| {
        ...
        crabc_rs::io::write(&file, b"m2").expect("write test metadata payload");
...
fn unlinkat_removedir_only_removes_directories() {
    with_scratch_directory(|root| {
        ...
        fs::mkdirat(&directory, "child", Mode::RWXU)
            .expect("M2 directory creation will be added before this test is enabled");
...
fn links_renames_and_bounded_readlink_use_the_direct_path_seam() {
    with_scratch_directory(|root| {
        ...
        for name in ["record", "renamed", "symbolic"] {
            fs::unlinkat(&directory, name, AtFlags::empty()).expect("remove M2 link fixture");
        }
        drop(directory);
        Ok(())
    });
...
fn permissions_and_timestamps_match_linux_rustix_contracts() {
    with_scratch_directory(|root| {
        ...
        for name in ["record", "symbolic"] {
            fs::unlinkat(&directory, name, AtFlags::empty()).expect("remove M2 permission fixture");
        }
        drop(directory);
        Ok(())
    });
...
fn raw_dir_preserves_record_lifetimes_alignment_and_long_names() {
    with_scratch_directory(|root| {
        ...
        let small_directory = fs::openat(CWD, root, OFlags::RDONLY | OFlags::DIRECTORY, Mode::empty())
            .expect("open second M2 directory descriptor");
