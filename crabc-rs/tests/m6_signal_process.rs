...
unsafe extern "C" fn atfork_a_prepare() { atfork_push(b'A'); }
unsafe extern "C" fn atfork_a_parent() { atfork_push(b'a'); }
unsafe extern "C" fn atfork_a_child() { atfork_push(b'a'); }
unsafe extern "C" fn atfork_b_prepare() { atfork_push(b'B'); }
unsafe extern "C" fn atfork_b_parent() { atfork_push(b'b'); }
unsafe extern "C" fn atfork_b_child() { atfork_push(b'b'); }
unsafe extern "C" fn atfork_c_prepare() { atfork_push(b'C'); }
unsafe extern "C" fn atfork_c_parent() { atfork_push(b'c'); }
unsafe extern "C" fn atfork_c_child() { atfork_push(b'c'); }
...
#[test]
fn m6_signal_process() {
    if let Ok(case) = std::env::var("CRABC_RS_M6_CASE") {
        match case.as_str() {
            "signals" => signal_case(),
            ...
            "spawn" => spawn_case(),
            "waitid" => waitid_case(),
            other => panic!("unknown M6 child case {other}"),
        }
        return;
    }
    ...
    for case in [
        ...
        let output = Command::new(std::env::current_exe().expect("locate test binary"))
            .args(["--exact", "m6_signal_process", "--nocapture"])
            .env("CRABC_RS_M6_CASE", case)
            .output()
            .expect("run isolated M6 child case");
        assert!(
            output.status.success(),
            "M6 {case} subprocess failed with {:?}, stdout: {}, stderr: {}",
            output.status.code(),
...
fn signal_case() {
    ...
    assert!(!signal::SignalSet::full().contains(unsafe {
        process::Signal::from_raw_unchecked(32)
    }));
    assert!(!signal::SignalSet::full().contains(unsafe {
        process::Signal::from_raw_unchecked(33)
    }));
    assert!(!signal::SignalSet::full().contains(unsafe {
        process::Signal::from_raw_unchecked(34)
    }));
    ...
    assert_eq!(process::Signal::from_named_raw(35), Some(process::Signal::RTMIN));
    ...
    let child = match unsafe { process::fork_raw() }.expect("fork isolated signal waiter") {
        ...
        process::ForkResult::Child => {
            ...
            assert!(signal::pending().expect("read initial pending set").is_empty());
            ...
            assert_eq!(info.sender_pid(), Some(process::getppid().expect("visible parent")));
    ...
    assert_eq!(io::read(&ready_reader, &mut ready[..]).expect("wait for signal child"), 1);
    ...
    assert_eq!(io::read(&result_reader, &mut queued[..]).expect("read queued value"), 4);
    ...
    assert_eq!(ATFORK_LOG.load(Ordering::SeqCst).to_le_bytes(), *b"handler!");
    ...
    assert!(info_action.flags().contains(signal::SigActionFlags::SIGINFO));
    ...
    assert_eq!(
        signal::timed_wait(&realtime_set, Some(&crabc_rs::time::Timespec::default()))
            .unwrap_err(),
        Errno::AGAIN,
    );
}
...
fn signal_fd_case() {
    ...
    let mut ready = [0_u8; 1];
    assert_eq!(io::read(&ready_reader, &mut ready[..]).expect("wait for signalfd child"), 1);
    signal::queue_process(child, process::Signal::USR1, 0x4d3c_2b1a)
        .expect("queue signal for signalfd child");
    let mut queued = [0_u8; 4];
    assert_eq!(io::read(&result_reader, &mut queued[..]).expect("read signalfd value"), 4);
...
fn realtime_case() {
    ...
    let mut ready = [0_u8; 1];
    assert_eq!(io::read(&ready_reader, &mut ready[..]).expect("wait for realtime child"), 1);
    signal::queue_process(child, realtime, 0x1234_5678).expect("queue realtime child signal");
    let mut queued = [0_u8; 4];
    assert_eq!(io::read(&result_reader, &mut queued[..]).expect("read realtime value"), 4);
...
fn process_control_case() {
    let child = match unsafe { process::fork_raw() }.expect("fork process-control child") {
        ...
        process::ForkResult::Child => {
            ...
            assert_eq!(process::getpgid(None).expect("read initial process group"), process::getpgrp());
            assert!(process::getsid(None).expect("read initial session").as_raw_pid() > 0);
            assert_eq!(process::setsid().expect("create isolated session"), pid);
            assert_eq!(process::getpgid(None).expect("read session process group"), pid);
...
fn fork_wait_case() {
    ...
    assert_eq!(process::wait(process::WaitOptions::NOHANG).unwrap_err(), Errno::CHILD);
}
...
fn atfork_case() {
    ...
    unsafe {
        process::register_atfork(Some(atfork_a_prepare), Some(atfork_a_parent), Some(atfork_a_child))
            .expect("register first atfork callback");
        process::register_atfork(Some(atfork_b_prepare), Some(atfork_b_parent), Some(atfork_b_child))
            .expect("register second atfork callback");
        process::register_atfork(Some(atfork_c_prepare), Some(atfork_c_parent), Some(atfork_c_child))
...
fn spawn_case() {
    ...
    let command = cstr(b"printf m6-spawn\0");
    let actions = [
    ...
    let child = prepared.spawn().expect("exec shell through prepared error-pipe spawn");
    drop(prepared);
    drop(writer);
    let mut output = [0_u8; 8];
    let read = io::read(&reader, &mut output[..]).expect("read prepared child stdout");
    assert_eq!(&output[..read], b"m6-spawn");
    assert_eq!(child.wait(process::WaitOptions::empty()).unwrap().unwrap().exit_status(), Some(0));

    let missing = process::PreparedExec::new(
        cstr(b"/crabc-rs-m6-definitely-missing\0"),
        &[cstr(b"missing\0")],
        &[],
    )
    ...
    let collision_missing = process::PreparedExec::new(
        cstr(b"/crabc-rs-m6-collision-missing\0"),
        &[cstr(b"missing\0")],
        &[],
    )
