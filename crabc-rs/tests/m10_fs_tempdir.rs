...
fn creates_private_unique_directories_with_random_suffixes() {
    let first = native_fs::create_temp_dir("/tmp", "crabc-rs-m10-temp-")
        .expect("create first temporary directory");
    let second = native_fs::create_temp_dir("/tmp", "crabc-rs-m10-temp-")
...
fn caller_owned_and_descriptor_relative_forms_preserve_byte_prefixes() {
    ...
    let length = native_fs::create_temp_dir_at_into(&parent, "crabc-rs-m10-at-", &mut basename)
        .expect("create descriptor-relative temporary directory");
    assert!(basename[..length].starts_with(b"crabc-rs-m10-at-"));
