...
def dynamic_header_probe(
    ...
    record["header_dependency_audit"] = dynamic_header_dependency_audit(
        dependencies, sysroot, probe
    )
    return record


def make_dependency_inputs(text: str) -> list[str]:
    r"""Parse one GCC ``-MD`` rule without interpreting it as shell input.

    GCC writes Make syntax: a rule separator is the first unescaped colon, and
    physical path characters use Make's ``\ ``, ``\#``, ``\:``, ``\\``, and
    ``$$`` spellings.  The source-consumer gate accepts only that bounded form
    so malformed dependency output cannot be normalized into another path.
    """
    ...
    while index < len(rule):
        ...
        if character == ":":
            ...
            break
        index += 1
    if separator is None or not rule[:separator].strip():
    ...
    while index < len(rule):
        ...
        if character == "\\":
            ...
            continue
        if character == "$":
            if index + 1 == len(rule) or rule[index + 1] != "$":
                raise LUA.RunnerError("sealed dynamic header dependency file has an unescaped dollar")
            current.append("$")
            index += 2
            continue
        # GCC can retain a physical colon in an input path. Only the first
        # unescaped colon terminates the Make target; later colons are input
        # characters unless they use the equivalent ``\:`` spelling above.
        if character == ":":
            current.append(character)
            index += 1
            continue
        if character == "#":
            raise LUA.RunnerError("sealed dynamic header dependency file has an unescaped dependency hash")
        current.append(character)
        index += 1
    finish_input()
    if not inputs:
...
def dynamic_header_dependency_audit(
    dependencies: Path, sysroot: Path, source: Path
) -> dict[str, object]:
    ...
    path_texts = make_dependency_inputs(text)
    paths: list[Path] = []
