#!/usr/bin/env python3
"""Verify that the legacy-numeric probe stays off the C callback ABI."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libnumeric_legacy_direct_probe.a"
    require(archive.is_file(), f"legacy-numeric probe archive does not exist: {archive}")
    return archive
...
def inspect(readelf: str, undefined_symbols: str, defined_symbols: str) -> dict[str, object]:
    ...
    require(
        "crabc_rs_numeric_legacy_direct_probe" in defined_symbols,
        "fixture does not define the legacy-numeric probe entry point",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"native legacy-numeric proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"native legacy-numeric proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
