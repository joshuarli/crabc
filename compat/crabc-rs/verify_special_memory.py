#!/usr/bin/env python3
"""Verify the special-memory probe stays off the C memory ABI."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libmemory_special_direct_probe.a"
    require(archive.is_file(), f"special-memory probe archive does not exist: {archive}")
    return archive
...
def inspect(readelf: str, undefined_symbols: str, defined_symbols: str) -> dict[str, object]:
    ...
    require(
        "crabc_rs_memory_special_direct_probe" in defined_symbols,
        "fixture does not define the special-memory probe entry point",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"native special-memory proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"native special-memory proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
