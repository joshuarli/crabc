#!/usr/bin/env python3
"""Verify the bounded M11 loader probe's private-runtime boundary."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libm11_loader_dlfcn_basic_probe.a"
    require(archive.is_file(), f"M11 loader probe archive does not exist: {archive}")
    return archive
...
def inspect(readelf: str, undefined_symbols: str) -> dict[str, object]:
    ...
    require(
        ...
        "fixture does not reference the versioned private loader runtime",
    )
    forbidden = tuple(
    ...
    require(
        not forbidden,
        "fixture references forbidden public dlfcn/errno symbol(s): " + ", ".join(forbidden),
    )
    return {
        "machine": "AArch64",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"M11 private loader runtime proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"M11 private loader runtime proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
