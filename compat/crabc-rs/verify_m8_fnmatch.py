#!/usr/bin/env python3
"""Verify that the M8 fnmatch probe is native and has no C ABI dependency."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libm8_fnmatch_direct_probe.a"
    require(archive.is_file(), f"M8 fnmatch probe archive does not exist: {archive}")
    return archive
...
def inspect(readelf: str, undefined_symbols: str, defined_symbols: str) -> dict[str, object]:
    ...
    require(
        "crabc_rs_m8_fnmatch_direct_probe" in defined_symbols,
        "fixture does not define the M8 fnmatch probe entry point",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"M8 native fnmatch proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"M8 native fnmatch proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
