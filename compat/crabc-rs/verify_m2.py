#!/usr/bin/env python3
"""Verify that the M2 filesystem probe stays off crabc's public C ABI."""
...
class VerificationError(ValueError):
    """The fixture does not demonstrate the M2 direct-operation contract."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libm2_direct_probe.a"
    require(archive.is_file(), f"M2 direct probe archive does not exist: {archive}")
    return archive
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"M2 direct syscall proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"M2 direct syscall proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
