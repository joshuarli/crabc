#!/usr/bin/env python3
"""Verify native CFile code reaches libc only through the private table."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libcfile_direct_probe.a"
    require(archive.is_file(), f"CFile probe archive does not exist: {archive}")
    return archive
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"private CFile runtime proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"private CFile runtime proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
