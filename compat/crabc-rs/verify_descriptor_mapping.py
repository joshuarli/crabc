#!/usr/bin/env python3
"""Verify that the direct probe stays off crabc's C ABI."""
...
class VerificationError(ValueError):
    """The fixture does not demonstrate the direct-operation contract."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libdescriptor_mapping_probe.a"
    require(archive.is_file(), f"direct probe archive does not exist: {archive}")
    return archive
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"direct syscall proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"direct syscall proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
