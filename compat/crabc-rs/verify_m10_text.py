#!/usr/bin/env python3
"""Verify that the M10 native text probe is AArch64 and off the C iconv ABI."""
...
class VerificationError(ValueError):
    """The fixture does not demonstrate the direct-native text contract."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libm10_text_direct_probe.a"
    require(archive.is_file(), f"M10 text probe archive does not exist: {archive}")
    return archive
...
def inspect(readelf: str, undefined_symbols: str, defined_symbols: str) -> dict[str, object]:
    ...
    require(
        "crabc_rs_m10_text_direct_probe" in defined_symbols,
        "fixture does not define the M10 text probe entry point",
    )
    forbidden = tuple(name for name in FORBIDDEN_SYMBOLS if has_symbol(undefined_symbols, name))
    require(
        not forbidden,
        "fixture references forbidden public C ABI/libc/errno symbol(s): " + ", ".join(forbidden),
    )
    return {
        "machine": "AArch64",
        "direct_native": True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"M10 native text proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"M10 native text proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
