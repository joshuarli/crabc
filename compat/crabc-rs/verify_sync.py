#!/usr/bin/env python3
"""Verify the native synchronization probe uses only direct futex syscalls."""
...
class VerificationError(ValueError):
    """The probe does not demonstrate the direct-native contract."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libsynchronization_probe.a"
    require(archive.is_file(), f"synchronization probe archive does not exist: {archive}")
    return archive
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"direct futex proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"direct futex proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
