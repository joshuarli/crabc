#!/usr/bin/env python3
"""Verify that M10 network-value and Ethernet codecs stay off the C ABI."""
...
class Probe:
    """One pure-native M10 network value probe."""
...
PROBES = {
    "network-address": Probe(
        "libm10_network_address_direct_probe.a",
        "crabc_rs_m10_network_address_direct_probe",
    ),
    "ipaddr": Probe(
        "libm10_network_ipaddr_direct_probe.a",
        "crabc_rs_m10_network_ipaddr_direct_probe",
    ),
    "ipv4-legacy": Probe(
        "libm10_ipv4_legacy_direct_probe.a",
        "crabc_rs_m10_ipv4_legacy_direct_probe",
    ),
    "ipv4-classful": Probe(
        "libm10_ipv4_classful_direct_probe.a",
        "crabc_rs_m10_ipv4_classful_direct_probe",
    ),
    "ethernet-address": Probe(
        "libm10_ethernet_address_direct_probe.a",
        "crabc_rs_m10_ethernet_address_direct_probe",
    ),
    "ethers": Probe(
        "libm10_ethers_direct_probe.a",
        "crabc_rs_m10_ethers_direct_probe",
    ),
}
...
def inspect(probe: Probe, readelf: str, undefined_symbols: str, defined_symbols: str) -> dict[str, object]:
    ...
    require(
        ...
        "fixture does not define the required M10 network probe entry point",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"M10 native {args.probe} proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"M10 native {args.probe} proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
