#!/usr/bin/env python3
"""Verify M8 native CFile code reaches libc only through the private table."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libm8_cfile_direct_probe.a"
    require(archive.is_file(), f"M8 CFile probe archive does not exist: {archive}")
    return archive
...
def inspect(readelf: str, undefined_symbols: str) -> dict[str, object]:
    ...
    require(
        ...
        "fixture does not reference the versioned private CFile runtime",
    )
    forbidden = tuple(
    ...
    require(
        not forbidden,
        "fixture references forbidden public stdio/TLS errno symbol(s): " + ", ".join(forbidden),
    )
    return {
        "machine": "AArch64",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"M8 private CFile runtime proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"M8 private CFile runtime proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
