#!/usr/bin/env python3
"""Verify that the M10 native ctype probe is AArch64 and off the C ABI."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libm10_ctype_direct_probe.a"
    require(archive.is_file(), f"M10 ctype probe archive does not exist: {archive}")
    return archive
...
def inspect(readelf: str, undefined_symbols: str, defined_symbols: str) -> dict[str, object]:
    ...
    require(
        "crabc_rs_m10_ctype_direct_probe" in defined_symbols,
        "fixture does not define the M10 ctype probe entry point",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"M10 native ctype proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"M10 native ctype proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
