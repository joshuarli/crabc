#!/usr/bin/env python3
"""Verify the M10 stateful-text probe remains off the public C text ABI."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libm10_text_stateful_direct_probe.a"
    require(archive.is_file(), f"M10 stateful-text probe archive does not exist: {archive}")
    return archive
...
def inspect(readelf: str, undefined_symbols: str, defined_symbols: str) -> dict[str, object]:
    ...
    require(
        "crabc_rs_m10_text_stateful_direct_probe" in defined_symbols,
        "fixture does not define the M10 stateful-text probe entry point",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"M10 native stateful-text proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"M10 native stateful-text proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
