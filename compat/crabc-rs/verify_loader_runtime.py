#!/usr/bin/env python3
"""Verify native loader code uses only the private singleton table."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libloader_runtime_probe.a"
    require(archive.is_file(), f"loader runtime probe archive does not exist: {archive}")
    return archive
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"private loader runtime proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"private loader runtime proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
