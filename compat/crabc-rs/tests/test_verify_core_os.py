...
SCRIPT = ROOT / "compat/crabc-rs/verify_core_os.py"
SPEC = importlib.util.spec_from_file_location("verify_core_os", SCRIPT)
...
class InspectionTests(unittest.TestCase):
    ...
    def test_rejects_public_abi_errno_or_missing_core_os_syscall(self) -> None:
        with self.assertRaises(checker.VerificationError):
            checker.inspect("Machine: AArch64\n", direct_syscalls() + "\n  80: bl <mmap>")
        incomplete = "  0: mov w8, #0x13\n  4: svc #0\n"
        with self.assertRaises(checker.VerificationError):
            checker.inspect("Machine: AArch64\n", incomplete)
