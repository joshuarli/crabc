"""Focused semantic-proof tests for the legacy-numeric verifier."""
...
SPEC = importlib.util.spec_from_file_location(
    "verify_numeric_legacy",
    ROOT / "compat" / "crabc-rs" / "verify_numeric_legacy.py",
)
...
class Verify0NumericLegacyTests(unittest.TestCase):
    def test_accepts_aarch64_native_entrypoint_without_forbidden_symbols(self) -> None:
        report = VERIFY.inspect(
            "Machine: AArch64",
            "",
            "0000000000000000 T crabc_rs_numeric_legacy_direct_probe\n",
        )
    ...
    def test_rejects_non_aarch64_or_missing_native_entrypoint(self) -> None:
        with self.assertRaisesRegex(VERIFY.VerificationError, "AArch64"):
            VERIFY.inspect("Machine: x86-64", "", "entry crabc_rs_numeric_legacy_direct_probe")
