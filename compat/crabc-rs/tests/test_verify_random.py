"""Focused semantic-proof tests for the random-state verifier."""
...
SPEC = importlib.util.spec_from_file_location(
    "verify_random", ROOT / "compat" / "crabc-rs" / "verify_random.py"
)
...
class Verify0RandomTests(unittest.TestCase):
    def test_accepts_direct_aarch64_entropy_without_c_symbols(self) -> None:
        report = VERIFY.inspect(
            "Machine: AArch64",
            "mov w8, #0x116\nsvc #0\n",
            "0000000000000000 T crabc_rs_random_direct_probe\n",
        )
    ...
    def test_rejects_missing_architecture_entrypoint_or_syscall(self) -> None:
        ...
        with self.assertRaisesRegex(VERIFY.VerificationError, "getrandom syscall"):
            VERIFY.inspect("Machine: AArch64", "svc #0", "entry crabc_rs_random_direct_probe")
            ...
            with self.subTest(symbol=symbol):
                with self.assertRaisesRegex(VERIFY.VerificationError, "forbidden"):
                    VERIFY.inspect(
                        "Machine: AArch64",
                        f"mov w8, #0x116\nsvc #0\nbl <{symbol}>\n",
                        "entry crabc_rs_random_direct_probe",
                    )
