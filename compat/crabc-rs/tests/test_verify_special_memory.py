"""Focused semantic-proof tests for the special-memory verifier."""
...
SPEC = importlib.util.spec_from_file_location(
    "verify_special_memory",
    ROOT / "compat" / "crabc-rs" / "verify_special_memory.py",
)
...
class Verify0MemorySpecialTests(unittest.TestCase):
    def test_accepts_aarch64_native_entrypoint_without_forbidden_symbols(self) -> None:
        report = VERIFY.inspect(
            "Machine: AArch64",
            "",
            "0000000000000000 T crabc_rs_memory_special_direct_probe\n",
        )
