#!/usr/bin/env python3
"""Verify that the fenv probe uses direct AArch64 FPCR/FPSR access."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libfenv_direct_probe.a"
    require(archive.is_file(), f"fenv probe archive does not exist: {archive}")
    return archive
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"direct fenv proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"direct fenv proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
