#!/usr/bin/env python3
"""Verify native resolver code uses direct Linux DNS syscalls."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libresolver_direct_probe.a"
    require(archive.is_file(), f"resolver probe archive does not exist: {archive}")
    return archive
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"direct resolver proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"direct resolver proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
