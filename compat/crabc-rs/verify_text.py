#!/usr/bin/env python3
"""Verify that the native text probe is AArch64 and off the C iconv ABI."""
...
def fixture_archive(target_dir: Path) -> Path:
    archive = target_dir / "release" / "examples" / "libtext_direct_probe.a"
    require(archive.is_file(), f"text probe archive does not exist: {archive}")
    return archive
...
def inspect(readelf: str, undefined_symbols: str, defined_symbols: str) -> dict[str, object]:
    ...
    require(
        "crabc_rs_text_direct_probe" in defined_symbols,
        "fixture does not define the text probe entry point",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"native text proof: PASS ({archive}) {report}")
    except VerificationError as error:
        print(f"native text proof: ERROR: {error}", file=sys.stderr)
        return 2
    return 0
