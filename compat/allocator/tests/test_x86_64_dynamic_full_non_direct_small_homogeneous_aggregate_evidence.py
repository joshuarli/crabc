#!/usr/bin/env python3
"""Contract tests for the private homogeneous full-non-direct-small aggregate lane."""
...
SCRIPT = ROOT / "compat/allocator/x86_64_dynamic_full_non_direct_small_homogeneous_aggregate_evidence.py"
spec = importlib.util.spec_from_file_location("dynamic_full_non_direct_small_homogeneous_aggregate_evidence", SCRIPT)
...
class SchemaTests(unittest.TestCase):
    def test_schema_binds_two_full_non_direct_small_pages_and_active_tls_map(self):
        schema = evidence.load_schema()
        self.assertEqual(schema, evidence._schema_template())
        values = schema["trace"]["expected_values"]
        prefix = "trace.dynamic_full_non_direct_small_homogeneous_aggregate."
        self.assertEqual(len(values), 69)
        self.assertEqual(values[f"{prefix}page_count"], 2)
        self.assertEqual(values[f"{prefix}ordinary_queue_count_before_thread_done"], 2)
        self.assertEqual(values[f"{prefix}small_page"], 1)
        self.assertEqual(values[f"{prefix}non_direct_small"], 1)
        ...
        self.assertEqual(values[f"{prefix}page0.page_map_slice_count_after_thread_done"], 1)
        self.assertEqual(values[f"{prefix}page1.page_map_slice_count_after_thread_done"], 1)
        self.assertEqual(values[f"{prefix}page0.slice_count_after_thread_done"], 1)
        self.assertEqual(values[f"{prefix}page1.slice_count_after_thread_done"], 1)
        self.assertEqual(values[f"{prefix}page0.used_after_unmapped_prefix"], 45)
        self.assertEqual(values[f"{prefix}page1.used_after_unmapped_prefix"], 45)
        self.assertEqual(values[f"{prefix}page0.used_after_reabandon_boundary"], 44)
        self.assertEqual(values[f"{prefix}page1.used_after_reabandon_boundary"], 44)
        self.assertEqual(values[f"{prefix}page1.page_map_slice_count_after_first_terminal"], 1)
        self.assertEqual(values[f"{prefix}page1.page_map_all_slices_registered_after_first_terminal"], 1)
        self.assertEqual(values[f"{prefix}page0.page_map_all_slices_registered_after_thread_done"], 1)
        self.assertEqual(values[f"{prefix}page1.page_map_all_slices_registered_after_thread_done"], 1)
        self.assertEqual(values[f"{prefix}page1.used_after_first_terminal"], 51)
    ...
    def test_schema_rejects_hash_scope_trace_and_lifecycle_drift(self):
        mutations = (
            ...
            lambda value: value["trace"]["expected_values"].update(
                {"trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.used_after_reabandon_boundary": 37}
            ),
...
class TraceAndCommandTests(unittest.TestCase):
    def test_report_construction_and_validation_are_repeatable_without_base_mutation(self):
        schema = evidence.load_schema()
        temporary = Path("/tmp/dynamic-full-non-direct-small-homogeneous-aggregate-pure-report")
    ...
    def test_report_validation_rejects_structurally_incomplete_report(self):
        schema = evidence.load_schema()
        temporary = Path("/tmp/dynamic-full-non-direct-small-homogeneous-aggregate-report")
        source = temporary / "source/mimalloc-3.5.0"
        stem = "dynamic-full-non-direct-small-homogeneous-aggregate"
        ...
        malformed = {
            ...
            "c_probe": {
                "build_command": build,
                "run_command": ["<temporary-evidence-root>/dynamic-full-non-direct-small-homogeneous-aggregate-c"],
                "source_sha256": evidence.sha256_bytes(evidence.C_TRACE_PROBE.encode()),
            },
        }
    ...
    def test_trace_requires_exact_two_page_shape(self):
        ...
        trace.pop("trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.used_after_thread_done")
        with self.assertRaises(evidence.EvidenceError):
            evidence.validate_trace(trace, description="missing aggregate field")
        trace = copy.deepcopy(schema["trace"]["expected_values"])
        trace["trace.dynamic_full_non_direct_small_homogeneous_aggregate.page0.used_after_reabandon_boundary"] = 37
        with self.assertRaises(evidence.EvidenceError):
    ...
    def test_c_command_requires_native_pthread_tls_contract(self):
        schema = evidence.load_schema()
        temporary = Path("/tmp/dynamic-full-non-direct-small-homogeneous-aggregate-command")
        source = temporary / "source/mimalloc-3.5.0"
        command = evidence.c_trace_command(
            "/usr/bin/musl-gcc",
            source,
            temporary / "dynamic-full-non-direct-small-homogeneous-aggregate.c",
            temporary / "dynamic-full-non-direct-small-homogeneous-aggregate-c",
            schema,
        )
    ...
    def test_probe_declares_two_page_worker_and_no_post_exit_theap_access(self):
        ...
        self.assertIn("pthread_join(producer", probe)
        self.assertIn("PAGE_COUNT 2", probe)
        self.assertIn("fixture->page_blocks[1] < BLOCKS_PER_PAGE", probe)
