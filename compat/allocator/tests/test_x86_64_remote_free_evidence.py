#!/usr/bin/env python3
"""Contract tests for native x86-64 live-owner remote-free evidence."""
...
SCRIPT_PATH = ROOT / "compat/allocator/x86_64_remote_free_evidence.py"
SPEC = importlib.util.spec_from_file_location("crabc_x86_64_remote_free_evidence", SCRIPT_PATH)
...
class SchemaTests(unittest.TestCase):
    ...
    def test_schema_rejects_private_contract_drift(self) -> None:
        mutations = (
            (lambda schema: schema["scope"].update({"public_mi_api_claimed": True}), "private boundary"),
            ...
            (lambda schema: schema["source_anchors"][0].update({"sha256": "0" * 64}), "source anchor contract"),
...
class CommandAndTraceTests(unittest.TestCase):
    def test_c_command_retains_fixed_release_and_pthread_profile(self) -> None:
        schema = EVIDENCE.load_schema()
        temporary = Path("/tmp/live-owner-evidence")
        source = temporary / "source/mimalloc-3.5.0"
        command = EVIDENCE.c_trace_command(
            "/usr/bin/musl-gcc",
            source,
            temporary / "live-owner-remote-free.c",
            temporary / "live-owner-remote-free-c",
            schema,
        )
    ...
    def test_trace_parser_and_comparator_reject_nonlogical_or_drifting_records(self) -> None:
        ...
        raw_address = "\n".join(
            [EVIDENCE.TRACE_BEGIN, "trace.live_owner_remote.pointer=0x1000", EVIDENCE.TRACE_END]
        )
        with self.assertRaisesRegex(EVIDENCE.EvidenceError, "raw address"):
            EVIDENCE.parse_trace(raw_address, description="test trace")

        missing = dict(trace)
        missing.pop("trace.live_owner_remote.valid")
        ...
        different["trace.live_owner_remote.collected_count"] = 1
...
class ReportTests(unittest.TestCase):
    ...
    def test_report_is_private_and_binds_both_probe_identities(self) -> None:
        ...
        self.assertEqual(report["comparison"], {"compared_value_count": 25, "status": "matched"})
    ...
    def test_report_rejects_weakened_c_rust_or_scope_evidence(self) -> None:
        mutations = (
            ...
            (
                lambda report: report["c_probe"].update({"elf": {}}),
                "C ELF identity",
            ),
            (
                lambda report: report["rust_probe"]["cargo_command"].remove("--locked"),
                "Rust command drifted",
            ),
            (
                lambda report: report["source"].update({"archive_sha256": "0" * 64}),
                "archive identity",
            ),
            (
                lambda report: report["scope"].update({"public_mi_api_claimed": True}),
                "source or private boundary",
            ),
            (
            ...
            (
                lambda report: report["rust_probe"].update({"passed_test_count": True}),
                "Rust selection",
            ),
            (
                lambda report: report["c_probe"]["trace"].update(
                    {"trace.live_owner_remote.valid": True}
                ),
                "non-integer values",
            ),
            (
                lambda report: report["c_probe"]["trace"].update(
                    {"trace.live_owner_remote.valid": 1.0}
                ),
                "non-integer values",
            ),
        )
