#!/usr/bin/env python3
"""Pure contracts for the dynamic full direct-small one-remote x86-64 lane."""
...
SCRIPT = ROOT / "compat/allocator/x86_64_dynamic_full_direct_small_one_remote_force_collect_to_mapped_evidence.py"
spec = importlib.util.spec_from_file_location("dynamic_full_direct_small_one_remote_evidence", SCRIPT)
...
class SchemaTests(unittest.TestCase):
    def test_schema_binds_the_fixed_dynamic_full_direct_small_one_remote_protocol(self):
        schema = evidence.load_schema()
        self.assertEqual(schema, evidence._schema_template())
        self.assertEqual(len(schema["trace"]["expected_values"]), 32)
        values = schema["trace"]["expected_values"]
        self.assertEqual(values["trace.dynamic_full_direct_small_one_remote_exit.request_size"], 1024)
        self.assertEqual(values["trace.dynamic_full_direct_small_one_remote_exit.block_size"], 1024)
        self.assertEqual(values["trace.dynamic_full_direct_small_one_remote_exit.capacity"], 64)
        self.assertEqual(values["trace.dynamic_full_direct_small_one_remote_exit.reserved"], 64)
        self.assertEqual(values["trace.dynamic_full_direct_small_one_remote_exit.slice_count"], 1)
        self.assertEqual(
            values[
                "trace.dynamic_full_direct_small_one_remote_exit.direct_cache_range_start"
            ],
            113,
        )
        self.assertEqual(
            values[
                "trace.dynamic_full_direct_small_one_remote_exit.direct_cache_range_end"
            ],
            128,
        )
        self.assertEqual(
        ...
        self.assertTrue(
            schema["scope"]["dynamic_full_direct_small_one_remote_regular_bin_only"]
        )
        ...
        self.assertIn(
            {
                ...
                "start_line": 245,
                "end_line": 268,
                "sha256": "e154cd246df21cf66e7bf2b966567c3c9bb58aad09e97434818da52581a0354c",
            },
    ...
    def test_schema_rejects_probe_scope_source_or_trace_drift(self):
        mutations = (
            ...
            lambda value: value["scope"].update(
                {"dynamic_full_direct_small_one_remote_regular_bin_only": False}
            ),
            lambda value: value["source_anchors"][3].update({"sha256": "0" * 64}),
            lambda value: value["rust_test"].update({"target_arch": "aarch64"}),
            lambda value: value["trace"]["expected_values"].update(
                {"trace.dynamic_full_direct_small_one_remote_exit.capacity": 63}
            ),
        )
...
class TraceAndReportTests(unittest.TestCase):
    def complete_report(self):
        ...
        temporary = Path("/tmp/dynamic-full-direct-small-one-remote-force-collect-to-mapped-evidence")
    ...
    def test_c_command_retains_pthread_tls_and_native_oracle_selection(self):
        schema = evidence.load_schema()
        stem = "dynamic-full-direct-small-one-remote-force-collect-to-mapped"
        temporary = Path("/tmp/dynamic-full-direct-small-one-remote-command")
        source = temporary / "source/mimalloc-3.5.0"
    ...
    def test_c_probe_keeps_the_live_direct_cache_observation_pre_exit(self):
        ...
        self.assertIn("page->used + 1 == capacity", source)
        self.assertNotIn("used_after_first_consumer_free", source)
    ...
    def test_parser_requires_an_exact_pointer_free_trace(self):
        ...
        with self.assertRaisesRegex(evidence.EvidenceError, "raw address"):
            evidence.parse_trace(
                ...
                "trace.dynamic_full_direct_small_one_remote_exit.pointer=0x1\n"
    ...
    def test_report_binds_both_probes_and_post_exit_scope(self):
        ...
        self.assertEqual(report["kind"], evidence.DYNAMIC_FULL_DIRECT_SMALL_ONE_REMOTE_EXIT_KIND)
        self.assertEqual(report["comparison"], {"compared_value_count": 32, "status": "matched"})
