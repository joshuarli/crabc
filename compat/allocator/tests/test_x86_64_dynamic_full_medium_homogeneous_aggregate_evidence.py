#!/usr/bin/env python3
"""Contract tests for the private homogeneous full-medium aggregate lane."""
...
SCRIPT = ROOT / "compat/allocator/x86_64_dynamic_full_medium_homogeneous_aggregate_evidence.py"
spec = importlib.util.spec_from_file_location("dynamic_full_medium_homogeneous_aggregate_evidence", SCRIPT)
...
class SchemaTests(unittest.TestCase):
    def test_schema_binds_two_full_medium_pages_and_active_tls_map(self):
        schema = evidence.load_schema()
        self.assertEqual(schema, evidence._schema_template())
        values = schema["trace"]["expected_values"]
        prefix = "trace.dynamic_full_medium_homogeneous_aggregate."
        ...
        self.assertEqual(values[f"{prefix}request_size"], 10248)
        self.assertEqual(values[f"{prefix}block_size"], 12288)
        self.assertEqual(values[f"{prefix}capacity"], 42)
        self.assertEqual(values[f"{prefix}slice_count"], 8)
        self.assertEqual(values[f"{prefix}page0.page_map_slice_count_after_thread_done"], 8)
        self.assertEqual(values[f"{prefix}page1.page_map_slice_count_after_thread_done"], 8)
        self.assertEqual(values[f"{prefix}page0.slice_count_after_thread_done"], 8)
        self.assertEqual(values[f"{prefix}page1.slice_count_after_thread_done"], 8)
        ...
        self.assertEqual(values[f"{prefix}page1.page_map_slice_count_after_first_terminal"], 8)
    ...
    def test_schema_rejects_hash_scope_trace_and_lifecycle_drift(self):
        mutations = (
            ...
            lambda value: value["trace"]["expected_values"].update(
                {"trace.dynamic_full_medium_homogeneous_aggregate.page1.used_after_reabandon_boundary": 37}
            ),
...
class TraceAndCommandTests(unittest.TestCase):
    def test_report_construction_and_validation_are_repeatable_without_base_mutation(self):
        schema = evidence.load_schema()
        temporary = Path("/tmp/dynamic-full-medium-homogeneous-aggregate-pure-report")
    ...
    def test_report_validation_rejects_structurally_incomplete_report(self):
        schema = evidence.load_schema()
        temporary = Path("/tmp/dynamic-full-medium-homogeneous-aggregate-report")
        source = temporary / "source/mimalloc-3.5.0"
        stem = "dynamic-full-medium-homogeneous-aggregate"
        ...
        malformed = {
            ...
            "c_probe": {
                "build_command": build,
                "run_command": ["<temporary-evidence-root>/dynamic-full-medium-homogeneous-aggregate-c"],
                "source_sha256": evidence.sha256_bytes(evidence.C_TRACE_PROBE.encode()),
            },
        }
    ...
    def test_trace_requires_exact_two_page_shape(self):
        ...
        trace.pop("trace.dynamic_full_medium_homogeneous_aggregate.page1.used_after_thread_done")
        with self.assertRaises(evidence.EvidenceError):
            evidence.validate_trace(trace, description="missing aggregate field")
        trace = copy.deepcopy(schema["trace"]["expected_values"])
        trace["trace.dynamic_full_medium_homogeneous_aggregate.page0.used_after_reabandon_boundary"] = 37
        with self.assertRaises(evidence.EvidenceError):
    ...
    def test_c_command_requires_native_pthread_tls_contract(self):
        schema = evidence.load_schema()
        temporary = Path("/tmp/dynamic-full-medium-homogeneous-aggregate-command")
        source = temporary / "source/mimalloc-3.5.0"
        command = evidence.c_trace_command(
            "/usr/bin/musl-gcc",
            source,
            temporary / "dynamic-full-medium-homogeneous-aggregate.c",
            temporary / "dynamic-full-medium-homogeneous-aggregate-c",
            schema,
        )
