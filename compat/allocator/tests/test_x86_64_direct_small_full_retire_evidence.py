#!/usr/bin/env python3
"""Pure contracts for native x86-64 direct-small full-retire evidence."""
...
SCRIPT = ROOT / "compat/allocator/x86_64_direct_small_full_retire_evidence.py"
spec = importlib.util.spec_from_file_location("direct_small_full_retire_evidence", SCRIPT)
...
class SchemaTests(unittest.TestCase):
    def test_schema_binds_the_private_fixed_trace_contract(self) -> None:
        ...
        self.assertEqual(
            ...
            },
            {
                ...
                for key in (
                    "trace.direct_small_full.direct_range_start",
                    "trace.direct_small_full.direct_range_end",
                    "trace.direct_small_full.retired.direct_range_matches",
                    "trace.direct_small_full.release.direct_range_empty",
                )
            },
        )

    def test_schema_rejects_probe_scope_or_trace_drift(self) -> None:
        mutations = (
            lambda value: value.update({"c_probe_sha256": "0" * 64}),
            lambda value: value["scope"].update({"full_direct_small_regular_bin_only": False}),
            lambda value: value["trace"]["expected_values"].update(
                {"trace.direct_small_full.valid": 0}
            ),
...
class CommandAndTraceTests(unittest.TestCase):
    def test_c_command_retains_fixed_release_pthread_and_tls_selection(self) -> None:
        schema = evidence.load_schema()
        temporary = Path("/tmp/direct-small-full-retire-evidence")
        source = temporary / "source/mimalloc-3.5.0"
        command = evidence.c_trace_command(
            "/usr/bin/musl-gcc",
            source,
            temporary / "direct-small-full-retire.c",
            temporary / "direct-small-full-retire-c",
            schema,
        )
    ...
    def test_trace_parser_requires_the_exact_pointer_free_record(self) -> None:
        ...
        with self.assertRaisesRegex(evidence.EvidenceError, "raw address"):
            evidence.parse_trace(
                ...
                "trace.direct_small_full.pointer=0x1\n"
                f"{evidence.TRACE_END}",
                description="test trace",
            )
        weakened = dict(trace)
        weakened["trace.direct_small_full.valid"] = 0
...
class ReportTests(unittest.TestCase):
    def complete_report(self) -> dict[str, object]:
        ...
        return evidence.report_from_results(
            ...
            c_probe={
                ...
                "run_command": [
                    "<temporary-evidence-root>/direct-small-full-retire-c"
                ],
    ...
    def test_report_is_private_and_binds_both_native_probe_identities(self) -> None:
        ...
        self.assertTrue(report["scope"]["full_direct_small_regular_bin_only"])
        ...
        mutations = (
            ...
            (
                lambda value: value["scope"].update(
                    {"full_direct_small_regular_bin_only": False}
                ),
                "private boundary",
            ),
            (
