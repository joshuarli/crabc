#!/usr/bin/env python3
"""Pure contracts for the dynamic full-medium one-remote post-exit x86-64 lane."""
...
SCRIPT = ROOT / "compat/allocator/x86_64_dynamic_full_medium_one_remote_force_collect_to_mapped_evidence.py"
spec = importlib.util.spec_from_file_location("dynamic_full_medium_one_remote_evidence", SCRIPT)
...
class SchemaTests(unittest.TestCase):
    def test_schema_binds_the_fixed_dynamic_full_medium_one_remote_protocol(self):
        schema = evidence.load_schema()
        self.assertEqual(schema, evidence._schema_template())
        self.assertEqual(len(schema["trace"]["expected_values"]), 29)
        values = schema["trace"]["expected_values"]
        self.assertEqual(values["trace.dynamic_full_medium_one_remote_exit.request_size"], 10248)
        self.assertEqual(values["trace.dynamic_full_medium_one_remote_exit.block_size"], 12288)
        self.assertEqual(values["trace.dynamic_full_medium_one_remote_exit.capacity"], 42)
        self.assertEqual(values["trace.dynamic_full_medium_one_remote_exit.slice_count"], 8)
        self.assertEqual(
            schema["rust_test"],
            {
    ...
    def test_schema_rejects_probe_scope_source_or_rust_selection_drift(self):
        mutations = (
            ...
            lambda value: value["trace"]["expected_values"].update(
                {"trace.dynamic_full_medium_one_remote_exit.capacity": 41}
            ),
        )
...
class TraceAndReportTests(unittest.TestCase):
    def complete_report(self):
        schema = evidence.load_schema()
        temporary = Path("/tmp/dynamic-full-medium-one-remote-force-collect-to-mapped-evidence")
        source = temporary / "source/mimalloc-3.5.0"
        c_command = evidence.normalize_command(
            evidence.c_trace_command(
                "/usr/bin/musl-gcc",
                source,
                temporary / "dynamic-full-medium-one-remote-force-collect-to-mapped.c",
                temporary / "dynamic-full-medium-one-remote-force-collect-to-mapped-c",
                schema,
            ),
            temporary,
        ...
        return evidence.report_from_results(
            ...
            c_probe={
                ...
                "run_command": [
                "<temporary-evidence-root>/dynamic-full-medium-one-remote-force-collect-to-mapped-c"
                ],
    ...
    def test_c_command_retains_pthread_tls_and_native_oracle_selection(self):
        schema = evidence.load_schema()
        temporary = Path("/tmp/dynamic-full-medium-one-remote-command")
        source = temporary / "source/mimalloc-3.5.0"
    ...
    def test_c_probe_checks_every_terminal_medium_page_map_slice(self):
        self.assertIn(
    ...
    def test_parser_requires_an_exact_pointer_free_trace(self):
        ...
        with self.assertRaisesRegex(evidence.EvidenceError, "raw address"):
            evidence.parse_trace(
                ...
                "trace.dynamic_full_medium_one_remote_exit.pointer=0x1\n"
    ...
    def test_report_binds_both_probes_and_post_exit_scope(self):
        ...
        self.assertEqual(report["kind"], evidence.DYNAMIC_FULL_MEDIUM_ONE_REMOTE_EXIT_KIND)
        self.assertEqual(
            report["comparison"], {"compared_value_count": 29, "status": "matched"}
        )
