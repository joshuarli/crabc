#!/usr/bin/env python3
"""Pure contracts for private native x86-64 direct-on-demand evidence."""
...
SCRIPT = ROOT / "compat/allocator/x86_64_direct_on_demand_evidence.py"
spec = importlib.util.spec_from_file_location("direct_on_demand_evidence", SCRIPT)
...
class SchemaTests(unittest.TestCase):
    def test_schema_binds_private_option_scope_direct_rust_test_and_geometry(self):
        schema = evidence.load_schema()
        self.assertEqual(
            evidence.SCHEMA_PATH,
            ROOT / "compat/allocator/x86_64-direct-on-demand-evidence-v3.5.0.json",
        )
        ...
        self.assertFalse(schema["scope"]["fresh_fallback_claimed"])
        self.assertEqual(schema["rust_test"]["test_filter"], evidence.RUST_TEST_FILTER)
        self.assertEqual(schema["trace"]["begin"], evidence.TRACE_BEGIN)
        self.assertEqual(schema["trace"]["end"], evidence.TRACE_END)
        self.assertEqual(
            schema["trace"]["expected_values"]["trace.on_demand_direct.seventeenth_slice_pcommitted"],
            8,
        )
    ...
    def test_default_schema_load_fails_closed_when_the_checked_in_contract_is_missing(self):
        with mock.patch.object(evidence, "SCHEMA_PATH", ROOT / "missing-direct-on-demand-schema.json"):
    ...
    def test_schema_rejects_option_scope_geometry_or_anchor_drift(self):
        mutations = (
            ...
            (lambda value: value["scope"].update({"fresh_fallback_claimed": True}), "private boundary"),
            (
                lambda value: value["trace"]["expected_values"].update(
                    {"trace.on_demand_direct.ninth_slice_pcommitted": 8}
                ),
                "fixed trace",
            ),
            (lambda value: value["source_anchors"][4].update({"sha256": "0" * 64}), "source anchor contract"),
...
class CommandAndTraceTests(unittest.TestCase):
    def test_c_command_retains_fixed_release_pthread_and_tls_selection(self):
        schema = evidence.load_schema()
        temporary = Path("/tmp/direct-on-demand-evidence")
        source = temporary / "source/mimalloc-3.5.0"
        command = evidence.c_trace_command(
            "/usr/bin/musl-gcc",
            source,
            temporary / "direct-on-demand.c",
            temporary / "direct-on-demand-c",
            schema,
        )
    ...
    def test_trace_parser_requires_exact_address_independent_record(self):
        ...
        changed = dict(trace)
        changed["trace.on_demand_direct.seventeenth_slice_pcommitted"] = 4
...
class ReportTests(unittest.TestCase):
    def complete_report(self) -> dict[str, object]:
        ...
        return evidence.report_from_results(
            ...
            c_probe={
                ...
                "run_command": ["<temporary-evidence-root>/direct-on-demand-c"],
    ...
    def test_report_is_private_and_binds_exact_native_probe_identities(self):
        ...
        self.assertEqual(report["comparison"], {"compared_value_count": 44, "status": "matched"})
        self.assertTrue(report["scope"]["oracle_option_setup_only"])
        self.assertFalse(report["scope"]["production_page_on_demand_policy_claimed"])
        self.assertFalse(report["scope"]["fresh_fallback_claimed"])
