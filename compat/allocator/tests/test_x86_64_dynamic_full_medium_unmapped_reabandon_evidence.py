#!/usr/bin/env python3
"""Pure contracts for the dynamic full-medium unmapped x86-64 lane."""
...
SCRIPT = ROOT / "compat/allocator/x86_64_dynamic_full_medium_unmapped_reabandon_evidence.py"
spec = importlib.util.spec_from_file_location("dynamic_full_medium_unmapped_evidence", SCRIPT)
...
class SchemaTests(unittest.TestCase):
    def test_schema_binds_the_fixed_dynamic_full_medium_unmapped_protocol(self):
        ...
        prefix = "trace.dynamic_full_medium_unmapped_exit."
        ...
        self.assertTrue(schema["scope"]["dynamic_full_medium_full_bin_only"])
    ...
    def test_schema_rejects_probe_scope_source_or_trace_drift(self):
        mutations = (
            ...
            lambda value: value["trace"]["expected_values"].update(
                {"trace.dynamic_full_medium_unmapped_exit.used_after_reabandon_boundary": 37}
            ),
        )
...
class TraceAndReportTests(unittest.TestCase):
    def complete_report(self):
        ...
        temporary = Path("/tmp/dynamic-full-medium-unmapped-reabandon-evidence")
    ...
    def test_c_command_retains_pthread_tls_and_native_oracle_selection(self):
        schema = evidence.load_schema()
        stem = "dynamic-full-medium-unmapped-reabandon"
        temporary = Path("/tmp/dynamic-full-medium-unmapped-reabandon-command")
        source = temporary / "source/mimalloc-3.5.0"
    ...
    def test_parser_requires_an_exact_pointer_free_trace(self):
        ...
        with self.assertRaisesRegex(evidence.EvidenceError, "raw address"):
            evidence.parse_trace(
                ...
                "trace.dynamic_full_medium_unmapped_exit.pointer=0x1\n"
    ...
    def test_report_binds_both_probes_and_post_exit_scope(self):
        ...
        self.assertEqual(report["kind"], evidence.DYNAMIC_FULL_MEDIUM_UNMAPPED_EXIT_KIND)
