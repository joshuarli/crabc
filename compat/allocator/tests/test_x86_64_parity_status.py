...
class X86_64ParityStatusTests(unittest.TestCase):
    ...
    def test_full_medium_homogeneous_aggregate_schema_profile_is_exact(self) -> None:
        schema = json.loads(
    ...
    def test_native_evidence_gates_are_target_scoped(self) -> None:
        ...
        self.assertEqual(
            gates["native-dynamic-full-non-direct-small-unmapped-reabandon-differential"][
                "command"
            ],
            "./compat/allocator/run-x86_64.sh "
        ...
        self.assertEqual(
            ...
            "compat/reports/allocator/x86_64/"
            "dynamic-full-non-direct-small-unmapped-reabandon.json",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            gates["native-dynamic-full-medium-homogeneous-aggregate-differential"]["command"],
            "./compat/allocator/run-x86_64.sh allocator-dynamic-full-medium-homogeneous-aggregate",
        )
        self.assertEqual(
        ...
        self.assertIn("nine named private Rust lifecycle/concurrency lanes", gates["native-bounded-lifecycle-concurrency"]["claim"])
        self.assertIn("13 selected tests", gates["native-bounded-lifecycle-concurrency"]["claim"])
        self.assertIn("dynamic arena-singleton owner moved to a joined receiver", gates["native-bounded-lifecycle-concurrency"]["claim"])
        ...
        for fragment in (
            ...
            "clears the direct-cache range before page-count detach",
            "bounded partial collector",
        ...
        for fragment in (
            ...
            "dynamic abandoned bitmap/count clear",
            "used 51",
            "Six sequential normal-collector frees",
            "used 45",
            "reserved / 8 == 6",
            "seventh free",
            "used 44",
        ...
        dynamic_full_medium_aggregate = gates[
            "native-dynamic-full-medium-homogeneous-aggregate-differential"
        ]["claim"]
        for fragment in (
            "67 address-independent values",
            "bounded dynamic homogeneous full-medium aggregate route",
            "exactly two distinct full BIN_FULL medium arena pages",
            "request 10248",
            "12288-byte blocks",
            "capacity/reserved 42",
            "eight arena slices each",
            "real mi_thread_done",
            "pthread_join()",
            "both members unmapped-abandoned",
            "five-free mostly-used boundary",
            ...
            "used 36 mapped",
            "complete eight-slice arena span",
            "bounded dynamic homogeneous full-medium aggregate owner-exit route",
            "private native x86-64 engine evidence only",
            "general lifecycle",
            "AArch64 evidence",
        ):
