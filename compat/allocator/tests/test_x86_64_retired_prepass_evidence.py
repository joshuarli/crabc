#!/usr/bin/env python3
"""Pure contracts for native x86-64 retired-page prepass evidence."""
...
SCRIPT = ROOT / "compat/allocator/x86_64_retired_prepass_evidence.py"
spec = importlib.util.spec_from_file_location("retired_prepass_evidence", SCRIPT)
...
class RetiredPrepassEvidenceTests(unittest.TestCase):
    ...
    def complete_report(self):
        ...
        c_command = evidence.normalize_command(
            evidence.c_command(
                ...
                Path("/tmp/evidence/retired-prepass.c"),
                Path("/tmp/evidence/retired-prepass-c"),
                schema,
            ),
            Path("/tmp/evidence"),
        ...
        c_probe = {
            ...
            "run_command": ["<temporary-evidence-root>/retired-prepass-c"],
        ...
        return {
            "c_probe": c_probe,
            "comparison": {"compared_value_count": 21, "status": "matched"},
            "format": 1,
            "kind": "mimalloc-x86_64-retired-prepass-differential-evidence",
    ...
    def test_schema_is_pinned_and_native_only(self):
        ...
        self.assertFalse(schema["scope"]["general_retired_page_claimed"])
    ...
    def test_schema_rejects_drift_and_emulation(self):
        mutations = (
            ...
            lambda value: value["trace"]["expected_values"].pop("trace.retired_prepass.valid"),
            lambda value: value["source_anchors"].__setitem__(
                0,
                {"member": "src/free.c", "start_line": 1, "end_line": 2, "sha256": "0" * 64},
            ),
        )
    ...
    def test_trace_rejects_missing_unexpected_noninteger_and_wrong_values(self):
        trace = dict(evidence.EXPECTED_TRACE_VALUES)
        for mutate in (
            lambda value: value.pop("trace.retired_prepass.valid"),
            lambda value: value.update({"trace.retired_prepass.extra": 1}),
            lambda value: value.update({"trace.retired_prepass.valid": True}),
            lambda value: value.update({"trace.retired_prepass.valid": 0}),
        ):
            value = dict(trace)
            mutate(value)
            with self.assertRaises(evidence.EvidenceError):
    ...
    def test_c_probe_uses_thread_done_prepass_without_a_forced_collection_shortcut(self):
        probe = evidence.C_TRACE_PROBE
        self.assertIn("mi_thread_done();", probe)
        self.assertNotIn("mi_heap_collect(", probe)
        self.assertNotIn("f.retired_page", probe)
        self.assertIn("#if MI_PAGE_MAP_FLAT != 0", probe)
        self.assertIn(
            "_mi_safe_ptr_page((const void*)(uintptr_t)f.retired_address)",
            probe,
        )
        self.assertIn("if (!f->retired_map_before_exit) goto fail;", probe)
        self.assertIn("if (!live_page_map_registered_after_teardown) {", probe)
        self.assertIn("f.live = NULL;", probe)
        self.assertIn("mi_memid_arena(lp->memid) != f->arena", probe)
    ...
    def test_report_rejects_provenance_and_probe_drift(self):
        ...
        weakened = copy.deepcopy(self.complete_report())
        weakened["c_probe"]["trace"]["trace.retired_prepass.valid"] = 0
        with self.assertRaises(evidence.EvidenceError):
            self.validate_report(weakened)
