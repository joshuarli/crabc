...
class X86_64SourceMapTests(unittest.TestCase):
    ...
    def test_statistics_collection_maps_the_selected_private_owner_events(self) -> None:
        ...
        for fragment in (
            ...
            "native C/Rust layout gate compares every member and tail offset",
            "selected MI_STAT=0 engine records",
            "selected mapped abandoned-page reclaim",
            "clear-once bitmap busy waits",
            "mapped binned bitmap chunk-bin transitions",
    ...
    def test_dynamic_full_medium_unmapped_reabandon_lane_is_scoped_to_reviewed_units(
        ...
        lane_evidence = {
            "compat/allocator/tests/test_x86_64_dynamic_full_medium_unmapped_reabandon_evidence.py",
            "compat/allocator/x86_64-dynamic-full-medium-unmapped-reabandon-evidence-v3.5.0.json",
        ...
        for unit_id in reviewed_unit_ids:
            with self.subTest(unit=unit_id):
                unit = units[unit_id]
                self.assertEqual(unit["status"], "partial")
                self.assertIn("34-field native C/Rust differential", unit["difference"])
                self.assertIn(
                    "dynamic full-medium unmapped-reabandon route", unit["difference"]
                )
                self.assertIn("10248-byte request", unit["difference"])
                self.assertIn("12288-byte blocks", unit["difference"])
                self.assertIn("capacity/reserved 42", unit["difference"])
                self.assertIn("eight slices", unit["difference"])
                self.assertIn("no remote mi_free", unit["difference"])
                self.assertIn("used == 42", unit["difference"])
                self.assertIn("Five normal-collector frees", unit["difference"])
                self.assertIn("used == 37", unit["difference"])
                self.assertIn("sixth free", unit["difference"])
                self.assertIn("used == 36", unit["difference"])
                self.assertIn("dynamic bitmap/count 1", unit["difference"])
                self.assertIn("all eight arena slices", unit["difference"])
    ...
    def test_dynamic_full_large_homogeneous_aggregate_lane_is_scoped_to_reviewed_units(
        self,
    ) -> None:
        reviewed_unit_ids = {
            "local-and-remote-free",
        ...
        lane_evidence = {
            "compat/allocator/tests/test_x86_64_dynamic_full_large_homogeneous_aggregate_evidence.py",
            "compat/allocator/x86_64-dynamic-full-large-homogeneous-aggregate-evidence-v3.5.0.json",
        ...
        self.assertEqual(
            expected[
                "trace.dynamic_full_large_homogeneous_aggregate.request_size"
            ],
            86706,
        )
        self.assertEqual(
            expected[
                "trace.dynamic_full_large_homogeneous_aggregate.block_size"
            ],
            98304,
        )
        self.assertEqual(
            expected[
                "trace.dynamic_full_large_homogeneous_aggregate.reserved"
            ],
            42,
        )
        self.assertEqual(
            expected[
                "trace.dynamic_full_large_homogeneous_aggregate.slice_count"
            ],
            64,
        )
        for page in ("page0", "page1"):
            with self.subTest(page=page):
                self.assertEqual(
                    expected[
                        f"trace.dynamic_full_large_homogeneous_aggregate.{page}.page_map_slice_count_after_thread_done"
                    ],
                    63,
                )
                self.assertEqual(
                    expected[
                        f"trace.dynamic_full_large_homogeneous_aggregate.{page}.unmapped_prefix_free_count"
                    ],
                    5,
                )
                self.assertEqual(
                    expected[
                        f"trace.dynamic_full_large_homogeneous_aggregate.{page}.used_after_unmapped_prefix"
                    ],
                    37,
                )
                self.assertEqual(
                    expected[
                        f"trace.dynamic_full_large_homogeneous_aggregate.{page}.used_after_reabandon_boundary"
                    ],
                    36,
                )
        for unit_id in reviewed_unit_ids:
    ...
    def test_dynamic_full_direct_small_one_remote_lane_is_scoped_to_reviewed_units(
        ...
        lane_evidence = {
            "compat/allocator/tests/test_x86_64_dynamic_full_direct_small_one_remote_force_collect_to_mapped_evidence.py",
            "compat/allocator/x86_64-dynamic-full-direct-small-one-remote-force-collect-to-mapped-evidence-v3.5.0.json",
        ...
        for unit_id in reviewed_unit_ids:
            with self.subTest(unit=unit_id):
                unit = units[unit_id]
                self.assertEqual(unit["status"], "partial")
                self.assertIn("32-field native C/Rust differential", unit["difference"])
                self.assertIn(
                    "dynamic full direct-small one-remote force-collect-to-mapped route",
                    unit["difference"],
                )
                self.assertIn("exact rounded direct-cache range [113, 128]", unit["difference"])
                self.assertIn(
                    "direct-cache clear-before-page-count-detach", unit["difference"]
                )
                self.assertIn("partial-collector client frees", unit["difference"])
    ...
    def test_dynamic_full_non_direct_small_unmapped_reabandon_lane_is_scoped_to_reviewed_units(
        self,
    ) -> None:
        reviewed_unit_ids = {
            "local-and-remote-free",
        ...
        lane_evidence = {
            "compat/allocator/tests/test_x86_64_dynamic_full_non_direct_small_unmapped_reabandon_evidence.py",
            "compat/allocator/x86_64-dynamic-full-non-direct-small-unmapped-reabandon-evidence-v3.5.0.json",
        ...
        for unit_id in reviewed_unit_ids:
            with self.subTest(unit=unit_id):
                unit = units[unit_id]
                self.assertEqual(unit["status"], "partial")
                self.assertIn("35-field native C/Rust differential", unit["difference"])
                self.assertIn(
                    "dynamic full non-direct-small unmapped-reabandon route",
                    unit["difference"],
                )
                self.assertIn("1032-byte non-direct-small", unit["difference"])
                self.assertIn("1280-byte blocks", unit["difference"])
                ...
                self.assertIn("empty direct-cache image", unit["difference"])
                self.assertIn("no remote mi_free", unit["difference"])
                self.assertIn("used == 51", unit["difference"])
                self.assertIn("Six normal-collector frees", unit["difference"])
                self.assertIn("used == 45", unit["difference"])
                self.assertIn("seventh free", unit["difference"])
                self.assertIn("used == 44", unit["difference"])
                self.assertIn("dynamic bitmap/count 1", unit["difference"])
                self.assertIn("one-slice release", unit["difference"])
                self.assertIn("not general lifecycle", unit["difference"])
