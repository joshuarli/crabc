#!/usr/bin/env python3
"""Pure contracts for the dynamic full non-direct-small unmapped x86-64 lane."""
...
SCRIPT = ROOT / "compat/allocator/x86_64_dynamic_full_non_direct_small_unmapped_reabandon_evidence.py"
spec = importlib.util.spec_from_file_location(
    "dynamic_full_non_direct_small_unmapped_evidence", SCRIPT
)
...
class SchemaTests(unittest.TestCase):
    def test_schema_binds_the_fixed_dynamic_full_non_direct_small_unmapped_protocol(self):
        schema = evidence.load_schema()
        self.assertEqual(schema, evidence._schema_template())
        self.assertEqual(len(schema["trace"]["expected_values"]), 35)
        values = schema["trace"]["expected_values"]
        prefix = "trace.dynamic_full_non_direct_small_unmapped_exit."
        ...
        self.assertEqual(values[f"{prefix}used_after_thread_done"], 51)
        self.assertEqual(values[f"{prefix}unmapped_prefix_free_count"], 6)
        self.assertEqual(values[f"{prefix}used_after_unmapped_prefix"], 45)
        self.assertEqual(values[f"{prefix}used_after_reabandon_boundary"], 44)
        ...
        self.assertTrue(schema["scope"]["dynamic_full_non_direct_small_regular_bin_only"])
        ...
        self.assertIn(
            {
                ...
                "start_line": 1072,
                "end_line": 1081,
                "sha256": "c0e6b4f003a664d9a0e78a4b7036b760ad37ba0f91755fc534facfdd05f779d4",
            },
        ...
        self.assertIn(
            {
                ...
                "start_line": 123,
                "end_line": 152,
                "sha256": "c7811179e91e8cd66dc0587e824265cff4db6ce660ba0639309d909dd0df519c",
            },
            schema["source_anchors"],
        )
        self.assertIn(
            {
                ...
                "start_line": 228,
                "end_line": 232,
                "sha256": "16c0e73a20b9a94bf994c4e83836c976f5683e3c6e8b18935782a934405adba0",
            },
            schema["source_anchors"],
        )
        self.assertIn(
            {
                ...
                "start_line": 378,
                "end_line": 417,
                "sha256": "c31e558c1bf6c292aecab8e4a4fe3ef8c2616d2f10d9ac6549fe987ad72cac62",
            },
            schema["source_anchors"],
        )
        self.assertIn(
    ...
    def test_schema_rejects_probe_scope_source_or_trace_drift(self):
        mutations = (
            ...
            lambda value: value["scope"].update(
                {"dynamic_full_non_direct_small_regular_bin_only": False}
            ),
            ...
            lambda value: value["trace"]["expected_values"].update(
                {"trace.dynamic_full_non_direct_small_unmapped_exit.capacity": 50}
            ),
        )
...
class TraceAndReportTests(unittest.TestCase):
    def complete_report(self):
        ...
        temporary = Path("/tmp/dynamic-full-non-direct-small-unmapped-reabandon-evidence")
    ...
    def test_c_command_retains_pthread_tls_and_native_oracle_selection(self):
        schema = evidence.load_schema()
        stem = "dynamic-full-non-direct-small-unmapped-reabandon"
        temporary = Path("/tmp/dynamic-full-non-direct-small-unmapped-reabandon-command")
        source = temporary / "source/mimalloc-3.5.0"
    ...
    def test_c_probe_keeps_the_live_theap_observation_pre_exit(self):
        ...
        self.assertIn("non_direct_small = block_size > MI_SMALL_SIZE_MAX && small_page;", source)
        self.assertIn("unmapped_prefix_free_count = reserved / 8;", source)
        self.assertIn("_mi_safe_ptr_page((const void*)saved_address) == page", source)
    ...
    def test_c_probe_checks_the_unmapped_prefix_mapped_boundary_and_terminal_release(self):
        source = evidence.C_TRACE_PROBE
        for observation in (
    ...
    def test_parser_requires_an_exact_pointer_free_trace(self):
        ...
        with self.assertRaisesRegex(evidence.EvidenceError, "raw address"):
            evidence.parse_trace(
                ...
                "trace.dynamic_full_non_direct_small_unmapped_exit.pointer=0x1\n"
    ...
    def test_report_binds_both_probes_and_post_exit_scope(self):
        ...
        self.assertEqual(
            report["kind"], evidence.DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_EXIT_KIND
        )
        self.assertEqual(report["comparison"], {"compared_value_count": 35, "status": "matched"})
