...
Private native Linux/x86-64 mimalloc evidence commands:
  ...
  allocator-process-done-pthread-key
  allocator-automatic-arena-reservation [--reader-tests]
  ...
  allocator-perf-engine [--smoke|--full] [--set architecture|matrix] [options] | allocator-perf-engine --reader-tests
  allocator-codegen-audit [options] | allocator-codegen-audit --reader-tests
...
case "$command" in
    ...
    allocator-m4)
        ...
        if [ "$#" -eq 0 ]; then
            ...
            m4_command=(python3 compat/allocator/x86_64_m4_gate.py --check)
    ...
    allocator-m8)
        ...
        if [ "$#" -eq 0 ]; then
            m8_command=(python3 -B compat/allocator/x86_64_m8_gate.py)
        elif [ "$#" -eq 1 ] && [ "$1" = --check ]; then
            m8_command=(python3 -B compat/allocator/x86_64_m8_gate.py --check)
        elif [ "$#" -eq 1 ] && [ "$1" = --reader-tests ]; then
            m8_command=(python3 -B compat/allocator/tests/test_x86_64_m8_gate.py)
        elif [ "$#" -eq 2 ] && [ "$1" = --gate ]; then
            m8_command=(python3 -B compat/allocator/x86_64_m8_gate.py --gate "$2")
        else
            fail "allocator-m8 accepts only --check, --gate ID, or --reader-tests"
        fi
    ...
    allocator-m7)
        ...
        if [ "$#" -eq 0 ]; then
            ...
            m7_command=(python3 compat/allocator/x86_64_m7_gate.py --check)
        elif [ "$#" -eq 1 ] && [ "$1" = --reader-tests ]; then
            ...
            fail "allocator-m7 accepts only --check, --gate ID, or --reader-tests"
        fi
        ensure_image
    ...
    allocator-m9)
        ...
        if [ "$#" -eq 1 ] && [ "$1" = --check ]; then
            ...
            while [ "$#" -gt 0 ]; do
                [ "$#" -ge 2 ] && [ "$1" = --report ] \
                    || fail "allocator-m9 accepts only --check, --reader-tests, or --report PATH..."
                m9_command+=(--report "$2")
                shift 2
            done
        fi
    ...
    allocator-m10)
        ...
        if [ "$#" -ge 1 ] && [ "$1" = --check ]; then
            ...
            m10_command=(python3 compat/allocator/x86_64_m10_gate.py --build-audit)
        elif [ "$#" -eq 1 ] && [ "$1" = --reader-tests ]; then
    ...
    allocator-lifecycle)
        ...
        ;;
    allocator-startup-regular-arena)
        ensure_image
        if [ "$#" -eq 0 ]; then
            run_in_container python3 compat/allocator/x86_64_startup_regular_arena_evidence.py
        elif [ "$#" -eq 1 ] && [ "$1" = --reader-tests ]; then
            run_in_container python3 compat/allocator/tests/test_x86_64_startup_regular_arena_evidence.py
            run_in_container python3 compat/allocator/tests/test_x86_64_source_map.py
        else
            fail "allocator-startup-regular-arena accepts only --reader-tests"
        fi
        ;;
    allocator-init-recursion)
    ...
    allocator-initialization-tld)
        ...
        if [ "$#" -eq 0 ]; then
            ...
            run_in_container python3 compat/allocator/test_x86_64_initialization_tld_evidence.py
            run_in_container python3 compat/allocator/tests/test_x86_64_source_map.py
        else
    ...
    allocator-fault-seam-inventory)
        ...
        if [ "$#" -eq 0 ]; then
        ...
        elif [ "$#" -eq 1 ] && [ "$1" = --metadata-publication-receiver ]; then
            run_in_container python3 compat/allocator/x86_64_fault_seam_inventory.py --offline --metadata-publication-receiver
            ...
            run_in_container python3 compat/allocator/x86_64_fault_seam_inventory.py --offline --canonical-m2-vm-c-compile-regression
            ...
            fail "allocator-fault-seam-inventory accepts only --os-publication-receiver, --metadata-publication-receiver, --compile-only, --canonical-m2-vm-c-compile-regression, --retry-helper-regression, --timeout-clock-helper-regression, --placement-warning-helper-regression, --mbind-boundary-regression, --huge-branch-diagnosis, or --reader-tests"
        fi
        ;;
    ...
    allocator-regular-mapped-reclaim)
        if [ "$#" -ne 0 ]; then
            [ "$#" -eq 1 ] && [ "$1" = "--offline" ] \
                || fail "allocator-regular-mapped-reclaim accepts only --offline"
        fi
        ensure_image
        run_in_container python3 compat/allocator/x86_64_regular_mapped_reclaim_evidence.py --offline
        ;;
    allocator-direct-small-allocation-adoption)
    ...
    allocator-cancellation-pthread-destructor)
        ...
        ;;
    allocator-process-done-pthread-key)
        [ "$#" -eq 0 ] || fail "allocator-process-done-pthread-key takes no arguments"
        ensure_image
        run_in_container python3 compat/allocator/x86_64_process_done_pthread_key_evidence.py --offline
        ;;
    allocator-automatic-arena-reservation)
        ensure_image
        if [ "$#" -eq 0 ]; then
            run_in_container python3 compat/allocator/x86_64_automatic_arena_reservation_evidence.py --offline
        elif [ "$#" -eq 1 ] && [ "$1" = --reader-tests ]; then
            run_in_container python3 compat/allocator/tests/test_x86_64_automatic_arena_reservation_evidence.py
            run_in_container python3 compat/allocator/tests/test_x86_64_source_map.py
        else
            fail "allocator-automatic-arena-reservation accepts only --reader-tests"
        fi
        ;;
    allocator-dynamic-os-aligned-singleton)
    ...
    allocator-perf)
        ...
        ;;
    allocator-perf-engine)
        ensure_image
        if [ "$#" -eq 1 ] && [ "$1" = --reader-tests ]; then
            run_in_container python3 compat/allocator/tests/test_perf_engine_x86_64.py
            exit 0
        fi
        run_in_container python3 compat/allocator/perf_engine_x86_64.py "$@"
        ;;
    allocator-native-local-trace)
        if [ "$#" -ne 0 ]; then
            [ "$#" -eq 2 ] && [ "$1" = --profile ] && { [ "$2" = initial ] || [ "$2" = worker ]; } || \
                fail "allocator-native-local-trace accepts only --profile initial|worker"
        fi
        ensure_image
        run_in_container python3 compat/allocator/native_local_trace_x86_64.py --offline "$@"
        ;;
    allocator-perf-integrated)
    ...
    allocator-codegen-audit)
        ensure_image
        if [ "$#" -eq 1 ] && [ "$1" = --reader-tests ]; then
            run_in_container python3 compat/allocator/tests/test_codegen_audit_x86_64.py
            exit 0
        fi
        run_in_container python3 compat/allocator/codegen_audit_x86_64.py "$@"
        ;;
    ...
    allocator-huge-numa-qualification)
        ...
        ;;
    allocator-arena-destroy)
        [ "$#" -eq 0 ] || fail "allocator-arena-destroy takes no arguments"
        ensure_image
    ...
    allocator-m2-metadata-ownership)
        ...
        ensure_image
        run_in_container python3 compat/allocator/m2_metadata_ownership_x86_64.py
        ;;
    allocator-heap-destroy)
        [ "$#" -eq 0 ] || fail "allocator-heap-destroy takes no arguments"
        ensure_image
        run_in_container python3 compat/allocator/heap_destroy.py
        ;;
    allocator-reclaim-on-free)
        [ "$#" -eq 0 ] || fail "allocator-reclaim-on-free takes no arguments"
        ensure_image
        run_in_container python3 compat/allocator/reclaim_on_free.py
        ;;
    allocator-subprocess-lifecycle)
        [ "$#" -eq 0 ] || fail "allocator-subprocess-lifecycle takes no arguments"
        ensure_image
        run_in_container python3 compat/allocator/subprocess_lifecycle.py
        ;;
    allocator-heap-lifecycle)
        [ "$#" -eq 0 ] || fail "allocator-heap-lifecycle takes no arguments"
        ensure_image
        run_in_container python3 compat/allocator/heap_lifecycle.py
        ;;
    allocator-m6-adapter)
    ...
    allocator-m6)
        ...
        if [ "$#" -eq 0 ]; then
            ...
            m6_command=(python3 compat/allocator/m6_gate.py --check)
        elif [ "$#" -eq 1 ] && [ "$1" = --reader-tests ]; then
