#!/usr/bin/env python3
"""Differentially prove one dynamic full-large one-remote force-collect-to-mapped route.

The native pinned-C fixture fills one arena-backed large `BIN_FULL` page on a
worker pthread. The consumer publishes exactly one remote free before the
worker calls real mi_thread_done. The source force collector makes the
otherwise-full page nonfull, abandons it into the mapped process route, and the
joined consumer then performs sequential client frees through final PageMap and
the complete arena-slice release.

This is narrow private Linux/x86-64 allocator-engine evidence. It is not a
public allocation API, public x86 crabc support, general teardown, general
remote-free routing, or AArch64 evidence.
"""
...
SCHEMA_PATH = ROOT / "compat/allocator/x86_64-dynamic-full-large-one-remote-force-collect-to-mapped-evidence-v3.5.0.json"
REPORT_DEFAULT = ROOT / "compat/reports/allocator/x86_64/dynamic-full-large-one-remote-force-collect-to-mapped.json"
RUST_TEST_SOURCE = ROOT / "crabc-mimalloc/src/dynamic_theap.rs"
EXPECTED_PROFILE = "linux-x86_64-private-dynamic-full-large-one-remote-force-collect-to-mapped"
RUST_TEST_FILTER = (
    "dynamic_theap::tests::"
    "x86_64_dynamic_full_large_one_remote_force_collect_to_mapped_trace_matches_pinned_c"
)
TRACE_BEGIN = "CRABC_MI_DYNAMIC_FULL_LARGE_ONE_REMOTE_EXIT_TRACE_BEGIN"
TRACE_END = "CRABC_MI_DYNAMIC_FULL_LARGE_ONE_REMOTE_EXIT_TRACE_END"
EXPECTED_SCOPE = {
    "aarch64_status_reused": False,
    "emulation_accepted": False,
    "dynamic_full_large_one_remote_full_bin_only": True,
...
EXPECTED_TRACE_VALUES = {
    "trace.dynamic_full_large_one_remote_exit.arena_backed": 1,
    "trace.dynamic_full_large_one_remote_exit.large_page": 1,
    "trace.dynamic_full_large_one_remote_exit.full_before_remote": 1,
    "trace.dynamic_full_large_one_remote_exit.full_queue_before_remote": 1,
    "trace.dynamic_full_large_one_remote_exit.direct_cache_empty_before_remote": 1,
    "trace.dynamic_full_large_one_remote_exit.remote_free_published_before_owner_exit_transition": 1,
    "trace.dynamic_full_large_one_remote_exit.owner_exit_transition_completed": 1,
    "trace.dynamic_full_large_one_remote_exit.joined_remote_producer_before_consumer_frees": 1,
    "trace.dynamic_full_large_one_remote_exit.mapped_after_thread_done": 1,
    "trace.dynamic_full_large_one_remote_exit.abandoned_after_thread_done": 1,
    "trace.dynamic_full_large_one_remote_exit.page_map_registered_after_thread_done": 1,
    "trace.dynamic_full_large_one_remote_exit.arena_page_bitmap_set_after_thread_done": 1,
    "trace.dynamic_full_large_one_remote_exit.full_queue_detached_after_thread_done": 1,
    "trace.dynamic_full_large_one_remote_exit.dynamic_abandoned_bitmap_set_after_thread_done": 1,
    "trace.dynamic_full_large_one_remote_exit.dynamic_abandoned_count_after_thread_done": 1,
    "trace.dynamic_full_large_one_remote_exit.request_size": 86706,
    "trace.dynamic_full_large_one_remote_exit.capacity": 42,
    "trace.dynamic_full_large_one_remote_exit.reserved": 42,
    "trace.dynamic_full_large_one_remote_exit.block_size": 98304,
    "trace.dynamic_full_large_one_remote_exit.slice_count": 64,
    "trace.dynamic_full_large_one_remote_exit.page_map_slice_count_after_owner_exit": 63,
    "trace.dynamic_full_large_one_remote_exit.used_after_force_collect": 41,
    "trace.dynamic_full_large_one_remote_exit.remaining_client_count_after_force_collect": 41,
    "trace.dynamic_full_large_one_remote_exit.nonfinal_consumer_free_keeps_mapped": 1,
    "trace.dynamic_full_large_one_remote_exit.page_map_tail_unregistered_after_owner_exit": 1,
    "trace.dynamic_full_large_one_remote_exit.page_map_unregistered_after_final_free": 1,
    "trace.dynamic_full_large_one_remote_exit.arena_page_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_large_one_remote_exit.arena_slice_released_after_final_free": 1,
    "trace.dynamic_full_large_one_remote_exit.dynamic_abandoned_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_large_one_remote_exit.dynamic_abandoned_count_after_final_free": 0,
    "trace.dynamic_full_large_one_remote_exit.valid": 1,
}
...
#error this private dynamic full-large one-remote fixture requires native Linux/x86_64
#endif
...
#define CRABC_DYNAMIC_FULL_LARGE_ONE_REMOTE_MAX_BLOCKS 64
...
typedef struct fixture_s {
  ...
  mi_heap_t* heap;
  void* blocks[CRABC_DYNAMIC_FULL_LARGE_ONE_REMOTE_MAX_BLOCKS];
  size_t block_count;
  bool setup_valid;
  bool worker_ready;
...
static void* producer_main(void* argument) {
  ...
  const size_t request = MI_MEDIUM_MAX_OBJ_SIZE + sizeof(void*);
  ...
  while (block_count < CRABC_DYNAMIC_FULL_LARGE_ONE_REMOTE_MAX_BLOCKS) {
    void* const block = mi_heap_malloc(heap, request);
    if (block == NULL) { failure_stage = 3; goto failed; }
    if (page == NULL) {
      page = _mi_ptr_page(block);
      if (page == NULL) { mi_free(block); failure_stage = 4; goto failed; }
    }
    if (_mi_ptr_page(block) != page) {
      mi_free(block); failure_stage = 5; goto failed;
    }
    fixture->blocks[block_count++] = block;
    if (mi_page_is_full(page)) break;
  }
  full = &theap->pages[MI_BIN_FULL];
  const bool full_before_remote = full != NULL && full->count == 1 && full->first == page
      && mi_page_is_in_full(page) && page->next == NULL && page->prev == NULL;
  const bool direct_empty_before_remote = direct_cache_is_empty(theap);
  if (page == NULL || block_count == 0 || full == NULL || block_count != (size_t)page->reserved
      || page->capacity != page->reserved || page->block_size <= MI_MEDIUM_MAX_OBJ_SIZE
      || page->memid.memkind != MI_MEM_ARENA
      || !mi_page_is_full(page) || !full_before_remote || !direct_empty_before_remote) {
    failure_stage = 6; goto failed;
  }
  fixture->heap = heap;
  fixture->block_count = block_count;
  fixture->full_queue_before_remote = full_before_remote;
  fixture->direct_cache_empty_before_remote = direct_empty_before_remote;
...
int main(void) {
  ...
  mi_page_queue_t* full = NULL;
  size_t bin = 0;
  const size_t request_size = MI_MEDIUM_MAX_OBJ_SIZE + sizeof(void*);
  size_t capacity = 0, reserved = 0, block_size = 0;
  ...
  int stage = 0;
  int arena_backed = 0, large_page = 0, full_before_remote = 0;
  int full_queue_before_remote = 0, direct_cache_empty_before_remote = 0;
  int remote_free_published_before_owner_exit_transition = 0;
  int owner_exit_transition_completed = 0, joined_remote_producer_before_consumer_frees = 0;
  ...
  if (!setup_valid || fixture.heap == NULL || block_count < 3
      || block_count > CRABC_DYNAMIC_FULL_LARGE_ONE_REMOTE_MAX_BLOCKS
      || fixture.blocks[0] == NULL || fixture.blocks[1] == NULL) goto output;
  page = _mi_safe_ptr_page(fixture.blocks[0]);
  if (page == NULL) goto output;
  capacity = page->capacity; reserved = page->reserved; block_size = page->block_size;
  ...
  arena_pages = mi_atomic_load_ptr_acquire(
      mi_arena_pages_t, &fixture.heap->arena_pages[arena->arena_idx]);
  ...
  full_queue_before_remote = fixture.full_queue_before_remote;
  direct_cache_empty_before_remote = fixture.direct_cache_empty_before_remote;
  if (!arena_backed || !large_page || !full_before_remote || !full_queue_before_remote
      || !direct_cache_empty_before_remote || capacity != block_count
      || reserved != 42 || capacity != 42 || block_size != 98304 || slice_count != 64) goto output;
  ...
  if (!remote_free_published_before_owner_exit_transition) goto output;
  if (pthread_mutex_lock(&fixture.mutex) != 0) goto output;
  ...
  owner_exit_transition_completed = fixture.producer_done;
  joined_remote_producer_before_consumer_frees =
      owner_exit_transition_completed && !producer_started && fixture.remote_free_published;
  if (!joined_remote_producer_before_consumer_frees) goto output;
  ...
  size_t page_area_size = 0;
  uint8_t* const page_start = mi_page_area(page, &page_area_size);
  uint8_t* const page_slice_start = mi_page_slice_start(page);
  if (page_start == NULL || page_slice_start == NULL || page_start < page_slice_start) goto output;
  if (page_area_size > MI_LARGE_PAGE_SIZE) {
    page_area_size = MI_LARGE_PAGE_SIZE - MI_ARENA_SLICE_SIZE;
  }
  page_map_slice_count_after_owner_exit = mi_slice_count_of_size(page_area_size)
      + (size_t)((page_start - page_slice_start) / MI_ARENA_SLICE_SIZE);
  if (page_map_slice_count_after_owner_exit == 0
      || page_map_slice_count_after_owner_exit > slice_count) goto output;
  page_map_registered_after_thread_done = true;
  for (size_t index = 0; index < page_map_slice_count_after_owner_exit; index++) {
    if (_mi_safe_ptr_page((const void*)(saved_slice_start + index * MI_ARENA_SLICE_SIZE)) != page) {
      page_map_registered_after_thread_done = false;
      break;
    }
  }
  page_map_tail_unregistered_after_owner_exit = true;
  for (size_t index = page_map_slice_count_after_owner_exit; index < slice_count; index++) {
    if (_mi_safe_ptr_page((const void*)(saved_slice_start + index * MI_ARENA_SLICE_SIZE)) != NULL) {
      page_map_tail_unregistered_after_owner_exit = false;
      break;
    }
  }
  ...
  dynamic_abandoned_count_after_thread_done =
      mi_atomic_load_relaxed(&fixture.heap->abandoned_count[bin]);
  if (!mapped_after_thread_done || !abandoned_after_thread_done
      ...
      || !full_queue_detached_after_thread_done || !dynamic_abandoned_bitmap_set_after_thread_done
      || dynamic_abandoned_count_after_thread_done != 1
      || page_map_slice_count_after_owner_exit != 63
      || !page_map_tail_unregistered_after_owner_exit
  ...
  nonfinal_consumer_free_keeps_mapped =
      page != NULL && mi_page_is_abandoned_mapped(page) && mi_page_is_abandoned(page)
      && page->used + 2 == capacity && page->next == NULL && page->prev == NULL;
  for (size_t index = 0; nonfinal_consumer_free_keeps_mapped
      && index < page_map_slice_count_after_owner_exit; index++) {
    if (_mi_safe_ptr_page((const void*)(saved_slice_start + index * MI_ARENA_SLICE_SIZE)) != page) {
      nonfinal_consumer_free_keeps_mapped = false;
    }
  }
  for (size_t index = page_map_slice_count_after_owner_exit;
      nonfinal_consumer_free_keeps_mapped && index < slice_count; index++) {
    if (_mi_safe_ptr_page((const void*)(saved_slice_start + index * MI_ARENA_SLICE_SIZE)) != NULL) {
      nonfinal_consumer_free_keeps_mapped = false;
    }
  }
  ...
  page_map_unregistered_after_final_free = true;
  ...
  dynamic_abandoned_count_after_final_free =
      mi_atomic_load_relaxed(&fixture.heap->abandoned_count[bin]);
  valid = request_size == 86706 && capacity == 42 && reserved == 42 && block_size == 98304
      && slice_count == 64 && arena_backed && large_page && full_before_remote
      && full_queue_before_remote && direct_cache_empty_before_remote
      && remote_free_published_before_owner_exit_transition && owner_exit_transition_completed
      && joined_remote_producer_before_consumer_frees && mapped_after_thread_done
      ...
      && dynamic_abandoned_count_after_thread_done == 1
      && page_map_slice_count_after_owner_exit == 63
      && page_map_tail_unregistered_after_owner_exit && used_after_force_collect == 41
      && remaining_client_count_after_force_collect == 41 && nonfinal_consumer_free_keeps_mapped
...
output:
  printf("CRABC_MI_DYNAMIC_FULL_LARGE_ONE_REMOTE_EXIT_TRACE_BEGIN\n");
#define OUT_N(k,v) printf("trace.dynamic_full_large_one_remote_exit.%s=%zu\n",k,(size_t)(v))
#define OUT_B(k,v) printf("trace.dynamic_full_large_one_remote_exit.%s=%d\n",k,(v)?1:0)
  OUT_B("arena_backed",arena_backed); OUT_B("large_page",large_page);
  OUT_B("full_before_remote",full_before_remote); OUT_B("full_queue_before_remote",full_queue_before_remote);
  OUT_B("direct_cache_empty_before_remote",direct_cache_empty_before_remote);
  OUT_B("remote_free_published_before_owner_exit_transition",remote_free_published_before_owner_exit_transition);
  OUT_B("owner_exit_transition_completed",owner_exit_transition_completed);
  OUT_B("joined_remote_producer_before_consumer_frees",joined_remote_producer_before_consumer_frees);
  ...
  OUT_N("request_size",request_size); OUT_N("capacity",capacity); OUT_N("reserved",reserved);
  OUT_N("block_size",block_size); OUT_N("slice_count",slice_count);
  OUT_N("page_map_slice_count_after_owner_exit",page_map_slice_count_after_owner_exit);
  ...
  OUT_B("page_map_tail_unregistered_after_owner_exit",page_map_tail_unregistered_after_owner_exit);
  ...
  OUT_B("valid",valid);
  printf("CRABC_MI_DYNAMIC_FULL_LARGE_ONE_REMOTE_EXIT_TRACE_END\n");
  if (producer_started) {
    if (pthread_mutex_lock(&fixture.mutex) == 0) {
      fixture.allow_thread_done = true; (void)pthread_cond_broadcast(&fixture.condition);
      (void)pthread_mutex_unlock(&fixture.mutex);
    }
  ...
  if (!valid) fprintf(stderr, "dynamic full-large one-remote force-collect fixture stopped at stage %d\n", stage);
  return valid ? 0 : 2;
}
'''


DYNAMIC_FULL_LARGE_ONE_REMOTE_EXIT_KIND = (
    "mimalloc-x86_64-dynamic-full-large-one-remote-force-collect-to-mapped-differential-evidence"
)
...
def _schema_template() -> dict:
    ...
    value["schema"] = "crabc-mimalloc-x86_64-dynamic-full-large-one-remote-force-collect-to-mapped-evidence"
...
def validate_c_command(command: list[str], schema: dict) -> None:
    ...
    if definitions != list(EXPECTED_COMPILE_DEFINITIONS) or definitions != list(
        ...
        raise EvidenceError("dynamic-full-large-one-remote C command compile definitions drifted")
    if (
        flags != list(schema["release_flags"])
        or "-pthread" not in command
        or "-ftls-model=initial-exec" not in command
    ):
        raise EvidenceError("dynamic-full-large-one-remote C command release pthread/TLS selection drifted")
...
def validate_normalized_c_command(command: object, schema: dict) -> None:
    expected = [
        ...
        *schema["release_flags"],
        f"{NORMALIZED_EVIDENCE_ROOT}/dynamic-full-large-one-remote-force-collect-to-mapped.c",
    ...
    if (
        ...
        raise EvidenceError("dynamic-full-large-one-remote report C command drifted")


def build_c_trace(
    compiler: str, readelf: str, source: Path, temporary: Path, schema: dict
) -> dict:
    probe_source = temporary / "dynamic-full-large-one-remote-force-collect-to-mapped.c"
    probe_binary = temporary / "dynamic-full-large-one-remote-force-collect-to-mapped-c"
    ...
    try:
        _base.run.require_success(
            ...
            "pinned C dynamic-full-large-one-remote fixture build",
        )
        header = _base.run.command_record((readelf, "-h", str(probe_binary)), cwd=source)
        _base.run.require_success(header, "pinned C dynamic-full-large-one-remote fixture ELF identity")
        elf = _base.run.parse_elf_identity(str(header["stdout"]), "x86_64")
        execution = _base.run.command_record((str(probe_binary),), cwd=source)
        _base.run.require_success(execution, "pinned C dynamic-full-large-one-remote fixture execution")
    except _base.run.HarnessError as error:
        raise EvidenceError(str(error)) from error
    trace = parse_trace(str(execution["stdout"]), description="pinned C dynamic-full-large-one-remote trace")
    validate_trace(trace, description="pinned C dynamic-full-large-one-remote trace")
    return {
...
def report_from_results(**kwargs):
    ...
    report["kind"] = DYNAMIC_FULL_LARGE_ONE_REMOTE_EXIT_KIND
    validate_report(report)
    return report


def validate_report(report: dict) -> None:
    if report.get("kind") != DYNAMIC_FULL_LARGE_ONE_REMOTE_EXIT_KIND:
        raise EvidenceError("dynamic-full-large-one-remote report kind drifted")
    ...
    if c_probe.get("source_sha256") != sha256_bytes(C_TRACE_PROBE.encode("utf-8")):
        raise EvidenceError("dynamic-full-large-one-remote report C source hash drifted")
    ...
    compatible["c_probe"]["build_command"] = [
        part.replace("dynamic-full-large-one-remote-force-collect-to-mapped", "regular-small")
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"allocator x86-64 dynamic-full-large-one-remote differential: FAIL: {error}",
            file=os.sys.stderr,
        )
        return 1
    print(
        "allocator x86-64 dynamic-full-large-one-remote differential: PASS "
