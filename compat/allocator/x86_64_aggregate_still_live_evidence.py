#!/usr/bin/env python3
"""Differential evidence for the aggregate post-exit ``StillLive`` edge.

This native Linux/x86-64 private-engine lane compares one real pinned-C
producer with the bounded Rust aggregate route.  The producer leaves two
clients on one nonfull medium page and one client on a distinct-bin nonfull
medium page, calls ``mi_thread_done()``, and returns.  The joined consumer
then frees the first page's first client (the aggregate route remains live),
the second page's client (one selected terminal release), and the first
page's survivor (the final terminal release).

The fixture is deliberately private engine evidence.  It neither enables a
public x86 API nor establishes general teardown, routing, reclamation, or
...
SCHEMA_PATH = ROOT / "compat/allocator/x86_64-aggregate-still-live-evidence-v3.5.0.json"
REPORT_DEFAULT = ROOT / "compat/reports/allocator/x86_64/aggregate-still-live.json"
LOCKFILE = ROOT / "Cargo.lock"
RUST_TEST_SOURCE = ROOT / "crabc-mimalloc/src/main_heap_page.rs"
TARGET = "x86_64-unknown-linux-musl"
RUST_TEST_FILTER = "main_heap_page::tests::x86_64_aggregate_still_live_trace_matches_pinned_c_protocol"
TRACE_BEGIN = "CRABC_MI_AGGREGATE_STILL_LIVE_TRACE_BEGIN"
TRACE_END = "CRABC_MI_AGGREGATE_STILL_LIVE_TRACE_END"
...
class EvidenceError(RuntimeError):
    """The bounded aggregate StillLive differential could not establish its claim."""
...
EXPECTED_PROFILE = "linux-x86_64-private-aggregate-still-live"
EXPECTED_SCOPE = {
    ...
    "one_two_client_nonfull_medium_page_and_one_distinct_one_client_page_only": True,
...
# The source ranges bind the exact public allocation/free entry points, local
# Theap teardown, aggregate abandoned-page free tail, same-origin-only reclaim
# selection, active two-level PageMap lookup, separately-created heap lifetime,
# and every private arena bitmap and terminal release observation made by the
# C fixture.
...
TRACE_FIELDS = (
    ...
    "abandoned_counts_one_one_after_join",
    "route_two_pages_after_first_free",
    ...
    "abandoned_counts_one_one_after_first_free",
    "route_one_page_after_second_free",
    ...
    "abandoned_counts_one_zero_after_second_free",
    ...
    "abandoned_counts_zero_zero_after_final_free",
    "route_empty_after_final_free",
    "valid",
)
EXPECTED_TRACE_VALUES = {
    f"trace.aggregate_still_live.{field}": 1 for field in TRACE_FIELDS
}
...
static void* producer_main(void* arg) {
  ...
  first_client = mi_heap_malloc(heap, MI_SMALL_MAX_OBJ_SIZE + 1);
  first_survivor = mi_heap_malloc(heap, MI_SMALL_MAX_OBJ_SIZE + 1);
  second_client = mi_heap_malloc(heap, MI_MEDIUM_MAX_OBJ_SIZE / 2);
  if (first_client == NULL || first_survivor == NULL || second_client == NULL) goto fail;
  first_page = _mi_ptr_page(first_client);
  second_page = _mi_ptr_page(second_client);
  if (first_page == NULL || second_page == NULL
      || first_client == first_survivor
  ...
  f->distinct_bins = (f->first_bin != f->second_bin);
  ...
  if (f->arena == NULL || f->arena->arena_idx >= MI_MAX_ARENAS
      || f->first_bin >= MI_ARENA_BIN_COUNT || f->second_bin >= MI_ARENA_BIN_COUNT
      || !f->arena_backed || !f->both_medium || !f->first_distinct_clients_share_page
      || !f->distinct_pages || !f->distinct_bins || !f->first_used_two
  ...
  // All fallible metadata work completed. This separately-created heap and
  // its arena-pages table remain owned until the explicit mi_heap_destroy()
  // below; real source teardown only destroys the producer Theap/TLD. Only
  // stable integer page identities and those still-owned arena structures
  // cross that teardown boundary.
  f->heap = heap;
  ...
  // Gate every post-teardown page metadata read through the active PageMap.
  // The pointer identities are integers, so no stale producer page is
  // dereferenced if teardown unexpectedly released either span.
  ...
  f->route_two_pages_before_join = (
      ...
      && mi_atomic_load_relaxed(&heap->abandoned_count[f->first_bin]) == 1
      && mi_atomic_load_relaxed(&heap->abandoned_count[f->second_bin]) == 1);
...
int main(void) {
  ...
  int distinct_pages = 0, distinct_bins = 0, first_used_two_before_exit = 0;
  int second_used_one_before_exit = 0, first_nonfull_before_exit = 0;
  int second_nonfull_before_exit = 0, slice_spans_nonempty_and_disjoint = 0;
  int pages_share_paired_arena = 0, route_two_pages_before_join = 0;
  ...
  int abandoned_counts_one_one_after_join = 0;
  int route_two_pages_after_first_free = 0;
  int first_page_map_registered_after_first_free = 0;
  int first_arena_page_bitmap_set_after_first_free = 0;
  int first_mapped_abandoned_after_first_free = 0;
  int first_used_one_after_first_free = 0;
  int second_page_map_registered_after_first_free = 0;
  int second_arena_page_bitmap_set_after_first_free = 0;
  int second_mapped_abandoned_after_first_free = 0;
  int second_used_one_after_first_free = 0;
  int abandoned_counts_one_one_after_first_free = 0;
  int route_one_page_after_second_free = 0;
  ...
  int abandoned_counts_one_zero_after_second_free = 0;
  ...
  int abandoned_counts_zero_zero_after_final_free = 0;
  int route_empty_after_final_free = 0;

  mi_thread_init();
  // Keep the selected source condition deterministic even when the native
  // environment supplied a different option. Value 0 permits only an origin
  // Theap reclaim; after `mi_thread_done` that heap-specific Theap must be
  // unavailable to the joined consumer, selecting the no-reclaim StillLive
  // tail instead of policy-disabling the attempt outright.
  ...
  first_arena_page_bitmap_set_after_join = mi_bitmap_is_setN(
      f.arena_pages->pages, f.first_slice, 1);
  second_arena_page_bitmap_set_after_join = mi_bitmap_is_setN(
  ...
  abandoned_counts_one_one_after_join = (
      mi_atomic_load_relaxed(&f.heap->abandoned_count[f.first_bin]) == 1
      && mi_atomic_load_relaxed(&f.heap->abandoned_count[f.second_bin]) == 1);
  if (!arena_backed || !both_medium || !first_distinct_clients_share_page || !distinct_pages
      || !distinct_bins || !first_used_two_before_exit || !second_used_one_before_exit
      ...
      || !second_mapped_abandoned_after_join || !abandoned_counts_one_one_after_join
      || !_mi_thread_is_initialized()
  ...
  // A1 is the only first-page client freed here. The first-survivor PageMap
  // lookup below is the lifetime gate before any post-free metadata read.
  ...
  first_arena_page_bitmap_set_after_first_free = mi_bitmap_is_setN(
      f.arena_pages->pages, f.first_slice, 1);
  ...
  second_arena_page_bitmap_set_after_first_free = mi_bitmap_is_setN(
      f.arena_pages->pages, f.second_slice, 1);
  ...
  if (!route_two_pages_after_first_free) {
    f.first_survivor = NULL;
    f.second_client = NULL;
    goto done;
  }

  // B is selectively terminal while A2 remains the one live aggregate page.
  ...
  route_one_page_after_second_free = (
      ...
      && first_used_one_after_second_free
      && abandoned_counts_one_zero_after_second_free);
  if (!route_one_page_after_second_free) {
    f.first_survivor = NULL;
    goto done;
  }
  ...
  f.first_survivor = NULL;
  // No page pointer is dereferenced after A2's terminal release.
  first_page_map_unregistered_after_final_free = (
  ...
  valid = arena_backed && both_medium && first_distinct_clients_share_page && distinct_pages
      ...
      && second_arena_page_bitmap_set_after_join && first_mapped_abandoned_after_join
      && second_mapped_abandoned_after_join && abandoned_counts_one_one_after_join
      && route_two_pages_after_first_free && first_page_map_registered_after_first_free
      && first_arena_page_bitmap_set_after_first_free && first_mapped_abandoned_after_first_free
      && first_used_one_after_first_free && second_page_map_registered_after_first_free
      && second_arena_page_bitmap_set_after_first_free && second_mapped_abandoned_after_first_free
      && second_used_one_after_first_free && abandoned_counts_one_one_after_first_free
      && route_one_page_after_second_free && second_page_map_unregistered_after_second_free
      ...
      && abandoned_counts_one_zero_after_second_free && first_page_map_unregistered_after_final_free
      && first_arena_page_bitmap_clear_after_final_free && first_arena_slice_released_after_final_free
      && abandoned_counts_zero_zero_after_final_free && route_empty_after_final_free;

done:
  if (started) pthread_join(worker, NULL);
  // A failed post-teardown map gate intentionally leaks rather than passes a
  // potentially stale client pointer to `mi_free` or `mi_heap_destroy`.
  ...
  if (valid) {
    printf("CRABC_MI_AGGREGATE_STILL_LIVE_TRACE_BEGIN\n");
    printf("trace.aggregate_still_live.arena_backed=%d\n", arena_backed);
    printf("trace.aggregate_still_live.both_medium=%d\n", both_medium);
    printf("trace.aggregate_still_live.first_distinct_clients_share_page=%d\n", first_distinct_clients_share_page);
    printf("trace.aggregate_still_live.distinct_pages=%d\n", distinct_pages);
    printf("trace.aggregate_still_live.distinct_bins=%d\n", distinct_bins);
    printf("trace.aggregate_still_live.first_used_two_before_exit=%d\n", first_used_two_before_exit);
    printf("trace.aggregate_still_live.second_used_one_before_exit=%d\n", second_used_one_before_exit);
    printf("trace.aggregate_still_live.first_nonfull_before_exit=%d\n", first_nonfull_before_exit);
    printf("trace.aggregate_still_live.second_nonfull_before_exit=%d\n", second_nonfull_before_exit);
    printf("trace.aggregate_still_live.slice_spans_nonempty_and_disjoint=%d\n", slice_spans_nonempty_and_disjoint);
    printf("trace.aggregate_still_live.pages_share_paired_arena=%d\n", pages_share_paired_arena);
    printf("trace.aggregate_still_live.route_two_pages_before_join=%d\n", route_two_pages_before_join);
    printf("trace.aggregate_still_live.producer_teardown_completed_before_consumer_free=%d\n", producer_teardown_completed_before_consumer_free);
    printf("trace.aggregate_still_live.consumer_joined_before_first_free=%d\n", consumer_joined_before_first_free);
    printf("trace.aggregate_still_live.first_page_map_registered_after_join=%d\n", first_page_map_registered_after_join);
    printf("trace.aggregate_still_live.second_page_map_registered_after_join=%d\n", second_page_map_registered_after_join);
    printf("trace.aggregate_still_live.first_arena_page_bitmap_set_after_join=%d\n", first_arena_page_bitmap_set_after_join);
    printf("trace.aggregate_still_live.second_arena_page_bitmap_set_after_join=%d\n", second_arena_page_bitmap_set_after_join);
    printf("trace.aggregate_still_live.first_mapped_abandoned_after_join=%d\n", first_mapped_abandoned_after_join);
    printf("trace.aggregate_still_live.second_mapped_abandoned_after_join=%d\n", second_mapped_abandoned_after_join);
    printf("trace.aggregate_still_live.abandoned_counts_one_one_after_join=%d\n", abandoned_counts_one_one_after_join);
    printf("trace.aggregate_still_live.route_two_pages_after_first_free=%d\n", route_two_pages_after_first_free);
    printf("trace.aggregate_still_live.first_page_map_registered_after_first_free=%d\n", first_page_map_registered_after_first_free);
    printf("trace.aggregate_still_live.first_arena_page_bitmap_set_after_first_free=%d\n", first_arena_page_bitmap_set_after_first_free);
    printf("trace.aggregate_still_live.first_mapped_abandoned_after_first_free=%d\n", first_mapped_abandoned_after_first_free);
    printf("trace.aggregate_still_live.first_used_one_after_first_free=%d\n", first_used_one_after_first_free);
    printf("trace.aggregate_still_live.second_page_map_registered_after_first_free=%d\n", second_page_map_registered_after_first_free);
    printf("trace.aggregate_still_live.second_arena_page_bitmap_set_after_first_free=%d\n", second_arena_page_bitmap_set_after_first_free);
    printf("trace.aggregate_still_live.second_mapped_abandoned_after_first_free=%d\n", second_mapped_abandoned_after_first_free);
    printf("trace.aggregate_still_live.second_used_one_after_first_free=%d\n", second_used_one_after_first_free);
    printf("trace.aggregate_still_live.abandoned_counts_one_one_after_first_free=%d\n", abandoned_counts_one_one_after_first_free);
    printf("trace.aggregate_still_live.route_one_page_after_second_free=%d\n", route_one_page_after_second_free);
    printf("trace.aggregate_still_live.second_page_map_unregistered_after_second_free=%d\n", second_page_map_unregistered_after_second_free);
    printf("trace.aggregate_still_live.second_arena_page_bitmap_clear_after_second_free=%d\n", second_arena_page_bitmap_clear_after_second_free);
    printf("trace.aggregate_still_live.second_arena_slice_released_after_second_free=%d\n", second_arena_slice_released_after_second_free);
    printf("trace.aggregate_still_live.first_page_map_registered_after_second_free=%d\n", first_page_map_registered_after_second_free);
    printf("trace.aggregate_still_live.first_arena_page_bitmap_set_after_second_free=%d\n", first_arena_page_bitmap_set_after_second_free);
    printf("trace.aggregate_still_live.first_mapped_abandoned_after_second_free=%d\n", first_mapped_abandoned_after_second_free);
    printf("trace.aggregate_still_live.first_used_one_after_second_free=%d\n", first_used_one_after_second_free);
    printf("trace.aggregate_still_live.abandoned_counts_one_zero_after_second_free=%d\n", abandoned_counts_one_zero_after_second_free);
    printf("trace.aggregate_still_live.first_page_map_unregistered_after_final_free=%d\n", first_page_map_unregistered_after_final_free);
    printf("trace.aggregate_still_live.first_arena_page_bitmap_clear_after_final_free=%d\n", first_arena_page_bitmap_clear_after_final_free);
    printf("trace.aggregate_still_live.first_arena_slice_released_after_final_free=%d\n", first_arena_slice_released_after_final_free);
    printf("trace.aggregate_still_live.abandoned_counts_zero_zero_after_final_free=%d\n", abandoned_counts_zero_zero_after_final_free);
    printf("trace.aggregate_still_live.route_empty_after_final_free=%d\n", route_empty_after_final_free);
    printf("trace.aggregate_still_live.valid=%d\n", valid);
    printf("CRABC_MI_AGGREGATE_STILL_LIVE_TRACE_END\n");
  }
  return valid ? 0 : 2;
}
'''
...
def load_schema(path: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise EvidenceError("cannot read aggregate-StillLive schema") from error
    required = {
    ...
    if not isinstance(schema, dict) or set(schema) != required or type(schema["format"]) is not int or schema["format"] != 1:
        raise EvidenceError("aggregate-StillLive schema fields drifted")
    if schema["schema"] != "crabc-mimalloc-x86_64-aggregate-still-live-evidence" or schema["profile"] != EXPECTED_PROFILE:
        raise EvidenceError("aggregate-StillLive schema identity drifted")
    if not exactly_matches(schema["target"], EXPECTED_TARGET) or not exactly_matches(schema["upstream"], EXPECTED_UPSTREAM) or not exactly_matches(schema["scope"], EXPECTED_SCOPE):
        raise EvidenceError("aggregate-StillLive boundary drifted")
    try:
        pin = run.load_pin()
    except run.HarnessError as error:
        raise EvidenceError("cannot validate pinned aggregate-StillLive upstream") from error
    ...
    for anchor in anchors:
        ...
        if (type(anchor["member"]) is not str or type(anchor["start_line"]) is not int
            ...
            raise EvidenceError("aggregate-StillLive source anchor type drifted")
...
def validate_source_anchors(schema: Mapping[str, Any], source: Path) -> list[dict[str, Any]]:
    ...
    for anchor in schema["source_anchors"]:
        ...
        if digest != anchor["sha256"]:
            raise EvidenceError(f"aggregate-StillLive source anchor drifted: {anchor['member']}")
...
def validate_c_command(command: Sequence[str], schema: Mapping[str, Any]) -> None:
    if [item for item in command if item in EXPECTED_COMPILE_DEFINITIONS] != list(schema["compile_definitions"]):
        raise EvidenceError("aggregate-StillLive C compile definitions drifted")
    if [item for item in command if item in run.CONFIGURATION_PROFILES["release"]] != list(schema["release_flags"]):
        raise EvidenceError("aggregate-StillLive C release flags drifted")
    if "-pthread" not in command or "-ftls-model=initial-exec" not in command:
        raise EvidenceError("aggregate-StillLive C command lacks pthread/TLS requirements")
...
def validate_normalized_c_command(command: object, schema: Mapping[str, Any]) -> None:
    expected = [
        ...
        *schema["release_flags"], f"{NORMALIZED_EVIDENCE_ROOT}/aggregate-still-live.c",
        *(f"{NORMALIZED_PINNED_SOURCE}/{member}" for member in schema["release_source_set"]),
        "-pthread", "-o", f"{NORMALIZED_EVIDENCE_ROOT}/aggregate-still-live-c",
    ]
    if not isinstance(command, list) or not command or Path(command[0]).name != "musl-gcc" or command[1:] != expected:
        raise EvidenceError("aggregate-StillLive normalized C command drifted")
...
def validate_normalized_rust_command(command: object) -> None:
    ...
    if not isinstance(command, list) or not command or Path(command[0]).name != "cargo" or command[1:] != expected:
        raise EvidenceError("aggregate-StillLive normalized Rust command drifted")


def build_c_trace(compiler: str, readelf: str, source: Path, temporary: Path, schema: Mapping[str, Any]) -> dict[str, Any]:
    probe = temporary / "aggregate-still-live.c"
    binary = temporary / "aggregate-still-live-c"
    ...
    try:
        run.require_success(run.command_record(command, cwd=source), "aggregate-StillLive C build")
        header = run.command_record((readelf, "-h", str(binary)), cwd=source)
        run.require_success(header, "aggregate-StillLive C ELF identity")
        elf = run.parse_elf_identity(str(header["stdout"]), "x86_64")
        execution = run.command_record((str(binary),), cwd=source)
        run.require_success(execution, "aggregate-StillLive C execution")
    except run.HarnessError as error:
        raise EvidenceError(str(error)) from error
    return {
        "build_command": normalize_command(command, temporary, source),
        "elf": elf,
        "run_command": [f"{NORMALIZED_EVIDENCE_ROOT}/aggregate-still-live-c"],
        "source_sha256": sha256_bytes(C_TRACE_PROBE.encode("utf-8")),
        "trace": parse_trace(str(execution["stdout"]), description="pinned C aggregate-StillLive trace"),
    }
...
def build_rust_trace(cargo: str, temporary: Path) -> dict[str, Any]:
    ...
    try:
        ...
        run.require_success(execution, "Rust aggregate-StillLive fixture")
        passed = run.parse_rust_test_count(str(execution["stdout"]) + "\n" + str(execution["stderr"]))
    except run.HarnessError as error:
        raise EvidenceError(str(error)) from error
    if passed != 1:
        raise EvidenceError(f"Rust aggregate-StillLive fixture passed {passed} tests")
    return {
        ...
        "trace": parse_trace(str(execution["stdout"]) + "\n" + str(execution["stderr"]), description="Rust aggregate-StillLive trace"),
    }


def compare_traces(c_trace: Mapping[str, int], rust_trace: Mapping[str, int]) -> dict[str, Any]:
    validate_trace(c_trace, description="pinned C aggregate-StillLive trace")
    validate_trace(rust_trace, description="Rust aggregate-StillLive trace")
    mismatch = [key for key in EXPECTED_TRACE_VALUES if c_trace[key] != rust_trace[key]]
    if mismatch:
        raise EvidenceError("C/Rust aggregate-StillLive mismatch: " + ", ".join(mismatch))
...
def validate_report(report: Mapping[str, Any]) -> None:
    ...
    if not isinstance(report, dict) or set(report) != required or report["format"] != 1 or report["status"] != "passed":
        raise EvidenceError("aggregate-StillLive report shape/status drifted")
    if report["kind"] != "mimalloc-x86_64-aggregate-still-live-differential-evidence" or report["profile"] != EXPECTED_PROFILE:
        raise EvidenceError("aggregate-StillLive report identity drifted")
    if not exactly_matches(report["target"], EXPECTED_TARGET) or not exactly_matches(report["upstream"], EXPECTED_UPSTREAM) or not exactly_matches(report["scope"], EXPECTED_SCOPE):
        raise EvidenceError("aggregate-StillLive report boundary drifted")
    if report["provenance"] not in ({"execution_mode": "native", "host_architecture": "x86_64"}, {"execution_mode": "native", "host_architecture": "amd64"}):
        raise EvidenceError("aggregate-StillLive report lacks native provenance")
    schema = load_schema()
    if not exactly_matches(report["trace"], schema["trace"]):
        raise EvidenceError("aggregate-StillLive report trace drifted")
    ...
    if not isinstance(source, dict) or set(source) != {"archive_sha256", "anchors", "release_flags", "release_source_set"} or source["archive_sha256"] != run.load_pin()["sha256"] or not exactly_matches(source["anchors"], schema["source_anchors"]) or not exactly_matches(source["release_flags"], schema["release_flags"]) or not exactly_matches(source["release_source_set"], schema["release_source_set"]):
        raise EvidenceError("aggregate-StillLive report source drifted")
    ...
    if not isinstance(c_probe, dict) or set(c_probe) != {"build_command", "elf", "run_command", "source_sha256", "trace"}:
        raise EvidenceError("aggregate-StillLive C probe shape drifted")
    if not isinstance(rust_probe, dict) or set(rust_probe) != {"cargo_command", "lockfile", "passed_test_count", "source", "target_dir", "trace"}:
        raise EvidenceError("aggregate-StillLive Rust probe shape drifted")
    validate_normalized_c_command(c_probe["build_command"], schema)
    validate_normalized_rust_command(rust_probe["cargo_command"])
    if not exactly_matches(c_probe["elf"], EXPECTED_C_ELF) or c_probe["run_command"] != [f"{NORMALIZED_EVIDENCE_ROOT}/aggregate-still-live-c"] or c_probe["source_sha256"] != sha256_bytes(C_TRACE_PROBE.encode("utf-8")):
        raise EvidenceError("aggregate-StillLive C probe identity drifted")
    if rust_probe["passed_test_count"] != 1 or not exactly_matches(rust_probe["target_dir"], {"isolated": True, "retained": False, "value": f"{NORMALIZED_EVIDENCE_ROOT}/rust-target"}):
        raise EvidenceError("aggregate-StillLive Rust result drifted")
    if not exactly_matches(rust_probe["lockfile"], {"path": relative(LOCKFILE), "sha256": sha256_file(LOCKFILE)}) or not exactly_matches(rust_probe["source"], {"path": relative(RUST_TEST_SOURCE), "sha256": sha256_file(RUST_TEST_SOURCE)}):
        raise EvidenceError("aggregate-StillLive Rust provenance drifted")
    ...
    if not exactly_matches(report["comparison"], compare_traces(c_probe["trace"], rust_probe["trace"])):
        raise EvidenceError("aggregate-StillLive comparison drifted")
...
def run_evidence(*, offline: bool, report_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise EvidenceError(str(error)) from error
    with tempfile.TemporaryDirectory(prefix="crabc-mimalloc-x86_64-aggregate-still-live-") as name:
        temporary = Path(name)
        try:
...
def main(arguments: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"allocator x86-64 aggregate-StillLive differential: FAIL: {error}", file=os.sys.stderr)
        return 1
    print(
        "allocator x86-64 aggregate-StillLive differential: PASS "
