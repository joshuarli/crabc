#!/usr/bin/env python3
"""Differentially prove one dynamic full-large unmapped exit route.

The native pinned-C fixture fills one arena-backed 98304-byte large page on
...
sequential frees release the full 64-slice arena span and its PageMap state.
...
SCHEMA_PATH = ROOT / "compat/allocator/x86_64-dynamic-full-large-unmapped-reabandon-evidence-v3.5.0.json"
REPORT_DEFAULT = ROOT / "compat/reports/allocator/x86_64/dynamic-full-large-unmapped-reabandon.json"
RUST_TEST_SOURCE = ROOT / "crabc-mimalloc/src/dynamic_theap.rs"
EXPECTED_PROFILE = "linux-x86_64-private-dynamic-full-large-unmapped-reabandon"
RUST_TEST_FILTER = (
    "dynamic_theap::tests::"
    "x86_64_dynamic_full_large_unmapped_reabandon_trace_matches_pinned_c"
)
TRACE_BEGIN = "CRABC_MI_DYNAMIC_FULL_LARGE_UNMAPPED_EXIT_TRACE_BEGIN"
TRACE_END = "CRABC_MI_DYNAMIC_FULL_LARGE_UNMAPPED_EXIT_TRACE_END"
EXPECTED_SCOPE = {
...
EXPECTED_TRACE_VALUES = {
    "trace.dynamic_full_large_unmapped_exit.arena_backed": 1,
    "trace.dynamic_full_large_unmapped_exit.large_page": 1,
    "trace.dynamic_full_large_unmapped_exit.full_before_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.full_queue_before_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.direct_cache_empty_before_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.no_remote_free_before_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.unmapped_after_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.abandoned_after_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.page_map_registered_after_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.page_map_slice_count_after_thread_done": 63,
    "trace.dynamic_full_large_unmapped_exit.page_map_tail_unregistered_after_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.arena_page_bitmap_set_after_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.full_queue_detached_after_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.dynamic_abandoned_bitmap_clear_after_thread_done": 1,
    "trace.dynamic_full_large_unmapped_exit.dynamic_abandoned_count_after_thread_done": 0,
    "trace.dynamic_full_large_unmapped_exit.request_size": 86706,
    "trace.dynamic_full_large_unmapped_exit.capacity": 42,
    "trace.dynamic_full_large_unmapped_exit.reserved": 42,
    "trace.dynamic_full_large_unmapped_exit.block_size": 98304,
    "trace.dynamic_full_large_unmapped_exit.slice_count": 64,
    "trace.dynamic_full_large_unmapped_exit.used_after_thread_done": 42,
    "trace.dynamic_full_large_unmapped_exit.unmapped_prefix_free_count": 5,
    "trace.dynamic_full_large_unmapped_exit.used_after_unmapped_prefix": 37,
    "trace.dynamic_full_large_unmapped_exit.unmapped_after_unmapped_prefix": 1,
    "trace.dynamic_full_large_unmapped_exit.mapped_after_reabandon_boundary": 1,
    "trace.dynamic_full_large_unmapped_exit.dynamic_abandoned_bitmap_set_after_reabandon_boundary": 1,
    "trace.dynamic_full_large_unmapped_exit.dynamic_abandoned_count_after_reabandon_boundary": 1,
    "trace.dynamic_full_large_unmapped_exit.used_after_reabandon_boundary": 36,
    "trace.dynamic_full_large_unmapped_exit.page_map_unregistered_after_final_free": 1,
    "trace.dynamic_full_large_unmapped_exit.arena_page_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_large_unmapped_exit.arena_slice_released_after_final_free": 1,
    "trace.dynamic_full_large_unmapped_exit.dynamic_abandoned_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_large_unmapped_exit.dynamic_abandoned_count_after_final_free": 0,
    "trace.dynamic_full_large_unmapped_exit.valid": 1,
}
...
#error this private dynamic full-large unmapped fixture requires native Linux/x86_64
#endif
...
#endif
#if !defined(MI_TLS_MODEL_LOCAL) || MI_TLS_MODEL_LOCAL != 1 \
    || defined(MI_TLS_MODEL_PTHREADS) || defined(MI_TLS_MODEL_FIXED) \
    || defined(MI_TLS_MODEL_WIN32)
#error this fixture requires pinned Linux local compiler TLS
#endif
#if defined(MI_USE_BUILTIN_THREAD_POINTER) || defined(MI_PRIM_THREAD_POINTER) \
    || defined(MI_NO_THREAD_POINTER) || !defined(MI_HAS_TLS_SLOT) \
    || MI_HAS_TLS_SLOT != 1 || MI_INTPTR_SIZE != 8
#error this fixture requires the pinned x86_64 FS TLS-slot fallback
#endif

#define CRABC_DYNAMIC_FULL_LARGE_UNMAPPED_MAX_BLOCKS 64
...
typedef struct fixture_s {
  ...
  mi_heap_t* heap;
  void* blocks[CRABC_DYNAMIC_FULL_LARGE_UNMAPPED_MAX_BLOCKS];
  size_t block_count;
  bool setup_valid;
  bool worker_ready;
...
static void* producer_main(void* argument) {
  ...
  while (block_count < CRABC_DYNAMIC_FULL_LARGE_UNMAPPED_MAX_BLOCKS) {
...
int main(void) {
  ...
  int stage = 0;
  int arena_backed = 0, large_page = 0, full_before_thread_done = 0;
  ...
  size_t page_map_slice_count_after_thread_done = 0;
  int page_map_tail_unregistered_after_thread_done = 0;
  ...
  if (!setup_valid || fixture.heap == NULL || block_count < 8
      || block_count > CRABC_DYNAMIC_FULL_LARGE_UNMAPPED_MAX_BLOCKS
  ...
  if (!arena_backed || !large_page || !full_before_thread_done
      ...
      || request_size != 86706 || capacity != 42 || reserved != 42
      || block_size != 98304 || slice_count != 64) goto output;
  stage = 4;
  if (pthread_mutex_lock(&fixture.mutex) != 0) goto output;
  ...
  page_map_slice_count_after_thread_done = mi_slice_count_of_size(page_area_size)
      + (size_t)((page_start - page_slice_start) / MI_ARENA_SLICE_SIZE);
  page_map_registered_after_thread_done = page_map_slice_count_after_thread_done > 0
      && page_map_slice_count_after_thread_done <= slice_count;
  for (size_t index = 0; index < page_map_slice_count_after_thread_done; index++) {
    if (_mi_safe_ptr_page((const void*)(saved_slice_start + index * MI_ARENA_SLICE_SIZE)) != page) {
      page_map_registered_after_thread_done = 0;
      break;
    }
  }
  page_map_tail_unregistered_after_thread_done = 1;
  for (size_t index = page_map_slice_count_after_thread_done; index < slice_count; index++) {
    if (_mi_safe_ptr_page((const void*)(saved_slice_start + index * MI_ARENA_SLICE_SIZE)) != NULL) {
      page_map_tail_unregistered_after_thread_done = 0;
      break;
    }
  }
  ...
  if (!unmapped_after_thread_done || !abandoned_after_thread_done
      ...
      || used_after_thread_done != capacity
      || page_map_slice_count_after_thread_done != 63
      || !page_map_tail_unregistered_after_thread_done) goto output;
  stage = 6;
  /* Large frees use the source full collector.  With reserved == 42,
  ...
  valid = request_size == 86706 && capacity == 42 && reserved == 42 && block_size == 98304
      && slice_count == 64 && arena_backed && large_page && full_before_thread_done
      && full_queue_before_thread_done && direct_cache_empty_before_thread_done
  ...
  stage = 9;
output:
  printf("CRABC_MI_DYNAMIC_FULL_LARGE_UNMAPPED_EXIT_TRACE_BEGIN\n");
#define OUT_N(k, v) printf("trace.dynamic_full_large_unmapped_exit.%s=%zu\n", k, (size_t)(v))
#define OUT_B(k, v) printf("trace.dynamic_full_large_unmapped_exit.%s=%d\n", k, (v) ? 1 : 0)
  OUT_B("arena_backed", arena_backed);
  OUT_B("large_page", large_page);
  ...
  OUT_N("page_map_slice_count_after_thread_done", page_map_slice_count_after_thread_done);
  OUT_B("page_map_tail_unregistered_after_thread_done", page_map_tail_unregistered_after_thread_done);
  ...
  OUT_B("valid", valid);
  printf("CRABC_MI_DYNAMIC_FULL_LARGE_UNMAPPED_EXIT_TRACE_END\n");
  if (producer_started) {
  ...
  if (!valid) {
    fprintf(stderr,
        "dynamic full-large unmapped reabandon fixture stopped at stage %d (worker=%d)\n",
...
DYNAMIC_FULL_LARGE_UNMAPPED_EXIT_KIND = (
    "mimalloc-x86_64-dynamic-full-large-unmapped-reabandon-differential-evidence"
)
...
def _schema_template() -> dict:
    ...
    value["schema"] = "crabc-mimalloc-x86_64-dynamic-full-large-unmapped-reabandon-evidence"
...
def validate_c_command(command: list[str], schema: dict) -> None:
    ...
    if schema.get("tls") != EXPECTED_TLS:
        raise EvidenceError("dynamic-full-large-unmapped C TLS contract drifted")
    if definitions != list(EXPECTED_COMPILE_DEFINITIONS) or definitions != list(
        schema["compile_definitions"]
    ):
        raise EvidenceError("dynamic-full-large-unmapped C command compile definitions drifted")
    if (
        ...
        raise EvidenceError(
            "dynamic-full-large-unmapped C command release pthread/TLS selection drifted"
        )
...
def validate_normalized_c_command(command: object, schema: dict) -> None:
    ...
    if (
        ...
        raise EvidenceError("dynamic-full-large-unmapped report C command drifted")
...
def build_c_trace(
    ...
    try:
        _base.run.require_success(
            ...
            "pinned C dynamic-full-large-unmapped fixture build",
        )
        header = _base.run.command_record((readelf, "-h", str(probe_binary)), cwd=source)
        _base.run.require_success(
            header, "pinned C dynamic-full-large-unmapped fixture ELF identity"
        )
        elf = _base.run.parse_elf_identity(str(header["stdout"]), "x86_64")
        execution = _base.run.command_record((str(probe_binary),), cwd=source)
        _base.run.require_success(
            execution, "pinned C dynamic-full-large-unmapped fixture execution"
        )
    except _base.run.HarnessError as error:
        raise EvidenceError(str(error)) from error
    trace = parse_trace(
        str(execution["stdout"]), description="pinned C dynamic-full-large-unmapped trace"
    )
    validate_trace(trace, description="pinned C dynamic-full-large-unmapped trace")
    return {
...
def report_from_results(**kwargs):
    ...
    report["kind"] = DYNAMIC_FULL_LARGE_UNMAPPED_EXIT_KIND
    validate_report(report)
    return report


def validate_report(report: dict) -> None:
    if report.get("kind") != DYNAMIC_FULL_LARGE_UNMAPPED_EXIT_KIND:
        raise EvidenceError("dynamic-full-large-unmapped report kind drifted")
    ...
    if c_probe.get("source_sha256") != sha256_bytes(C_TRACE_PROBE.encode("utf-8")):
        raise EvidenceError("dynamic-full-large-unmapped report C source hash drifted")
...
def main() -> int:
    ...
    try:
        ...
        print(
            "allocator x86-64 dynamic-full-large-unmapped differential: FAIL: " + str(error),
            file=os.sys.stderr,
        )
        return 1
    print(
        "allocator x86-64 dynamic-full-large-unmapped differential: PASS "
