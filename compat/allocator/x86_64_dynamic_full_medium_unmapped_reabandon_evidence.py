#!/usr/bin/env python3
"""Differentially prove one dynamic full-medium unmapped exit route.

The native pinned-C fixture fills one arena-backed 12288-byte medium page on
a worker pthread.  No remote free is published.  The worker runs real
``mi_thread_done`` and the joined consumer follows the source full collector:
five frees retain unmapped abandonment at ``used == 37`` and the sixth
publishes the dynamic mapped bitmap/count pair at ``used == 36``.  Remaining
sequential frees release the full eight-slice PageMap span and arena storage.

The full queue and direct-cache image are observed only while the worker Theap
is live.  The fixture never dereferences that Theap, its full queue, or its
direct cache after real ``mi_thread_done``.  This is narrow private native
Linux/x86-64 fixed-mimalloc engine evidence; it does not claim public x86
support, general teardown, general remote routing, or any AArch64 result.
"""
...
SCHEMA_PATH = ROOT / "compat/allocator/x86_64-dynamic-full-medium-unmapped-reabandon-evidence-v3.5.0.json"
REPORT_DEFAULT = ROOT / "compat/reports/allocator/x86_64/dynamic-full-medium-unmapped-reabandon.json"
RUST_TEST_SOURCE = ROOT / "crabc-mimalloc/src/dynamic_theap.rs"
EXPECTED_PROFILE = "linux-x86_64-private-dynamic-full-medium-unmapped-reabandon"
RUST_TEST_FILTER = (
    "dynamic_theap::tests::"
    "x86_64_dynamic_full_medium_unmapped_reabandon_trace_matches_pinned_c"
)
TRACE_BEGIN = "CRABC_MI_DYNAMIC_FULL_MEDIUM_UNMAPPED_EXIT_TRACE_BEGIN"
TRACE_END = "CRABC_MI_DYNAMIC_FULL_MEDIUM_UNMAPPED_EXIT_TRACE_END"
EXPECTED_SCOPE = {
...
EXPECTED_TRACE_VALUES = {
    "trace.dynamic_full_medium_unmapped_exit.arena_backed": 1,
    "trace.dynamic_full_medium_unmapped_exit.medium_page": 1,
    "trace.dynamic_full_medium_unmapped_exit.full_before_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.full_queue_before_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.direct_cache_empty_before_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.no_remote_free_before_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.producer_thread_done_completed": 1,
    "trace.dynamic_full_medium_unmapped_exit.producer_joined_before_consumer_frees": 1,
    "trace.dynamic_full_medium_unmapped_exit.unmapped_after_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.abandoned_after_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.page_map_registered_after_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.arena_page_bitmap_set_after_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.full_queue_detached_after_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.dynamic_abandoned_bitmap_clear_after_thread_done": 1,
    "trace.dynamic_full_medium_unmapped_exit.dynamic_abandoned_count_after_thread_done": 0,
    "trace.dynamic_full_medium_unmapped_exit.request_size": 10248,
    "trace.dynamic_full_medium_unmapped_exit.capacity": 42,
    "trace.dynamic_full_medium_unmapped_exit.reserved": 42,
    "trace.dynamic_full_medium_unmapped_exit.block_size": 12288,
    "trace.dynamic_full_medium_unmapped_exit.slice_count": 8,
    "trace.dynamic_full_medium_unmapped_exit.used_after_thread_done": 42,
    "trace.dynamic_full_medium_unmapped_exit.unmapped_prefix_free_count": 5,
    "trace.dynamic_full_medium_unmapped_exit.used_after_unmapped_prefix": 37,
    "trace.dynamic_full_medium_unmapped_exit.unmapped_after_unmapped_prefix": 1,
    "trace.dynamic_full_medium_unmapped_exit.mapped_after_reabandon_boundary": 1,
    "trace.dynamic_full_medium_unmapped_exit.dynamic_abandoned_bitmap_set_after_reabandon_boundary": 1,
    "trace.dynamic_full_medium_unmapped_exit.dynamic_abandoned_count_after_reabandon_boundary": 1,
    "trace.dynamic_full_medium_unmapped_exit.used_after_reabandon_boundary": 36,
    "trace.dynamic_full_medium_unmapped_exit.page_map_unregistered_after_final_free": 1,
    "trace.dynamic_full_medium_unmapped_exit.arena_page_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_medium_unmapped_exit.arena_slice_released_after_final_free": 1,
    "trace.dynamic_full_medium_unmapped_exit.dynamic_abandoned_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_medium_unmapped_exit.dynamic_abandoned_count_after_final_free": 0,
    "trace.dynamic_full_medium_unmapped_exit.valid": 1,
}
...
#error this private dynamic full-medium unmapped fixture requires native Linux/x86_64
#endif
...
#define CRABC_DYNAMIC_FULL_MEDIUM_UNMAPPED_MAX_BLOCKS 64
...
typedef struct fixture_s {
  ...
  mi_heap_t* heap;
  void* blocks[CRABC_DYNAMIC_FULL_MEDIUM_UNMAPPED_MAX_BLOCKS];
  size_t block_count;
  bool setup_valid;
  bool worker_ready;
  bool allow_thread_done;
  bool producer_done;
  bool full_queue_before_thread_done;
  bool direct_cache_empty_before_thread_done;
...
static void* producer_main(void* argument) {
  ...
  while (block_count < CRABC_DYNAMIC_FULL_MEDIUM_UNMAPPED_MAX_BLOCKS) {
  ...
  const bool full_queue = full != NULL && full->count == 1 && full->first == page
      && mi_page_is_in_full(page) && page->next == NULL && page->prev == NULL;
  const bool direct_empty = direct_cache_is_empty(theap);
  if (block_count != (size_t)page->reserved || page->capacity != page->reserved
      || page->block_size <= MI_SMALL_MAX_OBJ_SIZE || page->block_size > MI_MEDIUM_MAX_OBJ_SIZE
      || page->memid.memkind != MI_MEM_ARENA || !mi_page_is_full(page)
      || !full_queue || !direct_empty) { failure_stage = 6; goto failed; }
  fixture->heap = heap;
  fixture->block_count = block_count;
  fixture->full_queue_before_thread_done = full_queue;
  fixture->direct_cache_empty_before_thread_done = direct_empty;
...
int main(void) {
  ...
  int stage = 0;
  int arena_backed = 0, medium_page = 0, full_before_thread_done = 0;
  int full_queue_before_thread_done = 0, direct_cache_empty_before_thread_done = 0;
  ...
  const size_t request_size = MI_SMALL_MAX_OBJ_SIZE + sizeof(void*);
  ...
  (void)pthread_mutex_unlock(&fixture.mutex);
  if (!setup_valid || fixture.heap == NULL || block_count < 8
      || block_count > CRABC_DYNAMIC_FULL_MEDIUM_UNMAPPED_MAX_BLOCKS
  ...
  /* Capture process-heap bookkeeping before the worker's real thread exit.
     ...
     never inspects the departed Theap, its full queue, or direct cache. */
  ...
  medium_page = block_size > MI_SMALL_MAX_OBJ_SIZE && block_size <= MI_MEDIUM_MAX_OBJ_SIZE;
  full_before_thread_done = mi_page_is_full(page) && page->used == page->reserved;
  full_queue_before_thread_done = fixture.full_queue_before_thread_done;
  direct_cache_empty_before_thread_done = fixture.direct_cache_empty_before_thread_done;
  no_remote_free_before_thread_done = mi_page_thread_free(page) == NULL;
  if (!arena_backed || !medium_page || !full_before_thread_done
      || !full_queue_before_thread_done || !direct_cache_empty_before_thread_done
      || !no_remote_free_before_thread_done || capacity != block_count
      || request_size != 10248 || capacity != 42 || reserved != 42
      || block_size != 12288 || slice_count != 8) goto output;
  stage = 4;
  if (pthread_mutex_lock(&fixture.mutex) != 0) goto output;
  ...
  full_queue_detached_after_thread_done = !mi_page_is_in_full(page)
  ...
  stage = 6;
  /* Medium frees use the source full collector.  With reserved == 42,
     `reserved / 8 == 5` remains mostly-used at 37; the sixth free reaches
     36 and crosses the source reabandon-to-mapped threshold. */
  unmapped_prefix_free_count = reserved / 8;
  if (unmapped_prefix_free_count + 1 >= block_count) goto output;
  for (size_t index = 0; index < unmapped_prefix_free_count; index++) {
  ...
  if (!unmapped_after_unmapped_prefix
      ...
      || used_after_unmapped_prefix + unmapped_prefix_free_count != capacity) goto output;
  stage = 7;
  ...
  if (!mapped_after_reabandon_boundary || !dynamic_abandoned_bitmap_set_after_reabandon_boundary
      ...
      || used_after_reabandon_boundary + unmapped_prefix_free_count + 1 != capacity) goto output;
  stage = 8;
  ...
  page_map_unregistered_after_final_free = 1;
  for (size_t index = 0; index < slice_count; index++) {
    if (_mi_safe_ptr_page((const void*)(saved_slice_start + index * MI_ARENA_SLICE_SIZE)) != NULL) {
      page_map_unregistered_after_final_free = 0;
      break;
    }
  }
  ...
  valid = request_size == 10248 && capacity == 42 && reserved == 42 && block_size == 12288
      && slice_count == 8 && arena_backed && medium_page && full_before_thread_done
      && full_queue_before_thread_done && direct_cache_empty_before_thread_done
      && no_remote_free_before_thread_done && producer_thread_done_completed
      && producer_joined_before_consumer_frees && unmapped_after_thread_done
      && abandoned_after_thread_done && page_map_registered_after_thread_done
      && arena_page_bitmap_set_after_thread_done && full_queue_detached_after_thread_done
      && dynamic_abandoned_bitmap_clear_after_thread_done
      && dynamic_abandoned_count_after_thread_done == 0 && used_after_thread_done == 42
      && unmapped_prefix_free_count == 5 && used_after_unmapped_prefix == 37
  ...
  stage = 9;
output:
  printf("CRABC_MI_DYNAMIC_FULL_MEDIUM_UNMAPPED_EXIT_TRACE_BEGIN\n");
#define OUT_N(k, v) printf("trace.dynamic_full_medium_unmapped_exit.%s=%zu\n", k, (size_t)(v))
#define OUT_B(k, v) printf("trace.dynamic_full_medium_unmapped_exit.%s=%d\n", k, (v) ? 1 : 0)
  OUT_B("arena_backed", arena_backed);
  OUT_B("medium_page", medium_page);
  OUT_B("full_before_thread_done", full_before_thread_done);
  OUT_B("full_queue_before_thread_done", full_queue_before_thread_done);
  OUT_B("direct_cache_empty_before_thread_done", direct_cache_empty_before_thread_done);
  ...
  OUT_B("full_queue_detached_after_thread_done", full_queue_detached_after_thread_done);
  ...
  OUT_B("valid", valid);
  printf("CRABC_MI_DYNAMIC_FULL_MEDIUM_UNMAPPED_EXIT_TRACE_END\n");
  if (producer_started) {
  ...
  if (!valid) {
    fprintf(stderr,
        "dynamic full-medium unmapped reabandon fixture stopped at stage %d (worker=%d)\n",
...
DYNAMIC_FULL_MEDIUM_UNMAPPED_EXIT_KIND = (
    "mimalloc-x86_64-dynamic-full-medium-unmapped-reabandon-differential-evidence"
)
...
def _schema_template() -> dict:
    ...
    value["schema"] = "crabc-mimalloc-x86_64-dynamic-full-medium-unmapped-reabandon-evidence"
...
def validate_c_command(command: list[str], schema: dict) -> None:
    ...
    if definitions != list(EXPECTED_COMPILE_DEFINITIONS) or definitions != list(
        ...
        raise EvidenceError("dynamic-full-medium-unmapped C command compile definitions drifted")
    if (
        ...
        raise EvidenceError(
            "dynamic-full-medium-unmapped C command release pthread/TLS selection drifted"
        )


def validate_normalized_c_command(command: object, schema: dict) -> None:
    stem = "dynamic-full-medium-unmapped-reabandon"
    expected = [
        "-std=c11",
        "-fPIC",
        "-ftls-model=initial-exec",
    ...
    if (
        ...
        raise EvidenceError("dynamic-full-medium-unmapped report C command drifted")
...
def build_c_trace(
    ...
    try:
        _base.run.require_success(
            ...
            "pinned C dynamic-full-medium-unmapped fixture build",
        )
        header = _base.run.command_record((readelf, "-h", str(probe_binary)), cwd=source)
        _base.run.require_success(
            header, "pinned C dynamic-full-medium-unmapped fixture ELF identity"
        )
        elf = _base.run.parse_elf_identity(str(header["stdout"]), "x86_64")
        execution = _base.run.command_record((str(probe_binary),), cwd=source)
        _base.run.require_success(
            execution, "pinned C dynamic-full-medium-unmapped fixture execution"
        )
    except _base.run.HarnessError as error:
        raise EvidenceError(str(error)) from error
    trace = parse_trace(
        str(execution["stdout"]), description="pinned C dynamic-full-medium-unmapped trace"
    )
    validate_trace(trace, description="pinned C dynamic-full-medium-unmapped trace")
    return {
...
def report_from_results(**kwargs):
    ...
    report["kind"] = DYNAMIC_FULL_MEDIUM_UNMAPPED_EXIT_KIND
    validate_report(report)
    return report


def validate_report(report: dict) -> None:
    if report.get("kind") != DYNAMIC_FULL_MEDIUM_UNMAPPED_EXIT_KIND:
        raise EvidenceError("dynamic-full-medium-unmapped report kind drifted")
    ...
    if c_probe.get("source_sha256") != sha256_bytes(C_TRACE_PROBE.encode("utf-8")):
        raise EvidenceError("dynamic-full-medium-unmapped report C source hash drifted")
...
def main() -> int:
    ...
    try:
        ...
        print(
            "allocator x86-64 dynamic-full-medium-unmapped differential: FAIL: " + str(error),
            file=os.sys.stderr,
        )
        return 1
    print(
        "allocator x86-64 dynamic-full-medium-unmapped differential: PASS "
