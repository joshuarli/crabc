#!/usr/bin/env python3
"""Differentially prove two homogeneous dynamic full-large pages.

This is a private native Linux/x86-64 mimalloc v3.5.0 engine lane.  A real
worker pthread fills two same-size arena-backed full-large pages, runs
...
unmapped five-free prefix, mapped reabandon boundary, and terminal 64-slice
...
SCHEMA_PATH = ROOT / "compat/allocator/x86_64-dynamic-full-large-homogeneous-aggregate-evidence-v3.5.0.json"
REPORT_DEFAULT = ROOT / "compat/reports/allocator/x86_64/dynamic-full-large-homogeneous-aggregate.json"
RUST_TEST_SOURCE = ROOT / "crabc-mimalloc/src/dynamic_theap.rs"
EXPECTED_PROFILE = "linux-x86_64-private-dynamic-full-large-homogeneous-aggregate"
RUST_TEST_FILTER = (
    "dynamic_theap::tests::"
    "x86_64_dynamic_full_large_homogeneous_aggregate_trace_matches_pinned_c"
)
TRACE_BEGIN = "CRABC_MI_DYNAMIC_FULL_LARGE_HOMOGENEOUS_AGGREGATE_TRACE_BEGIN"
TRACE_END = "CRABC_MI_DYNAMIC_FULL_LARGE_HOMOGENEOUS_AGGREGATE_TRACE_END"
EXPECTED_SCOPE = {
    ...
    "dynamic_full_large_full_bin_only": True,
    "dynamic_full_large_homogeneous_aggregate_only": True,
    ...
    "two_or_more_full_large_pages_required": True,
}
EXPECTED_TLS = {
...
def _page_values(prefix: str) -> dict[str, int]:
    return {
        ...
        f"{prefix}.page_map_slice_count_after_thread_done": 63,
        f"{prefix}.page_map_tail_unregistered_after_thread_done": 1,
        f"{prefix}.slice_count_after_thread_done": 64,
...
EXPECTED_TRACE_VALUES = {
    "trace.dynamic_full_large_homogeneous_aggregate.arena_backed": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.large_page": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.page_count": 2,
    "trace.dynamic_full_large_homogeneous_aggregate.same_size": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.full_before_thread_done": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.full_queue_count_before_thread_done": 2,
    "trace.dynamic_full_large_homogeneous_aggregate.direct_cache_empty_before_thread_done": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.no_remote_free_before_thread_done": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.request_size": 86706,
    "trace.dynamic_full_large_homogeneous_aggregate.block_size": 98304,
    "trace.dynamic_full_large_homogeneous_aggregate.capacity": 42,
    "trace.dynamic_full_large_homogeneous_aggregate.reserved": 42,
    "trace.dynamic_full_large_homogeneous_aggregate.slice_count": 64,
    "trace.dynamic_full_large_homogeneous_aggregate.dynamic_abandoned_count_after_thread_done": 0,
    "trace.dynamic_full_large_homogeneous_aggregate.first_terminal_released_only": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.second_page_retained_after_first_terminal": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.dynamic_abandoned_count_after_first_terminal": 0,
    "trace.dynamic_full_large_homogeneous_aggregate.dynamic_abandoned_count_after_second_boundary": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.dynamic_abandoned_count_after_final_terminal": 0,
    "trace.dynamic_full_large_homogeneous_aggregate.route_empty_after_final_terminal": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.valid": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.page1.page_map_registered_after_first_terminal": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.page1.page_map_slice_count_after_first_terminal": 63,
    "trace.dynamic_full_large_homogeneous_aggregate.page1.page_map_tail_unregistered_after_first_terminal": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.page1.arena_page_bitmap_set_after_first_terminal": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.page1.unmapped_after_first_terminal": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.page1.abandoned_after_first_terminal": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.page1.dynamic_abandoned_bitmap_clear_after_first_terminal": 1,
    "trace.dynamic_full_large_homogeneous_aggregate.page1.used_after_first_terminal": 42,
}
EXPECTED_TRACE_VALUES.update(_page_values("trace.dynamic_full_large_homogeneous_aggregate.page0"))
EXPECTED_TRACE_VALUES.update(_page_values("trace.dynamic_full_large_homogeneous_aggregate.page1"))
...
static size_t page_map_count(mi_page_t* page, uintptr_t* slice_start, size_t slice_count) {
  ...
  if (area == NULL || start == NULL || area < start || area_size > MI_LARGE_PAGE_SIZE) return 0;
...
static void out_page_bool(size_t page, const char* name, int value) {
  printf("trace.dynamic_full_large_homogeneous_aggregate.page%zu.%s=%d\n", page, name, value ? 1 : 0);
}
static void out_page_number(size_t page, const char* name, size_t value) {
  printf("trace.dynamic_full_large_homogeneous_aggregate.page%zu.%s=%zu\n", page, name, value);
}
static void* producer_main(void* arg) {
  fixture_t* f = (fixture_t*)arg; const size_t request = MI_MEDIUM_MAX_OBJ_SIZE + sizeof(void*);
  ...
  for (size_t i = 0; i < PAGE_COUNT; i++) if (!mi_page_is_full(f->pages[i]) || f->pages[i]->capacity != f->pages[i]->reserved || f->pages[i]->block_size <= MI_MEDIUM_MAX_OBJ_SIZE || f->pages[i]->memid.memkind != MI_MEM_ARENA) { failure = 7; goto failed; }
...
int main(void) {
  ...
  int arena_backed = 0, large_page = 0, full = 0, direct = 0, no_remote = 0, same_size = 0; size_t dynamic_exit = 0, dynamic_first = 0, dynamic_second = 0, dynamic_final = 0;
  int page_map_reg[2] = {0,0}, page_map_tail[2] = {0,0}, unmapped[2] = {0,0}, abandoned[2] = {0,0}, arena_set[2] = {0,0}, detached[2] = {0,0};
  ...
  size_t page1_first_map_count = 0, page1_first_used = 0; int page1_first_map_registered = 0, page1_first_map_tail = 0, page1_first_arena_set = 0, page1_first_unmapped = 0, page1_first_abandoned = 0, page1_first_dynamic_clear = 0;
  int map_clear[2] = {0,0}, arena_clear[2] = {0,0}, slices_free[2] = {0,0}, abandoned_clear[2] = {0,0}; int first_only = 0, second_retained = 0, route_empty = 0; const size_t request = MI_MEDIUM_MAX_OBJ_SIZE + sizeof(void*);
  ...
  mi_page_t* before = f.pages[0]; if (before == NULL) goto output; block_size = before->block_size; capacity = before->capacity; reserved = before->reserved; slice_count = before->memid.mem.arena.slice_count; bin = _mi_bin(block_size); arena = mi_memid_arena(before->memid); if (arena == NULL || bin >= MI_ARENA_BIN_COUNT || slice_count != 64) goto output;
  ...
  arena_backed = before->memid.memkind == MI_MEM_ARENA; large_page = block_size > MI_MEDIUM_MAX_OBJ_SIZE; full = mi_page_is_full(f.pages[0]) && mi_page_is_full(f.pages[1]); direct = f.direct_cache_empty; no_remote = mi_page_thread_free(f.pages[0]) == NULL && mi_page_thread_free(f.pages[1]) == NULL; same_size = f.pages[1]->block_size == block_size && f.pages[1]->capacity == capacity && f.pages[1]->reserved == reserved;
  if (!arena_backed || !large_page || !full || !direct || !no_remote || !same_size || capacity != 42 || reserved != 42 || block_size != 98304 || f.full_queue == false) goto output;
  ...
  for (size_t p = 0; p < 2; p++) {
    mi_page_t* page = _mi_safe_ptr_page(f.blocks[p * 42]); if (page == NULL || page->block_size != block_size) goto output; page_slice_count[p] = page->memid.mem.arena.slice_count; size_t count = page_map_count(page, &starts[p], slice_count); map_count[p] = count; page_map_reg[p] = count == 63 && map_span_is(page, starts[p], count, true); page_map_tail[p] = map_span_is(page, starts[p] + count * MI_ARENA_SLICE_SIZE, slice_count - count, false); unmapped[p] = !mi_page_is_abandoned_mapped(page); abandoned[p] = mi_page_is_abandoned(page); arena_set[p] = mi_bitmap_is_setN(arena_pages->pages, page->memid.mem.arena.slice_index, 1); detached[p] = !mi_page_is_in_full(page) && page->next == NULL && page->prev == NULL && !mi_page_is_owned(page); used_exit[p] = page->used;
  }
...
def _schema_template() -> dict:
    ...
    value["schema"] = "crabc-mimalloc-x86_64-dynamic-full-large-homogeneous-aggregate-evidence"
...
def load_schema(path=None):
    ...
    try:
        ...
        raise EvidenceError("cannot read dynamic-full-large-homogeneous-aggregate schema") from error
    if not exactly_matches(schema, _schema_template()):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate checked-in schema drifted")
    try:
        pin = RUNNER.load_pin()
    except RUNNER.HarnessError as error:
        raise EvidenceError("cannot validate pinned dynamic-full-large-homogeneous-aggregate upstream") from error
    ...
    if not exactly_matches(observed, EXPECTED_UPSTREAM) or pin["sha256"] != EXPECTED_ARCHIVE_SHA256:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate upstream archive pin drifted")
    return schema
...
def validate_c_command(command, schema):
    ...
    if schema.get("tls") != EXPECTED_TLS or definitions != list(schema["compile_definitions"]):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate C command contract drifted")
    if flags != list(schema["release_flags"]) or "-pthread" not in command or "-ftls-model=initial-exec" not in command:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate C command release pthread/TLS drifted")
...
def validate_normalized_c_command(command, schema):
    ...
    if not isinstance(command, list) or not command or Path(command[0]).name != "musl-gcc" or command[1:] != expected:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report C command drifted")
...
def build_c_trace(compiler, readelf, source, temporary, schema):
    ...
    try:
        RUNNER.require_success(RUNNER.command_record(command, cwd=source), "pinned C dynamic full-large homogeneous aggregate build")
        header = RUNNER.command_record((readelf, "-h", str(probe_binary)), cwd=source); RUNNER.require_success(header, "pinned C dynamic full-large homogeneous aggregate ELF identity")
        elf = RUNNER.parse_elf_identity(str(header["stdout"]), "x86_64")
        execution = RUNNER.command_record((str(probe_binary),), cwd=source); RUNNER.require_success(execution, "pinned C dynamic full-large homogeneous aggregate execution")
    except RUNNER.HarnessError as error:
        raise EvidenceError(str(error)) from error
    trace = parse_trace(str(execution["stdout"]), description="pinned C dynamic full-large homogeneous aggregate trace"); validate_trace(trace, description="pinned C dynamic full-large homogeneous aggregate trace")
...
def compare_traces(c_trace, rust_trace):
    validate_trace(c_trace, description="pinned C dynamic full-large homogeneous aggregate trace")
    validate_trace(rust_trace, description="Rust dynamic full-large homogeneous aggregate trace")
    mismatches = [
    ...
    if mismatches:
        raise EvidenceError("Rust dynamic full-large homogeneous aggregate trace differs from pinned C: " + ", ".join(mismatches))
...
def report_from_results(*, schema, provenance, archive_sha256, anchors, c_probe, rust_probe):
    ...
    if not isinstance(c_trace, Mapping) or not isinstance(rust_trace, Mapping):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report inputs lack trace records")
    report = {
...
def validate_report(report):
    ...
    if not isinstance(report, dict) or set(report) != required:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report schema drifted")
    if report["format"] != 1 or report["status"] != "passed":
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report format/status drifted")
    if report["kind"] != kind:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report kind drifted")
    if not exactly_matches(report["target"], EXPECTED_TARGET) or not exactly_matches(report["upstream"], EXPECTED_UPSTREAM):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report target/upstream drifted")
    if report["profile"] != EXPECTED_PROFILE or not exactly_matches(report["scope"], EXPECTED_SCOPE):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report private boundary drifted")
    if not any(
        ...
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report lacks native x86-64 provenance")
    schema = load_schema()
    if not exactly_matches(report["trace"], schema["trace"]):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report trace contract drifted")
    source = report["source"]
    if not isinstance(source, dict) or set(source) != {"archive_sha256", "anchors", "release_flags", "release_source_set"}:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report source record is malformed")
    if source["archive_sha256"] != EXPECTED_ARCHIVE_SHA256:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report archive identity drifted")
    if not exactly_matches(source["anchors"], schema["source_anchors"]):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report source anchors drifted")
    if not exactly_matches(source["release_flags"], schema["release_flags"]):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report release flags drifted")
    if not exactly_matches(source["release_source_set"], schema["release_source_set"]):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report source set drifted")
    ...
    if not isinstance(c_probe, dict) or set(c_probe) != {"build_command", "elf", "run_command", "source_sha256", "trace"}:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report C probe record drifted")
    if not isinstance(rust_probe, dict) or set(rust_probe) != {"cargo_command", "lockfile", "passed_test_count", "source", "target_dir", "trace"}:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report Rust probe record drifted")
    if not exactly_matches(c_probe["elf"], EXPECTED_C_ELF):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report C ELF identity drifted")
    stem = "dynamic-full-large-homogeneous-aggregate"
    if c_probe["run_command"] != [f"{NORMALIZED_EVIDENCE_ROOT}/{stem}-c"]:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report C run command drifted")
    if c_probe["source_sha256"] != sha256_bytes(C_TRACE_PROBE.encode()):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report C source hash drifted")
    ...
    if type(rust_probe["passed_test_count"]) is not int or rust_probe["passed_test_count"] != 1:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report Rust test selection drifted")
    if not exactly_matches(rust_probe["target_dir"], {"isolated": True, "retained": False, "value": f"{NORMALIZED_EVIDENCE_ROOT}/rust-target"}):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report Rust target directory drifted")
    ...
    if not exactly_matches(rust_probe["lockfile"], {"path": relative(LOCKFILE), "sha256": sha256_file(LOCKFILE)}):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report Rust lockfile identity drifted")
    if not exactly_matches(rust_probe["source"], {"path": relative(RUST_TEST_SOURCE), "sha256": sha256_file(RUST_TEST_SOURCE)}):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report Rust source identity drifted")
    if not isinstance(c_probe["trace"], Mapping) or not isinstance(rust_probe["trace"], Mapping):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report lacks C/Rust traces")
    if not exactly_matches(report["comparison"], compare_traces(c_probe["trace"], rust_probe["trace"])):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report comparison drifted")


def validate_normalized_rust_command(command):
    if not isinstance(command, list) or not command or not all(isinstance(part, str) for part in command):
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report Rust command is malformed")
    expected = [
    ...
    if Path(command[0]).name != "cargo" or command[1:] != expected:
        raise EvidenceError("dynamic-full-large-homogeneous-aggregate report Rust command drifted")
...
def validate_source_anchors(schema, source):
    ...
    for anchor in schema["source_anchors"]:
        ...
        if observed != anchor["sha256"]:
            raise EvidenceError(f"pinned dynamic-full-large-homogeneous-aggregate source anchor drifted: {member}")
...
def build_rust_trace(cargo, temporary):
    ...
    try:
        ...
        RUNNER.require_success(execution, "Rust dynamic-full-large-homogeneous aggregate fixture")
        passed = RUNNER.parse_rust_test_count(str(execution["stdout"]) + "\n" + str(execution["stderr"]))
    except RUNNER.HarnessError as error:
        raise EvidenceError(str(error)) from error
    if passed != 1:
        raise EvidenceError(f"Rust dynamic-full-large-homogeneous aggregate fixture passed {passed} tests, expected one")
    trace = parse_trace(str(execution["stdout"]) + "\n" + str(execution["stderr"]), description="Rust dynamic-full-large-homogeneous aggregate trace")
    validate_trace(trace, description="Rust dynamic-full-large-homogeneous aggregate trace")
    return {
...
def run_evidence(*, offline, report_path):
    ...
    try:
        ...
        raise EvidenceError(str(error)) from error
    with tempfile.TemporaryDirectory(prefix="crabc-mimalloc-x86_64-dynamic-full-large-homogeneous-aggregate-") as temporary_name:
...
def main() -> int:
    ...
    try: report = run_evidence(offline=args.offline, report_path=args.report)
        ...
        print("allocator x86-64 dynamic-full-large-homogeneous-aggregate differential: FAIL: " + str(error), file=os.sys.stderr); return 1
    print("allocator x86-64 dynamic-full-large-homogeneous-aggregate differential: PASS " f"({report['comparison']['compared_value_count']} logical values; report: {relative(args.report)})"); return 0
