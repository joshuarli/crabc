#!/usr/bin/env python3
"""Differentially prove one full non-direct-small force-collect post-exit route.
...
SCHEMA_PATH = ROOT / "compat/allocator/x86_64-full-non-direct-small-force-collect-post-exit-evidence-v3.5.0.json"
REPORT_DEFAULT = ROOT / "compat/reports/allocator/x86_64/full-non-direct-small-force-collect-post-exit.json"
RUST_TEST_SOURCE = ROOT / "crabc-mimalloc/src/main_heap_page.rs"
EXPECTED_PROFILE = "linux-x86_64-private-full-non-direct-small-force-collect-post-exit-mapped-release"
RUST_TEST_FILTER = (
    "main_heap_page::tests::"
    "x86_64_full_non_direct_small_force_collect_post_exit_trace_matches_pinned_c"
)
TRACE_BEGIN = "CRABC_MI_FULL_NON_DIRECT_SMALL_EXIT_TRACE_BEGIN"
TRACE_END = "CRABC_MI_FULL_NON_DIRECT_SMALL_EXIT_TRACE_END"
EXPECTED_SCOPE = {
    "aarch64_status_reused": False,
    "emulation_accepted": False,
    "full_non_direct_small_regular_bin_only": True,
...
EXPECTED_TRACE_VALUES = {
    "trace.full_non_direct_small_exit.arena_backed": 1,
    "trace.full_non_direct_small_exit.small_page": 1,
    "trace.full_non_direct_small_exit.non_direct_small": 1,
    "trace.full_non_direct_small_exit.full_before_remote": 1,
    "trace.full_non_direct_small_exit.ordinary_regular_bin_before_remote": 1,
    "trace.full_non_direct_small_exit.direct_cache_empty_before_remote": 1,
    "trace.full_non_direct_small_exit.remote_free_published_before_thread_done": 1,
    "trace.full_non_direct_small_exit.producer_thread_done_completed": 1,
    "trace.full_non_direct_small_exit.producer_joined_before_consumer_frees": 1,
    "trace.full_non_direct_small_exit.mapped_after_thread_done": 1,
    "trace.full_non_direct_small_exit.abandoned_after_thread_done": 1,
    "trace.full_non_direct_small_exit.page_map_registered_after_thread_done": 1,
    "trace.full_non_direct_small_exit.arena_page_bitmap_set_after_thread_done": 1,
    "trace.full_non_direct_small_exit.ordinary_queue_detached_after_thread_done": 1,
    "trace.full_non_direct_small_exit.request_size": 1032,
    "trace.full_non_direct_small_exit.capacity": 51,
    "trace.full_non_direct_small_exit.block_size": 1280,
    "trace.full_non_direct_small_exit.slice_count": 1,
    "trace.full_non_direct_small_exit.used_after_force_collect": 50,
    "trace.full_non_direct_small_exit.remaining_client_count_after_force_collect": 50,
    "trace.full_non_direct_small_exit.nonfinal_consumer_free_keeps_mapped": 1,
    "trace.full_non_direct_small_exit.page_map_unregistered_after_final_free": 1,
    "trace.full_non_direct_small_exit.arena_page_bitmap_clear_after_final_free": 1,
    "trace.full_non_direct_small_exit.arena_slice_released_after_final_free": 1,
    "trace.full_non_direct_small_exit.valid": 1,
}
...
#error this private full-non-direct-small fixture requires native Linux/x86_64
#endif
...
#define CRABC_FULL_NON_DIRECT_SMALL_MAX_BLOCKS 128
...
typedef struct fixture_s {
  ...
  mi_heap_t* heap;
  void* blocks[CRABC_FULL_NON_DIRECT_SMALL_MAX_BLOCKS];
  size_t block_count;
  bool setup_valid;
  bool worker_ready;
...
static void* producer_main(void* argument) {
  ...
  while (block_count < CRABC_FULL_NON_DIRECT_SMALL_MAX_BLOCKS) {
  ...
  const bool regular_before_remote = (
      regular != NULL && regular->count == 1 && regular->first == page
      && !mi_page_is_in_full(page) && page->next == NULL && page->prev == NULL);
  const bool direct_empty_before_remote = direct_cache_is_empty(theap);
  if (page == NULL || regular == NULL || block_count == 0
      || block_count != (size_t)page->reserved
      ...
      || !regular_before_remote
      || !direct_empty_before_remote) {
    failure_stage = 6;
    goto failed;
  }
  ...
  fixture->ordinary_regular_bin_before_remote = regular_before_remote;
  ...
  // The real source MI_ABANDON pass collects the joined remote publication
  // before it decides between a full-unmapped and nonfull-mapped branch.
  mi_thread_done();
  if (pthread_mutex_lock(&fixture->mutex) != 0) return NULL;
...
int main(void) {
  ...
  int full_before_remote = 0;
  int ordinary_regular_bin_before_remote = 0;
  int direct_cache_empty_before_remote = 0;
  ...
  int mapped_after_thread_done = 0;
  int abandoned_after_thread_done = 0;
  int page_map_registered_after_thread_done = 0;
  int arena_page_bitmap_set_after_thread_done = 0;
  int ordinary_queue_detached_after_thread_done = 0;
  size_t used_after_force_collect = 0;
  size_t remaining_client_count_after_force_collect = 0;
  ...
  if (mi_reserve_os_memory_ex(mi_arena_min_size(), true, false, true, &fixture.arena_id) != 0
      || fixture.arena_id == _mi_arena_id_none()) {
    goto output;
  }
  stage = 2;
  ...
  if (!setup_valid || fixture.heap == NULL || block_count < 3
      || block_count > CRABC_FULL_NON_DIRECT_SMALL_MAX_BLOCKS
      || fixture.blocks[0] == NULL || fixture.blocks[1] == NULL) {
    fprintf(stderr,
            "full-non-direct-small worker setup failed: setup=%d worker-stage=%d "
            "heap=%d blocks=%zu first=%d second=%d\n",
            setup_valid, fixture.worker_failure_stage, fixture.heap != NULL,
            block_count, fixture.blocks[0] != NULL, fixture.blocks[1] != NULL);
    goto output;
  }

  // The worker is blocked before mi_thread_done. This public consumer free
  // therefore publishes an xthread-free entry but does not change used yet.
  ...
  if (!arena_backed || !small_page || !non_direct_small || !full_before_remote
      ...
      || capacity != block_count || slice_count == 0) {
    fprintf(stderr,
            "full-non-direct-small pre-remote shape failed: arena=%d small=%d "
            "non-direct=%d full=%d regular=%d direct-empty=%d capacity=%zu "
            "blocks=%zu slice-count=%zu block-size=%zu\n",
            arena_backed, small_page, non_direct_small, full_before_remote,
            ordinary_regular_bin_before_remote, direct_cache_empty_before_remote,
            capacity, block_count, slice_count, block_size);
    goto output;
  }
  stage = 4;
  ...
  // The old Theap is gone. Reacquire page metadata from a live consumer
  // client; neither an old producer page pointer nor its direct cache is read.
  ...
  arena_page_bitmap_set_after_thread_done = mi_bitmap_is_setN(
  ...
  const bool abandoned_bin_bitmap_set_after_thread_done = (
  ...
  if (!mapped_after_thread_done || !abandoned_after_thread_done
      ...
      || !ordinary_queue_detached_after_thread_done || !abandoned_bin_bitmap_set_after_thread_done
      || used_after_force_collect + 1 != capacity
      || remaining_client_count_after_force_collect != used_after_force_collect) {
    goto output;
  }
  stage = 7;

  // A first post-join client free must preserve the mapped page. The rest
  // then follow the same consumer route through one terminal final free.
  ...
  page_map_unregistered_after_final_free = (
      _mi_safe_ptr_page((const void*)saved_address) == NULL);
  ...
  const bool abandoned_bin_bitmap_clear_after_final_free = (
      arena_pages->pages_abandoned[bin] != NULL
      && mi_bitmap_is_clearN(arena_pages->pages_abandoned[bin], slice_index, 1));
  valid = (
      request_size == 1032 && capacity == 51 && block_size == 1280 && slice_count == 1
      && arena_backed && small_page && non_direct_small && full_before_remote
      && ordinary_regular_bin_before_remote && direct_cache_empty_before_remote
      ...
      && used_after_force_collect == 50 && remaining_client_count_after_force_collect == 50
      && nonfinal_consumer_free_keeps_mapped && page_map_unregistered_after_final_free
      && arena_page_bitmap_clear_after_final_free && arena_slice_released_after_final_free
      && abandoned_bin_bitmap_clear_after_final_free);
  stage = 9;

output:
  printf("CRABC_MI_FULL_NON_DIRECT_SMALL_EXIT_TRACE_BEGIN\n");
#define OUT_N(k, v) printf("trace.full_non_direct_small_exit.%s=%zu\n", k, (size_t)(v))
#define OUT_B(k, v) printf("trace.full_non_direct_small_exit.%s=%d\n", k, (v) ? 1 : 0)
  ...
  OUT_B("valid", valid);
  printf("CRABC_MI_FULL_NON_DIRECT_SMALL_EXIT_TRACE_END\n");
  ...
  if (!valid) {
    fprintf(stderr, "full-non-direct-small force-collect post-exit fixture stopped at stage %d\n", stage);
  }
  return valid ? 0 : 2;
}
'''
...
NON_DIRECT_EXIT_KIND = (
    "mimalloc-x86_64-full-non-direct-small-force-collect-post-exit-differential-evidence"
)
...
def _schema_template() -> dict:
    ...
    value["schema"] = "crabc-mimalloc-x86_64-full-non-direct-small-force-collect-post-exit-evidence"
...
def validate_c_command(command: list[str], schema: dict) -> None:
    ...
    if definitions != list(EXPECTED_COMPILE_DEFINITIONS) or definitions != list(
        ...
        raise EvidenceError("full-non-direct-small C command compile definitions drifted")
    if (
        flags != list(schema["release_flags"])
        or "-pthread" not in command
        or "-ftls-model=initial-exec" not in command
    ):
        raise EvidenceError("full-non-direct-small C command release pthread/TLS selection drifted")
...
def validate_normalized_c_command(command: object, schema: dict) -> None:
    expected = [
        ...
        *schema["release_flags"],
        f"{NORMALIZED_EVIDENCE_ROOT}/full-non-direct-small-force-collect-post-exit.c",
    ...
    if (
        ...
        raise EvidenceError("full-non-direct-small report C command drifted")


def build_c_trace(
    compiler: str, readelf: str, source: Path, temporary: Path, schema: dict
) -> dict:
    probe_source = temporary / "full-non-direct-small-force-collect-post-exit.c"
    probe_binary = temporary / "full-non-direct-small-force-collect-post-exit-c"
    ...
    try:
        _base.run.require_success(
            ...
            "pinned C full-non-direct-small fixture build",
        )
        header = _base.run.command_record((readelf, "-h", str(probe_binary)), cwd=source)
        _base.run.require_success(header, "pinned C full-non-direct-small fixture ELF identity")
        elf = _base.run.parse_elf_identity(str(header["stdout"]), "x86_64")
        execution = _base.run.command_record((str(probe_binary),), cwd=source)
        _base.run.require_success(execution, "pinned C full-non-direct-small fixture execution")
    except _base.run.HarnessError as error:
        raise EvidenceError(str(error)) from error
    trace = parse_trace(str(execution["stdout"]), description="pinned C full-non-direct-small trace")
    validate_trace(trace, description="pinned C full-non-direct-small trace")
    return {
...
def report_from_results(**kwargs):
    ...
    report["kind"] = NON_DIRECT_EXIT_KIND
    validate_report(report)
    return report


def validate_report(report: dict) -> None:
    if report.get("kind") != NON_DIRECT_EXIT_KIND:
        raise EvidenceError("full-non-direct-small report kind drifted")
    ...
    if c_probe.get("source_sha256") != sha256_bytes(C_TRACE_PROBE.encode("utf-8")):
        raise EvidenceError("full-non-direct-small report C source hash drifted")
    ...
    compatible["c_probe"]["build_command"] = [
        part.replace("full-non-direct-small-force-collect-post-exit", "regular-small")
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"allocator x86-64 full-non-direct-small differential: FAIL: {error}",
            file=os.sys.stderr,
        )
        return 1
    print(
        "allocator x86-64 full-non-direct-small differential: PASS "
