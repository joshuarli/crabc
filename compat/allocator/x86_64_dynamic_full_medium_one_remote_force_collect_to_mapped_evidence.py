#!/usr/bin/env python3
"""Differentially prove one dynamic full-medium one-remote force-collect-to-mapped route.

The native pinned-C fixture fills one arena-backed non-direct-small regular-bin
page on a worker pthread. The consumer publishes exactly one remote free before
the worker calls real mi_thread_done. The source force collector makes the
...
SCHEMA_PATH = ROOT / "compat/allocator/x86_64-dynamic-full-medium-one-remote-force-collect-to-mapped-evidence-v3.5.0.json"
REPORT_DEFAULT = ROOT / "compat/reports/allocator/x86_64/dynamic-full-medium-one-remote-force-collect-to-mapped.json"
RUST_TEST_SOURCE = ROOT / "crabc-mimalloc/src/dynamic_theap.rs"
EXPECTED_PROFILE = "linux-x86_64-private-dynamic-full-medium-one-remote-force-collect-to-mapped"
RUST_TEST_FILTER = (
    "dynamic_theap::tests::"
    "x86_64_dynamic_full_medium_one_remote_force_collect_to_mapped_trace_matches_pinned_c"
)
TRACE_BEGIN = "CRABC_MI_DYNAMIC_FULL_MEDIUM_ONE_REMOTE_EXIT_TRACE_BEGIN"
TRACE_END = "CRABC_MI_DYNAMIC_FULL_MEDIUM_ONE_REMOTE_EXIT_TRACE_END"
EXPECTED_SCOPE = {
    "aarch64_status_reused": False,
    "emulation_accepted": False,
    "dynamic_full_medium_one_remote_full_bin_only": True,
...
EXPECTED_TRACE_VALUES = {
    "trace.dynamic_full_medium_one_remote_exit.arena_backed": 1,
    "trace.dynamic_full_medium_one_remote_exit.medium_page": 1,
    "trace.dynamic_full_medium_one_remote_exit.full_before_remote": 1,
    "trace.dynamic_full_medium_one_remote_exit.full_queue_before_remote": 1,
    "trace.dynamic_full_medium_one_remote_exit.direct_cache_empty_before_remote": 1,
    "trace.dynamic_full_medium_one_remote_exit.remote_free_published_before_thread_done": 1,
    "trace.dynamic_full_medium_one_remote_exit.producer_thread_done_completed": 1,
    "trace.dynamic_full_medium_one_remote_exit.producer_joined_before_consumer_frees": 1,
    "trace.dynamic_full_medium_one_remote_exit.mapped_after_thread_done": 1,
    "trace.dynamic_full_medium_one_remote_exit.abandoned_after_thread_done": 1,
    "trace.dynamic_full_medium_one_remote_exit.page_map_registered_after_thread_done": 1,
    "trace.dynamic_full_medium_one_remote_exit.arena_page_bitmap_set_after_thread_done": 1,
    "trace.dynamic_full_medium_one_remote_exit.full_queue_detached_after_thread_done": 1,
    "trace.dynamic_full_medium_one_remote_exit.dynamic_abandoned_bitmap_set_after_thread_done": 1,
    "trace.dynamic_full_medium_one_remote_exit.dynamic_abandoned_count_after_thread_done": 1,
    "trace.dynamic_full_medium_one_remote_exit.request_size": 10248,
    "trace.dynamic_full_medium_one_remote_exit.capacity": 42,
    "trace.dynamic_full_medium_one_remote_exit.reserved": 42,
    "trace.dynamic_full_medium_one_remote_exit.block_size": 12288,
    "trace.dynamic_full_medium_one_remote_exit.slice_count": 8,
    "trace.dynamic_full_medium_one_remote_exit.used_after_force_collect": 41,
    "trace.dynamic_full_medium_one_remote_exit.remaining_client_count_after_force_collect": 41,
    "trace.dynamic_full_medium_one_remote_exit.nonfinal_consumer_free_keeps_mapped": 1,
    "trace.dynamic_full_medium_one_remote_exit.page_map_unregistered_after_final_free": 1,
    "trace.dynamic_full_medium_one_remote_exit.arena_page_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_medium_one_remote_exit.arena_slice_released_after_final_free": 1,
    "trace.dynamic_full_medium_one_remote_exit.dynamic_abandoned_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_medium_one_remote_exit.dynamic_abandoned_count_after_final_free": 0,
    "trace.dynamic_full_medium_one_remote_exit.valid": 1,
}
...
#error this private dynamic full-medium one-remote fixture requires native Linux/x86_64
#endif
...
#define CRABC_DYNAMIC_FULL_MEDIUM_ONE_REMOTE_MAX_BLOCKS 64
...
typedef struct fixture_s {
  ...
  mi_heap_t* heap;
  void* blocks[CRABC_DYNAMIC_FULL_MEDIUM_ONE_REMOTE_MAX_BLOCKS];
  size_t block_count;
  bool setup_valid;
  bool worker_ready;
...
static void* producer_main(void* argument) {
  ...
  while (block_count < CRABC_DYNAMIC_FULL_MEDIUM_ONE_REMOTE_MAX_BLOCKS) {
  ...
  if (page == NULL || block_count == 0 || full == NULL || block_count != (size_t)page->reserved
      || page->capacity != page->reserved || page->block_size <= MI_SMALL_MAX_OBJ_SIZE
      || page->block_size > MI_MEDIUM_MAX_OBJ_SIZE || page->memid.memkind != MI_MEM_ARENA
...
int main(void) {
  ...
  int stage = 0;
  int arena_backed = 0, medium_page = 0, full_before_remote = 0;
  ...
  if (!setup_valid || fixture.heap == NULL || block_count < 3
      || block_count > CRABC_DYNAMIC_FULL_MEDIUM_ONE_REMOTE_MAX_BLOCKS
  ...
  if (!arena_backed || !medium_page || !full_before_remote || !full_queue_before_remote
      || !direct_cache_empty_before_remote || capacity != block_count
      || reserved != 42 || capacity != 42 || block_size != 12288 || slice_count != 8) goto output;
  ...
  producer_joined_before_consumer_frees =
      producer_thread_done_completed && !producer_started && fixture.remote_free_published;
  ...
  valid = request_size == 10248 && capacity == 42 && reserved == 42 && block_size == 12288
      && slice_count == 8 && arena_backed && medium_page && full_before_remote
      ...
      && producer_joined_before_consumer_frees && mapped_after_thread_done
      ...
      && dynamic_abandoned_bitmap_set_after_thread_done
      && dynamic_abandoned_count_after_thread_done == 1 && used_after_force_collect == 41
...
output:
  printf("CRABC_MI_DYNAMIC_FULL_MEDIUM_ONE_REMOTE_EXIT_TRACE_BEGIN\n");
#define OUT_N(k,v) printf("trace.dynamic_full_medium_one_remote_exit.%s=%zu\n",k,(size_t)(v))
#define OUT_B(k,v) printf("trace.dynamic_full_medium_one_remote_exit.%s=%d\n",k,(v)?1:0)
  OUT_B("arena_backed",arena_backed); OUT_B("medium_page",medium_page);
  ...
  OUT_B("valid",valid);
  printf("CRABC_MI_DYNAMIC_FULL_MEDIUM_ONE_REMOTE_EXIT_TRACE_END\n");
  if (producer_started) {
  ...
  if (!valid) fprintf(stderr, "dynamic full-medium one-remote force-collect fixture stopped at stage %d\n", stage);
  return valid ? 0 : 2;
}
'''


DYNAMIC_FULL_MEDIUM_ONE_REMOTE_EXIT_KIND = (
    "mimalloc-x86_64-dynamic-full-medium-one-remote-force-collect-to-mapped-differential-evidence"
)
...
def _schema_template() -> dict:
    ...
    value["schema"] = "crabc-mimalloc-x86_64-dynamic-full-medium-one-remote-force-collect-to-mapped-evidence"
...
def validate_c_command(command: list[str], schema: dict) -> None:
    ...
    if definitions != list(EXPECTED_COMPILE_DEFINITIONS) or definitions != list(
        ...
        raise EvidenceError("dynamic-full-medium-one-remote C command compile definitions drifted")
    if (
        flags != list(schema["release_flags"])
        or "-pthread" not in command
        or "-ftls-model=initial-exec" not in command
    ):
        raise EvidenceError("dynamic-full-medium-one-remote C command release pthread/TLS selection drifted")
...
def validate_normalized_c_command(command: object, schema: dict) -> None:
    expected = [
        ...
        *schema["release_flags"],
        f"{NORMALIZED_EVIDENCE_ROOT}/dynamic-full-medium-one-remote-force-collect-to-mapped.c",
    ...
    if (
        ...
        raise EvidenceError("dynamic-full-medium-one-remote report C command drifted")


def build_c_trace(
    compiler: str, readelf: str, source: Path, temporary: Path, schema: dict
) -> dict:
    probe_source = temporary / "dynamic-full-medium-one-remote-force-collect-to-mapped.c"
    probe_binary = temporary / "dynamic-full-medium-one-remote-force-collect-to-mapped-c"
    ...
    try:
        _base.run.require_success(
            ...
            "pinned C dynamic-full-medium-one-remote fixture build",
        )
        header = _base.run.command_record((readelf, "-h", str(probe_binary)), cwd=source)
        _base.run.require_success(header, "pinned C dynamic-full-medium-one-remote fixture ELF identity")
        elf = _base.run.parse_elf_identity(str(header["stdout"]), "x86_64")
        execution = _base.run.command_record((str(probe_binary),), cwd=source)
        _base.run.require_success(execution, "pinned C dynamic-full-medium-one-remote fixture execution")
    except _base.run.HarnessError as error:
        raise EvidenceError(str(error)) from error
    trace = parse_trace(str(execution["stdout"]), description="pinned C dynamic-full-medium-one-remote trace")
    validate_trace(trace, description="pinned C dynamic-full-medium-one-remote trace")
    return {
...
def report_from_results(**kwargs):
    ...
    report["kind"] = DYNAMIC_FULL_MEDIUM_ONE_REMOTE_EXIT_KIND
    validate_report(report)
    return report


def validate_report(report: dict) -> None:
    if report.get("kind") != DYNAMIC_FULL_MEDIUM_ONE_REMOTE_EXIT_KIND:
        raise EvidenceError("dynamic-full-medium-one-remote report kind drifted")
    ...
    if c_probe.get("source_sha256") != sha256_bytes(C_TRACE_PROBE.encode("utf-8")):
        raise EvidenceError("dynamic-full-medium-one-remote report C source hash drifted")
    ...
    compatible["c_probe"]["build_command"] = [
        part.replace("dynamic-full-medium-one-remote-force-collect-to-mapped", "regular-small")
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"allocator x86-64 dynamic-full-medium-one-remote differential: FAIL: {error}",
            file=os.sys.stderr,
        )
        return 1
    print(
        "allocator x86-64 dynamic-full-medium-one-remote differential: PASS "
