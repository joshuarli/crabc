#!/usr/bin/env python3
"""Differentially prove one dynamic full non-direct-small unmapped exit route.
...
page in its ordinary regular bin.  No remote free is published.  The worker
runs real ``mi_thread_done`` and the joined consumer follows the source normal
failed-reclaim tail: six frees retain ordinary unmapped abandonment, the
seventh publishes the dynamic mapped bitmap/count pair, and the remaining
sequential frees perform terminal PageMap and arena-slice release.
...
SCHEMA_PATH = ROOT / "compat/allocator/x86_64-dynamic-full-non-direct-small-unmapped-reabandon-evidence-v3.5.0.json"
REPORT_DEFAULT = ROOT / "compat/reports/allocator/x86_64/dynamic-full-non-direct-small-unmapped-reabandon.json"
RUST_TEST_SOURCE = ROOT / "crabc-mimalloc/src/dynamic_theap.rs"
EXPECTED_PROFILE = "linux-x86_64-private-dynamic-full-non-direct-small-unmapped-reabandon"
RUST_TEST_FILTER = (
    "dynamic_theap::tests::"
    "x86_64_dynamic_full_non_direct_small_unmapped_reabandon_trace_matches_pinned_c"
)
TRACE_BEGIN = "CRABC_MI_DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_EXIT_TRACE_BEGIN"
TRACE_END = "CRABC_MI_DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_EXIT_TRACE_END"
EXPECTED_SCOPE = {
...
EXPECTED_TRACE_VALUES = {
    "trace.dynamic_full_non_direct_small_unmapped_exit.arena_backed": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.small_page": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.non_direct_small": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.full_before_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.ordinary_regular_bin_before_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.direct_cache_empty_before_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.no_remote_free_before_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.producer_thread_done_completed": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.producer_joined_before_consumer_frees": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.unmapped_after_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.abandoned_after_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.page_map_registered_after_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.arena_page_bitmap_set_after_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.ordinary_queue_detached_after_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.dynamic_abandoned_bitmap_clear_after_thread_done": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.dynamic_abandoned_count_after_thread_done": 0,
    "trace.dynamic_full_non_direct_small_unmapped_exit.request_size": 1032,
    "trace.dynamic_full_non_direct_small_unmapped_exit.capacity": 51,
    "trace.dynamic_full_non_direct_small_unmapped_exit.reserved": 51,
    "trace.dynamic_full_non_direct_small_unmapped_exit.block_size": 1280,
    "trace.dynamic_full_non_direct_small_unmapped_exit.slice_count": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.used_after_thread_done": 51,
    "trace.dynamic_full_non_direct_small_unmapped_exit.unmapped_prefix_free_count": 6,
    "trace.dynamic_full_non_direct_small_unmapped_exit.used_after_unmapped_prefix": 45,
    "trace.dynamic_full_non_direct_small_unmapped_exit.unmapped_after_unmapped_prefix": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.mapped_after_reabandon_boundary": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.dynamic_abandoned_bitmap_set_after_reabandon_boundary": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.dynamic_abandoned_count_after_reabandon_boundary": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.used_after_reabandon_boundary": 44,
    "trace.dynamic_full_non_direct_small_unmapped_exit.page_map_unregistered_after_final_free": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.arena_page_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.arena_slice_released_after_final_free": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.dynamic_abandoned_bitmap_clear_after_final_free": 1,
    "trace.dynamic_full_non_direct_small_unmapped_exit.dynamic_abandoned_count_after_final_free": 0,
    "trace.dynamic_full_non_direct_small_unmapped_exit.valid": 1,
}
...
#error this private dynamic full non-direct-small unmapped fixture requires native Linux/x86_64
#endif
...
#define CRABC_DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_MAX_BLOCKS 128
...
typedef struct fixture_s {
  ...
  mi_heap_t* heap;
  void* blocks[CRABC_DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_MAX_BLOCKS];
  size_t block_count;
  bool setup_valid;
  bool worker_ready;
...
static void* producer_main(void* argument) {
  ...
  while (block_count < CRABC_DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_MAX_BLOCKS) {
...
int main(void) {
  ...
  int stage = 0;
  int arena_backed = 0, small_page = 0, non_direct_small = 0, full_before_thread_done = 0;
  int ordinary_regular_bin_before_thread_done = 0, direct_cache_empty_before_thread_done = 0;
  ...
  if (!setup_valid || fixture.heap == NULL || block_count < 8
      || block_count > CRABC_DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_MAX_BLOCKS
  ...
  /* Capture dynamic-Heap bookkeeping before the worker's real thread exit.
  ...
  if (!arena_backed || !small_page || !non_direct_small || !full_before_thread_done
      || !ordinary_regular_bin_before_thread_done || !direct_cache_empty_before_thread_done
  ...
  valid = request_size == 1032 && capacity == 51 && reserved == 51 && block_size == 1280
      && slice_count == 1 && arena_backed && small_page && non_direct_small
      && full_before_thread_done && ordinary_regular_bin_before_thread_done
      && direct_cache_empty_before_thread_done && no_remote_free_before_thread_done
      ...
      && ordinary_queue_detached_after_thread_done
      && dynamic_abandoned_bitmap_clear_after_thread_done
      && dynamic_abandoned_count_after_thread_done == 0 && used_after_thread_done == 51
      && unmapped_prefix_free_count == 6 && used_after_unmapped_prefix == 45
  ...
  stage = 9;
output:
  printf("CRABC_MI_DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_EXIT_TRACE_BEGIN\n");
#define OUT_N(k, v) printf("trace.dynamic_full_non_direct_small_unmapped_exit.%s=%zu\n", k, (size_t)(v))
#define OUT_B(k, v) printf("trace.dynamic_full_non_direct_small_unmapped_exit.%s=%d\n", k, (v) ? 1 : 0)
  ...
  OUT_B("valid", valid);
  printf("CRABC_MI_DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_EXIT_TRACE_END\n");
  if (producer_started) {
  ...
  if (!valid) {
    fprintf(stderr,
        "dynamic full non-direct-small unmapped reabandon fixture stopped at stage %d (worker=%d)\n",
...
DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_EXIT_KIND = (
    "mimalloc-x86_64-dynamic-full-non-direct-small-unmapped-reabandon-differential-evidence"
)
...
def validate_c_command(command: list[str], schema: dict) -> None:
    ...
    if definitions != list(EXPECTED_COMPILE_DEFINITIONS) or definitions != list(
        ...
        raise EvidenceError("dynamic-full-non-direct-small-unmapped C command compile definitions drifted")
    if (
        ...
        raise EvidenceError(
            "dynamic-full-non-direct-small-unmapped C command release pthread/TLS selection drifted"
        )


def validate_normalized_c_command(command: object, schema: dict) -> None:
    stem = "dynamic-full-non-direct-small-unmapped-reabandon"
    expected = [
        "-std=c11",
        "-fPIC",
        "-ftls-model=initial-exec",
    ...
    if (
        ...
        raise EvidenceError("dynamic-full-non-direct-small-unmapped report C command drifted")
...
def build_c_trace(
    ...
    try:
        _base.run.require_success(
            ...
            "pinned C dynamic-full-non-direct-small-unmapped fixture build",
        )
        header = _base.run.command_record((readelf, "-h", str(probe_binary)), cwd=source)
        _base.run.require_success(
            header, "pinned C dynamic-full-non-direct-small-unmapped fixture ELF identity"
        )
        elf = _base.run.parse_elf_identity(str(header["stdout"]), "x86_64")
        execution = _base.run.command_record((str(probe_binary),), cwd=source)
        _base.run.require_success(
            execution, "pinned C dynamic-full-non-direct-small-unmapped fixture execution"
        )
    except _base.run.HarnessError as error:
        raise EvidenceError(str(error)) from error
    trace = parse_trace(
        str(execution["stdout"]),
        description="pinned C dynamic-full-non-direct-small-unmapped trace",
    )
    validate_trace(trace, description="pinned C dynamic-full-non-direct-small-unmapped trace")
    return {
...
def report_from_results(**kwargs):
    ...
    report["kind"] = DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_EXIT_KIND
    validate_report(report)
    return report


def validate_report(report: dict) -> None:
    if report.get("kind") != DYNAMIC_FULL_NON_DIRECT_SMALL_UNMAPPED_EXIT_KIND:
        raise EvidenceError("dynamic-full-non-direct-small-unmapped report kind drifted")
    ...
    if c_probe.get("source_sha256") != sha256_bytes(C_TRACE_PROBE.encode("utf-8")):
        raise EvidenceError("dynamic-full-non-direct-small-unmapped report C source hash drifted")
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"allocator x86-64 dynamic-full-non-direct-small-unmapped differential: FAIL: {error}",
            file=os.sys.stderr,
        )
        return 1
    print(
        "allocator x86-64 dynamic-full-non-direct-small-unmapped differential: PASS "
