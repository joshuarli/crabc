#!/usr/bin/env python3
"""Differentially prove two homogeneous dynamic full-non-direct-small pages.

This is a private native Linux/x86-64 mimalloc v3.5.0 engine lane.  A real
worker pthread fills two same-size arena-backed full non-direct-small pages in
one ordinary regular bin, runs ``mi_thread_done``, and is joined before the
consumer re-resolves either page through the persistent PageMap.  Each page
independently exercises six unmapped prefix frees, the mapped seventh-free
boundary, and terminal one-slice release.  The Rust side is a typed
current-thread owner-exit model; it does not claim literal pthread creation or
join parity.
"""
...
SCHEMA_PATH = ROOT / "compat/allocator/x86_64-dynamic-full-non-direct-small-homogeneous-aggregate-evidence-v3.5.0.json"
REPORT_DEFAULT = ROOT / "compat/reports/allocator/x86_64/dynamic-full-non-direct-small-homogeneous-aggregate.json"
RUST_TEST_SOURCE = ROOT / "crabc-mimalloc/src/dynamic_theap.rs"
EXPECTED_PROFILE = "linux-x86_64-private-dynamic-full-non-direct-small-homogeneous-aggregate"
RUST_TEST_FILTER = (
    "dynamic_theap::tests::"
    "x86_64_dynamic_full_non_direct_small_homogeneous_aggregate_trace_matches_pinned_c"
)
TRACE_BEGIN = "CRABC_MI_DYNAMIC_FULL_NON_DIRECT_SMALL_HOMOGENEOUS_AGGREGATE_TRACE_BEGIN"
TRACE_END = "CRABC_MI_DYNAMIC_FULL_NON_DIRECT_SMALL_HOMOGENEOUS_AGGREGATE_TRACE_END"
EXPECTED_SCOPE = {
    ...
    "dynamic_full_non_direct_small_regular_bin_only": True,
    "dynamic_full_non_direct_small_homogeneous_aggregate_only": True,
    ...
    "two_or_more_full_non_direct_small_pages_required": True,
}
EXPECTED_TLS = {
...
def _page_values(prefix: str) -> dict[str, int]:
    return {
        ...
        f"{prefix}.page_map_slice_count_after_thread_done": 1,
        f"{prefix}.page_map_all_slices_registered_after_thread_done": 1,
        f"{prefix}.slice_count_after_thread_done": 1,
        f"{prefix}.arena_page_bitmap_set_after_thread_done": 1,
        f"{prefix}.ordinary_queue_detached_after_thread_done": 1,
        f"{prefix}.used_after_thread_done": 51,
        f"{prefix}.unmapped_prefix_free_count": 6,
        f"{prefix}.used_after_unmapped_prefix": 45,
        ...
        f"{prefix}.used_after_reabandon_boundary": 44,
...
EXPECTED_TRACE_VALUES = {
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.arena_backed": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.small_page": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.non_direct_small": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.page_count": 2,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.same_size": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.full_before_thread_done": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.ordinary_regular_bin_before_thread_done": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.ordinary_queue_count_before_thread_done": 2,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.direct_cache_empty_before_thread_done": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.no_remote_free_before_thread_done": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.request_size": 1032,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.block_size": 1280,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.capacity": 51,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.reserved": 51,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.slice_count": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.dynamic_abandoned_count_after_thread_done": 0,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.first_terminal_released_only": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.second_page_retained_after_first_terminal": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.dynamic_abandoned_count_after_first_terminal": 0,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.dynamic_abandoned_count_after_second_boundary": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.dynamic_abandoned_count_after_final_terminal": 0,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.route_empty_after_final_terminal": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.valid": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.page_map_registered_after_first_terminal": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.page_map_slice_count_after_first_terminal": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.page_map_all_slices_registered_after_first_terminal": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.arena_page_bitmap_set_after_first_terminal": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.unmapped_after_first_terminal": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.abandoned_after_first_terminal": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.dynamic_abandoned_bitmap_clear_after_first_terminal": 1,
    "trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1.used_after_first_terminal": 51,
}
EXPECTED_TRACE_VALUES.update(_page_values("trace.dynamic_full_non_direct_small_homogeneous_aggregate.page0"))
EXPECTED_TRACE_VALUES.update(_page_values("trace.dynamic_full_non_direct_small_homogeneous_aggregate.page1"))
...
static bool direct_empty(const mi_theap_t* theap) {
  ...
  for (size_t i = 0; i < MI_PAGES_DIRECT; i++) {
    if (theap->pages_free_direct[i] != _mi_page_empty_get()) return false;
  }
  return true;
}
...
static bool ordinary_queue_has_two(const mi_page_queue_t* queue, mi_page_t* first, mi_page_t* second) {
  if (queue == NULL || queue->count != PAGE_COUNT || queue->first == NULL) return false;
  size_t count = 0;
  bool has_first = false;
  bool has_second = false;
  for (mi_page_t* page = queue->first; page != NULL && count <= PAGE_COUNT; page = page->next) {
...
static bool page_map_one(mi_page_t* page, uintptr_t* slice_start) {
  uint8_t* start = mi_page_slice_start(page);
  if (start == NULL) return false;
  *slice_start = (uintptr_t)start;
  return _mi_safe_ptr_page((const void*)start) == page;
}

static bool map_span_is(mi_page_t* page, uintptr_t start, size_t count, bool mapped) {
  (void)page;
  for (size_t i = 0; i < count; i++) {
    if ((_mi_safe_ptr_page((const void*)(start + i * MI_ARENA_SLICE_SIZE)) != NULL) != mapped) {
      return false;
    }
  }
  return true;
...
static void out_page_bool(size_t page, const char* name, int value) {
  printf("trace.dynamic_full_non_direct_small_homogeneous_aggregate.page%zu.%s=%d\n", page, name, value ? 1 : 0);
}

static void out_page_number(size_t page, const char* name, size_t value) {
  printf("trace.dynamic_full_non_direct_small_homogeneous_aggregate.page%zu.%s=%zu\n", page, name, value);
}
...
static void* producer_main(void* argument) {
  ...
  if (theap == NULL) {
    fixture->failure_stage = 2;
    goto failed;
  }

  while (fixture->block_count < MAX_BLOCKS && fixture->page_blocks[1] < BLOCKS_PER_PAGE) {
    void* block = mi_heap_malloc(heap, request);
    if (block == NULL) {
      failure = 3;
      goto failed;
    }
    ...
    size_t slot = (fixture->pages[0] == NULL || page == fixture->pages[0]) ? 0 : 1;
    if (fixture->pages[slot] == NULL) fixture->pages[slot] = page;
    if (page != fixture->pages[slot] || (slot == 1 && fixture->pages[0] == NULL)) {
      mi_free(block);
      failure = 5;
      goto failed;
    }
  ...
  fixture->ordinary_queue = ordinary_queue_has_two(
      mi_page_queue(theap, fixture->pages[0]->block_size),
      fixture->pages[0], fixture->pages[1]);
  ...
  fixture->heap = heap;
  signal_ready(fixture, fixture->ordinary_queue && fixture->direct_cache_empty);
...
failed:
  ...
  fixture->heap = NULL;
  fixture->failure_stage = failure != 0 ? failure : 8;
  signal_ready(fixture, false);
  return NULL;
}
...
int main(void) {
  ...
  int direct = 0;
  int no_remote = 0;
  size_t dynamic_exit = 0;
  size_t dynamic_first = 0;
  size_t dynamic_second = 0;
  size_t dynamic_final = 0;
  int page_map_reg[PAGE_COUNT] = {0, 0};
  ...
  size_t page1_first_map_count = 0;
  size_t page1_first_used = 0;
  int page1_first_map_registered = 0;
  int page1_first_all = 0;
  int page1_first_arena_set = 0;
  int page1_first_unmapped = 0;
  int page1_first_abandoned = 0;
  int page1_first_dynamic_clear = 0;
  int first_only = 0;
  ...
  (void)pthread_mutex_unlock(&fixture.mutex);
  if (!fixture.setup_valid || fixture.heap == NULL) goto output;
  mi_page_t* before = fixture.pages[0];
  if (before == NULL) goto output;
  ...
  full = mi_page_is_full(fixture.pages[0]) && mi_page_is_full(fixture.pages[1]);
  ordinary = fixture.ordinary_queue;
  direct = fixture.direct_cache_empty;
  no_remote = mi_page_thread_free(fixture.pages[0]) == NULL
      && mi_page_thread_free(fixture.pages[1]) == NULL;
  same_size = fixture.pages[1]->block_size == block_size
      && fixture.pages[1]->capacity == capacity
      && fixture.pages[1]->reserved == reserved;
  if (!arena_backed || !small_page || !non_direct_small || !full || !ordinary
      || !direct || !no_remote || !same_size || capacity != 51
      || reserved != 51 || block_size != 1280) goto output;
  ...
  if (!fixture.producer_done) goto output;
  ...
  for (size_t index = 0; index < PAGE_COUNT; index++) {
    ...
    map_count[index] = page_map_one(page, &starts[index]) ? 1 : 0;
    page_map_reg[index] = map_count[index] == 1
        && map_span_is(page, starts[index], 1, true);
    page_map_all[index] = page_map_reg[index] && map_count[index] == page_slice_count[index];
    ...
    arena_set[index] = mi_bitmap_is_setN(
        arena_pages->pages, page->memid.mem.arena.slice_index, 1);
    detached[index] = !mi_page_is_in_full(page)
        && page->next == NULL && page->prev == NULL && !mi_page_is_owned(page);
    used_exit[index] = page->used;
  }
  dynamic_exit = mi_atomic_load_relaxed(dynamic_abandoned_count);
  if (dynamic_exit != 0) goto output;

  for (size_t page_index = 0; page_index < PAGE_COUNT; page_index++) {
    size_t start = page_index * BLOCKS_PER_PAGE;
    prefix[page_index] = reserved / 8;
    if (prefix[page_index] != 6) goto output;
    for (size_t index = 0; index < prefix[page_index]; index++) {
      mi_free(fixture.blocks[start + index]);
      fixture.blocks[start + index] = NULL;
    }
    mi_page_t* page = _mi_safe_ptr_page(fixture.blocks[start + prefix[page_index]]);
    ...
    unmapped_prefix[page_index] = !mi_page_is_abandoned_mapped(page)
        ...
        && map_span_is(page, starts[page_index], 1, true)
        && (arena_pages->pages_abandoned[bin] == NULL
            || mi_bitmap_is_clearN(
                arena_pages->pages_abandoned[bin],
                page->memid.mem.arena.slice_index, 1))
        && mi_atomic_load_relaxed(dynamic_abandoned_count) == 0;
    if (!unmapped_prefix[page_index] || used_prefix[page_index] != 45) goto output;

    mi_free(fixture.blocks[start + prefix[page_index]]);
    fixture.blocks[start + prefix[page_index]] = NULL;
    page = _mi_safe_ptr_page(fixture.blocks[start + prefix[page_index] + 1]);
    if (page == NULL) goto output;
    ...
    if (!mapped_boundary[page_index] || !abandoned_set[page_index]
        || used_boundary[page_index] != 44
        || mi_atomic_load_relaxed(dynamic_abandoned_count) != 1) goto output;
    if (page_index == 1) dynamic_second = mi_atomic_load_relaxed(dynamic_abandoned_count);

    for (size_t index = prefix[page_index] + 1; index + 1 < BLOCKS_PER_PAGE; index++) {
      mi_free(fixture.blocks[start + index]);
      fixture.blocks[start + index] = NULL;
    }
    mi_free(fixture.blocks[start + BLOCKS_PER_PAGE - 1]);
    fixture.blocks[start + BLOCKS_PER_PAGE - 1] = NULL;
    map_clear[page_index] = map_span_is(page, starts[page_index], 1, false);
    arena_clear[page_index] = mi_bitmap_is_clearN(
        arena_pages->pages, page->memid.mem.arena.slice_index, 1);
    slices_free[page_index] = mi_bbitmap_is_setN(
        arena->slices_free, page->memid.mem.arena.slice_index, 1);
    abandoned_clear[page_index] = arena_pages->pages_abandoned[bin] == NULL
        || mi_bitmap_is_clearN(
            arena_pages->pages_abandoned[bin],
            page->memid.mem.arena.slice_index, 1);
    if (!map_clear[page_index] || !arena_clear[page_index]
        || !slices_free[page_index] || !abandoned_clear[page_index]) goto output;

    if (page_index == 0) {
      dynamic_first = mi_atomic_load_relaxed(dynamic_abandoned_count);
      page = _mi_safe_ptr_page(fixture.blocks[BLOCKS_PER_PAGE]);
      if (page == NULL) goto output;
      page1_first_map_count = page_map_one(page, &starts[1]) ? 1 : 0;
      page1_first_map_registered = page1_first_map_count == 1
          && map_span_is(page, starts[1], 1, true);
      page1_first_all = page1_first_map_registered && page1_first_map_count == 1;
      page1_first_arena_set = mi_bitmap_is_setN(
          arena_pages->pages, page->memid.mem.arena.slice_index, 1);
      page1_first_unmapped = !mi_page_is_abandoned_mapped(page)
          && mi_page_is_abandoned(page)
          && mi_page_thread_free(page) == NULL;
      page1_first_abandoned = mi_page_is_abandoned(page);
      page1_first_dynamic_clear = arena_pages->pages_abandoned[bin] == NULL
          || mi_bitmap_is_clearN(
              arena_pages->pages_abandoned[bin],
              page->memid.mem.arena.slice_index, 1);
      page1_first_used = page->used;
      second_retained = page1_first_map_registered && page1_first_all
          && page1_first_arena_set && page1_first_unmapped
          && page1_first_abandoned && page1_first_dynamic_clear
          && page1_first_used == 51;
      first_only = second_retained;
    }
    if (page_index == 1) dynamic_final = mi_atomic_load_relaxed(dynamic_abandoned_count);
  }

  route_empty = dynamic_final == 0;
  valid = arena_backed && small_page && non_direct_small && full && ordinary
      && direct && no_remote && same_size && page_slice_count[0] == 1
      && page_slice_count[1] == 1 && dynamic_exit == 0 && first_only
      && second_retained && dynamic_first == 0 && dynamic_second == 1
      && dynamic_final == 0 && page1_first_map_registered && page1_first_all
      && page1_first_arena_set && page1_first_unmapped && page1_first_abandoned
      && page1_first_dynamic_clear && page1_first_used == 51;
output:
  printf("CRABC_MI_DYNAMIC_FULL_NON_DIRECT_SMALL_HOMOGENEOUS_AGGREGATE_TRACE_BEGIN\n");
#define B(key, value) printf("trace.dynamic_full_non_direct_small_homogeneous_aggregate.%s=%d\n", key, (value) ? 1 : 0)
#define N(key, value) printf("trace.dynamic_full_non_direct_small_homogeneous_aggregate.%s=%zu\n", key, (size_t)(value))
  B("arena_backed", arena_backed);
  B("small_page", small_page);
  B("non_direct_small", non_direct_small);
  ...
  N("ordinary_queue_count_before_thread_done", fixture.ordinary_queue ? 2 : 0);
  B("direct_cache_empty_before_thread_done", direct);
  ...
  N("dynamic_abandoned_count_after_thread_done", dynamic_exit);
  for (size_t index = 0; index < PAGE_COUNT; index++) {
    ...
    out_page_bool(index, "page_map_registered_after_thread_done", page_map_reg[index]);
    ...
    out_page_number(index, "slice_count_after_thread_done", page_slice_count[index]);
    ...
    out_page_bool(index, "dynamic_abandoned_bitmap_set_after_reabandon_boundary", abandoned_set[index]);
    ...
    out_page_bool(index, "dynamic_abandoned_bitmap_clear_after_terminal_free", abandoned_clear[index]);
  }
  out_page_bool(1, "page_map_registered_after_first_terminal", page1_first_map_registered);
  out_page_number(1, "page_map_slice_count_after_first_terminal", page1_first_map_count);
  out_page_bool(1, "page_map_all_slices_registered_after_first_terminal", page1_first_all);
  out_page_bool(1, "arena_page_bitmap_set_after_first_terminal", page1_first_arena_set);
  out_page_bool(1, "unmapped_after_first_terminal", page1_first_unmapped);
  out_page_bool(1, "abandoned_after_first_terminal", page1_first_abandoned);
  out_page_bool(1, "dynamic_abandoned_bitmap_clear_after_first_terminal", page1_first_dynamic_clear);
  out_page_number(1, "used_after_first_terminal", page1_first_used);
  N("dynamic_abandoned_count_after_first_terminal", dynamic_first);
  N("dynamic_abandoned_count_after_second_boundary", dynamic_second);
  N("dynamic_abandoned_count_after_final_terminal", dynamic_final);
  B("first_terminal_released_only", first_only);
  B("second_page_retained_after_first_terminal", second_retained);
  B("route_empty_after_final_terminal", route_empty);
  B("valid", valid);
  printf("CRABC_MI_DYNAMIC_FULL_NON_DIRECT_SMALL_HOMOGENEOUS_AGGREGATE_TRACE_END\n");
  ...
  if (worker_heap != NULL) mi_heap_destroy(worker_heap);
  if (options_changed) {
...
def _schema_template() -> dict:
    ...
    value["schema"] = "crabc-mimalloc-x86_64-dynamic-full-non-direct-small-homogeneous-aggregate-evidence"
...
def load_schema(path=None):
    ...
    try:
        ...
        raise EvidenceError("cannot read dynamic-full-non-direct-small-homogeneous-aggregate schema") from error
    if not exactly_matches(schema, _schema_template()):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate checked-in schema drifted")
    try:
        pin = RUNNER.load_pin()
    except RUNNER.HarnessError as error:
        raise EvidenceError("cannot validate pinned dynamic-full-non-direct-small-homogeneous-aggregate upstream") from error
    ...
    if not exactly_matches(observed, EXPECTED_UPSTREAM) or pin["sha256"] != EXPECTED_ARCHIVE_SHA256:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate upstream archive pin drifted")
    return schema
...
def validate_c_command(command, schema):
    ...
    if schema.get("tls") != EXPECTED_TLS or definitions != list(schema["compile_definitions"]):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate C command contract drifted")
    if flags != list(schema["release_flags"]) or "-pthread" not in command or "-ftls-model=initial-exec" not in command:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate C command release pthread/TLS drifted")


def validate_normalized_c_command(command, schema):
    stem = "dynamic-full-non-direct-small-homogeneous-aggregate"
    ...
    if not isinstance(command, list) or not command or Path(command[0]).name != "musl-gcc" or command[1:] != expected:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report C command drifted")
...
def build_c_trace(compiler, readelf, source, temporary, schema):
    ...
    try:
        RUNNER.require_success(RUNNER.command_record(command, cwd=source), "pinned C dynamic full-non-direct-small homogeneous aggregate build")
        header = RUNNER.command_record((readelf, "-h", str(probe_binary)), cwd=source); RUNNER.require_success(header, "pinned C dynamic full-non-direct-small homogeneous aggregate ELF identity")
        elf = RUNNER.parse_elf_identity(str(header["stdout"]), "x86_64")
        execution = RUNNER.command_record((str(probe_binary),), cwd=source); RUNNER.require_success(execution, "pinned C dynamic full-non-direct-small homogeneous aggregate execution")
    except RUNNER.HarnessError as error:
        raise EvidenceError(str(error)) from error
    trace = parse_trace(str(execution["stdout"]), description="pinned C dynamic full-non-direct-small homogeneous aggregate trace"); validate_trace(trace, description="pinned C dynamic full-non-direct-small homogeneous aggregate trace")
...
def compare_traces(c_trace, rust_trace):
    validate_trace(c_trace, description="pinned C dynamic full-non-direct-small homogeneous aggregate trace")
    validate_trace(rust_trace, description="Rust dynamic full-non-direct-small homogeneous aggregate trace")
    mismatches = [
    ...
    if mismatches:
        raise EvidenceError("Rust dynamic full-non-direct-small homogeneous aggregate trace differs from pinned C: " + ", ".join(mismatches))
...
def report_from_results(*, schema, provenance, archive_sha256, anchors, c_probe, rust_probe):
    ...
    if not isinstance(c_trace, Mapping) or not isinstance(rust_trace, Mapping):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report inputs lack trace records")
    report = {
        "c_probe": dict(c_probe),
        "comparison": compare_traces(c_trace, rust_trace),
        "format": 1,
        "kind": "mimalloc-x86_64-dynamic-full-non-direct-small-homogeneous-aggregate-differential-evidence",
...
def validate_report(report):
    ...
    if not isinstance(report, dict) or set(report) != required:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report schema drifted")
    if report["format"] != 1 or report["status"] != "passed":
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report format/status drifted")
    if report["kind"] != kind:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report kind drifted")
    if not exactly_matches(report["target"], EXPECTED_TARGET) or not exactly_matches(report["upstream"], EXPECTED_UPSTREAM):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report target/upstream drifted")
    if report["profile"] != EXPECTED_PROFILE or not exactly_matches(report["scope"], EXPECTED_SCOPE):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report private boundary drifted")
    if not any(
        ...
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report lacks native x86-64 provenance")
    schema = load_schema()
    if not exactly_matches(report["trace"], schema["trace"]):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report trace contract drifted")
    source = report["source"]
    if not isinstance(source, dict) or set(source) != {"archive_sha256", "anchors", "release_flags", "release_source_set"}:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report source record is malformed")
    if source["archive_sha256"] != EXPECTED_ARCHIVE_SHA256:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report archive identity drifted")
    if not exactly_matches(source["anchors"], schema["source_anchors"]):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report source anchors drifted")
    if not exactly_matches(source["release_flags"], schema["release_flags"]):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report release flags drifted")
    if not exactly_matches(source["release_source_set"], schema["release_source_set"]):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report source set drifted")
    ...
    if not isinstance(c_probe, dict) or set(c_probe) != {"build_command", "elf", "run_command", "source_sha256", "trace"}:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report C probe record drifted")
    if not isinstance(rust_probe, dict) or set(rust_probe) != {"cargo_command", "lockfile", "passed_test_count", "source", "target_dir", "trace"}:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report Rust probe record drifted")
    if not exactly_matches(c_probe["elf"], EXPECTED_C_ELF):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report C ELF identity drifted")
    stem = "dynamic-full-non-direct-small-homogeneous-aggregate"
    if c_probe["run_command"] != [f"{NORMALIZED_EVIDENCE_ROOT}/{stem}-c"]:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report C run command drifted")
    if c_probe["source_sha256"] != sha256_bytes(C_TRACE_PROBE.encode()):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report C source hash drifted")
    ...
    if type(rust_probe["passed_test_count"]) is not int or rust_probe["passed_test_count"] != 1:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report Rust test selection drifted")
    if not exactly_matches(rust_probe["target_dir"], {"isolated": True, "retained": False, "value": f"{NORMALIZED_EVIDENCE_ROOT}/rust-target"}):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report Rust target directory drifted")
    ...
    if not exactly_matches(rust_probe["lockfile"], {"path": relative(LOCKFILE), "sha256": sha256_file(LOCKFILE)}):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report Rust lockfile identity drifted")
    if not exactly_matches(rust_probe["source"], {"path": relative(RUST_TEST_SOURCE), "sha256": sha256_file(RUST_TEST_SOURCE)}):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report Rust source identity drifted")
    if not isinstance(c_probe["trace"], Mapping) or not isinstance(rust_probe["trace"], Mapping):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report lacks C/Rust traces")
    if not exactly_matches(report["comparison"], compare_traces(c_probe["trace"], rust_probe["trace"])):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report comparison drifted")


def validate_normalized_rust_command(command):
    if not isinstance(command, list) or not command or not all(isinstance(part, str) for part in command):
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report Rust command is malformed")
    expected = [
    ...
    if Path(command[0]).name != "cargo" or command[1:] != expected:
        raise EvidenceError("dynamic-full-non-direct-small-homogeneous-aggregate report Rust command drifted")
...
def validate_source_anchors(schema, source):
    ...
    for anchor in schema["source_anchors"]:
        ...
        if observed != anchor["sha256"]:
            raise EvidenceError(f"pinned dynamic-full-non-direct-small-homogeneous-aggregate source anchor drifted: {member}")
...
def build_rust_trace(cargo, temporary):
    ...
    try:
        ...
        raise EvidenceError(str(error)) from error
    if passed != 1:
        raise EvidenceError(f"Rust dynamic-full-non-direct-small-homogeneous aggregate fixture passed {passed} tests, expected one")
    trace = parse_trace(str(execution["stdout"]) + "\n" + str(execution["stderr"]), description="Rust dynamic-full-non-direct-small-homogeneous aggregate trace")
    validate_trace(trace, description="Rust dynamic-full-non-direct-small-homogeneous aggregate trace")
    return {
...
def run_evidence(*, offline, report_path):
    ...
    try:
        ...
        raise EvidenceError(str(error)) from error
    with tempfile.TemporaryDirectory(prefix="crabc-mimalloc-x86_64-dynamic-full-non-direct-small-homogeneous-aggregate-") as temporary_name:
...
def main() -> int:
    ...
    try: report = run_evidence(offline=args.offline, report_path=args.report)
        ...
        print("allocator x86-64 dynamic-full-non-direct-small-homogeneous-aggregate differential: FAIL: " + str(error), file=os.sys.stderr); return 1
    print("allocator x86-64 dynamic-full-non-direct-small-homogeneous-aggregate differential: PASS " f"({report['comparison']['compared_value_count']} logical values; report: {relative(args.report)})"); return 0
