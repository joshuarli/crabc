...
| `x86_64_deferred_free_callback_evidence.py` and `x86_64-deferred-free-callback-evidence-v3.5.0.json` | Native x86-64-only private 12-field pinned-C/Rust differential for one crate-private deferred-free context: release/acquire registration order, callback heartbeat before selection, TLD recursion suppression around a legal nested direct-small allocation, Mini callbacks at generic 1k boundaries, Full at 10k, and the pinned C generic OOM forced retry. Rust invokes the user callback only after its persistent owner-cell borrow ends and revalidates the exact attachment lease before phase C. It is dispatched by `allocator-deferred-free-callback`; it does not claim public `mi_*` registration, teardown callback completion, a production backend, public runtime support, or AArch64 evidence. |
