...
| Path | Contract |
| --- | --- |
| `api-v3.5.0.json` | Deterministic, source-audited AArch64 public-header and root-CMake mode inventory. It separates external C declarations, static inlines, types, enum options, macros, override macros, C++ conveniences, and all 52 initial `MI_*` compile-time declarations. Every item and mode records target applicability, source-backed exclusions, implementation/evidence status, and an explicit blocker where parity is required but incomplete. Malloc-engine readiness remains owned by the architecture/M5 gates and cannot be inferred from the separate full Linux/AArch64 v3.5.0 parity track. Native x86-64 parity requires a separate architecture-qualified inventory. |
