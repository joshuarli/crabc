#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ <unistd.h> pathconf declaration gate.
#
...
# host-libc leakage. `pathconf` is unconditional in musl's Linux <unistd.h>;
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/pathconf_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/pathconf_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 unistd.h pathconf ABI: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pathconf-header.XXXXXX)"
    ...
    for language in c cxx; do
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                ...
                object="$work_dir/${variant}-${label}-pathconf.o"
...
printf 'x86 pinned-musl/project C/C++ <unistd.h> pathconf ABI: PASS\n'
