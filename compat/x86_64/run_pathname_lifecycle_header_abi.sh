#!/usr/bin/env bash
# Native Linux/x86-64 selected pathname-lifecycle C/C++ header ABI matrix.
#
# Pinned musl 1.2.6 is the declaration, layout, value, and C++ C-linkage
# oracle. The candidate uses raw GCC with only project headers and compiler
# builtin headers, so an ambient libc cannot conceal a header mismatch. This
# is compile-only evidence for one bounded static archive leaf, not family
# completion, filesystem policy, or public x86 support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/pathname_lifecycle_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/pathname_lifecycle_header_abi_probe.cpp"
...
readonly -a CXX_SYMBOLS=(chdir getcwd mkdir unlink rmdir remove rename link symlink readlink chmod fchmod truncate)

fail() {
    printf 'ERROR: x86 pathname-lifecycle headers: %s\n' "$*" >&2
    exit 1
}
...
check_trace() {
    ...
    for header in fcntl.h stdio.h sys/stat.h sys/types.h unistd.h; do
...
[ -f "$C_PROBE" ] || fail "missing C pathname-lifecycle ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ pathname-lifecycle ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pathname-lifecycle-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ pathname-lifecycle ABI matrix: PASS (%s profiles; compile-only)\n' \
