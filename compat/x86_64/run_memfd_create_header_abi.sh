#!/usr/bin/env bash
# Native Linux/x86-64 GNU <sys/mman.h> memfd_create header matrix.
#
# Pinned musl 1.2.6 is the C/C++ declaration, GNU-only feature-visibility,
# value, and C-linkage oracle. The candidate uses raw GCC with project headers
# only, so an ambient libc cannot conceal a mismatch. This compile-only gate
# selects neither a crabc archive nor runtime behavior.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/memfd_create_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/memfd_create_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 GNU memfd_create header ABI: %s\n' "$*" >&2
    exit 1
}
...
mode_arguments() {
    ...
    case "$mode" in
        normal)
            case "$profile" in
                c11-gnu|cxx17-gnu)
                    printf '%s\n' '-DCRABC_MEMFD_CREATE_REQUIRE_GNU'
                    ;;
            esac
            ;;
        gnu-hidden)
            printf '%s\n' '-DCRABC_MEMFD_CREATE_REQUIRE_GNU_HIDDEN'
            ;;
...
check_trace() {
    ...
    for header in sys/mman.h bits/mman.h sys/syscall.h bits/syscall.h; do
...
expect_hidden_failure() {
    ...
    if compile_profile "$tree" "$profile" gnu-hidden "$diagnostic" "$object"; then
        fail "$tree $profile unexpectedly exposes GNU memfd_create"
    fi
    grep -Fq 'memfd_create' "$diagnostic" ||
        fail "$tree $profile hidden GNU diagnostic does not name memfd_create"
}
...
[ -f "$C_PROBE" ] || fail "missing C GNU memfd_create header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ GNU memfd_create header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-memfd-create-header.XXXXXX)"
...
printf 'x86 pinned-musl/project GNU memfd_create C/C++ header ABI matrix: PASS (%s profiles; compile-only)\n' \
