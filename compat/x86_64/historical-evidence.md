# Historical snapshot (source 50fcb75d84e6a57938b617691d4fdcde0992bae0)

This file preserves the former compat/x86_64/README.md verbatim below this
banner. Its instructions, commands, and status claims are historical evidence,
not the live queue. Return to [README.md](README.md) and [plan.md](../../plan.md)
...
get/set-concurrency status, plus `pthread_attr_t` record initialization,
validation, and metadata queries. Those commands are isolated C ABI leaves in
the still-planned `libc.posix-runtime` and `libc.pthread-tls` families, not a
...
./scripts/dev-x86_64.sh tcp-header-abi
...
Its current 393 comparable red source-form rows, one
...
non-addressable record or field facts explicit. Its zero current comparable
byte-layout mismatch rows, 1,280 matched rows, one `aio.h:c11-strict`
...
macro visibility across all 1,337 rows: 296 current identity-mismatch rows,
984 matched rows, while the one `aio.h:c11-strict` oracle-not-applicable row
and 56 project-only rows retain checked candidate fact summaries and digests.
Its 1,918 same-identity source-form
differences across 302 rows—including 97 form-only rows—remain separately
...
`tcp-header-abi` compares source-faithful project and pinned-musl C/C++
`<netinet/tcp.h>` in strict, POSIX, XOPEN, GNU, and BSD feature profiles. It
checks unconditional TCP option/state and netlink vocabulary, GNU/BSD
option-parser constants and the x86 `struct tcphdr` layout, plus GNU-only
`tcp_info`, MD5, repair-window, and zero-copy-receive records. It is
compile-only header evidence, not TCP socket-option or transport behavior,
archive linkage, installed-header completion, family completion, or public x86
support.
...
`libc-pthread-attributes` is a private `pthread_attr_t` record-metadata
artifact under the same still-planned `libc.pthread-tls` family. Its
project-header C fixture first runs against pinned musl 1.2.6 and then through
a `-nostdlib -static` candidate. It selects exactly the 18 standard metadata
...
scheduler policy, and scheduler parameters. The private
`libc/src/c_abi/x86_64/pthread_attr.rs` record retains musl's 56-byte,
align-eight representation, 131072-byte default stack, 8192-byte default
guard, direct status validation, raw scheduler metadata, and the unset-stack
getter's no-output rule. `pthread_create` deliberately stays null-attribute-
only, so this does not select custom-stack or detached-at-create behavior,
scheduler or guard runtime effects, GNU default attributes,
`pthread_getattr_np`/live-thread inspection, TLS/runtime state, family
...
`libc-pthread-create-join-tls`, `libc-pthread-identity`, `libc-c11-lifecycle`,
`libc-pthread-detach`, `libc-thrd-sleep`, `libc-thrd-yield`, `libc-pthread-cpuclock`, `libc-pthread-name`, `libc-pthread-attributes`, `libc-pthread-barrierattr-pshared`, `libc-pthread-barrier`, `libc-pthread-spin-destroy`, `libc-pthread-mutex-normal`,
