...
if family == 'dynamic':
    ...
    search = receipt_contract.validate(
        receipt, format=expected_format, label='owned syslog dynamic receipt',
        fail=lambda message: require(False, message),
    )
    receipt_contract.require_runpath(
        search, '/usr/lib', label='owned syslog dynamic receipt', fail=lambda message: require(False, message),
    )
else:
