#!/usr/bin/env bash
# Native Linux/x86-64 bounded static pthread mutexattr type-setter evidence.
#
...
# archive. It proves exactly pthread_mutexattr_settype's input-first
# 0/1/2-or-EINVAL branch and low-two-bit caller-record update. It does not
# select the getter, attribute lifecycle, mutex initialization or operation,
# threads, TLS, synchronization, cancellation, CRT, loader, sysroot, or
# public x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static pthread mutexattr type setter: %s\n' "$*" >&2
    exit 1
}
...
assert_direct_record_path() {
    local disassembly="$work_dir/pthread-mutexattr-settype-disassembly"

    objdump -d --disassemble=pthread_mutexattr_settype "$candidate" >"$disassembly"
    if grep -Eq '[[:space:]](syscall|call)([[:space:]]|$)|%fs:' "$disassembly"; then
        fail "pthread_mutexattr_settype must remain a direct TLS-free, syscall-free record setter"
    fi
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pthread-mutexattr-type-setter.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-pthread-mutexattr-type-setter-reference"
candidate="$work_dir/crabc-static-pthread-mutexattr-type-setter-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_mutexattr_type_setter_probe.c >/dev/null 2>"$header_trace"
for header in pthread.h bits/alltypes.h features.h errno.h limits.h; do
    grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
        fail "fixture did not use project $header"
done

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -pthread -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_pthread_mutexattr_type_setter_probe.c \
    -o "$reference"
if timeout "$EXECUTION_TIMEOUT" "$reference"; then
    :
else
    status=$?
    fail "pinned-musl mutexattr type-setter fixture exited ${status}"
fi
...
grep -Eq '[[:space:]][TW][[:space:]]pthread_mutexattr_settype$' "$archive_symbols" ||
    fail "archive does not define pthread_mutexattr_settype"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    -DCRABC_PTHREAD_MUTEXATTR_TYPE_SETTER_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_pthread_mutexattr_type_setter_probe.c \
    compat/x86_64/libc_pthread_mutexattr_type_setter_start.S "$archive" -o "$candidate"
...
grep -Eq '[[:space:]]pthread_mutexattr_settype$' "$candidate_symbols" ||
    fail "candidate does not define pthread_mutexattr_settype"
for unselected in pthread_mutexattr_gettype pthread_mutexattr_init \
    pthread_mutexattr_destroy pthread_mutexattr_setpshared \
    pthread_mutexattr_getpshared pthread_mutexattr_setprotocol \
    pthread_mutexattr_getprotocol pthread_mutexattr_setprioceiling \
    pthread_mutexattr_getprioceiling pthread_mutexattr_setrobust \
    pthread_mutexattr_getrobust pthread_mutex_init pthread_mutex_destroy \
    pthread_mutex_lock pthread_mutex_trylock pthread_mutex_unlock \
    pthread_mutex_timedlock pthread_mutex_consistent pthread_cond_init \
    pthread_cond_destroy pthread_cond_wait pthread_cond_timedwait \
    __errno_location __crabc_x86_static_tls_bootstrap; do
...
if timeout "$EXECUTION_TIMEOUT" "$candidate"; then
    ...
    status=$?
    fail "freestanding mutexattr type-setter fixture exited ${status}"
fi

printf 'x86 static crabc-libc pthread mutexattr type setter: PASS\n'
