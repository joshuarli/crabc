...
selected_archive="$work_dir/libcrabc-sched-priority-max.a"
selected_object="$work_dir/owner/sched_get_priority_max.o"
...
for header in errno.h sched.h sys/syscall.h features.h bits/alltypes.h bits/syscall.h; do
...
mapfile -t members < <(archive_member_for_symbol "$archive" sched_get_priority_max)
[ "${#members[@]}" -eq 1 ] ||
    fail "sched_get_priority_max must have exactly one crate object owner"
mapfile -t errno_members < <(archive_member_for_symbol "$archive" __errno_location)
[ "${#errno_members[@]}" -eq 1 ] ||
    fail "sched_get_priority_max requires exactly one errno owner"
mkdir "$work_dir/owner"
(
    cd "$work_dir/owner"
    ar x "$archive" "${members[0]}" "${errno_members[0]}"
    # Rust codegen batches this leaf with min and getscheduler. Relink the
    # reachable section graph from the max root instead of copying the member:
    # that preserves hidden errno data while dropping sibling entry sections.
    ld -r --gc-sections --undefined=sched_get_priority_max \
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    compat/x86_64/libc_sched_get_priority_max_start.S "$selected_archive" -o "$candidate"
