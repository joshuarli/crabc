/* Static crabc-libc x86-64 selected C11 thrd_yield fixture.
 *
 ...
 * selected crabc archive. It proves only C11 thrd_yield's direct raw Linux
 * sched_yield route: a normal yield and a fixture-local seccomp-forced EPERM
 * both leave C errno unchanged, exactly as musl's void-returning wrapper does.
 * The local BPF filter is test scaffolding, not a selected seccomp API. This
 * does not select sched_yield, scheduler policy or parameters, affinity,
 * pthread lifecycle/synchronization/TSS/cancellation, CRT, loader, sysroot,
 * general C11/pthread behavior, or public x86 support.
 */
 ...
 * otherwise ignored raw sched_yield error observable through errno stability.
 * Public seccomp headers and a general filter API remain unselected. */
...
_Static_assert(SYS_sched_yield == 24 && SYS_prctl == 157 && SYS_seccomp == 317,
    "x86 selected and fixture-only thrd_yield syscalls");
_Static_assert(__builtin_types_compatible_p(__typeof__(&thrd_yield),
    void (*)(void)), "thrd_yield declaration");
...
static int check_normal_yield_preserves_errno(void)
{
    ...
    thrd_yield();
    return errno == preserved_errno ? 0 : 1;
}

static int check_forced_error_preserves_errno(void)
{
    ...
    errno = preserved_errno;
    thrd_yield();
    return errno == preserved_errno ? 0 : 2;
}

int crabc_x86_64_thrd_yield_probe(void)
{
    int status;

    status = check_normal_yield_preserves_errno();
    if (status != 0)
        return 10 + status;
    status = check_forced_error_preserves_errno();
    if (status != 0)
        return 20 + status;
    return 0;
}

#ifndef CRABC_THRD_YIELD_FREESTANDING
int main(void)
{
    return crabc_x86_64_thrd_yield_probe();
}
#endif
