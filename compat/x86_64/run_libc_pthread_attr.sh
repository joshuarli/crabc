...
fail() {
    printf 'ERROR: x86 static pthread attributes: %s\n' "$*" >&2
    exit 1
}
...
assert_direct_record_paths() {
    ...
    for symbol in "${pthread_attr_symbols[@]}"; do
...
readonly -a pthread_attr_symbols=(
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_attr_probe.c >/dev/null 2>"$header_trace"
for header in errno.h limits.h pthread.h sched.h bits/alltypes.h; do
...
if timeout "$EXECUTION_TIMEOUT" "$reference"; then
    ...
    status=$?
    fail "pinned-musl pthread-attribute fixture exited ${status}"
fi
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_PTHREAD_ATTR_FREESTANDING \
    ...
    compat/x86_64/libc_pthread_attr_start.S "$archive" -o "$candidate"
...
for unselected in pthread_create pthread_join pthread_detach pthread_exit \
    pthread_getattr_np pthread_getattr_default_np pthread_setattr_default_np \
