#!/usr/bin/env bash
# Native Linux/x86-64 ualarm C/C++ declaration gate.
#
...
# historical `unsigned int ualarm(unsigned int, unsigned int)` spelling and
# its GNU/BSD/XOPEN<700 selectors; it selects no timer control or signal
# delivery policy.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 unistd.h ualarm ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/ualarm_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/ualarm_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-ualarm-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-ualarm-cxx.o"
candidate_cxx_object="$work_dir/candidate-ualarm-cxx.o"

default_definitions=()
strict_definitions=(-D__STRICT_ANSI__)
posix_definitions=(-D_POSIX_C_SOURCE=200809L)
xopen_700_definitions=(-D_XOPEN_SOURCE=700)
xopen_600_definitions=(-D_XOPEN_SOURCE=600 -DCRABC_EXPECT_UALARM)
bsd_definitions=(-D_BSD_SOURCE -DCRABC_EXPECT_UALARM)
gnu_definitions=(-D_GNU_SOURCE -DCRABC_EXPECT_UALARM)
...
if ! "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
    ...
    fail "project GNU ualarm header contract drifted"
fi
...
for object in "$oracle_cxx_object" "$candidate_cxx_object"; do
    ...
    grep -Eq '[[:space:]]ualarm$' <<<"$undefined" ||
        fail "C++ probe does not retain C linkage for ualarm"
    if grep -Eq '_Z.*ualarm' <<<"$undefined"; then
        fail "C++ probe retained a mangled ualarm reference"
    fi
done

# The selector is exact: GNU/BSD and XOPEN=600 expose ualarm, while default,
...
for definitions_name in default_definitions strict_definitions posix_definitions \
    ...
    declare -n definitions="$definitions_name"
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -DCRABC_REQUIRE_UALARM_HIDDEN -fsyntax-only "$c_probe" \
        >"$work_dir/oracle-c-${definitions_name}.out" 2>&1; then
        fail "pinned musl exposes ualarm outside its selected C profiles"
    fi
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -DCRABC_REQUIRE_UALARM_HIDDEN \
        -I "$ROOT_DIR/include" -fsyntax-only "$c_probe" \
        >"$work_dir/project-c-${definitions_name}.out" 2>&1; then
        fail "project unistd.h exposes ualarm outside its selected C profiles"
    fi
    if "$ORACLE_CC" -std=c++17 -x c++ -U_GNU_SOURCE "${definitions[@]}" \
        -DCRABC_REQUIRE_UALARM_HIDDEN -fsyntax-only "$cxx_probe" \
        >"$work_dir/oracle-cxx-${definitions_name}.out" 2>&1; then
        fail "pinned musl exposes ualarm outside its selected C++ profiles"
    fi
    if "$ORACLE_CC" -std=c++17 -x c++ -U_GNU_SOURCE "${definitions[@]}" \
        -DCRABC_REQUIRE_UALARM_HIDDEN -I "$ROOT_DIR/include" \
        -fsyntax-only "$cxx_probe" \
...
printf 'x86 pinned-musl/project C/C++ <unistd.h> ualarm ABI: PASS\n'
