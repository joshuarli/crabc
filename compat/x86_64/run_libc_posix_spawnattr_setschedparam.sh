#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc posix_spawnattr_setschedparam evidence.
#
...
readonly SOURCE="$ROOT_DIR/libc/src/c_abi/x86_64/posix_spawnattr_setschedparam.rs"

fail() {
    printf 'ERROR: x86 static libc posix_spawnattr_setschedparam: %s\n' "$*" >&2
    exit 1
}
...
assert_ignored_pointer_enosys_boundary() {
    ...
    objdump -d --disassemble=posix_spawnattr_setschedparam "$binary" >"$disassembly"
    grep -Eq '\$0x26(,|[[:space:]]|$)' "$disassembly" ||
        fail "posix_spawnattr_setschedparam does not materialize ENOSYS=38"
    if grep -Eq '[[:space:]](call|syscall)([[:space:]]|$)|\(%rdi\)|\(%rsi\)|%fs:' \
        "$disassembly"; then
        fail "posix_spawnattr_setschedparam widened beyond musl's ignored-pointer ENOSYS return"
    fi
}
...
[ -f "$SOURCE" ] || fail "missing posix_spawnattr_setschedparam source"

bash "$ROOT_DIR/compat/x86_64/run_musl_oracle.sh" >/dev/null
bash "$ROOT_DIR/compat/x86_64/run_posix_spawnattr_setschedparam_header_abi.sh" >/dev/null
grep -Fqx $'posix_spawnattr_setschedparam\tposix_spawnattr_sched.lo\tT\tGLOBAL\t0\t8' \
    "$AARCH64_STATIC_ABI" ||
    fail "AArch64 musl ABI oracle lost posix_spawnattr_setschedparam ownership"
for marker in \
    'src/process/posix_spawnattr_sched.c::posix_spawnattr_setschedparam' \
    'return ENOSYS;' \
    'pub extern "C" fn posix_spawnattr_setschedparam'; do
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-posix-spawnattr-setschedparam.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
selected_archive="$work_dir/libcrabc-posix-spawnattr-setschedparam.a"
selected_object="$work_dir/owner/posix_spawnattr_setschedparam.o"
reference="$work_dir/musl-posix-spawnattr-setschedparam-reference"
candidate="$work_dir/crabc-static-posix-spawnattr-setschedparam-candidate"
...
object_undefined="$work_dir/posix-spawnattr-setschedparam-undefined"
object_relocations="$work_dir/posix-spawnattr-setschedparam-relocations"
object_disassembly="$work_dir/posix-spawnattr-setschedparam-disassembly"
...
readelf --symbols --wide "$musl_object" | grep -E \
    '[[:space:]]FUNC[[:space:]]+GLOBAL[[:space:]].*[[:space:]]posix_spawnattr_setschedparam$' >/dev/null ||
    fail "pinned musl posix_spawnattr_sched.lo lacks strong posix_spawnattr_setschedparam"

"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_posix_spawnattr_setschedparam_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_posix_spawnattr_setschedparam_probe.c \
    -o "$reference"
"$reference" || fail "pinned-musl posix_spawnattr_setschedparam fixture failed"
...
grep -Eq "[[:space:]][TW][[:space:]]posix_spawnattr_setschedparam$" "$archive_symbols" ||
    fail "archive does not define posix_spawnattr_setschedparam"

mapfile -t members < <(archive_member_for_symbol "$archive" posix_spawnattr_setschedparam)
[ "${#members[@]}" -eq 1 ] || fail "posix_spawnattr_setschedparam must have exactly one crate object owner"
...
(
    ...
    objcopy --only-section=.text.posix_spawnattr_setschedparam \
        --keep-symbol=posix_spawnattr_setschedparam "${members[0]}" \
...
if [ "${exports[*]}" != "posix_spawnattr_setschedparam" ]; then
    printf 'expected: %s\nactual:   %s\n' "posix_spawnattr_setschedparam" "${exports[*]}" >&2
    fail "posix_spawnattr_setschedparam object export surface drifted"
fi
...
if [ -s "$object_undefined" ]; then
    ...
    fail "posix_spawnattr_setschedparam object unexpectedly depends on another symbol"
fi
...
if grep -Eq '[[:space:]](call|syscall)([[:space:]]|$)' "$object_disassembly"; then
    fail "posix_spawnattr_setschedparam object unexpectedly performs a call or syscall"
fi
assert_ignored_pointer_enosys_boundary "$object" "$work_dir/object-posix-spawnattr-setschedparam-disassembly"

"$ORACLE_CC" -std=c11 -DCRABC_POSIX_SPAWNATTR_SETSCHEDPARAM_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    -Wl,--gc-sections -Wl,-Map,"$link_map" \
    compat/x86_64/libc_posix_spawnattr_setschedparam_probe.c \
    compat/x86_64/libc_posix_spawnattr_setschedparam_start.S "$selected_archive" \
    -o "$candidate"
...
awk '$4 == "FUNC" && $5 == "GLOBAL" && $8 == "posix_spawnattr_setschedparam" { found = 1 }
     END { exit(found ? 0 : 1) }' "$candidate_symbols" ||
    fail "candidate lacks global posix_spawnattr_setschedparam"
...
assert_ignored_pointer_enosys_boundary "$candidate" "$work_dir/candidate-posix-spawnattr-setschedparam-disassembly"

"$candidate" || fail "freestanding posix_spawnattr_setschedparam fixture failed"

printf 'x86 static libc posix_spawnattr_setschedparam: PASS\n'
