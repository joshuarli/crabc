#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``tan``/``tanf`` leaf.
...
# and tangent kernels. The reducer's floor/scalbn calls are included as private
...
PRIVATE_RENAMES = {
    "__tan": "crabc_x86_math_tan_kernel_tan",
    "__tandf": "crabc_x86_math_tan_kernel_tandf",
    "__rem_pio2": "crabc_x86_math_tan_reduce_pio2",
    "__rem_pio2f": "crabc_x86_math_tan_reduce_pio2f",
    "__rem_pio2_large": "crabc_x86_math_tan_reduce_pio2_large",
    "floor": "crabc_x86_math_tan_provider_floor",
    "scalbn": "crabc_x86_math_tan_provider_scalbn",
}
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_tan_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_tan_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_tan_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_tan_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 tan/tanf translation.",
        " * Generated by compat/x86_64/generate_libc_math_tan.py; see",
        " * math_tan.rs for the source/license/ABI contract.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-tan-") as temporary:
