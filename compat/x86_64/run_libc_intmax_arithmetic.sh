#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc intmax arithmetic evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc intmax arithmetic: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-intmax-arithmetic.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"; archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-intmax-arithmetic-reference"; candidate="$work_dir/crabc-static-intmax-arithmetic-candidate"
...
cd "$ROOT_DIR"
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H compat/x86_64/libc_intmax_arithmetic_probe.c >/dev/null 2>"$trace"
for header in stddef.h inttypes.h stdint.h features.h bits/alltypes.h; do grep -Fq "$ROOT_DIR/include/$header" "$trace" || fail "fixture did not use project $header"; done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector -I"$ROOT_DIR/include" compat/x86_64/libc_intmax_arithmetic_probe.c -o "$reference"
"$reference" || fail "pinned-musl intmax arithmetic fixture failed"
...
for symbol in imaxabs imaxdiv; do grep -Eq "[[:space:]][TW][[:space:]]${symbol}$" "$archive_symbols" || fail "archive does not define $symbol"; done
for unselected in atoi atol atoll strtoimax strtoumax strtol strtoul strtoll strtoull a64l l64a; do if grep -Eq "[[:space:]][TW][[:space:]]${unselected}$" "$archive_symbols"; then fail "archive accidentally exports unselected $unselected"; fi; done
...
"$candidate" || fail "freestanding intmax arithmetic fixture failed"
printf 'x86 static libc intmax arithmetic: PASS\n'
