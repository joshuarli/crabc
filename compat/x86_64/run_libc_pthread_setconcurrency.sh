#!/usr/bin/env bash
# Native Linux/x86-64 bounded static pthread_setconcurrency evidence.
#
...
# archive. It proves exactly musl's stateless negative/zero/positive status
# table. It does not select thread creation, a stored concurrency setting,
# pthread_getconcurrency, scheduler policy, attributes, synchronization,
# cancellation, TLS, CRT, loader, sysroot, or public x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static pthread_setconcurrency: %s\n' "$*" >&2
    exit 1
}
...
assert_direct_status_path() {
    local disassembly="$work_dir/pthread-setconcurrency-disassembly"

    objdump -d --disassemble=pthread_setconcurrency "$candidate" >"$disassembly"
    if grep -Eq '[[:space:]](syscall|call)([[:space:]]|$)|%fs:' "$disassembly"; then
        fail "pthread_setconcurrency must remain a direct TLS-free, syscall-free status leaf"
    fi
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pthread-setconcurrency.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-pthread-setconcurrency-reference"
candidate="$work_dir/crabc-static-pthread-setconcurrency-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_setconcurrency_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -pthread -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_pthread_setconcurrency_probe.c \
    -o "$reference"
if timeout "$EXECUTION_TIMEOUT" "$reference"; then
    :
else
    status=$?
    fail "pinned-musl pthread_setconcurrency fixture exited ${status}"
fi
...
grep -Eq '[[:space:]][TW][[:space:]]pthread_setconcurrency$' "$archive_symbols" ||
    fail "archive does not define pthread_setconcurrency"
if grep -Eq '[[:space:]][TW][[:space:]]pthread_getconcurrency$' "$archive_symbols"; then
    fail "archive accidentally exports unselected pthread_getconcurrency"
fi
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    -DCRABC_PTHREAD_SETCONCURRENCY_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_pthread_setconcurrency_probe.c \
    compat/x86_64/libc_pthread_setconcurrency_start.S "$archive" -o "$candidate"
...
grep -Eq '[[:space:]]pthread_setconcurrency$' "$candidate_symbols" ||
    fail "candidate does not define pthread_setconcurrency"
for unselected in pthread_getconcurrency pthread_create pthread_detach pthread_join \
...
if timeout "$EXECUTION_TIMEOUT" "$candidate"; then
    ...
    status=$?
    fail "freestanding pthread_setconcurrency fixture exited ${status}"
fi

printf 'x86 static crabc-libc pthread_setconcurrency: PASS\n'
