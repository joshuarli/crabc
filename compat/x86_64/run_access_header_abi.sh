#!/usr/bin/env bash
# Native Linux/x86-64 direct <unistd.h>/<fcntl.h> access-header ABI matrix.
#
# Pinned musl 1.2.6 is the declaration, feature-selection, and C++ C-linkage
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/access_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/access_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 direct access header ABI: %s\n' "$*" >&2
    exit 1
}
...
mode_arguments() {
    ...
    case "$mode" in
        normal)
            case "$profile" in
                c11-gnu|cxx17-gnu) printf '%s\n' '-DCRABC_ACCESS_REQUIRE_GNU' ;;
            esac
            ;;
        gnu-hidden) printf '%s\n' '-DCRABC_ACCESS_REQUIRE_GNU_HIDDEN' ;;
...
check_trace() {
    ...
    trace_has_header "$trace" "$root" fcntl.h ||
        fail "$profile $tree trace omitted ${root}/fcntl.h"
    trace_has_header "$trace" "$root" unistd.h ||
        fail "$profile $tree trace omitted ${root}/unistd.h"
}
...
check_cxx_symbols() {
    ...
    local undefined
    local symbol
    local -a expected=(access faccessat)

    if profile_requires_gnu "$profile"; then
        expected+=(eaccess euidaccess)
    fi
...
expect_hidden_failure() {
    ...
    if compile_profile "$tree" "$profile" gnu-hidden "$diagnostic" "$object"; then
        fail "$tree $profile unexpectedly exposes GNU eaccess/euidaccess declarations"
    fi
    grep -Fq 'eaccess' "$diagnostic" ||
        fail "$tree $profile hidden GNU declaration diagnostic does not name eaccess"
}
...
[ -f "$C_PROBE" ] || fail "missing C access-header ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ access-header ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-access-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ direct access-header ABI matrix: PASS (%s profiles; compile-only)\n' \
