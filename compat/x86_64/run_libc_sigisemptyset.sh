#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc sigisemptyset evidence.
#
# The one-symbol GNU project-header body first executes against pinned musl
# 1.2.6, then as a true `-nostdlib -static` candidate. It proves musl's exact
# first-word predicate, ignored public sigset_t tail, stale errno, and no
# caller-storage writes. It does not select signal actions, masks, delivery,
...
fail() {
    printf 'ERROR: x86 static libc sigisemptyset: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-sigisemptyset.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-sigisemptyset-reference"
candidate="$work_dir/crabc-static-sigisemptyset-candidate"
...
archive_predicate_disassembly="$work_dir/archive-sigisemptyset-disassembly"
candidate_symbols="$work_dir/candidate-symbols"
candidate_program_headers="$work_dir/candidate-program-headers"
candidate_dynamic="$work_dir/candidate-dynamic"
candidate_relocations="$work_dir/candidate-relocations"
candidate_disassembly="$work_dir/candidate-disassembly"
predicate_disassembly="$work_dir/sigisemptyset-disassembly"
errno_disassembly="$work_dir/errno-disassembly"

cd "$ROOT_DIR"
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_sigisemptyset_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_sigisemptyset_probe.c -o "$reference"
"$reference"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap sigisemptyset; do
...
objdump -d --disassemble=sigisemptyset "$archive" >"$archive_predicate_disassembly"
...
if grep -Eq '[[:space:]](call|callq|syscall)([[:space:]]|$)' \
    "$archive_predicate_disassembly"; then
    fail "archive sigisemptyset must remain a direct no-call/no-syscall predicate"
fi

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_SIGISEMPTYSET_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_sigisemptyset_probe.c \
    compat/x86_64/libc_sigisemptyset_start.S "$archive" -o "$candidate"
...
for unrelated in sigaction signal sigemptyset sigfillset sigaddset sigdelset \
    sigismember sigandset sigorset sigprocmask sigpending sigsuspend sigpause \
...
if grep -Eqi 'arch_prctl|mov[[:space:]]+%rsi,[[:space:]]*%fs:0' \
    compat/x86_64/libc_sigisemptyset_start.S; then
    fail "fixture start must not install a private FS base"
fi
objdump -d --disassemble=sigisemptyset "$candidate" >"$predicate_disassembly"
if grep -Eq '[[:space:]](call|callq|syscall)([[:space:]]|$)' \
    "$predicate_disassembly"; then
    fail "candidate sigisemptyset must remain a direct no-call/no-syscall predicate"
fi

"$candidate"
printf 'x86 static crabc-libc sigisemptyset: PASS\n'
