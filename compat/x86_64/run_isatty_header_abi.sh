#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ isatty declaration gate.
#
...
fail() {
    printf 'ERROR: x86 unistd.h isatty ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/isatty_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/isatty_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-isatty-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-isatty-cxx.o"
candidate_cxx_object="$work_dir/candidate-isatty-cxx.o"

# Musl declares isatty independently of the ordinary strict/POSIX/XSI/GNU/BSD
# selector profiles. Keep each profile explicit rather than assuming a host
# default feature macro.
...
if ! "$ORACLE_CC" -std=c11 -fno-builtin -I "$ROOT_DIR/include" -H \
    ...
    fail "project C isatty header contract drifted"
fi
...
printf 'x86 pinned-musl/project C/C++ <unistd.h> isatty ABI: PASS\n'
