#!/usr/bin/env bash
# Native Linux/x86-64 permanent-stdin __fseterr evidence.
#
...
# It selects only the fixed stdin F_ERR transition observed through existing
# status entries; it is not general FILE, stream-I/O, or public-x86 evidence.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc permanent-stdin __fseterr: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_stdio_permanent_fseterr_stdin_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-stdio-permanent-fseterr-stdin.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-stdio-permanent-fseterr-stdin-reference"
candidate="$work_dir/crabc-static-stdio-permanent-fseterr-stdin-candidate"
...
fseterr_disassembly="$work_dir/fseterr-disassembly"
oracle_archive_symbols="$work_dir/oracle-archive-symbols"

cd "$ROOT_DIR"
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_stdio_permanent_fseterr_stdin_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -fno-builtin \
    ...
    compat/x86_64/libc_stdio_permanent_fseterr_stdin_probe.c -o "$reference"
timeout "$EXECUTION_TIMEOUT" "$reference" ||
    fail "pinned-musl permanent-stdin __fseterr fixture failed"

nm -A --defined-only "$ORACLE_ARCHIVE" >"$oracle_archive_symbols" 2>/dev/null
grep -Eq "[[:space:]]T[[:space:]]__fseterr$" "$oracle_archive_symbols" ||
    fail "pinned-musl archive omits strong __fseterr"
...
for symbol in __crabc_x86_static_tls_bootstrap ferror clearerr; do
    grep -Eq "[[:space:]][TW][[:space:]]${symbol}$" "$archive_symbols" ||
        fail "archive does not define ${symbol}"
done
grep -Eq "[[:space:]]T[[:space:]]__fseterr$" "$archive_symbols" ||
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L \
    -DCRABC_STDIO_PERMANENT_FSETERR_STDIN_FREESTANDING -I"$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_stdio_permanent_fseterr_stdin_probe.c \
    compat/x86_64/libc_stdio_permanent_fseterr_stdin_start.S "$archive" -o "$candidate"
readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --program-headers --wide "$candidate" >"$candidate_program_headers"
readelf --dynamic --wide "$candidate" >"$candidate_dynamic" || true
readelf --relocs --wide "$candidate" >"$candidate_relocations"
objdump -d "$candidate" >"$candidate_disassembly"
objdump -d --disassemble=__fseterr "$candidate" >"$fseterr_disassembly"
for symbol in __fseterr ferror clearerr stdin; do
...
if grep -Eq '[[:space:]]syscall$' "$fseterr_disassembly"; then
    fail "__fseterr unexpectedly contains a syscall path"
fi
...
timeout "$EXECUTION_TIMEOUT" "$candidate" ||
    fail "freestanding permanent-stdin __fseterr fixture failed"

printf 'x86 static crabc-libc permanent-stdin __fseterr: PASS\n'
