#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc sigrtmin evidence.
#
# The one-symbol project-header body first executes against pinned musl 1.2.6,
# then as a true `-nostdlib -static` candidate. It proves musl's exact fixed
# 35 result, the public SIGRTMIN value, stale errno, and no call or syscall in
# the selected leaf. The C++ POSIX/GNU feature matrix keeps its direct C
# spelling and macro expression unmangled. This does not select delivery,
# actions, masks, process signaling, waits, descriptors, timers, pthread
# policy, a general signal runtime, or public x86 support.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly STATIC_C_ABI_EXPORTS="$ROOT_DIR/compat/x86_64/static_c_abi_exports.txt"
readonly CXX_HEADER_PROBE="$ROOT_DIR/compat/x86_64/sigrtmin_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 static libc sigrtmin: %s\n' "$*" >&2
    exit 1
}
...
assert_cxx_header_contract() {
    ...
    # C++ POSIX/GNU feature matrix: both header trees retain an exact C
    # function type and an unmangled direct reference; the macro expression
    # stays valid in either header spelling.
    for profile in posix gnu; do
        ...
        oracle_object="$work_dir/oracle-sigrtmin-${profile}-cxx.o"
        project_object="$work_dir/project-sigrtmin-${profile}-cxx.o"
        cxx_header_trace="$work_dir/sigrtmin-${profile}-cxx-header-trace"

        "$ORACLE_CC" -std=c++17 -x c++ "${feature_args[@]}" \
            -DCRABC_EXPECT_SIGRTMIN -fno-builtin -fsyntax-only \
            "$CXX_HEADER_PROBE"
        "$ORACLE_CC" -std=c++17 -x c++ "${feature_args[@]}" \
            -DCRABC_EXPECT_SIGRTMIN -fno-builtin -I"$ROOT_DIR/include" \
        ...
        "$ORACLE_CC" -std=c++17 -x c++ "${feature_args[@]}" \
            -DCRABC_EXPECT_SIGRTMIN -fno-builtin -c "$CXX_HEADER_PROBE" \
            -o "$oracle_object"
        "$ORACLE_CC" -std=c++17 -x c++ "${feature_args[@]}" \
            -DCRABC_EXPECT_SIGRTMIN -fno-builtin -I"$ROOT_DIR/include" \
        ...
        for object in "$oracle_object" "$project_object"; do
            ...
            symbol=__libc_current_sigrtmin
...
[ -f "$CXX_HEADER_PROBE" ] || fail "missing C++ sigrtmin header probe"

bash "$ROOT_DIR/compat/x86_64/run_musl_oracle.sh" >/dev/null
bash "$ROOT_DIR/compat/x86_64/run_signal_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-sigrtmin.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-sigrtmin-reference"
candidate="$work_dir/crabc-static-sigrtmin-candidate"
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -U_GNU_SOURCE \
    -I"$ROOT_DIR/include" -E -H compat/x86_64/libc_sigrtmin_probe.c \
...
# Compile the reference with pinned musl's own headers and runtime, then the
# candidate with project-first headers and only its selected static archive.
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -U_GNU_SOURCE \
    -fno-builtin -fno-stack-protector \
    compat/x86_64/libc_sigrtmin_probe.c -o "$reference"
"$reference"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap __libc_current_sigrtmin; do
...
objdump -d --disassemble="__libc_current_sigrtmin" "$archive" \
    >"$work_dir/archive-sigrtmin-disassembly"
if grep -Eq '[[:space:]](call|callq|syscall)([[:space:]]|$)' \
    "$work_dir/archive-sigrtmin-disassembly"; then
    fail "archive __libc_current_sigrtmin must remain a direct no-call/no-syscall bridge"
fi

"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -U_GNU_SOURCE \
    -DCRABC_SIGRTMIN_FREESTANDING -I"$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_sigrtmin_probe.c \
    compat/x86_64/libc_sigrtmin_start.S "$archive" \
    -o "$candidate"
...
for unrelated in __libc_current_sigrtmax sigaction signal sigemptyset sigfillset \
    sigaddset sigdelset sigismember sigisemptyset sigandset sigorset sigprocmask \
    sigpending sigsuspend sigpause sigtimedwait sigwaitinfo sigwait kill killpg \
    raise sigqueue signalfd signalfd4 timerfd_create timerfd_settime timerfd_gettime \
...
if grep -Eqi 'arch_prctl|mov[[:space:]]+%rsi,[[:space:]]*%fs:0' \
    compat/x86_64/libc_sigrtmin_start.S; then
    fail "fixture start must not install a private FS base"
fi
objdump -d --disassemble="__libc_current_sigrtmin" "$candidate" \
    >"$work_dir/candidate-sigrtmin-disassembly"
if grep -Eq '[[:space:]](call|callq|syscall)([[:space:]]|$)' \
    "$work_dir/candidate-sigrtmin-disassembly"; then
    fail "candidate __libc_current_sigrtmin must remain a direct no-call/no-syscall bridge"
fi

"$candidate"
printf 'x86 static crabc-libc sigrtmin: PASS\n'
