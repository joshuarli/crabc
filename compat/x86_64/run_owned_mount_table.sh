...
if not temporary.is_dir() or temporary.resolve() != temporary or not temporary.is_relative_to(root / '.work'):
    raise SystemExit('owned mount-table TMPDIR must be a physical checkout .work directory')
if sys.argv[3]:
    product = Path(sys.argv[3])
    if not product.is_dir() or not product.is_relative_to(root / '.work'):
        raise SystemExit('owned mount-table product must be a checkout .work directory')
PY
readonly work="$(mktemp -d "$TMPDIR/owned-mount-table.XXXXXX")"
chmod a+rx "$work"
printf 'owned mount-table evidence: %s\n' "$work"
...
"$provided_dynamic/bin/crabc-cc-dynamic" --dynamic-pie -std=c11 -fno-builtin -c "$ROOT/compat/x86_64/owned_mount_table_probe.c" -o "$work/workload.o"
sha256sum "$ROOT/compat/x86_64/owned_mount_table_probe.c" "$work/workload.o" >"$work/input.sha256"
assert_symbols() {
    readelf --wide "$2" "$1" >"$3"
    python3 -B - "$3" <<'PY'
from pathlib import Path
import sys
names = {'setmntent', 'getmntent', 'getmntent_r', 'addmntent', 'endmntent', 'hasmntopt'}
seen = set()
