/* C++17 linkage probe for the sealed suppressed-count scanf artifact.
 *
 * used references let the runner prove that the project stdio header requests
 * unmangled C spellings for only the existing sscanf/vsscanf declarations.
 * This is not a general count, runtime, or stdio evidence claim.
 */
...
#if !defined(CRABC_STDIO_FIXED_SUPPRESSED_COUNT_SCAN_HEADER_CXX17)
#error "the C++17 suppressed-count scanf header profile must be selected"
#endif
...
int crabc_x86_64_stdio_fixed_suppressed_count_scan_header_abi_probe_cpp()
{
    return 0;
}
