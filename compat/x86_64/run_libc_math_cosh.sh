#!/usr/bin/env bash
# Pinned-musl differential for the private x86 scalar cosh/coshf artifact.
set -euo pipefail
...
readonly PRIVATE_PROVIDERS=(
	crabc_x86_math_cosh_provider_expm1
	crabc_x86_math_cosh_provider_expm1f
	crabc_x86_math_cosh_provider_expo2
	crabc_x86_math_cosh_provider_expo2f
	crabc_x86_math_cosh_provider_exp
	crabc_x86_math_cosh_provider_expf
	crabc_x86_math_cosh_data_exp
	crabc_x86_math_cosh_data_exp2f
	crabc_x86_math_cosh_helper_oflow
	crabc_x86_math_cosh_helper_oflowf
	crabc_x86_math_cosh_helper_uflow
	crabc_x86_math_cosh_helper_uflowf
	crabc_x86_math_cosh_helper_xflow
	crabc_x86_math_cosh_helper_xflowf
)

fail() { printf 'ERROR: x86 static libc cosh/coshf: %s\n' "$*" >&2; exit 1; }
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_cosh_probe.c >/dev/null 2>"$header_trace"
for header in fenv.h float.h math.h stddef.h stdint.h features.h bits/alltypes.h; do
	grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
		fail "fixture did not use project $header"
done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
	compat/x86_64/libc_math_cosh_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl cosh/coshf fixture failed"
...
for mode in sse x87; do
	...
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -c compat/x86_64/math_cosh_header_abi_probe.cpp \
		-o "$header_cxx_reference"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" \
		-c compat/x86_64/math_cosh_header_abi_probe.cpp -o "$header_cxx_candidate"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only \
		compat/x86_64/math_cosh_header_abi_probe.cpp >/dev/null 2>"$cxx_header_trace"
	...
	for object in "$header_cxx_reference" "$header_cxx_candidate"; do
		...
		if grep -Eq '_Z.*cosh' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled hyperbolic-cosine reference"
		fi
	done
done
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_COSH_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_cosh_probe.c \
	compat/x86_64/libc_math_cosh_start.S "$archive" -o "$candidate"
...
for unselected in coshl sinh sinhf sinhl tanh tanhf tanhl exp expf expl exp2 exp2f \
	...
	acosh acoshf acoshl asinh asinhf asinhl atanh atanhf atanhl pow powf powl \
...
"$candidate" >"$candidate_output" || fail "freestanding cosh/coshf fixture failed"
