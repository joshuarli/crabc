#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 GNU ``exp10``/``pow10`` leaf.

This provenance tool is outside the Rust build. The checked assembly is the
build input, so the selected static archive never invokes a foreign C compiler.
"""
...
# Musl exposes binary64 exp10 and GNU's weak same-address pow10 alias from one
# source. Its small-integral table path delegates to modf/exp2, while its large
# path delegates to pow; every provider and its data/helper closure remains
# local so this artifact does not select public siblings.
PUBLIC_SOURCES = ("src/math/exp10.c",)
PRIVATE_SOURCES = (
    "src/math/modf.c",
    "src/math/exp2.c",
    "src/math/pow.c",
...
PRIVATE_RENAMES = {
    "modf": "crabc_x86_math_exp10_provider_modf",
    "exp2": "crabc_x86_math_exp10_provider_exp2",
    "pow": "crabc_x86_math_exp10_provider_pow",
    "__exp_data": "crabc_x86_math_exp10_data_exp",
    "__pow_log_data": "crabc_x86_math_exp10_data_pow_log",
    "__math_invalid": "crabc_x86_math_exp10_helper_invalid",
    "__math_oflow": "crabc_x86_math_exp10_helper_oflow",
    "__math_uflow": "crabc_x86_math_exp10_helper_uflow",
    "__math_xflow": "crabc_x86_math_exp10_helper_xflow",
    "fabs": "crabc_x86_math_exp10_provider_fabs",
}

# Preserve musl's generic scalar and all-rounding-mode arithmetic. These flags
# rule out an implicit long-double, FMA, AVX, or packed-vector rewrite.
COMPILE_FLAGS = (
    "-D_XOPEN_SOURCE=700",
    "-std=c99",
    "-ffreestanding",
...
def transform_assembly(relative: str, text: str) -> str:
    """Namespace TU-local labels and keep only the selected two aliases public."""
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_exp10_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_exp10_{tag}_\1",
        text,
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_exp10_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 GNU exp10/pow10 translation.",
        " * Generated by compat/x86_64/generate_libc_math_exp10.py; see",
        " * math_exp10.rs for the source/license/ABI contract.",
        " *",
        " * Arm and normal musl-authored portions retain their notices below;",
        " * musl's MIT license is recorded by compat/upstreams.toml.",
        " */",
    ]
    blocks.extend(retained_notices(source_root))
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-exp10-") as temporary:
