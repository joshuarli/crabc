/* Selected Linux/x86-64 mkfifoat C header ABI facts.
 *
...
typedef int (*crabc_mkfifoat_signature)(int, const char *, mode_t);
...
_Static_assert(__builtin_types_compatible_p(__typeof__(&mkfifoat),
                                             crabc_mkfifoat_signature),
               "mkfifoat declaration");

int crabc_x86_64_mkfifoat_header_abi_probe(void)
{
    return S_IFIFO;
}
