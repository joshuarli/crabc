#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc credential-observation evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc credential observation: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-credential-observation.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"; archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-credential-observation-reference"; candidate="$work_dir/crabc-static-credential-observation-candidate"
...
cd "$ROOT_DIR"
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H compat/x86_64/libc_credential_observation_probe.c >/dev/null 2>"$trace"
for header in errno.h stdint.h sys/types.h unistd.h sys/syscall.h features.h; do grep -Fq "$ROOT_DIR/include/$header" "$trace" || fail "fixture did not use project $header"; done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector -I"$ROOT_DIR/include" compat/x86_64/libc_credential_observation_probe.c -o "$reference"
"$reference" || fail "pinned-musl credential-observation fixture failed"
...
for unselected in getlogin getlogin_r getusershell setusershell endusershell getgrouplist initgroups getgrgid getgrnam setfsuid setfsgid fork _Fork vfork execve syscall; do if grep -Eq "[[:space:]][TW][[:space:]]${unselected}$" "$archive_symbols"; then fail "archive accidentally exports unselected $unselected"; fi; done
...
for symbol in getgroups getresuid getresgid; do
    symbol_disassembly="$work_dir/${symbol}-disassembly"
    objdump -d --disassemble="$symbol" "$candidate" >"$symbol_disassembly"
    grep -Eq '[[:space:]]syscall[[:space:]]*$' "$symbol_disassembly" \
        || fail "candidate ${symbol} lacks a direct syscall instruction"
done
if grep -Eq 'crabc_core|mimalloc|sha_crypt' "$symbols" "$disassembly"; then fail "candidate selects unowned runtime symbols"; fi
"$candidate" || fail "freestanding credential-observation fixture failed"
printf 'x86 static libc credential observation: PASS\n'
