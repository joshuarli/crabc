...
for unselected in getusershell setusershell endusershell getgrouplist initgroups getgrgid getgrnam setfsuid setfsgid _Fork vfork execve syscall; do if grep -Eq "[[:space:]][TW][[:space:]]${unselected}$" "$archive_symbols"; then fail "archive accidentally exports unselected $unselected"; fi; done
