#!/usr/bin/env bash
# Native Linux/x86-64 permanent-line-I/O <stdio.h> declaration/linkage proof.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_line_io_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_line_io_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 stdio permanent line-I/O header ABI: %s\n' "$*" >&2
    exit 1
}
...
compile_c() {
    ...
    run_compiler "$compiler" -x c -std=c11 -nostdinc -I "$include_root" \
        ...
        -DCRABC_STDIO_PERMANENT_LINE_IO_C11 -fsyntax-only "$C_PROBE" \
        >/dev/null 2>"$trace"
}
...
compile_cxx() {
    ...
    run_compiler "$compiler" -x c++ -std=c++17 -nostdinc -nostdinc++ \
        ...
        -H -fno-builtin -DCRABC_STDIO_PERMANENT_LINE_IO_CXX17 \
...
assert_cxx_c_linkage() {
    ...
    if grep -Eq '_Z.*(fgets|fputs|puts)' <<<"$undefined"; then
        fail "$tree C++ probe retained a mangled permanent-line-I/O reference"
    fi
}
...
[ -f "$C_PROBE" ] || fail "missing C permanent-line-I/O header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ permanent-line-I/O header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-stdio-permanent-line-io-header.XXXXXX)"
...
printf 'x86 stdio permanent line-I/O header ABI: PASS\n'
