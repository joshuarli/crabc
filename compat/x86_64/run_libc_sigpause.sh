#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc sigpause evidence.
#
# The one-symbol project-header C body first executes against pinned musl
# 1.2.6, then as a true `-nostdlib -static` candidate. Two runner-owned FIFOs
...
readonly EXECUTION_TIMEOUT=10

fail() {
    printf 'ERROR: x86 static libc sigpause: %s\n' "$*" >&2
    exit 1
}
...
run_interrupted_wait() {
    ...
    local ready_fifo="$work_dir/${label// /-}-ready"
    local release_fifo="$work_dir/${label// /-}-release"
    local ready_byte program_pid watchdog_pid status

    mkfifo "$ready_fifo" "$release_fifo"
    # Keep both FIFO directions open in the runner so setup cannot block on an
    # open race. The fixture only writes fd 3 then reads fd 4.
    exec 8<>"$ready_fifo"
    exec 9<>"$release_fifo"
    "$program" 3>&8 4<&9 &
    program_pid=$!

    if ! IFS= read -r -n 1 -t "$EXECUTION_TIMEOUT" -u 8 ready_byte; then
        kill -KILL "$program_pid" 2>/dev/null || true
        wait "$program_pid" 2>/dev/null || true
        exec 8>&-
        exec 9>&-
        fail "${label} did not reach its blocked-mask readiness point"
    fi
    if [ "$ready_byte" != R ]; then
        ...
        exec 8>&-
        exec 9>&-
        fail "${label} reported an invalid readiness byte"
    fi

    # SIGUSR1 is blocked by the fixture at this point; queue it before the
    # selected call and then release the fixture to enter sigpause atomically.
    kill -USR1 "$program_pid" || {
        kill -KILL "$program_pid" 2>/dev/null || true
        wait "$program_pid" 2>/dev/null || true
        exec 8>&-
        exec 9>&-
        fail "could not queue SIGUSR1 for ${label}"
    }
    printf G >&9
    (
...
require_native_linux_x86_64
for tool in ar awk cargo cmp diff grep mkdir mkfifo nm objdump readelf rustup sleep sort; do
    require_tool "$tool"
done
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-sigpause.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-sigpause-reference"
candidate="$work_dir/crabc-static-sigpause-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_sigpause_probe.c >/dev/null 2>"$header_trace"
for header in errno.h signal.h stddef.h stdint.h sys/syscall.h bits/alltypes.h bits/signal.h bits/syscall.h; do
    grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
        fail "fixture omitted project $header"
done

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_sigpause_probe.c -o "$reference"
run_interrupted_wait "$reference" "pinned-musl-reference"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap sigpause; do
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap sigpause \
    crabc_x86_64_sigpause_restorer; do
    grep -Eq "[[:space:]]${symbol}$" "$candidate_symbols" ||
        fail "candidate does not define ${symbol}"
done
for unrelated in sigaction signal sigemptyset sigfillset sigaddset sigdelset \
    sigismember sigprocmask sigpending sigsuspend sigtimedwait sigwaitinfo sigwait \
    kill killpg raise sigqueue signalfd signalfd4 timerfd_create timerfd_settime \
    timerfd_gettime epoll_create epoll_wait eventfd inotify_init pthread_create \
...
grep -Fq 'call __crabc_x86_static_tls_bootstrap' compat/x86_64/libc_sigpause_start.S ||
    fail "fixture start does not delegate first-thread TLS to libc"
if grep -Eqi 'arch_prctl|mov[[:space:]]+%rsi,[[:space:]]*%fs:0' \
    compat/x86_64/libc_sigpause_start.S; then
