#!/usr/bin/env bash
# Native Linux/x86-64 XSI unlockpt C/C++ declaration gate.
#
# Pinned musl 1.2.6 is the declaration and C-linkage oracle. `unlockpt` is
# visible under X/Open, GNU, and BSD profiles but hidden by strict/POSIX
# profiles. This compile-only boundary selects no PTY opening, naming, grant,
# session, terminal, or generic-ioctl policy.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 stdlib.h unlockpt ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/unlockpt_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/unlockpt_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-unlockpt-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-unlockpt-cxx.o"
candidate_cxx_object="$work_dir/candidate-unlockpt-cxx.o"
...
compile_visible_profile() {
    ...
    for variant in oracle project; do
        ...
        "$ORACLE_CC" -std=c11 -U_GNU_SOURCE "$selector" \
            -DCRABC_EXPECT_UNLOCKPT -fno-builtin -fsyntax-only \
...
if ! "$ORACLE_CC" -std=c11 -U_GNU_SOURCE -D_XOPEN_SOURCE=700 \
    -DCRABC_EXPECT_UNLOCKPT -fno-builtin -I "$ROOT_DIR/include" -H \
    -fsyntax-only "$c_probe" >/dev/null 2>"$header_trace"; then
    sed -n '1,160p' "$header_trace" >&2
    fail "project C unlockpt header contract drifted"
fi
...
"$ORACLE_CC" -std=c++17 -x c++ -U_GNU_SOURCE -D_XOPEN_SOURCE=700 \
    -DCRABC_EXPECT_UNLOCKPT -fno-builtin -c "$cxx_probe" \
    -o "$oracle_cxx_object"
"$ORACLE_CC" -std=c++17 -x c++ -U_GNU_SOURCE -D_XOPEN_SOURCE=700 \
    -DCRABC_EXPECT_UNLOCKPT -fno-builtin -I "$ROOT_DIR/include" \
...
for language in c cxx; do
    ...
    for selector in '' -D_POSIX_SOURCE -D_POSIX_C_SOURCE=200809L; do
        if "$ORACLE_CC" "${compiler_args[@]}" -U_GNU_SOURCE "$selector" \
            -DCRABC_REQUIRE_UNLOCKPT_HIDDEN -fno-builtin -fsyntax-only "$probe" \
            >/dev/null 2>"$work_dir/oracle-${language}-hidden-errors"; then
            fail "pinned musl exposes unlockpt outside X/Open/GNU/BSD ${language}"
        fi
        if "$ORACLE_CC" "${compiler_args[@]}" -U_GNU_SOURCE "$selector" \
            -DCRABC_REQUIRE_UNLOCKPT_HIDDEN -fno-builtin -I "$ROOT_DIR/include" \
            -fsyntax-only "$probe" \
            >/dev/null 2>"$work_dir/project-${language}-hidden-errors"; then
            fail "project stdlib.h exposes unlockpt outside X/Open/GNU/BSD ${language}"
        fi
    done
done

printf 'x86 pinned-musl/project C/C++ <stdlib.h> unlockpt ABI: PASS\n'
