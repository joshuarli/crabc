#!/usr/bin/env bash
# Native Linux/x86-64 selected static ceil/ceilf evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc math ceil: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-ceil.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-math-ceil-reference"
candidate="$work_dir/crabc-static-math-ceil-candidate"
reference_records="$work_dir/musl-records"
candidate_records="$work_dir/candidate-records"
header_cxx_reference="$work_dir/musl-math-ceil-header.o"
header_cxx_candidate="$work_dir/project-math-ceil-header.o"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_ceil_probe.c >/dev/null 2>"$header_trace"
for header in fenv.h float.h math.h stdint.h features.h bits/alltypes.h; do
	grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
		fail "fixture did not use project $header"
done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
	compat/x86_64/libc_math_ceil_probe.c -o "$reference"
"$reference" >"$reference_records" || fail "pinned-musl math ceil fixture failed"
[ -s "$reference_records" ] || fail "pinned-musl math ceil emitted no differential records"
...
for mode in sse x87; do
	...
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -c compat/x86_64/math_ceil_header_abi_probe.cpp \
		-o "$header_cxx_reference"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" \
		-c compat/x86_64/math_ceil_header_abi_probe.cpp -o "$header_cxx_candidate"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only \
		compat/x86_64/math_ceil_header_abi_probe.cpp >/dev/null 2>"$cxx_header_trace"
	...
	for object in "$header_cxx_reference" "$header_cxx_candidate"; do
		...
		for symbol in ceil ceilf; do
			grep -Eq "[[:space:]]${symbol}$" <<<"$undefined" ||
				fail "C++ ${mode} probe does not retain unmangled ${symbol}"
		done
		if grep -Eq '_Z.*ceil' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled ceil reference"
		fi
	done
done
...
for symbol in ceil ceilf feclearexcept fegetenv fegetround fesetenv fesetround fetestexcept; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_CEIL_FREESTANDING \
	...
	compat/x86_64/libc_math_ceil_probe.c compat/x86_64/libc_math_ceil_start.S \
	"$archive" -o "$candidate"
...
for unselected in ceill floor floorf floorl fmod fmodf fmodl modf modff modfl \
	...
	round roundf roundl sqrt sqrtf sqrtl cbrt cbrtf cbrtl \
...
if grep -Eq 'crabc_core|mimalloc|sha_crypt|libm|math_special|math_complex|math_elementary_long_double|math_fmod|math_cbrt' \
	"$candidate_symbols" "$disassembly"; then
	fail "candidate selects an unowned math/runtime dependency"
fi
for instruction in addsd subsd addss; do
	grep -Eq "[[:space:]]${instruction}([[:space:]]|$)" "$disassembly" ||
		fail "candidate lacks ${instruction} musl fixed-direction arithmetic"
done
"$candidate" >"$candidate_records" || fail "freestanding math ceil fixture failed"
if ! cmp -s "$reference_records" "$candidate_records"; then
	cmp -l "$reference_records" "$candidate_records" | head -n 16 >&2 || true
	fail "pinned-musl and freestanding ceil differential records differ"
fi

printf 'x86 static libc math ceil: PASS\n'
