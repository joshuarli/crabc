#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``exp2``/``exp2f`` leaf.
...
# `exp2.c` shares its binary64 table with musl's exp/pow family, while
# `exp2f.c` has a distinct binary32 table. This leaf compiles both data sources
# and all range-scaling helpers, renames them, and makes them local.
PUBLIC_SOURCES = (
    "src/math/exp2.c",
    "src/math/exp2f.c",
)
...
PRIVATE_RENAMES = {
    "__exp_data": "crabc_x86_math_exp2_data",
    "__exp2f_data": "crabc_x86_math_exp2_data_f",
    "__math_oflow": "crabc_x86_math_exp2_provider_oflow",
    "__math_uflow": "crabc_x86_math_exp2_provider_uflow",
    "__math_xflow": "crabc_x86_math_exp2_provider_xflow",
    "__math_oflowf": "crabc_x86_math_exp2_provider_oflowf",
    "__math_uflowf": "crabc_x86_math_exp2_provider_uflowf",
    "__math_xflowf": "crabc_x86_math_exp2_provider_xflowf",
}

# Preserve musl's all-rounding-mode code paths and fixed scalar arithmetic.
COMPILE_FLAGS = (
    "-D_XOPEN_SOURCE=700",
    "-std=c99",
    "-ffreestanding",
    ...
    "-fno-builtin-exp2",
    "-fno-builtin-exp2f",
    "-fno-builtin-__math_oflow",
    "-fno-builtin-__math_uflow",
    "-fno-builtin-__math_xflow",
    "-fno-builtin-__math_oflowf",
    "-fno-builtin-__math_uflowf",
    "-fno-builtin-__math_xflowf",
    "-fno-tree-vectorize",
...
def transform_assembly(relative: str, text: str) -> str:
    """Namespace TU-local labels and retain private providers as local."""
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_exp2_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_exp2_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_exp2_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def retained_notices(source_root: Path) -> list[str]:
    ...
    for relative in NOTICE_SOURCES:
        for comment in re.findall(r"/\*.*?\*/", (source_root / relative).read_text(), re.DOTALL):
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "libc/src/c_abi/x86_64/math_exp2_musl_x86_64.S")
    ...
    if actual_digest != EXPECTED_MUSL_TREE_DIGEST:
        raise SystemExit(f"musl source tree digest mismatch: expected {EXPECTED_MUSL_TREE_DIGEST}, got {actual_digest}")
    compiler_version = subprocess.run([args.cc, "--version"], check=True, capture_output=True, text=True).stdout.splitlines()[0]
    if "15.2.0" not in compiler_version:
        raise SystemExit(f"expected pinned GCC 15.2.0, got: {compiler_version}")

    include_flags = (
        f"-I{source_root / 'src/internal'}", f"-I{source_root / 'src/include'}",
        f"-I{source_root / 'arch/x86_64'}", f"-I{source_root / 'arch/generic'}",
        f"-I{source_root / 'include'}",
    )
    blocks = [
        "/*", " * Fixed musl 1.2.6 generic x86-64 exp2/exp2f translation.",
        " * Generated by compat/x86_64/generate_libc_math_exp2.py; see",
        " * math_exp2.rs for the source/license/ABI contract.", " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-exp2-") as temporary:
        temporary_root = Path(temporary)
        for relative in (*PUBLIC_SOURCES, *PRIVATE_SOURCES):
            output = temporary_root / (Path(relative).stem + ".S")
            subprocess.run(
                [args.cc, *COMPILE_FLAGS, *include_flags, "-S", str(source_root / relative), "-o", str(output)],
                check=True,
            )
            blocks.extend((f"/* musl-1.2.6/{relative} */", transform_assembly(relative, output.read_text())))
