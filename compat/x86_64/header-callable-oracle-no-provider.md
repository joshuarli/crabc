...
separate symbol roster into the runner. Every member must still be both a
checked candidate external declaration and a pinned-reference external
function in that compiler inventory.
...
dump. Every selected defined POSIX `nm` row, including GNU IFUNC or UNIQUE
extensions and aliases, fails. An inspection command failure also fails; empty
output alone is never treated as an absence.
...
unresolved-name diagnostic. The accepted failure shape is the pinned GCC/ld
context and intended `undefined reference` lines followed by its `collect2`
terminal line; another fatal linker line fails the audit.
