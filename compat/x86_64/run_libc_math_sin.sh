#!/usr/bin/env bash
# Pinned-musl differential for the private x86 scalar sin/sinf artifact.
set -euo pipefail
...
readonly SELECTED_SYMBOLS=(sin sinf)
readonly FENV_SIBLINGS=(feclearexcept fegetenv fegetround fesetenv fesetround fetestexcept)
readonly PRIVATE_PROVIDERS=(
	crabc_x86_math_sin_kernel_sin
	crabc_x86_math_sin_kernel_cos
	crabc_x86_math_sin_kernel_sindf
	crabc_x86_math_sin_kernel_cosdf
	crabc_x86_math_sin_reduce_pio2
	crabc_x86_math_sin_reduce_pio2f
	crabc_x86_math_sin_reduce_pio2_large
	crabc_x86_math_sin_provider_floor
	crabc_x86_math_sin_provider_scalbn
)

fail() { printf 'ERROR: x86 static libc sin/sinf: %s\n' "$*" >&2; exit 1; }
...
bash "$ROOT_DIR/compat/x86_64/run_math_sin_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-sin.XXXXXX)"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_sin_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -fno-builtin \
	-fno-stack-protector compat/x86_64/libc_math_sin_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl sin/sinf fixture failed"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_SIN_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_sin_probe.c \
	compat/x86_64/libc_math_sin_start.S "$archive" -o "$candidate"
...
for unselected in sinl sincos sincosf sincosl cos cosf cosl tan tanf tanl \
...
"$candidate" >"$candidate_output" || fail "freestanding sin/sinf fixture failed"
if ! cmp -s "$reference_output" "$candidate_output"; then
	cmp -l "$reference_output" "$candidate_output" | sed -n '1,120p' >&2 || true
	fail "candidate sin/sinf record stream differs from pinned musl"
fi

printf 'x86 static libc sin/sinf: PASS (%s records)\n' "$record_count"
