/* Linux/x86-64 sched_setparam declaration, record, and ABI probe. */
...
typedef int (*sched_setparam_signature)(pid_t, const struct sched_param *);

_Static_assert(__builtin_types_compatible_p(__typeof__(&sched_setparam),
    sched_setparam_signature), "sched_setparam declaration");
...
static sched_setparam_signature sched_setparam_function = sched_setparam;

int crabc_x86_64_sched_setparam_header_abi_probe(void)
{
    const struct sched_param parameter = { 0 };

    return sched_setparam_function(0, &parameter) == -1 ? 0 : 1;
}
