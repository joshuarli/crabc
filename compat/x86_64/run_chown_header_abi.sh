#!/usr/bin/env bash
# Native Linux/x86-64 selected chown C/C++ header ABI matrix.
#
# Pinned musl 1.2.6 is the declaration, uid_t/gid_t layout, and unmangled C++
...
# proves one following pathname ownership leaf, not sibling ownership APIs,
# credential policy, pathname policy, or public x86 support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/chown_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/chown_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 chown headers: %s\n' "$*" >&2
    exit 1
}
...
check_cxx_symbol() {
    ...
    grep -Eq '[[:space:]]chown$' <<<"$undefined" ||
        fail "$tree $profile C++ probe does not retain C linkage for chown"
    if grep -Eq '_Z.*chown' <<<"$undefined"; then
        fail "$tree $profile C++ probe retained a mangled chown reference"
    fi
}
...
[ -f "$C_PROBE" ] || fail "missing C chown ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ chown ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-chown-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ chown ABI matrix: PASS (%s profiles; visible=%s hidden=%s; compile-only)\n' \
