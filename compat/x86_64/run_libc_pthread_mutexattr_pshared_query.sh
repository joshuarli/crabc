#!/usr/bin/env bash
# Native Linux/x86-64 bounded static pthread mutex-attribute pshared-query evidence.
#
...
# archive. It proves exactly pthread_mutexattr_getpshared's raw four-byte
# record projection: only bit 7 is observed and the caller-owned word is
# unchanged. It does not select the setter, attribute lifecycle, mutex
# initialization or operation, threads, TLS, synchronization, cancellation,
...
fail() {
    printf 'ERROR: x86 static pthread mutexattr pshared query: %s\n' "$*" >&2
    exit 1
}
...
assert_no_unselected_mutexattr_exports() {
    ...
    for unselected in pthread_mutexattr_setpshared \
        ...
        [ "$symbols_path" = "$candidate_symbols" ] ||
            fail "mutexattr pshared-query sibling exclusions apply only to the final candidate"
...
assert_direct_record_path() {
    local disassembly="$work_dir/pthread_mutexattr_getpshared-disassembly"

    objdump -d --disassemble=pthread_mutexattr_getpshared "$candidate" >"$disassembly"
    if grep -Eq '[[:space:]](syscall|call)([[:space:]]|$)|%fs:' "$disassembly"; then
        fail "pthread_mutexattr_getpshared must remain a direct TLS-free, syscall-free record query"
    fi
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pthread-mutexattr-pshared-query.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-pthread-mutexattr-pshared-query-reference"
candidate="$work_dir/crabc-static-pthread-mutexattr-pshared-query-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_mutexattr_pshared_query_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -pthread -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_pthread_mutexattr_pshared_query_probe.c \
    -o "$reference"
if timeout "$EXECUTION_TIMEOUT" "$reference"; then
    :
else
    status=$?
    fail "pinned-musl mutexattr pshared-query fixture exited ${status}"
fi
...
grep -Eq '[[:space:]][TW][[:space:]]pthread_mutexattr_getpshared$' "$archive_symbols" ||
    fail "archive does not define pthread_mutexattr_getpshared"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    -DCRABC_PTHREAD_MUTEXATTR_PSHARED_QUERY_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_pthread_mutexattr_pshared_query_probe.c \
    compat/x86_64/libc_pthread_mutexattr_pshared_query_start.S "$archive" -o "$candidate"
...
grep -Eq '[[:space:]]pthread_mutexattr_getpshared$' "$candidate_symbols" ||
    fail "candidate does not define pthread_mutexattr_getpshared"
...
for unselected in __errno_location __crabc_x86_static_tls_bootstrap \
    ...
    pthread_mutexattr_setpshared; do
...
if timeout "$EXECUTION_TIMEOUT" "$candidate"; then
    ...
    status=$?
    fail "freestanding mutexattr pshared-query fixture exited ${status}"
fi

printf 'x86 static crabc-libc pthread mutexattr pshared query: PASS\n'
