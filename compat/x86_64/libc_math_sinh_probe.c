/*
 * Static Linux/x86-64 sinh/sinhf C ABI differential regression.
 *
 ...
 * hyperbolic sine: sinhl, other hyperbolic surface, public exp/expm1 ABI,
...
#include <stddef.h>
#include <stdint.h>
#ifndef CRABC_MATH_SINH_FREESTANDING
#include <unistd.h>
#endif
...
#define SINH_RECORD_STORAGE_WORDS (SINH_RECORD_COUNT * SINH_RECORD_WORDS)
...
static int record_binary64(size_t *cursor, int rounding_mode, uint64_t input)
{
	...
	if (fesetround(rounding_mode) != 0 || feclearexcept(FE_ALL_EXCEPT) != 0)
		return 1;
	result = direct_sinh(double_from_bits(input));
	if (*cursor + SINH_RECORD_WORDS > SINH_RECORD_STORAGE_WORDS)
		return 2;
	crabc_x86_64_math_sinh_records[(*cursor)++] = input;
	crabc_x86_64_math_sinh_records[(*cursor)++] = double_bits(result);
	crabc_x86_64_math_sinh_records[(*cursor)++] =
...
static int record_binary32(size_t *cursor, int rounding_mode, uint32_t input)
{
	...
	if (fesetround(rounding_mode) != 0 || feclearexcept(FE_ALL_EXCEPT) != 0)
		return 1;
	result = direct_sinhf(float_from_bits(input));
	if (*cursor + SINH_RECORD_WORDS > SINH_RECORD_STORAGE_WORDS)
		return 2;
	crabc_x86_64_math_sinh_records[(*cursor)++] =
		UINT64_C(0x0000000100000000) | input;
	crabc_x86_64_math_sinh_records[(*cursor)++] = float_bits(result);
	crabc_x86_64_math_sinh_records[(*cursor)++] =
		((uint64_t)(uint32_t)rounding_mode << 32) |
		(uint32_t)fegetround();
	crabc_x86_64_math_sinh_records[(*cursor)++] =
...
int crabc_x86_64_math_sinh_probe(void)
{
	fenv_t original;
	size_t cursor = 0;
	size_t input_index;
...
int main(void)
{
	int status = crabc_x86_64_math_sinh_probe();

	if (status != 0)
		return status;
	return write_all(crabc_x86_64_math_sinh_records,
