#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc clock_settime error-ABI evidence.
#
...
# only invalid/rejected clock IDs and never asks Linux to alter CLOCK_REALTIME.
# This is an error-translation ABI artifact, not clock mutation or time support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc clock_settime: %s\n' "$*" >&2
    exit 1
}
...
assert_clock_settime_syscall() {
    local disassembly="$work_dir/clock-settime-disassembly"

    objdump -d --disassemble=clock_settime "$candidate" >"$disassembly"
    grep -Eq '%[re]di' "$disassembly" ||
        fail "clock_settime lacks its clock-ID argument register"
    grep -Eq '\$0xe3(,|[[:space:]]|$)' "$disassembly" ||
        fail "clock_settime lacks syscall 227"
    grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$disassembly" ||
        fail "clock_settime lacks the Linux syscall instruction"
    grep -Eq '%fs:|__errno_location' "$disassembly" ||
        fail "clock_settime must publish raw failure through errno TLS"
}
...
bash "$ROOT_DIR/compat/x86_64/run_clock_settime_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-clock-settime.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-clock-settime-reference"
candidate="$work_dir/crabc-static-clock-settime-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I "$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_clock_settime_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I "$ROOT_DIR/include" compat/x86_64/libc_clock_settime_probe.c \
    -o "$reference"
"$reference" || fail "pinned-musl clock_settime fixture failed"
...
for selected in __errno_location clock_settime; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_CLOCK_SETTIME_FREESTANDING \
    -I "$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie \
    -ffreestanding -fno-builtin -fno-stack-protector -Wl,-e,_start \
    -Wl,--no-undefined compat/x86_64/libc_clock_settime_probe.c \
    compat/x86_64/libc_clock_settime_start.S "$archive" -o "$candidate"
...
for unselected in clock_gettime clock_getres clock_nanosleep nanosleep \
    timer_create timer_delete timer_gettime timer_settime getitimer setitimer \
    sched_getcpu sched_getscheduler sched_yield sched_getparam \
    sched_setparam sched_setscheduler sched_rr_get_interval time timegm \
    gmtime_r localtime_r; do
...
assert_clock_settime_syscall
env -i "$candidate" || fail "freestanding clock_settime fixture failed"
printf 'x86 static crabc-libc clock_settime: PASS\n'
