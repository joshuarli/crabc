#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc timer_settime error ABI.
#
...
# only fresh-process invalid nonnegative opaque timer values with a valid
# request and initialized old-value storage, never a valid POSIX timer. This
# is a rejected-handle/error/output-preservation ABI artifact, not timer
# control or lifecycle support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc timer_settime: %s\n' "$*" >&2
    exit 1
}
...
assert_timer_settime_syscall() {
    local disassembly="$work_dir/timer-settime-disassembly"

    objdump -d --disassemble=timer_settime "$candidate" >"$disassembly"
    # System V AMD64 places the first three C arguments in rdi/rsi/rdx and
    # the fourth in rcx; the raw Linux syscall branch must move old_value to
    # r10 for timer_settime=223 before translating an error through errno.
    grep -Eq '\$0xdf(,|[[:space:]]|$)' "$disassembly" ||
        fail "timer_settime lacks syscall 223"
    grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$disassembly" ||
        fail "timer_settime lacks the Linux syscall instruction"
    grep -Fq '%r10' "$disassembly" ||
        fail "timer_settime lacks fourth-argument r10 placement"
    grep -Eq '%fs:|__errno_location' "$disassembly" ||
        fail "timer_settime must publish raw failure through errno TLS"
}
...
bash "$ROOT_DIR/compat/x86_64/run_timer_settime_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-timer-settime.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-timer-settime-reference"
candidate="$work_dir/crabc-static-timer-settime-candidate"
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -I "$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_timer_settime_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -fno-builtin \
    ...
    compat/x86_64/libc_timer_settime_probe.c -o "$reference"
"$reference" || fail "pinned-musl timer_settime fixture failed"
...
for selected in __errno_location timer_settime; do
    grep -Eq "[[:space:]][TW][[:space:]]${selected}$" "$archive_symbols" ||
        fail "archive does not define ${selected}"
done
for marker in 'src/time/timer_settime.c' 'SYS_TIMER_SETTIME' \
    'raw_syscall::syscall4' 'c_status(result)'; do
    grep -Fq "$marker" libc/src/c_abi/x86_64/timer_settime.rs ||
        fail "timer_settime source lacks ${marker}"
done
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L \
    -DCRABC_TIMER_SETTIME_FREESTANDING -I "$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_timer_settime_probe.c \
    compat/x86_64/libc_timer_settime_start.S "$archive" -o "$candidate"
...
for unselected in timer_create timer_delete timer_getoverrun timer_gettime \
...
assert_timer_settime_syscall
env -i "$candidate" || fail "freestanding timer_settime fixture failed"
printf 'x86 static crabc-libc timer_settime: PASS\n'
