#!/usr/bin/env bash
# Pinned-musl differential for the private x86 scalar tanh/tanhf artifact.
set -euo pipefail
...
readonly SELECTED_SYMBOLS=(tanh tanhf)
readonly FENV_SIBLINGS=(feclearexcept fegetenv fegetround fesetenv fesetround fetestexcept)
readonly PRIVATE_PROVIDERS=(
	crabc_x86_math_tanh_provider_expm1
	crabc_x86_math_tanh_provider_expm1f
)

fail() { printf 'ERROR: x86 static libc tanh/tanhf: %s\n' "$*" >&2; exit 1; }
...
bash "$ROOT_DIR/compat/x86_64/run_math_tanh_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-tanh.XXXXXX)"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_tanh_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -fno-builtin \
	-fno-stack-protector compat/x86_64/libc_math_tanh_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl tanh/tanhf fixture failed"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_TANH_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_tanh_probe.c \
	compat/x86_64/libc_math_tanh_start.S "$archive" -o "$candidate"
...
for unselected in sin sinf sinl sincos sincosf sincosl cos cosf cosl tan tanf \
	tanl sinh sinhf sinhl cosh coshf coshl tanhl asin asinf asinl acos \
...
"$candidate" >"$candidate_output" || fail "freestanding tanh/tanhf fixture failed"
if ! cmp -s "$reference_output" "$candidate_output"; then
	cmp -l "$reference_output" "$candidate_output" | sed -n '1,120p' >&2 || true
	fail "candidate tanh/tanhf record stream differs from pinned musl"
fi

printf 'x86 static libc tanh/tanhf: PASS (%s records)\n' "$record_count"
