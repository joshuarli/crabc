#!/usr/bin/env bash
# Native Linux/x86-64 complete math.elementary-long-double C++ ABI evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 math.elementary-long-double header ABI: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-math-elementary-long-double-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
probe="$ROOT_DIR/compat/x86_64/math_elementary_long_double_header_abi_probe.cpp"
...
printf 'x86 pinned-musl/project math.elementary-long-double header ABI: PASS\n'
