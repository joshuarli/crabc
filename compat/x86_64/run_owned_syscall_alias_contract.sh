...
compile_probe_object contract "$CONTRACT_SOURCE"
compile_probe_object override "$OVERRIDE_SOURCE"

# Every provider consumes these installed-header objects. Recompiling the
# oracle source would change the input whose cross-provider ABI is measured.
run probe-object-seal python3 -B - "$WORK" "$CONTRACT_SOURCE" "$OVERRIDE_SOURCE" <<'PY'
import hashlib
import json
from pathlib import Path
import sys
...
rows = {}
for probe, source in zip(('contract', 'override'), sys.argv[2:], strict=True):
    obj = work / f'{probe}.o'
    rows[probe] = {
        'object_path': str(obj),
...
(work / 'probe-object-seal.json').write_text(json.dumps(rows, sort_keys=True) + '\n')
PY

run oracle-contract-link "$ORACLE_CC" -static -no-pie \
...
run oracle-override-link "$ORACLE_CC" -static -no-pie \
...
for mode in pie non-pie; do
    case "$mode" in
        pie) oracle_flags=(-pie) ;;
        non-pie) oracle_flags=(-no-pie) ;;
    esac
    run "oracle-dynamic-$mode-contract-link" "$ORACLE_CC" \
        "${oracle_flags[@]}" "$WORK/contract.o" \
        -Wl,--dynamic-linker,/lib/ld-musl-x86_64.so.1 -o "$WORK/oracle-dynamic-$mode-contract"
    run "oracle-dynamic-$mode-override-link" "$ORACLE_CC" \
        "${oracle_flags[@]}" "$WORK/override.o" \
...
run probe-object-link-proof python3 -B - "$WORK" "$(dirname "$READER")" <<'PY'
import hashlib
import json
from pathlib import Path
import sys
...
from owned_syscall_alias_contract_reader import require_same_probe_object

rows = json.loads((work / 'probe-object-seal.json').read_text())
for probe in ('contract', 'override'):
    row = rows[probe]
    for kind in ('source', 'object'):
        actual = hashlib.sha256(Path(row[f'{kind}_path']).read_bytes()).hexdigest()
...
(work / 'probe-object-link-proof.json').write_text(json.dumps(rows, sort_keys=True) + '\n')
PY
