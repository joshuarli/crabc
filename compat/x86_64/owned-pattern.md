...
baseline. The candidate must then make exactly musl's opens, `getdents64` and
file-status calls and closes (net of musl's nscd sockets), and no more `fcntl`
or `mmap`/`munmap`/`mremap` calls. musl's `fopen("re")` sets `FD_CLOEXEC`
twice on a descriptor it opened with `O_CLOEXEC`; the owned `fopen` sets it
once (one fewer `fcntl` per passwd lookup in the corpus). Musl's `opendir` makes one `open` plus
