#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc bsearch evidence.
#
...
# The candidate must extract only bsearch, not qsort/qsort_r/__qsort_r or a
# search-container/runtime dependency.
set -euo pipefail
...
assert_static_closure() {
    ...
    objdump -d --disassemble=bsearch "$candidate_path" >"$bsearch_disassembly"
    ...
    if grep -Eq '[[:space:]]TLS[[:space:]]|TLSGD|TLSLD|TLSDESC|GOTTPOFF|DTPMOD(64)?|DTPOFF(32|64)?|__tls_get_addr' \
        ...
        fail "bsearch candidate unexpectedly retains TLS"
    fi
    ...
    if grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$bsearch_disassembly"; then
        fail "bsearch unexpectedly performs a syscall"
    fi
}
...
assert_candidate_excludes_sorting() {
    ...
    for symbol in __qsort_r qsort qsort_r lfind lsearch tdelete tfind tsearch \
        hcreate hsearch; do
    ...
    if grep -Eq '(__qsort_r|qsort_r|qsort|lfind|lsearch)' "$bsearch_disassembly"; then
        fail "bsearch implementation retains a sorting/search helper"
    fi
}
...
candidate="$work_dir/crabc-static-bsearch-candidate"
...
disassembly="$work_dir/candidate-disassembly"; bsearch_disassembly="$work_dir/bsearch-disassembly"
cd "$ROOT_DIR"

"$ORACLE_CC" -std=c11 -I "$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_bsearch_probe.c >/dev/null 2>"$trace"
...
grep -Eq '[[:space:]][TW][[:space:]]bsearch$' "$archive_symbols" ||
    fail "archive does not define bsearch"

"$ORACLE_CC" -std=c11 -DCRABC_BSEARCH_FREESTANDING -I "$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_bsearch_probe.c compat/x86_64/libc_bsearch_start.S \
    "$archive" -o "$candidate"
