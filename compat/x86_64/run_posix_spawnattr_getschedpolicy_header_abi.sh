#!/usr/bin/env bash
# Native Linux/x86-64 POSIX spawn-attribute scheduler-policy C/C++ declaration gate.
#
...
# ambient libc cannot mask this unconditional ignored-pointer declaration or
# C++ linkage. It proves the one error-return spelling, not scheduler policy,
# spawn execution, lifecycle, file actions, signals, or public x86 support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_getschedpolicy_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_getschedpolicy_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 spawn.h posix_spawnattr_getschedpolicy headers: %s\n' "$*" >&2
    exit 1
}
...
check_cxx_linkage() {
    ...
    grep -Eq '[[:space:]]posix_spawnattr_getschedpolicy$' <<<"$undefined" ||
        fail "$profile $tree C++ probe does not retain C linkage for posix_spawnattr_getschedpolicy"
    if grep -Eq '_Z[0-9].*posix_spawnattr_getschedpolicy' <<<"$undefined"; then
        fail "$profile $tree C++ probe retained a mangled posix_spawnattr_getschedpolicy reference"
    fi
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-posix-spawnattr-getschedpolicy-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional posix_spawnattr_getschedpolicy declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <spawn.h> posix_spawnattr_getschedpolicy ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
