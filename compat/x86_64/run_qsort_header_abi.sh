#!/usr/bin/env bash
# Native Linux/x86-64 stdlib qsort C/C++ declaration gate.
#
# Pinned musl 1.2.6 is the declaration and C-linkage oracle. `qsort` is
# unconditional in stdlib.h, so strict, POSIX, X/Open, GNU, and BSD profiles
# all retain one two-argument comparator ABI. Project headers are placed first
# only for the candidate pass; this gate does not link libc.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 stdlib qsort ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/qsort_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/qsort_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-qsort-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-qsort-cxx.o"
candidate_cxx_object="$work_dir/candidate-qsort-cxx.o"
...
if ! "$ORACLE_CC" -std=c11 -U_GNU_SOURCE -D__STRICT_ANSI__ -fno-builtin \
    ...
    fail "project C qsort header contract drifted"
fi
...
printf 'x86 pinned-musl/project C/C++ <stdlib.h> qsort ABI: PASS\n'
