#!/usr/bin/env bash
# Native Linux/x86-64 legacy res_init C/C++ declaration gate.
#
...
# This proves a single historical no-argument spelling, not resolver state,
# configuration, DNS, sockets, netdb, or any general <resolv.h> completion.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/res_init_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/res_init_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 resolv.h res_init headers: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-res-init-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional res_init declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <resolv.h> res_init ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
