#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc ns_get16 evidence.
#
...
fail() {
    printf 'ERROR: x86 static libc ns_get16: %s\n' "$*" >&2
    exit 1
}
...
extract_selected_member() {
    ...
    [ "${#matches[@]}" = 1 ] || fail "ns_get16 must have exactly one selected archive member"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-ns-get16.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-ns-get16-reference"
candidate="$work_dir/crabc-static-ns-get16-candidate"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
musl_archive="$("$ORACLE_CC" -print-file-name=libc.a)"
musl_object="$work_dir/musl-ns-parse.o"
musl_disassembly="$work_dir/musl-ns-get16-disassembly"
header_trace="$work_dir/header-trace"
archive_symbols="$work_dir/archive-symbols"
selected_c_abi_symbols="$work_dir/selected-c-abi-symbols"
expected_c_abi_symbols="$work_dir/expected-c-abi-symbols"
selected_members="$work_dir/selected-ns-get16-members"
selected_member_names="$work_dir/selected-ns-get16-member-names"
...
if grep -Eq '\b(call|syscall)\b|R_X86_64_' "$musl_disassembly"; then
    fail "pinned musl ns_get16 text unexpectedly depends on another boundary"
fi

"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_ns_get16_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_ns_get16_probe.c \
    -o "$reference"
if env -i LC_ALL=C TZ=UTC "$reference"; then
    :
else
    status=$?
    fail "pinned-musl ns_get16 fixture exited ${status}"
fi
...
grep -Eq '[[:space:]][T][[:space:]]ns_get16$' "$archive_symbols" ||
    fail "archive does not define ns_get16"
selected_member="$(extract_selected_member "$archive" "$selected_members" \
    "$selected_member_names")"
[ -f "$selected_member" ] || fail "selected ns_get16 member is missing"

"$ORACLE_CC" -std=c11 -DCRABC_NS_GET16_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie \
    -ffreestanding -fno-builtin -fno-stack-protector -Wl,-e,_start \
    -Wl,--gc-sections -Wl,--no-undefined compat/x86_64/libc_ns_get16_probe.c \
    compat/x86_64/libc_ns_get16_start.S "$selected_member" -o "$candidate"

readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --program-headers --wide "$candidate" >"$candidate_program_headers"
readelf --dynamic --wide "$candidate" >"$candidate_dynamic" || true
readelf --relocs --wide "$candidate" >"$candidate_relocations"
objdump -d --disassemble=ns_get16 "$candidate" >"$candidate_disassembly"
grep -Eq '[[:space:]][T][[:space:]]ns_get16$' <(nm -g --defined-only "$candidate") ||
    fail "archive-free candidate does not retain ns_get16"
...
if grep -Eq '\b(call|syscall)\b' "$candidate_disassembly"; then
    fail "ns_get16 candidate calls an unselected runtime boundary"
fi
for unselected in dn_expand dn_skipname ns_get32 ns_put16 ns_put32 ns_initparse \
...
if env -i LC_ALL=C TZ=UTC "$candidate"; then
    ...
    status=$?
    fail "freestanding ns_get16 fixture exited ${status}"
fi

printf 'x86 static crabc-libc ns_get16: PASS\n'
