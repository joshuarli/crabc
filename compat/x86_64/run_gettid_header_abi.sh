#!/usr/bin/env bash
# Native Linux/x86-64 GNU gettid C/C++ declaration gate.
#
...
# host-libc header leakage. `gettid` is a GNU-only x86 LP64 declaration; it is
# hidden outside feature selectors under strict, POSIX.1-2008, X/Open 700, and
# BSD selection.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/gettid_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/gettid_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 unistd.h gettid ABI: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-gettid-header.XXXXXX)"
        ...
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                run_compiler "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
                    -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
                    -D_GNU_SOURCE -DCRABC_EXPECT_GETTID \
                    -Werror=implicit-function-declaration "${include_args[@]}" \
                    -fsyntax-only "$C_PROBE"
            else
                object="$work_dir/${variant}-gettid.o"
                run_compiler "$compiler" -std=c++17 -x c++ -U_GNU_SOURCE \
                    -U_BSD_SOURCE -U_XOPEN_SOURCE -U_POSIX_C_SOURCE \
                    -U_DEFAULT_SOURCE -D_GNU_SOURCE -DCRABC_EXPECT_GETTID \
                    -nostdinc++ "${include_args[@]}" -c "$CXX_PROBE" -o "$object"
                undefined="$(nm --undefined-only "$object")"
                grep -Eq '[[:space:]]gettid$' <<<"$undefined" ||
                    fail "C++ probe does not retain C linkage for gettid (${variant})"
                if grep -Eq '_Z[0-9].*gettid' <<<"$undefined"; then
                    fail "C++ probe retained a mangled gettid reference (${variant})"
                fi
            fi
        done
    done
...
run_compiler "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
    -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
    -D_GNU_SOURCE -DCRABC_EXPECT_GETTID "${include_args[@]}" \
...
printf 'x86 pinned-musl/project C/C++ <unistd.h> gettid ABI: PASS\n'
