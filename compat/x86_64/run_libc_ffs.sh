...
for unselected in fls flsl flsll memccpy mempcpy strcasecmp strncasecmp bcopy bzero strtol malloc free; do if grep -Eq "[[:space:]][TW][[:space:]]${unselected}$" "$archive_symbols"; then fail "archive accidentally exports unselected $unselected"; fi; done
