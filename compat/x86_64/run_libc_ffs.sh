...
# `strtol` belongs to the separately selected integer-parsing artifact in
# this shared archive, not the find-first-set fixture.
for unselected in fls flsl flsll memccpy mempcpy strcasecmp strncasecmp bcopy bzero malloc free; do if grep -Eq "[[:space:]][TW][[:space:]]${unselected}$" "$archive_symbols"; then fail "archive accidentally exports unselected $unselected"; fi; done
