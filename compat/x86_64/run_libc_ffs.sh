#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc find-first-set evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc find-first-set: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-ffs.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"; archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-ffs-reference"; candidate="$work_dir/crabc-static-ffs-candidate"
...
cd "$ROOT_DIR"
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H compat/x86_64/libc_ffs_probe.c >/dev/null 2>"$trace"
for header in stdint.h strings.h features.h bits/alltypes.h; do grep -Fq "$ROOT_DIR/include/$header" "$trace" || fail "fixture did not use project $header"; done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector -I"$ROOT_DIR/include" compat/x86_64/libc_ffs_probe.c -o "$reference"
"$reference" || fail "pinned-musl find-first-set fixture failed"
...
# `strtol`, fixed-locale case comparison, and the legacy bcopy/bzero adapters
# belong to separately selected siblings in this shared archive, not the
# find-first-set fixture.
for unselected in fls flsl flsll memccpy mempcpy malloc free; do if grep -Eq "[[:space:]][TW][[:space:]]${unselected}$" "$archive_symbols"; then fail "archive accidentally exports unselected $unselected"; fi; done
...
"$candidate" || fail "freestanding find-first-set fixture failed"
printf 'x86 static libc find-first-set: PASS\n'
