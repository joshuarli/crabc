...
# one caller-owned 16-bit wire-write function through C and C++. It selects no
# resolver state or `/etc/resolv.conf`.
...
check_cxx_c_linkage() {
    ...
    for symbol in dn_skipname ns_get16 ns_get32 ns_put16; do
        printf '%s\n' "$undefined" | grep -Eq "[[:space:]]${symbol}$" ||
            fail "$tree C++ probe does not retain C linkage for ${symbol}"
    done
    for mangled in '_Z.*dn_skipname' '_Z.*ns_get16' '_Z.*ns_get32' '_Z.*ns_put16'; do
