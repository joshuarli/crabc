...
# 32-bit wire-read function, one caller-owned 16-bit wire-write function, one
# caller-owned 32-bit wire-write function, and one caller-owned resource-record
# span function through C and C++. It selects no resolver state or `/etc/resolv.conf`.
