...
# 16-bit wire-read function, one caller-owned 32-bit wire-read function, and
# one caller-owned 16-bit wire-write function, and one caller-owned 32-bit
...
check_cxx_c_linkage() {
    ...
    for symbol in dn_skipname ns_get16 ns_get32 ns_put16 ns_put32; do
        printf '%s\n' "$undefined" | grep -Eq "[[:space:]]${symbol}$" ||
            fail "$tree C++ probe does not retain C linkage for ${symbol}"
    done
    for mangled in '_Z.*dn_skipname' '_Z.*ns_get16' '_Z.*ns_get32' '_Z.*ns_put16' '_Z.*ns_put32'; do
