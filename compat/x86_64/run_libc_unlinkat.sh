#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc unlinkat evidence.
#
...
# archive. It proves one caller-directed directory-entry removal entry, not
# unlink/rmdir, a general pathname policy, libc.so, CRT, loader, sysroot, or
# public x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc unlinkat: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_unlinkat_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-unlinkat.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-unlinkat-reference"
candidate="$work_dir/crabc-static-unlinkat-candidate"
...
unlinkat_disassembly="$work_dir/unlinkat-disassembly"

cd "$ROOT_DIR"
mkdir "$reference_work" "$candidate_work"
"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_unlinkat_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_unlinkat_probe.c -o "$reference"
(cd "$reference_work" && "$reference") ||
    fail "pinned-musl unlinkat fixture failed"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap unlinkat; do
...
grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$unlinkat_disassembly" ||
    fail "unlinkat lacks a direct Linux syscall"
grep -Eq '\$0x107,%e?ax' "$unlinkat_disassembly" ||
...
(cd "$candidate_work" && "$candidate") ||
    fail "freestanding unlinkat fixture failed"

printf 'x86 static crabc-libc unlinkat: PASS\n'
