#!/usr/bin/env bash
# Native Linux/x86-64 selected static fmax/fmaxf/fmin/fminf evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc math minmax: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-minmax.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-math-minmax-reference"
candidate="$work_dir/crabc-static-math-minmax-candidate"
header_cxx_reference="$work_dir/musl-math-minmax-header.o"
header_cxx_candidate="$work_dir/project-math-minmax-header.o"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_minmax_probe.c >/dev/null 2>"$header_trace"
for header in fenv.h float.h math.h stdint.h features.h bits/alltypes.h; do
	grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
		fail "fixture did not use project $header"
done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
	compat/x86_64/libc_math_minmax_probe.c -o "$reference"
"$reference" || fail "pinned-musl math minmax fixture failed"
...
for mode in sse x87; do
	...
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -c compat/x86_64/math_minmax_header_abi_probe.cpp \
		-o "$header_cxx_reference"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" \
		-c compat/x86_64/math_minmax_header_abi_probe.cpp -o "$header_cxx_candidate"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only \
		compat/x86_64/math_minmax_header_abi_probe.cpp >/dev/null 2>"$cxx_header_trace"
	...
	for object in "$header_cxx_reference" "$header_cxx_candidate"; do
		...
		for symbol in fmax fmaxf fmin fminf; do
			grep -Eq "[[:space:]]${symbol}$" <<<"$undefined" ||
				fail "C++ ${mode} probe does not retain unmangled ${symbol}"
		done
		if grep -Eq '_Z.*(fmax|fmin)' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled math-minmax reference"
		fi
	done
done
...
for symbol in fmax fmaxf fmin fminf feclearexcept fegetenv feraiseexcept fesetenv fesetround fetestexcept; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_MINMAX_FREESTANDING \
	...
	compat/x86_64/libc_math_minmax_probe.c compat/x86_64/libc_math_minmax_start.S \
	"$archive" -o "$candidate"
...
for unselected in fmaxl fminl fdim fdimf fdiml fabs fabsf fabsl copysign copysignf copysignl \
...
for instruction in ucomisd ucomiss movq movd; do
	grep -Eq "[[:space:]]${instruction}([[:space:]]|$)" "$disassembly" ||
		fail "candidate lacks ${instruction}"
done
"$candidate" || fail "freestanding math-minmax fixture failed"

printf 'x86 static libc math minmax: PASS\n'
