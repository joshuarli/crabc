/* Static crabc-libc x86-64 selected nanosleep fixture.
 *
...
_Static_assert(sizeof(sigset_t) == 128 && sizeof(struct sigaction) == 152,
    "x86 signal records");
_Static_assert(SYS_nanosleep == 35 && SYS_setitimer == 38,
...
int crabc_x86_64_nanosleep_probe(void)
{
    int status;
...
#ifndef CRABC_NANOSLEEP_FREESTANDING
int main(void)
{
    return crabc_x86_64_nanosleep_probe();
}
#endif
