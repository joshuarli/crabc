#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc process-context evidence.
#
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_process_context_probe.c >/dev/null 2>"$header_trace"
for header in errno.h sys/stat.h sys/types.h unistd.h sys/syscall.h bits/syscall.h; do
...
for symbol in __errno_location getpid getppid getuid getgid geteuid getegid \
    umask setsid setpgid getpgid getsid getpgrp setpgrp; do
    grep -Eq "[[:space:]][TW][[:space:]]${symbol}$" "$archive_symbols" \
        || fail "archive does not define ${symbol}"
done
for unselected in fork _Fork vfork clone execve kill raise \
    gettid syscall setfsuid setfsgid malloc free calloc \
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_PROCESS_CONTEXT_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie \
    -ffreestanding -fno-builtin -fno-stack-protector -Wl,-e,_start \
    -Wl,--no-undefined compat/x86_64/libc_process_context_probe.c \
    compat/x86_64/libc_process_context_start.S "$archive" -o "$candidate"
