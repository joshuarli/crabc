#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc linkat evidence.
#
...
# archive. It proves one caller-supplied-dirfd hard-link entry, not a pathname
# lifecycle, CWD/namespace policy, libc.so, CRT, loader, sysroot, or public
# x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc linkat: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_linkat_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-linkat.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-linkat-reference"
candidate="$work_dir/crabc-static-linkat-candidate"
...
linkat_disassembly="$work_dir/linkat-disassembly"

cd "$ROOT_DIR"
mkdir "$reference_work" "$candidate_work"
"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_linkat_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_linkat_probe.c -o "$reference"
(cd "$reference_work" && "$reference") ||
    fail "pinned-musl linkat fixture failed"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap linkat; do
...
grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$linkat_disassembly" ||
    fail "linkat lacks a direct Linux syscall"
grep -Eq '\$0x109,%e?ax' "$linkat_disassembly" ||
    fail "linkat lacks Linux x86-64 linkat=265"
grep -Eq '%r10' "$linkat_disassembly" ||
    fail "linkat lacks the Linux fourth-word r10 transfer"
grep -Eq '%r8' "$linkat_disassembly" ||
...
(cd "$candidate_work" && "$candidate") ||
    fail "freestanding linkat fixture failed"

printf 'x86 static crabc-libc linkat: PASS\n'
