#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc posix_spawnattr_getpgroup evidence.
#
...
# crabc-libc object. It proves musl's caller-owned process-group readback
# without selecting spawn execution, child lifecycle, file actions, attribute
# initialization or mutation, libc.so, a CRT, a loader, a sysroot, or public
# x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc posix_spawnattr_getpgroup: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_posix_spawnattr_getpgroup_header_abi.sh" >/dev/null
grep -Fqx $'posix_spawnattr_getpgroup\tposix_spawnattr_getpgroup.lo\tT\tGLOBAL\t0\t10' \
    "$AARCH64_STATIC_ABI" ||
    fail "AArch64 musl ABI oracle lost posix_spawnattr_getpgroup ownership"

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-posix-spawnattr-getpgroup.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
selected_archive="$work_dir/libcrabc-posix-spawnattr-getpgroup.a"
selected_object="$work_dir/owner/posix_spawnattr_getpgroup.o"
reference="$work_dir/musl-posix-spawnattr-getpgroup-reference"
candidate="$work_dir/crabc-static-posix-spawnattr-getpgroup-candidate"
musl_archive="$("$ORACLE_CC" -print-file-name=libc.a)"
musl_object="$work_dir/musl-posix-spawnattr-getpgroup.o"
header_trace="$work_dir/header-trace"
archive_symbols="$work_dir/archive-symbols"
selected_symbols="$work_dir/selected-c-abi-symbols"
expected_symbols="$work_dir/expected-c-abi-symbols"
object_undefined="$work_dir/posix-spawnattr-getpgroup-undefined"
object_relocations="$work_dir/posix-spawnattr-getpgroup-relocations"
object_disassembly="$work_dir/posix-spawnattr-getpgroup-disassembly"
...
ar p "$musl_archive" posix_spawnattr_getpgroup.lo >"$musl_object"
readelf --symbols --wide "$musl_object" | grep -E \
    '[[:space:]]FUNC[[:space:]]+GLOBAL[[:space:]].*[[:space:]]posix_spawnattr_getpgroup$' >/dev/null ||
    fail "pinned musl posix_spawnattr_getpgroup.lo lacks strong posix_spawnattr_getpgroup"

"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_posix_spawnattr_getpgroup_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_posix_spawnattr_getpgroup_probe.c \
    -o "$reference"
"$reference" || fail "pinned-musl posix_spawnattr_getpgroup fixture failed"
...
grep -Eq "[[:space:]][TW][[:space:]]posix_spawnattr_getpgroup$" "$archive_symbols" ||
    fail "archive does not define posix_spawnattr_getpgroup"

mapfile -t members < <(archive_member_for_symbol "$archive" posix_spawnattr_getpgroup)
[ "${#members[@]}" -eq 1 ] || fail "posix_spawnattr_getpgroup must have exactly one crate object owner"
...
(
    ...
    objcopy --only-section=.text.posix_spawnattr_getpgroup \
        --keep-symbol=posix_spawnattr_getpgroup "${members[0]}" \
...
if [ "${exports[*]}" != "posix_spawnattr_getpgroup" ]; then
    printf 'expected: %s\nactual:   %s\n' "posix_spawnattr_getpgroup" "${exports[*]}" >&2
    fail "posix_spawnattr_getpgroup object export surface drifted"
fi
...
if [ -s "$object_undefined" ]; then
    ...
    fail "posix_spawnattr_getpgroup object unexpectedly depends on another symbol"
fi
...
if grep -Eq '[[:space:]](call|syscall)([[:space:]]|$)' "$object_disassembly"; then
    fail "posix_spawnattr_getpgroup object unexpectedly performs a call or syscall"
fi

"$ORACLE_CC" -std=c11 -DCRABC_POSIX_SPAWNATTR_GETPGROUP_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    -Wl,-Map,"$link_map" compat/x86_64/libc_posix_spawnattr_getpgroup_probe.c \
    compat/x86_64/libc_posix_spawnattr_getpgroup_start.S "$selected_archive" \
    -o "$candidate"
...
awk '$4 == "FUNC" && $5 == "GLOBAL" && $8 == "posix_spawnattr_getpgroup" { found = 1 }
     END { exit(found ? 0 : 1) }' "$candidate_symbols" ||
    fail "candidate lacks global posix_spawnattr_getpgroup"
...
"$candidate" || fail "freestanding posix_spawnattr_getpgroup fixture failed"

printf 'x86 static libc posix_spawnattr_getpgroup: PASS\n'
