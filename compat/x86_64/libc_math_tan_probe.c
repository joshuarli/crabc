 ...
 * inputs. It selects only binary64/binary32 tangent: tanl, sincos, fenv policy,
...
#define TAN_F64_CASES 32
#define TAN_F32_CASES 32
#define TAN_ROUNDING_CASES 4
#define TAN_RECORD_WORDS 4
#define TAN_RECORD_COUNT ((TAN_F64_CASES + TAN_F32_CASES) * TAN_ROUNDING_CASES)
#define TAN_RECORD_STORAGE_WORDS (TAN_RECORD_COUNT * TAN_RECORD_WORDS)
...
static const uint64_t binary64_inputs[TAN_F64_CASES] = {
...
static const uint32_t binary32_inputs[TAN_F32_CASES] = {
...
int crabc_x86_64_math_tan_probe(void)
{
	...
	for (mode_index = 0; mode_index < TAN_ROUNDING_CASES && status == 0;
		mode_index++) {
		for (input_index = 0; input_index < TAN_F64_CASES && status == 0;
			input_index++)
			status = record_binary64(&cursor, rounding_modes[mode_index],
				binary64_inputs[input_index]);
		for (input_index = 0; input_index < TAN_F32_CASES && status == 0;
			input_index++)
