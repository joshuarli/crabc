#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ <sched.h> scheduler-priority bounds ABI gate.
#
# Pinned musl 1.2.6 supplies the declaration/linkage oracle. This checks only
# the two read-only scalar declarations and external-C spelling; it selects no
# scheduler policy, parameters, priority mutation, affinity, thread runtime,
# clock/timer behavior, or public x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 sched.h priority bounds ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/sched_priority_bounds_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/sched_priority_bounds_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-sched-priority-bounds-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-sched-priority-bounds-cxx.o"
candidate_cxx_object="$work_dir/candidate-sched-priority-bounds-cxx.o"
...
printf 'x86 pinned-musl/project C/C++ <sched.h> priority bounds ABI: PASS\n'
