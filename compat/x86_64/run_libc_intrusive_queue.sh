...
# The candidate must extract exactly the paired two-link mutation leaf, not a
# search container, allocation, callback, errno, or runtime boundary.
set -euo pipefail
...
assert_static_closure() {
    ...
    objdump -d --disassemble=insque "$candidate_path" >"$insque_disassembly"
    objdump -d --disassemble=remque "$candidate_path" >"$remque_disassembly"
    ...
    if grep -Eq '[[:space:]]TLS[[:space:]]|TLSGD|TLSLD|TLSDESC|GOTTPOFF|DTPMOD(64)?|DTPOFF(32|64)?|__tls_get_addr' \
        ...
        fail "intrusive-queue candidate unexpectedly retains TLS"
    fi
    ...
    if grep -Eq '[[:space:]]syscall([[:space:]]|$)' \
        ...
        fail "intrusive queue unexpectedly performs a syscall"
    fi
    # The entry shim has one exit syscall. A second syscall would be selected
    # by insque or remque, which has no kernel boundary.
    local syscall_count
...
assert_candidate_excludes_unowned_algorithms() {
    ...
    for symbol in bsearch lfind lsearch __qsort_r qsort qsort_r tdelete \
        tdestroy tfind tsearch twalk __tsearch_balance hcreate hcreate_r \
        hdestroy hdestroy_r hsearch hsearch_r memcpy memmove; do
        if awk -v symbol="$symbol" '$8 == symbol { found = 1 } END { exit !found }' \
            "$symbols"; then
            fail "candidate accidentally selects ${symbol}"
        fi
    done
    if grep -Eq '(<(bsearch|lfind|lsearch|__qsort_r|qsort|qsort_r|tdelete|tfind|tsearch|hsearch|memcpy|memmove)>)' \
        "$disassembly"; then
        fail "intrusive-queue implementation retains an unrelated helper"
    fi
}
...
disassembly="$work_dir/candidate-disassembly"; insque_disassembly="$work_dir/insque-disassembly"
remque_disassembly="$work_dir/remque-disassembly"
cd "$ROOT_DIR"

"$ORACLE_CC" -std=c11 -I "$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_intrusive_queue_probe.c >/dev/null 2>"$trace"
...
timeout "$EXECUTION_TIMEOUT" "$reference" ||
    fail "pinned-musl intrusive-queue fixture failed"
...
"$ORACLE_CC" -std=c11 -DCRABC_INTRUSIVE_QUEUE_FREESTANDING -I "$ROOT_DIR/include" \
...
timeout "$EXECUTION_TIMEOUT" "$candidate" ||
    fail "freestanding intrusive-queue fixture failed"

printf 'x86 static libc insque/remque: PASS\n'
