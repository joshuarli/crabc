...
readonly SCENARIOS='lifo capacity reentrant worker concurrent contention fork atexit-chain atexit-contention'
...
expected_output() {
    case "$1" in
        ...
        # 16 threads x 40 concurrent registrations all run, then the count.
        atexit-contention) printf 640D ;;
        *) return 2 ;;
    esac
}
