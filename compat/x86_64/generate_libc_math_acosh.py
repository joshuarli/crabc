#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``acosh``/``acoshf`` leaf.
...
# The public binary64/binary32 entries select musl's log, log1p, and sqrt
# algorithms plus their fixed tables and domain helpers. Every provider is
# compiled under a private name, so this leaf selects no existing public math
# sibling or ambient libm entry.
PUBLIC_SOURCES = (
    "src/math/acosh.c",
    "src/math/acoshf.c",
)
...
PRIVATE_RENAMES = {
    "log": "crabc_x86_math_acosh_provider_log",
    "logf": "crabc_x86_math_acosh_provider_logf",
    "log1p": "crabc_x86_math_acosh_provider_log1p",
    "log1pf": "crabc_x86_math_acosh_provider_log1pf",
    "sqrt": "crabc_x86_math_acosh_provider_sqrt",
    "sqrtf": "crabc_x86_math_acosh_provider_sqrtf",
    "__log_data": "crabc_x86_math_acosh_provider_log_data",
    "__logf_data": "crabc_x86_math_acosh_provider_logf_data",
    "__rsqrt_tab": "crabc_x86_math_acosh_provider_rsqrt_tab",
    "__math_divzero": "crabc_x86_math_acosh_provider_divzero",
    "__math_divzerof": "crabc_x86_math_acosh_provider_divzerof",
    "__math_invalid": "crabc_x86_math_acosh_provider_invalid",
    "__math_invalidf": "crabc_x86_math_acosh_provider_invalidf",
}
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_acosh_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_acosh_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_acosh_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_acosh_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 acosh/acoshf translation.",
        " * Generated by compat/x86_64/generate_libc_math_acosh.py; see",
        " * math_acosh.rs for the source/license/ABI contract.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-acosh-") as temporary:
