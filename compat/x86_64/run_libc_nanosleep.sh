#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc nanosleep evidence.
#
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_nanosleep_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_nanosleep_probe.c \
    -o "$reference"
if "$reference"; then
    :
else
    status=$?
    fail "pinned-musl nanosleep fixture exited ${status}"
fi
...
for symbol in __errno_location nanosleep; do
...
if grep -Eq '[[:space:]]sleep$' "$candidate_symbols"; then
    fail "nanosleep candidate unexpectedly pulls separately selected sleep"
fi
