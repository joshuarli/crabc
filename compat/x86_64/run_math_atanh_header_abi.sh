#!/usr/bin/env bash
# Native Linux/x86-64 C++ declaration/linkage evidence for atanh/atanhf.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly SYMBOLS=(atanh atanhf)

fail() { printf 'ERROR: x86 math atanh header ABI: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-math-atanh-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
probe="$ROOT_DIR/compat/x86_64/math_atanh_header_abi_probe.cpp"
...
for mode in sse x87; do
	...
	for object in "$reference" "$candidate"; do
		...
		if grep -Eq '_Z.*atanh' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled atanh reference"
		fi
	done
done

printf 'x86 pinned-musl/project atanh/atanhf header ABI: PASS\n'
