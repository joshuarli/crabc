#!/usr/bin/env bash
# Native Linux/x86-64 selected <sys/mman.h> mapping-synchronization ABI matrix.
#
# Pinned musl 1.2.6 is the declaration, constant, feature-visibility, and C
# linkage oracle. The candidate uses raw GCC with project headers only, so an
# ambient libc cannot satisfy the `msync` surface. Musl exposes this narrow
# declaration and its MS_* constants unconditionally in every selected
# profile. This compile-only gate proves neither runtime behavior nor the
# upstream syscall_cp cancellation boundary, a complete mapping header, family
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/memory_sync_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/memory_sync_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 memory-sync header ABI: %s\n' "$*" >&2
    exit 1
}
...
profile_arguments() {
    case "$1" in
        ...
        # GCC predefines _GNU_SOURCE for C++; remove it so this exercises the
        # macro-free C++17 profile rather than an ambient GNU selection.
...
[ -f "$C_PROBE" ] || fail "missing C memory-sync header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ memory-sync header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-memory-sync-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        if ! compile_profile "$tree" "$profile" "$trace" "$object"; then
            fail "$tree $profile unconditional msync profile failed: $(first_diagnostic "$trace")"
        fi
...
printf 'x86 pinned-musl/project C/C++ unconditional msync ABI matrix: PASS (%s profiles; compile-only)\n' \
