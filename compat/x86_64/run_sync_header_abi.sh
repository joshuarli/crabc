#!/usr/bin/env bash
# Native Linux/x86-64 selected `sync` C/C++ header ABI matrix.
#
# Pinned musl 1.2.6 owns the `unistd.h` feature-selection and unmangled C
# linkage oracle. The candidate pass uses raw GCC with project headers only,
# so an ambient libc cannot satisfy the declaration. This checks only the
# historical X/Open/GNU/BSD `void sync(void)` spelling, not a filesystem API,
# writeback schedule, durability policy, or public x86 support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/sync_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/sync_header_abi_probe.cpp"
readonly EXPECTED_PROFILE_COUNT=8
readonly EXPECTED_VISIBLE_PROFILE_COUNT=5
readonly EXPECTED_HIDDEN_PROFILE_COUNT=3
readonly -a PROFILES=(c-default c11-gnu cxx17-gnu c11-strict c11-posix-2008 c11-xopen-700 c11-bsd cxx17-strict)

fail() {
    printf 'ERROR: x86 sync headers: %s\n' "$*" >&2
    exit 1
}
...
profile_is_visible() {
    case "$1" in
        c-default|c11-gnu|cxx17-gnu|c11-xopen-700|c11-bsd) return 0 ;;
        *) return 1 ;;
    esac
}
...
check_trace() {
    ...
    for header in features.h unistd.h; do
...
[ -f "$C_PROBE" ] || fail "missing C sync ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ sync ABI probe"
...
for profile in "${PROFILES[@]}"; do
    ...
    if profile_is_visible "$profile"; then
        expectation=CRABC_EXPECT_SYNC
        visible_count=$((visible_count + 1))
        if ! compile_profile reference "$profile" "$expectation" "$reference_trace" "$reference_object"; then
        ...
        if ! compile_profile candidate "$profile" "$expectation" "$candidate_trace" "$candidate_object"; then
        ...
        expectation=CRABC_REQUIRE_SYNC_HIDDEN
        hidden_count=$((hidden_count + 1))
        if compile_profile reference "$profile" "$expectation" "$reference_trace" "$reference_object"; then
            fail "$profile pinned musl unexpectedly exposes sync"
        fi
        check_trace reference "$profile" "$reference_trace"
        if compile_profile candidate "$profile" "$expectation" "$candidate_trace" "$candidate_object"; then
            fail "$profile project headers unexpectedly expose sync"
        fi
        check_trace candidate "$profile" "$candidate_trace"
    fi
done
...
printf 'x86 pinned-musl/project C/C++ sync ABI matrix: PASS (%s profiles; visible=%s hidden=%s; compile-only)\n' \
