#!/usr/bin/env bash
# Native Linux/x86-64 compile-only <string.h>/<strings.h> byte-string ABI slice.
#
...
# strverscmp/strchrnul and GNU/BSD bcopy/bzero are checked explicitly against
# both header trees.
set -euo pipefail
...
# The historical bcopy/bzero declarations are visible in both selected GNU
# and BSD C profiles; C++ linkage is already proved by the GNU-selected pass.
"$ORACLE_CC" -std=c11 "${bsd_definitions[@]}" -fsyntax-only "$c_probe"
"$ORACLE_CC" -std=c11 "${bsd_definitions[@]}" -I "$ROOT_DIR/include" \
    -fsyntax-only "$c_probe"

# Under strict POSIX selectors, musl exposes the ordinary byte-string set but
# keeps GNU strverscmp/strchrnul and BSD bcopy/bzero declarations hidden. A
# successful opt-in C reference is therefore a header-gating regression in
# either oracle or candidate headers.
strict_definitions=(-D_POSIX_C_SOURCE=200809L -DCRABC_REQUIRE_STRCHRNUL -DCRABC_REQUIRE_STRVERSCMP -DCRABC_REQUIRE_BCOPY -DCRABC_REQUIRE_BZERO)
if "$ORACLE_CC" -std=c11 "${strict_definitions[@]}" -fsyntax-only "$c_probe" \
    >/dev/null 2>"$work_dir/oracle-strict-errors"; then
    fail "pinned musl exposes feature-gated byte-string declarations under strict POSIX"
fi
if "$ORACLE_CC" -std=c11 "${strict_definitions[@]}" -I "$ROOT_DIR/include" \
    ...
    fail "project headers expose feature-gated byte-string declarations under strict POSIX"
fi
printf 'x86 pinned-musl/project C/C++ <string.h>/<strings.h> byte-string ABI: PASS\n'
