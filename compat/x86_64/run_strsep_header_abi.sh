#!/usr/bin/env bash
# Native Linux/x86-64 compile-only <string.h> strsep ABI slice.
#
...
# strsep is GNU/BSD-selected and stays hidden in default/strict/POSIX/XOPEN C.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 string.h strsep ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/strsep_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/strsep_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-strsep-header.XXXXXX)"
...
for definitions_name in gnu_definitions bsd_definitions; do
    declare -n definitions="$definitions_name"
...
if ! "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
    ...
    fail "project GNU strsep header contract drifted"
fi
...
# These C selectors must reject the GNU/BSD declaration in both header trees.
...
for definitions_name in default_definitions strict_definitions posix_definitions xopen_definitions; do
    declare -n definitions="$definitions_name"
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -DCRABC_REQUIRE_STRSEP_HIDDEN \
        -fsyntax-only "$c_probe" >"$work_dir/oracle-${definitions_name}.out" 2>&1; then
        fail "pinned musl exposes strsep outside the GNU/BSD selectors"
    fi
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -I "$ROOT_DIR/include" \
        -DCRABC_REQUIRE_STRSEP_HIDDEN -fsyntax-only "$c_probe" \
        >"$work_dir/project-${definitions_name}.out" 2>&1; then
        fail "project string.h exposes strsep outside the GNU/BSD selectors"
    fi
done

printf 'x86 pinned-musl/project C/C++ <string.h> strsep ABI: PASS\n'
