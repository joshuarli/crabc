/* Static crabc-libc x86-64 caller-buffered UTC gmtime_r fixture.
 *
 ...
 * crabc archive. It admits only caller-owned UTC `struct tm` conversion. It
 * does not select non-reentrant storage, local conversion, timezone or
 * environment state, formatting/parsing, clocks, timers, cancellation, CRT,
...
_Static_assert(__builtin_types_compatible_p(__typeof__(&gmtime_r),
    struct tm *(*)(const time_t *, struct tm *)), "gmtime_r declaration");
...
{
    const time_t input = 0;
    struct tm output;

    errno = E2BIG;
    if (gmtime_r(&input, &output) != &output || errno != E2BIG)
        return 1;
    return !check_normalized(&output, 0, 0, 0, 1, 0, 70, 4, 0);
}

static int pre_epoch(void)
{
    const time_t input = -1;
    struct tm output;

    errno = ERANGE;
    if (gmtime_r(&input, &output) != &output || errno != ERANGE)
        return 1;
    return !check_normalized(&output, 59, 59, 23, 31, 11, 69, 3, 364);
}

static int leap_day(void)
{
    const time_t input = (time_t)951782400;
    struct tm output;

    errno = E2BIG;
    if (gmtime_r(&input, &output) != &output || errno != E2BIG)
        return 1;
    return !check_normalized(&output, 0, 0, 0, 29, 1, 100, 2, 59);
}

static int overflow(void)
{
    const time_t input = INT64_MAX;
    ...
    const struct tm before = output;

    errno = 0;
    if (gmtime_r(&input, &output) != NULL || errno != EOVERFLOW)
        return 1;
    return !same_tm(&output, &before);
}

int crabc_x86_64_gmtime_r_probe(void)
{
    int status = epoch();
...
#ifndef CRABC_GMTIME_R_FREESTANDING
int main(void)
{
    return crabc_x86_64_gmtime_r_probe();
}
#endif
