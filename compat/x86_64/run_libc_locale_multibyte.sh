...
# objects, environment lookup,
# wide-streams, collation, iconv, libc.so, a CRT, or public x86 support.
set -euo pipefail
...
for unselected in newlocale duplocale uselocale freelocale nl_langinfo iconv \
    iconv_open iconv_close mbsnrtowcs wcsnrtombs wcsftime_l wcscoll wcscoll_l \
...
if grep -Eq 'crabc_core|mimalloc|sha_crypt|newlocale|uselocale|duplocale|freelocale|iconv' \
