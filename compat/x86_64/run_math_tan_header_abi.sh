#!/usr/bin/env bash
# Native Linux/x86-64 C++ declaration/linkage evidence for tan/tanf.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly SYMBOLS=(tan tanf)

fail() { printf 'ERROR: x86 math tan header ABI: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-math-tan-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
probe="$ROOT_DIR/compat/x86_64/math_tan_header_abi_probe.cpp"
...
for mode in sse x87; do
	...
	for object in "$reference" "$candidate"; do
		...
		if grep -Eq '_Z.*tan' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled tan reference"
		fi
	done
done

printf 'x86 pinned-musl/project tan/tanf header ABI: PASS\n'
