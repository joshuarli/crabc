/*
 * Selected Linux 5.10 UAPI wrapper ABI facts for native x86-64.
 *
 * This deliberately contains no functions, link step, or device operation.
 * The paired runner compiles it first through pinned musl and then through the
 * project headers under seven isolated language/feature profiles.  It proves
 * only the named forwarding constants, ioctl encodings, and LP64 layouts.
 */
#include <stddef.h>
...
_Static_assert(KD_TEXT == 0 && KD_GRAPHICS == 1, "KD text/graphics modes");
_Static_assert(KDSETMODE == 0x4b3a && KDGETMODE == 0x4b3b, "KD mode ioctls");
_Static_assert(KDGKBENT == 0x4b46 && KDSKBENT == 0x4b47, "KD keyboard ioctls");
_Static_assert(GIO_FONTX == 0x4b6b && PIO_FONTX == 0x4b6c, "KD font ioctls");
_Static_assert(KDFONTOP == 0x4b72, "KD font operation ioctl");
...
_Static_assert(MIN_NR_CONSOLES == 1 && MAX_NR_CONSOLES == 63, "VT console limits");
_Static_assert(VT_OPENQRY == 0x5600 && VT_GETMODE == 0x5601, "VT query ioctls");
_Static_assert(VT_SETMODE == 0x5602 && VT_GETSTATE == 0x5603, "VT state ioctls");
_Static_assert(VT_SENDSIG == 0x5604 && VT_RELDISP == 0x5605, "VT signal ioctls");
_Static_assert(VT_ACTIVATE == 0x5606 && VT_WAITACTIVE == 0x5607, "VT activation ioctls");
_Static_assert(VT_DISALLOCATE == 0x5608 && VT_RESIZE == 0x5609, "VT resize ioctls");
_Static_assert(VT_RESIZEX == 0x560a && VT_LOCKSWITCH == 0x560b, "VT extended ioctls");
_Static_assert(VT_UNLOCKSWITCH == 0x560c && VT_GETHIFONTMASK == 0x560d, "VT switch ioctls");
_Static_assert(VT_WAITEVENT == 0x560e && VT_SETACTIVATE == 0x560f, "VT event ioctls");
_Static_assert(VT_AUTO == 0 && VT_PROCESS == 1 && VT_ACKACQ == 2, "VT process modes");
_Static_assert(VT_EVENT_SWITCH == 1 && VT_EVENT_BLANK == 2, "VT event bits");
_Static_assert(VT_EVENT_UNBLANK == 4 && VT_EVENT_RESIZE == 8, "VT event bits");
...
_Static_assert(SOUND_VERSION == 0x030802, "OSS ABI version");
_Static_assert(AFMT_U8 == 0x00000008 && AFMT_S16_LE == 0x00000010,
               "OSS little-endian sample formats");
_Static_assert(AFMT_S16_BE == 0x00000020 && AFMT_S16_NE == AFMT_S16_LE,
               "OSS native sample format");
_Static_assert(PCM_ENABLE_INPUT == 1 && PCM_ENABLE_OUTPUT == 2, "OSS PCM directions");
_Static_assert(SOUND_MIXER_NRDEVICES == 25, "OSS mixer count");
_Static_assert(SOUND_MIXER_VOLUME == 0 && SOUND_MIXER_PCM == 4 && SOUND_MIXER_NONE == 31,
               "OSS mixer identifiers");
_Static_assert(SOUND_MASK_VOLUME == 1 && SOUND_MASK_PCM == 16, "OSS mixer masks");
_Static_assert(SNDCTL_DSP_SYNC == _SIO('P', 1), "OSS sync request encoding");
...
_Static_assert(SNDCTL_DSP_SPEED == 0xc0045002U, "OSS speed ioctl value");
_Static_assert(SNDCTL_DSP_GETOSPACE == 0x8010500cU, "OSS output-space ioctl value");
_Static_assert(SNDCTL_DSP_GETIPTR == 0x800c5011U, "OSS input-pointer ioctl value");
_Static_assert(SOUND_MIXER_READ_VOLUME == 0x80044d00U, "OSS mixer-read ioctl value");
_Static_assert(SOUND_MIXER_WRITE_PCM == 0xc0044d04U, "OSS mixer-write ioctl value");
_Static_assert(OSS_GETVERSION == 0x80044d76U, "OSS version ioctl value");
_Static_assert(_IOC_DIR(SNDCTL_DSP_SPEED) == (_IOC_READ | _IOC_WRITE),
               "OSS speed ioctl direction");
_Static_assert(_IOC_TYPE(SNDCTL_DSP_SPEED) == 'P' && _IOC_NR(SNDCTL_DSP_SPEED) == 2,
               "OSS speed ioctl type and number");
_Static_assert(_IOC_SIZE(SNDCTL_DSP_SPEED) == sizeof(int), "OSS speed ioctl size");
_Static_assert(_IOC_SIZE(SNDCTL_DSP_GETOSPACE) == sizeof(struct audio_buf_info),
               "OSS output-space ioctl record size");
_Static_assert(_IOC_SIZE(SNDCTL_DSP_GETIPTR) == sizeof(struct count_info),
               "OSS input-pointer ioctl record size");
_Static_assert(_IOC_SIZE(SOUND_MIXER_READ_VOLUME) == sizeof(int),
