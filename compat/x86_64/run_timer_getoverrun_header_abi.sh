#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ <time.h> timer_getoverrun ABI gate.
#
...
# not POSIX timer state, pthread timer representation, or time support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 time.h timer_getoverrun ABI: %s\n' "$*" >&2
    exit 1
}
        ...
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                if "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
                    ...
                    -D__STRICT_ANSI__ -DCRABC_TIMER_GETOVERRUN_EXPECT_HIDDEN \
                    -Werror=implicit-function-declaration "${include_args[@]}" \
                    -fsyntax-only "$c_probe" >"$errors" 2>&1; then
                    fail "timer_getoverrun is visible under strict C11 (${variant})"
                fi
                ...
                >"$errors" 2>&1; then
                fail "timer_getoverrun is visible under strict C++17 (${variant})"
            fi
        done
    done
}
    ...
    for language in c cxx; do
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                ...
                object="$work_dir/${variant}-${profile}-timer-getoverrun-cxx.o"
                "$compiler" -std=c++17 -x c++ -U_GNU_SOURCE -U_BSD_SOURCE \
                    -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
                    "$definition" "${include_args[@]}" -c "$cxx_probe" -o "$object"
                undefined="$(nm --undefined-only "$object")"
                grep -Eq '[[:space:]]timer_getoverrun$' <<<"$undefined" ||
                    fail "C++ probe does not retain C linkage for timer_getoverrun (${variant}, ${profile})"
                if grep -Eq '_Z.*timer_getoverrun' <<<"$undefined"; then
                    fail "C++ probe retained a mangled timer_getoverrun reference (${variant}, ${profile})"
                fi
            fi
        done
    done
...
c_probe="$ROOT_DIR/compat/x86_64/timer_getoverrun_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/timer_getoverrun_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-timer-getoverrun-header.XXXXXX)"
...
if ! "$ORACLE_CC" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
    ...
    fail "project POSIX C timer_getoverrun header contract drifted"
fi
...
printf 'x86 pinned-musl/project C/C++ <time.h> timer_getoverrun ABI: PASS\n'
