...
# `strtoimax` and `strtoumax` belong to the separately selected
# integer-parsing artifact in this shared archive, not this arithmetic fixture.
for unselected in calloc malloc free realloc; do if grep -Eq "[[:space:]][TW][[:space:]]${unselected}$" "$archive_symbols"; then fail "archive accidentally exports unselected $unselected"; fi; done
