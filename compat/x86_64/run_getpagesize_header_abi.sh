#!/usr/bin/env bash
# Native Linux/x86-64 compile-only <unistd.h> getpagesize ABI slice.
#
...
# getpagesize is GNU/BSD-selected and stays hidden in default/strict/POSIX/XOPEN
# C profiles.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 unistd.h getpagesize ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/getpagesize_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/getpagesize_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-getpagesize-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-getpagesize-cxx.o"
candidate_cxx_object="$work_dir/candidate-getpagesize-cxx.o"
...
if ! "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
    ...
    fail "project GNU getpagesize header contract drifted"
fi
...
for object in "$oracle_cxx_object" "$candidate_cxx_object"; do
    ...
    grep -Eq '[[:space:]]getpagesize$' <<<"$undefined" ||
        fail "C++ probe does not retain C linkage for getpagesize"
    if grep -Eq '_Z11getpagesizev' <<<"$undefined"; then
        fail "C++ probe retained a mangled getpagesize reference"
    fi
done
...
for definitions_name in default_definitions strict_definitions posix_definitions xopen_definitions; do
    declare -n definitions="$definitions_name"
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -DCRABC_REQUIRE_GETPAGESIZE_HIDDEN \
        -fsyntax-only "$c_probe" >"$work_dir/oracle-c-${definitions_name}.out" 2>&1; then
        fail "pinned musl exposes getpagesize outside GNU/BSD C selectors"
    fi
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -DCRABC_REQUIRE_GETPAGESIZE_HIDDEN \
        -I "$ROOT_DIR/include" -fsyntax-only "$c_probe" \
        >"$work_dir/project-c-${definitions_name}.out" 2>&1; then
        fail "project unistd.h exposes getpagesize outside GNU/BSD C selectors"
    fi
    if "$ORACLE_CC" -std=c++17 -x c++ -U_GNU_SOURCE "${definitions[@]}" \
        -DCRABC_REQUIRE_GETPAGESIZE_HIDDEN -fsyntax-only "$cxx_probe" \
        >"$work_dir/oracle-cxx-${definitions_name}.out" 2>&1; then
        fail "pinned musl exposes getpagesize outside GNU/BSD C++ selectors"
    fi
    if "$ORACLE_CC" -std=c++17 -x c++ -U_GNU_SOURCE "${definitions[@]}" \
        -DCRABC_REQUIRE_GETPAGESIZE_HIDDEN -I "$ROOT_DIR/include" \
        -fsyntax-only "$cxx_probe" >"$work_dir/project-cxx-${definitions_name}.out" 2>&1; then
        fail "project unistd.h exposes getpagesize outside GNU/BSD C++ selectors"
    fi
done

printf 'x86 pinned-musl/project C/C++ <unistd.h> getpagesize ABI: PASS\n'
