#!/usr/bin/env bash
# Native Linux/x86-64 permanent-stdin __fseterr <stdio_ext.h> ABI proof.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_fseterr_stdin_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_fseterr_stdin_header_abi_probe.cpp"
readonly -a PROFILES=(c11 cxx17)

fail() {
    printf 'ERROR: x86 stdio permanent-stdin __fseterr header ABI: %s\n' "$*" >&2
    exit 1
}
    ...
    case "$1" in
        c11)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
                ...
                -DCRABC_STDIO_PERMANENT_FSETERR_STDIN_C11
            ;;
        cxx17)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
                -U_POSIX_C_SOURCE -U_LARGEFILE64_SOURCE -U_DEFAULT_SOURCE \
                -DCRABC_STDIO_PERMANENT_FSETERR_STDIN_CXX17
            ;;
...
[ -f "$C_PROBE" ] || fail "missing C permanent-stdin __fseterr header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ permanent-stdin __fseterr header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-stdio-permanent-fseterr-stdin-header.XXXXXX)"
...
printf 'x86 stdio permanent-stdin __fseterr header ABI: PASS\n'
