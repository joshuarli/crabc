#!/usr/bin/env bash
# Native Linux/x86-64 bounded static pthread condition-attribute clock evidence.
#
...
fail() {
    printf 'ERROR: x86 static pthread condattr clock: %s\n' "$*" >&2
    exit 1
}
...
assert_no_unselected_condattr_exports() {
    ...
    for unselected in pthread_condattr_init pthread_condattr_destroy \
        ...
        [ "$symbols_path" = "$candidate_symbols" ] ||
            fail "condattr clock sibling exclusions apply only to the final candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_condattr_clock_probe.c >/dev/null 2>"$header_trace"
for header in errno.h pthread.h time.h bits/alltypes.h; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_PTHREAD_CONDATTR_CLOCK_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_pthread_condattr_clock_probe.c \
    compat/x86_64/libc_pthread_condattr_clock_start.S "$archive" -o "$candidate"
...
assert_no_unselected_condattr_exports "$candidate_symbols"
for unselected in __errno_location __crabc_x86_static_tls_bootstrap \
    ...
    pthread_mutex_unlock pthread_cond_timedwait clock_gettime clock_nanosleep; do
