#!/usr/bin/env bash
# Native Linux/x86-64 selected linkat C/C++ header ABI matrix.
#
# Pinned musl 1.2.6 is the declaration, LP64 scalar-layout, and unmangled C++
# C-linkage oracle. The candidate uses raw GCC with only project headers and
# compiler builtin headers, so an ambient libc cannot hide a mismatch. This
# proves one descriptor-relative hard-link leaf, not ordinary link, pathname
# policy, allocation, CWD state, or public x86 support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/linkat_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/linkat_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 linkat headers: %s\n' "$*" >&2
    exit 1
}
...
check_cxx_symbol() {
    ...
    grep -Eq '[[:space:]]linkat$' <<<"$undefined" ||
        fail "$tree $profile C++ probe does not retain C linkage for linkat"
    if grep -Eq '_Z.*linkat' <<<"$undefined"; then
        fail "$tree $profile C++ probe retained a mangled linkat reference"
    fi
}
...
[ -f "$C_PROBE" ] || fail "missing C linkat ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ linkat ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-linkat-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ linkat ABI matrix: PASS (%s profiles; visible=%s hidden=%s; compile-only)\n' \
