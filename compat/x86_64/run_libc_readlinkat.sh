#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc readlinkat evidence.
#
...
# archive. It proves caller-owned non-NUL output, direct four-word request
# placement, the zero-size dummy boundary, and raw Linux result agreement,
# not ordinary readlink, *at family completion, pathname policy, allocation,
# CWD state, a Rust facade, or public x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc readlinkat: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_readlinkat_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-readlinkat.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-readlinkat-reference"
candidate="$work_dir/crabc-static-readlinkat-candidate"
...
readlinkat_disassembly="$work_dir/readlinkat-disassembly"

cd "$ROOT_DIR"
mkdir "$reference_work" "$candidate_work"
"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_readlinkat_probe.c >/dev/null 2>"$header_trace"
for header in errno.h fcntl.h stdint.h sys/syscall.h sys/types.h unistd.h; do
    grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
        fail "fixture did not use the project $header header"
done

"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_readlinkat_probe.c \
    -o "$reference"
(cd "$reference_work" && "$reference") ||
    fail "pinned-musl readlinkat fixture failed"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap readlinkat; do
...
grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$readlinkat_disassembly" ||
    fail "readlinkat lacks a direct Linux syscall"
grep -Eq '\$0x10b,%e?ax' "$readlinkat_disassembly" ||
    fail "readlinkat lacks Linux x86-64 readlinkat=267"
grep -Eq '%r10' "$readlinkat_disassembly" ||
...
(cd "$candidate_work" && "$candidate") ||
    fail "freestanding readlinkat fixture failed"

printf 'x86 static crabc-libc readlinkat: PASS\n'
