#!/usr/bin/env bash
# Native Linux/x86-64 permanent-stdin __freading <stdio_ext.h> ABI proof.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_freading_stdin_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_freading_stdin_header_abi_probe.cpp"
readonly -a PROFILES=(c11 cxx17)

fail() {
    printf 'ERROR: x86 stdio permanent-stdin __freading header ABI: %s\n' "$*" >&2
    exit 1
}
    ...
    case "$1" in
        c11)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
                ...
                -DCRABC_STDIO_PERMANENT_FREADING_STDIN_C11
            ;;
        cxx17)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
                -U_POSIX_C_SOURCE -U_LARGEFILE64_SOURCE -U_DEFAULT_SOURCE \
                -DCRABC_STDIO_PERMANENT_FREADING_STDIN_CXX17
            ;;
...
assert_cxx_c_linkage() {
    ...
    grep -Fxq __freading <<<"$undefined" ||
        fail "$tree C++ probe does not retain C spelling __freading"
    if grep -Eq '_Z.*__freading' <<<"$undefined"; then
        fail "$tree C++ probe retained a mangled __freading reference"
    fi
}
...
[ -f "$C_PROBE" ] || fail "missing C permanent-stdin __freading header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ permanent-stdin __freading header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-stdio-permanent-freading-stdin-header.XXXXXX)"
...
printf 'x86 stdio permanent-stdin __freading header ABI: PASS\n'
