#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 x86 log2/log2f assembly translation.
...
compiler while retaining musl's fixed scalar table reduction and its exact
IEEE exceptional paths.
"""
...
# The two public entries share precisely these data and exceptional-expression
# providers.  They are renamed and localized in the checked assembly so their
# implementation closure does not become another x86 archive export.
PRIVATE_SOURCES = (
...
RENAME = {
    name: f"crabc_x86_math_log2_{name}"
    for name in PRIVATE_SYMBOLS
}

# Fixed SSE evaluation, no contracted FMA, and standard excess precision keep
# binary32/binary64 results and the musl source operation order reproducible.
COMPILE_FLAGS = (
    "-D_XOPEN_SOURCE=700",
    "-std=c99",
    "-ffreestanding",
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    for name in sorted(typed - globals_and_weak, key=len, reverse=True):
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_log2_{tag}_{name}")
    ...
    text = re.sub(
        ...
        rf".Lcrabc_x86_math_log2_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_log2_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_log2_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 x86-64 log2/log2f assembly translation.",
        " * Generated by compat/x86_64/generate_libc_math_log2.py; see",
        " * math_log2.rs for the source/license/ABI contract.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-math-log2-generate.") as temp:
        temp_root = Path(temp)
