...
require_native_linux_x86_64
for tool in ar awk cargo cmp diff grep mkdir mktemp nm objcopy objdump readelf rustup sort timeout; do
    require_tool "$tool"
done
...
selected_object="$work_dir/owner/pthread_getconcurrency.o"
...
(
    ...
    # Rust archive membership follows codegen batching; isolate this emitted
    # function section so unrelated global-assembly leaves cannot widen the
    # one-symbol static candidate.
    objcopy --only-section=.text.pthread_getconcurrency \
        --keep-symbol=pthread_getconcurrency "${members[0]}" \
