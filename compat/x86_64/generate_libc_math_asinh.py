#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``asinh``/``asinhf`` leaf.
...
# asinh{,f} needs musl's log/log1p and square-root reconstruction paths.
# Every provider is renamed and localized, including the log/square-root
# tables and IEEE error helpers, so this leaf selects no public provider ABI.
PUBLIC_SOURCES = (
    "src/math/asinh.c",
    "src/math/asinhf.c",
)
...
PRIVATE_RENAMES = {
    "log": "crabc_x86_math_asinh_provider_log",
    "logf": "crabc_x86_math_asinh_provider_logf",
    "log1p": "crabc_x86_math_asinh_provider_log1p",
    "log1pf": "crabc_x86_math_asinh_provider_log1pf",
    "sqrt": "crabc_x86_math_asinh_provider_sqrt",
    "sqrtf": "crabc_x86_math_asinh_provider_sqrtf",
    "__log_data": "crabc_x86_math_asinh_data_log",
    "__logf_data": "crabc_x86_math_asinh_data_logf",
    "__rsqrt_tab": "crabc_x86_math_asinh_data_rsqrt",
    "__math_invalid": "crabc_x86_math_asinh_helper_invalid",
    "__math_invalidf": "crabc_x86_math_asinh_helper_invalidf",
    "__math_divzero": "crabc_x86_math_asinh_helper_divzero",
    "__math_divzerof": "crabc_x86_math_asinh_helper_divzerof",
}
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_asinh_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_asinh_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_asinh_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_asinh_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 asinh/asinhf translation.",
        " * Generated by compat/x86_64/generate_libc_math_asinh.py; see",
        " * math_asinh.rs for the source/license/ABI contract.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-asinh-") as temporary:
