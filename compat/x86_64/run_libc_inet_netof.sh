#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc classful IPv4 netof evidence.
#
...
# object, never from libc.a. Musl keeps inet_netof beside inet_network,
# inet_makeaddr, and inet_lnaof in inet_legacy.c; this leaf selects exactly
# the local raw-word network-part extraction, not inet_network's inet_addr
# dependency or any resolver, DNS, network-database, interface, or socket
# behavior.
set -euo pipefail
export LC_ALL=C
...
fail() {
    printf 'ERROR: x86 static libc classful IPv4 netof: %s\n' "$*" >&2
    exit 1
}
...
extract_selected_member() {
    ...
    [ "${#matches[@]}" = 1 ] || fail "inet_netof must have exactly one selected archive member"
...
candidate="$work_dir/crabc-static-inet-netof-candidate"
...
selected_members="$work_dir/selected-inet-netof-members"
selected_member_names="$work_dir/selected-inet-netof-member-names"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_inet_netof_probe.c \
    -o "$reference"
if env -i LC_ALL=C TZ=UTC "$reference"; then
    :
else
    status=$?
    fail "pinned-musl classful IPv4 netof fixture exited ${status}"
fi
...
grep -Eq '[[:space:]][TW][[:space:]]inet_netof$' "$archive_symbols" ||
    fail "archive does not define inet_netof"
selected_member="$(extract_selected_member "$archive" "$selected_members" \
    "$selected_member_names")"
[ -f "$selected_member" ] || fail "selected classful IPv4 netof member is missing"

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_INET_NETOF_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie \
    -ffreestanding -fno-builtin -fno-stack-protector -Wl,-e,_start \
    -Wl,--gc-sections -Wl,--no-undefined compat/x86_64/libc_inet_netof_probe.c \
    compat/x86_64/libc_inet_netof_start.S "$selected_member" -o "$candidate"

readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --program-headers --wide "$candidate" >"$candidate_program_headers"
readelf --dynamic --wide "$candidate" >"$candidate_dynamic" || true
readelf --relocs --wide "$candidate" >"$candidate_relocations"
objdump -d --disassemble=inet_netof "$candidate" >"$candidate_disassembly"
grep -Eq '[[:space:]]inet_netof$' "$candidate_symbols" ||
    fail "archive-free candidate does not define inet_netof"
...
if grep -Eq '\b(call|syscall)\b' "$candidate_disassembly"; then
    fail "classful IPv4 netof implementation calls an unselected runtime boundary"
fi
for unselected in htonl htons ntohl ntohs __inet_aton inet_addr inet_aton \
    ...
    __h_errno_location h_errno hstrerror freeaddrinfo gai_strerror \
...
if env -i LC_ALL=C TZ=UTC "$candidate"; then
    ...
    status=$?
    fail "archive-free classful IPv4 netof fixture exited ${status}"
fi

printf 'x86 static crabc-libc classful IPv4 netof: PASS\n'
