...
fail() {
    printf 'ERROR: x86 static libc splice: %s\n' "$*" >&2
    exit 1
}
...
assert_splice_syscall_path() {
    objdump -d --disassemble=splice "$candidate" >"$splice_disassembly"
    grep -Eq '\$0x113,%(e|r)ax' "$splice_disassembly" ||
        fail "splice lacks Linux syscall 275"
    grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$splice_disassembly" ||
        fail "splice lacks a direct syscall"
}
...
assert_candidate_excludes_descriptor_policy() {
    ...
    for symbol in open openat close read write lseek pread pwrite fsync \
        fdatasync sync syncfs fcntl dup dup2 dup3 pipe pipe2 fallocate \
        posix_fallocate posix_fadvise readahead sendfile tee vmsplice \
        copy_file_range sync_file_range; do
...
candidate="$work_dir/crabc-static-splice-candidate"
...
candidate_disassembly="$work_dir/candidate-disassembly"; splice_disassembly="$work_dir/splice-disassembly"
cd "$ROOT_DIR"
...
grep -Eq '[[:space:]][TW][[:space:]]splice$' "$archive_symbols" ||
    fail "archive does not define splice"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_SPLICE_FREESTANDING \
    -I "$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_splice_probe.c compat/x86_64/libc_splice_start.S \
    "$archive" -o "$candidate"
assert_static_closure
for symbol in __errno_location splice; do
