#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ <unistd.h> sysconf declaration gate.
#
...
# host-libc leakage. `sysconf` is unconditional in musl's Linux <unistd.h>;
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/sysconf_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/sysconf_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 unistd.h sysconf ABI: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-sysconf-header.XXXXXX)"
    ...
    for language in c cxx; do
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                ...
                object="$work_dir/${variant}-${label}-sysconf.o"
                ...
                grep -Eq '[[:space:]]sysconf$' <<<"$undefined" ||
                    fail "C++ probe does not retain C linkage for sysconf (${variant}, ${label})"
                if grep -Eq '_Z[0-9].*sysconf' <<<"$undefined"; then
                    fail "C++ probe retained a mangled sysconf reference (${variant}, ${label})"
                fi
            fi
        done
    done
...
printf 'x86 pinned-musl/project C/C++ <unistd.h> sysconf ABI: PASS\n'
