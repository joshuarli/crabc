/*
 * Static Linux/x86-64 cos/cosf C ABI differential regression.
 *
 ...
 * inputs. It selects only binary64/binary32 cosine: cosl, sin/tan public
 * surface, fenv policy, special math, and general libm remain outside this
 * leaf.
 */
...
#include <stddef.h>
#include <stdint.h>
#ifndef CRABC_MATH_COS_FREESTANDING
#include <unistd.h>
#endif
...
#define COS_RECORD_STORAGE_WORDS (COS_RECORD_COUNT * COS_RECORD_WORDS)
...
static int record_binary64(size_t *cursor, int rounding_mode, uint64_t input)
{
	...
	if (fesetround(rounding_mode) != 0 || feclearexcept(FE_ALL_EXCEPT) != 0)
		return 1;
	result = direct_cos(double_from_bits(input));
	if (*cursor + COS_RECORD_WORDS > COS_RECORD_STORAGE_WORDS)
		return 2;
	crabc_x86_64_math_cos_records[(*cursor)++] = input;
	crabc_x86_64_math_cos_records[(*cursor)++] = double_bits(result);
...
static int record_binary32(size_t *cursor, int rounding_mode, uint32_t input)
{
	...
	if (fesetround(rounding_mode) != 0 || feclearexcept(FE_ALL_EXCEPT) != 0)
		return 1;
	result = direct_cosf(float_from_bits(input));
	if (*cursor + COS_RECORD_WORDS > COS_RECORD_STORAGE_WORDS)
		return 2;
	crabc_x86_64_math_cos_records[(*cursor)++] =
		UINT64_C(0x0000000100000000) | input;
	crabc_x86_64_math_cos_records[(*cursor)++] = float_bits(result);
	crabc_x86_64_math_cos_records[(*cursor)++] =
		((uint64_t)(uint32_t)rounding_mode << 32) |
		(uint32_t)fegetround();
	crabc_x86_64_math_cos_records[(*cursor)++] =
...
int crabc_x86_64_math_cos_probe(void)
{
	fenv_t original;
	size_t cursor = 0;
	size_t input_index;
	...
	if (fegetenv(&original) != 0 || fesetenv(FE_DFL_ENV) != 0)
		return 1;
	for (mode_index = 0; mode_index < COS_ROUNDING_CASES && status == 0;
		mode_index++) {
...
int main(void)
{
	int status = crabc_x86_64_math_cos_probe();

	if (status != 0)
		return status;
	return write_all(crabc_x86_64_math_cos_records,
		sizeof(crabc_x86_64_math_cos_records));
}
#endif
