#!/usr/bin/env bash
# Installed _Fork behavior against pinned musl 1.2.6.
#
...
# scenarios retain raw stdout, stderr, and process status before their
...
readonly probe="$ROOT/compat/x86_64/owned_underscore_fork_probe.c"
...
fail() {
    printf 'owned underscore fork: %s\n' "$*" >&2
    exit 1
}
...
if not temporary.is_dir() or temporary.resolve() != temporary or not temporary.is_relative_to(root / '.work'):
    raise SystemExit('underscore-fork TMPDIR must be a physical checkout .work directory')
for product, name in ((static_product, 'static'), (dynamic_product, 'dynamic')):
    if product and (not product.is_dir() or not product.is_relative_to(root / '.work')):
        raise SystemExit(f'underscore-fork {name} product must be a checkout .work directory')
PY
...
try:
    if family == 'static':
        ...
        raise SystemExit(f'underscore-fork has an unknown product family: {family}')
except ProductEvidenceError as error:
    raise SystemExit(f'underscore-fork {family} product payload is invalid: {error}') from error
PY
}
...
readonly work="$(mktemp -d "$TMPDIR/owned-underscore-fork.XXXXXX")"
chmod a+rx "$work"
printf 'underscore-fork evidence: %s\n' "$work"
...
try:
    ...
    raise SystemExit(f'underscore-fork sealed link evidence: {error}') from error
...
if not expected_linkages:
    raise SystemExit('retained underscore-fork link identities have no expected modes')
records = {}
...
if set(records) != expected_linkages:
    raise SystemExit('retained underscore-fork link identities omit a product mode')
Path(sys.argv[1]).write_text(
    json.dumps(
        {
            'schema': 'crabc.x86_64-owned-underscore-fork-link-identities/v1',
...
try:
    ...
    raise SystemExit(f'underscore-fork installed-driver dependency output is invalid: {error}') from error
if not dependencies:
    raise SystemExit('underscore-fork installed-driver dependency output is empty')
dependency_paths = []
for name in dependencies:
    path = Path(name).resolve(strict=True)
    if path != source_path and not path.is_relative_to(headers):
        raise SystemExit(f'underscore-fork dependency escaped the installed headers: {path}')
    dependency_paths.append(path)
if source_path not in dependency_paths:
    raise SystemExit('underscore-fork dependency output omits the workload source')

def digest(path):
    return hashlib.sha256(Path(path).read_bytes()).hexdigest()

record = {
    'schema': 'crabc.x86_64-owned-underscore-fork-compile/v1',
...
for scenario in ordinary errors signal; do
...
printf 'owned underscore fork: PASS (one installed object through pinned musl; %s; main/worker/signal _Fork, callbacks, TLS, exec, masks, syscall-error rollback; raw status/stdout/stderr and sealed link identities retained); evidence: %s\n' \
    "$matrix" "$work"
