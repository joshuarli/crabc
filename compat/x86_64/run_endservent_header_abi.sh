#!/usr/bin/env bash
# Native Linux/x86-64 legacy service-terminator C/C++ declaration gate.
#
...
# ambient libc cannot mask a declaration or C++ linkage mismatch. endservent
# is unconditional in musl's netdb.h; this checks strict, POSIX, X/Open, and
# GNU C/C++ profiles without selecting service enumeration, database, or
# resolver behavior.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/endservent_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/endservent_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 endservent headers: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-endservent-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional endservent declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <netdb.h> endservent ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
