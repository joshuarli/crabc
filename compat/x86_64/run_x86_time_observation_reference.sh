...
fail() {
    printf 'ERROR: x86 time observation reference: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-time-observation.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
probe="$work_dir/x86-time-observation-reference"
...
printf 'x86 pinned-musl realtime observation reference: PASS\n'
