...
class AArch64ParityInventoryTests(unittest.TestCase):
    ...
    def test_sleep_wrapper_remains_a_private_posix_runtime_artifact(self) -> None:
        ...
        self.assertEqual(posix_runtime["verified_artifact_count"], 98)
        self.assertIn(
    ...
    def test_fchdir_remains_a_private_posix_runtime_artifact(self) -> None:
        report = inventory.validate_inventory()
        posix_runtime = next(
            row for row in report["families"] if row["id"] == "libc.posix-runtime"
        )
    ...
    def test_sched_get_priority_max_remains_a_private_posix_runtime_artifact(
        self,
    ) -> None:
        report = inventory.validate_inventory()
        posix_runtime = next(
        ...
        self.assertIn(
            {
                ...
                "id": "static-c-sched-get-priority-max",
            },
            report["selected_private_artifacts"],
        )
        self.assertFalse(report["x86_boundary"]["promotion_ready"])
    ...
    def test_sched_get_priority_min_remains_a_private_posix_runtime_artifact(
        self,
    ) -> None:
        report = inventory.validate_inventory()
        posix_runtime = next(
        ...
        self.assertIn(
            {
                "family": "libc.posix-runtime",
                "id": "static-c-sched-get-priority-min",
            },
            report["selected_private_artifacts"],
        )
        self.assertFalse(report["x86_boundary"]["promotion_ready"])
