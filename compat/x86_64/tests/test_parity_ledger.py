...
class X86ParityLedgerTests(unittest.TestCase):
    ...
    def test_float_parse_artifact_and_capability_slice_stay_closed(self) -> None:
        ...
        assert isinstance(artifacts, list) and len(artifacts) == 49
        artifact = next(
            entry
            for entry in artifacts
    ...
    def test_stdio_permanent_freading_stdin_remains_a_closed_non_capability_artifact(
        ...
        for phrase in (
            ...
            "before dereference",
            "stdio_ext.h",
            "separately selected `__fsetlocking(stdin)`, `__freadable(stdin)`, `__fbufsize(stderr)`, and `__flbf(stderr)` siblings",
            "does not select `stdio.stream-io`",
