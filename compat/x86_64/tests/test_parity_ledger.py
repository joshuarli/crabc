...
class X86ParityLedgerTests(unittest.TestCase):
    ...
    def test_getdtablesize_artifact_keeps_its_nonpromoting_static_contract(self) -> None:
        data = self.data()
        family = self.family(data, "libc.posix-runtime")
        self.assertEqual(family["status"], "planned")
        artifacts = family["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-getdtablesize"
        )
        self.assertNotIn("capabilities", artifact)
        self.assertIn(
            "does not change or promote the broad", artifact["description"]
        ...
        artifact["description"] = artifact["description"].replace(
            "without claiming musl error-path parity", "claiming musl error-path parity"
        )
        with self.assertRaisesRegex(
            ledger.LedgerError, "without claiming musl error-path parity"
        ):
            ledger.validate_ledger(data)
    ...
    def test_fpathconf_artifact_keeps_its_selected_table_contract(self) -> None:
        data = self.data()
        family = self.family(data, "libc.posix-runtime")
        self.assertEqual(family["status"], "planned")
        artifacts = family["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-fpathconf"
        )
        self.assertNotIn("capabilities", artifact)
        for phrase in (
            "src/conf/fpathconf.c",
            "21-entry fixed selector table",
            "valid selectors deliberately ignore the descriptor",
            "negative selectors are explicitly outside this musl differential artifact",
            "only `fpathconf` plus its required `__errno_location`",
            "does not change or promote the broad",
        ...
        self.assertIn("src/conf/fpathconf.c", artifact["oracle"][0]["role"])
        ...
        artifact["capabilities"] = ["filesystem.configuration"]
        with self.assertRaisesRegex(ledger.LedgerError, "must not carry capabilities"):
            ledger.validate_ledger(data)
        ...
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-system-configuration"
        with self.assertRaisesRegex(
            ledger.LedgerError, "closed libc-fpathconf command"
        ):
            ledger.validate_ledger(data)

    def test_pathconf_artifact_keeps_its_delegated_table_contract(self) -> None:
        data = self.data()
        family = self.family(data, "libc.posix-runtime")
        self.assertEqual(family["status"], "planned")
        artifacts = family["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-pathconf"
        )
        self.assertNotIn("capabilities", artifact)
        for phrase in (
            "src/conf/pathconf.c",
        ...
        self.assertIn("src/conf/pathconf.c", artifact["oracle"][0]["role"])
        self.assertIn("src/conf/fpathconf.c", artifact["oracle"][0]["role"])
    ...
    def test_intrusive_queue_artifact_keeps_insque_and_remque_atomic(self) -> None:
        data = self.data()
        family = self.family(data, "libc.c-abi-compat")
        self.assertEqual(family["status"], "planned")
        artifacts = family["verified_artifact"]
        ...
        self.assertIn("queue container", artifact["description"])
        self.assertIn("public x86 support", artifact["description"])
        ...
        evidence[0]["scope"] = evidence[0]["scope"].replace(
            "intentionally stale removed-node links", "removed records"
        )
        with self.assertRaisesRegex(
            ledger.LedgerError,
