...
class X86ParityLedgerTests(unittest.TestCase):
    ...
    def test_math_exp10_remains_a_closed_binary64_alias_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-exp10"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "libc/src/c_abi/x86_64/math_exp10_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_exp10.py",
            "compat/x86_64/math_exp10_header_abi_probe.cpp",
            "compat/x86_64/run_math_exp10_header_abi.sh",
            "compat/x86_64/libc_math_exp10_probe.c",
            "compat/x86_64/libc_math_exp10_start.S",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "GNU binary64 decimal-exponential alias artifact",
            "`exp10`",
            "`pow10`",
            "weak same-address",
            "GCC 15.2.0",
            "raw integral/fractional splitting",
            "exact `[-15, 15]` decimal table",
            "fractional exp2 reconstruction",
            "large-magnitude pow fallback",
            "eleven-source closure",
            "localized closure",
            "no undefined callable symbols",
            "requested/observed rounding directions",
            "compiler-builtins",
            "binary32 `exp10f`/`pow10f`",
            "binary80 `exp10l`/`pow10l`",
            "fenv API/policy",
            "public modf/exp2/pow/fabs functions",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
            "promotion",
        ...
        changed_artifact["description"] = changed_artifact["description"].replace(
            "weak same-address", "same-address"
        )
        with self.assertRaisesRegex(ledger.LedgerError, "weak same-address"):
            ledger.validate_ledger(changed)
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-exp10 command"):
            ledger.validate_ledger(changed)
    ...
    def test_math_log_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-log"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            "docker/Dockerfile.x86_64",
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/math_log.rs",
            "libc/src/c_abi/x86_64/math_log_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_log.py",
            "compat/x86_64/math_log_header_abi_probe.cpp",
            "compat/x86_64/run_math_log_header_abi.sh",
            "compat/x86_64/libc_math_log_probe.c",
            "compat/x86_64/libc_math_log_start.S",
            "compat/x86_64/run_libc_math_log.sh",
            "compat/x86_64/run_libc_text_math_locale_stdio_composition.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/validate_parity_ledger.py",
            "scripts/check_structure.py",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 natural logarithm artifact",
            "`log`",
            "`logf`",
            "GCC 15.2.0 assembly translation",
            "direct source closure",
            "private binary64/binary32 tables and domain helpers",
            "close-to-one directed-zero",
            "all four MXCSR rounding modes",
            "quiet/signaling NaNs",
            "compiler-builtins",
            "`logl`",
            "adjacent log1p/log2/log10/exp/pow functions",
            "fenv API/policy",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-log command"):
            ledger.validate_ledger(changed)
    ...
    def test_math_sin_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-sin"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            "docker/Dockerfile.x86_64",
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/math_sin.rs",
            "libc/src/c_abi/x86_64/math_sin_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_sin.py",
            "compat/x86_64/math_sin_header_abi_probe.cpp",
            "compat/x86_64/run_math_sin_header_abi.sh",
            "compat/x86_64/libc_math_sin_probe.c",
            "compat/x86_64/libc_math_sin_start.S",
            "compat/x86_64/run_libc_math_sin.sh",
            "compat/x86_64/run_libc_text_math_locale_stdio_composition.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/validate_parity_ledger.py",
            "scripts/check_structure.py",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 sine artifact",
            "`sin`",
            "`sinf`",
            "GCC 15.2.0 assembly translation",
            "direct source closure",
            "private fixed kernel, argument-reduction, and floor/scalbn providers",
            "fixed multiword `2/pi` data",
            "all four MXCSR rounding modes",
            "quiet/signaling NaNs",
            "compiler-builtins",
            "`sinl`",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-math-sin"},
        )
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-sin command"):
            ledger.validate_ledger(changed)

    def test_math_tan_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-tan"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            "docker/Dockerfile.x86_64",
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/math_tan.rs",
            "libc/src/c_abi/x86_64/math_tan_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_tan.py",
            "compat/x86_64/math_tan_header_abi_probe.cpp",
            "compat/x86_64/run_math_tan_header_abi.sh",
            "compat/x86_64/libc_math_tan_probe.c",
            "compat/x86_64/libc_math_tan_start.S",
            "compat/x86_64/run_libc_math_tan.sh",
            "compat/x86_64/run_libc_text_math_locale_stdio_composition.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/validate_parity_ledger.py",
            "scripts/check_structure.py",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 tangent artifact",
            "`tan`",
            "`tanf`",
            "GCC 15.2.0 assembly translation",
            "direct source closure",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-tan command"):
            ledger.validate_ledger(changed)

    def test_math_tanh_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-tanh"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            "docker/Dockerfile.x86_64",
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/math_tanh.rs",
            "libc/src/c_abi/x86_64/math_tanh_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_tanh.py",
            "compat/x86_64/math_tanh_header_abi_probe.cpp",
            "compat/x86_64/run_math_tanh_header_abi.sh",
            "compat/x86_64/libc_math_tanh_probe.c",
            "compat/x86_64/libc_math_tanh_start.S",
            "compat/x86_64/run_libc_math_tanh.sh",
            "compat/x86_64/run_libc_text_math_locale_stdio_composition.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/validate_parity_ledger.py",
            "scripts/check_structure.py",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 hyperbolic-tangent artifact",
            "`tanh`",
            "`tanhf`",
            "GCC 15.2.0 assembly translation",
            "direct source closure",
            "private `expm1.c`/`expm1f.c` providers",
            "all four MXCSR rounding modes",
            "quiet/signaling NaNs",
            "compiler-builtins",
            "`tanhl`",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-tanh command"):
            ledger.validate_ledger(changed)

    def test_math_atanh_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-atanh"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            "docker/Dockerfile.x86_64",
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/math_atanh.rs",
            "libc/src/c_abi/x86_64/math_atanh_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_atanh.py",
            "compat/x86_64/math_atanh_header_abi_probe.cpp",
            "compat/x86_64/run_math_atanh_header_abi.sh",
            "compat/x86_64/libc_math_atanh_probe.c",
            "compat/x86_64/libc_math_atanh_start.S",
            "compat/x86_64/run_libc_math_atanh.sh",
            "compat/x86_64/run_libc_text_math_locale_stdio_composition.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/validate_parity_ledger.py",
            "scripts/check_structure.py",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 inverse-hyperbolic-tangent artifact",
            "`atanh`",
            "`atanhf`",
            "GCC 15.2.0 assembly translation",
            "direct source closure",
            "private `log1p.c`/`log1pf.c` providers",
            "all four MXCSR rounding modes",
            "quiet/signaling NaNs",
            "compiler-builtins",
            "`atanhl`",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-atanh command"):
            ledger.validate_ledger(changed)

    def test_math_acosh_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-acosh"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-acosh command"):
            ledger.validate_ledger(changed)

    def test_math_sincos_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-sincos"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            "docker/Dockerfile.x86_64",
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/math_sincos.rs",
            "libc/src/c_abi/x86_64/math_sincos_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_sincos.py",
            "compat/x86_64/math_sincos_header_abi_probe.cpp",
            "compat/x86_64/run_math_sincos_header_abi.sh",
            "compat/x86_64/libc_math_sincos_probe.c",
            "compat/x86_64/libc_math_sincos_start.S",
            "compat/x86_64/run_libc_math_sincos.sh",
            "compat/x86_64/run_libc_text_math_locale_stdio_composition.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/validate_parity_ledger.py",
            "scripts/check_structure.py",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "GNU binary32/binary64 dual-output trigonometric artifact",
            "`sincos`",
            "`sincosf`",
            "GCC 15.2.0 assembly translation",
            "direct source closure",
            "source-ordered output stores",
            "independent and aliased output pointers",
            "all four MXCSR rounding modes",
            "quiet/signaling NaNs",
            "compiler-builtins",
            "`sincosl`",
            "public sin/cos functions",
            "fenv API/policy",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-sincos command"):
            ledger.validate_ledger(changed)

    def test_math_pow_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-pow"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "compat/x86_64/math_pow_header_abi_probe.cpp",
            "compat/x86_64/run_math_pow_header_abi.sh",
            "compat/x86_64/libc_math_pow_probe.c",
            "compat/x86_64/libc_math_pow_start.S",
            "compat/x86_64/run_libc_math_pow.sh",
            "compat/x86_64/run_libc_text_math_locale_stdio_composition.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/validate_parity_ledger.py",
            "scripts/check_structure.py",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 scalar power artifact",
            "`pow`",
            "`powf`",
            "GCC 15.2.0 assembly translation",
            "direct source closure",
            "signed-integral-exponent rules",
            "signed-zero parity",
            "fixed logarithm/exponential table paths",
            "all four MXCSR rounding modes",
            "quiet/signaling NaNs",
            "compiler-builtins",
            "`powl`",
            "public exp/log/exp2/fabs functions",
            "fenv API/policy",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-pow command"):
            ledger.validate_ledger(changed)
