...
class X86ParityLedgerTests(unittest.TestCase):
    ...
    def test_checked_in_ledger_is_closed_and_not_a_public_support_claim(self) -> None:
        ...
        self.assertEqual(report["verified_artifact_count"], 227)
    ...
    def test_float_parse_artifact_and_capability_slice_stay_closed(self) -> None:
        ...
        assert isinstance(artifacts, list) and len(artifacts) == 45
        artifact = next(
            entry
            for entry in artifacts
    ...
    def test_stdio_permanent_freadable_stdin_remains_a_closed_non_capability_artifact(
        ...
        self.assertEqual(family["status"], "planned")
        artifacts = family["verified_artifact"]
        assert isinstance(artifacts, list) and len(artifacts) ==  44
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict)
