...
class X86ParityLedgerTests(unittest.TestCase):
    ...
    def test_foundations_remain_narrow_and_source_or_artifact_scoped(self) -> None:
        ...
        self.assertIn(
            "Thirty-three separately verified static artifacts", pthread_tls["description"]
        )
        self.assertIn(
    ...
    def test_pthread_tls_artifacts_are_verified_without_promoting_pthread_parity(
        ...
        mutexattr_pshared_query = by_id[
            "static-c-pthread-mutexattr-pshared-query"
        ]
        mutexattr_type_query = by_id["static-c-pthread-mutexattr-type-query"]
        mutexattr_type_setter = by_id["static-c-pthread-mutexattr-type-setter"]
        mutex_prioceiling_query = by_id[
            "static-c-pthread-mutex-prioceiling-query"
        ]
        ...
        self.assertEqual(
            condattr_pshared["native_evidence"][0]["command"],
        ...
        for phrase in (
            ...
            "private/shared 0/1",
            "only bit 31",
            "all low thirty-one raw clock-record bits",
            "raw high bit on get",
            "invalid values preserve the complete word",
            "caller-owned raw record storage",
            "attribute lifecycle function, selecting a condition clock, or calling `pthread_cond_init`",
            "private-condition initializer continues to reject every non-null attribute",
            "not condition initialization, condition waiting, or process-shared condition operation",
            "init/destroy lifecycle",
            "condition clock operations",
            ...
            self.assertIn(phrase, condattr_pshared["description"])
        condattr_pshared_scope = condattr_pshared["native_evidence"][0]["scope"]
        for phrase in (
            "shared bit-31 update while retaining the low raw word",
            "private bit-31 clear while retaining a different low raw word",
            "raw high-bit getter behavior",
            "invalid `2` and `-1` complete-word preservation",
            "no lifecycle, clock, or condition-initialization call",
            "exactly pthread_condattr_setpshared/getpshared",
            "PT_TLS, errno/bootstrap, syscall, helper call",
        ...
        self.assertEqual(
            mutexattr_pshared_query["native_evidence"][0]["command"],
            "./scripts/dev-x86_64.sh libc-pthread-mutexattr-pshared-query",
        )
        for phrase in (
            "still-planned `libc.pthread-tls`",
            "One dependency-free entry",
            "only `pthread_mutexattr_getpshared`",
            "four-byte, four-byte-aligned public `pthread_mutexattr_t` word",
            "`a->__attr / 128U % 2`",
            "`PTHREAD_PROCESS_PRIVATE`/`PTHREAD_PROCESS_SHARED` `0`/`1` vocabulary",
            "without changing the raw word",
            "caller-owned raw record storage",
            "`pthread_mutexattr_setpshared`, an attribute lifecycle function, or any mutex entry",
            "no setter, record-construction, validation, or consumption contract",
            "normal-mutex artifact continues to reject every non-null attribute",
            "not process-shared mutex operation, a cross-process capability claim, or a mutex state machine",
            "the setter, init/destroy lifecycle, type/protocol/prioceiling/robust attributes",
            "mutex initialization/locking/destruction",
            "threads, TCB/TLS ownership, synchronization, cancellation",
            "general pthread/TLS behavior or x86-64 parity",
            "family completion, promotion, or public x86 support",
        ):
            self.assertIn(phrase, mutexattr_pshared_query["description"])
        mutexattr_pshared_query_scope = mutexattr_pshared_query["native_evidence"][0][
            "scope"
        ]
        for phrase in (
            "all-zero and bit-7 set output",
            "noncanonical raw words with bit 7 clear/set",
            "complete caller-word preservation",
            "no setpshared, lifecycle, mutex-initialization, process-sharing, or synchronization call",
            "exactly pthread_mutexattr_getpshared",
            "PT_TLS, errno/bootstrap, syscall, helper call",
            "setpshared, init/destroy lifecycle, every other mutex attribute",
            "process-shared mutex operation",
            "family completion, promotion, and public x86 support",
        ):
            self.assertIn(phrase, mutexattr_pshared_query_scope)
        self.assertEqual(
            mutexattr_type_query["native_evidence"][0]["command"],
            "./scripts/dev-x86_64.sh libc-pthread-mutexattr-type-query",
        )
        for phrase in (
            "still-planned `libc.pthread-tls`",
            "One dependency-free entry",
            "only `pthread_mutexattr_gettype`",
            "four-byte, four-byte-aligned public `pthread_mutexattr_t` word",
            "`a->__attr & 3`",
            "raw `0` through `3`",
            "`PTHREAD_MUTEX_NORMAL`/`PTHREAD_MUTEX_DEFAULT`/`PTHREAD_MUTEX_RECURSIVE`/`PTHREAD_MUTEX_ERRORCHECK` vocabulary",
            ...
            self.assertIn(phrase, mutexattr_type_query["description"])
        mutexattr_type_query_scope = mutexattr_type_query["native_evidence"][0][
            "scope"
        ]
        for phrase in (
            "normal, recursive, error-checking, noncanonical raw low-bit 0/1/2, and raw-3 output",
            "complete caller-word preservation",
            "no settype, lifecycle, mutex-initialization, mutex-type operation, or synchronization call",
            "exactly pthread_mutexattr_gettype",
            "PT_TLS, errno/bootstrap, syscall, helper call",
            "settype, init/destroy lifecycle, every other mutex attribute",
            "recursive/error-checking mutex operation",
            "family completion, promotion, and public x86 support",
        ):
            self.assertIn(phrase, mutexattr_type_query_scope)
        self.assertEqual(
            mutexattr_type_setter["native_evidence"][0]["command"],
        ...
        for phrase in (
            ...
            "This raw setter establishes no initialized record, record-validation, lifecycle, or consumption contract",
            "normal-mutex artifact continues to reject every non-null attribute",
            "not recursive or error-checking mutex operation, a mutex state machine, or a mutex capability claim",
            ...
            self.assertIn(phrase, mutexattr_type_setter["description"])
        mutexattr_type_setter_scope = mutexattr_type_setter["native_evidence"][0][
            "scope"
        ]
        for phrase in (
            ...
            self.assertIn(phrase, mutexattr_type_setter_scope)
        self.assertEqual(
            mutex_prioceiling_query["native_evidence"][0]["command"],
            "./scripts/dev-x86_64.sh libc-pthread-mutex-prioceiling-query",
        )
        for phrase in (
            "still-planned `libc.pthread-tls`",
            "One dependency-free entry",
            "only `pthread_mutex_getprioceiling`",
            "unconditional direct `EINVAL`",
            "does not read either opaque pointer",
            "optional caller ceiling slot",
            "mutex initializer, lock, unlock, destruction, setter, or scheduler API",
            "not priority-protect mutex operation, a priority-ceiling state claim, or a mutex state machine",
            "`pthread_mutex_setprioceiling`",
            "every mutex attribute",
            "mutex initialization/locking/destruction",
            "scheduler policy/priority APIs",
            "threads, TCB/TLS ownership, synchronization, cancellation",
            "general pthread/TLS behavior or x86-64 parity",
            "family completion, promotion, and public x86 support",
        ):
            self.assertIn(phrase, mutex_prioceiling_query["description"])
        mutex_prioceiling_query_scope = mutex_prioceiling_query["native_evidence"][0][
            "scope"
        ]
        for phrase in (
            "direct EINVAL for null and zeroed opaque mutex-pointer forms",
            "null/valid ceiling slots",
            "complete optional-ceiling preservation",
            "no setter, lifecycle, mutex-initialization, mutex-operation, scheduler, or synchronization call",
            "exactly pthread_mutex_getprioceiling",
            "PT_TLS, errno/bootstrap, syscall, helper call",
            "setprioceiling, every mutex attribute",
            "priority-protect mutex operation",
            "family completion, promotion, and public x86 support",
        ):
            self.assertIn(phrase, mutex_prioceiling_query_scope)
        self.assertEqual(
            setconcurrency["native_evidence"][0]["command"],
            "./scripts/dev-x86_64.sh libc-pthread-setconcurrency",
        )
        for phrase in (
            "still-planned `libc.pthread-tls`",
            "One dependency-free entry",
            "only `pthread_setconcurrency(int)`",
            "`EINVAL` for each negative input",
            "zero for the no-op zero input",
            "`EAGAIN` for each positive input",
            "neither stores the input nor publishes through C `errno`",
            "INT_MIN, -1, 0, 1, and INT_MAX",
            "freestanding candidate deliberately owns no TLS",
            "not an accepted concurrency-setting state, a scheduler contract, or a pthread runtime claim",
            ...
            self.assertIn(phrase, setconcurrency["description"])
        setconcurrency_scope = setconcurrency["native_evidence"][0]["scope"]
        for phrase in (
            "EINVAL for INT_MIN/-1",
            "no-op zero request",
            "EAGAIN for 1/INT_MAX",
            "preserved errno on the pinned-musl reference path",
            "freestanding candidate has no errno/TLS owner",
            "exactly pthread_setconcurrency",
            "PT_TLS, errno/bootstrap, syscall, helper call",
            "pthread_getconcurrency, stored concurrency state",
