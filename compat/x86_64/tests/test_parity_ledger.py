...
class X86ParityLedgerTests(unittest.TestCase):
    ...
    def test_math_exp_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-exp"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/fenv.rs",
            "libc/src/c_abi/x86_64/math_exp.rs",
            "libc/src/c_abi/x86_64/math_exp_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_exp.py",
            "compat/x86_64/math_exp_header_abi_probe.cpp",
            "compat/x86_64/libc_math_exp_probe.c",
            "compat/x86_64/libc_math_exp_start.S",
            "compat/x86_64/run_libc_math_exp.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/tests/test_runner.py",
            "compat/x86_64/validate_parity_ledger.py",
            "STATUS.md",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 exponential artifact",
            "`exp`",
            "`expf`",
            "GCC 15.2.0",
            "table argument reduction",
            "close-to-zero rule",
            "specialcase",
            "gradual subnormal",
            "overflow/underflow",
            ...
            "binary80 `expl`",
            "fenv API/policy",
            "exp2/expm1 and all log families",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
            "promotion",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-math-exp"},
        )
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-exp command"):
            ledger.validate_ledger(changed)

    def test_math_cos_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-cos"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/fenv.rs",
            "libc/src/c_abi/x86_64/math_cos.rs",
            "libc/src/c_abi/x86_64/math_cos_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_cos.py",
            "compat/x86_64/math_cos_header_abi_probe.cpp",
            "compat/x86_64/libc_math_cos_probe.c",
            "compat/x86_64/libc_math_cos_start.S",
            "compat/x86_64/run_libc_math_cos.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/tests/test_runner.py",
            "compat/x86_64/validate_parity_ledger.py",
            "STATUS.md",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 cosine artifact",
            "`cos`",
            "`cosf`",
            "GCC 15.2.0",
            "tiny-input inexact behavior",
            "quadrant selection",
            "fixed generic sine/cosine polynomials",
            "fixed multiword `2/pi` data",
            "localized kernel/reduction closure",
            "no undefined callable symbols",
            "requested/observed rounding directions",
            "compiler-builtins",
            "binary80 `cosl`",
            "fenv API/policy",
            "public sin/tan/hyperbolic/inverse-trig/log/exp/pow functions",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
            "promotion",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-math-cos"},
        )
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-cos command"):
            ledger.validate_ledger(changed)

    def test_math_cosh_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-cosh"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/fenv.rs",
            "libc/src/c_abi/x86_64/math_cosh.rs",
            "libc/src/c_abi/x86_64/math_cosh_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_cosh.py",
            "compat/x86_64/math_cosh_header_abi_probe.cpp",
            "compat/x86_64/libc_math_cosh_probe.c",
            "compat/x86_64/libc_math_cosh_start.S",
            "compat/x86_64/run_libc_math_cosh.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/tests/test_runner.py",
            "compat/x86_64/validate_parity_ledger.py",
            "STATUS.md",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 hyperbolic-cosine artifact",
            "`cosh`",
            "`coshf`",
            "GCC 15.2.0",
            "raw magnitude classification",
            "exact tiny-input one behavior",
            "stable expm1 reconstruction",
            "reciprocal exponential reconstruction",
            "overflow-safe exponential scaling",
            "localized exponent closure",
            "no undefined callable symbols",
            "requested/observed rounding directions",
            "compiler-builtins",
            "binary80 `coshl`",
            "fenv API/policy",
            "public sinh/tanh and direct/inverse trigonometry",
            "public exp/expm1/log/pow functions",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
            "promotion",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-math-cosh"},
        )
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-cosh command"):
            ledger.validate_ledger(changed)

    def test_math_asinh_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-asinh"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/fenv.rs",
            "libc/src/c_abi/x86_64/math_asinh.rs",
            "libc/src/c_abi/x86_64/math_asinh_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_asinh.py",
            "compat/x86_64/math_asinh_header_abi_probe.cpp",
            "compat/x86_64/libc_math_asinh_probe.c",
            "compat/x86_64/libc_math_asinh_start.S",
            "compat/x86_64/run_libc_math_asinh.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/tests/test_runner.py",
            "compat/x86_64/validate_parity_ledger.py",
            "STATUS.md",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 inverse-hyperbolic-sine artifact",
            "`asinh`",
            "`asinhf`",
            "GCC 15.2.0",
            "raw sign/magnitude classification",
            "signed tiny-input behavior",
            "log-plus-ln2 large reconstruction",
            "reciprocal-square-root middle reconstruction",
            "log1p near-one reconstruction",
            "final sign restoration",
            "localized log/square-root closure",
            "no undefined callable symbols",
            "requested/observed rounding directions",
            "compiler-builtins",
            "binary80 `asinhl`",
            "fenv API/policy",
            "public acosh/atanh/cosh/sinh/tanh and direct trigonometry",
            "public log/log1p/sqrt/exp/pow functions",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
            "promotion",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-math-asinh"},
        )
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-asinh command"):
            ledger.validate_ledger(changed)

    def test_math_exp10f_remains_a_closed_binary32_alias_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-exp10f"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "libc/src/c_abi/x86_64/math_exp10f_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_exp10f.py",
            "compat/x86_64/math_exp10f_header_abi_probe.cpp",
            "compat/x86_64/libc_math_exp10f_probe.c",
            "compat/x86_64/libc_math_exp10f_start.S",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "GNU binary32 decimal-exponential artifact",
            "`exp10f`",
            "`pow10f`",
            "weak same-address",
            "GCC 15.2.0",
            "raw integral/fractional splitting",
            "exact `[-7, 7]` decimal table",
            "binary32 fractional exp2 reconstruction",
            "binary64 large-magnitude fallback",
            "twelve-source closure",
            "localized closure",
            "no undefined callable symbols",
            "requested/observed rounding directions",
            "compiler-builtins",
            "binary64 `exp10`/`pow10`",
            "binary80 `exp10l`/`pow10l`",
            "fenv API/policy",
            "public modf/exp2/exp/pow functions",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
            "promotion",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-math-exp10f"},
        )
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-exp10f command"):
            ledger.validate_ledger(changed)

    def test_math_sinh_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-sinh"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/fenv.rs",
            "libc/src/c_abi/x86_64/math_sinh.rs",
            "libc/src/c_abi/x86_64/math_sinh_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_sinh.py",
            "compat/x86_64/math_sinh_header_abi_probe.cpp",
            "compat/x86_64/libc_math_sinh_probe.c",
            "compat/x86_64/libc_math_sinh_start.S",
            "compat/x86_64/run_libc_math_sinh.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/tests/test_runner.py",
            "compat/x86_64/validate_parity_ledger.py",
            "STATUS.md",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 hyperbolic-sine artifact",
            "`sinh`",
            "`sinhf`",
            "GCC 15.2.0",
            "raw sign/magnitude classification",
            "signed tiny-input behavior",
            "stable expm1 reconstruction",
            "fixed expm1 polynomial/reduction",
            "overflow-safe exponential scaling",
            "localized exponent closure",
            "no undefined callable symbols",
            "requested/observed rounding directions",
            ...
            "binary80 `sinhl`",
            "fenv API/policy",
            "public cosh/tanh and direct/inverse trigonometry",
            "public exp/expm1/log/pow functions",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-math-sinh"},
        )
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-sinh command"):
            ledger.validate_ledger(changed)
