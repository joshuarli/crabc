...
class X86ParityLedgerTests(unittest.TestCase):
    ...
    def test_checked_in_ledger_is_closed_and_not_a_public_support_claim(self) -> None:
        ...
        self.assertEqual(report["verified_artifact_count"], 255)
    ...
    def test_float_parse_artifact_and_capability_slice_stay_closed(self) -> None:
        ...
        assert isinstance(artifacts, list) and len(artifacts) == 50
        artifact = next(
            entry
            for entry in artifacts
    ...
    def test_stdio_permanent_freading_stdin_remains_a_closed_non_capability_artifact(
        self,
    ) -> None:
        data = self.data()
        family = self.family(data, "libc.text-math-locale-stdio")
        ...
        artifact = next(
            ...
            for entry in artifacts
            if isinstance(entry, dict)
            and entry["id"] == "static-c-stdio-permanent-freading-stdin"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "libc/src/c_abi/x86_64/stdio_standard.rs",
        ...
        for phrase in (
            ...
            "before dereference",
            "stdio_ext.h",
            "separately selected `__fsetlocking(stdin)`, `__fseterr(stdin)`, `__freadable(stdin)`, `__fbufsize(stderr)`, and `__flbf(stderr)` siblings",
            "does not select `stdio.stream-io`",
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-freading-stdin description omits does not select `stdio.stream-io`",
        ):
            ledger.validate_ledger(changed)
    ...
    def test_stdio_permanent_fsetlocking_stdin_remains_a_closed_non_capability_artifact(
        self,
    ) -> None:
        data = self.data()
        family = self.family(data, "libc.text-math-locale-stdio")
        ...
        artifact = next(
            ...
            for entry in artifacts
            if isinstance(entry, dict)
            and entry["id"] == "static-c-stdio-permanent-fsetlocking-stdin"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "libc/src/c_abi/x86_64/stdio_standard.rs",
            "include/stdio_ext.h",
            "compat/x86_64/stdio_permanent_fsetlocking_stdin_header_abi_probe.c",
            "compat/x86_64/stdio_permanent_fsetlocking_stdin_header_abi_probe.cpp",
            "compat/x86_64/run_stdio_permanent_fsetlocking_stdin_header_abi.sh",
            "compat/x86_64/libc_stdio_permanent_fsetlocking_stdin_probe.c",
            "compat/x86_64/libc_stdio_permanent_fsetlocking_stdin_start.S",
            "compat/x86_64/run_libc_stdio_permanent_fsetlocking_stdin.sh",
            "compat/x86_64/run_libc_stdio_permanent_freading_stdin.sh",
            "compat/x86_64/run_libc_stdio_permanent_freadable_stdin.sh",
            "compat/x86_64/run_libc_stdio_permanent_fwritable_stderr.sh",
            "compat/x86_64/run_libc_stdio_permanent_fbufsize_stderr.sh",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-stdio-permanent-fsetlocking-stdin"},
        )
        for phrase in (
            "__fsetlocking(stdin, request)",
            "adds exactly one static C ABI export but no capability",
            "direct and function-pointer calls",
            "FSETLOCKING_QUERY",
            ...
            "neither dereferences FILE nor establishes or changes a lock setting",
            "stdio_ext.h",
            "macro values `0`/`1`/`2`",
            "separately selected `__freading(stdin)`, `__freadable(stdin)`, `__fwritable(stderr)`, `__fbufsize(stderr)`, and `__flbf(stderr)` observations",
            "does not select `stdio.stream-io`",
            "no lock configuration",
            "FLOCK/FUNLOCK",
            "lock-free",
            "input, output, buffering, cursor",
            "general FILE claim",
            "byte/block I/O",
            "multiple streams",
            "general stdio",
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-fsetlocking-stdin description omits does not select `stdio.stream-io`",
        ):
            ledger.validate_ledger(changed)
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-fsetlocking-stdin must use its closed native command",
        ):
            ledger.validate_ledger(changed)

    def test_stdio_permanent_fseterr_stdin_remains_a_closed_non_capability_artifact(
        self,
    ) -> None:
        data = self.data()
        family = self.family(data, "libc.text-math-locale-stdio")
        ...
        artifact = next(
            ...
            for entry in artifacts
            if isinstance(entry, dict)
            and entry["id"] == "static-c-stdio-permanent-fseterr-stdin"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "libc/src/c_abi/x86_64/stdio_standard.rs",
            "include/stdio_ext.h",
            "compat/x86_64/stdio_permanent_fseterr_stdin_header_abi_probe.c",
            "compat/x86_64/stdio_permanent_fseterr_stdin_header_abi_probe.cpp",
            "compat/x86_64/run_stdio_permanent_fseterr_stdin_header_abi.sh",
            "compat/x86_64/libc_stdio_permanent_fseterr_stdin_probe.c",
            "compat/x86_64/libc_stdio_permanent_fseterr_stdin_start.S",
            "compat/x86_64/run_libc_stdio_permanent_fseterr_stdin.sh",
            "compat/x86_64/run_libc_stdio_permanent_fsetlocking_stdin.sh",
            "compat/x86_64/run_libc_stdio_permanent_fwritable_stderr.sh",
            "compat/x86_64/run_libc_stdio_permanent_freadable_stdin.sh",
            "compat/x86_64/static_c_abi_exports.txt",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-stdio-permanent-fseterr-stdin"},
        )
        for phrase in (
            "__fseterr(stdin)",
            "adds exactly one static C ABI export but no capability",
            "direct and function-pointer calls",
            "Existing `ferror(stdin)` begins at zero",
            "becomes nonzero after each selected `__fseterr` call",
            "`clearerr(stdin)`",
            "src/stdio/ext2.c",
            "strong `__fseterr(FILE *)`",
            "`f->flags |= F_ERR`",
            "stdio_ext.h",
            "void (FILE *)",
            "does not select `stdio.stream-io`",
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-fseterr-stdin description omits does not select `stdio.stream-io`",
        ):
            ledger.validate_ledger(changed)
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-fseterr-stdin must use its closed native command",
        ):
            ledger.validate_ledger(changed)

    def test_stdio_permanent_freadable_stdin_remains_a_closed_non_capability_artifact(
        self,
    ) -> None:
        data = self.data()
        family = self.family(data, "libc.text-math-locale-stdio")
        ...
        artifact = next(
            ...
            for entry in artifacts
            if isinstance(entry, dict)
            and entry["id"] == "static-c-stdio-permanent-freadable-stdin"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "libc/src/c_abi/x86_64/stdio_standard.rs",
            "include/stdio_ext.h",
            "compat/x86_64/stdio_permanent_freadable_stdin_header_abi_probe.c",
            "compat/x86_64/stdio_permanent_freadable_stdin_header_abi_probe.cpp",
            "compat/x86_64/run_stdio_permanent_freadable_stdin_header_abi.sh",
            "compat/x86_64/libc_stdio_permanent_freadable_stdin_probe.c",
            "compat/x86_64/libc_stdio_permanent_freadable_stdin_start.S",
            "compat/x86_64/run_libc_stdio_permanent_freadable_stdin.sh",
            "compat/x86_64/static_c_abi_exports.txt",
            "scripts/dev-x86_64.sh",
        ):
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-stdio-permanent-freadable-stdin"},
        )
        for phrase in (
            "__freadable(stdin)",
            "adds exactly one static C ABI export but no capability",
        ...
        artifact["description"] = artifact["description"].replace(
            "stdio.stream-io", "other.stream-io"
        )
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-freadable-stdin description omits does not select `stdio.stream-io`",
        ):
            ledger.validate_ledger(changed)
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-freadable-stdin must use its closed native command",
        ):
            ledger.validate_ledger(changed)

    def test_stdio_permanent_fwritable_stderr_remains_a_closed_non_capability_artifact(
        self,
    ) -> None:
        data = self.data()
        family = self.family(data, "libc.text-math-locale-stdio")
        ...
        artifact = next(
            ...
            for entry in artifacts
            if isinstance(entry, dict)
            and entry["id"] == "static-c-stdio-permanent-fwritable-stderr"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "libc/src/c_abi/x86_64/stdio_standard.rs",
            "include/stdio_ext.h",
            "compat/x86_64/stdio_permanent_fwritable_stderr_header_abi_probe.c",
            "compat/x86_64/stdio_permanent_fwritable_stderr_header_abi_probe.cpp",
            "compat/x86_64/run_stdio_permanent_fwritable_stderr_header_abi.sh",
            "compat/x86_64/libc_stdio_permanent_fwritable_stderr_probe.c",
            "compat/x86_64/libc_stdio_permanent_fwritable_stderr_start.S",
            "compat/x86_64/run_libc_stdio_permanent_fwritable_stderr.sh",
            "compat/x86_64/static_c_abi_exports.txt",
            "scripts/dev-x86_64.sh",
        ):
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-stdio-permanent-fwritable-stderr"},
        )
        for phrase in (
            "__fwritable(stderr)",
            "adds exactly one static C ABI export but no capability",
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-fwritable-stderr description omits does not select `stdio.stream-io`",
        ):
            ledger.validate_ledger(changed)
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-fwritable-stderr must use its closed native command",
        ):
            ledger.validate_ledger(changed)

    def test_stdio_permanent_fbufsize_stderr_remains_a_closed_non_capability_artifact(
        self,
    ) -> None:
        data = self.data()
        family = self.family(data, "libc.text-math-locale-stdio")
        ...
        artifact = next(
            ...
            for entry in artifacts
            if isinstance(entry, dict)
            and entry["id"] == "static-c-stdio-permanent-fbufsize-stderr"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "libc/src/c_abi/x86_64/stdio_standard.rs",
            "include/stdio_ext.h",
            "compat/x86_64/stdio_permanent_fbufsize_stderr_header_abi_probe.c",
            "compat/x86_64/stdio_permanent_fbufsize_stderr_header_abi_probe.cpp",
            "compat/x86_64/run_stdio_permanent_fbufsize_stderr_header_abi.sh",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-stdio-permanent-fbufsize-stderr"},
        )
        for phrase in (
            "__fbufsize(stderr)",
            "adds exactly one static C ABI export but no capability",
            ...
            "separately selected `__fsetlocking(stdin)` sibling",
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-fbufsize-stderr description omits does not select `stdio.stream-io`",
        ):
            ledger.validate_ledger(changed)
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-fbufsize-stderr must use its closed native command",
        ):
            ledger.validate_ledger(changed)

    def test_stdio_permanent_flbf_stderr_remains_a_closed_non_capability_artifact(
        self,
    ) -> None:
        data = self.data()
        family = self.family(data, "libc.text-math-locale-stdio")
        ...
        artifact = next(
            ...
            for entry in artifacts
            if isinstance(entry, dict)
            and entry["id"] == "static-c-stdio-permanent-flbf-stderr"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "libc/src/c_abi/x86_64/stdio_standard.rs",
            "include/stdio_ext.h",
            "compat/x86_64/stdio_permanent_flbf_stderr_header_abi_probe.c",
            "compat/x86_64/stdio_permanent_flbf_stderr_header_abi_probe.cpp",
            "compat/x86_64/run_stdio_permanent_flbf_stderr_header_abi.sh",
            "compat/x86_64/libc_stdio_permanent_flbf_stderr_probe.c",
            "compat/x86_64/libc_stdio_permanent_flbf_stderr_start.S",
            "compat/x86_64/run_libc_stdio_permanent_flbf_stderr.sh",
            "compat/x86_64/run_libc_stdio_permanent_freadable_stdin.sh",
            "compat/x86_64/run_libc_stdio_permanent_fbufsize_stderr.sh",
            "compat/x86_64/static_c_abi_exports.txt",
        ...
        self.assertEqual(
            {evidence["command"] for evidence in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-stdio-permanent-flbf-stderr"},
        )
        for phrase in (
            "__flbf(stderr)",
            "adds exactly one static C ABI export but no capability",
            ...
            "before dereference",
            "stdio_ext.h",
            "separately selected __fsetlocking(stdin), __freadable(stdin), and __fbufsize(stderr) siblings",
            "separately selected __freading(stdin) and __fwritable(stderr) siblings",
            "does not select stdio.stream-io",
            "general FILE line-buffer",
            "__fwriting",
            "FLOCK/FUNLOCK",
            "byte/block I/O",
            "multiple streams",
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-flbf-stderr description omits does not select stdio.stream-io",
        ):
            ledger.validate_ledger(changed)
        ...
        with self.assertRaisesRegex(
            ledger.LedgerError,
            "static-c-stdio-permanent-flbf-stderr must use its closed native command",
        ):
            ledger.validate_ledger(changed)
    ...
    def test_math_exp2_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-exp2"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/fenv.rs",
            "libc/src/c_abi/x86_64/math_exp2.rs",
            "libc/src/c_abi/x86_64/math_exp2_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_exp2.py",
            "compat/x86_64/math_exp2_header_abi_probe.cpp",
            "compat/x86_64/run_math_exp2_header_abi.sh",
            "compat/x86_64/libc_math_exp2_probe.c",
            "compat/x86_64/libc_math_exp2_start.S",
            "compat/x86_64/run_libc_math_exp2.sh",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 base-two exponential artifact",
            "`exp2`",
            "`exp2f`",
            "GCC 15.2.0 assembly translation",
            "private binary64/binary32 tables",
            "local range-scaling helpers",
            "WANT_ROUNDING",
            "all four MXCSR rounding modes",
            "quiet/signaling NaNs",
            "compiler-builtins",
            "`exp2l`",
            "adjacent exp/log/pow functions",
            "fenv API/policy",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-exp2 command"):
            ledger.validate_ledger(changed)

    def test_math_expm1_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-expm1"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            ...
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/fenv.rs",
            "libc/src/c_abi/x86_64/math_expm1.rs",
            "libc/src/c_abi/x86_64/math_expm1_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_expm1.py",
            "compat/x86_64/math_expm1_header_abi_probe.cpp",
            "compat/x86_64/run_math_expm1_header_abi.sh",
            "compat/x86_64/libc_math_expm1_probe.c",
            "compat/x86_64/libc_math_expm1_start.S",
            "compat/x86_64/run_libc_math_expm1.sh",
            "compat/x86_64/run_libc_text_math_locale_stdio_composition.sh",
            "compat/x86_64/aarch64_parity_inventory.py",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/validate_parity_ledger.py",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 exponential-minus-one artifact",
            "`expm1`",
            "`expm1f`",
            "GCC 15.2.0 assembly translation",
            "direct no-call closure",
            "raw-subnormal `FORCE_EVAL`",
            "all four MXCSR rounding modes",
            "quiet/signaling NaNs",
            "compiler-builtins",
            "`expm1l`",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-expm1 command"):
            ledger.validate_ledger(changed)

    def test_math_log10_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        self.assertEqual(text_math["status"], "planned")
        artifacts = text_math["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-math-log10"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "compat/upstreams.toml",
            "docker/Dockerfile.x86_64",
            "libc/src/c_abi/x86_64/static_c_abi.rs",
            "libc/src/c_abi/x86_64/math_log10.rs",
            "libc/src/c_abi/x86_64/math_log10_musl_x86_64.S",
            "compat/x86_64/generate_libc_math_log10.py",
            "compat/x86_64/math_log10_header_abi_probe.cpp",
            "compat/x86_64/run_math_log10_header_abi.sh",
            "compat/x86_64/libc_math_log10_probe.c",
            "compat/x86_64/libc_math_log10_start.S",
            "compat/x86_64/run_libc_math_log10.sh",
            "compat/x86_64/run_libc_text_math_locale_stdio_composition.sh",
            "compat/x86_64/aarch64_parity_inventory.json",
            "compat/x86_64/validate_parity_ledger.py",
            "scripts/check_structure.py",
            ...
            self.assertIn(owner, artifact["source_owners"])
        for phrase in (
            "binary32/binary64 base-ten logarithm artifact",
            "`log10`",
            "`log10f`",
            "GCC 15.2.0 assembly translation",
            "direct no-call closure",
            ...
            "compiler-builtins",
            "`log10l`",
            "adjacent log/exp/pow functions",
            "fenv API/policy",
            "special and complex functions",
            "binary80/x87 math",
            "family completion",
        ...
        assert isinstance(evidence, list) and isinstance(evidence[0], dict)
        evidence[0]["command"] = "./scripts/dev-x86_64.sh libc-fenv"
        with self.assertRaisesRegex(ledger.LedgerError, "closed libc-math-log10 command"):
            ledger.validate_ledger(changed)
    ...
    def test_wcswcs_remains_a_closed_non_capability_artifact(self) -> None:
        data = self.data()
        text_math = self.family(data, "libc.text-math-locale-stdio")
        artifacts = text_math["verified_artifact"]
        assert isinstance(artifacts, list) and len(artifacts) == 50
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-wcswcs"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "libc/src/c_abi/x86_64/static_c_abi.rs",
        ...
        evidence[0]["scope"] = evidence[0]["scope"].replace(
            "no input mutation", "input mutation"
        )
        with self.assertRaisesRegex(
            ledger.LedgerError,
    ...
    def test_sched_cpucount_artifact_keeps_its_bytewise_contract(self) -> None:
        data = self.data()
        artifacts = self.family(data, "libc.posix-runtime")["verified_artifact"]
        assert isinstance(artifacts, list)
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-sched-cpucount"
        )
        artifact["description"] = artifact["description"].replace(
            "CPU_COUNT_S", "CPU_COUNT_NOT_SELECTED"
        )
        with self.assertRaisesRegex(ledger.LedgerError, "CPU_COUNT_S"):
            ledger.validate_ledger(data)
    ...
    def test_linkat_artifact_keeps_its_direct_hard_link_boundary(self) -> None:
        data = self.data()
        family = self.family(data, "libc.posix-runtime")
        self.assertEqual(family["status"], "planned")
        artifacts = family["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-linkat"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "COMPATIBILITY-PROFILE.md",
        ...
        prerequisites = artifact["x86_abi_prerequisites"]
        assert isinstance(prerequisites, list)
        syscall_abi = next(item for item in prerequisites if "linkat=265" in item)
        assert isinstance(syscall_abi, str)
        for phrase in (
            "rdi",
            "rsi",
            ...
            self.assertIn(phrase, syscall_abi)
        source_mapping = next(item for item in prerequisites if "src/unistd/linkat.c" in item)
        assert isinstance(source_mapping, str)
        for phrase in (
        ...
        self.assertIn("CRABC_LINKAT_FREESTANDING", start)
    ...
    def test_lchown_artifact_keeps_its_no_follow_ownership_boundary(self) -> None:
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-lchown"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            "COMPATIBILITY-PROFILE.md",
        ...
        prerequisites = artifact["x86_abi_prerequisites"]
        assert isinstance(prerequisites, list)
        syscall_abi = next(item for item in prerequisites if "lchown=94" in item)
        assert isinstance(syscall_abi, str)
        for phrase in ("rdi", "rsi", "rdx", "-4095", "initial-TLS errno"):
            self.assertIn(phrase, syscall_abi)
        source_mapping = next(item for item in prerequisites if "src/unistd/lchown.c" in item)
        assert isinstance(source_mapping, str)
        for phrase in (
            "#ifdef SYS_lchown",
        ...
        self.assertIn("CRABC_LCHOWN_FREESTANDING", start)
    ...
    def test_in6addr_any_artifact_keeps_its_private_data_boundary(self) -> None:
        ...
        assert isinstance(prerequisites, list)
        data_abi = next(
            item for item in prerequisites if "const struct in6_addr in6addr_any" in item
        )
        ...
        source_mapping = next(
            item for item in prerequisites if "src/network/in6addr_any.c" in item
        )
        assert isinstance(source_mapping, str)
        for phrase in (
            "9fa28ece75d8a2191de7c5bb53bed224c5947417",
        ...
        with self.assertRaisesRegex(ledger.LedgerError, "omits all zero"):
            ledger.validate_ledger(data)
    ...
    def test_inet_netof_artifact_keeps_its_archive_free_private_boundary(self) -> None:
        data = self.data()
        family = self.family(data, "libc.resolver")
        self.assertEqual(family["status"], "planned")
        artifacts = family["verified_artifact"]
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-inet-netof"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
        ...
        self.assertIn("inet_netof", exports)
        self.assertFalse(
            exports
            & {
                "h_errno",
    ...
    def test_dn_skipname_artifact_keeps_its_private_codec_boundary(self) -> None:
        data = self.data()
        family = self.family(data, "libc.resolver")
        self.assertEqual(family["status"], "planned")
        self.assertIn("libc/src/c_abi/x86_64/dn_skipname.rs", family["source_owners"])
        artifacts = family["verified_artifact"]
        assert isinstance(artifacts, list)
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-dn-skipname"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
        ...
        c_abi = next(item for item in prerequisites if "SysV AMD64 LP64" in item)
        assert isinstance(c_abi, str)
        for phrase in ("rdi/rsi", "eax", "source..end byte range in one allocation"):
            self.assertIn(phrase, c_abi)
        source_mapping = next(
            item for item in prerequisites if "src/network/dn_skipname.c" in item
        )
        assert isinstance(source_mapping, str)
        for phrase in (
            "9fa28ece75d8a2191de7c5bb53bed224c5947417",
            "83-byte `dn_skipname.lo`",
            "no undefined references, calls, or syscalls",
            "64 through 191",
        ):
            self.assertIn(phrase, source_mapping)
        ...
        artifact["description"] = artifact["description"].replace(
            "64 through 191", "64-191"
        )
        with self.assertRaisesRegex(ledger.LedgerError, "omits 64 through 191"):
            ledger.validate_ledger(data)
    ...
    def test_dn_expand_artifact_keeps_its_private_codec_boundary(self) -> None:
        data = self.data()
        family = self.family(data, "libc.resolver")
        self.assertEqual(family["status"], "planned")
        self.assertIn("libc/src/c_abi/x86_64/dn_expand.rs", family["source_owners"])
        artifacts = family["verified_artifact"]
        assert isinstance(artifacts, list)
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-dn-expand"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
        ...
        for phrase in (
            ...
            "r8d",
            "eax",
            "base..end message range in one allocation",
        ):
            self.assertIn(phrase, c_abi)
        source_mapping = next(
            item for item in prerequisites if "src/network/dn_expand.c" in item
        )
        assert isinstance(source_mapping, str)
        for phrase in (
            "9fa28ece75d8a2191de7c5bb53bed224c5947417",
            "292-byte `dn_expand.lo`",
            "global hidden `__dn_expand`",
            "weak default `dn_expand`",
            "same address",
            "no undefined code reference, call, or syscall",
            "either top compression bit",
        ):
            self.assertIn(phrase, source_mapping)
        ...
        self.assertIn("high-offset 14-bit pointer following", source_oracle["role"])
        ...
        artifact["description"] = artifact["description"].replace(
            "254 bytes", "bounded output"
        )
        with self.assertRaisesRegex(ledger.LedgerError, "omits 254 bytes"):
            ledger.validate_ledger(data)
    ...
    def test_ns_put16_artifact_keeps_its_private_codec_boundary(self) -> None:
        data = self.data()
        family = self.family(data, "libc.resolver")
        self.assertEqual(family["status"], "planned")
        self.assertIn("libc/src/c_abi/x86_64/ns_put16.rs", family["source_owners"])
        artifacts = family["verified_artifact"]
        assert isinstance(artifacts, list)
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-ns-put16"
        )
        self.assertNotIn("capabilities", artifact)
        for owner in (
            ...
            "compat/x86_64/run_nameser_header_abi.sh",
            "compat/x86_64/static_c_abi_exports.txt",
            "compat/x86_64/libc_ns_put16_probe.c",
            "compat/x86_64/libc_ns_put16_start.S",
            "compat/x86_64/run_libc_ns_put16.sh",
            "compat/x86_64/validate_parity_ledger.py",
            "scripts/dev-x86_64.sh",
            "scripts/check_structure.py",
        ):
        ...
        self.assertEqual(
            {entry["command"] for entry in artifact["native_evidence"]},
            {"./scripts/dev-x86_64.sh libc-ns-put16"},
        )
        for phrase in (
            "Private native x86 static `ns_put16` caller-owned 16-bit nameserver wire-write C ABI artifact",
            "still-planned `libc.resolver`",
            "archive-free true `-nostdlib -static` candidate",
            "exactly one extracted crabc object",
            "never `libc.a`",
            "C `unsigned`'s low 16 bits",
            "NS_PUT16",
            "`/etc/hosts`",
            "`/etc/resolv.conf`",
            "DNS packet I/O",
        ...
        for phrase in (
            ...
            "rsi",
            "returns void",
            "at least two writable bytes in one allocation",
            "no alignment",
    ...
    def test_getloadavg_artifact_stays_a_private_load_snapshot_leaf(self) -> None:
        ...
        artifact = next(
            entry
            for entry in artifacts
            if isinstance(entry, dict) and entry["id"] == "static-c-getloadavg"
        )
        self.assertNotIn("capabilities", artifact)
        self.assertEqual(
            {entry["command"] for entry in artifact["native_evidence"]},
