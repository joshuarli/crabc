...
COMMANDS = (
    ...
    "owned-dynamic-io-cancellation", "project-header-extension-policy", "owned-locale", "owned-wordexp", "owned-stdio", "owned-aio",
)
        ...
        for command in COMMANDS:
            for arguments in ([], [str(self.dynamic)], ["--static-sysroot", str(self.static)]):
                with self.subTest(command=command, arguments=arguments):
                    ...
                    required_missing = (command == "owned-pthread-signal" and arguments != [str(self.dynamic)]) or (command in ("owned-locale", "owned-wordexp", "owned-stdio") and len(arguments) == 2)
