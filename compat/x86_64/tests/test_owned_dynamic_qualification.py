...
class OwnedDynamicQualificationTests(unittest.TestCase):
    ...
    def test_process_control_case_stays_bound_to_its_residual_same_object_runner(self):
        self.assertEqual(
            qualification.CASES["process-control"],
    ...
    def test_each_product_requires_runtime_regressions(self):
        for product in qualification.PRODUCTS:
            for case in ("mimalloc-startup-errno", "stdio-allocator-interposition", "c-allocation-interposition", "pthread-cpuclock", "signal-handler-fork", "wide-calendar", "regex", "monetary", "fmtmsg", "underscore-fork", "utmpx", "account-files", "locale", "wordexp", "stdio", "aio", "numeric-calendar", "rand"):
