...
class X86_64CoreRunnerTests(unittest.TestCase):
    ...
    def test_math_acosh_runner_keeps_the_binary32_binary64_static_boundary(self) -> None:
        dispatcher = RUNNER.read_text(encoding="utf-8")
        runner = (ROOT / "compat" / "x86_64" / "run_libc_math_acosh.sh").read_text(
            encoding="utf-8"
        )
        probe = (
            ROOT / "compat" / "x86_64" / "libc_math_acosh_probe.c"
        ).read_text(encoding="utf-8")
        header = (
            ROOT / "compat" / "x86_64" / "math_acosh_header_abi_probe.cpp"
        ).read_text(encoding="utf-8")
        header_runner = (
        ...
        for required in (
            ...
            self.assertIn(required, probe)
        for required in ("double_unary", "float_unary", "direct_acosh", "direct_acoshf"):
            self.assertIn(required, header)
        for required in (
            "math_acosh_header_abi_probe.cpp",
            "-mfpmath=387",
            "unmangled",
        ):
            self.assertIn(required, header_runner)
        ...
        for required in (
            ...
            "\t.globl\tacosh\n",
            "\t.globl\tacoshf\n",
            "\t.local crabc_x86_math_acosh_provider_log",
            "\t.local crabc_x86_math_acosh_provider_log_data",
            "\t.local crabc_x86_math_acosh_provider_rsqrt_tab",
            "\t.local crabc_x86_math_acosh_provider_invalidf",
            "cvtsd2ss",
            "cvtss2sd",
        ):
            self.assertIn(required, assembly)

    def test_math_sincos_runner_keeps_the_binary32_binary64_static_boundary(self) -> None:
        dispatcher = RUNNER.read_text(encoding="utf-8")
        runner = (ROOT / "compat" / "x86_64" / "run_libc_math_sincos.sh").read_text(
            encoding="utf-8"
        )
        probe = (
            ROOT / "compat" / "x86_64" / "libc_math_sincos_probe.c"
        ).read_text(encoding="utf-8")
        header = (
            ROOT / "compat" / "x86_64" / "math_sincos_header_abi_probe.cpp"
        ).read_text(encoding="utf-8")
        header_runner = (
            ROOT / "compat" / "x86_64" / "run_math_sincos_header_abi.sh"
        ).read_text(encoding="utf-8")
        leaf = (
            ROOT / "libc" / "src" / "c_abi" / "x86_64" / "math_sincos.rs"
        ).read_text(encoding="utf-8")
        assembly = (
            ROOT / "libc" / "src" / "c_abi" / "x86_64" / "math_sincos_musl_x86_64.S"
        ).read_text(encoding="utf-8")
        generator = (
            ROOT / "compat" / "x86_64" / "generate_libc_math_sincos.py"
        ).read_text(encoding="utf-8")
        ...
        for required in (
            ...
            "/workspace/compat/x86_64/run_libc_math_sincos.sh",
        ):
            self.assertIn(required, dispatcher)
        for required in (
            "-nostdlib -static",
        ...
        for required in (
            "direct_sincos",
            "direct_sincosf",
            "SINCOS_RECORD_WORDS 5",
            "SINCOS_RECORDS_PER_CALL 2",
            "SINCOS_F64_ALIAS_TAG",
            "SINCOS_F32_ALIAS_TAG",
            "binary64_inputs",
            "binary32_inputs",
            "FE_TONEAREST",
            "FE_DOWNWARD",
            ...
            self.assertIn(required, probe)
        for required in ("double_sincos", "float_sincos", "direct_sincos", "direct_sincosf"):
            self.assertIn(required, header)
        for required in (
            "math_sincos_header_abi_probe.cpp",
            "-mfpmath=387",
            "unmangled",
        ):
            self.assertIn(required, header_runner)
        ...
        for required in (
            ...
            "\t.globl\tsincos\n",
            "\t.globl\tsincosf\n",
            "\t.local crabc_x86_math_sincos_kernel_sin",
            "\t.local crabc_x86_math_sincos_reduce_pio2_large",
            "\t.local crabc_x86_math_sincos_provider_scalbn",
            "cvtsd2ss",
            "cvtss2sd",
        ):
            self.assertIn(required, assembly)
