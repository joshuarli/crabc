...
class X86_64CoreRunnerTests(unittest.TestCase):
    ...
    def test_libc_static_c_abi_readlinkat_artifact_stays_bounded(self) -> None:
        static_root = (
            ROOT / "libc" / "src" / "c_abi" / "x86_64" / "static_c_abi.rs"
        ).read_text(encoding="utf-8")
        implementation = (
        ...
        header_c_probe = (
            ROOT / "compat" / "x86_64" / "readlinkat_header_abi_probe.c"
        ).read_text(encoding="utf-8")
        header_cxx_probe = (
            ROOT / "compat" / "x86_64" / "readlinkat_header_abi_probe.cpp"
        ).read_text(encoding="utf-8")
        header_runner = (
        ...
        start = (
            ROOT / "compat" / "x86_64" / "libc_readlinkat_start.S"
        ).read_text(encoding="utf-8")
        artifact_runner = (
        ...
        self.assertIn('#[path = "readlinkat.rs"]', static_root)
        for required in (
        ...
        for required in (
            ...
            "readlinkat=267",
            "%r10",
            "for symbol in __errno_location __crabc_x86_static_tls_bootstrap readlinkat",
            "readlink|fchdir|linkat|symlinkat|unlinkat|renameat|renameat2|open_by_handle_at",
            "unowned runtime dependency",
        ):
            self.assertIn(required, artifact_runner)
        self.assertNotIn("--whole-archive", artifact_runner)

        self.assertIn("readlinkat", static_export_names)
        self.assertFalse(
            static_export_names
            & {
                "fchdir",
        ...
        self.assertIn('id = "static-c-readlinkat"', parity_ledger)
        self.assertIn(
            'command = "./scripts/dev-x86_64.sh libc-readlinkat"',
            parity_ledger,
        )
        self.assertIn("run_readlinkat_header_abi()", runner)
        self.assertIn(
            "/workspace/compat/x86_64/run_readlinkat_header_abi.sh",
            runner,
        )
        self.assertIn("run_libc_readlinkat_probe()", runner)
        self.assertIn(
            "/workspace/compat/x86_64/run_libc_readlinkat.sh",
            runner,
        )
        self.assertIn(
