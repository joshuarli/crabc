...
class X86_64CoreRunnerTests(unittest.TestCase):
    ...
    def test_script_is_valid_and_has_a_closed_command_set(self) -> None:
        ...
        self.assertIn("libc-pthread-mutexattr-protocol-query", source)
    ...
    def test_libc_static_c_abi_pthread_condattr_pshared_stays_record_only(
        self,
    ) -> None:
        """Keep condattr pshared as an unconsumed raw bit-record pair."""
        ...
        pshared = (
            ...
            / "c_abi"
            / "x86_64"
            / "pthread_condattr_pshared.rs"
        ).read_text(encoding="utf-8")
        probe_path = (
            ROOT
            / "compat"
            / "x86_64"
            / "libc_pthread_condattr_pshared_probe.c"
        )
        start_path = (
            ROOT
            / "compat"
            / "x86_64"
            / "libc_pthread_condattr_pshared_start.S"
        )
        artifact_runner_path = (
            ROOT
            / "compat"
            / "x86_64"
            / "run_libc_pthread_condattr_pshared.sh"
        )
        c_header_probe = (
            ROOT / "compat" / "x86_64" / "pthread_c11_header_abi_probe.c"
        ).read_text(encoding="utf-8")
        ...
        for path in (probe_path, start_path, artifact_runner_path):
            self.assertTrue(
                path.is_file(), f"missing condattr pshared input: {path}"
            )
        probe = probe_path.read_text(encoding="utf-8")
        start = start_path.read_text(encoding="utf-8")
        artifact_runner = artifact_runner_path.read_text(encoding="utf-8")

        self.assertIn('#[path = "pthread_condattr_pshared.rs"]', static_root)
        for required in (
            ...
            "low thirty-one clock-record bits",
            "no selected condition initializer consumes a record here",
        ...
        for required in (
            "#include <errno.h>",
            "#include <pthread.h>",
            "sizeof(pthread_condattr_t) == 4",
            "PTHREAD_PROCESS_PRIVATE == 0 && PTHREAD_PROCESS_SHARED == 1",
            "pthread_condattr_setpshared",
            "pthread_condattr_getpshared",
            "CRABC_SHARED_COND_ATTRIBUTE_WORD 0x92345678U",
            "CRABC_PRIVATE_COND_ATTRIBUTE_WORD 0x7edcba98U",
            "pthread_condattr_setpshared(&attr, 2) != EINVAL",
            "pthread_condattr_setpshared(&attr, -1) != EINVAL",
            "CRABC_PTHREAD_CONDATTR_PSHARED_FREESTANDING",
        ):
            self.assertIn(required, probe)
        for unselected in (
        ...
        for required in (
            "crabc_x86_64_pthread_condattr_pshared_probe",
            "mov $60, %eax",
            "syscall",
        ):
            self.assertIn(required, start)
        ...
        for header_probe in (c_header_probe, cxx_header_probe):
            for required in (
                "crabc_pthread_condattr_setpshared_signature",
                "crabc_pthread_condattr_getpshared_signature",
                "pthread_condattr_setpshared signature",
                "pthread_condattr_getpshared signature",
            ):
                self.assertIn(required, header_probe)
        self.assertIn("crabc_force_pthread_condattr_setpshared", cxx_header_probe)
        self.assertIn("crabc_force_pthread_condattr_getpshared", cxx_header_probe)
        self.assertIn(
        ...
        self.assertIn('id = "static-c-pthread-condattr-pshared"', parity_ledger)
        self.assertIn(
            'command = "./scripts/dev-x86_64.sh libc-pthread-condattr-pshared"',
            parity_ledger,
        )
        self.assertIn("run_libc_pthread_condattr_pshared_probe()", runner)
        self.assertIn(
            "/workspace/compat/x86_64/run_libc_pthread_condattr_pshared.sh",
            runner,
        )
    ...
    def test_libc_static_c_abi_pthread_mutexattr_robust_query_stays_record_only(
        self,
    ) -> None:
        """Keep the raw mutexattr robustness getter apart from robust mutexes."""
        ...
        query = (
            ...
            / "c_abi"
            / "x86_64"
            / "pthread_mutexattr_robust_query.rs"
        ).read_text(encoding="utf-8")
        probe_path = (
            ROOT
            / "compat"
            / "x86_64"
            / "libc_pthread_mutexattr_robust_query_probe.c"
        )
        start_path = (
            ROOT
            / "compat"
            / "x86_64"
            / "libc_pthread_mutexattr_robust_query_start.S"
        )
        artifact_runner_path = (
            ROOT
            / "compat"
            / "x86_64"
            / "run_libc_pthread_mutexattr_robust_query.sh"
        )
        c_header_probe = (
            ROOT / "compat" / "x86_64" / "pthread_c11_header_abi_probe.c"
        ).read_text(encoding="utf-8")
        ...
        for path in (probe_path, start_path, artifact_runner_path):
            self.assertTrue(path.is_file(), f"missing mutexattr robust-query input: {path}")
        probe = probe_path.read_text(encoding="utf-8")
        start = start_path.read_text(encoding="utf-8")
        artifact_runner = artifact_runner_path.read_text(encoding="utf-8")

        self.assertIn('#[path = "pthread_mutexattr_robust_query.rs"]', static_root)
        for required in (
            "src/thread/pthread_attr_get.c::pthread_mutexattr_getrobust",
            "*robust = a->__attr / 4U % 2",
            "four-byte `pthread_mutexattr_t`",
            "musl's setter probes and caches kernel robust-list support",
            "does not select `pthread_mutexattr_setrobust`",
            "no allocation,",
            "C-`errno`",
        ...
        for required in (
            ...
            "CRABC_ROBUST_CLEAR_WORD 0xfffffffbU",
            "CRABC_ROBUST_SET_WORD 0xfffffff4U",
            "CRABC_PTHREAD_MUTEXATTR_ROBUST_QUERY_FREESTANDING",
        ):
            self.assertIn(required, probe)
        for unselected in (
            ...
            self.assertNotIn(unselected, probe)
        for required in (
            "crabc_x86_64_pthread_mutexattr_robust_query_probe",
            "mov $60, %eax",
            "syscall",
        ):
            self.assertIn(required, start)
        ...
        for required in (
            ...
            self.assertIn(required, artifact_runner)
        self.assertNotIn("--whole-archive", artifact_runner)
        self.assertIn("pthread_mutexattr_getrobust", static_exports)
        self.assertTrue(
            {
                "pthread_mutexattr_setrobust",
                "pthread_mutexattr_init",
        ...
        for header_probe in (c_header_probe, cxx_header_probe):
            for required in (
                "crabc_pthread_mutexattr_getrobust_signature",
                "pthread_mutexattr_getrobust signature",
            ):
                self.assertIn(required, header_probe)
        self.assertIn("crabc_force_pthread_mutexattr_getrobust", cxx_header_probe)
        self.assertIn("pthread_mutexattr_getrobust", header_runner)
        self.assertIn(
            "pthread_mutexattr_getprotocol|pthread_mutexattr_getrobust|pthread_cond_init",
            header_runner,
        )
        self.assertIn(
            'id = "static-c-pthread-mutexattr-robust-query"', parity_ledger
        )
        self.assertIn(
            'command = "./scripts/dev-x86_64.sh libc-pthread-mutexattr-robust-query"',
            parity_ledger,
        )
        self.assertIn("run_libc_pthread_mutexattr_robust_query_probe()", runner)
        self.assertIn(
            "/workspace/compat/x86_64/run_libc_pthread_mutexattr_robust_query.sh",
            runner,
        )
        self.assertIn("    libc-pthread-mutexattr-robust-query) ;;", runner)
        self.assertIn(
            '    libc-pthread-mutexattr-robust-query)\n        [ "$#" -eq 0 ] || fail "libc-pthread-mutexattr-robust-query takes no arguments"',
            runner,
        )

    def test_libc_static_c_abi_pthread_mutexattr_protocol_query_stays_record_only(
        self,
    ) -> None:
        """Keep the raw mutexattr protocol getter apart from PI mutexes."""
        ...
        query = (
            ...
            / "c_abi"
            / "x86_64"
            / "pthread_mutexattr_protocol_query.rs"
        ).read_text(encoding="utf-8")
        probe_path = (
            ROOT
            / "compat"
            / "x86_64"
            / "libc_pthread_mutexattr_protocol_query_probe.c"
        )
        start_path = (
            ROOT
            / "compat"
            / "x86_64"
            / "libc_pthread_mutexattr_protocol_query_start.S"
        )
        artifact_runner_path = (
            ROOT
            / "compat"
            / "x86_64"
            / "run_libc_pthread_mutexattr_protocol_query.sh"
        )
        c_header_probe = (
            ROOT / "compat" / "x86_64" / "pthread_c11_header_abi_probe.c"
        ).read_text(encoding="utf-8")
        ...
        for path in (probe_path, start_path, artifact_runner_path):
            self.assertTrue(path.is_file(), f"missing mutexattr protocol-query input: {path}")
        probe = probe_path.read_text(encoding="utf-8")
        start = start_path.read_text(encoding="utf-8")
        artifact_runner = artifact_runner_path.read_text(encoding="utf-8")

        self.assertIn('#[path = "pthread_mutexattr_protocol_query.rs"]', static_root)
        for required in (
            "src/thread/pthread_attr_get.c::pthread_mutexattr_getprotocol",
            "*protocol = a->__attr / 8U % 2",
            "four-byte `pthread_mutexattr_t`",
            "`FUTEX_LOCK_PI` support",
            "does not select `pthread_mutexattr_setprotocol`",
            "no allocation,",
            "C-`errno`",
        ...
        for required in (
            "#include <pthread.h>",
            "sizeof(pthread_mutexattr_t) == 4",
            "PTHREAD_PRIO_NONE == 0 && PTHREAD_PRIO_INHERIT == 1",
            "PTHREAD_PRIO_PROTECT == 2",
            "pthread_mutexattr_getprotocol",
            "CRABC_PROTOCOL_CLEAR_WORD 0xfffffff7U",
            "CRABC_PROTOCOL_SET_WORD 0xfffffff8U",
            "CRABC_PTHREAD_MUTEXATTR_PROTOCOL_QUERY_FREESTANDING",
        ):
            self.assertIn(required, probe)
        for unselected in (
            ...
            self.assertNotIn(unselected, probe)
        for required in (
            "crabc_x86_64_pthread_mutexattr_protocol_query_probe",
            "mov $60, %eax",
            "syscall",
        ):
            self.assertIn(required, start)
        ...
        for required in (
            ...
            self.assertIn(required, artifact_runner)
        self.assertNotIn("--whole-archive", artifact_runner)
        self.assertIn("pthread_mutexattr_getprotocol", static_exports)
        self.assertTrue(
            {
                "pthread_mutexattr_setprotocol",
                "pthread_mutexattr_init",
        ...
        for header_probe in (c_header_probe, cxx_header_probe):
            for required in (
                "crabc_pthread_mutexattr_getprotocol_signature",
                "pthread_mutexattr_getprotocol signature",
            ):
                self.assertIn(required, header_probe)
        self.assertIn("crabc_force_pthread_mutexattr_getprotocol", cxx_header_probe)
        self.assertIn("pthread_mutexattr_getprotocol", header_runner)
        self.assertIn(
            "pthread_mutexattr_getprotocol|pthread_mutexattr_getrobust|pthread_cond_init",
            header_runner,
        )
        self.assertIn(
            'id = "static-c-pthread-mutexattr-protocol-query"', parity_ledger
        )
        self.assertIn(
            'command = "./scripts/dev-x86_64.sh libc-pthread-mutexattr-protocol-query"',
            parity_ledger,
        )
        self.assertIn("run_libc_pthread_mutexattr_protocol_query_probe()", runner)
        self.assertIn(
            "/workspace/compat/x86_64/run_libc_pthread_mutexattr_protocol_query.sh",
            runner,
        )
        self.assertIn("    libc-pthread-mutexattr-protocol-query) ;;", runner)
        self.assertIn(
    ...
    def test_libc_static_c_abi_pthread_c11_once_artifact_stays_private_and_exact(
        ...
        for required in (
            ...
            "thrd_create thrd_detach thrd_join thrd_exit thrd_sleep thrd_yield thrd_current thrd_equal",
            "call_once",
            "pthread_mutexattr_getprotocol|pthread_mutexattr_getrobust|pthread_cond_init|pthread_rwlockattr_getpshared|pthread_barrierattr_setpshared|pthread_barrierattr_getpshared|pthread_condattr_setpshared|pthread_condattr_getpshared|pthread_condattr_setclock|pthread_condattr_getclock|pthread_once",
