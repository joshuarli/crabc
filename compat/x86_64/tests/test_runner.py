...
class X86_64CoreRunnerTests(unittest.TestCase):
    ...
    def test_libc_static_c_abi_confstr_artifact_stays_section_isolated(self) -> None:
        """The selected configuration string must not widen configuration/runtime support."""
        static_root = (
            ROOT / "libc" / "src" / "c_abi" / "x86_64" / "static_c_abi.rs"
        ).read_text(encoding="utf-8")
        source_path = (
        ...
        header_c_path = ROOT / "compat" / "x86_64" / "confstr_header_abi_probe.c"
        header_cxx_path = (
        ...
        probe_path = ROOT / "compat" / "x86_64" / "libc_confstr_probe.c"
        start_path = ROOT / "compat" / "x86_64" / "libc_confstr_start.S"
        artifact_runner_path = ROOT / "compat" / "x86_64" / "run_libc_confstr.sh"
        for path in (
            source_path,
            header_c_path,
            header_cxx_path,
            ...
            self.assertTrue(path.is_file(), f"missing confstr artifact input: {path}")
        ...
        for forbidden in ("copy_nonoverlapping", "snprintf(", "raw_syscall", "alloc::"):
            self.assertNotIn(forbidden, confstr_body)
        ...
        for required in (
            ...
            self.assertIn(required, artifact_runner)
        self.assertNotIn("--whole-archive", artifact_runner)
        self.assertIn("confstr", static_exports)
        self.assertIn('id = "static-c-confstr"', parity_ledger)
        self.assertIn(
            'command = "./scripts/dev-x86_64.sh libc-confstr"', parity_ledger
        )
        self.assertIn("run_confstr_header_abi()", runner)
        self.assertIn("run_libc_confstr()", runner)
        self.assertIn(
            "/workspace/compat/x86_64/run_confstr_header_abi.sh", runner
        )
        self.assertIn("/workspace/compat/x86_64/run_libc_confstr.sh", runner)
        self.assertIn(
    ...
    def test_libc_static_c_abi_pathconf_artifact_stays_section_isolated(self) -> None:
        """The selected pathconf wrapper must not widen configuration/runtime support."""
        static_root = (
            ROOT / "libc" / "src" / "c_abi" / "x86_64" / "static_c_abi.rs"
        ).read_text(encoding="utf-8")
        source_path = (
        ...
        header_c_path = ROOT / "compat" / "x86_64" / "pathconf_header_abi_probe.c"
        header_cxx_path = (
        ...
        probe_path = ROOT / "compat" / "x86_64" / "libc_pathconf_probe.c"
        start_path = ROOT / "compat" / "x86_64" / "libc_pathconf_start.S"
        artifact_runner_path = ROOT / "compat" / "x86_64" / "run_libc_pathconf.sh"
        for path in (
            source_path,
            header_c_path,
            header_cxx_path,
            ...
            self.assertTrue(path.is_file(), f"missing pathconf artifact input: {path}")
        ...
        for forbidden in ("raw_syscall", 'pub extern "C" fn fpathconf', "getauxval", "alloc::"):
            self.assertNotIn(forbidden, pathconf_body)

        for header_probe in (header_c, header_cxx):
            for required in ("pathconf_signature", "pathconf", "const char", "long"):
                self.assertIn(required, header_probe)
        for required in (
            "CANDIDATE_CC=/usr/bin/gcc",
            "-nostdinc",
        ...
        for required in (
            ...
            self.assertIn(required, artifact_runner)
        self.assertNotIn("--whole-archive", artifact_runner)
        self.assertIn("pathconf", static_exports)
        self.assertIn('id = "static-c-pathconf"', parity_ledger)
        self.assertIn(
            'command = "./scripts/dev-x86_64.sh libc-pathconf"', parity_ledger
        )
        self.assertIn("run_pathconf_header_abi()", runner)
        self.assertIn("run_libc_pathconf()", runner)
        self.assertIn(
            "/workspace/compat/x86_64/run_pathconf_header_abi.sh", runner
        )
        self.assertIn("/workspace/compat/x86_64/run_libc_pathconf.sh", runner)
        self.assertIn(
    ...
    def test_libc_static_c_abi_sysconf_artifact_stays_section_isolated(self) -> None:
        """The direct sysconf table entries must not promote configuration support."""
        static_root = (
            ROOT / "libc" / "src" / "c_abi" / "x86_64" / "static_c_abi.rs"
        ).read_text(encoding="utf-8")
        source_path = (
        ...
        header_c_path = ROOT / "compat" / "x86_64" / "sysconf_header_abi_probe.c"
        header_cxx_path = (
        ...
        probe_path = ROOT / "compat" / "x86_64" / "libc_sysconf_probe.c"
        start_path = ROOT / "compat" / "x86_64" / "libc_sysconf_start.S"
        artifact_runner_path = ROOT / "compat" / "x86_64" / "run_libc_sysconf.sh"
        for path in (
            source_path,
            header_c_path,
            header_cxx_path,
            ...
            self.assertTrue(path.is_file(), f"missing sysconf artifact input: {path}")
        ...
        for forbidden in (
            ...
            self.assertNotIn(forbidden, sysconf_body)

        for header_probe in (header_c, header_cxx):
            for required in ("sysconf_signature", "sysconf", "int", "long"):
                self.assertIn(required, header_probe)
        for required in (
            "CANDIDATE_CC=/usr/bin/gcc",
            "-nostdinc",
        ...
        for required in (
            ...
            self.assertIn(required, artifact_runner)
        self.assertNotIn("--whole-archive", artifact_runner)
        self.assertIn("sysconf", static_exports)
        self.assertIn('id = "static-c-sysconf"', parity_ledger)
        self.assertIn(
            'command = "./scripts/dev-x86_64.sh libc-sysconf"', parity_ledger
        )
        self.assertIn("run_sysconf_header_abi()", runner)
        self.assertIn("run_libc_sysconf()", runner)
        self.assertIn(
            "/workspace/compat/x86_64/run_sysconf_header_abi.sh", runner
        )
        self.assertIn("/workspace/compat/x86_64/run_libc_sysconf.sh", runner)
        self.assertIn(
