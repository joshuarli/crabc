#!/usr/bin/env python3
"""Behavioral checks for the installed numeric/calendar runner interface."""
...
RUNNER = ROOT / "compat/x86_64/run_owned_numeric_calendar.sh"


class OwnedNumericCalendarTests(unittest.TestCase):
    ...
    def test_paths_reject_symlink_and_parent_components_before_evidence_creation(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="owned-numeric-calendar-path.", dir=scratch) as temporary:
            base = Path(temporary)
            ...
            for argument in (alias, raw_parent, ROOT):
                with self.subTest(argument=argument):
                    ...
                    self.assertIn("owned numeric calendar products dynamic product", result.stderr)
                    self.assertEqual(result.stdout, "")
