#!/usr/bin/env bash
# Native Linux/x86-64 selected mkfifo C/C++ header ABI matrix.
#
# Pinned musl 1.2.6 is the declaration, layout, value, and unmangled C++
# C-linkage oracle. The candidate uses raw GCC with only project headers and compiler
# builtin headers, so an ambient libc cannot hide a header mismatch. This
# proves exactly `mkfifo`, not the mknod family, path policy, or public x86
# support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/mkfifo_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/mkfifo_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 mkfifo headers: %s\n' "$*" >&2
    exit 1
}
...
check_trace() {
    ...
    for header in sys/stat.h sys/types.h; do
...
check_cxx_symbol() {
    ...
    grep -Eq '[[:space:]]mkfifo$' <<<"$undefined" ||
        fail "$tree $profile C++ probe does not retain C linkage for mkfifo"
    if grep -Eq '_Z6mkfifo' <<<"$undefined"; then
        fail "$tree $profile C++ probe retained a mangled mkfifo reference"
    fi
}
...
[ -f "$C_PROBE" ] || fail "missing C mkfifo ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ mkfifo ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-mkfifo-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ mkfifo ABI matrix: PASS (%s profiles; compile-only)\n' \
