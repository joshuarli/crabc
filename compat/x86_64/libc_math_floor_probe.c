/*
 * Static Linux/x86-64 floor/floorf C ABI differential regression.
 *
 ...
 * ambient MXCSR direction. It owns only binary64 `floor` and binary32
 * `floorf`: binary80 `floorl`, ceiling, general fenv policy, and all other
 * libm entries remain outside this artifact.
 */
...
#include <stddef.h>
#include <stdint.h>
#ifndef CRABC_MATH_FLOOR_FREESTANDING
#include <unistd.h>
#endif

#define FLOOR_F64_CASES 27
#define FLOOR_F32_CASES 27
#define FLOOR_ROUNDING_CASES 4
#define FLOOR_RECORD_WORDS 4
#define FLOOR_RECORD_COUNT \
	((FLOOR_F64_CASES + FLOOR_F32_CASES) * FLOOR_ROUNDING_CASES)
#define FLOOR_RECORD_STORAGE_WORDS \
	(FLOOR_RECORD_COUNT * FLOOR_RECORD_WORDS)
...
static double_unary_function volatile direct_floor = (floor);
static float_unary_function volatile direct_floorf = (floorf);

/* The freestanding start object writes these exact 6,912 bytes with syscall. */
uint64_t crabc_x86_64_math_floor_records[FLOOR_RECORD_STORAGE_WORDS];

static const uint64_t binary64_inputs[FLOOR_F64_CASES] = {
...
static const uint32_t binary32_inputs[FLOOR_F32_CASES] = {
...
static const int rounding_modes[FLOOR_ROUNDING_CASES] = {
...
static int record_binary64(size_t *cursor, int rounding_mode, uint64_t input)
{
	...
	if (fesetround(rounding_mode) != 0 || feclearexcept(FE_ALL_EXCEPT) != 0)
		return 1;
	result = direct_floor(double_from_bits(input));
	if (*cursor + FLOOR_RECORD_WORDS > FLOOR_RECORD_STORAGE_WORDS)
		return 2;
	crabc_x86_64_math_floor_records[(*cursor)++] = input;
	crabc_x86_64_math_floor_records[(*cursor)++] = double_bits(result);
	crabc_x86_64_math_floor_records[(*cursor)++] =
...
static int record_binary32(size_t *cursor, int rounding_mode, uint32_t input)
{
	...
	if (fesetround(rounding_mode) != 0 || feclearexcept(FE_ALL_EXCEPT) != 0)
		return 1;
	result = direct_floorf(float_from_bits(input));
	if (*cursor + FLOOR_RECORD_WORDS > FLOOR_RECORD_STORAGE_WORDS)
		return 2;
	crabc_x86_64_math_floor_records[(*cursor)++] =
		UINT64_C(0x0000000100000000) | input;
	crabc_x86_64_math_floor_records[(*cursor)++] = float_bits(result);
	crabc_x86_64_math_floor_records[(*cursor)++] =
		((uint64_t)(uint32_t)rounding_mode << 32) |
		(uint32_t)fegetround();
	crabc_x86_64_math_floor_records[(*cursor)++] =
...
int crabc_x86_64_math_floor_probe(void)
{
	fenv_t original;
	size_t cursor = 0;
	size_t input_index;
	...
	if (fegetenv(&original) != 0 || fesetenv(FE_DFL_ENV) != 0)
		return 1;
	for (mode_index = 0; mode_index < FLOOR_ROUNDING_CASES && status == 0;
		mode_index++) {
		for (input_index = 0; input_index < FLOOR_F64_CASES && status == 0;
			input_index++)
			status = record_binary64(&cursor, rounding_modes[mode_index],
				binary64_inputs[input_index]);
		for (input_index = 0; input_index < FLOOR_F32_CASES && status == 0;
			input_index++)
			status = record_binary32(&cursor, rounding_modes[mode_index],
				binary32_inputs[input_index]);
	}
	if (cursor != FLOOR_RECORD_STORAGE_WORDS && status == 0)
		status = 3;
...
int main(void)
{
	int status = crabc_x86_64_math_floor_probe();

	if (status != 0)
		return status;
	return write_all(crabc_x86_64_math_floor_records,
		sizeof(crabc_x86_64_math_floor_records));
}
#endif
