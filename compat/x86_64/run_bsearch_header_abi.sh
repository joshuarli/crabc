#!/usr/bin/env bash
# Native Linux/x86-64 stdlib bsearch C/C++ declaration gate.
#
# Pinned musl 1.2.6 is the declaration and C-linkage oracle. `bsearch` is
# unconditional in stdlib.h, so strict, POSIX, X/Open, GNU, and BSD profiles
# all retain the same C and C++ function-pointer ABI. Project headers are
...
fail() {
    printf 'ERROR: x86 stdlib bsearch ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/bsearch_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/bsearch_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-bsearch-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-bsearch-cxx.o"
candidate_cxx_object="$work_dir/candidate-bsearch-cxx.o"
...
if ! "$ORACLE_CC" -std=c11 -U_GNU_SOURCE -D__STRICT_ANSI__ -fno-builtin \
    ...
    fail "project C bsearch header contract drifted"
fi
...
printf 'x86 pinned-musl/project C/C++ <stdlib.h> bsearch ABI: PASS\n'
