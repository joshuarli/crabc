#!/usr/bin/env bash
# Native Linux/x86-64 complete musl ether.c C/C++ declaration gate.
#
...
# ambient libc cannot mask declarations, the six-octet layout, or C++ linkage.
# All ether.c declarations are unconditional in musl's netinet/ether.h; this
# checks strict, POSIX, X/Open, GNU, and BSD C plus strict/GNU C++ profiles.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/ether_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/ether_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu c11-bsd cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 ether headers: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-ether-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional ether.c declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <netinet/ether.h> ether.c ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
