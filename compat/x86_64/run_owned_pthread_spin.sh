...
if not temporary.is_dir() or temporary.resolve() != temporary or not temporary.is_relative_to(root / '.work'):
    raise SystemExit('Pthread spin TMPDIR must be a physical checkout .work directory')
if sys.argv[3]:
    product = Path(sys.argv[3]).resolve(strict=True)
    if not product.is_dir() or not product.is_relative_to(root / '.work'):
        raise SystemExit('Pthread spin product must be a checkout .work directory')
PY
readonly work="$(mktemp -d "$TMPDIR/owned-pthread-spin.XXXXXX")"
chmod a+rx "$work"
printf 'Pthread spin evidence: %s\n' "$work"
provided_dynamic="${1:-}"
...
"$installed/bin/crabc-cc-dynamic" --dynamic-pie -std=c11 -fno-builtin \
    -c "$ROOT/compat/x86_64/owned_pthread_spin_probe.c" -o "$work/workload.o"
...
grep -qx owned-pthread-spin-ok "$work/oracle.stdout"
