#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc usleep evidence.
#
...
# microsecond nanosleep adapter: zero/short completion with stale errno and
# deterministic raw-SIGALRM interruption across the one-second normalization
# boundary and UINT_MAX. Fixture-only raw signal setup does not select signal
# actions, masks, process timers, or a C sleep-policy runtime.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc usleep: %s\n' "$*" >&2
    exit 1
}
...
assert_usleep_delegation() {
    local disassembly="$work_dir/usleep-disassembly"

    objdump -d --disassemble=usleep "$candidate" >"$disassembly"
    if grep -Eq '(call|jmp).*<nanosleep>' "$disassembly"; then
        assert_named_syscall nanosleep 23
        return
    fi
    # The optimized static link may inline the selected sibling. In that
    # case, retain a direct check for precisely its nanosleep syscall body.
    grep -Eq '\$0x23(,|[[:space:]]|\$)' "$disassembly" ||
        fail "usleep neither delegates to nor inlines the selected nanosleep seam"
    if ! grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$disassembly"; then
        fail "inlined usleep nanosleep path lacks its syscall instruction"
    fi
}
...
bash "$ROOT_DIR/compat/x86_64/run_usleep_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-usleep.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-usleep-reference"
candidate="$work_dir/crabc-static-usleep-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_usleep_probe.c >/dev/null 2>"$header_trace"
for header in errno.h features.h limits.h signal.h stddef.h stdint.h unistd.h sys/time.h \
    sys/select.h sys/syscall.h bits/alltypes.h bits/signal.h bits/syscall.h; do
    grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
        fail "fixture omitted project $header"
done

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_usleep_probe.c -o "$reference"
"$reference"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap usleep nanosleep; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_USLEEP_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_usleep_probe.c compat/x86_64/libc_usleep_start.S \
    "$archive" -o "$candidate"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap usleep \
    crabc_x86_64_usleep_restorer; do
    grep -Eq "[[:space:]]${symbol}$" "$candidate_symbols" ||
        fail "candidate does not define ${symbol}"
done
for unrelated in sleep alarm ualarm getitimer setitimer timer_create timer_delete \
...
if grep -Eqi 'arch_prctl|mov[[:space:]]+%rsi,[[:space:]]*%fs:0' \
    compat/x86_64/libc_usleep_start.S; then
...
"$candidate"
printf 'x86 static crabc-libc usleep: PASS\n'
