#!/usr/bin/env bash
# Native Linux/x86-64 selected static log2/log2f evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc math log2: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-log2.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-math-log2-reference"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_log2_probe.c >/dev/null 2>"$header_trace"
for header in fenv.h float.h math.h stddef.h stdint.h features.h bits/alltypes.h; do
	grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
		fail "fixture did not use project $header"
done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
	compat/x86_64/libc_math_log2_probe.c -lm -o "$reference"
"$reference" >"$reference_records" || fail "pinned-musl math log2 fixture failed"
[ -s "$reference_records" ] || fail "pinned-musl math log2 emitted no differential records"
...
for mode in sse x87; do
	...
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -c compat/x86_64/math_log2_header_abi_probe.cpp \
		-o "$header_cxx_reference"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" \
		-c compat/x86_64/math_log2_header_abi_probe.cpp -o "$header_cxx_candidate"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only \
		compat/x86_64/math_log2_header_abi_probe.cpp >/dev/null 2>"$cxx_header_trace"
	...
	for object in "$header_cxx_reference" "$header_cxx_candidate"; do
		...
		for symbol in log2 log2f; do
			grep -Eq "[[:space:]]${symbol}$" <<<"$undefined" ||
				fail "C++ ${mode} probe does not retain unmangled ${symbol}"
		done
		if grep -Eq '_Z.*log2' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled log2 reference"
		fi
	done
done
...
for symbol in log2 log2f feclearexcept fegetenv fegetround fesetenv fesetround fetestexcept; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_LOG2_FREESTANDING \
	...
	compat/x86_64/libc_math_log2_probe.c compat/x86_64/libc_math_log2_start.S \
	"$archive" -o "$candidate"
...
for private in __log2_data __log2f_data __math_divzero __math_divzerof __math_invalid __math_invalidf; do
	if grep -Eq "[[:space:]](GLOBAL|WEAK)[[:space:]].*[[:space:]]${private}$" \
		"$candidate_symbols"; then
		fail "candidate exports unlocalized ${private} helper"
	fi
done
# Excludes binary80 log2l log10/log10f and the remaining log/exp/trig families.
for unselected in log logf logl log1p log1pf log1pl log2l log10 log10f log10l \
...
if grep -Eq 'crabc_core|mimalloc|sha_crypt|libm|math_special|math_complex|math_elementary_long_double|math_fmod|math_cbrt|math_ceil|math_floor|math_round|math_minmax' \
	"$candidate_symbols" "$disassembly"; then
	fail "candidate selects an unowned math/runtime dependency"
fi
for instruction in divsd mulsd addsd subsd divss mulss subss; do
	grep -Eq "[[:space:]]${instruction}([[:space:]]|$)" "$disassembly" ||
		fail "candidate lacks ${instruction} musl log2 scalar arithmetic"
done
