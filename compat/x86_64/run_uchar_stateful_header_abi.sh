#!/usr/bin/env bash
# Native Linux/x86-64 C11/C++17 stateful <uchar.h> declaration/linkage gate.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/uchar_stateful_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/uchar_stateful_header_abi_probe.cpp"
readonly -a CXX_SYMBOLS=(c16rtomb mbrtoc16 mbrtoc32)
...
fail() {
    printf 'ERROR: x86 uchar.h stateful ABI: %s\n' "$*" >&2
    exit 1
}
...
[ -f "$C_PROBE" ] || fail "missing C uchar stateful header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ uchar stateful header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-uchar-stateful-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C11/C++17 stateful uchar header ABI: PASS\n'
