#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``sinh``/``sinhf`` leaf.
...
# sinh{,f} needs musl's expm1 and overflow reconstruction paths. Every
...
PRIVATE_RENAMES = {
    "expm1": "crabc_x86_math_sinh_provider_expm1",
    "expm1f": "crabc_x86_math_sinh_provider_expm1f",
    "__expo2": "crabc_x86_math_sinh_provider_expo2",
    "__expo2f": "crabc_x86_math_sinh_provider_expo2f",
    "exp": "crabc_x86_math_sinh_provider_exp",
    "expf": "crabc_x86_math_sinh_provider_expf",
    "__exp_data": "crabc_x86_math_sinh_data_exp",
    "__exp2f_data": "crabc_x86_math_sinh_data_exp2f",
    "__math_oflow": "crabc_x86_math_sinh_helper_oflow",
    "__math_oflowf": "crabc_x86_math_sinh_helper_oflowf",
    "__math_uflow": "crabc_x86_math_sinh_helper_uflow",
    "__math_uflowf": "crabc_x86_math_sinh_helper_uflowf",
    "__math_xflow": "crabc_x86_math_sinh_helper_xflow",
    "__math_xflowf": "crabc_x86_math_sinh_helper_xflowf",
}
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_sinh_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_sinh_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_sinh_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_sinh_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 sinh/sinhf translation.",
        " * Generated by compat/x86_64/generate_libc_math_sinh.py; see",
        " * math_sinh.rs for the source/license/ABI contract.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-sinh-") as temporary:
