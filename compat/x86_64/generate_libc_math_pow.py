#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``pow``/``powf`` leaf.
...
from math_scalar_corrections import corrected_source
...
# The public binary64/binary32 entries use their own fixed logarithm and
# exponential tables plus typed exceptional-result helpers. The local fabs
# source keeps this leaf independent from the separately selected bit-sign ABI.
PUBLIC_SOURCES = (
    "src/math/pow.c",
    "src/math/powf.c",
)
...
PRIVATE_RENAMES = {
    "__exp_data": "crabc_x86_math_pow_provider_exp_data",
    "__pow_log_data": "crabc_x86_math_pow_provider_log_data",
    "__exp2f_data": "crabc_x86_math_pow_provider_exp2f_data",
    "__powf_log2_data": "crabc_x86_math_pow_provider_log2f_data",
    "__math_invalid": "crabc_x86_math_pow_provider_invalid",
    "__math_invalidf": "crabc_x86_math_pow_provider_invalidf",
    "__math_oflow": "crabc_x86_math_pow_provider_oflow",
    "__math_oflowf": "crabc_x86_math_pow_provider_oflowf",
    "__math_uflow": "crabc_x86_math_pow_provider_uflow",
    "__math_uflowf": "crabc_x86_math_pow_provider_uflowf",
    "__math_xflow": "crabc_x86_math_pow_provider_xflow",
    "__math_xflowf": "crabc_x86_math_pow_provider_xflowf",
    "fabs": "crabc_x86_math_pow_provider_fabs",
}
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_pow_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_pow_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_pow_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_pow_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 pow/powf translation.",
        " * Generated by compat/x86_64/generate_libc_math_pow.py; see",
        " * math_pow.rs for the source/license/ABI contract.",
        " * With explicit native conformance deltas in math_scalar_corrections.py.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-pow-") as temporary:
        temporary_root = Path(temporary)
        for relative in (*PUBLIC_SOURCES, *PRIVATE_SOURCES):
            assembly = temporary_root / (Path(relative).stem + ".S")
            original = (source_root / relative).read_text()
            corrected = corrected_source(relative, original)
            input_path = source_root / relative
            if corrected != original:
                input_path = temporary_root / Path(relative).name
                input_path.write_text(corrected)
            subprocess.run(
                [
                    arguments.cc,
                    *COMPILE_FLAGS,
