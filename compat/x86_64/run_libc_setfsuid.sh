#!/usr/bin/env bash
# Native Linux/x86-64 static crabc-libc setfsuid evidence.
#
...
# Linux syscall 122, including the unusual previous-filesystem-UID result and
...
readonly SOURCE="$ROOT_DIR/libc/src/c_abi/x86_64/setfsuid.rs"

fail() { printf 'ERROR: x86 static libc setfsuid: %s\n' "$*" >&2; exit 1; }
...
assert_setfsuid_boundary() {
    ...
    objdump -d --disassemble=setfsuid "$binary" >"$disassembly"
    grep -Eq '\$0x7a(,|[[:space:]]|$)' "$disassembly" ||
        fail "setfsuid does not issue syscall 122"
    grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$disassembly" ||
        fail "setfsuid lacks a syscall instruction"
    grep -Eq '%fs:(0x0|0x[0-9a-f]+|-)' "$disassembly" ||
        fail "setfsuid does not retain the initial-TLS errno path"
    if grep -Eq '\$0x7b(,|[[:space:]]|$)' "$disassembly"; then
        fail "setfsuid unexpectedly issues the paired group syscall"
    fi
}
...
[ -f "$SOURCE" ] || fail "missing setfsuid source"

bash "$ROOT_DIR/compat/x86_64/run_musl_oracle.sh" >/dev/null
bash "$ROOT_DIR/compat/x86_64/run_setfsuid_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-setfsuid.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-setfsuid-reference"
candidate="$work_dir/crabc-static-setfsuid-candidate"
...
"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_setfsuid_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_setfsuid_probe.c -o "$reference"
"$reference"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap setfsuid; do
...
for marker in 'src/linux/setfsuid.c::setfsuid' 'SYS_SETFSUID' \
    'pub unsafe extern "C" fn setfsuid'; do
    grep -Fq "$marker" "$SOURCE" || fail "source lacks ${marker}"
done
if grep -Fq 'SYS_SETFSGID' "$SOURCE" || grep -Fq 'fn setfsgid' "$SOURCE"; then
    fail "source must not select the paired group filesystem credential"
fi
...
assert_setfsuid_boundary "$archive" "$work_dir/archive-setfsuid-disassembly"

"$ORACLE_CC" -std=c11 -DCRABC_SETFSUID_FREESTANDING -I"$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_setfsuid_probe.c \
    compat/x86_64/libc_setfsuid_start.S "$archive" -o "$candidate"
...
for unrelated in setfsgid setuid setgid setgroups setresuid setresgid \
...
if grep -Eqi 'arch_prctl|mov[[:space:]]+%rsi,[[:space:]]*%fs:0' \
    compat/x86_64/libc_setfsuid_start.S; then
    fail "fixture start must not install a private FS base"
fi
assert_setfsuid_boundary "$candidate" "$work_dir/candidate-setfsuid-disassembly"

"$candidate"
printf 'x86 static crabc-libc setfsuid: PASS\n'
