#!/usr/bin/env bash
# Native Linux/x86-64 GNU sched_setaffinity C/C++ header ABI gate.
#
...
# `int sched_setaffinity(pid_t, size_t, const cpu_set_t *)` spelling, LP64
# layout, and C++ C linkage. This does not select CPU_* helpers, affinity
# observation, pthreads, or public x86 support.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly C_PROBE="$ROOT_DIR/compat/x86_64/sched_setaffinity_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/sched_setaffinity_header_abi_probe.cpp"
readonly VISIBILITY_PROBE="$ROOT_DIR/compat/x86_64/sched_setaffinity_header_visibility_probe.c"

fail() { printf 'ERROR: x86 sched_setaffinity header ABI: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-sched-setaffinity-header.XXXXXX)"
...
for tree in oracle project; do
    ...
    for profile in strict posix xopen; do
        ...
        if "$ORACLE_CC" -std=c11 "${feature_args[@]}" -Werror=implicit-function-declaration \
            ...
            fail "$tree $profile unexpectedly exposes sched_setaffinity"
        fi
        if "$ORACLE_CC" -std=c++17 -x c++ "${feature_args[@]}" \
            "${include_args[@]}" -fsyntax-only "$VISIBILITY_PROBE" >/dev/null 2>&1; then
            fail "$tree $profile C++ unexpectedly exposes sched_setaffinity"
        fi
    done
    ...
    grep -Eq '[[:space:]]sched_setaffinity$' <<<"$undefined" ||
        fail "$tree GNU C++ witness lacks unmangled sched_setaffinity"
    if grep -Eq '_Z.*sched_setaffinity' <<<"$undefined"; then
        fail "$tree GNU C++ witness retained a mangled sched_setaffinity reference"
    fi
done
...
printf 'x86 pinned-musl/project sched_setaffinity GNU C/C++ header ABI: PASS\n'
