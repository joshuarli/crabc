#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc mkfifo evidence.
#
...
# archive. It proves the one FIFO-creation entry, not a special-node family,
# process umask API, path-policy framework, libc.so, CRT, loader, sysroot, or
# public x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc mkfifo: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_mkfifo_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-mkfifo.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-mkfifo-reference"
candidate="$work_dir/crabc-static-mkfifo-candidate"
...
mkfifo_disassembly="$work_dir/mkfifo-disassembly"

cd "$ROOT_DIR"
mkdir "$reference_work" "$candidate_work"
"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_mkfifo_probe.c >/dev/null 2>"$header_trace"
for header in errno.h stdint.h sys/stat.h sys/syscall.h sys/types.h; do
    grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
        fail "fixture did not use the project $header header"
done

"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_mkfifo_probe.c -o "$reference"
(cd "$reference_work" && (umask 000; "$reference")) ||
    fail "pinned-musl mkfifo fixture failed"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap mkfifo; do
    grep -Eq "[[:space:]][TW][[:space:]]${symbol}$" "$archive_symbols" ||
        fail "archive does not define $symbol"
done

"$ORACLE_CC" -std=c11 -DCRABC_MKFIFO_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_mkfifo_probe.c compat/x86_64/libc_mkfifo_start.S \
    "$archive" -o "$candidate"
assert_static_closure "$candidate"

if grep -Eq '[[:space:]](mknod|mknodat|mkfifoat)$' "$work_dir/candidate-symbols"; then
    fail "mkfifo candidate exports an unselected special-node entry"
fi
objdump -d --disassemble=mkfifo "$candidate" >"$mkfifo_disassembly"
grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$mkfifo_disassembly" ||
    fail "mkfifo lacks a direct Linux syscall"
grep -Eq '\$0x103,%e?ax' "$mkfifo_disassembly" ||
    fail "mkfifo lacks Linux x86-64 mknodat=259"
if grep -Eq 'call.*(mknod|mknodat|mkfifoat|umask)' "$mkfifo_disassembly"; then
    fail "mkfifo delegates to an unselected C entry"
fi

(cd "$candidate_work" && (umask 000; "$candidate")) ||
    fail "freestanding mkfifo fixture failed"

printf 'x86 static crabc-libc mkfifo: PASS\n'
