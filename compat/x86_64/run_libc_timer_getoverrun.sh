#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc timer_getoverrun error ABI.
#
...
# only nonnegative invalid opaque timer values and never observes a valid POSIX
# timer. This is an error-translation ABI artifact, not timer support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc timer_getoverrun: %s\n' "$*" >&2
    exit 1
}
...
assert_timer_getoverrun_syscall() {
    local disassembly="$work_dir/timer-getoverrun-disassembly"

    objdump -d --disassemble=timer_getoverrun "$candidate" >"$disassembly"
    # The single System V AMD64 argument arrives in rdi and is already in the
    # kernel's first syscall register, so the optimized direct wrapper need
    # not spell a register-to-register move in its disassembly.
    grep -Eq '\$0xe1(,|[[:space:]]|$)' "$disassembly" ||
        fail "timer_getoverrun lacks syscall 225"
    grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$disassembly" ||
        fail "timer_getoverrun lacks the Linux syscall instruction"
    grep -Eq '%fs:|__errno_location' "$disassembly" ||
        fail "timer_getoverrun must publish raw failure through errno TLS"
}
...
bash "$ROOT_DIR/compat/x86_64/run_timer_getoverrun_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-timer-getoverrun.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-timer-getoverrun-reference"
candidate="$work_dir/crabc-static-timer-getoverrun-candidate"
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -I "$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_timer_getoverrun_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -fno-builtin \
    ...
    compat/x86_64/libc_timer_getoverrun_probe.c -o "$reference"
"$reference" || fail "pinned-musl timer_getoverrun fixture failed"
...
for selected in __errno_location timer_getoverrun; do
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L \
    -DCRABC_TIMER_GETOVERRUN_FREESTANDING -I "$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_timer_getoverrun_probe.c \
    compat/x86_64/libc_timer_getoverrun_start.S "$archive" -o "$candidate"
...
for unselected in timer_create timer_delete timer_gettime timer_settime \
...
assert_timer_getoverrun_syscall
env -i "$candidate" || fail "freestanding timer_getoverrun fixture failed"
printf 'x86 static crabc-libc timer_getoverrun: PASS\n'
