/* Static crabc-libc x86-64 lchown compatibility fixture.
 *
 ...
 * symlinkat, newfstatat, lchown, and unlinkat calls create, observe, compare,
 * and remove only fixture-owned entries. `lchown` is the only candidate C
 * entry. A dangling symlink plus the all-ones no-change uid/gid words proves
 * final-symlink no-follow success without requiring CAP_CHOWN; this is not a
 * wider ownership/credential API, pathname policy, allocation, CWD state, or
...
_Static_assert(AT_FDCWD == FIXTURE_AT_FDCWD &&
               ...
               "x86 lchown fixture constants");
_Static_assert(SYS_lchown == 94 && SYS_mkdirat == 258 &&
                   SYS_newfstatat == 262 && SYS_unlinkat == 263 &&
                   SYS_symlinkat == 266,
               "Linux x86 lchown fixture syscall numbers");
_Static_assert(EFAULT == FIXTURE_EFAULT && EINTR == FIXTURE_EINTR &&
                   ENOENT == FIXTURE_ENOENT,
               "Linux x86 lchown errno values");
_Static_assert(__builtin_types_compatible_p(__typeof__(&lchown),
    int (*)(const char *, uid_t, gid_t)), "lchown declaration");
...
int crabc_x86_64_lchown_probe(void)
{
    static const char directory[] = "lchown-root";
    static const char dangling[] = "lchown-root/dangling";
    static const char missing[] = "lchown-root/missing";
    ...
    if (status == 0) {
        ...
        errno = EINTR;
        if (lchown(dangling, (uid_t)-1, (gid_t)-1) != 0)
            status = 10;
        else if (errno != EINTR)
            status = 11;
    }
    if (status == 0) {
        if (raw_syscall3(SYS_lchown, (long)(uintptr_t)dangling,
            (long)(uint32_t)UINT32_MAX, (long)(uint32_t)UINT32_MAX) != 0)
            status = 12;
    }
    if (status == 0) {
        errno = 0;
        if (lchown(missing, (uid_t)-1, (gid_t)-1) != -1 || errno != ENOENT)
            status = 20;
    }
    if (status == 0) {
        errno = 0;
        if (lchown((const char *)0, (uid_t)-1, (gid_t)-1) != -1 ||
            errno != EFAULT)
            status = 21;
    }
    if (status == 0) {
        errno = 0;
        if (lchown("", (uid_t)-1, (gid_t)-1) != -1 || errno != ENOENT)
            status = 22;
    }
...
#ifndef CRABC_LCHOWN_FREESTANDING
int main(void)
{
    return crabc_x86_64_lchown_probe();
}
#endif
