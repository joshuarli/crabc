#!/usr/bin/env bash
# Native Linux/x86-64 compile-only <unistd.h> ABI slice.
#
...
fail() {
    printf 'ERROR: x86 unistd.h ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/unistd_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/unistd_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-unistd-header.XXXXXX)"
...
printf 'x86 pinned-musl C/C++ <unistd.h> ABI: PASS\n'
