#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``cosh``/``coshf`` leaf.
...
# cosh{,f} needs musl's expm1 and overflow reconstruction paths. Every
# provider is renamed and localized, including the exp tables/error helpers,
# so this leaf neither selects nor calls a separately public exp/expm1 ABI.
PUBLIC_SOURCES = (
    "src/math/cosh.c",
    "src/math/coshf.c",
)
...
PRIVATE_RENAMES = {
    "expm1": "crabc_x86_math_cosh_provider_expm1",
    "expm1f": "crabc_x86_math_cosh_provider_expm1f",
    "__expo2": "crabc_x86_math_cosh_provider_expo2",
    "__expo2f": "crabc_x86_math_cosh_provider_expo2f",
    "exp": "crabc_x86_math_cosh_provider_exp",
    "expf": "crabc_x86_math_cosh_provider_expf",
    "__exp_data": "crabc_x86_math_cosh_data_exp",
    "__exp2f_data": "crabc_x86_math_cosh_data_exp2f",
    "__math_oflow": "crabc_x86_math_cosh_helper_oflow",
    "__math_oflowf": "crabc_x86_math_cosh_helper_oflowf",
    "__math_uflow": "crabc_x86_math_cosh_helper_uflow",
    "__math_uflowf": "crabc_x86_math_cosh_helper_uflowf",
    "__math_xflow": "crabc_x86_math_cosh_helper_xflow",
    "__math_xflowf": "crabc_x86_math_cosh_helper_xflowf",
}
...
def transform_assembly(relative: str, text: str) -> str:
    """Namespace TU-local labels and keep all source closure providers local."""
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_cosh_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_cosh_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_cosh_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_cosh_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 cosh/coshf translation.",
        " * Generated by compat/x86_64/generate_libc_math_cosh.py; see",
        " * math_cosh.rs for the source/license/ABI contract.",
        " *",
        " * The musl-distributed portions retain the Sun and Arm notices below;",
        " * musl's MIT license for the 1.2.6 distribution is recorded by",
        " * compat/upstreams.toml.",
        " */",
    ]
    blocks.extend(retained_notices(source_root))
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-cosh-") as temporary:
