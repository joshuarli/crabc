#!/usr/bin/env bash
# Native Linux/x86-64 POSIX spawn-attribute setpgroup C/C++ declaration gate.
#
...
# ambient libc cannot mask the unconditional declaration, record layout, or
# its C++ linkage. This proves one caller-storage assignment spelling, not
# spawn execution, child lifecycle, file actions, other attribute behavior,
# signals, or scheduling policy.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_setpgroup_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_setpgroup_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 spawn.h posix_spawnattr_setpgroup headers: %s\n' "$*" >&2
    exit 1
}
...
check_cxx_linkage() {
    ...
    grep -Eq '[[:space:]]posix_spawnattr_setpgroup$' <<<"$undefined" ||
        fail "$profile $tree C++ probe does not retain C linkage for posix_spawnattr_setpgroup"
    if grep -Eq '_Z[0-9].*posix_spawnattr_setpgroup' <<<"$undefined"; then
        fail "$profile $tree C++ probe retained a mangled posix_spawnattr_setpgroup reference"
    fi
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-posix-spawnattr-setpgroup-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional posix_spawnattr_setpgroup declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <spawn.h> posix_spawnattr_setpgroup ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
