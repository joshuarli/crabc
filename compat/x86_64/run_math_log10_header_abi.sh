#!/usr/bin/env bash
# Native Linux/x86-64 C++ declaration/linkage evidence for log10/log10f.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly SYMBOLS=(log10 log10f)

fail() { printf 'ERROR: x86 math log10 header ABI: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-math-log10-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
probe="$ROOT_DIR/compat/x86_64/math_log10_header_abi_probe.cpp"
...
for mode in sse x87; do
	...
	for object in "$reference" "$candidate"; do
		...
		if grep -Eq '_Z.*log10' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled log10 reference"
		fi
	done
done

printf 'x86 pinned-musl/project log10/log10f header ABI: PASS\n'
