#!/usr/bin/env bash
# Native Linux/x86-64 search.h insque/remque C/C++ declaration gate.
#
...
c_probe="$ROOT_DIR/compat/x86_64/intrusive_queue_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/intrusive_queue_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-intrusive-queue-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-intrusive-queue-cxx.o"
candidate_cxx_object="$work_dir/candidate-intrusive-queue-cxx.o"
...
if ! "$ORACLE_CC" -std=c11 -U_GNU_SOURCE -D__STRICT_ANSI__ -fno-builtin \
    ...
    fail "project C intrusive-queue header contract drifted"
fi
...
for object in "$oracle_cxx_object" "$candidate_cxx_object"; do
    ...
    if printf '%s\n' "$undefined" | grep -Eq '_Z[0-9].*(insque|remque)'; then
        fail "C++ probe retained a mangled intrusive-queue reference"
    fi
done

printf 'x86 pinned-musl/project C/C++ <search.h> insque/remque ABI: PASS\n'
