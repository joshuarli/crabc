#!/usr/bin/env bash
# Native Linux/x86-64 selected static trunc/truncf evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc math trunc: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-trunc.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-math-trunc-reference"
candidate="$work_dir/crabc-static-math-trunc-candidate"
header_cxx_reference="$work_dir/musl-math-trunc-header.o"
header_cxx_candidate="$work_dir/project-math-trunc-header.o"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_trunc_probe.c >/dev/null 2>"$header_trace"
for header in fenv.h float.h math.h stdint.h features.h bits/alltypes.h; do
	grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
		fail "fixture did not use project $header"
done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
	compat/x86_64/libc_math_trunc_probe.c -o "$reference"
if "$reference"; then
	:
else
	status=$?
	fail "pinned-musl math trunc fixture failed with status ${status}"
fi
...
for mode in sse x87; do
	...
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -c compat/x86_64/math_trunc_header_abi_probe.cpp \
		-o "$header_cxx_reference"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" \
		-c compat/x86_64/math_trunc_header_abi_probe.cpp -o "$header_cxx_candidate"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only \
		compat/x86_64/math_trunc_header_abi_probe.cpp >/dev/null 2>"$cxx_header_trace"
	...
	for object in "$header_cxx_reference" "$header_cxx_candidate"; do
		...
		for symbol in trunc truncf; do
			grep -Eq "[[:space:]]${symbol}$" <<<"$undefined" ||
				fail "C++ ${mode} probe does not retain unmangled ${symbol}"
		done
		if grep -Eq '_Z.*trunc' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled trunc reference"
		fi
	done
done
...
for symbol in trunc truncf feclearexcept fegetenv feraiseexcept fesetenv fesetround fetestexcept; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_TRUNC_FREESTANDING \
	...
	compat/x86_64/libc_math_trunc_probe.c compat/x86_64/libc_math_trunc_start.S \
	"$archive" -o "$candidate"
...
for unselected in truncl fabs fabsf fabsl copysign copysignf copysignl fdim fdimf fdiml \
	fmax fmaxf fmaxl fmin fminf fminl rint rintf rintl nearbyint nearbyintf nearbyintl \
	sqrt sqrtf sqrtl cbrt cbrtf cbrtl; do
...
for instruction in addsd addss; do
	grep -Eq "[[:space:]]${instruction}([[:space:]]|$)" "$disassembly" ||
		fail "candidate lacks ${instruction} force evaluation"
done
"$candidate" || fail "freestanding math trunc fixture failed"

printf 'x86 static libc math trunc: PASS\n'
