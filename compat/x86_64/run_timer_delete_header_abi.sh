#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ <time.h> timer_delete ABI gate.
#
...
# exact opaque-timer external-C declaration. This is header-only evidence,
# not POSIX timer deletion, pthread timer representation, or time support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 time.h timer_delete ABI: %s\n' "$*" >&2
    exit 1
}
        ...
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                if "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
                    ...
                    -D__STRICT_ANSI__ -DCRABC_TIMER_DELETE_EXPECT_HIDDEN \
                    -Werror=implicit-function-declaration "${include_args[@]}" \
                    -fsyntax-only "$c_probe" >"$errors" 2>&1; then
                    fail "timer_delete is visible under strict C11 (${variant})"
                fi
                ...
                >"$errors" 2>&1; then
                fail "timer_delete is visible under strict C++17 (${variant})"
            fi
        done
    done
}
    ...
    for language in c cxx; do
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                ...
                object="$work_dir/${variant}-${profile}-timer-delete-cxx.o"
                "$compiler" -std=c++17 -x c++ -U_GNU_SOURCE -U_BSD_SOURCE \
                    -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
                    "$definition" "${include_args[@]}" -c "$cxx_probe" -o "$object"
                undefined="$(nm --undefined-only "$object")"
                grep -Eq '[[:space:]]timer_delete$' <<<"$undefined" ||
                    fail "C++ probe does not retain C linkage for timer_delete (${variant}, ${profile})"
                if grep -Eq '_Z.*timer_delete' <<<"$undefined"; then
                    fail "C++ probe retained a mangled timer_delete reference (${variant}, ${profile})"
                fi
            fi
        done
    done
...
c_probe="$ROOT_DIR/compat/x86_64/timer_delete_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/timer_delete_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-timer-delete-header.XXXXXX)"
...
if ! "$ORACLE_CC" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
    ...
    fail "project POSIX C timer_delete header contract drifted"
fi
...
printf 'x86 pinned-musl/project C/C++ <time.h> timer_delete ABI: PASS\n'
