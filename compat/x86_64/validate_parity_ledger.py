...
def require_ldso_fixed_graph_dlfcn_artifact(family: Mapping[str, Any]) -> None:
    """Ratchet real fixed-graph handle state without claiming dynamic loading."""
    artifacts = require_verified_artifacts(
        family.get("verified_artifact"),
        "family[ldso.dynamic-runtime].verified_artifact",
        family.get("status", ""),
    )
    matching = [entry for entry in artifacts if entry.get("id") == "ldso-fixed-graph-dlfcn"]
    require(
        len(matching) == 1,
        "ldso.dynamic-runtime needs exactly one ldso-fixed-graph-dlfcn artifact",
    )
    require(
        family.get("status") == "planned",
        "ldso-fixed-graph-dlfcn must not promote ldso.dynamic-runtime",
    )
    artifact = matching[0]
    require("capabilities" not in artifact, "ldso-fixed-graph-dlfcn must not select capabilities")
    description = artifact["description"]
    assert isinstance(description, str)
    for phrase in (
        "still-planned `ldso.dynamic-runtime`",
    ...
    require(
        set(string_list(artifact["source_owners"], "ldso-fixed-graph-dlfcn source owners"))
        == expected_sources,
        "ldso-fixed-graph-dlfcn source owners drifted",
    )
    prerequisite_text = " ".join(
        string_list(
            artifact["x86_abi_prerequisites"],
            "ldso-fixed-graph-dlfcn ABI prerequisites",
        )
    )
    for phrase in (
        "STB_WEAK",
        ...
        "0x43524142435f5844",
        "ABI size 64",
        "open/symbol/close/address/snapshot/information",
        "256-byte caller-owned payload",
        "exact addresses of loader-owned identity tokens",
        "bounded atomic reference counts",
        "RTLD_GLOBAL=0x100",
        "defined STB_GLOBAL/STB_WEAK ordinary non-TLS type",
        "checked PT_LOAD containment",
    ):
        require(
            phrase in prerequisite_text,
            f"ldso-fixed-graph-dlfcn ABI prerequisites omit {phrase}",
        )
    evidence = artifact["native_evidence"]
    assert isinstance(evidence, list)
    require(
        ...
        "ldso-fixed-graph-dlfcn must use the dedicated native command",
    )
    scope = evidence[0]["scope"]
    assert isinstance(scope, str)
    for phrase in (
        "private ET_DYN",
        "64-byte record",
        "weak-main GLOB_DAT admission plus strong-main and weak-DSO rejection",
        "no ambient libc/loader dependency, PT_TLS, or public dl symbols",
        "forged/stale handle rejection",
        "close-without-unload",
        "malformed-record status 127",
        "strong-import status 127",
        "DSO-import status 127",
        "no runtime mapping, scope promotion, finalization/unload",
        "public x86 support",
    ):
        require(phrase in scope, f"ldso-fixed-graph-dlfcn evidence scope omits {phrase}")
    runner = (ROOT / "compat" / "x86_64" / "run_ldso_fixed_graph_dlfcn.sh").read_text()
    for phrase in (
        "crabc_fixed_graph_dlfcn",
        "crabc_fixed_graph_dlfcn_malformed",
        "x86_64_initial_graph_source_root.rs",
        "env -i PATH=/usr/bin:/bin",
        ...
        require(phrase in runner, f"ldso-fixed-graph-dlfcn runner omits {phrase}")
    require(
        "run_ldso_fixed_graph_dlfcn.sh" in (ROOT / "scripts" / "dev-x86_64.sh").read_text(),
        "ldso-fixed-graph-dlfcn dispatcher binding is missing",
    )
...
def require_ldso_bounded_runtime_dlopen_artifact(family: Mapping[str, Any]) -> None:
    ...
    for phrase in (
        ...
        "before its bounded constructor array exactly once",
    ...
    for phrase in (
        ...
        "`DT_INIT` target",
        "`DT_FINI` target",
        "main/mid/leaf DT_INIT remains reject-only",
        "main/mid/leaf DT_FINI remains reject-only",
        "immediately before the bounded init array",
        "never retained or dispatched",
        "inert on ordinary final close",
        "DT_FINI_ARRAY remains rejected",
    ...
    for phrase in (
        ...
        "two concurrent TLS-free RTLD_NODELETE clone opens",
...
def require_static_pthread_rwlock_artifact(family: Mapping[str, Any]) -> None:
    ...
    require(
        len(artifacts) == 22,
        "libc.pthread-tls must retain exactly twenty-two private verified artifacts",
    )
    require(
        family.get("status") == "planned",
    ...
    for phrase in (
        "Twenty-two separately verified static artifacts",
...
def require_math_bit_sign_artifact(family: Mapping[str, Any]) -> None:
    """Keep the selected binary32/binary64 sign masks below scalar math parity."""
    artifacts = require_verified_artifacts(
        family.get("verified_artifact"),
        "family[libc.text-math-locale-stdio].verified_artifact",
        family.get("status", ""),
    )
    matching = [entry for entry in artifacts if entry.get("id") == "static-c-math-bit-sign"]
    require(
        len(matching) == 1,
        "libc.text-math-locale-stdio must contain exactly one static-c-math-bit-sign artifact",
    )
    artifact = matching[0]
    require(
        "capabilities" not in artifact,
        "static-c-math-bit-sign must remain a non-capability artifact",
    )
    description = artifact["description"]
    assert isinstance(description, str)
    for symbol in MATH_BIT_SIGN_SYMBOLS:
        require(
            f"`{symbol}`" in description,
            f"static-c-math-bit-sign description omits {symbol}",
        )
    for phrase in (
        ...
        require(
            phrase in description,
            f"static-c-math-bit-sign description omits {phrase}",
        )

    owners = nonempty_strings(
        artifact["source_owners"], "static-c-math-bit-sign.source_owners"
    )
    for owner in (
        ...
        require(owner in owners, f"static-c-math-bit-sign omits {owner}")

    prerequisites = " ".join(
        nonempty_strings(
            artifact["x86_abi_prerequisites"],
            "static-c-math-bit-sign.x86_abi_prerequisites",
        )
    )
    for phrase in (
        "src/math/fabs.c",
        ...
        require(
            phrase in prerequisites,
            f"static-c-math-bit-sign prerequisites omit {phrase}",
        )
    header_prerequisites = " ".join(
        nonempty_strings(
            artifact["x86_header_prerequisites"],
    ...
    for phrase in ("parenthesized", "C++17", "-mfpmath=387", "unmangled C"):
        require(
            phrase in header_prerequisites,
            f"static-c-math-bit-sign header prerequisites omit {phrase}",
        )
    ...
    require(
        ...
        "static-c-math-bit-sign must use the closed libc-math-bit-sign command",
    )
    scope = evidence[0]["scope"]
    assert isinstance(scope, str)
    for phrase in (
        ...
        require(phrase in scope, f"static-c-math-bit-sign evidence omits {phrase}")

    static_root = (
        ROOT / "libc" / "src" / "c_abi" / "x86_64" / "static_c_abi.rs"
    ).read_text(encoding="utf-8")
    require(
        '#[path = "math_bit_sign.rs"]\nmod math_bit_sign;' in static_root,
        "x86 static C ABI must compose the math_bit_sign leaf",
    )
    implementation = (
        ROOT / "libc" / "src" / "c_abi" / "x86_64" / "math_bit_sign.rs"
    ).read_text(encoding="utf-8")
    for symbol in MATH_BIT_SIGN_SYMBOLS:
        require(
            f".global {symbol}" in implementation,
            f"math_bit_sign leaf omits {symbol}",
        )
    for snippet in (
        "src/math/fabs.c",
        ...
        require(snippet in implementation, f"math_bit_sign leaf omits {snippet}")
    ...
    for snippet in (
        ...
        require(snippet in runner, f"libc-math-bit-sign runner omits {snippet}")
    dispatcher = (ROOT / "scripts" / "dev-x86_64.sh").read_text(encoding="utf-8")
    for snippet in (
        "libc-math-bit-sign)",
...
def require_static_pthread_cpuclock_artifact(
    ...
    matching = [
        entry for entry in artifacts if entry.get("id") == "static-c-pthread-cpuclock"
    ]
    require(
        len(matching) == 1,
    ...
    require(
        family.get("status") == "planned",
        "pthread CPU-clock must not promote libc.pthread-tls",
    )
    ...
    for phrase in (
        ...
        require(
            phrase in family_description,
            f"libc.pthread-tls description omits {phrase} after pthread CPU clock",
        )
    ...
    for phrase in (
        ...
        require(phrase in description, f"pthread CPU-clock description omits {phrase}")
    ...
    require(
        set(string_list(artifact["source_owners"], "pthread CPU-clock source owners"))
        == expected_sources,
        "pthread CPU-clock source owners drifted",
    )
    ...
    for phrase in (
        ...
        require(
            phrase in prerequisite_text,
            f"pthread CPU-clock ABI prerequisites omit {phrase}",
        )
    header_text = " ".join(artifact["x86_header_prerequisites"])
    for phrase in (
        "errno.h",
        ...
        require(
            phrase in header_text,
            f"pthread CPU-clock header prerequisites omit {phrase}",
        )
    ...
    require(
        "pthread_getcpuclockid" in static_exports,
    ...
    for unselected in (
        ...
        require(
            unselected not in static_exports,
            f"pthread CPU-clock must not expose unselected {unselected}",
        )
    dispatcher = (ROOT / "scripts" / "dev-x86_64.sh").read_text(encoding="utf-8")
    for snippet in (
...
def require_static_pthread_name_artifact(
    family: Mapping[str, Any],
) -> None:
    """Ratchet the selected GNU task-name pair without pthread promotion."""
    ...
    matching = [
        entry for entry in artifacts if entry.get("id") == "static-c-pthread-name"
    ]
    require(
        len(matching) == 1,
    ...
    require(
        family.get("status") == "planned",
        "pthread task names must not promote libc.pthread-tls",
    )
    ...
    for phrase in (
        ...
        require(
            phrase in family_description,
            f"libc.pthread-tls description omits {phrase} after pthread task names",
        )
    ...
    for phrase in (
        ...
        "musl procfs task-name I/O",
        "cancellation",
        "general `prctl` C API",
        "scheduler/affinity attributes",
        "lifecycle, synchronization, TSS",
        "general pthread/TLS behavior or x86-64 parity",
        "family completion",
        "promotion",
        "public x86 support",
    ):
        require(phrase in description, f"pthread task-name description omits {phrase}")
    ...
    expected_sources = {
        ...
        "compat/x86_64/libc_pthread_name_start.S",
        "compat/x86_64/run_libc_pthread_name.sh",
        "compat/x86_64/tests/test_runner.py",
        "compat/x86_64/tests/test_parity_ledger.py",
        "compat/x86_64/validate_parity_ledger.py",
        "compat/x86_64/README.md",
    ...
    require(
        set(string_list(artifact["source_owners"], "pthread task-name source owners"))
        == expected_sources,
        "pthread task-name source owners drifted",
    )
    ...
    for phrase in (
        ...
        require(
            phrase in prerequisite_text,
            f"pthread task-name ABI prerequisites omit {phrase}",
        )
    header_text = " ".join(artifact["x86_header_prerequisites"])
    for phrase in (
        "_GNU_SOURCE",
        ...
        "PR_SET_NAME=15",
        "PR_GET_NAME=16",
        "int (*)(pthread_t, const char *)",
        "int (*)(pthread_t, char *, size_t)",
        "28-context C/C++ pthread/C11",
        "unmangled C++ references",
        "strict profiles retain feature hiding",
    ):
        require(phrase in header_text, f"pthread task-name header prerequisites omit {phrase}")
    ...
    require(
        ...
        "pthread task names must use their closed native command",
    )
    scope = evidence[0]["scope"]
    assert isinstance(scope, str)
    for phrase in (
        ...
        "exact 16-byte task-comm boundary",
        ...
        "no pthread errno publication",
        "interpreter/DT_NEEDED/unresolved symbol",
        "musl procfs target naming",
        "general prctl C API",
        "family completion, promotion, and public x86 support",
    ):
        require(phrase in scope, f"pthread task-name evidence scope omits {phrase}")
    ...
    require(
        ...
        "pthread task-name static export contract is incomplete",
    )
    for unselected in (
        "pthread_getattr_np",
        "pthread_tryjoin_np",
        ...
        require(
            unselected not in static_exports,
            f"pthread task names must not expose unselected {unselected}",
        )
    dispatcher = (ROOT / "scripts" / "dev-x86_64.sh").read_text(encoding="utf-8")
    for snippet in (
        "run_libc_pthread_name_probe()",
        "run_libc_pthread_name.sh",
        "libc-pthread-name)",
    ):
        require(snippet in dispatcher, f"pthread task-name dispatcher omits {snippet}")
...
def require_static_thrd_yield_artifact(family: Mapping[str, Any]) -> None:
    ...
    matching = [
        entry for entry in artifacts if entry.get("id") == "static-c-thrd-yield"
    ]
    require(
        len(matching) == 1,
    ...
    require(
        family.get("status") == "planned",
        "thrd_yield must not promote libc.pthread-tls",
    )
    ...
    for phrase in (
        ...
        require(
            phrase in family_description,
            f"libc.pthread-tls description omits {phrase} after thrd_yield",
        )
    ...
    for phrase in (
        ...
        require(phrase in description, f"thrd_yield description omits {phrase}")
    ...
    expected_sources = {
        ...
        "libc/src/lib.rs",
        "libc/src/c_abi/x86_64/static_c_abi.rs",
        "libc/src/c_abi/x86_64/thrd_yield.rs",
        "libc/src/c_abi/x86_64/errno.rs",
        "libc/src/c_abi/x86_64/syscall.rs",
        "include/bits/alltypes.h",
        "include/bits/syscall.h",
        ...
        "compat/x86_64/run_pthread_c11_header_abi.sh",
        "compat/x86_64/static_c_abi_exports.txt",
        "compat/x86_64/libc_thrd_yield_probe.c",
        "compat/x86_64/libc_thrd_yield_start.S",
        "compat/x86_64/run_libc_thrd_yield.sh",
        "compat/x86_64/tests/test_runner.py",
        "compat/x86_64/tests/test_parity_ledger.py",
        "compat/x86_64/validate_parity_ledger.py",
        "compat/x86_64/README.md",
    ...
    require(
        set(string_list(artifact["source_owners"], "thrd_yield source owners"))
        == expected_sources,
        "thrd_yield source owners drifted",
    )
    ...
    for phrase in (
        ...
        "EPERM",
        "Variant-II `%fs:0`",
        "does not create a C11 thread",
    ):
        require(
            phrase in prerequisite_text,
            f"thrd_yield ABI prerequisites omit {phrase}",
        )
    header_text = " ".join(artifact["x86_header_prerequisites"])
    for phrase in (
        "threads.h",
        ...
        "void (*)(void)",
        "28-context C/C++",
        "unmangled C-linkage reference",
        "not a broad header or pthread/C11 implementation claim",
    ):
        require(
            phrase in header_text,
            f"thrd_yield header prerequisites omit {phrase}",
        )
    ...
    require(
        ...
        "thrd_yield must use its closed native command",
    )
    scope = evidence[0]["scope"]
    assert isinstance(scope, str)
    for phrase in (
        ...
        "reject an interpreter/DT_NEEDED/unresolved symbol",
        "scheduler policy/parameters/fairness",
        "affinity/attributes",
        "C11 lifecycle/synchronization/TSS/cancellation",
        "family completion, promotion, and public x86 support",
    ):
        require(phrase in scope, f"thrd_yield evidence scope omits {phrase}")
