...
`libc-inet-netof` is a distinct private static C classful IPv4 network-part
extraction artifact under still-planned `libc.resolver`. Its project-header C
body runs through pinned musl 1.2.6 and then through an archive-free
`-nostdlib -static` candidate whose final link receives only the one extracted
...
`libc-memccpy`, `libc-aio-error`, `libc-byte-strings`, `libc-random-entropy`, `libc-memory-search`,
