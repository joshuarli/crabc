...
unselected.
`libc-math-sin` is the separate non-promoting `static-c-math-sin` artifact for
binary64/binary32 `sin`/`sinf`. Its project-header C fixture and default-SSE/
`-mfpmath=387` C++ signature probes run first through pinned musl and then
through one garbage-collected `-nostdlib -static` candidate. The checked GCC
15.2.0 translation of musl 1.2.6 `sin.c`/`sinf.c` is the complete direct source
closure: it localizes binary64/binary32 kernels, moderate and large `pi/2`
reduction with fixed multiword `2/pi` data, and its floor/scalbn providers
rather than calling ambient libm or sharing an existing math artifact. The
256-record raw differential covers signed zero, tiny/subnormal and normal
bounds, quadrant-neighbor values, moderate and huge reductions, finite
extremes, infinities, quiet/signaling NaNs, result payloads, flags, and
requested versus observed MXCSR direction in all four modes. Final-link proof
requires strong crabc-owned definitions, local closure providers, and scalar
`addsd`/`addss`/`subsd`/`subss`/`mulsd`/`mulss`/`cvtsd2ss`/`cvtss2sd`, while
rejecting weak compiler-builtins fallback, `sinl`, sincos, adjacent
cos/tan/hyperbolic/inverse-trig/log/exp/pow functions, fenv API/policy,
special/complex/binary80 math, dynamic linkage, TLS, and ambient libm. Family
completion, promotion, full x86-64 parity, and public x86 support remain
unselected.
`libc-math-tan` is the separate non-promoting `static-c-math-tan` artifact
for binary64/binary32 `tan`/`tanf`. Its project-header C fixture and
default-SSE/`-mfpmath=387` C++ signature probes run first through pinned musl
and then through one garbage-collected `-nostdlib -static` candidate. The
checked GCC 15.2.0 translation of musl 1.2.6 `tan.c`/`tanf.c` is the complete
direct source closure: it localizes the tangent kernels, moderate and large
`pi/2` reduction with fixed multiword `2/pi` data, and its floor/scalbn
providers rather than calling ambient libm or sharing an existing math
artifact. The 256-record raw differential covers signed zero, tiny/subnormal
and normal bounds, pole-adjacent quadrant values, moderate and huge
reductions, finite extremes, infinities, quiet/signaling NaNs, result payloads,
...
`divsd`/`cvtsd2ss`/`cvtss2sd`, while rejecting weak compiler-builtins
fallback, `tanl`, sincos, adjacent sin/cos/hyperbolic/inverse-trig/log/exp/pow
functions, fenv API/policy, special/complex/binary80 math, dynamic linkage,
TLS, and ambient libm. Family completion, promotion, full x86-64 parity, and
public x86 support remain unselected.
`libc-math-tanh` is the separate non-promoting `static-c-math-tanh` artifact
for binary64/binary32 `tanh`/`tanhf`. Its project-header C fixture and
default-SSE/`-mfpmath=387` C++ signature probes run first through pinned musl
and then through one garbage-collected `-nostdlib -static` candidate. The
checked GCC 15.2.0 translation of musl 1.2.6 `tanh.c`/`tanhf.c` plus renamed
local `expm1.c`/`expm1f.c` providers is the complete direct source closure:
it retains source threshold selection, subnormal force-evaluation, saturation,
and signed reconstruction without calling ambient libm or a public `expm1`
sibling. The 256-record raw differential covers signed zero, tiny/subnormal
and normal bounds, threshold and saturation neighborhoods, finite extremes,
infinities, quiet/signaling NaNs, result payloads, flags, and requested versus
observed MXCSR direction in all four modes. Final-link proof requires strong
crabc-owned definitions, local closure providers, and scalar
`addsd`/`addss`/`subsd`/`subss`/`mulsd`/`mulss`/`divsd`/`divss`/`cvtsd2ss`,
while rejecting weak compiler-builtins fallback, `tanhl`, adjacent
tan/sin/cos/hyperbolic/inverse-trig/log/exp/pow functions, fenv API/policy,
special/complex/binary80 math, dynamic linkage, TLS, and ambient libm. Family
completion, promotion, full x86-64 parity, and public x86 support remain
unselected.
`libc-math-atanh` is the separate non-promoting `static-c-math-atanh` artifact
for binary64/binary32 `atanh`/`atanhf`. Its project-header C fixture and
default-SSE/`-mfpmath=387` C++ signature probes run first through pinned musl
and then through one garbage-collected `-nostdlib -static` candidate. The
checked GCC 15.2.0 translation of musl 1.2.6 `atanh.c`/`atanhf.c` plus renamed
local `log1p.c`/`log1pf.c` providers is the complete direct source closure: it
retains source tiny-input force-evaluation, half-branch, pole/domain, and
signed reconstruction behavior without calling ambient libm or a public
`log1p` sibling. The 256-record raw differential covers signed zero,
tiny/subnormal and normal bounds, half and pole/domain neighborhoods, finite
extremes, infinities, quiet/signaling NaNs, result payloads, flags, and
requested versus observed MXCSR direction in all four modes. Final-link proof
requires strong crabc-owned definitions, local closure providers, and scalar
`addsd`/`addss`/`subsd`/`subss`/`mulsd`/`mulss`/`divsd`/`divss`/`cvtsd2ss`,
while rejecting weak compiler-builtins fallback, `atanhl`, adjacent
