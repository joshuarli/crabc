...
Its current 753 comparable red source-form rows, one
...
macro visibility across all 1,337 rows: 650 current identity-mismatch rows,
630 matched rows, while the one `aio.h:c11-strict` oracle-not-applicable row
and 56 project-only rows retain checked candidate fact summaries and digests.
Its 3,481 same-identity source-form
differences across 557 rows—including 103 form-only rows—remain separately
