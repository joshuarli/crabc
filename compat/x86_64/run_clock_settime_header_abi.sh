#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ <time.h> clock_settime ABI gate.
#
# Pinned musl 1.2.6 is the declaration and linkage oracle. Strict C11/C++17
# must hide this POSIX name; POSIX, X/Open, and GNU profiles expose its exact
# `int (clockid_t, const struct timespec *)` external-C spelling. This is
# header-only evidence, not clock mutation, authority, timer, or time support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 time.h clock_settime ABI: %s\n' "$*" >&2
    exit 1
}
        ...
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                if "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
                    ...
                    -D__STRICT_ANSI__ -DCRABC_CLOCK_SETTIME_EXPECT_HIDDEN \
                    -Werror=implicit-function-declaration "${include_args[@]}" \
                    -fsyntax-only "$c_probe" >"$errors" 2>&1; then
                    fail "clock_settime is visible under strict C11 (${variant})"
                fi
                ...
                >"$errors" 2>&1; then
                fail "clock_settime is visible under strict C++17 (${variant})"
            fi
        done
    done
}
    ...
    for language in c cxx; do
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                ...
                object="$work_dir/${variant}-${profile}-clock-settime-cxx.o"
...
c_probe="$ROOT_DIR/compat/x86_64/clock_settime_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/clock_settime_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-clock-settime-header.XXXXXX)"
...
if ! "$ORACLE_CC" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
    ...
    fail "project POSIX C clock_settime header contract drifted"
fi
...
printf 'x86 pinned-musl/project C/C++ <time.h> clock_settime ABI: PASS\n'
