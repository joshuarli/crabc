#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc alarm evidence.
#
# The one-symbol project-header C body first executes through pinned musl
# 1.2.6, then as a true `-nostdlib -static` candidate. It proves only the
# historical real-interval-timer adapter: fractional old-value ceiling,
# one-shot whole-second replacement, disarm return, and stale errno. The
# fixture's raw setup/inspection is containment only; it does not select
# public setitimer, signal handlers/actions/masks, waits, or delivery policy.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc alarm: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-alarm.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-alarm-reference"
candidate="$work_dir/crabc-static-alarm-candidate"
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -U_GNU_SOURCE \
    -I"$ROOT_DIR/include" -E -H compat/x86_64/libc_alarm_probe.c \
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -U_GNU_SOURCE \
    ...
    compat/x86_64/libc_alarm_probe.c -o "$reference"
"$reference"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap alarm; do
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -U_GNU_SOURCE \
    -DCRABC_ALARM_FREESTANDING -I"$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_alarm_probe.c compat/x86_64/libc_alarm_start.S \
    "$archive" -o "$candidate"
...
for unrelated in getitimer setitimer ualarm timer_create timer_delete \
...
if grep -Eqi 'arch_prctl|mov[[:space:]]+%rsi,[[:space:]]*%fs:0' \
    compat/x86_64/libc_alarm_start.S; then
...
"$candidate"
printf 'x86 static crabc-libc alarm: PASS\n'
