#!/usr/bin/env bash
# Native Linux/x86-64 selected-static crabc-libc munlockall evidence.
#
# One direct musl wrapper first executes through pinned musl 1.2.6, then
# through a true -nostdlib -static candidate. The release request is confined
# to a disposable process. It admits only munlockall, not mlockall, per-range
# locks, mapping policy, allocator/runtime, or public x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc munlockall: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-munlockall.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-munlockall-reference"
candidate="$work_dir/crabc-static-munlockall-candidate"
...
"$ORACLE_CC" -std=c11 -D__STRICT_ANSI__ -U_GNU_SOURCE \
    -I "$ROOT_DIR/include" -E -H compat/x86_64/libc_munlockall_probe.c \
...
"$ORACLE_CC" -std=c11 -D__STRICT_ANSI__ -U_GNU_SOURCE -fno-builtin \
    ...
    compat/x86_64/libc_munlockall_probe.c -o "$reference"
"$reference" || fail "pinned-musl munlockall fixture failed"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap munlockall; do
...
"$ORACLE_CC" -std=c11 -D__STRICT_ANSI__ -U_GNU_SOURCE \
    -DCRABC_MUNLOCKALL_FREESTANDING -I "$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_munlockall_probe.c \
    compat/x86_64/libc_munlockall_start.S "$archive" -o "$candidate"
...
for unrelated in mlock mlock2 mlockall munlock msync mremap mmap mprotect \
    ...
    if grep -Eq "[[:space:]]${unrelated}$" "$candidate_symbols"; then
        fail "munlockall candidate unexpectedly pulls ${unrelated}"
    fi
done
...
if grep -Eqi 'arch_prctl|mov[[:space:]]+%rsi,[[:space:]]*%fs:0' \
    compat/x86_64/libc_munlockall_start.S; then
...
"$candidate"
printf 'x86 static crabc-libc munlockall: PASS\n'
