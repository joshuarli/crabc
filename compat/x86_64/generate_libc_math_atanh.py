#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``atanh``/``atanhf`` leaf.
...
# The public binary64/binary32 entries call only their matching log1p source.
# It is compiled under a private name, so this leaf does not select the
# separately evidenced public log1p/log1pf C ABI.
PUBLIC_SOURCES = (
    "src/math/atanh.c",
    "src/math/atanhf.c",
)
...
PRIVATE_RENAMES = {
    "log1p": "crabc_x86_math_atanh_provider_log1p",
    "log1pf": "crabc_x86_math_atanh_provider_log1pf",
}

# Preserve musl's generic scalar path and all-rounding-mode arithmetic.
COMPILE_FLAGS = (
    "-D_XOPEN_SOURCE=700",
    "-std=c99",
    "-ffreestanding",
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_atanh_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_atanh_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_atanh_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_atanh_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 atanh/atanhf translation.",
        " * Generated by compat/x86_64/generate_libc_math_atanh.py; see",
        " * math_atanh.rs for the source/license/ABI contract.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-atanh-") as temporary:
