#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc time-observation evidence.
#
...
# archive. It proves only direct clock observation and conversion; it is not
# calendar/timezone state, clock mutation, timer policy, a vDSO resolver,
...
fail() { printf 'ERROR: x86 static libc time observation: %s\n' "$*" >&2; exit 1; }
...
assert_named_syscall() {
    ...
    grep -Eq "\\\$0x${syscall_word}(,|[[:space:]]|$)" "$disassembly" ||
        fail "${symbol} lacks syscall ${syscall_word}"
    grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$disassembly" ||
        fail "${symbol} lacks syscall instruction"
}
...
candidate="$work_dir/crabc-static-time-observation-candidate"

cd "$ROOT_DIR"
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_time_observation_probe.c >/dev/null 2>"$work_dir/header-trace"
for header in errno.h time.h sys/time.h sys/syscall.h bits/alltypes.h bits/syscall.h; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_TIME_OBSERVATION_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_time_observation_probe.c \
    compat/x86_64/libc_time_observation_start.S "$archive" -o "$candidate"
