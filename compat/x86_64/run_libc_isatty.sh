#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc isatty evidence.
#
# The pinned-musl/project-header fixture proves only the zero-policy C ABI
# descriptor observation: TIOCGWINSZ success maps to one without changing
# errno, and raw EBADF/ENOTTY errors map to zero. Raw devpts setup lives only
# in the fixture, so it selects no public PTY or terminal-control family.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc isatty: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_isatty_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-isatty.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-isatty-reference"
candidate="$work_dir/crabc-static-isatty-candidate"
...
if ! "$ORACLE_CC" -std=c11 -fno-builtin -I "$ROOT_DIR/include" -H \
    -fsyntax-only compat/x86_64/libc_isatty_probe.c \
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I "$ROOT_DIR/include" compat/x86_64/libc_isatty_probe.c -o "$reference"
if timeout "$EXECUTION_TIMEOUT" "$reference"; then
    :
else
    reference_status=$?
    fail "pinned-musl isatty fixture exited ${reference_status}"
fi
...
for symbol in __errno_location isatty; do
    grep -Eq "[[:space:]][TW][[:space:]]${symbol}$" "$archive_symbols" ||
        fail "archive does not define ${symbol}"
done

"$ORACLE_CC" -std=c11 -DCRABC_ISATTY_FREESTANDING -I "$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_isatty_probe.c compat/x86_64/libc_isatty_start.S \
    "$archive" -o "$candidate"
...
grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$isatty_disassembly" ||
    fail "isatty lacks a direct ioctl syscall"
grep -Eq '\$0x5413,%esi|\$0x5413,%rsi' "$isatty_disassembly" ||
    fail "isatty lacks the fixed TIOCGWINSZ request"
if grep -Eq '\$0x5401|\$0x5402|\$0x5403|\$0x5404|\$0x5414' \
    "$isatty_disassembly"; then
    fail "isatty unexpectedly selects a termios-control request"
fi
if grep -Eq '[[:space:]](tcgetattr|tcsetattr|tcgetwinsize|tcsetwinsize|ttyname|ttyname_r|getpass|openpty|forkpty|login_tty|vhangup|posix_openpt|grantpt|unlockpt|ptsname|ptsname_r)$' \
    "$work_dir/candidate-symbols"; then
    fail "isatty candidate selects an excluded terminal helper"
fi
...
if timeout "$EXECUTION_TIMEOUT" "$candidate"; then
    ...
    candidate_status=$?
    fail "freestanding isatty fixture exited ${candidate_status}"
fi

printf 'x86 static crabc-libc isatty: PASS\n'
