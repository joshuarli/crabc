...
Linux/x86-64 static consumer in ordinary `ET_EXEC` and static-PIE `ET_DYN`
modes, then repeats the same smoke from one extracted package. It is a
...
<installed>/usr/include` dependencies for all three translation units, where
...
suite, including allocator, complete pthread, stdio, filesystem, socket, and
