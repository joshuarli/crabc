...
# memccpy is XOPEN/GNU/BSD-selected and stays hidden in strict/POSIX C.
set -euo pipefail
...
for definitions_name in strict_definitions posix_definitions xopen_definitions \
    gnu_definitions bsd_definitions; do
...
if ! "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
    "${xopen_definitions[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only "$c_probe" \
    >/dev/null 2>"$header_trace"; then
    sed -n '1,160p' "$header_trace" >&2
    fail "project XOPEN memccpy header contract drifted"
fi
for header in string.h features.h bits/alltypes.h; do
    grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
        fail "C probe did not use project $header"
done
for definitions_name in strict_definitions posix_definitions gnu_definitions \
    bsd_definitions; do
    declare -n definitions="$definitions_name"
    "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
...
# Strict and POSIX C must reject the feature-selected declaration in both
# header trees. Do not repeat negative C++ checks: g++ enables GNU names.
for definitions_name in strict_definitions posix_definitions; do
    declare -n definitions="$definitions_name"
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -DCRABC_REQUIRE_MEMCCPY_HIDDEN \
        -fsyntax-only "$c_probe" >"$work_dir/oracle-${definitions_name}.out" 2>&1; then
        fail "pinned musl exposes memccpy outside XOPEN/GNU/BSD selectors"
    fi
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -I "$ROOT_DIR/include" \
        -DCRABC_REQUIRE_MEMCCPY_HIDDEN -fsyntax-only "$c_probe" \
        >"$work_dir/project-${definitions_name}.out" 2>&1; then
        fail "project string.h exposes memccpy outside XOPEN/GNU/BSD selectors"
    fi
done
