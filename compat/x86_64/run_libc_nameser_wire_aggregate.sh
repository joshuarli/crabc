#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc nameser wire/data aggregate.
#
...
fail() {
    printf 'ERROR: x86 static libc nameser wire aggregate: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-nameser-wire-aggregate.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-nameser-wire-aggregate-reference"
candidate="$work_dir/crabc-static-nameser-wire-aggregate-candidate"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
musl_archive="$("$ORACLE_CC" -print-file-name=libc.a)"
musl_ns_parse="$work_dir/musl-ns-parse.o"
musl_dn_expand="$work_dir/musl-dn-expand.o"
musl_ns_parse_symbols="$work_dir/musl-ns-parse-symbols"
musl_dn_expand_symbols="$work_dir/musl-dn-expand-symbols"
...
ns_skiprr_disassembly="$work_dir/ns-skiprr-disassembly"
...
ar p "$musl_archive" ns_parse.lo >"$musl_ns_parse"
ar p "$musl_archive" dn_expand.lo >"$musl_dn_expand"
readelf --symbols --wide "$musl_ns_parse" >"$musl_ns_parse_symbols"
readelf --symbols --wide "$musl_dn_expand" >"$musl_dn_expand_symbols"
grep -Eq '[[:space:]]FILE[[:space:]]+LOCAL[[:space:]]+DEFAULT[[:space:]]+ABS[[:space:]]+ns_parse\.c$' "$musl_ns_parse_symbols" ||
    fail "pinned musl ns_parse object no longer maps to ns_parse.c"
grep -Eq '[[:space:]]FUNC[[:space:]]+GLOBAL[[:space:]]+DEFAULT[[:space:]]+[0-9]+[[:space:]]+ns_skiprr$' "$musl_ns_parse_symbols" ||
    fail "pinned musl ns_parse object no longer defines ns_skiprr"
grep -Eq '[[:space:]]128[[:space:]]+OBJECT[[:space:]]+GLOBAL[[:space:]]+DEFAULT[[:space:]]+[0-9]+[[:space:]]+_ns_flagdata$' "$musl_ns_parse_symbols" ||
    fail "pinned musl ns_parse object no longer defines _ns_flagdata"
grep -Eq '[[:space:]]FILE[[:space:]]+LOCAL[[:space:]]+DEFAULT[[:space:]]+ABS[[:space:]]+dn_expand\.c$' "$musl_dn_expand_symbols" ||
    fail "pinned musl dn_expand object no longer maps to dn_expand.c"
assert_dn_expand_alias "$musl_dn_expand_symbols" "pinned-musl static archive"
...
if env -i LC_ALL=C TZ=UTC "$reference"; then
    ...
    status=$?
    fail "pinned-musl nameser wire aggregate fixture exited ${status}"
fi
...
for symbol in __dn_expand dn_expand dn_skipname _ns_flagdata ns_get16 ns_get32 \
    ns_put16 ns_put32 ns_skiprr; do
    grep -Eq "[[:space:]][TWDVBR][[:space:]]${symbol}$" "$archive_symbols" ||
...
"$ORACLE_CC" -std=c11 -DCRABC_NAMESER_WIRE_AGGREGATE_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie \
    -ffreestanding -fno-builtin -fno-stack-protector -Wl,-e,_start \
    -Wl,--gc-sections -Wl,--no-undefined \
    compat/x86_64/libc_nameser_wire_aggregate_probe.c \
    compat/x86_64/libc_nameser_wire_aggregate_start.S "$selected_member" "$archive" \
    -o "$candidate"
...
objdump -d --disassemble=ns_skiprr "$candidate" >"$ns_skiprr_disassembly"
for symbol in __dn_expand dn_expand dn_skipname _ns_flagdata ns_get16 ns_get32 \
    ns_put16 ns_put32 ns_skiprr __errno_location __crabc_x86_static_tls_bootstrap; do
    grep -Eq "[[:space:]]${symbol}$" "$candidate_symbols" ||
        fail "candidate does not define ${symbol}"
done
awk '$3 == "128" && $4 == "OBJECT" && $8 == "_ns_flagdata" { found = 1 } END { exit !found }' "$candidate_symbols" ||
    fail "candidate does not retain the 128-byte immutable _ns_flagdata object"
...
if grep -Eq 'TLSGD|TLSLD|TLSDESC|GOTTPOFF|DTPMOD(64)?|DTPOFF(32|64)?|__tls_get_addr' \
    "$candidate_relocations" "$candidate_symbols" "$ns_skiprr_disassembly"; then
    fail "candidate relocations retain a dynamic TLS model"
fi
objdump -d --disassemble=__errno_location "$candidate" >"$errno_disassembly"
grep -Eq '%fs:0x0|%fs:-' "$errno_disassembly" ||
    fail "candidate errno does not use direct fs initial TLS"
grep -Eq 'call.*<dn_skipname>' "$ns_skiprr_disassembly" ||
    fail "ns_skiprr does not call its selected dn_skipname dependency"
grep -Eq 'call.*<ns_get16>' "$ns_skiprr_disassembly" ||
...
for forbidden_candidate_symbol in ns_initparse ns_parserr ns_name_uncompress __res_state res_init \
    ...
    endhostent endnetent endprotoent h_errno hstrerror inet_addr inet_aton \
...
if grep -Eq 'crabc_core|mimalloc|sha_crypt' \
    "$candidate_symbols" "$ns_skiprr_disassembly"; then
...
if env -i LC_ALL=C TZ=UTC "$candidate"; then
    ...
    status=$?
    fail "freestanding nameser wire aggregate fixture exited ${status}"
fi

printf 'x86 static crabc-libc nameser wire aggregate: PASS\n'
