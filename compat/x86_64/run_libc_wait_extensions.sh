#!/usr/bin/env bash
# Native Linux/x86-64 private static crabc-libc wait3/wait4 evidence.
#
# The same GNU project-header C fixture runs against pinned musl 1.2.6 and a
# true `-nostdlib -static` candidate. Raw child/pipe/exit operations belong to
# the fixture only; the selected C ABI surface remains exactly wait3/wait4.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc wait extensions: %s\n' "$*" >&2
    exit 1
}
...
assert_named_syscall() {
    ...
    grep -Eq "\\\$0x${syscall_word}" "$disassembly" ||
        fail "${symbol} lacks Linux wait4 syscall ${syscall_word}"
...
candidate="$work_dir/crabc-static-wait-extensions-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I "$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_wait_extensions_probe.c >/dev/null 2>"$header_trace"
for header in errno.h sys/resource.h sys/time.h sys/syscall.h sys/types.h \
    sys/wait.h bits/syscall.h features.h; do
...
if grep -Eq 'crabc_core|mimalloc|sha_crypt' "$archive_symbols"; then
    fail "archive selects an unowned runtime dependency"
fi

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    -DCRABC_WAIT_EXTENSIONS_FREESTANDING -I "$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_wait_extensions_probe.c \
    compat/x86_64/libc_wait_extensions_start.S "$archive" -o "$candidate"
...
for symbol in __errno_location wait3 wait4; do
...
# wait4=61 (0x3d) takes rusage in Linux/x86-64 r10. wait3 may retain musl's
# source-level delegation as a call or inline it, but in either case it must
# reach the same wait4 syscall path.
assert_named_syscall wait4 3d
wait4_disassembly="$work_dir/wait4-disassembly"
objdump -d --disassemble=wait4 "$candidate" >"$wait4_disassembly"
grep -Fq '%r10' "$wait4_disassembly" ||
    fail "wait4 lacks the x86 r10 rusage argument path"
wait3_disassembly="$work_dir/wait3-disassembly"
objdump -d --disassemble=wait3 "$candidate" >"$wait3_disassembly"
if ! grep -Eq '<wait4>|\$0x3d' "$wait3_disassembly"; then
