#!/usr/bin/env bash
# Native Linux/x86-64 bounded static pthread mutex-attribute type-query evidence.
#
...
# archive. It proves exactly pthread_mutexattr_gettype's raw four-byte record
# projection: only bits 0 and 1 are observed, including raw value 3, and the
# caller-owned word is unchanged. It does not select the setter, attribute
# lifecycle, mutex initialization or operation, threads, TLS, synchronization,
...
fail() {
    printf 'ERROR: x86 static pthread mutexattr type query: %s\n' "$*" >&2
    exit 1
}
...
assert_no_unselected_mutexattr_exports() {
    ...
    for unselected in pthread_mutexattr_settype \
        ...
        [ "$symbols_path" = "$candidate_symbols" ] ||
            fail "mutexattr type-query sibling exclusions apply only to the final candidate"
...
assert_direct_record_path() {
    local disassembly="$work_dir/pthread_mutexattr_gettype-disassembly"

    objdump -d --disassemble=pthread_mutexattr_gettype "$candidate" >"$disassembly"
    if grep -Eq '[[:space:]](syscall|call)([[:space:]]|$)|%fs:' "$disassembly"; then
        fail "pthread_mutexattr_gettype must remain a direct TLS-free, syscall-free record query"
    fi
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pthread-mutexattr-type-query.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-pthread-mutexattr-type-query-reference"
candidate="$work_dir/crabc-static-pthread-mutexattr-type-query-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_mutexattr_type_query_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -pthread -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_pthread_mutexattr_type_query_probe.c \
    -o "$reference"
if timeout "$EXECUTION_TIMEOUT" "$reference"; then
    :
else
    status=$?
    fail "pinned-musl mutexattr type-query fixture exited ${status}"
fi
...
grep -Eq '[[:space:]][TW][[:space:]]pthread_mutexattr_gettype$' "$archive_symbols" ||
    fail "archive does not define pthread_mutexattr_gettype"
...
for marker in 'src/thread/pthread_attr_get.c::pthread_mutexattr_gettype' \
    '*type = a->__attr & 3' \
    'does not select `pthread_mutexattr_settype`'; do
    grep -Fq "$marker" libc/src/c_abi/x86_64/pthread_mutexattr_type_query.rs ||
        fail "pthread mutexattr type-query source lacks ${marker}"
done
if grep -Eq 'use super|raw_syscall::|static_tls::|pthread_mutex::|atomic::' \
    libc/src/c_abi/x86_64/pthread_mutexattr_type_query.rs; then
    fail "pthread mutexattr type-query source must not import a runtime seam"
fi

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    -DCRABC_PTHREAD_MUTEXATTR_TYPE_QUERY_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_pthread_mutexattr_type_query_probe.c \
    compat/x86_64/libc_pthread_mutexattr_type_query_start.S "$archive" -o "$candidate"
...
grep -Eq '[[:space:]]pthread_mutexattr_gettype$' "$candidate_symbols" ||
    fail "candidate does not define pthread_mutexattr_gettype"
...
if timeout "$EXECUTION_TIMEOUT" "$candidate"; then
    ...
    status=$?
    fail "freestanding mutexattr type-query fixture exited ${status}"
fi

printf 'x86 static crabc-libc pthread mutexattr type query: PASS\n'
