#!/usr/bin/env bash
# Native Linux/x86-64 TCP header declaration, feature-gate, and layout gate.
#
# Pinned musl 1.2.6 is the declaration/value/layout oracle. The candidate uses
# raw GCC with project headers and only compiler builtin headers, so an ambient
# libc cannot mask a netinet/tcp.h mismatch. This is compile-only evidence: it
# makes no claim about TCP socket-option behavior or a crabc C implementation.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/tcp_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/tcp_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu c11-bsd cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 TCP header ABI: %s\n' "$*" >&2
    exit 1
}
...
check_trace() {
    ...
    local trace="$3"
    local root path
    local -a required=(netinet/tcp.h features.h)
    ...
    case "$profile" in
        c11-gnu|c11-bsd|cxx17-gnu)
            for header in sys/types.h sys/socket.h stdint.h; do
                grep -Fq "$root/$header" "$trace" ||
                    fail "$profile $tree trace omitted gated $root/$header"
            done
            ;;
        *)
            for header in sys/types.h sys/socket.h stdint.h; do
                if grep -Fq "$root/$header" "$trace"; then
                    fail "$profile $tree trace unexpectedly included gated $root/$header"
                fi
            done
            ;;
    esac
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-tcp-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree TCP declaration/layout failed"
        check_trace "$tree" "$profile" "$trace"
    done
done

printf 'x86 pinned-musl/project C/C++ <netinet/tcp.h> ABI: PASS (%s feature profiles)\n' \
    "${#PROFILES[@]}"
