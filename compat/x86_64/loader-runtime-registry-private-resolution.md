...
global, default-visible `NOTYPE` symbol.  The loader's build provenance and
the complete ELF facts bind that source decision to the supplied shared
product and interpreter.
...
`validate-report`.  A copied PASS line cannot replace those joins.
