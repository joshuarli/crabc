#!/usr/bin/env bash
# Pinned-musl binary80 scalar-math differential, raw-closure, and installed-link gate.
#
# The target implementation is already Rust-owned: fmal/hypotl are checked
# generator output included by math_elementary_long_double.rs, and log1pl is
...
readonly ELEMENTARY_ASSEMBLY_SHA256=80bfd051eea4798b31abf2487c9ffd915a59058698b51b4459bb0c02c5e4b7ba
...
readonly ROOT_SIBLINGS=(sqrtl)

fail() { printf 'ERROR: x86 owned binary80 math: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d "$TMPDIR/crabc-x86-owned-math-binary80.XXXXXX")"
cleanup() {
	local status=$?
	trap - EXIT
	if [ "$status" -eq 0 ]; then
		...
		printf 'x86 owned binary80 math: retained failure evidence at %s\n' "$work_dir" >&2
	fi
	exit "$status"
}
trap cleanup EXIT
...
probe="$ROOT_DIR/compat/x86_64/owned_static_math_binary80_consumer.c"
start="$ROOT_DIR/compat/x86_64/owned_static_math_binary80_consumer_start.S"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H "$probe" \
	>/dev/null 2>"$trace"
for header in fenv.h float.h math.h stddef.h stdint.h unistd.h features.h bits/alltypes.h; do
	grep -Fq "$ROOT_DIR/include/$header" "$trace" || fail "fixture did not use project $header"
done

# The installed musl stream is the semantic reference for the sealed driver.
# The separate raw stream keeps CRT, allocator, and ambient libm out of the
# direct binary80 provider closure.
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_OWNED_STATIC_BINARY80_MATH_FREESTANDING \
...
for symbol in "${SYMBOLS[@]}"; do
	grep -Eq "[[:space:]]T[[:space:]]${symbol}$" "$archive_symbols" ||
		fail "feature archive does not provide a strong Rust-owned ${symbol}"
done

# The aggregate archive intentionally retains unrelated runtime state. Extract
# the source-derived fmal/hypotl/log1pl closure for the raw no-ambient-runtime
# proof; the two installed executions cover the real CRT/TLS product path.
mkdir "$raw_members"
...
if grep -Eq 'crabc_core|mimalloc|float_parse|math_complex|libm' "$candidate_symbols" "$disassembly"; then
	fail "raw candidate selects an unowned runtime dependency"
fi
for instruction in fldt fstpt fsqrt fyl2xp1 faddp fmulp; do
	grep -Eq "[[:space:]]${instruction}([[:space:]]|$)" "$disassembly" ||
		fail "raw candidate lacks selected ${instruction} path"
done
for sibling in "${FENV_SIBLINGS[@]}" "${SPECIAL_SIBLINGS[@]}" "${ROOT_SIBLINGS[@]}"; do
	grep -Eq "(call|jmp)[[:space:]].*<${sibling}>" "$disassembly" ||
		fail "raw candidate does not retain source-level ${sibling} sibling call"
done
if grep -Eq '[[:space:]](v[a-z0-9]+|addp[sd]|subp[sd]|mulp[sd]|divp[sd]|sqrtp[sd]|vfmadd|vfnmadd|vfmsub|vfnmsub)([[:space:]]|$)' \
	"$disassembly"; then
	fail "raw candidate accidentally retains vector/FMA scalar-provider replacement"
fi
...
cmp -s "$reference_raw_output" "$candidate_raw_output" || {
	...
	fail "raw binary80 result/fenv/rounding record stream differs from pinned musl"
}
...
def fail(message: str) -> None:
    raise SystemExit(f"owned binary80 math installed receipt: {message}")
...
PY
	...
	cmp -s "$reference_installed_output" "$candidate_output" || {
		...
		fail "${label} installed binary80 result/fenv/rounding record stream differs from pinned musl"
	}
}
...
printf 'x86 owned binary80 math: PASS (%s records; raw x87/fenv + installed link in ET_EXEC and static PIE)\n' \
	"$EXPECTED_RECORDS"
