#!/usr/bin/env bash
# Pinned-musl differential for the private x86 scalar sincos/sincosf artifact.
set -euo pipefail
...
readonly EXPECTED_RECORDS=512
readonly SELECTED_SYMBOLS=(sincos sincosf)
readonly FENV_SIBLINGS=(feclearexcept fegetenv fegetround fesetenv fesetround fetestexcept)
readonly PRIVATE_PROVIDERS=(
	crabc_x86_math_sincos_kernel_sin
	crabc_x86_math_sincos_kernel_cos
	crabc_x86_math_sincos_kernel_sindf
	crabc_x86_math_sincos_kernel_cosdf
	crabc_x86_math_sincos_reduce_pio2
	crabc_x86_math_sincos_reduce_pio2f
	crabc_x86_math_sincos_reduce_pio2_large
	crabc_x86_math_sincos_provider_floor
	crabc_x86_math_sincos_provider_scalbn
)

fail() { printf 'ERROR: x86 static libc sincos/sincosf: %s\n' "$*" >&2; exit 1; }
...
bash "$ROOT_DIR/compat/x86_64/run_math_sincos_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-sincos.XXXXXX)"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_sincos_probe.c >/dev/null 2>"$trace"
...
"$reference" >"$reference_output" || fail "pinned-musl sincos/sincosf fixture failed"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_SINCOS_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_sincos_probe.c \
	compat/x86_64/libc_math_sincos_start.S "$archive" -o "$candidate"
...
for unselected in sin sinf sinl sincosl cos cosf cosl tan tanf tanl \
	...
	acoshf acoshl atanh atanhf atanhl log logf logl log1p log1pf log1pl \
	log2 log2f log2l log10 log10f log10l exp expf expl exp2 exp2f exp2l \
