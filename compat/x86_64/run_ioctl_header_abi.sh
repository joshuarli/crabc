#!/usr/bin/env bash
# Native Linux/x86-64 direct <sys/ioctl.h> ABI profile matrix.
#
# Pinned musl is the declaration and macro oracle. The candidate side uses raw
# GCC with only project headers and raw-GCC builtin headers, so an ambient
# libc cannot conceal a public-header mismatch. This is compile-only evidence:
# it proves neither callable linkage nor device/request behavior.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/ioctl_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/ioctl_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 sys/ioctl.h ABI: %s\n' "$*" >&2
    exit 1
}
...
[ -f "$C_PROBE" ] || fail "missing C ioctl header ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ ioctl header ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-ioctl-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ <sys/ioctl.h> ABI matrix: PASS (%s profiles; compile-only)\n' \
