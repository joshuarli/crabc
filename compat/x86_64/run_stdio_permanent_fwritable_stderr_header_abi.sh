#!/usr/bin/env bash
# Native Linux/x86-64 permanent-stderr __fwritable <stdio_ext.h> ABI proof.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_fwritable_stderr_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_fwritable_stderr_header_abi_probe.cpp"
readonly -a PROFILES=(c11 cxx17)

fail() {
    printf 'ERROR: x86 stdio permanent-stderr __fwritable header ABI: %s\n' "$*" >&2
    exit 1
}
    ...
    case "$1" in
        c11)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
                ...
                -DCRABC_STDIO_PERMANENT_FWRITABLE_STDERR_C11
            ;;
        cxx17)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
                -U_POSIX_C_SOURCE -U_LARGEFILE64_SOURCE -U_DEFAULT_SOURCE \
                -DCRABC_STDIO_PERMANENT_FWRITABLE_STDERR_CXX17
            ;;
...
assert_cxx_c_linkage() {
    ...
    grep -Fxq __fwritable <<<"$undefined" ||
        fail "$tree C++ probe does not retain C spelling __fwritable"
    if grep -Eq '_Z.*__fwritable' <<<"$undefined"; then
        fail "$tree C++ probe retained a mangled __fwritable reference"
    fi
}
...
[ -f "$C_PROBE" ] || fail "missing C permanent-stderr __fwritable header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ permanent-stderr __fwritable header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-stdio-permanent-fwritable-stderr-header.XXXXXX)"
...
printf 'x86 stdio permanent-stderr __fwritable header ABI: PASS\n'
