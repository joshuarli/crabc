#!/usr/bin/env bash
# Native Linux/x86-64 selected lchown C/C++ header ABI matrix.
#
...
# proves one no-follow pathname ownership leaf, not sibling ownership APIs,
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/lchown_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/lchown_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 lchown headers: %s\n' "$*" >&2
    exit 1
}
...
[ -f "$C_PROBE" ] || fail "missing C lchown ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ lchown ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-lchown-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ lchown ABI matrix: PASS (%s profiles; visible=%s hidden=%s; compile-only)\n' \
