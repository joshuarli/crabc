#!/usr/bin/env bash
# Pinned-musl differential for the private x86 scalar acosh/acoshf artifact.
set -euo pipefail
...
readonly PRIVATE_PROVIDERS=(
	...
	crabc_x86_math_acosh_provider_logf_data
	crabc_x86_math_acosh_provider_rsqrt_tab
	crabc_x86_math_acosh_provider_divzero
	crabc_x86_math_acosh_provider_divzerof
	crabc_x86_math_acosh_provider_invalid
	crabc_x86_math_acosh_provider_invalidf
)

fail() { printf 'ERROR: x86 static libc acosh/acoshf: %s\n' "$*" >&2; exit 1; }
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_acosh_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -fno-builtin \
	-fno-stack-protector compat/x86_64/libc_math_acosh_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl acosh/acoshf fixture failed"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_ACOSH_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_acosh_probe.c \
	compat/x86_64/libc_math_acosh_start.S "$archive" -o "$candidate"
...
for unselected in sin sinf sinl sincos sincosf sincosl cos cosf cosl tan tanf \
	tanl tanh tanhf tanhl sinh sinhf sinhl cosh coshf coshl asinh asinhf asinhl \
	acoshl asin asinf asinl acos acosf acosl atan atanf atanl atanh atanhf atanhl \
	atan2 atan2f atan2l log logf logl log1p log1pf log1pl log2 log2f log2l \
	log10 log10f log10l exp expf expl exp2 exp2f exp2l exp10 exp10f exp10l \
	pow powf powl fma fmaf fmal \
...
"$candidate" >"$candidate_output" || fail "freestanding acosh/acoshf fixture failed"
