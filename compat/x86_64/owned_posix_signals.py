...
def contained_directory(path, label):
    ...
    if not physical.is_dir() or not physical.is_relative_to(ROOT / ".work"):
        raise ValueError(f"signal-full {label} must be a physical checkout .work directory")
    return physical


def command(arguments, output):
    with Path(str(output) + ".stdout").open("wb") as stdout, Path(str(output) + ".stderr").open("wb") as stderr:
        subprocess.run([str(argument) for argument in arguments], check=True, cwd=ROOT, stdout=stdout, stderr=stderr)
...
def audit_link(executable, receipt_path, workload, product):
    receipt = json.loads(receipt_path.read_text())
    records = receipt["input_receipts"]
    if not any(record["path"] == str(workload) and record["sha256"] == digest(workload) for record in records):
        raise ValueError("signal link receipt omitted the shared workload object")
    if receipt.get("output_sha256", receipt.get("output", {}).get("sha256")) != digest(executable):
        raise ValueError("signal link receipt output identity differs")
    for record in records:
        if digest(product / record["path"]) != record["sha256"]:
            raise ValueError("signal link input identity differs")
...
def run(arguments):
    if len(arguments) > 1:
        raise ValueError("usage: run_owned_posix_signals.sh [DYNAMIC_SYSROOT]")
    temporary = contained_directory(os.environ.get("TMPDIR", ""), "TMPDIR")
    product = contained_directory(arguments[0], "product") if arguments else None
    ...
    try:
        ...
        if not arguments:
            ...
            for mode in ("static", "static-pie"):
                ...
                command([static / "bin/crabc-cc", "-" + mode, "--link-receipt", receipt.relative_to(ROOT), workload, "-o", executable], work / f"link-{mode}")
                audit_link(executable, receipt, workload, static)
        ...
        for mode in ("pie", "non-pie"):
            ...
            audit_link(executable, Path(str(executable) + ".crabc-link.json"), workload, product)
        ...
        receipt = {"source_sha256": source_identity, "revision": revision, "schema": "crabc.x86_64-owned-posix-signals/v1", "workload_object_sha256": digest(workload),
                   ...
                   "reused_cases": contract["reused_cases"], "failures": failures}
