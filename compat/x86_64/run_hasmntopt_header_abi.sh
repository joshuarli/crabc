#!/usr/bin/env bash
# Native Linux/x86-64 selected hasmntopt C/C++ header ABI matrix.
#
# Pinned musl 1.2.6 owns this unconditional declaration, `struct mntent`
# layout, and unmangled C++ C-linkage oracle. The candidate uses raw GCC with
# only project headers and compiler builtin headers, so an ambient libc cannot
# hide a mismatch. This proves one caller-owned option-string lookup, not
# mount-table streams, parsing, state, pathname policy, or public x86 support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/hasmntopt_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/hasmntopt_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 hasmntopt headers: %s\n' "$*" >&2
    exit 1
}
...
[ -f "$C_PROBE" ] || fail "missing C hasmntopt ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ hasmntopt ABI probe"
...
printf 'x86 pinned-musl/project C/C++ hasmntopt ABI matrix: PASS (%s profiles; visible=%s hidden=%s; compile-only)\n' \
