#!/usr/bin/env bash
# Native Linux/x86-64 pthread spin-destruction C/C++ declaration gate.
#
...
# ambient libc cannot mask an ABI or C++ linkage mismatch. This checks one
# unconditionally declared opaque-pointer spelling, not spin initialization,
# lock/trylock/unlock, synchronization, or a general pthread runtime.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/pthread_spin_destroy_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/pthread_spin_destroy_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 pthread_spin_destroy headers: %s\n' "$*" >&2
    exit 1
}
...
check_cxx_linkage() {
    ...
    grep -Eq '[[:space:]]pthread_spin_destroy$' <<<"$undefined" ||
        fail "$profile $tree C++ probe does not retain C linkage for pthread_spin_destroy"
    if grep -Eq '_Z[0-9].*pthread_spin_destroy' <<<"$undefined"; then
        fail "$profile $tree C++ probe retained a mangled pthread_spin_destroy reference"
    fi
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pthread-spin-destroy-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional pthread_spin_destroy declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <pthread.h> pthread_spin_destroy ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
