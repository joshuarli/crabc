#!/usr/bin/env bash
# Pinned-musl differential for the private x86 scalar exp2/exp2f artifact.
set -euo pipefail
...
readonly LOCAL_PROVIDERS=(
	crabc_x86_math_exp2_data
	crabc_x86_math_exp2_data_f
	crabc_x86_math_exp2_provider_oflow
	crabc_x86_math_exp2_provider_uflow
	crabc_x86_math_exp2_provider_xflow
	crabc_x86_math_exp2_provider_oflowf
	crabc_x86_math_exp2_provider_uflowf
	crabc_x86_math_exp2_provider_xflowf
)

fail() { printf 'ERROR: x86 static libc exp2/exp2f: %s\n' "$*" >&2; exit 1; }
...
bash "$ROOT_DIR/compat/x86_64/run_math_exp2_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-exp2.XXXXXX)"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_exp2_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -fno-builtin \
	-fno-stack-protector compat/x86_64/libc_math_exp2_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl exp2/exp2f fixture failed"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_EXP2_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_exp2_probe.c \
	compat/x86_64/libc_math_exp2_start.S "$archive" -o "$candidate"
...
for provider in "${LOCAL_PROVIDERS[@]}"; do
	grep -Fq "$provider" "$candidate_symbols" || fail "candidate lacks local ${provider}"
	if grep -Eq "[[:space:]]GLOBAL[[:space:]].*${provider}$" "$candidate_symbols"; then
		fail "candidate exposes private ${provider}"
	fi
done
for unselected in exp2l exp expf expl expm1 expm1f expm1l exp10 exp10f exp10l \
...
for instruction in addsd addss subsd mulsd mulss cvtsd2ss cvtss2sd; do
...
printf 'x86 static libc exp2/exp2f: PASS (%s records)\n' "$record_count"
