#!/usr/bin/env python3
"""Reconstruct one bounded installed locale component receipt.

This reader is deliberately specific to ``run_owned_locale.sh``.  It does not
turn the selected locale, multibyte, and fixed iconv probe into family closure:
the report's flags remain non-promoting.  It instead makes a retained producer
report useful as a component input by replaying every recorded source, product,
tool, command, link, copied-payload, and oracle relation.
"""
...
class LocaleReceiptError(RuntimeError):
    """A locale component receipt is incomplete or no longer reconstructs."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise LocaleReceiptError(message)
...
def read_json(path: Path, description: str) -> object:
    try:
        ...
        raise LocaleReceiptError(f"{description} is not valid JSON: {path}") from error
...
def physical_directory(path: Path, description: str) -> Path:
    try:
        ...
        raise LocaleReceiptError(f"{description} is unreadable: {path}") from error
...
def identity(root: Path, path: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise LocaleReceiptError(f"receipt artifact escapes checkout: {path}") from error
...
def tracked_source_identity(root: Path, relative: str) -> dict[str, object]:
    path = checkout_path(root, relative, "locale reader source")
...
def source_product_seal(root: Path, static: Path | None, dynamic: Path) -> dict[str, object]:
    ...
    dynamic = checkout_path(root, dynamic.relative_to(root).as_posix(), "dynamic locale product", directory=True)
    try:
    ...
    if static is not None:
        static = checkout_path(root, static.relative_to(root).as_posix(), "static locale product", directory=True)
        try:
            static_manifest, _ = products._validate_static_product(static)
        except products.ProductEvidenceError as error:
            raise LocaleReceiptError(f"static locale product validation failed: {error}") from error
...
def tool_roster(root: Path, dynamic: Path, static: Path | None) -> dict[str, object]:
    ...
    spec = importlib.util.spec_from_file_location("owned_locale_component_tools", helper)
...
def mounted(root: Path, path: Path) -> str:
    try:
        ...
        raise LocaleReceiptError(f"command path escapes checkout: {path}") from error


def command_plan(paths: Mapping[str, object], tools: Mapping[str, object], mode: str) -> dict[str, list[str]]:
    """Return the exact retained argv map for the finite locale component."""

    require(mode in {FULL_MODE, DYNAMIC_MODE}, "locale execution mode differs")
    ...
    require(isinstance(executables, Mapping), "locale executable map differs")
...
def parse_argv(raw: bytes, label: str) -> list[str]:
    try:
        ...
        raise LocaleReceiptError(f"{label} retained argv is invalid JSON") from error
...
def header_trace_paths(stderr: bytes) -> tuple[str, ...]:
    ...
    try:
        ...
        raise LocaleReceiptError("installed header trace is not UTF-8") from error
    paths: list[str] = []
    for line in lines:
...
def validate_report(root: Path, report_path: Path, *, require_static: bool = False) -> dict[str, object]:
    ...
    report = read_json(report_path, "locale component report")
    expected_fields = {
    ...
    require(isinstance(report, dict) and set(report) == expected_fields, "locale component report fields differ")
    require(report["schema"] == SCHEMA and report["source_mount"] == SOURCE_MOUNT and report["scope"] == list(SCOPE),
            "locale component report contract differs")
    require(report["family_completion"] is False and report["promotion_ready"] is False and report["public_support"] is False,
            "locale component receipt is promoting")
    mode = report["execution_mode"]
    require(mode in {FULL_MODE, DYNAMIC_MODE}, "locale component execution mode differs")
    if require_static:
        require(mode == FULL_MODE, "locale component acceptance requires static/static-pie evidence")
    ...
    require(isinstance(products_record, dict) and set(products_record) == expected_product_keys,
            "locale component product roster differs")
    dynamic = checkout_path(root, products_record["dynamic"], "dynamic locale product", directory=True)
    static = checkout_path(root, products_record["static"], "static locale product", directory=True) if mode == FULL_MODE else None
    ...
    paths: dict[str, object] = {
        "root": root, "work": work,
        "probe": checkout_path(root, SOURCE_PATHS["probe"], "locale probe"),
        "runner": checkout_path(root, SOURCE_PATHS["runner"], "locale runner"),
        "reader": checkout_path(root, SOURCE_PATHS["reader"], "locale reader"),
    ...
    require(isinstance(commands, dict) and set(commands) == set(plan), "locale command roster differs")
    ...
    require(isinstance(links, dict) and set(links) == set(expected_links), "locale product link roster differs")
...
def main() -> int:
    ...
    except (LocaleReceiptError, OSError, ValueError, products.ProductEvidenceError, copies.CryptRuntimeEvidenceError) as error:
        parser.exit(1, f"owned locale component receipt failed: {error}\n")
    print("owned locale component receipt: valid; bounded component evidence remains non-promoting")
    return 0
