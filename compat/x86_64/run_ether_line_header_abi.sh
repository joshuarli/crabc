#!/usr/bin/env bash
# Native Linux/x86-64 legacy Ethernet-line C/C++ declaration gate.
#
...
# ambient libc cannot mask the declaration, six-octet layout, or C++ linkage.
# The declaration is unconditional in musl's netinet/ether.h; this checks
# strict, POSIX, X/Open, and GNU C/C++ profiles without selecting the adjacent
# Ethernet conversion, /etc/ethers, resolver, socket, or interface behavior.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/ether_line_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/ether_line_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 ether_line headers: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-ether-line-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional ether_line declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <netinet/ether.h> ether_line ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
