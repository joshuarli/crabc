#!/usr/bin/env bash
# Pinned-musl differential for the private x86 scalar pow/powf artifact.
set -euo pipefail
...
readonly PRIVATE_PROVIDERS=(
	crabc_x86_math_pow_provider_exp_data
	crabc_x86_math_pow_provider_log_data
	crabc_x86_math_pow_provider_exp2f_data
	crabc_x86_math_pow_provider_log2f_data
	crabc_x86_math_pow_provider_invalid
	crabc_x86_math_pow_provider_invalidf
	crabc_x86_math_pow_provider_oflow
	crabc_x86_math_pow_provider_oflowf
	crabc_x86_math_pow_provider_uflow
	crabc_x86_math_pow_provider_uflowf
	crabc_x86_math_pow_provider_xflow
	crabc_x86_math_pow_provider_xflowf
	crabc_x86_math_pow_provider_fabs
)

fail() { printf 'ERROR: x86 static libc pow/powf: %s\n' "$*" >&2; exit 1; }
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_pow_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -fno-builtin \
	-fno-stack-protector compat/x86_64/libc_math_pow_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl pow/powf fixture failed"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_POW_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_pow_probe.c \
	compat/x86_64/libc_math_pow_start.S "$archive" -o "$candidate"
...
for unselected in powl exp expf expl exp2 exp2f exp2l expm1 expm1f expm1l \
	exp10 exp10f exp10l pow10 pow10f pow10l log logf logl log1p log1pf log1pl \
	log2 log2f log2l log10 log10f log10l fabs fabsf fabsl fma fmaf fmal \
	sin sinf sinl sincos sincosf sincosl cos cosf cosl tan tanf tanl \
	sinh sinhf sinhl cosh coshf coshl tanh tanhf tanhl asin asinf asinl acos \
	acosf acosl atan atanf atanl atan2 atan2f atan2l asinh asinhf asinhl acosh \
	acoshf acoshl atanh atanhf atanhl fmod fmodf fmodl remainder remainderf \
	remainderl remquo remquof remquol modf modff modfl floor floorf floorl \
	ceil ceilf ceill trunc truncf truncl round roundf roundl sqrt sqrtf sqrtl \
	cbrt cbrtf cbrtl scalbn scalbnf scalbnl scalbln scalblnf cabs cabsf cabsl \
