...
extract_selected_member() {
    ...
    (
        ...
        for member in "${members[@]}"; do
            ...
            if printf '%s\n' "$definitions" |
                grep -Eq '[[:space:]][T][[:space:]]ns_put32$'; then
                if printf '%s\n' "$definitions" |
                    grep -Eq '[[:space:]][T][[:space:]](dn_expand|dn_skipname|ns_get16|ns_get32|ns_put16|ns_initparse|ns_parserr|ns_skiprr|ns_name_uncompress)$'; then
...
readelf --symbols --wide "$musl_object" |
    grep -Eq '[[:space:]]5[[:space:]]+FUNC[[:space:]]+GLOBAL[[:space:]]+DEFAULT[[:space:]]+[0-9]+[[:space:]]+ns_put32$' ||
