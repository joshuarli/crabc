#!/usr/bin/env bash
# Native Linux/x86-64 permanent-stream-status <stdio.h> declaration/linkage proof.
#
# Pinned musl 1.2.6 is the header oracle. The candidate compiles against only
# the project and compiler-builtin include trees, so an ambient libc cannot
# hide a declaration or C++ linkage mismatch. This selects no runtime.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_status_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_status_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 stdio permanent-stream status header ABI: %s\n' "$*" >&2
    exit 1
}
...
compile_c() {
    ...
    run_compiler "$compiler" -x c -std=c11 -nostdinc -I "$include_root" \
        ...
        -DCRABC_STDIO_PERMANENT_STATUS_C11 -fsyntax-only "$C_PROBE" \
        >/dev/null 2>"$trace"
}
...
compile_cxx() {
    ...
    run_compiler "$compiler" -x c++ -std=c++17 -nostdinc -nostdinc++ \
        ...
        -H -fno-builtin -DCRABC_STDIO_PERMANENT_STATUS_CXX17 \
...
[ -f "$C_PROBE" ] || fail "missing C permanent-stream-status header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ permanent-stream-status header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-stdio-permanent-status-header.XXXXXX)"
...
printf 'x86 stdio permanent-stream status header ABI: PASS\n'
