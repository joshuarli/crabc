#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc mapping-synchronization gate.
#
# The same project-header C fixture executes through pinned musl 1.2.6 before
# a true -nostdlib -static candidate linked only with the selected archive. It
# selects exactly direct no-cancellation `msync`: valid Linux MS flag forms,
# zero-length and stale-errno success, and invalid mode/alignment EINVAL.
# Its raw disposable mapping is private and anonymous, so this does not prove
# file-backed shared-map writeback, invalidation effects, persistence, or
# durability. Pinned musl routes this API through syscall_cp; this static
# archive has no cancellation state and deliberately does not claim that
# contract. It does not select mremap, mlock*, shared memory, VM-wide
# synchronization, mapping
# policy, allocator, libc.so, CRT, loader, sysroot, or public x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc memory sync: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_memory_sync_header_abi.sh" >/dev/null
bash "$ROOT_DIR/compat/x86_64/run_x86_msync_reference.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-memory-sync.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-memory-sync-reference"
candidate="$work_dir/crabc-static-memory-sync-candidate"
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_memory_sync_probe.c \
    -o "$reference"
if "$reference"; then
    :
else
...
for symbol in __errno_location msync; do
...
for unrelated in mlock mlock2 munlock mlockall munlockall mmap mprotect munmap \
    madvise mincore posix_madvise mremap remap_file_pages shm_open shm_unlink \
    memfd_create malloc free calloc realloc pthread_create pthread_exit \
    pthread_join pthread_cancel pthread_testcancel __syscall_cp __cancel __vm_wait; do
    if grep -Eq "[[:space:]]${unrelated}$" "$candidate_symbols"; then
        fail "memory-sync candidate unexpectedly pulls ${unrelated}"
    fi
done
...
assert_fixture_tls_capacity
if grep -Eq 'TLSGD|TLSLD|TLSDESC|GOTTPOFF|DTPMOD(64)?|DTPOFF(32|64)?|__tls_get_addr|__syscall_cp|pthread_cancel|crabc_core|mimalloc|sha_crypt' \
    "$candidate_relocations" "$candidate_symbols" "$candidate_disassembly"; then
    fail "candidate retains dynamic TLS, cancellation, or an unowned dependency"
fi
...
if "$candidate"; then
    ...
    status=$?
    fail "freestanding memory-sync fixture exited ${status}"
fi

printf 'x86 static crabc-libc memory synchronization: PASS\n'
