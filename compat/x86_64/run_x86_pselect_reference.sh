#!/usr/bin/env bash
# Native Linux/x86-64 pinned-musl pselect6 ABI and behavior reference.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 pselect reference: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pselect.XXXXXX)"
...
printf 'x86 pinned-musl pselect ABI/behavior reference: PASS\n'
