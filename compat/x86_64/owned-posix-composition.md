...
Run `./scripts/dev-x86_64.sh owned-posix-composition [DYNAMIC_SYSROOT]`.
The runner compiles one object with installed project headers and the installed
dynamic driver, then uses that object for pinned musl and every candidate.
Without a supplied product it runs static and static PIE as well as dynamic
PIE/non-PIE through kernel and direct interpreter entry. The registered
`posix-composition` case allows the dynamic coordinator to replay its supplied
installed, second, and extracted products. Static reproduction/extraction and
the family receipt remain coordinator obligations.
