...
fail() {
	printf 'ERROR: x86 C fenv source-only probe: %s\n' "$*" >&2
	exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-c-fenv.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
object="$work_dir/fenv.o"
reference="$work_dir/musl-fenv-reference"
candidate="$work_dir/crabc-fenv-candidate"
...
cd "$ROOT_DIR"
"$ORACLE_CC" -std=c11 -fno-builtin compat/x86_64/libc_fenv_probe.c -o "$reference"
"$reference"

cc -E -H -I"$ROOT_DIR/include" compat/x86_64/libc_fenv_probe.c \
	>/dev/null 2>"$header_trace"
grep -Fq "$ROOT_DIR/include/fenv.h" "$header_trace" || {
...
rustup run nightly-2026-07-24 rustc --edition=2021 \
	...
	-C code-model=small \
	-C panic=abort \
	compat/x86_64/libc_fenv_probe.rs \
	-o "$object"
...
for symbol in feclearexcept feraiseexcept __fesetround fegetround fegetenv \
	fesetenv fetestexcept fegetexceptflag feholdexcept fesetexceptflag \
	fesetround feupdateenv __flt_rounds; do
...
if grep -Eq 'crabc_core|crabc_libc|__tls_get_addr' "$object_relocations"; then
	fail "source-only fenv object depends on a runtime artifact or dynamic TLS"
fi
...
cc -no-pie -fno-builtin -I"$ROOT_DIR/include" \
	compat/x86_64/libc_fenv_probe.c "$object" -o "$candidate"
readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --dyn-syms --wide "$candidate" >"$candidate_dynamic_symbols"
for symbol in feclearexcept feraiseexcept fegetround fegetenv fesetenv \
	fetestexcept fesetround; do
	grep -Eq "[[:space:]]${symbol}$" "$candidate_symbols" \
		|| fail "candidate does not define ${symbol}"
done
if grep -Eq '[[:space:]]UND[[:space:]].*(feclear|feraise|feget|feset|fetest|fehold|feupdate)' \
	"$candidate_dynamic_symbols"; then
	fail "candidate leaves a fenv symbol to the ambient C runtime"
fi
"$candidate"

printf 'x86 C fenv source-only probe: PASS\n'
