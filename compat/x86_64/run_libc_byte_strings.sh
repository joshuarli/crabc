#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc byte-string evidence.
#
...
fail() {
    printf 'ERROR: x86 static libc byte strings: %s\n' "$*" >&2
    exit 1
}
...
assert_selected_c_abi_surface() {
    ...
    # Inspect only crabc's C object members; compiler-builtins support does not
    # belong to the closed C export manifest. The hidden signal restorer is the
    # one audited frame-internal exception shared by existing static artifacts.
