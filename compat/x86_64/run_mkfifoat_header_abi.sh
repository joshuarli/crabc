#!/usr/bin/env bash
# Native Linux/x86-64 selected mkfifoat C/C++ header ABI matrix.
#
...
# proves exactly `mkfifoat`, not the mknod family, path policy, or public x86
# support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/mkfifoat_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/mkfifoat_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 mkfifoat headers: %s\n' "$*" >&2
    exit 1
}
...
[ -f "$C_PROBE" ] || fail "missing C mkfifoat ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ mkfifoat ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-mkfifoat-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ mkfifoat ABI matrix: PASS (%s profiles; compile-only)\n' \
