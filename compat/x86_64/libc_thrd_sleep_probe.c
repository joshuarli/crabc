/* Static crabc-libc x86-64 selected C11 thrd_sleep fixture.
 *
 ...
 * crabc libc.a. It proves only C11 thrd_sleep's zero/-1/-2 convention over
 * the selected clock_nanosleep path: zero completes, -1 means interruption,
 * and -2 is every other error. Fixture-local raw setitimer calls and the
 * already selected simple sigaction/mask boundary merely make interruption
 * deterministic; this body always calls C11 thrd_sleep, never the candidate
 * clock_nanosleep entry directly. It does not select cancellation,
 * thrd_yield, synchronization, TSS, lifecycle, CRT, loader, sysroot, or
 * public x86 support.
 */
...
_Static_assert(sizeof(sigset_t) == 128 && sizeof(struct sigaction) == 152,
    "x86 signal records");
_Static_assert(SYS_setitimer == 38, "x86 fixture timer syscall number");
_Static_assert(__builtin_types_compatible_p(__typeof__(&thrd_sleep),
    int (*)(const struct timespec *, struct timespec *)),
    "thrd_sleep declaration");

enum {
    thrd_sleep_interrupted = -1,
    thrd_sleep_error = -2,
};
...
static int check_immediate_and_error_conventions(void)
{
    ...
    if (thrd_sleep(&zero, 0) != thrd_success || errno != preserved_errno)
        return 1;

    errno = preserved_errno;
    if (thrd_sleep(&invalid, 0) != thrd_sleep_error ||
        errno != preserved_errno)
        return 2;

    errno = preserved_errno;
    if (thrd_sleep(0, 0) != thrd_sleep_error || errno != preserved_errno)
        return 3;
...
static int check_relative_interruption(void)
{
    ...
    if (result != thrd_sleep_interrupted || !signal_delivered ||
        !positive_remainder(&remaining) || errno != preserved_errno)
        status = 3;
...
int crabc_x86_64_thrd_sleep_probe(void)
{
    int status;
...
#ifndef CRABC_THRD_SLEEP_FREESTANDING
int main(void)
{
    return crabc_x86_64_thrd_sleep_probe();
}
#endif
