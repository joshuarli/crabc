...
fail() { printf 'ERROR: x86 static libc log10/log10f: %s\n' "$*" >&2; exit 1; }
...
bash "$ROOT_DIR/compat/x86_64/run_math_log10_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-log10.XXXXXX)"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -fno-builtin \
	-fno-stack-protector compat/x86_64/libc_math_log10_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl log10/log10f fixture failed"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_LOG10_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_log10_probe.c \
	compat/x86_64/libc_math_log10_start.S "$archive" -o "$candidate"
...
for unselected in log10l log logf logl log1p log1pf log1pl log2 log2f log2l \
...
for instruction in addsd addss subsd subss mulsd mulss divsd divss; do
...
"$candidate" >"$candidate_output" || fail "freestanding log10/log10f fixture failed"
if ! cmp -s "$reference_output" "$candidate_output"; then
	cmp -l "$reference_output" "$candidate_output" | sed -n '1,120p' >&2 || true
	fail "candidate log10/log10f record stream differs from pinned musl"
fi

printf 'x86 static libc log10/log10f: PASS (%s records)\n' "$record_count"
