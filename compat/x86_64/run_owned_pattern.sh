#!/usr/bin/env bash
# Pinned-musl filename-pattern differential through sealed installed x86 products.
#
# One C object built from the project headers is linked first with pinned musl,
# then with the owned static and dynamic products. Each runtime receives an
# isolated root so glob's directory, passwd, tilde, callback, and credential
# paths cannot reach host state.
set -euo pipefail
ulimit -c 0

readonly ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly PROBE="$ROOT/compat/x86_64/owned_pattern_probe.c"
readonly INTERPRETER=/lib/ld-crabc-x86_64.so.1
readonly OWNED_SYMBOLS='fnmatch glob globfree'
readonly CASES=(
    fnmatch-escaped
    fnmatch-range
    fnmatch-nested-class
    glob-literal
    glob-nested
    glob-dangling-mark
    all
)

[ "$#" -le 1 ] || { printf 'usage: %s [DYNAMIC_SYSROOT]\n' "$0" >&2; exit 2; }

provided_dynamic="${1:-}"
if [ -n "$provided_dynamic" ]; then
    provided_dynamic="$(realpath -e "$provided_dynamic")"
fi
python3 -B - "$ROOT" "${TMPDIR:-}" "$provided_dynamic" <<'PY'
from pathlib import Path
import sys

root = Path(sys.argv[1])
temporary = Path(sys.argv[2])
product_argument = sys.argv[3]
if not temporary.is_dir() or temporary.resolve() != temporary or not temporary.is_relative_to(root / '.work'):
    raise SystemExit('owned pattern TMPDIR must be a physical checkout .work directory')
if product_argument:
    product = Path(product_argument)
    if not product.is_dir() or not product.is_relative_to(root / '.work'):
        raise SystemExit('owned pattern product must be a checkout .work directory')
PY

readonly work="$(mktemp -d "$TMPDIR/owned-pattern.XXXXXX")"
chmod a+rx "$work"
printf 'owned pattern evidence: %s\n' "$work"

prepare_root() {
    local root="$1"
    mkdir -p "$root/etc" "$root/fixture/dir" "$root/fixture/home" \
        "$root/fixture/userhome" "$root/fixture/blocked"
    : >"$root/fixture/a.txt"
    : >"$root/fixture/b.txt"
    : >"$root/fixture/z.txt"
    : >"$root/fixture/.hidden"
    : >"$root/fixture/star*"
    : >"$root/fixture/dir/child.txt"
    : >"$root/fixture/home/home.txt"
    : >"$root/fixture/userhome/user.txt"
    : >"$root/fixture/blocked/private.txt"
    ln -s dir "$root/fixture/link-dir"
    ln -s absent "$root/fixture/dangling"
    printf '%s\n' \
        'root:x:0:0:root:/fixture/home:/bin/sh' \
        'tester:x:4242:4242:tester:/fixture/userhome:/bin/sh' \
        'nobody:x:65534:65534:nobody:/nonexistent:/bin/sh' >"$root/etc/passwd"
    chmod 755 "$root" "$root/etc" "$root/fixture" "$root/fixture/dir" \
        "$root/fixture/home" "$root/fixture/userhome"
    chmod 700 "$root/fixture/blocked"
}

run_in_root() {
    local root="$1" output="$2"
    shift 2
    timeout 30 env -i PATH="$PATH" HOME=/fixture/home chroot "$root" "$@" \
        >"$output" 2>"${output%.stdout}.stderr"
}

run_case_in_root() {
    local root="$1" label="$2" selector="$3" output
    output="$work/$label-$selector.stdout"
    if ! run_in_root "$root" "$output" /consumer "$selector"; then
        printf 'owned pattern %s %s: child failed\n' "$label" "$selector" >&2
        return 1
    fi
    if ! grep -qx "owned-pattern-$selector-ok" "$output"; then
        printf 'owned pattern %s %s: success marker missing\n' "$label" "$selector" >&2
        return 1
    fi
}

compare_case_output() {
    local label="$1" selector="$2"
    if ! cmp "$work/oracle-$selector.stdout" "$work/$label-$selector.stdout"; then
        printf 'owned pattern %s %s: stdout differs from pinned musl\n' "$label" "$selector" >&2
        return 1
    fi
    if ! cmp "$work/oracle-$selector.stderr" "$work/$label-$selector.stderr"; then
        printf 'owned pattern %s %s: stderr differs from pinned musl\n' "$label" "$selector" >&2
        return 1
    fi
}

run_oracle_cases() {
    local selector
    for selector in "${CASES[@]}"; do
        run_case_in_root "$work/oracle-root" oracle "$selector"
    done
}

assert_static_symbols() {
    local archive="$1" table symbol
    table="$work/static-symbols.txt"
    nm -g --defined-only "$archive" >"$table"
    for symbol in $OWNED_SYMBOLS; do
        [ "$(awk -v symbol="$symbol" '$3 == symbol && $2 == "T" { count++ } END { print count + 0 }' "$table")" -eq 1 ] || {
            printf 'owned pattern: static strong symbol missing or duplicate: %s\n' "$symbol" >&2
            return 1
        }
    done
}

assert_dynamic_symbols() {
    local shared="$1" table symbol
    table="$work/dynamic-symbols.txt"
    readelf --dyn-syms -W "$shared" >"$table"
    for symbol in $OWNED_SYMBOLS; do
        [ "$(awk -v symbol="$symbol" '$4 == "FUNC" && $5 == "GLOBAL" && $6 == "DEFAULT" && $7 != "UND" && $8 == symbol { count++ } END { print count + 0 }' "$table")" -eq 1 ] || {
            printf 'owned pattern: dynamic strong symbol missing or duplicate: %s\n' "$symbol" >&2
            return 1
        }
    done
}

run_static_mode() {
    local product="$1" mode="$2" candidate root selector failures=0
    candidate="$work/consumer-static-$mode"
    "$product/bin/crabc-cc" "-$mode" "$work/workload.o" -o "$candidate"
    root="$work/static-$mode-root"
    mkdir "$root"
    prepare_root "$root"
    cp "$candidate" "$root/consumer"
    for selector in "${CASES[@]}"; do
        if ! run_case_in_root "$root" "static-$mode" "$selector"; then
            failures=1
            continue
        fi
        if ! compare_case_output "static-$mode" "$selector"; then
            failures=1
        fi
    done
    [ "$failures" -eq 0 ]
}

run_dynamic_mode() {
    local product="$1" mode="$2" entry="$3" candidate root selector failures=0
    candidate="$work/consumer-dynamic-$mode"
    if [ ! -f "$candidate" ]; then
        "$product/bin/crabc-cc-dynamic" "--dynamic-$mode" "$work/workload.o" -o "$candidate"
    fi
    root="$work/dynamic-$mode-$entry-root"
    cp -a "$product" "$root"
    prepare_root "$root"
    cp "$candidate" "$root/consumer"
    for selector in "${CASES[@]}"; do
        local output="$work/dynamic-$mode-$entry-$selector.stdout"
        if [ "$entry" = direct ]; then
            if ! run_in_root "$root" "$output" "$INTERPRETER" /consumer "$selector"; then
                printf 'owned pattern dynamic-%s-%s %s: child failed\n' "$mode" "$entry" "$selector" >&2
                failures=1
                continue
            fi
        elif ! run_in_root "$root" "$output" /consumer "$selector"; then
            printf 'owned pattern dynamic-%s-%s %s: child failed\n' "$mode" "$entry" "$selector" >&2
            failures=1
            continue
        fi
        if ! grep -qx "owned-pattern-$selector-ok" "$output"; then
            printf 'owned pattern dynamic-%s-%s %s: success marker missing\n' "$mode" "$entry" "$selector" >&2
            failures=1
            continue
        fi
        if ! compare_case_output "dynamic-$mode-$entry" "$selector"; then
            failures=1
        fi
    done
    [ "$failures" -eq 0 ]
}

bash "$ROOT/compat/x86_64/run_musl_oracle.sh" >/dev/null
"$ORACLE_CC" -std=c11 -I"$ROOT/include" -E -H "$PROBE" \
    >/dev/null 2>"$work/oracle.headers"
for header in dirent.h errno.h fnmatch.h glob.h locale.h stdlib.h string.h sys/types.h unistd.h; do
    grep -Fq "$ROOT/include/$header" "$work/oracle.headers"
done

if [ -z "$provided_dynamic" ]; then
    provided_dynamic="$work/dynamic-product"
    python3 -B "$ROOT/scripts/build_x86_64_owned_dynamic_sysroot.py" \
        --output "$provided_dynamic" >"$work/dynamic-build.json"
fi
readonly installed="$provided_dynamic"

# This is the sole consumer object. Its project-header declarations are bound
# to an independently linked, actual pinned-musl static oracle before any
# candidate link. All candidate modes below reuse these exact bytes.
"$installed/bin/crabc-cc-dynamic" --dynamic-pie -std=c11 -fno-builtin \
    -c "$PROBE" -o "$work/workload.o"
"$ORACLE_CC" -static -fno-pie -no-pie "$work/workload.o" -o "$work/oracle"
mkdir "$work/oracle-root"
prepare_root "$work/oracle-root"
cp "$work/oracle" "$work/oracle-root/consumer"
run_oracle_cases
printf 'owned pattern pinned-musl oracle: PASS\n'

if [ "$#" -eq 0 ]; then
    python3 -B "$ROOT/scripts/build_x86_64_owned_sysroot.py" \
        --output "$work/static-product" >"$work/static-build.json"
    assert_static_symbols "$work/static-product/usr/lib/libc.a"
    run_static_mode "$work/static-product" static
    run_static_mode "$work/static-product" static-pie
fi

assert_dynamic_symbols "$installed/usr/lib/libc.so"
for mode in pie non-pie; do
    for entry in kernel direct; do
        run_dynamic_mode "$installed" "$mode" "$entry"
    done
done

printf 'owned pattern: PASS (same project-header object with pinned musl; C/POSIX/C.UTF-8 fnmatch; invalid bytes and classes; glob sort, append, offsets, tilde, mark, trailing slash, ownership, and dropped-privilege unreadable callback; static/static-PIE/dynamic PIE/non-PIE kernel/direct); evidence: %s\n' "$work"
