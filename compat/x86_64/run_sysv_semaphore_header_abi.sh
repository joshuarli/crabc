#!/usr/bin/env bash
# Native Linux/x86-64 SysV semaphore header ABI matrix.
#
# Pinned musl 1.2.6 owns the selected sys/sem.h and sys/ipc.h declaration,
# layout, value, feature-selection, and C++ C-linkage contract.  The candidate
# uses raw GCC with only project headers and raw-GCC builtin headers, so an
# ambient libc cannot conceal a public-header mismatch.  This is compile-only
# evidence: it proves no crabc static/shared artifact linkage, runtime
# behavior, or C-runtime-family completion.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/sysv_semaphore_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/sysv_semaphore_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 SysV semaphore headers: %s\n' "$*" >&2
    exit 1
}
...
profile_arguments() {
    ...
    case "$profile" in
        ...
        # GCC predefines _GNU_SOURCE for C++; remove it so this profile
        # exercises the macro-free C++17 namespace just like strict C.
...
mode_arguments() {
    ...
    case "$mode" in
        normal)
            case "$profile" in
                c11-gnu|cxx17-gnu)
                    printf '%s\n' '-DCRABC_SYSV_SEMAPHORE_REQUIRE_GNU'
                    ;;
            esac
            ;;
        gnu-hidden)
            printf '%s\n' '-DCRABC_SYSV_SEMAPHORE_REQUIRE_GNU_HIDDEN'
            ;;
...
check_trace() {
    ...
    trace_has_header "$trace" "$root" sys/sem.h ||
        fail "$profile $tree trace omitted ${root}/sys/sem.h"
    trace_has_header "$trace" "$root" sys/ipc.h ||
        fail "$profile $tree trace omitted ${root}/sys/ipc.h"
}
...
check_cxx_symbols() {
    ...
    local undefined
    local symbol
    local -a expected=(semctl semget semop)
...
expect_hidden_failure() {
    ...
    if compile_profile "$tree" "$profile" gnu-hidden "$diagnostic" "$object"; then
        fail "$tree $profile unexpectedly exposes GNU semtimedop"
    fi
    grep -Fq 'semtimedop' "$diagnostic" ||
        fail "$tree $profile hidden GNU diagnostic does not name semtimedop"
}
...
[ -f "$C_PROBE" ] || fail "missing C SysV semaphore header ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ SysV semaphore header ABI probe"
[ "${#PROFILES[@]}" = "$EXPECTED_PROFILE_COUNT" ] || fail "profile roster drifted"
[ "${#GNU_PROFILES[@]}" = "$EXPECTED_GNU_PROFILE_COUNT" ] ||
    fail "GNU profile roster drifted"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-sysv-semaphore-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ SysV semaphore header ABI matrix: PASS (%s profiles; compile-only)\n' \
