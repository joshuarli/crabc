#!/usr/bin/env bash
# Native Linux/x86-64 permanent-standard-stream byte-I/O evidence.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc stdio permanent byte-I/O: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_stdio_permanent_byte_io_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-stdio-permanent-byte-io.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-stdio-permanent-byte-io-reference"
candidate="$work_dir/crabc-static-stdio-permanent-byte-io-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_stdio_permanent_byte_io_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_stdio_permanent_byte_io_probe.c \
    -o "$reference"
timeout "$EXECUTION_TIMEOUT" "$reference" ||
    fail "pinned-musl permanent byte-I/O fixture failed"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap \
    fgetc getc getchar fputc putc putchar ungetc fflush \
    close dup dup2 pipe read write; do
...
for unselected in fgetc_unlocked fputc_unlocked getc_unlocked getchar_unlocked \
    putc_unlocked putchar_unlocked gets getw putw getdelim getline \
...
for syscall_name in SYS_READ SYS_WRITE; do
    grep -Fq "raw_syscall::$syscall_name" \
        ...
        fail "byte-I/O implementation omits raw ${syscall_name} ownership"
done

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    -DCRABC_STDIO_PERMANENT_BYTE_IO_FREESTANDING -I"$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_stdio_permanent_byte_io_probe.c \
    compat/x86_64/libc_stdio_permanent_byte_io_start.S "$archive" -o "$candidate"
readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --program-headers --wide "$candidate" >"$candidate_program_headers"
readelf --dynamic --wide "$candidate" >"$candidate_dynamic" || true
readelf --relocs --wide "$candidate" >"$candidate_relocations"
objdump -d "$candidate" >"$candidate_disassembly"
for symbol in fgetc getc getchar fputc putc putchar ungetc fflush \
    close dup dup2 pipe read write stdin stdout stderr; do
...
timeout "$EXECUTION_TIMEOUT" "$candidate" ||
    fail "freestanding permanent byte-I/O fixture failed"

printf 'x86 static crabc-libc permanent byte-I/O: PASS\n'
