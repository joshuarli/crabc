...
| `process.credentials` | 9 | `owned-credentials-profile` runs all nine setters in isolated mapped user-namespace children with raw status/`errno` and real/effective/saved-ID transcripts. It preserves the selected four-alias `-1`/`EOPNOTSUPP` no-mutation profile and caller-coordinated direct setters; `credentials-profile` registers its dynamic replay. |
