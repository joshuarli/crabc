# Owned POSIX runtime family contract
...
It defines the contract and remaining evidence, not family completion. The
ledger remains `status = "planned"`; no focused runner, selected export, private static leaf,
...
The catalog is the coordinator's spelling roster. The frozen ledger
remains authoritative; the catalog copies each capability's exact `symbols`
list from `compat/crabc-rs/coverage.toml`. The following table describes the
installed workload bindings. The complete matrix passed at `730cafc1`;
its retained receipt is `.work/x86_64/posix-family-matrix/execution.json`.
The remaining native aggregate and declared family prerequisites apply to
every row. Later source changes require fresh qualification.

| Frozen capability | Names | Current installed binding |
| --- | ---: | --- |
...
| `filesystem.stat-compat` | 4 | `owned-posix-filesystem` supplies the source-verified local `extern` declarations over installed `sys/stat.h` types and runs all four version-insensitive aliases, including dirfd-relative and `AT_SYMLINK_NOFOLLOW` paths, through its installed linkage matrix and registered dynamic replay. |
...
| `filesystem.extensions` | 5 | `owned-posix-filesystem` selects and exercises `mktemp`, both temporary-name functions, and both file-handle entries. Its matched transcript retains raw handle return/`errno` values so filesystem support or authority cannot hide a difference. |
| `process.control` | 44 | `owned-process-control` covers all 44 names from one installed-header object; `process-trio` and `spawn` keep their deeper creation and rollback matrices. The case records its stated direct-`execveat` `fexecve` difference rather than calling it generic musl parity. |
| `process.credentials` | 9 | `owned-credentials-profile` runs all nine setters in isolated mapped user-namespace children with raw status/`errno` and real/effective/saved-ID transcripts, then proves musl's all-thread `__synccall` contract with live workers as container root; `credentials-profile` registers its dynamic replay. |
...
| `process.signal` | 34 | `signal-full` supplies the 23 residual primary spellings through static/static-PIE and dynamic PIE/non-PIE kernel/direct entries, while `owned-posix-signals.toml` names the helper/reporting, wait-cancellation, pthread, and timer cases it reuses. `owned-posix-composition` adds one bounded multi-threaded signal/`FILE`/syslog/fork/exec/spawn scenario. |
| `system.kernel-admin` | 42 | `linux-control` covers 18 names, `syslog` covers five, `system-cancellation` covers `system`, and `kernel-residual` runs the remaining 18 spellings from one installed-driver object. `owned-posix-composition` supplies a bounded logger/`FILE`/signal/fork state scenario. `gethostid.rs` retains its private constant-zero artifact matching musl. |
...
| `credentials-profile` | `owned-credentials-profile` | In a disposable user namespace, exercise all nine setters with current-ID, no-change, and rejected inputs, isolating each call in a child process. As container root, compare real single-threaded transitions and then the all-thread contract with musl: every thread observes each setter's change from initial-thread, worker, and post-`pthread_exit` callers; a first-thread `EPERM` changes no thread; and transitions stay complete while threads are created, exit, fork, and are targeted by `pthread_kill`. |
...
The credential row is a blocking contract for the governing profile.
Closing `process.credentials` requires an installed-product proof that all
nine setters follow musl's all-thread `__synccall` rendezvous, byte-for-byte
against pinned musl, in every installed linkage mode.
...
- `libc-test-harness/README.md`: functional, math, regression, and API results
...
The native aggregate consumes one current `installed` dynamic product from the
validated family matrix. Each component retains its full source and entry
roster: five differential sources, ten default OS-test suites, twelve
signal/process scenarios, ten pthread-stress iterations, and the full
libc-test inventory. The six-static/twelve-dynamic product multiplier belongs
to the workload matrix above; optional static aggregate replays are additional
evidence. Every component must retain raw results, compiler/header/object and
link identities, and its consumed execution roots. Matched failures do not
qualify. Native classifications require explicit source or profile evidence;
the AArch64 math and pthread exceptions do not transfer automatically.

The integrated component contracts are `owned-differential.md`,
`owned-signal-process.md` and `owned-pthread-stress.md`. The native pthread
profile replaces exactly two pinned main-call lines whose stdio cancellation
expectations do not apply to musl's FILE contract. Its remaining workload must
...
`./scripts/dev-x86_64.sh owned-posix-native --family-execution FILE --crypt-profile FILE --atomic-addressable-profile FILE --wordexp-profile FILE --wordexp-expected-native-inputs FILE --output NEW_DIR`
...
and atomic companions must use the identical installed product. The wordexp
companion must validate with the separately captured native-input seal and
contain all four dynamic `source-policy` cells for that same product.
Only the finite credential, crypt, atomic-addressability, strptime, wordexp,
and corrected-math dispositions in `owned-posix-native-dispositions.md`
can qualify with raw failure; original sources, counts, statuses, and
diagnostics remain intact. Every other failure remains incomplete. Host validation
