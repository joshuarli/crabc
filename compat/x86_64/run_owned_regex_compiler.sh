#!/usr/bin/env bash
# Native Linux/x86-64 compiler-only check for the in-progress owned TRE port.
#
...
# does not run regexec/regerror, alter the selected bounded regex leaf, or
# make a runtime/support claim.
set -euo pipefail
export LC_ALL=C
...
for symbol in regexec regerror; do
    if grep -Eq "[[:space:]]${symbol}$" "$work_dir/symbols"; then
        fail "compiler-only checkpoint unexpectedly defines ${symbol}"
    fi
done
