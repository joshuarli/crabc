#!/usr/bin/env bash
# Native Linux/x86-64 compile-only GNU <fcntl.h> tee ABI slice.
#
...
# tee is GNU-selected and stays hidden in default/strict/POSIX/XOPEN/BSD C.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 fcntl.h tee ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/tee_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/tee_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-tee-header.XXXXXX)"
...
gnu_definitions=(-D_GNU_SOURCE -DCRABC_EXPECT_TEE)
...
if ! "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
    ...
    fail "project GNU tee header contract drifted"
fi
...
for definitions_name in default_definitions strict_definitions posix_definitions xopen_definitions \
    ...
    declare -n definitions="$definitions_name"
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -DCRABC_REQUIRE_TEE_HIDDEN \
        -fsyntax-only "$c_probe" >"$work_dir/oracle-${definitions_name}.out" 2>&1; then
        fail "pinned musl exposes tee outside the GNU selector"
    fi
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -I "$ROOT_DIR/include" \
        -DCRABC_REQUIRE_TEE_HIDDEN -fsyntax-only "$c_probe" \
        >"$work_dir/project-${definitions_name}.out" 2>&1; then
        fail "project fcntl.h exposes tee outside the GNU selector"
    fi
done

printf 'x86 pinned-musl/project C/C++ GNU <fcntl.h> tee ABI: PASS\n'
