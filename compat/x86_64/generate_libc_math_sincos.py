#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``sincos``/``sincosf`` leaf.
...
# The GNU binary64/binary32 entries use fixed generic argument reduction and
# sin/cos kernels. The reducer's floor/scalbn calls are private sources, so
# this leaf does not select their separately evidenced public C ABI.
PUBLIC_SOURCES = (
    "src/math/sincos.c",
    "src/math/sincosf.c",
)
...
PRIVATE_RENAMES = {
    "__sin": "crabc_x86_math_sincos_kernel_sin",
    "__cos": "crabc_x86_math_sincos_kernel_cos",
    "__sindf": "crabc_x86_math_sincos_kernel_sindf",
    "__cosdf": "crabc_x86_math_sincos_kernel_cosdf",
    "__rem_pio2": "crabc_x86_math_sincos_reduce_pio2",
    "__rem_pio2f": "crabc_x86_math_sincos_reduce_pio2f",
    "__rem_pio2_large": "crabc_x86_math_sincos_reduce_pio2_large",
    "floor": "crabc_x86_math_sincos_provider_floor",
    "scalbn": "crabc_x86_math_sincos_provider_scalbn",
}
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_sincos_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_sincos_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_sincos_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_sincos_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 sincos/sincosf translation.",
        " * Generated by compat/x86_64/generate_libc_math_sincos.py; see",
        " * math_sincos.rs for the source/license/ABI contract.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-sincos-") as temporary:
