#!/usr/bin/env bash
# Pinned-musl differential for the private x86 scalar atanh/atanhf artifact.
set -euo pipefail
...
readonly SELECTED_SYMBOLS=(atanh atanhf)
readonly FENV_SIBLINGS=(feclearexcept fegetenv fegetround fesetenv fesetround fetestexcept)
readonly PRIVATE_PROVIDERS=(
	crabc_x86_math_atanh_provider_log1p
	crabc_x86_math_atanh_provider_log1pf
)

fail() { printf 'ERROR: x86 static libc atanh/atanhf: %s\n' "$*" >&2; exit 1; }
...
bash "$ROOT_DIR/compat/x86_64/run_math_atanh_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-math-atanh.XXXXXX)"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_atanh_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -fno-builtin \
	-fno-stack-protector compat/x86_64/libc_math_atanh_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl atanh/atanhf fixture failed"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_ATANH_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_atanh_probe.c \
	compat/x86_64/libc_math_atanh_start.S "$archive" -o "$candidate"
...
for unselected in sin sinf sinl sincos sincosf sincosl cos cosf cosl tan tanf \
	...
	acosh acoshf acoshl atanhl asin asinf asinl acos acosf acosl atan atanf atanl \
...
"$candidate" >"$candidate_output" || fail "freestanding atanh/atanhf fixture failed"
if ! cmp -s "$reference_output" "$candidate_output"; then
	cmp -l "$reference_output" "$candidate_output" | sed -n '1,120p' >&2 || true
	fail "candidate atanh/atanhf record stream differs from pinned musl"
fi

printf 'x86 static libc atanh/atanhf: PASS (%s records)\n' "$record_count"
