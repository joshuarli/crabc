#!/usr/bin/env bash
# Native Linux/x86-64 POSIX posix_close C/C++ declaration gate.
#
...
# ambient libc cannot mask a declaration or linkage mismatch. `posix_close`
# is unconditional in musl's unistd.h: this checks strict, POSIX, X/Open, and
# GNU C/C++ profiles without selecting descriptor behavior or public x86
# support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/posix_close_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/posix_close_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 posix_close headers: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-posix-close-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional posix_close declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <unistd.h> posix_close ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
