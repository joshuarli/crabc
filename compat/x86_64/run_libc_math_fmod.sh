#!/usr/bin/env bash
# Native Linux/x86-64 selected static fmod/fmodf evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc math fmod: %s\n' "$*" >&2; exit 1; }
...
reference="$work_dir/musl-math-fmod-reference"
candidate="$work_dir/crabc-static-math-fmod-candidate"
header_cxx_reference="$work_dir/musl-math-fmod-header.o"
header_cxx_candidate="$work_dir/project-math-fmod-header.o"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_fmod_probe.c >/dev/null 2>"$header_trace"
...
if "$reference"; then
	...
	status=$?
	fail "pinned-musl math fmod fixture failed with status ${status}"
fi
...
for mode in sse x87; do
	...
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -c compat/x86_64/math_fmod_header_abi_probe.cpp \
		-o "$header_cxx_reference"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" \
		-c compat/x86_64/math_fmod_header_abi_probe.cpp -o "$header_cxx_candidate"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only \
		compat/x86_64/math_fmod_header_abi_probe.cpp >/dev/null 2>"$cxx_header_trace"
	...
	for object in "$header_cxx_reference" "$header_cxx_candidate"; do
		...
		for symbol in fmod fmodf; do
			grep -Eq "[[:space:]]${symbol}$" <<<"$undefined" ||
				fail "C++ ${mode} probe does not retain unmangled ${symbol}"
		done
		if grep -Eq '_Z.*fmod' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled fmod reference"
		fi
	done
done
...
for symbol in fmod fmodf feclearexcept fegetenv feraiseexcept fesetenv fesetround fetestexcept; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_FMOD_FREESTANDING \
	...
	compat/x86_64/libc_math_fmod_probe.c compat/x86_64/libc_math_fmod_start.S \
	"$archive" -o "$candidate"
...
for unselected in fmodl remainder remainderf remainderl remquo remquof remquol \
	modf modff modfl fdim fdimf fdiml fmax fmaxf fmaxl fmin fminf fminl \
	rint rintf rintl nearbyint nearbyintf nearbyintl trunc truncf truncl \
	round roundf roundl ceil ceilf ceill floor floorf floorl sqrt sqrtf sqrtl \
...
for instruction in divsd divss; do
