/* Source-only Linux/x86-64 <strings.h> find-first-set declaration probe. */
...
typedef int (*ffs_signature)(int);
typedef int (*ffsl_signature)(long);
typedef int (*ffsll_signature)(long long);
...
typedef int (*hidden_ffs_signature)(int);
static hidden_ffs_signature ffs_must_be_hidden = ffs;
#endif

int crabc_x86_64_ffs_header_abi_probe(void)
{
#if defined(CRABC_EXPECT_FFS)
