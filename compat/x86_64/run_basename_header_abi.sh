#!/usr/bin/env bash
# Native Linux/x86-64 basename C/C++ declaration gate.
#
...
# host-libc header leakage. basename is unconditional in musl's <libgen.h>, so
# strict, POSIX, X/Open, GNU, and BSD C/C++ profiles all retain one exact
# unmangled char * (char *) declaration.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/basename_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/basename_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 libgen.h basename ABI: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-basename-header.XXXXXX)"
        ...
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                run_compiler "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
                    ...
                    -DCRABC_EXPECT_BASENAME -Werror=implicit-function-declaration \
                    "${include_args[@]}" -fsyntax-only "$C_PROBE"
            else
                object="$work_dir/${variant}-${label}-basename.o"
...
run_compiler "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
    ...
    -DCRABC_EXPECT_BASENAME "${include_args[@]}" -H -fsyntax-only "$C_PROBE" \
...
grep -Fq "$PROJECT_INCLUDE/libgen.h" "$header_trace" ||
    fail "strict C probe did not use the project <libgen.h>"

printf 'x86 pinned-musl/project C/C++ <libgen.h> basename ABI: PASS\n'
