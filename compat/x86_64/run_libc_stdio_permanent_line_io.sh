#!/usr/bin/env bash
# Native Linux/x86-64 permanent-standard-stream line-I/O evidence.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc stdio permanent line-I/O: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_stdio_permanent_line_io_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-stdio-permanent-line-io.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-stdio-permanent-line-io-reference"
candidate="$work_dir/crabc-static-stdio-permanent-line-io-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_stdio_permanent_line_io_probe.c >/dev/null 2>"$trace"
for header in errno.h fcntl.h stdio.h unistd.h features.h bits/alltypes.h; do
    grep -Fq "$ROOT_DIR/include/$header" "$trace" ||
        fail "fixture did not use the project $header header"
done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_stdio_permanent_line_io_probe.c \
    -o "$reference"
timeout "$EXECUTION_TIMEOUT" "$reference" ||
    fail "pinned-musl permanent line-I/O fixture failed"

nm -A --defined-only "$ORACLE_ARCHIVE" >"$oracle_archive_symbols" 2>/dev/null
for symbol in fgets fputs puts; do
    grep -Eq "[[:space:]]T[[:space:]]${symbol}$" "$oracle_archive_symbols" ||
        fail "pinned-musl archive omits strong ${symbol}"
done
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap fgets fputs puts \
    fflush feof fcntl read write; do
    grep -Eq "[[:space:]][TW][[:space:]]${symbol}$" "$archive_symbols" ||
        fail "archive does not define ${symbol}"
done
for unselected in fgets_unlocked fputs_unlocked gets getw putw getdelim getline \
    fopencookie popen pclose fmemopen open_memstream open_wmemstream; do
...
for syscall_name in SYS_READ SYS_WRITE; do
    grep -Fq "raw_syscall::$syscall_name" \
        ...
        fail "line-I/O implementation omits raw ${syscall_name} ownership"
done

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    -DCRABC_STDIO_PERMANENT_LINE_IO_FREESTANDING -I"$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_stdio_permanent_line_io_probe.c \
    compat/x86_64/libc_stdio_permanent_line_io_start.S "$archive" -o "$candidate"
readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --program-headers --wide "$candidate" >"$candidate_program_headers"
readelf --dynamic --wide "$candidate" >"$candidate_dynamic" || true
readelf --relocs --wide "$candidate" >"$candidate_relocations"
objdump -d "$candidate" >"$candidate_disassembly"
for symbol in fgets fputs puts fflush feof fcntl read write; do
...
timeout "$EXECUTION_TIMEOUT" "$candidate" ||
    fail "freestanding permanent line-I/O fixture failed"

printf 'x86 static crabc-libc permanent line-I/O: PASS\n'
