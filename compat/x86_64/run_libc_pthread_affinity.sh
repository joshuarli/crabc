#!/usr/bin/env bash
# Native Linux/x86-64 bounded static pthread-affinity evidence.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static pthread affinity: %s\n' "$*" >&2
    exit 1
}
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_affinity_probe.c >/dev/null 2>"$header_trace"
for header in errno.h pthread.h sched.h stdint.h bits/alltypes.h; do
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap \
    pthread_getaffinity_np pthread_setaffinity_np pthread_create pthread_join \
    pthread_self; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    -DCRABC_PTHREAD_AFFINITY_FREESTANDING -I"$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_pthread_affinity_probe.c \
    compat/x86_64/libc_pthread_affinity_start.S "$archive" -o "$candidate"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap \
    pthread_getaffinity_np pthread_setaffinity_np pthread_create pthread_join \
    pthread_self __crabc_x86_pthread_clone; do
