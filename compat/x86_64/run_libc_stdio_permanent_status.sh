#!/usr/bin/env bash
# Native Linux/x86-64 permanent-standard-stream status evidence.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc permanent-stream status: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_stdio_permanent_status_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-stdio-permanent-status.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-stdio-permanent-status-reference"
candidate="$work_dir/crabc-static-stdio-permanent-status-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_stdio_permanent_status_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_stdio_permanent_status_probe.c \
    -o "$reference"
timeout "$EXECUTION_TIMEOUT" "$reference" ||
    fail "pinned-musl permanent-stream-status fixture failed"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap \
    feof ferror clearerr fgetc close dup dup2 pipe; do
...
timeout "$EXECUTION_TIMEOUT" "$candidate" ||
    fail "freestanding permanent-stream-status fixture failed"

printf 'x86 static crabc-libc permanent-stream status: PASS\n'
