#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc memory-search evidence.
#
# The fixture first runs against pinned musl 1.2.6, then as a true
# -nostdlib/-static executable linked only with the selected archive.  Its
# closed public surface is exactly memchr, GNU memrchr, and GNU memmem.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc memory search: %s\n' "$*" >&2
    exit 1
}
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_memory_search_probe.c >/dev/null 2>"$header_trace"
