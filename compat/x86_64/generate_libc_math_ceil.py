#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 x86 ceil/ceilf assembly translation.
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    text = re.sub(
        ...
        rf".Lcrabc_x86_math_ceil_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_ceil_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_ceil_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 x86-64 ceil/ceilf assembly translation.",
        " * Generated by compat/x86_64/generate_libc_math_ceil.py; see",
        " * math_ceil.rs for the source/license/ABI contract.",
        " *",
        " * The musl-authored portions retain musl's MIT license; the exact",
        " * release license is recorded by compat/upstreams.toml.",
        " */",
    ]
    with tempfile.TemporaryDirectory(prefix="crabc-math-ceil-generate.") as temp:
        temp_root = Path(temp)
