#!/usr/bin/env bash
# Native Linux/x86-64 C11/C++17 locale/multibyte header ABI gate.
#
...
# mismatch. The C++ object retains unmangled C spellings for inspection. This
# is compile-only evidence: it neither links crabc-libc nor selects `locale_t`,
# locale-object APIs, `_l` APIs, collation, iconv, wide stdio, a C runtime,
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/locale_multibyte_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/locale_multibyte_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 locale/multibyte header ABI: %s\n' "$*" >&2
    exit 1
}
...
assert_header_provenance() {
    ...
    for header in locale.h limits.h stdlib.h wchar.h features.h bits/alltypes.h; do
...
[ -f "$C_PROBE" ] || fail "missing C locale/multibyte header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ locale/multibyte header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-locale-multibyte-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C11/C++17 locale/multibyte header ABI: PASS\n'
