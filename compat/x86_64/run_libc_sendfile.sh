#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc sendfile evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc sendfile: %s\n' "$*" >&2; exit 1; }
...
assert_sendfile_syscall_path() {
    objdump -d --disassemble=sendfile "$candidate" >"$work_dir/disassembly"
    grep -Eq '\$0x28,%(e|r)ax' "$work_dir/disassembly" || fail "sendfile lacks Linux syscall 40"
    grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$work_dir/disassembly" || fail "sendfile lacks syscall"
}
require_native_linux_x86_64
for tool in ar awk cargo cat cmp diff grep mapfile mkdir nm objdump readelf rustup sort wc; do require_tool "$tool"; done
[ -x "$ORACLE_CC" ] || fail "missing pinned musl oracle compiler"
bash "$ROOT_DIR/compat/x86_64/run_musl_oracle.sh" >/dev/null
bash "$ROOT_DIR/compat/x86_64/run_sendfile_header_abi.sh" >/dev/null
bash "$ROOT_DIR/compat/x86_64/run_x86_sendfile_reference.sh" >/dev/null
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-sendfile.XXXXXX)"; trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"; archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
candidate="$work_dir/crabc-static-sendfile-candidate"; header_trace="$work_dir/header-trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_sendfile_probe.c >/dev/null 2>"$header_trace"
for header in errno.h fcntl.h sys/sendfile.h sys/syscall.h sys/types.h unistd.h; do grep -Fq "$ROOT_DIR/include/$header" "$header_trace" || fail "fixture did not use project $header"; done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_sendfile_probe.c \
    -o "$work_dir/oracle"
"$work_dir/oracle"
...
grep -Eq '[[:space:]][TW][[:space:]]sendfile$' "$archive_symbols" || fail "archive lacks sendfile"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_SENDFILE_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie \
    -ffreestanding -fno-builtin -fno-stack-protector -Wl,-e,_start \
    -Wl,--no-undefined compat/x86_64/libc_sendfile_probe.c \
    compat/x86_64/libc_sendfile_start.S "$archive" -o "$candidate"
...
for symbol in __errno_location sendfile; do grep -Eq "[[:space:]]$symbol$" "$candidate_symbols" || fail "candidate lacks $symbol"; done
for unrelated in fcntl open openat close dup dup2 dup3 read write pipe pipe2 splice copy_file_range; do grep -Eq "[[:space:]]$unrelated$" "$candidate_symbols" && fail "candidate unexpectedly pulls $unrelated"; done
...
grep -Eq '[[:space:]]TLS[[:space:]]' "$candidate_program_headers" || fail "candidate lacks TLS"; assert_fixture_tls_capacity; assert_sendfile_syscall_path
"$candidate"
printf 'x86 static crabc-libc sendfile: PASS\n'
