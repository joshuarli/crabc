...
__attribute__((constructor)) static void hold_constructor_for_foreign_reads(void)
{
    ...
    /* Same-thread loader reads remain legal while this batch is initializing. */
    ...
    /* Both workers announce immediately before entering their loader calls.
     * Keep the constructor open briefly so an unguarded implementation has
     * time to return both reads before initialization completes. */
