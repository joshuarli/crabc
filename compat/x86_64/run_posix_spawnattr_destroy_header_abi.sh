#!/usr/bin/env bash
# Native Linux/x86-64 POSIX spawn-attribute destroy C/C++ declaration gate.
#
...
# This proves one opaque-pointer no-op spelling, not spawn execution, child
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_destroy_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_destroy_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 spawn.h posix_spawnattr_destroy headers: %s\n' "$*" >&2
    exit 1
}
...
check_cxx_linkage() {
    ...
    grep -Eq '[[:space:]]posix_spawnattr_destroy$' <<<"$undefined" ||
        fail "$profile $tree C++ probe does not retain C linkage for posix_spawnattr_destroy"
    if grep -Eq '_Z[0-9].*posix_spawnattr_destroy' <<<"$undefined"; then
        fail "$profile $tree C++ probe retained a mangled posix_spawnattr_destroy reference"
    fi
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-posix-spawnattr-destroy-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional posix_spawnattr_destroy declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <spawn.h> posix_spawnattr_destroy ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
