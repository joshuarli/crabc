...
fail() {
    printf 'owned pthread timed feature contract: %s\n' "$*" >&2
    exit 1
}
...
if candidate.exists() or candidate.is_symlink():
    raise SystemExit("owned pthread timed feature receipt directory must be fresh")
if not candidate.is_relative_to(root / ".work"):
    raise SystemExit("owned pthread timed feature receipt directory must remain below checkout .work")
print(candidate)
PY
)"
...
if retained_value:
    ...
    if static != expected_static or dynamic != expected_dynamic:
        raise SystemExit(
            "owned pthread timed feature contract retained inputs must be the sealed "
...
for product, label in ((static, "static product"), (dynamic, "dynamic product")):
    if overlaps(temporary, product):
        raise SystemExit(f"owned pthread timed feature contract TMPDIR overlaps {label}")
if receipt:
    ...
    if receipt_path.exists() or receipt_path.is_symlink() or not receipt_path.is_relative_to(root / ".work"):
        raise SystemExit("owned pthread timed feature contract receipt directory is unsafe")
    for product, label in ((static, "static product"), (dynamic, "dynamic product")):
...
paths = [Path(value).resolve(strict=True) for value in sys.argv[2:]]
output.write_text(json.dumps({
    "format": "owned-pthread-timed-feature-contract-inputs-v1",
    "inputs": {
        name: {"path": str(path), "sha256": hashlib.sha256(path.read_bytes()).hexdigest()}
        for name, path in zip(names, paths)
    },
...
[ "$(cat "$WORK/oracle.stdout")" = 'owned-pthread-timed-feature-contract-ok' ] ||
...
for binary in "$WORK"/static-contract "$WORK"/static-pie-contract "$WORK"/dynamic-*-contract; do
    readelf --symbols --wide "$binary" >"$binary.symbols.txt"
done
...
printf 'owned pthread timed feature contract: PASS (four pinned-musl-shaped timed aliases, exact source feature route, selected product bytes, final-link extraction, and bounded timeout/join behavior across five executable modes); evidence: %s\n' "$WORK"
