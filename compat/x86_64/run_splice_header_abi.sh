#!/usr/bin/env bash
# Native Linux/x86-64 GNU splice C/C++ declaration gate.
#
...
# host-libc header leakage. `splice` is a GNU-only x86 LP64 declaration. Its
# strict, POSIX.1-2008, X/Open 700, and BSD C profiles keep the spelling
# hidden; the C++ driver follows musl's extension-visible mode.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/splice_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/splice_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 fcntl.h splice ABI: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-splice-header.XXXXXX)"
        ...
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                run_compiler "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
                    -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
                    -D_GNU_SOURCE -DCRABC_EXPECT_SPLICE \
                    -Werror=implicit-function-declaration "${include_args[@]}" \
                    -fsyntax-only "$C_PROBE"
            else
                object="$work_dir/${variant}-splice.o"
...
compile_cxx_extension_profile() {
    local label="$1"
    shift
    local variant object undefined

    for variant in oracle project; do
        set_profile_args "$variant"
        object="$work_dir/${variant}-splice-${label}.o"
        run_compiler "$compiler" -std=c++17 -x c++ -U_GNU_SOURCE \
...
compile_cxx_extension_profile strict -D__STRICT_ANSI__
compile_cxx_extension_profile posix -D_POSIX_C_SOURCE=200809L
compile_cxx_extension_profile xopen -D_XOPEN_SOURCE=700
compile_cxx_extension_profile bsd -D_BSD_SOURCE
compile_gnu_profile
...
run_compiler "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
    -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
    -D_GNU_SOURCE -DCRABC_EXPECT_SPLICE "${include_args[@]}" \
...
printf 'x86 pinned-musl/project C/C++ <fcntl.h> splice ABI: PASS\n'
