#!/usr/bin/env bash
# Native Linux/x86-64 permanent-stdin __fsetlocking <stdio_ext.h> ABI proof.
#
# Pinned musl 1.2.6 supplies the declaration, request-constant, and C++
# C-linkage oracle. The raw candidate uses only project and compiler-builtin
# include trees. This compile-only matrix selects neither an archive nor a
# runtime/FILE/locking model.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_fsetlocking_stdin_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_fsetlocking_stdin_header_abi_probe.cpp"
readonly -a PROFILES=(c11 cxx17)

fail() {
    printf 'ERROR: x86 stdio permanent-stdin __fsetlocking header ABI: %s\n' "$*" >&2
    exit 1
}
    ...
    case "$1" in
        c11)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
                ...
                -DCRABC_STDIO_PERMANENT_FSETLOCKING_STDIN_C11
            ;;
        cxx17)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
                -U_POSIX_C_SOURCE -U_LARGEFILE64_SOURCE -U_DEFAULT_SOURCE \
                -DCRABC_STDIO_PERMANENT_FSETLOCKING_STDIN_CXX17
            ;;
...
[ -f "$C_PROBE" ] || fail "missing C permanent-stdin __fsetlocking header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ permanent-stdin __fsetlocking header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-stdio-permanent-fsetlocking-stdin-header.XXXXXX)"
...
printf 'x86 stdio permanent-stdin __fsetlocking header ABI: PASS\n'
