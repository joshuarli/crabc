#!/usr/bin/env bash
# Native Linux/x86-64 permanent-stderr __flbf evidence.
#
...
# It observes only the fixed process-lifetime stderr line-buffer predicate. It
...
fail() {
    printf 'ERROR: x86 static libc permanent-stderr __flbf: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_stdio_permanent_flbf_stderr_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-stdio-permanent-flbf-stderr.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-stdio-permanent-flbf-stderr-reference"
candidate="$work_dir/crabc-static-stdio-permanent-flbf-stderr-candidate"
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -fno-builtin \
    ...
    compat/x86_64/libc_stdio_permanent_flbf_stderr_probe.c -o "$reference"
timeout "$EXECUTION_TIMEOUT" "$reference" ||
    fail "pinned-musl permanent-stderr __flbf fixture failed"

nm -A --defined-only "$ORACLE_ARCHIVE" >"$oracle_archive_symbols" 2>/dev/null
grep -Eq "[[:space:]]T[[:space:]]__flbf$" "$oracle_archive_symbols" ||
    fail "pinned-musl archive omits strong __flbf"
...
grep -Eq "[[:space:]]T[[:space:]]__flbf$" "$archive_symbols" ||
...
for unselected in __fwriting __freading __fpending __fpurge \
...
"$ORACLE_CC" -std=c11 -D_POSIX_C_SOURCE=200809L \
    -DCRABC_STDIO_PERMANENT_FLBF_STDERR_FREESTANDING -I"$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_stdio_permanent_flbf_stderr_probe.c \
    compat/x86_64/libc_stdio_permanent_flbf_stderr_start.S "$archive" -o "$candidate"
readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --program-headers --wide "$candidate" >"$candidate_program_headers"
readelf --dynamic --wide "$candidate" >"$candidate_dynamic" || true
readelf --relocs --wide "$candidate" >"$candidate_relocations"
objdump -d "$candidate" >"$candidate_disassembly"
objdump -d --disassemble=__flbf "$candidate" >"$flbf_disassembly"
for symbol in __flbf stderr; do
...
if grep -Eq '[[:space:]]syscall$' "$flbf_disassembly"; then
    fail "__flbf unexpectedly contains a syscall path"
fi
...
timeout "$EXECUTION_TIMEOUT" "$candidate" ||
    fail "freestanding permanent-stderr __flbf fixture failed"

printf 'x86 static crabc-libc permanent-stderr __flbf: PASS\n'
