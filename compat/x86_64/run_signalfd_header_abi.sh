#!/usr/bin/env bash
# Native Linux/x86-64 pinned-musl/project <sys/signalfd.h> ABI matrix.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/signalfd_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/signalfd_header_abi_probe.cpp"
readonly -a PROFILES=(c-default c11-gnu c11-strict c11-posix-2008 c11-xopen-700 c11-bsd cxx17-gnu cxx17-strict)

fail() {
    printf 'ERROR: x86 signalfd header ABI: %s\n' "$*" >&2
    exit 1
}
...
check_trace() {
    ...
    for header in sys/signalfd.h stdint.h fcntl.h bits/alltypes.h; do
...
[ -f "$C_PROBE" ] && [ -f "$CXX_PROBE" ] || fail "missing signalfd header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-signalfd-header.XXXXXX)"
...
printf 'x86 pinned-musl/project signalfd header ABI: PASS (16 rows)\n'
