...
extract_selected_member() {
    ...
    (
        ...
        for member in "${members[@]}"; do
            ...
            if grep -Eq '[[:space:]][T][[:space:]]ns_get32$' <<<"$definitions"; then
                if grep -Eq '[[:space:]][T][[:space:]](dn_expand|dn_skipname|ns_get16|ns_put16|ns_put32|ns_initparse|ns_parserr|ns_skiprr|ns_name_uncompress)$' <<<"$definitions"; then
...
readelf --symbols --wide "$musl_object" |
    grep -E '[[:space:]]7[[:space:]]+FUNC[[:space:]]+GLOBAL[[:space:]]+DEFAULT[[:space:]]+[0-9]+[[:space:]]+ns_get32$' >/dev/null ||
