...
int main(void)
{
    ...
    if (pthread_join(symbol_reader, 0) || pthread_join(iterate_reader, 0))
        return 4;
    if (constructor_barrier_early_completions() != 2
        || constructor_barrier_completed() != 2
        || constructor_barrier_symbol_results() != 1
        || constructor_barrier_iterate_results() != 1) {
        fprintf(stderr, "constructor visibility failed: early=%d completed=%d dlsym=%d iterate=%d\n",
    ...
    puts("constructor visibility: same-thread reentry and foreign dlsym/iterate visibility");
    return 0;
}
