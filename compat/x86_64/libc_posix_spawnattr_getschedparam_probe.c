/* Native Linux/x86-64 static posix_spawnattr_getschedparam C ABI regression.
 *
 ...
 * scheduler-parameter observation, scheduler behavior, spawn execution, file
...
#ifndef CRABC_POSIX_SPAWNATTR_GETSCHEDPARAM_FREESTANDING
#include <errno.h>
#endif
...
enum { CRABC_POSIX_SPAWNATTR_GETSCHEDPARAM_ENOSYS = 38 };

#ifndef CRABC_POSIX_SPAWNATTR_GETSCHEDPARAM_FREESTANDING
_Static_assert(CRABC_POSIX_SPAWNATTR_GETSCHEDPARAM_ENOSYS == ENOSYS,
...
int crabc_x86_64_posix_spawnattr_getschedparam_probe(void)
{
    const posix_spawnattr_getschedparam_signature function =
    ...
    if (posix_spawnattr_getschedparam(&attributes, &parameter) !=
        CRABC_POSIX_SPAWNATTR_GETSCHEDPARAM_ENOSYS)
        return 1;
    ...
    if (function((const posix_spawnattr_t *)0, &parameter) !=
        CRABC_POSIX_SPAWNATTR_GETSCHEDPARAM_ENOSYS)
        return 4;
    if (!bytes_match((const unsigned char *)&parameter,
                     (const unsigned char *)&expected_parameter,
    ...
    if (function(&attributes, (struct sched_param *)0) !=
        CRABC_POSIX_SPAWNATTR_GETSCHEDPARAM_ENOSYS)
        return 6;
    if (!bytes_match((const unsigned char *)&attributes,
                     (const unsigned char *)&expected_attributes,
    ...
    if (function((const posix_spawnattr_t *)0, (struct sched_param *)0) !=
        CRABC_POSIX_SPAWNATTR_GETSCHEDPARAM_ENOSYS)
        return 8;
...
int main(void)
{
    return crabc_x86_64_posix_spawnattr_getschedparam_probe();
}
#endif
