#!/usr/bin/env bash
# Native Linux/x86-64 <sys/quota.h> conversion-macro ABI proof.
#
# Pinned musl 1.2.6 is the declaration and macro oracle. This header-only
# gate proves exact unconditional dbtob/btodb/fs_to_dq_blocks/dqoff macro
# spelling, LP64 result types, product-before-division, and unsigned-modular
# dqoff arithmetic through C and C++ compile-time assertions.
...
fail() {
    printf 'ERROR: x86 quota conversion macros header ABI: %s\n' "$*" >&2
    exit 1
}
...
[ -f "$C_PROBE" ] || fail "missing C quota conversion macro probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ quota conversion macro probe"
...
# Musl exposes these conversion macros unconditionally. Each C and C++ profile
...
printf 'x86 pinned-musl/project C/C++ <sys/quota.h> conversion macros: PASS\n'
