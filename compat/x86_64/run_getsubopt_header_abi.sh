#!/usr/bin/env bash
# Native Linux/x86-64 getsubopt <stdlib.h> C/C++ header ABI proof.
#
# Pinned musl 1.2.6 and the project header tree must agree that getsubopt is
# hidden in strict C/C++ and visible with POSIX, X/Open, GNU, and BSD feature
# selection, with one exact unmangled C function-pointer type.  This is a
# declaration-only proof; runtime behavior is owned by run_libc_getsubopt.sh.
set -euo pipefail
export LC_ALL=C

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly CANDIDATE_CC=/usr/bin/gcc
readonly PROJECT_INCLUDE="$ROOT_DIR/include"
readonly C_PROBE="$ROOT_DIR/compat/x86_64/getsubopt_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/getsubopt_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 getsubopt header ABI: %s\n' "$*" >&2
    exit 1
}

require_native_linux_x86_64() {
    [ "$(uname -s)" = Linux ] || fail "requires native Linux"
    case "$(uname -m)" in
        x86_64|amd64) ;;
        *) fail "refuses emulation on $(uname -m)" ;;
    esac
}

require_tool() {
    command -v "$1" >/dev/null 2>&1 || fail "requires $1"
}

require_native_linux_x86_64
for tool in env grep mktemp nm realpath sed uname; do
    require_tool "$tool"
done
[ -x "$ORACLE_CC" ] || fail "missing pinned musl compiler"
[ -x "$CANDIDATE_CC" ] || fail "missing raw native candidate compiler"

run_compiler() {
    local compiler="$1"
    shift

    env -u CPATH -u C_INCLUDE_PATH -u CPLUS_INCLUDE_PATH -u LIBRARY_PATH \
        -u GCC_EXEC_PREFIX -u GCC_SPECS -u COMPILER_PATH \
        "$compiler" "$@"
}

trace_paths() {
    sed -n -E 's/^[. ]+ (\/[^[:space:]]+).*$/\1/p' "$1"
}

candidate_compiler_builtin_include="$(run_compiler "$CANDIDATE_CC" \
    -print-file-name=include)"
case "$candidate_compiler_builtin_include" in
    /*) ;;
    *) fail "raw candidate compiler did not report an absolute builtin include directory" ;;
esac
candidate_compiler_builtin_include="$(realpath "$candidate_compiler_builtin_include")"
[ -d "$candidate_compiler_builtin_include" ] ||
    fail "missing raw candidate compiler builtin include directory"

work_dir="$(mktemp -d /tmp/crabc-x86-64-getsubopt-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT

compile_positive_profile() {
    local definition="$1"
    local language variant compiler include_args=() object undefined

    for language in c cxx; do
        for variant in oracle project; do
            case "$variant" in
                oracle)
                    compiler="$ORACLE_CC"
                    include_args=()
                    ;;
                project)
                    compiler="$CANDIDATE_CC"
                    include_args=(
                        -nostdinc
                        -I "$PROJECT_INCLUDE"
                        -isystem "$candidate_compiler_builtin_include"
                    )
                    ;;
                *) fail "unknown header tree: $variant" ;;
            esac
            if [ "$language" = c ]; then
                run_compiler "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
                    -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
                    "$definition" -DCRABC_EXPECT_GETSUBOPT \
                    -Werror=implicit-function-declaration "${include_args[@]}" \
                    -fsyntax-only "$C_PROBE"
            else
                object="$work_dir/${variant}-${definition#-D}-getsubopt.o"
                run_compiler "$compiler" -std=c++17 -x c++ -U_GNU_SOURCE -U_BSD_SOURCE \
                    -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
                    "$definition" -DCRABC_EXPECT_GETSUBOPT \
                    -nostdinc++ "${include_args[@]}" -c "$CXX_PROBE" -o "$object"
                undefined="$(nm --undefined-only "$object")"
                printf '%s\n' "$undefined" | grep -Eq '[[:space:]]getsubopt$' ||
                    fail "C++ probe does not retain C linkage for getsubopt (${variant}, ${definition})"
                if printf '%s\n' "$undefined" | grep -Eq '_Z[0-9].*getsubopt'; then
                    fail "C++ probe retained a mangled getsubopt reference (${variant}, ${definition})"
                fi
            fi
        done
    done
}

for language in c cxx; do
    for variant in oracle project; do
        case "$variant" in
            oracle)
                compiler="$ORACLE_CC"
                include_args=()
                ;;
            project)
                compiler="$CANDIDATE_CC"
                include_args=(
                    -nostdinc
                    -I "$PROJECT_INCLUDE"
                    -isystem "$candidate_compiler_builtin_include"
                )
                ;;
            *) fail "unknown header tree: $variant" ;;
        esac
        errors="$work_dir/${variant}-${language}-strict-errors"
        if [ "$language" = c ]; then
            if run_compiler "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
                -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
                -D__STRICT_ANSI__ -DCRABC_REQUIRE_GETSUBOPT_HIDDEN \
                -Werror=implicit-function-declaration "${include_args[@]}" \
                -fsyntax-only "$C_PROBE" >"$errors" 2>&1; then
                fail "getsubopt is visible under strict C (${variant})"
            fi
        elif run_compiler "$compiler" -std=c++17 -x c++ -U_GNU_SOURCE -U_BSD_SOURCE \
            -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
            -D__STRICT_ANSI__ -DCRABC_REQUIRE_GETSUBOPT_HIDDEN \
            -nostdinc++ "${include_args[@]}" -fsyntax-only "$CXX_PROBE" >"$errors" 2>&1; then
            fail "getsubopt is visible under strict C++ (${variant})"
        fi
    done
done

compile_positive_profile -D_POSIX_C_SOURCE=200809L
compile_positive_profile -D_XOPEN_SOURCE=700
compile_positive_profile -D_GNU_SOURCE
compile_positive_profile -D_BSD_SOURCE

header_trace="$work_dir/project-posix-header-trace"
run_compiler "$CANDIDATE_CC" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
    -U_XOPEN_SOURCE -U_POSIX_C_SOURCE -U_DEFAULT_SOURCE \
    -D_POSIX_C_SOURCE=200809L -DCRABC_EXPECT_GETSUBOPT -nostdinc \
    -I "$PROJECT_INCLUDE" -isystem "$candidate_compiler_builtin_include" \
    -H -fsyntax-only "$C_PROBE" >/dev/null 2>"$header_trace"
while IFS= read -r path; do
    case "$path" in
        "$PROJECT_INCLUDE"/*|"$candidate_compiler_builtin_include"/*) ;;
        *) fail "project POSIX header trace escaped its declared roots: $path" ;;
    esac
done < <(trace_paths "$header_trace")
for header in stdlib.h features.h bits/alltypes.h; do
    grep -Fq "$PROJECT_INCLUDE/$header" "$header_trace" ||
        fail "POSIX C probe did not use the project <$header>"
done

printf 'x86 pinned-musl/project C/C++ <stdlib.h> getsubopt ABI: PASS\n'
