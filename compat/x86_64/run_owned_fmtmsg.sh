...
readonly probe="$ROOT/compat/x86_64/owned_fmtmsg_probe.c"
...
if not temporary.is_dir() or temporary.resolve() != temporary or not temporary.is_relative_to(root / '.work'):
    raise SystemExit('fmtmsg TMPDIR must be a physical checkout .work directory')
for product, name in ((static_product, 'static'), (dynamic_product, 'dynamic')):
    if product and (not product.is_dir() or not product.is_relative_to(root / '.work')):
        raise SystemExit(f'fmtmsg {name} product must be a checkout .work directory')
PY
...
try:
    if family == 'static':
        ...
        raise SystemExit(f'fmtmsg has an unknown product family: {family}')
except ProductEvidenceError as error:
    raise SystemExit(f'fmtmsg {family} product payload is invalid: {error}') from error
PY
}
...
readonly work="$(mktemp -d "$TMPDIR/owned-fmtmsg.XXXXXX")"
chmod a+rx "$work"
printf 'fmtmsg evidence: %s\n' "$work"
...
prepare_root() {
    ...
    [ -e "$root/dev/full" ] || mknod "$root/dev/full" c 1 7
}
...
try:
    ...
    raise SystemExit(f'fmtmsg sealed link evidence: {error}') from error
...
if not expected_linkages:
    raise SystemExit('retained fmtmsg link identities have no expected modes')
records = {}
...
if set(records) != expected_linkages:
    raise SystemExit('retained fmtmsg link identities omit a product mode')
Path(sys.argv[1]).write_text(
    json.dumps(
        {
            'schema': 'crabc.x86_64-owned-fmtmsg-link-identities/v1',
...
try:
    ...
    raise SystemExit(f'fmtmsg installed-driver dependency output is invalid: {error}') from error
if not dependencies:
    raise SystemExit('fmtmsg installed-driver dependency output is empty')
dependency_paths = []
for name in dependencies:
    path = Path(name).resolve(strict=True)
    if path != source_path and not path.is_relative_to(headers):
        raise SystemExit(f'fmtmsg dependency escaped the installed headers: {path}')
    dependency_paths.append(path)
if source_path not in dependency_paths:
    raise SystemExit('fmtmsg dependency output omits the workload source')

def digest(path):
    return hashlib.sha256(Path(path).read_bytes()).hexdigest()

record = {
    'schema': 'crabc.x86_64-owned-fmtmsg-compile/v1',
