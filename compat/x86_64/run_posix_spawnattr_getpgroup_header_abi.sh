#!/usr/bin/env bash
# Native Linux/x86-64 POSIX spawn-attribute process-group C/C++ declaration gate.
#
...
# ambient libc cannot mask this unconditional two-pointer declaration, C++
# linkage, or the x86 process-group member type and offset. This proves one
# attribute readback spelling, not spawn execution, lifecycle, file actions,
# signals, scheduler attributes, or public x86 support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_getpgroup_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_getpgroup_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 spawn.h posix_spawnattr_getpgroup headers: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-posix-spawnattr-getpgroup-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional posix_spawnattr_getpgroup declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <spawn.h> posix_spawnattr_getpgroup ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
