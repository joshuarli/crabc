...
# The repository-wide default-static export ratchet intentionally remains
# unchanged until the root-owned integration records this new callable. Other
# independently reviewed native providers are also not yet in that roster, so
# this local check retains their raw surplus instead of pretending it is a
# closed baseline. It still rejects a baseline regression and requires exactly
# the selected strong `tgkill` extension; the root-owned canonical ratchet is
# the later complete-surface judge.
