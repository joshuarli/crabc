#!/usr/bin/env bash
# Native Linux/x86-64 proto.c C/C++ declaration and layout gate.
#
# Pinned musl 1.2.6 is the declaration, layout, and C-linkage oracle. The
# candidate uses raw GCC with project headers and only compiler builtin
# headers, so an ambient libc cannot mask a netdb declaration or linkage
# mismatch. The five legacy protocol-database declarations are unconditional
# in musl's netdb.h; this checks every frozen C/C++ feature profile without
# selecting protocol lookup/runtime behavior.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/protocol_database_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/protocol_database_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu c11-bsd cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 protocol database headers: %s\n' "$*" >&2
    exit 1
}
...
check_trace() {
    ...
    for header in netdb.h features.h stdint.h sys/socket.h; do
...
check_cxx_linkage() {
    ...
    for symbol in endprotoent getprotobyname getprotobynumber getprotoent setprotoent; do
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-protocol-database-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional proto.c declaration/layout failed"
...
printf 'x86 pinned-musl/project C/C++ <netdb.h> proto.c ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
