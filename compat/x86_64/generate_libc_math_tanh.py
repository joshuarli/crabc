#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``tanh``/``tanhf`` leaf.
...
# The public binary64/binary32 entries call only their matching expm1 source.
# It is compiled under a private name, so this leaf does not select the
# separately evidenced public expm1/expm1f C ABI.
PUBLIC_SOURCES = (
    "src/math/tanh.c",
    "src/math/tanhf.c",
)
...
PRIVATE_RENAMES = {
    "expm1": "crabc_x86_math_tanh_provider_expm1",
    "expm1f": "crabc_x86_math_tanh_provider_expm1f",
}
...
def transform_assembly(relative: str, text: str) -> str:
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_tanh_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_tanh_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_tanh_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_tanh_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 tanh/tanhf translation.",
        " * Generated by compat/x86_64/generate_libc_math_tanh.py; see",
        " * math_tanh.rs for the source/license/ABI contract.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-tanh-") as temporary:
