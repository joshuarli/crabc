#!/usr/bin/env bash
# Native Linux/x86-64 compile-only GNU <fcntl.h> sync_file_range ABI slice.
#
...
# sync_file_range is GNU-selected and stays hidden in default/strict/POSIX/
# XOPEN/BSD C profiles.
set -euo pipefail
...
c_probe="$ROOT_DIR/compat/x86_64/sync_file_range_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/sync_file_range_header_abi_probe.cpp"
...
gnu_definitions=(-D_GNU_SOURCE -DCRABC_EXPECT_SYNC_FILE_RANGE)
...
# against whichever host fcntl.h happens to be installed.
if ! "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
    ...
    fail "project GNU sync_file_range header contract drifted"
fi
for header in fcntl.h features.h sys/types.h bits/fcntl.h; do
...
for definitions_name in default_definitions strict_definitions posix_definitions xopen_definitions \
    ...
    declare -n definitions="$definitions_name"
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -DCRABC_REQUIRE_SYNC_FILE_RANGE_HIDDEN \
        -fsyntax-only "$c_probe" >"$work_dir/oracle-${definitions_name}.out" 2>&1; then
        fail "pinned musl exposes sync_file_range outside the GNU selector"
    fi
    if "$ORACLE_CC" -std=c11 -Werror=implicit-function-declaration \
        "${definitions[@]}" -I "$ROOT_DIR/include" \
        -DCRABC_REQUIRE_SYNC_FILE_RANGE_HIDDEN -fsyntax-only "$c_probe" \
        >"$work_dir/project-${definitions_name}.out" 2>&1; then
        fail "project fcntl.h exposes sync_file_range outside the GNU selector"
    fi
done

printf 'x86 pinned-musl/project C/C++ GNU <fcntl.h> sync_file_range ABI: PASS\n'
