#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc classful IPv4 evidence.
#
...
# object, never from libc.a. Musl keeps inet_makeaddr and inet_lnaof beside
# inet_network and inet_netof in inet_legacy.c; this leaf selects exactly the
# two local arithmetic functions, not inet_network's inet_addr dependency or
# any resolver, DNS, network-database, interface, or socket behavior.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc classful IPv4: %s\n' "$*" >&2
    exit 1
}
...
extract_selected_member() {
    ...
    (
        ...
        for member in "${members[@]}"; do
            ...
            if grep -Eq '[[:space:]][TW][[:space:]]inet_makeaddr$' <<<"$definitions"; then
                grep -Eq '[[:space:]][TW][[:space:]]inet_lnaof$' <<<"$definitions" ||
                    fail "inet_makeaddr archive member does not also define inet_lnaof"
                printf '%s\n' "$member"
            elif grep -Eq '[[:space:]][TW][[:space:]]inet_lnaof$' <<<"$definitions"; then
                fail "inet_lnaof is split from inet_makeaddr"
            fi
        done
    ) >"$matches_path"
    mapfile -t matches <"$matches_path"
    [ "${#matches[@]}" = 1 ] || fail "inet_makeaddr and inet_lnaof must have exactly one selected archive member"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-inet-classful.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-inet-classful-reference"
candidate="$work_dir/crabc-static-inet-classful-candidate"
...
selected_members="$work_dir/selected-inet-classful-members"
selected_member_names="$work_dir/selected-inet-classful-member-names"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_inet_classful_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_inet_classful_probe.c \
    -o "$reference"
if env -i LC_ALL=C TZ=UTC "$reference"; then
    :
else
    status=$?
    fail "pinned-musl classful IPv4 fixture exited ${status}"
fi
...
for symbol in inet_makeaddr inet_lnaof; do
    grep -Eq "[[:space:]][TW][[:space:]]${symbol}$" "$archive_symbols" ||
        fail "archive does not define ${symbol}"
done
selected_member="$(extract_selected_member "$archive" "$selected_members" \
    "$selected_member_names")"
[ -f "$selected_member" ] || fail "selected classful IPv4 member is missing"

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_INET_CLASSFUL_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie \
    -ffreestanding -fno-builtin -fno-stack-protector -Wl,-e,_start \
    -Wl,--gc-sections -Wl,--no-undefined compat/x86_64/libc_inet_classful_probe.c \
    compat/x86_64/libc_inet_classful_start.S "$selected_member" -o "$candidate"

readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --program-headers --wide "$candidate" >"$candidate_program_headers"
readelf --dynamic --wide "$candidate" >"$candidate_dynamic" || true
readelf --relocs --wide "$candidate" >"$candidate_relocations"
objdump -d --disassemble=inet_makeaddr "$candidate" >"$candidate_disassembly"
objdump -d --disassemble=inet_lnaof "$candidate" >>"$candidate_disassembly"
for symbol in inet_makeaddr inet_lnaof; do
    grep -Eq "[[:space:]]${symbol}$" "$candidate_symbols" ||
        fail "archive-free candidate does not define ${symbol}"
done
...
if grep -Eq '\b(call|syscall)\b' "$candidate_disassembly"; then
    fail "classful IPv4 implementation calls an unselected runtime boundary"
fi
for unselected in htonl htons ntohl ntohs __inet_aton inet_addr inet_aton \
    inet_ntoa inet_ntop inet_pton inet_network inet_netof __h_errno_location \
    h_errno hstrerror freeaddrinfo gai_strerror getaddrinfo gethostbyaddr \
    gethostbyname gethostbyname2 gethostent getnameinfo getnetbyaddr \
    getnetbyname getnetent getprotobyname getprotobynumber getprotoent \
    getservbyname getservbyport getservent if_indextoname if_nameindex \
    if_nametoindex if_freenameindex socket bind connect send recv malloc free \
    calloc realloc; do
...
if env -i LC_ALL=C TZ=UTC "$candidate"; then
    ...
    status=$?
    fail "archive-free classful IPv4 fixture exited ${status}"
fi

printf 'x86 static crabc-libc classful IPv4: PASS\n'
