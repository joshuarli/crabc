#!/usr/bin/env bash
# Pinned-musl differential for the private x86 GNU exp10/pow10 artifact.
set -euo pipefail
...
readonly PRIVATE_PROVIDERS=(
	...
	crabc_x86_math_exp10_provider_exp2
	crabc_x86_math_exp10_provider_pow
	crabc_x86_math_exp10_data_exp
	crabc_x86_math_exp10_data_pow_log
	crabc_x86_math_exp10_helper_invalid
	crabc_x86_math_exp10_helper_oflow
	crabc_x86_math_exp10_helper_uflow
	crabc_x86_math_exp10_helper_xflow
	crabc_x86_math_exp10_provider_fabs
)

fail() { printf 'ERROR: x86 static libc exp10/pow10: %s\n' "$*" >&2; exit 1; }
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_exp10_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -fno-builtin \
	-fno-stack-protector compat/x86_64/libc_math_exp10_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl exp10/pow10 fixture failed"
...
grep -Eq '[[:space:]]T[[:space:]]exp10$' "$archive_symbols" ||
	fail "archive does not provide a strong crabc-owned exp10"
grep -Eq '[[:space:]]W[[:space:]]pow10$' "$archive_symbols" ||
	fail "archive does not preserve musl weak pow10 alias"
for provider in "${PRIVATE_PROVIDERS[@]}"; do
	grep -Eq "[[:space:]][tr][[:space:]]${provider}$" "$archive_symbols" ||
		fail "archive lacks local ${provider}"
	if grep -Eq "[[:space:]][TWVDBR][[:space:]]${provider}$" "$archive_globals"; then
		fail "archive exposes private ${provider}"
	fi
done

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_EXP10_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_exp10_probe.c \
	compat/x86_64/libc_math_exp10_start.S "$archive" -o "$candidate"
...
exp10_value="$(awk '$4 == "FUNC" && $5 == "GLOBAL" && $6 == "DEFAULT" && $8 == "exp10" { print $2 }' "$candidate_symbols")"
pow10_value="$(awk '$4 == "FUNC" && $5 == "WEAK" && $6 == "DEFAULT" && $8 == "pow10" { print $2 }' "$candidate_symbols")"
[ -n "$exp10_value" ] || fail "candidate does not retain strong exp10"
[ -n "$pow10_value" ] || fail "candidate does not retain weak pow10"
[ "$exp10_value" = "$pow10_value" ] ||
	fail "candidate does not retain musl weak same-address exp10/pow10 alias"
if grep -Eq '[[:space:]]FUNC[[:space:]]+WEAK[[:space:]].*[[:space:]]exp10$' \
	"$candidate_symbols"; then
	fail "candidate falls through to weak compiler-builtins exp10"
fi
for provider in "${PRIVATE_PROVIDERS[@]}"; do
...
for unselected in exp10f exp10l pow10f pow10l exp expf expl exp2 exp2f exp2l \
...
for instruction in addsd subsd mulsd divsd; do
...
"$candidate" >"$candidate_output" || fail "freestanding exp10/pow10 fixture failed"
if ! cmp -s "$reference_output" "$candidate_output"; then
	cmp -l "$reference_output" "$candidate_output" | sed -n '1,120p' >&2 || true
	fail "candidate exp10/pow10 record stream differs from pinned musl"
fi

printf 'x86 static libc exp10/pow10: PASS (%s records)\n' "$record_count"
