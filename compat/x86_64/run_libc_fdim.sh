#!/usr/bin/env bash
# Native Linux/x86-64 selected static fdim/fdimf evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc fdim: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-fdim.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-fdim-reference"
candidate="$work_dir/crabc-static-fdim-candidate"
header_cxx_reference="$work_dir/musl-fdim-header.o"
header_cxx_candidate="$work_dir/project-fdim-header.o"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_fdim_probe.c >/dev/null 2>"$header_trace"
for header in fenv.h float.h math.h stdint.h features.h bits/alltypes.h; do
	grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
		fail "fixture did not use project $header"
done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
	compat/x86_64/libc_fdim_probe.c -o "$reference"
"$reference" || fail "pinned-musl fdim fixture failed"
...
for mode in sse x87; do
	...
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -c compat/x86_64/fdim_header_abi_probe.cpp \
		-o "$header_cxx_reference"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" \
		-c compat/x86_64/fdim_header_abi_probe.cpp -o "$header_cxx_candidate"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only \
		compat/x86_64/fdim_header_abi_probe.cpp >/dev/null 2>"$cxx_header_trace"
	...
	for object in "$header_cxx_reference" "$header_cxx_candidate"; do
		...
		for symbol in fdim fdimf; do
			grep -Eq "[[:space:]]${symbol}$" <<<"$undefined" ||
				fail "C++ ${mode} probe does not retain unmangled ${symbol}"
		done
		if grep -Eq '_Z.*fdim' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled fdim reference"
		fi
	done
done
...
for symbol in fdim fdimf feclearexcept fegetenv fesetenv fesetround fetestexcept; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_FDIM_FREESTANDING \
	...
	compat/x86_64/libc_fdim_probe.c compat/x86_64/libc_fdim_start.S \
	"$archive" -o "$candidate"
...
for unselected in fdiml exp10 exp10f exp10l pow10 pow10f pow10l \
	rint rintf rintl nearbyint nearbyintf nearbyintl; do
...
for instruction in subsd subss ucomisd ucomiss; do
	grep -Eq "[[:space:]]${instruction}([[:space:]]|$)" "$disassembly" ||
		fail "candidate lacks ${instruction}"
done
"$candidate" || fail "freestanding fdim fixture failed"

printf 'x86 static libc fdim: PASS\n'
