...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/endhostent_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/endhostent_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 endhostent headers: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-endhostent-header.XXXXXX)"
