#!/usr/bin/env bash
# Native Linux/x86-64 sched_getparam C/C++ header ABI gate.
#
# Pinned musl 1.2.6 and the project header tree must retain the same public
# `int sched_getparam(pid_t, struct sched_param *)` spelling and LP64 record
# layout under strict, POSIX, X/Open, and GNU profiles. This proves declaration
# and C++ C-linkage only; it does not select scheduler records, policy,
# mutation, affinity, lifecycle, or public x86 support.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly C_PROBE="$ROOT_DIR/compat/x86_64/sched_getparam_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/sched_getparam_header_abi_probe.cpp"

fail() { printf 'ERROR: x86 sched_getparam header ABI: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-sched-getparam-header.XXXXXX)"
...
for tree in oracle project; do
    ...
    for profile in strict posix xopen gnu; do
        ...
        grep -Eq '[[:space:]]sched_getparam$' <<<"$undefined" ||
            fail "$tree $profile C++ witness lacks unmangled sched_getparam"
        if grep -Eq '_Z.*sched_getparam' <<<"$undefined"; then
            fail "$tree $profile C++ witness retained a mangled sched_getparam reference"
        fi
    done
done
...
printf 'x86 pinned-musl/project sched_getparam C/C++ header ABI: PASS\n'
