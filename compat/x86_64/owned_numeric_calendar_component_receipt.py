...
SOURCE_PATHS = {
    "probe": "compat/x86_64/owned_numeric_calendar_probe.c",
    "runner": "compat/x86_64/run_owned_numeric_calendar.sh",
...
class NumericCalendarReceiptError(RuntimeError):
    """A numeric calendar component receipt is incomplete or no longer reconstructs."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise NumericCalendarReceiptError(message)
...
def read_json(path: Path, description: str) -> object:
    try:
        ...
        raise NumericCalendarReceiptError(f"{description} is not valid JSON: {path}") from error
...
def physical_directory(path: Path, description: str) -> Path:
    try:
        ...
        raise NumericCalendarReceiptError(f"{description} is unreadable: {path}") from error
...
def identity(root: Path, path: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise NumericCalendarReceiptError(f"receipt artifact escapes checkout: {path}") from error
...
def tracked_source_identity(root: Path, relative: str) -> dict[str, object]:
    path = checkout_path(root, relative, "numeric calendar reader source")
...
def source_product_seal(root: Path, static: Path | None, dynamic: Path) -> dict[str, object]:
    ...
    if static is not None:
        static = checkout_path(root, static.relative_to(root).as_posix(), "static numeric calendar product", directory=True)
        try:
            static_manifest, _ = products._validate_static_product(static)
        except products.ProductEvidenceError as error:
            raise NumericCalendarReceiptError(f"static numeric calendar product validation failed: {error}") from error
...
def tool_roster(root: Path, dynamic: Path, static: Path | None) -> dict[str, object]:
    ...
    spec = importlib.util.spec_from_file_location("owned_numeric_calendar_component_tools", helper)
...
def mounted(root: Path, path: Path) -> str:
    try:
        ...
        raise NumericCalendarReceiptError(f"command path escapes checkout: {path}") from error


def command_plan(paths: Mapping[str, object], tools: Mapping[str, object], mode: str) -> dict[str, list[str]]:
    """Return the exact retained argv map for the finite numeric calendar component."""

    require(mode in {FULL_MODE, DYNAMIC_MODE}, "numeric calendar execution mode differs")
    ...
    for name, linkage in (("dynamic-pie", "pie"), ("dynamic-non-pie", "non-pie")):
        ...
        plan[f"{name}-link"] = [tool("dynamic_driver"), f"--dynamic-{linkage}", "-std=c11", m(workload), "-o", m(executable)]
...
def parse_argv(raw: bytes, label: str) -> list[str]:
    try:
        ...
        raise NumericCalendarReceiptError(f"{label} retained argv is invalid JSON") from error
...
def header_trace_paths(stderr: bytes) -> tuple[str, ...]:
    ...
    try:
        ...
        raise NumericCalendarReceiptError("installed header trace is not UTF-8") from error
    paths: list[str] = []
    for line in lines:
...
def validate_report(root: Path, report_path: Path, *, require_static: bool = False) -> dict[str, object]:
    root = physical_directory(root, "checkout root")
    report_path = physical_file(report_path, "numeric calendar component report")
    require(report_path.parent.is_relative_to(root / ".work") and report_path.name == "owned-numeric-calendar-products.json",
            "numeric calendar component report is not retained below checkout .work")
    ...
    require(report["schema"] == SCHEMA and report["source_mount"] == SOURCE_MOUNT and report["scope"] == list(SCOPE),
            "numeric calendar component report contract differs")
    require(report["family_completion"] is False and report["promotion_ready"] is False and report["public_support"] is False,
            "numeric calendar component receipt is promoting")
    mode = report["execution_mode"]
    require(mode in {FULL_MODE, DYNAMIC_MODE}, "numeric calendar component execution mode differs")
    if require_static:
        require(mode == FULL_MODE, "numeric calendar component acceptance requires static/static-pie evidence")
    ...
    require(isinstance(products_record, dict) and set(products_record) == expected_product_keys,
            "numeric calendar component product roster differs")
    dynamic = checkout_path(root, products_record["dynamic"], "dynamic numeric calendar product", directory=True)
    static = checkout_path(root, products_record["static"], "static numeric calendar product", directory=True) if mode == FULL_MODE else None
    ...
    require(isinstance(seals, dict) and set(seals) == {"source-product-before", "source-product-after", "tools-before", "tools-after"},
            "numeric calendar seal roster differs")
    ...
    require(isinstance(commands, dict) and set(commands) == set(plan), "numeric calendar command roster differs")
    ...
    require(raw["oracle-run"]["stdout"] == ORACLE_STDOUT and raw["oracle-run"]["stderr"] == b"",
            "pinned musl numeric calendar oracle transcript differs")

    links = report["links"]
    expected_links = ("static", "static-pie", "dynamic-pie", "dynamic-non-pie") if mode == FULL_MODE else ("dynamic-pie", "dynamic-non-pie")
    require(isinstance(links, dict) and set(links) == set(expected_links), "numeric calendar product link roster differs")
    ...
    require(isinstance(payloads, dict) and set(payloads) == {"pie", "non-pie"}, "numeric calendar copied payload roster differs")
...
def main() -> int:
    ...
    except (NumericCalendarReceiptError, OSError, ValueError, products.ProductEvidenceError, copies.CryptRuntimeEvidenceError) as error:
        parser.exit(1, f"owned numeric calendar component receipt failed: {error}\n")
    print("owned numeric calendar component receipt: valid; bounded component evidence remains non-promoting")
    return 0
