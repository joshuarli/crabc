...
# dl_iterate_phdr. The constructor also reenters both APIs itself. A reader
# completing before the constructor returns is an observable publication bug.
...
if [ "$status" -ne 0 ]; then
    printf 'general constructor read barrier: FAIL status=%s; evidence: %s\n' "$status" "$work" >&2
...
[ "$(<"$work/constructor-barrier-candidate.stdout")" = \
    'constructor barrier: same-thread reentry and foreign dlsym/iterate wait' ]
printf 'general constructor read barrier: PASS (foreign dlsym/iterate wait, same-thread reentry); evidence: %s\n' "$work"
