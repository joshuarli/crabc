...
# inet_ntoa, inet_makeaddr, inet_lnaof, and inet_netof
...
check_cxx_c_linkage() {
    ...
    local undefined
    local symbol
    local -a expected=(inet_pton inet_ntop inet_aton inet_addr inet_ntoa inet_makeaddr inet_lnaof inet_netof)
    ...
    if printf '%s\n' "$undefined" | grep -Eq '_Z.*inet_(pton|ntop|aton|addr|ntoa|makeaddr|lnaof|netof)'; then
