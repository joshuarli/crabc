#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc chown evidence.
#
...
# archive. It proves one following pathname-ownership entry, not a sibling
...
fail() {
    printf 'ERROR: x86 static libc chown: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_chown_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-chown.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-chown-reference"
candidate="$work_dir/crabc-static-chown-candidate"
...
chown_disassembly="$work_dir/chown-disassembly"

cd "$ROOT_DIR"
mkdir "$reference_work" "$candidate_work"
"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_chown_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_chown_probe.c -o "$reference"
(cd "$reference_work" && "$reference") ||
    fail "pinned-musl chown fixture failed"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap chown; do
...
grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$chown_disassembly" ||
    fail "chown lacks a direct Linux syscall"
grep -Eq '\$0x5c,%e?ax' "$chown_disassembly" ||
...
(cd "$candidate_work" && "$candidate") ||
    fail "freestanding chown fixture failed"

printf 'x86 static crabc-libc chown: PASS\n'
