...
def transform_assembly(relative: str, text: str) -> str:
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_scalar_completion_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(
        ...
        for index, relative in enumerate((*PUBLIC_SOURCES, *PRIVATE_SOURCES)):
            ...
            input_relative = "src/math/fmaf.c" if relative == "src/math/x86_64/fmaf.c" else relative
            original = (source_root / input_relative).read_text()
            corrected = corrected_source(input_relative, original)
            input_path = source_root / relative
            if corrected != original:
                input_path = temporary_root / f"{index:03}.c"
