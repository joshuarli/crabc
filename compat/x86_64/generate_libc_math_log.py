#!/usr/bin/env python3
"""Generate the fixed musl 1.2.6 scalar x86 ``log``/``logf`` leaf.
...
# The public kernels use two independent Arm tables and four direct domain
# providers. Keep all providers private so this small surface neither imports
# a selected math.special provider nor becomes a source of one.
PUBLIC_SOURCES = (
    "src/math/log.c",
    "src/math/logf.c",
)
...
PRIVATE_RENAMES = {
    "__log_data": "crabc_x86_math_log_data",
    "__logf_data": "crabc_x86_math_log_data_f",
    "__math_divzero": "crabc_x86_math_log_provider_divzero",
    "__math_divzerof": "crabc_x86_math_log_provider_divzerof",
    "__math_invalid": "crabc_x86_math_log_provider_invalid",
    "__math_invalidf": "crabc_x86_math_log_provider_invalidf",
}

# Preserve musl's all-rounding-mode branches and fixed scalar SSE arithmetic.
COMPILE_FLAGS = (
    "-D_XOPEN_SOURCE=700",
    "-std=c99",
    "-ffreestanding",
    ...
    "-fno-builtin-log",
    "-fno-builtin-logf",
    "-fno-builtin-__math_divzero",
    "-fno-builtin-__math_divzerof",
    "-fno-builtin-__math_invalid",
    "-fno-builtin-__math_invalidf",
    "-fno-tree-vectorize",
...
def transform_assembly(relative: str, text: str) -> str:
    """Namespace TU-local labels and keep non-public providers local."""
    ...
    for name in sorted(
        ...
        if not name.startswith(".L"):
            text = replace_symbol(text, name, f"crabc_x86_math_log_{tag}_{name}")
    text = re.sub(
        r"(?<![A-Za-z0-9_.])\.L([A-Za-z0-9_.$]+)",
        rf".Lcrabc_x86_math_log_{tag}_\1",
        text,
    )
    ...
    text = re.sub(
        ...
        rf"\1\2.crabc_x86_math_log_{tag}",
        text,
        flags=re.MULTILINE,
    )
    return text.rstrip() + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "libc/src/c_abi/x86_64/math_log_musl_x86_64.S",
    )
    ...
    blocks = [
        "/*",
        " * Fixed musl 1.2.6 generic x86-64 log/logf translation.",
        " * Generated by compat/x86_64/generate_libc_math_log.py; see",
        " * math_log.rs for the source/license/ABI contract.",
        " *",
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-x86-math-log-") as temporary:
