#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc socket transport evidence.
#
...
fail() {
    printf 'ERROR: x86 static libc socket transport: %s\n' "$*" >&2
    exit 1
}
...
assert_selected_c_abi_surface() {
    ...
    # only its C object members, so compiler-builtins do not blur the closed
    # C export contract. The hidden signal restorer remains the one audited
    # frame-internal exception.
...
for symbol in __errno_location socket socketpair bind listen accept accept4 \
    connect send recv sendto recvfrom shutdown getsockname getpeername; do
...
# The behavior fixture exercises each entry point locally. These emitted-code
# gates additionally pin every selected wrapper to its direct x86 Linux
# syscall boundary. Together with the fixture's SOCK_CLOEXEC | SOCK_NONBLOCK
# observations on all selected descriptor-creation paths, this proves the
# Linux-5.10 direct atomic-flag path without a fcntl fallback.
