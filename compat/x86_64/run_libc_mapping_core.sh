#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc mapping-core evidence.
#
...
fail() {
    printf 'ERROR: x86 static libc mapping core: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_mman_header_abi.sh" >/dev/null
bash "$ROOT_DIR/compat/x86_64/run_x86_mapping_reference.sh" >/dev/null
...
candidate="$work_dir/crabc-static-mapping-core-candidate"
...
for header in errno.h stdint.h sys/mman.h sys/syscall.h sys/types.h bits/mman.h \
    bits/syscall.h; do
...
if "$reference"; then
    ...
    status=$?
    fail "pinned-musl mapping-core fixture exited ${status}"
fi
...
for symbol in __errno_location madvise mincore mmap mprotect munmap posix_madvise; do
...
if grep -Eq 'TLSGD|TLSLD|TLSDESC|GOTTPOFF|DTPMOD(64)?|__tls_get_addr|crabc_core|mimalloc|sha_crypt|__vm_wait' \
    "$archive_relocations"; then
    fail "archive selects dynamic TLS, unowned runtime state, or external VM wait"
fi

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MAPPING_CORE_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_mapping_core_probe.c \
    compat/x86_64/libc_mapping_core_start.S "$archive" -o "$candidate"
...
for unrelated in msync mremap mlock mlock2 munlock mlockall munlockall \
    ...
    if grep -Eq "[[:space:]]${unrelated}$" "$candidate_symbols"; then
        fail "mapping-core candidate unexpectedly pulls ${unrelated}"
    fi
done
...
assert_fixture_tls_capacity
if grep -Eq 'TLSGD|TLSLD|TLSDESC|GOTTPOFF|DTPMOD(64)?|DTPOFF(32|64)?|__tls_get_addr|crabc_core|mimalloc|sha_crypt|__vm_wait' \
    "$candidate_relocations" "$candidate_symbols" "$candidate_disassembly"; then
    fail "candidate retains dynamic TLS, unowned runtime state, or external VM wait"
fi
