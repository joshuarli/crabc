#!/usr/bin/env bash
# Native Linux/x86-64 bounded static pthread mutex priority-ceiling query evidence.
#
...
# archive. It proves exactly pthread_mutex_getprioceiling's unconditional
# direct EINVAL result and no writes through an optional ceiling pointer. It
# does not select priority-protect mutex support, its setter, mutex operation,
# scheduler state, threads, TLS, synchronization, cancellation, CRT, loader,
...
fail() {
    printf 'ERROR: x86 static pthread mutex priority-ceiling query: %s\n' "$*" >&2
    exit 1
}
...
assert_no_unselected_mutex_exports() {
    ...
    for unselected in pthread_mutex_setprioceiling \
        ...
        pthread_mutexattr_getprotocol pthread_mutexattr_setrobust \
        pthread_mutexattr_getrobust; do
...
assert_direct_stub_path() {
    local disassembly="$work_dir/pthread_mutex_getprioceiling-disassembly"

    objdump -d --disassemble=pthread_mutex_getprioceiling "$candidate" >"$disassembly"
    if grep -Eq '[[:space:]](syscall|call)([[:space:]]|$)|%fs:' "$disassembly"; then
        fail "pthread_mutex_getprioceiling must remain a direct TLS-free, syscall-free stub"
    fi
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pthread-mutex-prioceiling-query.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-pthread-mutex-prioceiling-query-reference"
candidate="$work_dir/crabc-static-pthread-mutex-prioceiling-query-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_mutex_prioceiling_query_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -pthread -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_pthread_mutex_prioceiling_query_probe.c \
    -o "$reference"
if timeout "$EXECUTION_TIMEOUT" "$reference"; then
    :
else
    status=$?
    fail "pinned-musl mutex priority-ceiling query fixture exited ${status}"
fi
...
grep -Eq '[[:space:]][TW][[:space:]]pthread_mutex_getprioceiling$' "$archive_symbols" ||
    fail "archive does not define pthread_mutex_getprioceiling"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    -DCRABC_PTHREAD_MUTEX_PRIOCEILING_QUERY_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_pthread_mutex_prioceiling_query_probe.c \
    compat/x86_64/libc_pthread_mutex_prioceiling_query_start.S "$archive" -o "$candidate"
...
grep -Eq '[[:space:]]pthread_mutex_getprioceiling$' "$candidate_symbols" ||
    fail "candidate does not define pthread_mutex_getprioceiling"
assert_no_unselected_mutex_exports "$candidate_symbols"
for unselected in __errno_location __crabc_x86_static_tls_bootstrap \
    pthread_cond_init pthread_cond_destroy pthread_cond_wait pthread_mutex_setprioceiling; do
...
if timeout "$EXECUTION_TIMEOUT" "$candidate"; then
    ...
    status=$?
    fail "freestanding mutex priority-ceiling query fixture exited ${status}"
fi

printf 'x86 static crabc-libc pthread mutex priority-ceiling query: PASS\n'
