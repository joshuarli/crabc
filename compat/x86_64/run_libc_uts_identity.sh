#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc UTS-identity evidence.
#
...
fail() {
    printf 'ERROR: x86 static libc UTS identity: %s\n' "$*" >&2
    exit 1
}
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_uts_identity_probe.c >/dev/null 2>"$header_trace"
for header in errno.h sys/syscall.h bits/syscall.h sys/utsname.h unistd.h; do
...
for symbol in __errno_location uname gethostname sethostname getdomainname setdomainname; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_UTS_IDENTITY_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie \
    -ffreestanding -fno-builtin -fno-stack-protector -Wl,-e,_start \
    -Wl,--no-undefined compat/x86_64/libc_uts_identity_probe.c \
    compat/x86_64/libc_uts_identity_start.S "$archive" -o "$candidate"
