#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc confstr evidence.
#
# The existing musl-mapped system-configuration source owner is linked only
# with `--gc-sections`. The final true-static candidate retains `confstr` and
...
fail() {
    printf 'ERROR: x86 static libc confstr: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_confstr_header_abi.sh" >/dev/null

grep -Fqx $'confstr\tconfstr.lo\tT\tGLOBAL\t0\t78' "$AARCH64_STATIC_ABI" ||
    fail "AArch64 musl ABI oracle lost confstr ownership"

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-confstr.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-confstr-reference"
candidate="$work_dir/crabc-static-confstr-candidate"
header_trace="$work_dir/header-trace"
archive_symbols="$work_dir/archive-symbols"
selected_symbols="$work_dir/selected-c-abi-symbols"
expected_symbols="$work_dir/expected-c-abi-symbols"
owner_symbols="$work_dir/confstr-owner-symbols"
owner_disassembly="$work_dir/confstr-owner-disassembly"
...
confstr_disassembly="$work_dir/confstr-disassembly"
errno_disassembly="$work_dir/errno-location-disassembly"

cd "$ROOT_DIR"
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_confstr_probe.c >/dev/null 2>"$header_trace"
for header in errno.h unistd.h features.h bits/alltypes.h; do
    grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
        fail "fixture did not use project $header"
done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_confstr_probe.c -o "$reference"
"$reference" || fail "pinned-musl confstr fixture failed"
...
grep -Eq '[[:space:]]T[[:space:]]confstr$' "$archive_symbols" ||
    fail "archive does not define strong confstr"

mapfile -t members < <(archive_member_for_symbol "$archive" confstr)
[ "${#members[@]}" -eq 1 ] ||
    fail "confstr must have exactly one existing archive source owner"
...
grep -Eq '^confstr[[:space:]]+T[[:space:]]' "$owner_symbols" ||
    fail "existing source owner does not define confstr"
objdump -d --disassemble=confstr "$owner" >"$owner_disassembly"
if grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$owner_disassembly"; then
    fail "existing source confstr unexpectedly performs a syscall"
fi

"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_CONFSTR_FREESTANDING \
    -O2 -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    -Wl,--gc-sections compat/x86_64/libc_confstr_probe.c \
    compat/x86_64/libc_confstr_start.S "$archive" -o "$candidate"
...
objdump -d --disassemble=confstr "$candidate" >"$confstr_disassembly"
objdump -d --disassemble=__errno_location "$candidate" >"$errno_disassembly"

for symbol in confstr __errno_location; do
...
if grep -Eq '[[:space:]](sysconf|fpathconf|pathconf|getpagesize|getdtablesize|getauxval|getrlimit|setrlimit|prlimit|getrusage|getpriority|setpriority|nice)$' \
...
if grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$confstr_disassembly"; then
    fail "candidate confstr unexpectedly performs a syscall"
fi
if awk '/[[:space:]]call/ && $0 !~ /<__errno_location>/ { exit 1 }' \
    "$confstr_disassembly"; then
    :
else
    fail "candidate confstr calls outside its explicit errno seam"
fi
grep -Eq '%fs:' "$errno_disassembly" ||
    fail "candidate errno accessor does not use direct initial-TLS FS access"

"$candidate" || fail "freestanding confstr fixture failed"

printf 'x86 static libc confstr: PASS\n'
