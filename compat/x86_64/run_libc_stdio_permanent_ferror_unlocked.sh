#!/usr/bin/env bash
# Native Linux/x86-64 permanent-standard-stream ferror_unlocked evidence.
#
...
# archive. It proves musl's weak, same-address `ferror_unlocked` alias of
# `ferror` for permanent stdin observation only. It is not a lock-free FILE
# claim, another status alias, general FILE/path-stream I/O, or public-x86
# evidence.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static libc permanent-stream ferror_unlocked: %s\n' "$*" >&2
    exit 1
}
...
bash "$ROOT_DIR/compat/x86_64/run_stdio_permanent_ferror_unlocked_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-stdio-permanent-ferror-unlocked.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
target_dir="$work_dir/cargo-target"
archive="$target_dir/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-stdio-permanent-ferror-unlocked-reference"
candidate="$work_dir/crabc-static-stdio-permanent-ferror-unlocked-candidate"
...
ferror_disassembly="$work_dir/ferror-disassembly"
errno_disassembly="$work_dir/errno-disassembly"
oracle_archive_symbols="$work_dir/oracle-archive-symbols"

cd "$ROOT_DIR"
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_stdio_permanent_ferror_unlocked_probe.c >/dev/null 2>"$trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_stdio_permanent_ferror_unlocked_probe.c \
    -o "$reference"
timeout "$EXECUTION_TIMEOUT" "$reference" ||
    fail "pinned-musl permanent-stream ferror_unlocked fixture failed"

nm -A --defined-only "$ORACLE_ARCHIVE" >"$oracle_archive_symbols" 2>/dev/null
grep -Eq "[[:space:]]T[[:space:]]ferror$" "$oracle_archive_symbols" ||
    fail "pinned-musl archive omits strong ferror"
grep -Eq "[[:space:]]W[[:space:]]ferror_unlocked$" "$oracle_archive_symbols" ||
    fail "pinned-musl archive omits weak ferror_unlocked"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap \
    ferror ferror_unlocked fgetc close dup dup2; do
    grep -Eq "[[:space:]][TW][[:space:]]${symbol}$" "$archive_symbols" ||
        fail "archive does not define ${symbol}"
done
grep -Eq '[[:space:]][BDR][[:space:]]stdin$' "$archive_symbols" ||
    fail "archive does not define permanent stdin data"
grep -Eq "[[:space:]]W[[:space:]]ferror_unlocked$" "$archive_symbols" ||
    fail "archive does not define weak ferror_unlocked"
for unselected in _IO_ferror_unlocked clearerr_unlocked fgetc_unlocked \
    getc_unlocked getchar_unlocked fputc_unlocked putc_unlocked \
    putchar_unlocked fdopen freopen fopencookie popen pclose fmemopen \
    open_memstream; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE \
    ...
    compat/x86_64/libc_stdio_permanent_ferror_unlocked_start.S "$archive" -o "$candidate"
readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --program-headers --wide "$candidate" >"$candidate_program_headers"
readelf --dynamic --wide "$candidate" >"$candidate_dynamic" || true
readelf --relocs --wide "$candidate" >"$candidate_relocations"
objdump -d "$candidate" >"$candidate_disassembly"
objdump -d --disassemble=ferror "$candidate" >"$ferror_disassembly"
objdump -d --disassemble=__errno_location "$candidate" >"$errno_disassembly"
for symbol in ferror ferror_unlocked fgetc close dup dup2 stdin; do
    grep -Eq "[[:space:]]${symbol}$" "$candidate_symbols" ||
        fail "candidate lacks ${symbol}"
done
assert_weak_same_address_alias "$candidate_symbols" ferror_unlocked ferror candidate
for unselected in feof_unlocked fileno_unlocked _IO_ferror_unlocked \
    clearerr_unlocked fgetc_unlocked getc_unlocked getchar_unlocked \
    fputc_unlocked putc_unlocked putchar_unlocked; do
...
if grep -Eq '[[:space:]]syscall$' "$ferror_disassembly"; then
    fail "ferror unexpectedly contains a syscall path"
fi
...
timeout "$EXECUTION_TIMEOUT" "$candidate" ||
    fail "freestanding permanent-stream ferror_unlocked fixture failed"

printf 'x86 static crabc-libc permanent-stream ferror_unlocked: PASS\n'
