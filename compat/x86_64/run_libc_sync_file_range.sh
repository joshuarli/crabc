...
#
# The pinned-musl/project-header fixture proves only a direct descriptor-range
# writeback request: its wrapper has Linux's raw result/errno translation,
# leaves a regular descriptor's shared position unchanged, rejects invalid
# flags with EINVAL, and maps a bad descriptor to EBADF. Fixture-local raw
# file creation, dirtying, seeking, and cleanup are evidence plumbing, not
# selected descriptor/pathname/writeback-policy or durability APIs.
set -euo pipefail
...
assert_sync_file_range_syscall_path() {
    objdump -d --disassemble=sync_file_range "$candidate" >"$sync_file_range_disassembly"
    grep -Eq '\$0x115,%(e|r)ax' "$sync_file_range_disassembly" ||
        fail "sync_file_range lacks Linux syscall 277"
    grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$sync_file_range_disassembly" ||
        fail "sync_file_range lacks a direct syscall"
}
...
assert_candidate_excludes_descriptor_policy() {
    ...
    for symbol in open openat close read write lseek fsync fdatasync sync syncfs \
        fcntl dup dup2 dup3 pipe pipe2 fallocate posix_fallocate posix_fadvise \
        readahead sendfile tee splice vmsplice copy_file_range; do
...
candidate_disassembly="$work_dir/candidate-disassembly"; sync_file_range_disassembly="$work_dir/sync-file-range-disassembly"
cd "$ROOT_DIR"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
    ...
    -o "$reference"
timeout "$EXECUTION_TIMEOUT" "$reference" || fail "pinned-musl sync_file_range fixture failed"
...
grep -Eq '[[:space:]][TW][[:space:]]sync_file_range$' "$archive_symbols" ||
    fail "archive does not define sync_file_range"
...
assert_static_closure
for symbol in __errno_location sync_file_range; do
...
assert_sync_file_range_syscall_path
assert_candidate_excludes_descriptor_policy
timeout "$EXECUTION_TIMEOUT" "$candidate" || fail "freestanding sync_file_range fixture failed"
