...
static struct timespec deadline_after(time_t seconds)
{
...
static int test_mutex_timedlock(void)
{
    ...
    struct timespec future = deadline_after(5);
...
static int test_condition_timedwait(void)
{
    ...
    if (pthread_create(&thread, 0, signaler, 0))
        return 22;
    if (pthread_cond_timedwait(&condition, &condition_mutex, &future))
        return 23;
    if (!atomic_load(&signaler_ready) || pthread_mutex_unlock(&condition_mutex)
...
{
    void *result = 0;
    struct timespec future = deadline_after(30);
    (void)unused;
