#!/usr/bin/env bash
# Native Linux/x86-64 sched_setparam C/C++ header ABI gate.
#
# Pinned musl 1.2.6 and the project header tree must retain the same public
# `int sched_setparam(pid_t, const struct sched_param *)` spelling and LP64
# record layout under strict, POSIX, X/Open, and GNU profiles. This proves
# declaration and C++ C-linkage only; it does not select scheduler records,
# policy, mutation, affinity, lifecycle, or public x86 support.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly C_PROBE="$ROOT_DIR/compat/x86_64/sched_setparam_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/sched_setparam_header_abi_probe.cpp"

fail() { printf 'ERROR: x86 sched_setparam header ABI: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-sched-setparam-header.XXXXXX)"
...
for tree in oracle project; do
    ...
    for profile in strict posix xopen gnu; do
        ...
        grep -Eq '[[:space:]]sched_setparam$' <<<"$undefined" ||
            fail "$tree $profile C++ witness lacks unmangled sched_setparam"
        if grep -Eq '_Z.*sched_setparam' <<<"$undefined"; then
            fail "$tree $profile C++ witness retained a mangled sched_setparam reference"
        fi
    done
done
...
printf 'x86 pinned-musl/project sched_setparam C/C++ header ABI: PASS\n'
