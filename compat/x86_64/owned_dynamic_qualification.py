...
CASES = {
    ...
    "signal-handler-fork": ("run_owned_signal_handler_fork.sh", None),
    ...
    "pthread-cpuclock": ("run_owned_pthread_cpuclock.sh", None),
    ...
    "math-fenv-all-entry": ("run_owned_math_fenv_all_entry.sh", None),
    ...
    for product in PRODUCTS:
        for name, observation in ((f"{product}-consumer", expected), (f"non-pie-{product}", expected), (f"spawn-{product}", b"")):
            ...
            receipt_contract.require_runpath(
                search, "/usr/lib", label="base dynamic link receipt",
                fail=lambda message: require(False, message),
            )
