#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ <time.h> timer_gettime ABI gate.
#
...
# exact opaque-timer and itimerspec external-C declaration. This is
# header-only evidence, not POSIX timer query, lifecycle, or time support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 time.h timer_gettime ABI: %s\n' "$*" >&2
    exit 1
}
        ...
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                if "$compiler" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE \
                    ...
                    -D__STRICT_ANSI__ -DCRABC_TIMER_GETTIME_EXPECT_HIDDEN \
                    -Werror=implicit-function-declaration "${include_args[@]}" \
                    -fsyntax-only "$c_probe" >"$errors" 2>&1; then
                    fail "timer_gettime is visible under strict C11 (${variant})"
                fi
                ...
                >"$errors" 2>&1; then
                fail "timer_gettime is visible under strict C++17 (${variant})"
            fi
        done
    done
}
    ...
    for language in c cxx; do
        for variant in oracle project; do
            ...
            if [ "$language" = c ]; then
                ...
                object="$work_dir/${variant}-${profile}-timer-gettime-cxx.o"
...
c_probe="$ROOT_DIR/compat/x86_64/timer_gettime_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/timer_gettime_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-timer-gettime-header.XXXXXX)"
...
if ! "$ORACLE_CC" -std=c11 -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
    ...
    fail "project POSIX C timer_gettime header contract drifted"
fi
...
printf 'x86 pinned-musl/project C/C++ <time.h> timer_gettime ABI: PASS\n'
