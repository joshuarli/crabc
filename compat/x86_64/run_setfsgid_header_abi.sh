#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ setfsgid declaration gate.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/setfsgid_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/setfsgid_header_abi_probe.cpp"

fail() { printf 'ERROR: x86 sys/fsuid.h setfsgid ABI: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-setfsgid-header.XXXXXX)"
...
for tree in oracle project; do
    ...
    object="$work_dir/$tree-setfsgid-cxx.o"
    "$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
        "${include_args[@]}" -c "$CXX_PROBE" -o "$object"
    undefined="$(nm --undefined-only "$object")"
    grep -Eq '[[:space:]]setfsgid$' <<<"$undefined" ||
        fail "$tree C++ witness lacks unmangled setfsgid"
    if grep -Eq '_Z.*setfsgid' <<<"$undefined"; then
        fail "$tree C++ witness retained a mangled setfsgid reference"
    fi
done
