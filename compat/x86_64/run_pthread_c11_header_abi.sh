...
assert_cxx_c_linkage() {
    ...
    if [ "$profile" = cxx17-gnu ]; then
        required_symbols+=(pthread_sigmask)
    fi
    for symbol in "${required_symbols[@]}"; do
        printf '%s\n' "$symbols" | grep -Fxq "$symbol" ||
            fail "$label does not request C-linkage symbol $symbol"
    done
    if printf '%s\n' "$symbols" | grep -Eq '(^|.*)_Z.*(pthread_create|pthread_detach|pthread_self|pthread_equal|pthread_mutex_init|pthread_mutex_destroy|pthread_mutex_lock|pthread_mutex_trylock|pthread_mutex_unlock|pthread_sigmask|thrd_create|thrd_detach|thrd_join|thrd_exit|thrd_sleep|thrd_current|thrd_equal)'; then
