...
assert_cxx_c_linkage() {
    ...
    if printf '%s\n' "$symbols" | grep -Eq '(^|.*)_Z.*(pthread_create|pthread_detach|pthread_self|pthread_equal|pthread_sigmask|thrd_create|thrd_detach|thrd_join|thrd_exit|thrd_current|thrd_equal)'; then
