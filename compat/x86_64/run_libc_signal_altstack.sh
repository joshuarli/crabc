#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc alternate-stack evidence.
#
...
fail() {
    printf 'ERROR: x86 static libc signal altstack: %s\n' "$*" >&2
    exit 1
}
...
assert_selected_c_abi_surface() {
    ...
    # Inspect only crate-owned C objects. Compiler-builtins archive members
    # are toolchain support, not part of the selected C ABI ratchet.
...
candidate="$work_dir/crabc-static-signal-altstack-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_signal_altstack_probe.c >/dev/null 2>"$header_trace"
for header in errno.h features.h signal.h stddef.h stdint.h sys/syscall.h bits/syscall.h \
...
for symbol in __errno_location sigaltstack sigaction signal sigemptyset raise; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_SIGNAL_ALTSTACK_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_signal_altstack_probe.c \
    compat/x86_64/libc_signal_altstack_start.S "$archive" -o "$candidate"
...
for symbol in __errno_location sigaltstack sigaction signal sigemptyset raise \
...
for unrelated in syscall malloc free calloc realloc tgkill signalfd pthread_create \
    pthread_exit pthread_join pthread_kill pthread_sigmask sysconf getauxval clone fork vfork posix_spawn wait \
    waitpid waitid; do
    if grep -Eq "[[:space:]]${unrelated}$" "$candidate_symbols"; then
        fail "signal-altstack candidate unexpectedly pulls ${unrelated}"
    fi
done
if grep -Eq '[[:space:]](__gxx_personality_v0|__cxa_[[:alnum:]_]+|_Unwind_[[:alnum:]_]+)$' \
    "$candidate_symbols"; then
    fail "signal-altstack candidate unexpectedly pulls a C++ runtime"
fi
