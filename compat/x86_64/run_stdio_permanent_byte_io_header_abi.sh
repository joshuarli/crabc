#!/usr/bin/env bash
# Native Linux/x86-64 permanent-byte-I/O <stdio.h> declaration/linkage proof.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_byte_io_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_byte_io_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 stdio permanent byte-I/O header ABI: %s\n' "$*" >&2
    exit 1
}
...
compile_c() {
    ...
    run_compiler "$compiler" -x c -std=c11 -nostdinc -I "$include_root" \
        ...
        -DCRABC_STDIO_PERMANENT_BYTE_IO_C11 -fsyntax-only "$C_PROBE" \
        >/dev/null 2>"$trace"
}
...
compile_cxx() {
    ...
    run_compiler "$compiler" -x c++ -std=c++17 -nostdinc -nostdinc++ \
        ...
        -H -fno-builtin -DCRABC_STDIO_PERMANENT_BYTE_IO_CXX17 \
...
[ -f "$C_PROBE" ] || fail "missing C permanent-byte-I/O header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ permanent-byte-I/O header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-stdio-permanent-byte-io-header.XXXXXX)"
...
printf 'x86 stdio permanent byte-I/O header ABI: PASS\n'
