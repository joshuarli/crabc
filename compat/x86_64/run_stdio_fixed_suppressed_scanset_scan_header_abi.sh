#!/usr/bin/env bash
# Native Linux/x86-64 C11/C++17 suppressed-scanset scanf declaration/linkage proof.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/stdio_fixed_suppressed_scanset_scan_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/stdio_fixed_suppressed_scanset_scan_header_abi_probe.cpp"

fail() {
    printf 'ERROR: x86 stdio suppressed-scanset scan header ABI: %s\n' "$*" >&2
    exit 1
}
...
compile_c() {
    ...
    run_compiler "$compiler" -x c -std=c11 -nostdinc -I "$include_root" \
        ...
        -DCRABC_STDIO_FIXED_SUPPRESSED_SCANSET_SCAN_HEADER_C11 \
...
compile_cxx() {
    ...
    run_compiler "$compiler" -x c++ -std=c++17 -nostdinc -nostdinc++ \
        ...
        -DCRABC_STDIO_FIXED_SUPPRESSED_SCANSET_SCAN_HEADER_CXX17 \
...
[ -f "$C_PROBE" ] || fail "missing C suppressed-scanset-scan header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ suppressed-scanset-scan header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-stdio-fixed-suppressed-scanset-scan-header.XXXXXX)"
...
printf 'x86 stdio suppressed-scanset-scan header ABI: PASS\n'
