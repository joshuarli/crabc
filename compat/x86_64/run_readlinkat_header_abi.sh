#!/usr/bin/env bash
# Native Linux/x86-64 selected readlinkat C/C++ header ABI matrix.
#
# Pinned musl 1.2.6 is the declaration, LP64 byte-count layout, and unmangled
...
# This proves one caller-buffered descriptor-relative C leaf, not ordinary
# readlink, pathname policy, allocation, CWD state, or public x86 support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/readlinkat_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/readlinkat_header_abi_probe.cpp"
...
fail() {
    printf 'ERROR: x86 readlinkat headers: %s\n' "$*" >&2
    exit 1
}
...
[ -f "$C_PROBE" ] || fail "missing C readlinkat ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ readlinkat ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-readlinkat-header.XXXXXX)"
...
printf 'x86 pinned-musl/project C/C++ readlinkat ABI matrix: PASS (%s profiles; visible=%s hidden=%s; compile-only)\n' \
