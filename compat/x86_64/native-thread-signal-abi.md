...
`run_native_thread_signal_abi.sh` accepts one validated installed static
product and one validated installed dynamic product:

```sh
bash compat/x86_64/run_native_thread_signal_abi.sh \
...
Run it only inside the pinned native Docker image with an explicit
checkout-local `TMPDIR`; the retained command logs identify the exact image,
source commit, source files, installed products, headers, objects, link
commands, raw ELF tables, and process transcripts. It does not need a new
public dispatcher command.
...
promotion, or public support. The normal static export roster, header-extension
accounting, parity route, and generated ledgers are integrated separately by
the owning root workflows. Until those workflows fold in all independently
reviewed static providers, the runner preserves the raw surplus over the old
roster, rejects any missing old row, and requires the one expected strong
`tgkill` addition. That local check is not a replacement for the canonical
static-surface ratchet.
