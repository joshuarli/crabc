#!/usr/bin/env bash
# Native Linux/x86-64 bounded static pthread CPU-clock evidence.
#
...
fail() {
    printf 'ERROR: x86 static pthread CPU clock: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pthread-cpuclock.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-pthread-cpuclock-reference"
candidate="$work_dir/crabc-static-pthread-cpuclock-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_cpuclock_probe.c >/dev/null 2>"$header_trace"
for header in errno.h pthread.h stdint.h sys/syscall.h time.h bits/alltypes.h \
    bits/syscall.h; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -pthread -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_pthread_cpuclock_probe.c \
    -o "$reference"
if "$reference"; then
    :
else
    status=$?
    fail "pinned-musl pthread CPU-clock fixture exited ${status}"
fi
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap \
    pthread_getcpuclockid pthread_self clock_gettime; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_PTHREAD_CPUCLOCK_FREESTANDING \
    ...
    -Wl,--no-undefined compat/x86_64/libc_pthread_cpuclock_probe.c \
    compat/x86_64/libc_pthread_cpuclock_start.S "$archive" -o "$candidate"
...
if "$candidate"; then
    ...
    status=$?
    fail "freestanding pthread CPU-clock fixture exited ${status}"
fi

printf 'x86 static crabc-libc pthread CPU clock: PASS\n'
