#!/usr/bin/env bash
# Pinned-musl differential for the private x86 scalar cos/cosf artifact.
set -euo pipefail
...
readonly PRIVATE_PROVIDERS=(
	...
	crabc_x86_math_cos_kernel_cos
	crabc_x86_math_cos_kernel_sindf
	crabc_x86_math_cos_kernel_cosdf
	crabc_x86_math_cos_reduce_pio2
	crabc_x86_math_cos_reduce_pio2f
	crabc_x86_math_cos_reduce_pio2_large
	crabc_x86_math_cos_provider_floor
	crabc_x86_math_cos_provider_scalbn
)

fail() { printf 'ERROR: x86 static libc cos/cosf: %s\n' "$*" >&2; exit 1; }
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
	compat/x86_64/libc_math_cos_probe.c >/dev/null 2>"$header_trace"
for header in fenv.h float.h math.h stddef.h stdint.h features.h bits/alltypes.h; do
	grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
		fail "fixture did not use project $header"
done
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
	compat/x86_64/libc_math_cos_probe.c -lm -o "$reference"
"$reference" >"$reference_output" || fail "pinned-musl cos/cosf fixture failed"
...
for mode in sse x87; do
	...
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -c compat/x86_64/math_cos_header_abi_probe.cpp \
		-o "$header_cxx_reference"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" \
		-c compat/x86_64/math_cos_header_abi_probe.cpp -o "$header_cxx_candidate"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only \
		compat/x86_64/math_cos_header_abi_probe.cpp >/dev/null 2>"$cxx_header_trace"
	...
	for object in "$header_cxx_reference" "$header_cxx_candidate"; do
		...
		if grep -Eq '_Z.*cos' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled cosine reference"
		fi
	done
done
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_COS_FREESTANDING \
	-I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
	-fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
	-Wl,--gc-sections compat/x86_64/libc_math_cos_probe.c \
	compat/x86_64/libc_math_cos_start.S "$archive" -o "$candidate"
...
for unselected in cosl sin sinf sinl sincos sincosf sincosl tan tanf tanl \
...
for instruction in addsd addss subsd subss mulsd cvtsd2ss cvtss2sd; do
...
printf 'x86 static libc cos/cosf: PASS (%s records)\n' "$record_count"
