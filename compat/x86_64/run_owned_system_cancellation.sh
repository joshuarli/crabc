...
for item in objects:
    ...
    if local_dependencies != source_files:
        fail(f"{role} local dependency roster drifted")
...
# The canonical compile receipt is a two-role aggregate: it proves that the
# consumer and fixed child remain distinct, with their own source and installed
# header closures. Each resulting executable has one canonical object, so the
# shared validator can independently prove its sealed product, receipt, and
# ELF boundary without duplicating those product contracts here.
audit_link() {
    local family="$1" product="$2" mode="$3" role="$4"
    local candidate="$5" object_path="$6" receipt="$7"
    case "$family/$mode" in
        static/static|static/static-pie|dynamic/pie|dynamic/non-pie) ;;
        *) fail "unknown product linkage: $family/$mode" ;;
    esac
    python3 -B - "$ROOT" "$product" "$object_path" "$candidate" "$receipt" \
        "$mode" "$role" "$work/compile.json" "$family" <<'PY_AUDIT'
from hashlib import sha256
import json
from pathlib import Path
import sys
...
(
    checkout_text, product_text, object_text, candidate_text, receipt_text,
    mode, role, compile_text, family,
) = sys.argv[1:]
checkout = Path(checkout_text).resolve(strict=True)
product = Path(product_text).resolve(strict=True)
object_path = Path(object_text).resolve(strict=True)
candidate = Path(candidate_text).resolve(strict=True)
receipt = Path(receipt_text).resolve(strict=True)
compile_path = Path(compile_text).resolve(strict=True)

if role not in {"consumer", "child"}:
    raise SystemExit("system cancellation artifact: unknown canonical role")
sys.path.insert(0, str(checkout / "compat/x86_64"))
from owned_posix_product_evidence import validate_link

def digest(path: Path) -> str:
    if not path.is_file() or path.is_symlink():
        raise SystemExit(f"system cancellation artifact: non-physical artifact: {path}")
    return sha256(path.read_bytes()).hexdigest()

identity = validate_link(product, object_path, candidate, receipt, mode)
evidence = {
    "schema": "crabc.system-cancellation-link/v2",
    "family": family,
    "linkage": mode,
    "role": role,
    "canonical_compile_receipt": {
        "path": str(compile_path), "sha256": digest(compile_path),
    },
    "canonical_object": {
        "path": str(object_path), "sha256": digest(object_path),
    },
    "sealed_link": identity,
}
evidence_path = Path(str(candidate) + ".evidence.json")
if evidence_path.exists() or evidence_path.is_symlink():
    raise SystemExit("system cancellation artifact: link evidence path is unsafe")
...
run_product() {
    ...
    for mode in "${modes[@]}"; do
        ...
        for role in consumer child; do
            ...
            audit_link "$family" "$product" "$mode" "$role" "$candidate" "$object_path" "$receipt"
            if [ "$role" = child ]; then
