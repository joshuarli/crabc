#!/usr/bin/env bash
# Native Linux/x86-64 bounded static pthread condition-attribute pshared evidence.
#
...
# archive. It proves exactly pthread_condattr_setpshared/getpshared's
# four-byte record behavior: accepted 0/1 values replace only bit 31, retaining
# the low clock-record bits; invalid values preserve the complete word.
# It does not select attribute lifecycle, condition clock selection,
# initialization or operation, process-shared condition operation, thread,
# TLS, synchronization, cancellation, CRT, loader, sysroot, or public x86
# support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 static pthread condattr pshared: %s\n' "$*" >&2
    exit 1
}
...
assert_no_unselected_condattr_exports() {
    ...
    for unselected in pthread_condattr_init pthread_condattr_destroy \
        ...
        [ "$symbols_path" = "$candidate_symbols" ] ||
            fail "condattr pshared sibling exclusions apply only to the final candidate"
...
assert_direct_record_path() {
    ...
    for symbol in pthread_condattr_setpshared pthread_condattr_getpshared; do
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-pthread-condattr-pshared.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-pthread-condattr-pshared-reference"
candidate="$work_dir/crabc-static-pthread-condattr-pshared-candidate"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_pthread_condattr_pshared_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -pthread -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_pthread_condattr_pshared_probe.c \
    -o "$reference"
if timeout "$EXECUTION_TIMEOUT" "$reference"; then
    :
else
    status=$?
    fail "pinned-musl condattr pshared fixture exited ${status}"
fi
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_PTHREAD_CONDATTR_PSHARED_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie -ffreestanding \
    -fno-builtin -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_pthread_condattr_pshared_probe.c \
    compat/x86_64/libc_pthread_condattr_pshared_start.S "$archive" -o "$candidate"
...
for unselected in __errno_location __crabc_x86_static_tls_bootstrap \
    ...
    pthread_mutex_unlock pthread_cond_timedwait; do
...
if timeout "$EXECUTION_TIMEOUT" "$candidate"; then
    ...
    status=$?
    fail "freestanding condattr pshared fixture exited ${status}"
fi

printf 'x86 static crabc-libc pthread condattr pshared: PASS\n'
