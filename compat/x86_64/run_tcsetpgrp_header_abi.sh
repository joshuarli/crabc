#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ tcsetpgrp declaration gate.
#
# Pinned musl 1.2.6 is the unconditional <unistd.h> declaration and C-linkage
# oracle. The project-header pass selects neither foreground-group behavior nor
...
fail() {
    printf 'ERROR: x86 unistd.h tcsetpgrp ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/tcsetpgrp_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/tcsetpgrp_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-tcsetpgrp-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-tcsetpgrp-cxx.o"
candidate_cxx_object="$work_dir/candidate-tcsetpgrp-cxx.o"

# Musl declares tcsetpgrp independently of the ordinary strict/POSIX/XSI/GNU/
...
if ! "$ORACLE_CC" -std=c11 -fno-builtin -I "$ROOT_DIR/include" -H \
    ...
    fail "project C tcsetpgrp header contract drifted"
fi
...
for object in "$oracle_cxx_object" "$candidate_cxx_object"; do
    ...
    grep -Eq '[[:space:]]tcsetpgrp$' <<<"$undefined" ||
        fail "C++ probe does not retain C linkage for tcsetpgrp"
    if grep -Eq '_Z9tcsetpgrpii' <<<"$undefined"; then
        fail "C++ probe retained a mangled tcsetpgrp reference"
    fi
done

printf 'x86 pinned-musl/project C/C++ <unistd.h> tcsetpgrp ABI: PASS\n'
