#!/usr/bin/env bash
# Native Linux/x86-64 selected static cbrt/cbrtf evidence.
set -euo pipefail
...
fail() { printf 'ERROR: x86 static libc math cbrt: %s\n' "$*" >&2; exit 1; }
...
candidate="$work_dir/crabc-static-math-cbrt-candidate"
reference_records="$work_dir/musl-records"
candidate_records="$work_dir/candidate-records"
header_cxx_reference="$work_dir/musl-math-cbrt-header.o"
header_cxx_candidate="$work_dir/project-math-cbrt-header.o"
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -fno-builtin -fno-stack-protector \
	compat/x86_64/libc_math_cbrt_probe.c -o "$reference"
"$reference" >"$reference_records" || fail "pinned-musl math cbrt fixture failed"
[ -s "$reference_records" ] || fail "pinned-musl math cbrt emitted no differential records"
...
for mode in sse x87; do
	...
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -c compat/x86_64/math_cbrt_header_abi_probe.cpp \
		-o "$header_cxx_reference"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" \
		-c compat/x86_64/math_cbrt_header_abi_probe.cpp -o "$header_cxx_candidate"
	"$ORACLE_CC" -std=c++17 -x c++ -D_GNU_SOURCE -fno-builtin \
		"${arguments[@]}" -I "$ROOT_DIR/include" -H -fsyntax-only \
		compat/x86_64/math_cbrt_header_abi_probe.cpp >/dev/null 2>"$cxx_header_trace"
	...
	for object in "$header_cxx_reference" "$header_cxx_candidate"; do
		...
		for symbol in cbrt cbrtf; do
			grep -Eq "[[:space:]]${symbol}$" <<<"$undefined" ||
				fail "C++ ${mode} probe does not retain unmangled ${symbol}"
		done
		if grep -Eq '_Z.*cbrt' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled cbrt reference"
		fi
	done
done
...
for symbol in cbrt cbrtf feclearexcept fegetenv fegetround fesetenv fesetround fetestexcept; do
...
"$ORACLE_CC" -std=c11 -D_GNU_SOURCE -DCRABC_MATH_CBRT_FREESTANDING \
	...
	compat/x86_64/libc_math_cbrt_probe.c compat/x86_64/libc_math_cbrt_start.S \
	"$archive" -o "$candidate"
...
for unselected in cbrtl fmod fmodf fmodl modf modff modfl fma fmaf fmal \
	...
	sqrt sqrtf sqrtl cabs cabsf cabsl carg cargf cargl cproj cprojf cprojl; do
...
if grep -Eq 'crabc_core|mimalloc|sha_crypt|libm|math_special|math_complex|math_elementary_long_double|math_fmod' \
	"$candidate_symbols" "$disassembly"; then
	fail "candidate selects an unowned math/runtime dependency"
fi
for instruction in divsd mulsd cvtsd2ss; do
	grep -Eq "[[:space:]]${instruction}([[:space:]]|$)" "$disassembly" ||
		fail "candidate lacks ${instruction} musl arithmetic"
done
if grep -Eq '\bvfm(add|sub)' "$disassembly"; then
	fail "candidate unexpectedly contracts musl arithmetic with FMA"
fi
"$candidate" >"$candidate_records" || fail "freestanding math cbrt fixture failed"
