#!/usr/bin/env bash
# Native Linux/x86-64 munlockall C/C++ declaration evidence.
#
# Pinned musl 1.2.6 is the declaration and C-linkage oracle. This compile-only
# matrix selects the zero-argument munlockall spelling; it does not claim
# whole-process lock policy, mlockall behavior, or public x86 support.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 sys/mman.h munlockall ABI: %s\n' "$*" >&2
    exit 1
}
    ...
    for variant in oracle project; do
        ...
        if [ "$language" = c ]; then
            "$ORACLE_CC" -std=c11 -U_GNU_SOURCE "$@" "${include_args[@]}" \
                -fsyntax-only "$ROOT_DIR/compat/x86_64/munlockall_header_abi_probe.c" ||
                fail "${variant} C ${profile} declaration/profile drifted"
        else
            "$ORACLE_CC" -std=c++17 -x c++ -U_GNU_SOURCE "$@" \
                "${include_args[@]}" -fsyntax-only \
                "$ROOT_DIR/compat/x86_64/munlockall_header_abi_probe.cpp" ||
                fail "${variant} C++ ${profile} declaration/profile drifted"
        fi
    done
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-munlockall-header.XXXXXX)"
...
if ! "$ORACLE_CC" -std=c11 -D__STRICT_ANSI__ -U_GNU_SOURCE \
    ...
    "$ROOT_DIR/compat/x86_64/munlockall_header_abi_probe.c" \
    >/dev/null 2>"$header_trace"; then
    sed -n '1,160p' "$header_trace" >&2
    fail "project munlockall header contract drifted"
fi
...
for variant in oracle project; do
    include_args=()
    [ "$variant" = project ] && include_args=(-I "$ROOT_DIR/include")
    object="$work_dir/${variant}-munlockall-cxx.o"
    "$ORACLE_CC" -std=c++17 -x c++ -D__STRICT_ANSI__ -U_GNU_SOURCE \
        "${include_args[@]}" -c \
        "$ROOT_DIR/compat/x86_64/munlockall_header_abi_probe.cpp" -o "$object"
    undefined="$(nm --undefined-only "$object")"
    grep -Eq '[[:space:]]munlockall$' <<<"$undefined" ||
        fail "C++ probe does not retain C linkage for munlockall (${variant})"
    if grep -Eq '_Z[0-9].*munlockall' <<<"$undefined"; then
        fail "C++ probe retained mangled munlockall reference (${variant})"
    fi
done

printf 'x86 pinned-musl/project C/C++ <sys/mman.h> munlockall ABI: PASS\n'
