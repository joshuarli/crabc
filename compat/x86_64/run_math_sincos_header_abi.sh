#!/usr/bin/env bash
# Native Linux/x86-64 C++ declaration/linkage evidence for sincos/sincosf.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly SYMBOLS=(sincos sincosf)

fail() { printf 'ERROR: x86 math sincos header ABI: %s\n' "$*" >&2; exit 1; }
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-math-sincos-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
probe="$ROOT_DIR/compat/x86_64/math_sincos_header_abi_probe.cpp"
...
for mode in sse x87; do
	...
	for object in "$reference" "$candidate"; do
		...
		if grep -Eq '_Z.*sincos' <<<"$undefined"; then
			fail "C++ ${mode} probe retained a mangled sincos reference"
		fi
	done
done

printf 'x86 pinned-musl/project sincos/sincosf header ABI: PASS\n'
