#!/usr/bin/env bash
# Native Linux/x86-64 static crabc-libc personality evidence.
#
...
# Linux syscall 135, including the all-ones non-mutating query result and stale
# errno on ordinary returns, without selecting process-control policy.
set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
readonly ORACLE_CC=/usr/local/bin/crabc-x86_64-musl-gcc
readonly STATIC_C_ABI_EXPORTS="$ROOT_DIR/compat/x86_64/static_c_abi_exports.txt"
readonly SOURCE="$ROOT_DIR/libc/src/c_abi/x86_64/personality.rs"

fail() { printf 'ERROR: x86 static libc personality: %s\n' "$*" >&2; exit 1; }
...
assert_personality_boundary() {
    ...
    objdump -d --disassemble=personality "$binary" >"$disassembly"
    grep -Eq '\$0x87(,|[[:space:]]|$)' "$disassembly" ||
        fail "personality does not issue syscall 135"
    grep -Eq '[[:space:]]syscall([[:space:]]|$)' "$disassembly" ||
        fail "personality lacks a syscall instruction"
    grep -Eq '%fs:(0x0|0x[0-9a-f]+|-)' "$disassembly" ||
        fail "personality does not retain the initial-TLS errno path"
    if grep -Eq '\$0x9d(,|[[:space:]]|$)' "$disassembly"; then
        fail "personality unexpectedly issues prctl"
    fi
}
...
[ -f "$SOURCE" ] || fail "missing personality source"

bash "$ROOT_DIR/compat/x86_64/run_musl_oracle.sh" >/dev/null
bash "$ROOT_DIR/compat/x86_64/run_personality_header_abi.sh" >/dev/null

work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-personality.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
reference="$work_dir/musl-personality-reference"
candidate="$work_dir/crabc-static-personality-candidate"
...
"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_personality_probe.c >/dev/null 2>"$header_trace"
for header in errno.h stdint.h sys/personality.h sys/syscall.h bits/syscall.h; do
    grep -Fq "$ROOT_DIR/include/$header" "$header_trace" ||
        fail "fixture omitted project $header"
done

"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_personality_probe.c -o "$reference"
"$reference"
...
for symbol in __errno_location __crabc_x86_static_tls_bootstrap personality; do
...
for marker in 'src/linux/personality.c::personality' 'SYS_PERSONALITY' \
    'pub unsafe extern "C" fn personality'; do
    grep -Fq "$marker" "$SOURCE" || fail "source lacks ${marker}"
done
if grep -Eq 'SYS_PRCTL|SYS_CAPGET|SYS_CAPSET|SYS_SETNS|SYS_UNSHARE' "$SOURCE"; then
    fail "source must not select a broader administrative syscall"
fi
...
assert_personality_boundary "$archive" "$work_dir/archive-personality-disassembly"

"$ORACLE_CC" -std=c11 -DCRABC_PERSONALITY_FREESTANDING -I"$ROOT_DIR/include" \
    -nostdlib -static -fno-pie -no-pie -ffreestanding -fno-builtin \
    -fno-stack-protector -Wl,-e,_start -Wl,--no-undefined \
    compat/x86_64/libc_personality_probe.c \
    compat/x86_64/libc_personality_start.S "$archive" -o "$candidate"
...
for unrelated in prctl capget capset setns unshare iopl ioperm \
    setfsuid setfsgid setuid setgid setgroups setresuid setresgid \
...
if grep -Eqi 'arch_prctl|mov[[:space:]]+%rsi,[[:space:]]*%fs:0' \
    compat/x86_64/libc_personality_start.S; then
    fail "fixture start must not install a private FS base"
fi
assert_personality_boundary "$candidate" "$work_dir/candidate-personality-disassembly"

"$candidate"
printf 'x86 static crabc-libc personality: PASS\n'
