...
closure_disassembly="$work_dir/candidate-closure-disassembly"
...
: >"$closure_disassembly"
for symbol in "${SELECTED_SYMBOLS[@]}" "${PRIVATE_PROVIDERS[@]}"; do
	objdump -d --disassemble="$symbol" "$candidate" >>"$closure_disassembly"
done
for symbol in "${SELECTED_SYMBOLS[@]}" "${FENV_SIBLINGS[@]}"; do
...
if grep -Eq 'crabc_core|mimalloc|float_parse|math_special|math_elementary_long_double|libm' \
	"$closure_disassembly"; then
	fail "candidate selects an unowned math/runtime dependency"
fi
# The aggregate archive can retain unrelated local-only complex-math text in
# an extracted codegen member. It has no public C export and is not part of
# the tan entry's source closure; global runtime/provider boundaries above
# remain the candidate admission check.
for instruction in addsd addss subsd subss mulsd mulss divsd cvtsd2ss cvtss2sd; do
	grep -Eq "[[:space:]]${instruction}([[:space:]]|$)" "$closure_disassembly" ||
		fail "candidate lacks scalar ${instruction} path"
done
if grep -Eq '[[:space:]](fldt|fstpt)([[:space:]]|$)' "$closure_disassembly"; then
	fail "candidate accidentally retains binary80 instructions"
fi
if grep -Eq '[[:space:]](v[a-z0-9]+|addp[sd]|subp[sd]|mulp[sd]|divp[sd]|sqrtp[sd])([[:space:]]|$)' "$closure_disassembly"; then
	fail "candidate accidentally retains AVX or packed-SIMD math"
fi
if grep -Eq 'vfmadd|vfnmadd|vfmsub|vfnmsub' "$closure_disassembly"; then
