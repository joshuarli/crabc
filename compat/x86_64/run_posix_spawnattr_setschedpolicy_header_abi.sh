#!/usr/bin/env bash
# Native Linux/x86-64 POSIX spawn-attribute setschedpolicy C/C++ declaration gate.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_setschedpolicy_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/posix_spawnattr_setschedpolicy_header_abi_probe.cpp"
readonly -a PROFILES=(c11-strict c11-posix-2008 c11-xopen-700 c11-gnu cxx17-strict cxx17-gnu)

fail() {
    printf 'ERROR: x86 spawn.h posix_spawnattr_setschedpolicy headers: %s\n' "$*" >&2
    exit 1
}
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-posix-spawnattr-setschedpolicy-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    for tree in reference candidate; do
        ...
        compile_profile "$tree" "$profile" "$trace" "$object" ||
            fail "$profile $tree unconditional posix_spawnattr_setschedpolicy declaration failed"
...
printf 'x86 pinned-musl/project C/C++ <spawn.h> posix_spawnattr_setschedpolicy ABI: PASS (%s unconditional profiles)\n' \
    "${#PROFILES[@]}"
