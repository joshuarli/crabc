#!/usr/bin/env bash
# Native Linux/x86-64 selected GNU renameat2 C/C++ header ABI matrix.
#
# Pinned musl 1.2.6 is the declaration, GNU-profile, LP64-layout, and
# unmangled C++ C-linkage oracle. The candidate uses raw GCC with only project
# and compiler builtin headers, so an ambient libc cannot hide a mismatch.
# This proves one GNU declaration and flag vocabulary, not ordinary rename,
# a pathname family, or public x86 support.
set -euo pipefail
export LC_ALL=C
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/renameat2_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/renameat2_header_abi_probe.cpp"
readonly EXPECTED_PROFILE_COUNT=8
readonly EXPECTED_VISIBLE_PROFILE_COUNT=2
readonly EXPECTED_HIDDEN_PROFILE_COUNT=6
readonly -a PROFILES=(c-default c11-gnu cxx17-gnu c11-strict c11-posix-2008 c11-xopen-700 c11-bsd cxx17-strict)

fail() {
    printf 'ERROR: x86 stdio.h renameat2 headers: %s\n' "$*" >&2
    exit 1
}
...
compile_profile() {
    ...
    case "$expectation" in
        gnu-visible)
            arguments+=(-DCRABC_EXPECT_RENAMEAT2 -DCRABC_EXPECT_RENAMEAT2_GNU_FLAGS)
            ;;
        hidden) arguments+=(-DCRABC_REQUIRE_RENAMEAT2_HIDDEN) ;;
        *) fail "unknown renameat2 visibility expectation: $expectation" ;;
    esac
    case "$profile" in
        c-default)
            source="$C_PROBE"
...
check_trace() {
    ...
    for header in stdio.h features.h; do
...
[ -f "$C_PROBE" ] || fail "missing C renameat2 ABI probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ renameat2 ABI probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-renameat2-header.XXXXXX)"
...
for profile in "${PROFILES[@]}"; do
    ...
    expectation="$(profile_expectation "$profile")"
    if [ "$expectation" != hidden ]; then
        ...
        if compile_profile reference "$profile" hidden "$reference_trace" "$reference_object"; then
            fail "$profile pinned-musl unexpectedly exposes renameat2"
        fi
        grep -Fq renameat2 "$reference_trace" ||
            fail "$profile pinned-musl hidden check did not diagnose renameat2"
        if compile_profile candidate "$profile" hidden "$candidate_trace" "$candidate_object"; then
            fail "$profile project headers unexpectedly expose renameat2"
        fi
        grep -Fq renameat2 "$candidate_trace" ||
            fail "$profile project hidden check did not diagnose renameat2"
    fi
done
...
printf 'x86 pinned-musl/project C/C++ <stdio.h> renameat2 ABI: PASS\n'
