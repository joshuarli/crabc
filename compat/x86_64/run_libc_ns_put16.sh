#!/usr/bin/env bash
# Native Linux/x86-64 selected static crabc-libc ns_put16 evidence.
#
...
# extracted crabc object. It selects only ns_parse.c's caller-owned 16-bit wire
# write; it does not select resolver state, `/etc/resolv.conf`, DNS packet I/O,
...
fail() {
    printf 'ERROR: x86 static libc ns_put16: %s\n' "$*" >&2
    exit 1
}
...
extract_selected_member() {
    ...
    (
        ...
        for member in "${members[@]}"; do
            ...
            if grep -Eq '[[:space:]][T][[:space:]]ns_put16$' <<<"$definitions"; then
                if grep -Eq '[[:space:]][T][[:space:]](dn_expand|dn_skipname|ns_get16|ns_get32|ns_put32|ns_initparse|ns_parserr|ns_skiprr|ns_name_uncompress)$' <<<"$definitions"; then
                    fail "ns_put16 archive member also defines a nameserver sibling"
                fi
                printf '%s\n' "$member"
            fi
        done
    ...
    [ "${#matches[@]}" = 1 ] || fail "ns_put16 must have exactly one selected archive member"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-libc-ns-put16.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
cargo_target="$work_dir/cargo-target"
reference="$work_dir/musl-ns-put16-reference"
candidate="$work_dir/crabc-static-ns-put16-candidate"
archive="$cargo_target/x86_64-unknown-linux-musl/debug/libc.a"
musl_archive="$("$ORACLE_CC" -print-file-name=libc.a)"
musl_object="$work_dir/musl-ns-parse.o"
musl_disassembly="$work_dir/musl-ns-put16-disassembly"
header_trace="$work_dir/header-trace"
archive_symbols="$work_dir/archive-symbols"
selected_c_abi_symbols="$work_dir/selected-c-abi-symbols"
expected_c_abi_symbols="$work_dir/expected-c-abi-symbols"
selected_members="$work_dir/selected-ns-put16-members"
selected_member_names="$work_dir/selected-ns-put16-member-names"
...
readelf --symbols --wide "$musl_object" |
    grep -E '[[:space:]]10[[:space:]]+FUNC[[:space:]]+GLOBAL[[:space:]]+DEFAULT[[:space:]]+[0-9]+[[:space:]]+ns_put16$' >/dev/null ||
    fail "pinned musl ns_put16 object layout drifted"
objdump -dr --disassemble=ns_put16 "$musl_object" >"$musl_disassembly"
if grep -Eq '\b(call|syscall)\b|R_X86_64_' "$musl_disassembly"; then
    fail "pinned musl ns_put16 text unexpectedly depends on another boundary"
fi

"$ORACLE_CC" -std=c11 -I"$ROOT_DIR/include" -E -H \
    compat/x86_64/libc_ns_put16_probe.c >/dev/null 2>"$header_trace"
...
"$ORACLE_CC" -std=c11 -fno-builtin -fno-stack-protector \
    -I"$ROOT_DIR/include" compat/x86_64/libc_ns_put16_probe.c \
    -o "$reference"
if env -i LC_ALL=C TZ=UTC "$reference"; then
    :
else
    status=$?
    fail "pinned-musl ns_put16 fixture exited ${status}"
fi
...
grep -Eq '[[:space:]][T][[:space:]]ns_put16$' "$archive_symbols" ||
    fail "archive does not define ns_put16"
selected_member="$(extract_selected_member "$archive" "$selected_members" \
    "$selected_member_names")"
[ -f "$selected_member" ] || fail "selected ns_put16 member is missing"

"$ORACLE_CC" -std=c11 -DCRABC_NS_PUT16_FREESTANDING \
    -I"$ROOT_DIR/include" -nostdlib -static -fno-pie -no-pie \
    -ffreestanding -fno-builtin -fno-stack-protector -Wl,-e,_start \
    -Wl,--gc-sections -Wl,--no-undefined compat/x86_64/libc_ns_put16_probe.c \
    compat/x86_64/libc_ns_put16_start.S "$selected_member" -o "$candidate"

readelf --symbols --wide "$candidate" >"$candidate_symbols"
readelf --program-headers --wide "$candidate" >"$candidate_program_headers"
readelf --dynamic --wide "$candidate" >"$candidate_dynamic" || true
readelf --relocs --wide "$candidate" >"$candidate_relocations"
objdump -d --disassemble=ns_put16 "$candidate" >"$candidate_disassembly"
grep -Eq '[[:space:]][T][[:space:]]ns_put16$' <(nm -g --defined-only "$candidate") ||
    fail "archive-free candidate does not retain ns_put16"
...
if grep -Eq '\b(call|syscall)\b' "$candidate_disassembly"; then
    fail "ns_put16 candidate calls an unselected runtime boundary"
fi
for unselected in dn_expand dn_skipname ns_get16 ns_get32 ns_put32 ns_initparse \
...
if env -i LC_ALL=C TZ=UTC "$candidate"; then
    ...
    status=$?
    fail "freestanding ns_put16 fixture exited ${status}"
fi

printf 'x86 static crabc-libc ns_put16: PASS\n'
