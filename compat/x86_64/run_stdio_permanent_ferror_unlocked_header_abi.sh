#!/usr/bin/env bash
# Native Linux/x86-64 permanent-stream ferror_unlocked <stdio.h> ABI proof.
#
...
readonly C_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_ferror_unlocked_header_abi_probe.c"
readonly CXX_PROBE="$ROOT_DIR/compat/x86_64/stdio_permanent_ferror_unlocked_header_abi_probe.cpp"
readonly -a PROFILES=(
    c11-gnu
    c11-bsd
    cxx17-gnu
...
fail() {
    printf 'ERROR: x86 stdio permanent-stream ferror_unlocked header ABI: %s\n' "$*" >&2
    exit 1
}
    ...
    case "$1" in
        c11-gnu)
            printf '%s\0' -U_BSD_SOURCE -U_XOPEN_SOURCE -U_POSIX_C_SOURCE \
                ...
                -DCRABC_STDIO_PERMANENT_FERROR_UNLOCKED_C11_GNU
            ;;
        c11-bsd)
            printf '%s\0' -U_GNU_SOURCE -U_XOPEN_SOURCE -U_POSIX_C_SOURCE \
                -U_LARGEFILE64_SOURCE -U_DEFAULT_SOURCE -D_BSD_SOURCE \
                -DCRABC_STDIO_PERMANENT_FERROR_UNLOCKED_C11_BSD
            ;;
        cxx17-gnu)
            printf '%s\0' -U_BSD_SOURCE -U_XOPEN_SOURCE -U_POSIX_C_SOURCE \
                -U_LARGEFILE64_SOURCE -U_DEFAULT_SOURCE -D_GNU_SOURCE \
                -DCRABC_STDIO_PERMANENT_FERROR_UNLOCKED_CXX17_GNU
            ;;
        cxx17-bsd)
            printf '%s\0' -U_GNU_SOURCE -U_XOPEN_SOURCE -U_POSIX_C_SOURCE \
                -U_LARGEFILE64_SOURCE -U_DEFAULT_SOURCE -D_BSD_SOURCE \
                -DCRABC_STDIO_PERMANENT_FERROR_UNLOCKED_CXX17_BSD
            ;;
        c11-strict)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
                -U_POSIX_C_SOURCE -U_LARGEFILE64_SOURCE -U_DEFAULT_SOURCE \
                -DCRABC_STDIO_PERMANENT_FERROR_UNLOCKED_REQUIRE_HIDDEN
            ;;
        c11-posix-2008)
            printf '%s\0' -U_GNU_SOURCE -U_BSD_SOURCE -U_XOPEN_SOURCE \
...
assert_hidden() {
    ...
    set -e
    [ "$status" -ne 0 ] ||
        fail "$tree $profile unexpectedly declares ferror_unlocked"
    grep -Eq 'ferror_unlocked|undeclared|not declared' "$diagnostic" ||
        fail "$tree $profile did not diagnose hidden ferror_unlocked"
}
...
assert_cxx_c_linkage() {
    ...
    grep -Fxq ferror_unlocked <<<"$undefined" ||
        fail "$tree C++ probe does not retain C spelling ferror_unlocked"
    if grep -Eq '_Z.*ferror_unlocked' <<<"$undefined"; then
        fail "$tree C++ probe retained a mangled ferror_unlocked reference"
    fi
}
...
[ -f "$C_PROBE" ] || fail "missing C permanent-stream ferror_unlocked header probe"
[ -f "$CXX_PROBE" ] || fail "missing C++ permanent-stream ferror_unlocked header probe"
...
work_dir="$(mktemp -d /tmp/crabc-x86-64-stdio-permanent-ferror-unlocked-header.XXXXXX)"
...
printf 'x86 stdio permanent-stream ferror_unlocked header ABI: PASS\n'
