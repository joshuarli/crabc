#!/usr/bin/env bash
# Native Linux/x86-64 C/C++ tcgetpgrp declaration gate.
#
# Pinned musl 1.2.6 is the unconditional <unistd.h> declaration and C-linkage
# oracle. The project-header pass selects neither terminal behavior nor a
# static archive result; its direct archive proof is separate.
set -euo pipefail
...
fail() {
    printf 'ERROR: x86 unistd.h tcgetpgrp ABI: %s\n' "$*" >&2
    exit 1
}
...
c_probe="$ROOT_DIR/compat/x86_64/tcgetpgrp_header_abi_probe.c"
cxx_probe="$ROOT_DIR/compat/x86_64/tcgetpgrp_header_abi_probe.cpp"
work_dir="$(mktemp -d /tmp/crabc-x86-64-tcgetpgrp-header.XXXXXX)"
trap 'rm -rf -- "$work_dir"' EXIT
header_trace="$work_dir/header-trace"
oracle_cxx_object="$work_dir/oracle-tcgetpgrp-cxx.o"
candidate_cxx_object="$work_dir/candidate-tcgetpgrp-cxx.o"

# Musl declares tcgetpgrp independently of the ordinary strict/POSIX/XSI/GNU/
...
if ! "$ORACLE_CC" -std=c11 -fno-builtin -I "$ROOT_DIR/include" -H \
    ...
    fail "project C tcgetpgrp header contract drifted"
fi
...
for object in "$oracle_cxx_object" "$candidate_cxx_object"; do
    ...
    grep -Eq '[[:space:]]tcgetpgrp$' <<<"$undefined" ||
        fail "C++ probe does not retain C linkage for tcgetpgrp"
    if grep -Eq '_Z9tcgetpgrpi' <<<"$undefined"; then
        fail "C++ probe retained a mangled tcgetpgrp reference"
    fi
done

printf 'x86 pinned-musl/project C/C++ <unistd.h> tcgetpgrp ABI: PASS\n'
