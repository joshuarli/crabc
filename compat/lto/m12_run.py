#!/usr/bin/env python3
"""Collect bounded Milestone 12 crabc-rs LTO evidence.
...
whose manifest is supplied explicitly (and defaults to the M12 fixture
...
Use ``--manifest`` when a caller keeps the M12 fixture in a different
...
# The M12 fixture is deliberately selected through its manifest.  A caller
# may use --manifest for a companion fixture kept under another M12 directory.
DEFAULT_MANIFEST = LTO_ROOT / "m12-crabc-rs-fixture/Cargo.toml"
DEFAULT_STOCK_STD_MANIFEST = LTO_ROOT / "m12-std-fixture/Cargo.toml"
DEFAULT_REPORT = ROOT / "compat/reports/lto/m12/latest.json"
...
DEFAULT_ENTRY_SYMBOL = "crabc_rs_m12_getpid_witness"
DEFAULT_STDOUT = b"m12-crabc-rs:ok\n"
...
def fixture_metadata(manifest: Path) -> dict[str, object]:
    """Read the M12 package without assuming a source filename or bin name."""

    try:
        with manifest.open("rb") as stream:
            raw = tomllib.load(stream)
    except (OSError, tomllib.TOMLDecodeError) as error:
        raise RunnerError(f"invalid M12 fixture manifest: {manifest}") from error
    package = raw.get("package")
    if not isinstance(package, dict) or not isinstance(package.get("name"), str):
        raise RunnerError(f"M12 fixture manifest lacks package.name: {manifest}")
    package_name = package["name"]
    if not package_name:
        raise RunnerError(f"M12 fixture package.name is empty: {manifest}")
    targets = raw.get("bin")
    bin_names: list[str] = []
    ...
    m12_metadata = metadata.get("crabc-lto", {}) if isinstance(metadata, dict) else {}
    if not isinstance(m12_metadata, dict):
        m12_metadata = {}
    ...
    if not source_files:
        raise RunnerError(f"M12 fixture manifest has no Rust source files: {manifest.parent}")
    lockfile = manifest.with_name("Cargo.lock")
    entry_symbol = m12_metadata.get("entry-symbol", DEFAULT_ENTRY_SYMBOL)
    expected_stdout = m12_metadata.get("stdout", DEFAULT_STDOUT.decode())
    if not isinstance(entry_symbol, str) or not entry_symbol:
        raise RunnerError("M12 package.metadata.crabc-lto.entry-symbol must be a non-empty string")
    if not isinstance(expected_stdout, str):
        raise RunnerError("M12 package.metadata.crabc-lto.stdout must be a string")
    return {
        "manifest": manifest,
    ...
    environment.update(
        {
            ...
            "OLDPWD": "/tmp",
            "LC_ALL": "C",
            "CRABC_M12_LTO_FIXTURE": "aarch64-musl",
        }
    )
    return environment
...
def validate_manifest(manifest: Path) -> dict[str, object]:
    ...
    if not manifest.is_file():
        raise RunnerError(f"M12 fixture manifest unavailable: {manifest}")
...
def capability_reasons(
    ...
    if isinstance(lockfile, Path) and not lockfile.is_file():
        reasons.append(f"M12 fixture requires a checked-in Cargo.lock: {lockfile}")
...
def function_disassembly(disassembly: str, symbol: str) -> str:
    ...
    if match is None:
        raise RunnerError(f"M12 witness function is absent from disassembly: {symbol}")
...
def inspect_direct_route(
    *,
    ...
    if "AArch64" not in readelf_text:
        raise RunnerError("M12 fixture is not an AArch64 ELF")
    if not entry_symbol or entry_symbol not in nm_text:
        raise RunnerError(f"M12 entry symbol is absent from symbol evidence: {entry_symbol}")
    import re
    ...
    if not witness_getpid:
        raise RunnerError("M12 witness lacks direct getpid syscall 172 followed by svc")
    if "svc" not in witness:
        raise RunnerError("M12 witness contains no AArch64 svc instruction")
    ...
    if missing:
        raise RunnerError("M12 fixture is missing direct syscall path(s): " + ", ".join(missing))
    ...
    if branch_forbidden:
        raise RunnerError(
            "M12 fixture branches to forbidden public C/TLS symbol(s): "
...
def artifact_inspection(
    ...
    reject_glibc(file_text + nm_text + readelf_text + disassembly, "M12 ELF inspection")
    route = (
...
def build_fixture(
    ...
    *,
    ...
    reject_glibc(output_text, "M12 compiler output")
...
def patched_interpreter_bytes(binary: bytes, interpreter: str) -> bytes:
    ...
    if len(binary) < 64 or binary[:4] != b"\x7fELF" or binary[4] != 2 or binary[5] != 1:
        raise RunnerError("M12 stock-std output is not an ELF64 little-endian executable")
    if int.from_bytes(binary[18:20], "little") != 183:
        raise RunnerError("M12 stock-std output is not an AArch64 ELF")
    ...
    for index in range(phnum):
        ...
        if offset + 56 > len(result):
            raise RunnerError("M12 stock-std program headers exceed the file")
        ...
        if len(encoded) > file_size or file_offset + file_size > len(result):
            raise RunnerError("M12 stock-std interpreter path does not fit PT_INTERP")
        result[file_offset : file_offset + file_size] = encoded + b"\0" * (file_size - len(encoded))
        return bytes(result)
    raise RunnerError("M12 stock-std output has no PT_INTERP segment")
...
def stock_std_comparison(
    ...
    runtime = Path(tempfile.mkdtemp(prefix="m12-", dir="/tmp"))
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--manifest", type=Path, default=Path(os.environ.get("CRABC_M12_LTO_MANIFEST", DEFAULT_MANIFEST)))
    parser.add_argument(
        "--stock-std-manifest",
        type=Path,
        default=Path(os.environ.get("CRABC_M12_STOCK_STD_MANIFEST", DEFAULT_STOCK_STD_MANIFEST)),
    )
    parser.add_argument("--target-dir", type=Path, default=ROOT / "target/m12-lto")
...
def run(args: argparse.Namespace) -> tuple[str, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="crabc-m12-lto-") as temporary_name:
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        print(f"m12-lto: ERROR: {error}", file=sys.stderr)
        return 2
    print(f"m12-lto: {result.upper()}: report: {report}")
