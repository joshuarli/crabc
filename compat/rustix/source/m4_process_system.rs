...
fn main() {
    ...
    let name = format!("/crabc-rs-m4-source-{}", pid.as_raw_pid());
    ...
    assert!(mount::unmount("/crabc-rs-m4-source-not-mounted", mount::UnmountFlags::empty()).is_err());
    println!("m4-process-system ok");
}
