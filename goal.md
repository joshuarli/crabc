...
| C `pthread_mutex_cond_ping_pong` ×10,000 | 1.0145×–1.0161× CPU upper bounds across three 31-sample `pthread-cond-fetch-add-*-31` reports; 6.0009–6.0029 vs 6.0014–6.0026 marked calls/op | The direct-matched parent/worker protocol proves each handoff's two protected increments. The condition sequence and waiter counters now use the inline AArch64 acquire/release fetch-add exclusive loop instead of LLVM's outlined LSE capability dispatch; the futex boundary and 10-iteration pthread-stress result are unchanged. The CPU gate remains red. |
