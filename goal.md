...
is historical. Three current `pthread-initial-tp-cleanup-{matrix,repeat,repeat2}-31`
reports establish a still-red 0.9206×–0.9478× CPU upper-bound range. Creators
claim a free slot before detached-worker reclamation, reset it only after a
successful claim, and allocate stack/TLS once. The combined block keeps TLS
above the downward-growing stack. Its slot records the original bridge TP, so
unchanged worker TLS releases with the combined mapping after exit; a refreshed
migrated TP retains the exact dynamic-TLS query and replacement cleanup. Exit
skips the fixed TSD destructor scan when no destructor-bearing value exists,
while an exact non-null bitset clears only occupied slots before reuse.
