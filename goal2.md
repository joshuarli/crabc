# Goal 2 — validate crabc with a broad, measured software corpus

## Position in the roadmap

[`goal.md`](goal.md) comes first. It closes the known structural performance
...
performance claim in `goal.md`: many corpus commands are brief; several are
...
`goal.md` performance gates under realistic composition:
...
- each C-runtime workload passes the same per-workload CPU, peak-memory, and
  syscall rules defined in `goal.md`;
...
Goal 2 uses the sound measurement rules of `goal.md`, not ad hoc command
timing:
...
The `goal.md` gates apply per workload, not as an average across packages:
...
- Prefer structural and scalar fixes; SIMD remains the last measured step
  except for fully proved established math kernels, exactly as `goal.md` and
  `SCOPE.md` require.
- Keep package semantics and output checks green through every optimization.

**Exit:** every mandatory C workload passes the `goal.md` gates or has a
...
After `goal.md` is complete, Goal 2 is complete when the C and native corpus
