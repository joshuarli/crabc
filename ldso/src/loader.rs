...
core::arch::global_asm!(
    ...
    "__tlsdesc_dynamic:",
    // AArch64's TLSDESC resolver ABI is stricter than the ordinary procedure
    // call ABI: the resolver may change only x0, x1, x30, and flags. The Rust
    // helper below necessarily follows the ordinary ABI, so this veneer owns
    // the complete preservation frame around it. Keep the frame 16-byte
    // aligned and save every SIMD&FP register as well as x2..x29.
    "sub sp, sp, #752",
    "stp x2, x3, [sp, #0]",
    "stp x4, x5, [sp, #16]",
    "stp x6, x7, [sp, #32]",
