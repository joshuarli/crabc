...
artifact is a twenty-third private static artifact in that same still-planned
...
uses caller-owned raw storage without a lifecycle call; no selected barrier
initializer consumes the record. It therefore does not select barrier
...
The separate private `static-c-math-cosh` artifact records only binary64
`cosh` and binary32 `coshf`: `./scripts/dev-x86_64.sh libc-math-cosh` runs
project-header C and default-SSE/`-mfpmath=387` C++ function-pointer fixtures
through pinned musl and one freestanding static candidate. Its checked GCC
15.2.0 translation of musl 1.2.6 `cosh.c`/`coshf.c` keeps the exact
...
quiet/signaling NaNs, all four requested-and-observed rounding directions,
and exception flags. Strong target-owned definitions and final ELF checks
reject weak compiler-builtins fallback, public closure helpers, `coshl`, other
...
The separate private `static-c-math-asinh` artifact records only binary64
`asinh` and binary32 `asinhf`: `./scripts/dev-x86_64.sh libc-math-asinh` runs
project-header C and default-SSE/`-mfpmath=387` C++ function-pointer fixtures
through pinned musl and one freestanding static candidate. Its checked GCC
15.2.0 translation of musl 1.2.6 `asinh.c`/`asinhf.c` keeps the exact
fifteen-source log/log1p/square-root closure private: local log and log1p
providers, log/logf and reciprocal-square-root tables, and IEEE
invalid/divide-by-zero helpers. The 256-record differential covers signed
zero, tiny/normal/subnormal and reconstruction thresholds, high finite values,
infinities, quiet/signaling NaNs, all four requested-and-observed rounding
directions, and exception flags. Strong target-owned definitions and final ELF
checks reject weak compiler-builtins fallback, public closure helpers,
`asinhl`, other hyperbolic surface, public log/log1p/square-root ABI, fenv
API/policy, special/complex/binary80 math, family completion, promotion, and
public x86 support.

The separate private `static-c-math-exp10f` artifact records only GNU binary32
strong `exp10f` and musl's weak same-address `pow10f` alias:
`./scripts/dev-x86_64.sh libc-math-exp10f` runs project-header GNU C and
default-SSE/`-mfpmath=387` C++ function-pointer fixtures through pinned musl
and one freestanding static candidate. Its checked GCC 15.2.0 translation of
musl 1.2.6 `exp10f.c` keeps the exact twelve-source modff/exp2 closure private:
the binary32 fractional and binary64 fallback providers, exp tables, and six
overflow/underflow error helpers. The 256-record differential calls both
aliases over signed zero, tiny/normal/subnormal and table/fallback thresholds,
...
compiler-builtins fallback, public closure helpers, binary64/long-double
`exp10`/`pow10`, public modf/exp2 ABI, fenv API/policy,
special/complex/binary80 math, family completion, promotion, and public x86
support.
