//! Present personality and LSDA targets must remain in their declared loads.
use std::ffi::c_void;
...
// target that ordinary machine memory could access but unwind metadata cannot.
#[unsafe(no_mangle)]
...
unsafe fn write_personality_eh_frame(eh_frame: *mut u8, text: usize, target: usize, encoding: u8) {
    unsafe {
        // CIE: zP with an absolute direct or indirect 64-bit personality pointer.
        ...
        eh_frame.add(16).write(encoding);
        (eh_frame.add(17) as *mut u64).write_unaligned(target as u64);
...
unsafe fn write_direct_personality_eh_frame(eh_frame: *mut u8, text: usize, target: usize) {
    unsafe { write_personality_eh_frame(eh_frame, text, target, 0) }
}

unsafe fn write_indirect_personality_eh_frame(eh_frame: *mut u8, text: usize, target: usize) {
    unsafe { write_personality_eh_frame(eh_frame, text, target, 0x80) }
}

unsafe fn write_direct_lsda_eh_frame(eh_frame: *mut u8, text: usize, target: usize) {
    unsafe {
        // CIE: zL with an absolute direct 64-bit FDE LSDA pointer.
        ...
        (fde.add(16) as *mut u64).write_unaligned(TEXT_SPAN as u64);
        fde.add(24).write(8);
        (fde.add(25) as *mut u64).write_unaligned(target as u64);
...
unsafe fn run_case(
    write_eh_frame: unsafe fn(*mut u8, usize, usize),
    target_kind: Target,
    expected: i32,
) {
    unsafe {
        ...
        let target = match target_kind {
            Target::Zero => 0,
            Target::IndirectZero => {
                (allocation.add(CELL) as *mut usize).write_unaligned(0);
                allocation.add(CELL) as usize
            }
            Target::Guarded => allocation.add(PAGE) as usize,
        };
        let text = (_Unwind_RaiseException as usize) & !(PAGE - 1);
        write_header(allocation.add(HEADER), allocation.add(EH_FRAME) as usize);
        write_eh_frame(allocation.add(EH_FRAME), text, target);
        ...
        assert_eq!(_Unwind_RaiseException(&mut exception), expected);
...
fn main() {
    unsafe {
        // Absence of zP and a direct zero LSDA retain their normal result.
        run_case(write_direct_lsda_eh_frame, Target::Zero, END_OF_STACK);
        // Present null personalities have no valid Rust function-pointer value.
        run_case(write_direct_personality_eh_frame, Target::Zero, FATAL_PHASE1_ERROR);
        run_case(write_indirect_personality_eh_frame, Target::IndirectZero, FATAL_PHASE1_ERROR);
        // Nonzero present targets must still belong to their required loads.
        run_case(write_direct_personality_eh_frame, Target::Guarded, FATAL_PHASE1_ERROR);
        run_case(write_direct_lsda_eh_frame, Target::Guarded, FATAL_PHASE1_ERROR);
    }
    println!("direct metadata targets rejected");
}
