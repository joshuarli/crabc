...
const M4_TCGETS: u32 = 0x5401;
const M4_TCSETS: u32 = 0x5402;
const M4_TCSETSW: u32 = 0x5403;
const M4_TCSETSF: u32 = 0x5404;
const M4_TCSBRK: u32 = 0x5409;
const M4_TIOCGWINSZ: u32 = 0x5413;
const M4_TIOCSWINSZ: u32 = 0x5414;
const M4_TCXONC: u32 = 0x540a;
const M4_TCFLSH: u32 = 0x540b;
const M4_TIOCGPGRP: u32 = 0x540f;
const M4_TIOCSPGRP: u32 = 0x5410;
const M4_TIOCGSID: u32 = 0x5429;
const M4_TIOCSCTTY: u32 = 0x540e;
const M4_TIOCSPTLCK: u32 = 0x4004_5431;
const M4_TIOCGPTN: u32 = 0x8004_5430;

const M4_PTY_O_RDWR: i32 = 2;
const M4_PTY_O_NOCTTY: i32 = 0x100;
const M4_PTY_O_CLOEXEC: i32 = 0x80000;

const M4_CBAUD: c_uint = 0o10017;
const M4_CIBAUD: c_uint = 0o2003600000;
const M4_TERM_F_GETFD: c_int = 1;

unsafe fn m4_ioctl(fd: c_int, request: u32, argument: *mut u8) -> c_int {
    syscall_result(sys_ioctl(fd, request, argument)) as c_int
}

unsafe fn m4_ioctl_value(fd: c_int, request: u32, value: c_int) -> c_int {
    m4_ioctl(fd, request, value as isize as *mut u8)
}

#[no_mangle]
pub unsafe extern "C" fn tcgetattr(fd: c_int, tio: *mut m4_termios) -> c_int {
    m4_ioctl(fd, M4_TCGETS, tio as *mut u8)
}
...
pub unsafe extern "C" fn tcsetattr(
    ...
    m4_ioctl(
        fd,
        M4_TCSETS + action as u32,
        tio as *const m4_termios as *mut u8,
    )
}

#[no_mangle]
pub unsafe extern "C" fn tcflush(fd: c_int, queue: c_int) -> c_int {
    m4_ioctl_value(fd, M4_TCFLSH, queue)
}

#[no_mangle]
pub unsafe extern "C" fn tcflow(fd: c_int, action: c_int) -> c_int {
    m4_ioctl_value(fd, M4_TCXONC, action)
}
...
pub unsafe extern "C" fn tcdrain(fd: c_int) -> c_int {
    ...
    m4_ioctl_value(fd, M4_TCSBRK, 1)
}

#[repr(C)]
pub struct m4_winsize {
    ws_row: u16,
    ws_col: u16,
    ws_xpixel: u16,
    ws_ypixel: u16,
...
#[no_mangle]
pub unsafe extern "C" fn tcgetwinsize(fd: c_int, wsz: *mut m4_winsize) -> c_int {
    m4_ioctl(fd, M4_TIOCGWINSZ, wsz as *mut u8)
}

#[no_mangle]
pub unsafe extern "C" fn tcsetwinsize(fd: c_int, wsz: *const m4_winsize) -> c_int {
    m4_ioctl(fd, M4_TIOCSWINSZ, wsz as *mut m4_winsize as *mut u8)
}
...
pub unsafe extern "C" fn tcsendbreak(fd: c_int, _duration: c_int) -> c_int {
    ...
    // durations in musl.
    m4_ioctl_value(fd, M4_TCSBRK, 0)
}

#[no_mangle]
pub unsafe extern "C" fn tcgetpgrp(fd: c_int) -> c_int {
    let mut pgrp: c_int = 0;
    if m4_ioctl(fd, M4_TIOCGPGRP, &mut pgrp as *mut c_int as *mut u8) < 0 {
        return -1;
    }
    pgrp
}
...
pub unsafe extern "C" fn tcsetpgrp(fd: c_int, pgrp: c_int) -> c_int {
    let mut pgrp = pgrp;
    m4_ioctl(fd, M4_TIOCSPGRP, &mut pgrp as *mut c_int as *mut u8)
}

#[no_mangle]
pub unsafe extern "C" fn tcgetsid(fd: c_int) -> c_int {
    let mut sid: c_int = 0;
    if m4_ioctl(fd, M4_TIOCGSID, &mut sid as *mut c_int as *mut u8) < 0 {
        return -1;
    }
    sid
}
...
#[no_mangle]
pub unsafe extern "C" fn cfgetospeed(tio: *const m4_termios) -> c_uint {
    (*tio).c_cflag & M4_CBAUD
}

#[no_mangle]
pub unsafe extern "C" fn cfgetispeed(tio: *const m4_termios) -> c_uint {
    ((*tio).c_cflag & M4_CIBAUD) / (M4_CIBAUD / M4_CBAUD)
}

#[no_mangle]
pub unsafe extern "C" fn cfsetospeed(tio: *mut m4_termios, speed: c_uint) -> c_int {
    if speed & !M4_CBAUD != 0 {
        ERRNO = EINVAL;
        return -1;
    }
    (*tio).c_cflag = ((*tio).c_cflag & !M4_CBAUD) | speed;
    0
}

#[no_mangle]
pub unsafe extern "C" fn cfsetispeed(tio: *mut m4_termios, speed: c_uint) -> c_int {
    if speed & !M4_CBAUD != 0 {
        ERRNO = EINVAL;
        return -1;
    }
    (*tio).c_cflag = ((*tio).c_cflag & !M4_CIBAUD)
        | speed * (M4_CIBAUD / M4_CBAUD);
    0
}

#[no_mangle]
pub unsafe extern "C" fn cfsetspeed(tio: *mut m4_termios, speed: c_uint) -> c_int {
...
pub unsafe extern "C" fn isastream(fd: c_int) -> c_int {
    ...
    let result = syscall_result(sys_fcntl(fd, M4_TERM_F_GETFD, 0));
...
unsafe fn m4_pty_number(fd: c_int, number: *mut c_int) -> c_int {
    let r = sys_ioctl(fd, M4_TIOCGPTN, number as *mut u8);
    if r < 0 { (-r) as c_int } else { 0 }
}

unsafe fn m4_pty_name_from_number(number: c_int, buf: *mut c_char, len: usize) -> c_int {
...
pub unsafe extern "C" fn grantpt(fd: c_int) -> c_int {
    ...
    let mut number = 0;
    let error = m4_pty_number(fd, &mut number);
    if error != 0 {
        ERRNO = error;
        return -1;
    }
...
pub unsafe extern "C" fn unlockpt(fd: c_int) -> c_int {
    ...
    let r = sys_ioctl(fd, M4_TIOCSPTLCK, &mut unlock as *mut c_int as *mut u8);
    if r < 0 {
        ERRNO = (-r) as c_int;
        return -1;
    }
...
pub unsafe extern "C" fn ptsname_r(fd: c_int, buf: *mut c_char, len: usize) -> c_int {
    ...
    m4_pty_name_from_number(number, buf, len)
}
...
unsafe fn m4_openpty_error(master: c_int, slave: c_int, error: c_int) -> c_int {
...
pub unsafe extern "C" fn openpty(
    ...
    let m = posix_openpt(M4_PTY_O_RDWR | M4_PTY_O_NOCTTY);
    if m < 0 { return -1; }
    if grantpt(m) < 0 {
        return m4_openpty_error(m, -1, ERRNO);
    }
    if unlockpt(m) < 0 {
        return m4_openpty_error(m, -1, ERRNO);
    }
    ...
    if name_error != 0 {
        return m4_openpty_error(m, -1, name_error);
    }
    let s = {
        let r = sys_open(slave_name.as_ptr(), (M4_PTY_O_RDWR | M4_PTY_O_NOCTTY) as i64, 0);
        if r < 0 {
            return m4_openpty_error(m, -1, (-r) as c_int);
        }
        r as c_int
    };
    ...
    if !tio.is_null() && tcsetattr(s, 0, tio) < 0 {
        return m4_openpty_error(m, s, ERRNO);
    }
...
pub unsafe extern "C" fn login_tty(fd: c_int) -> c_int {
    ...
    let r = sys_ioctl(fd, M4_TIOCSCTTY, core::ptr::null_mut());
    if r < 0 {
        ERRNO = (-r) as c_int;
        return -1;
    }
...
pub unsafe extern "C" fn forkpty(
    ...
    let pipe_result = sys_pipe2(status_pipe.as_mut_ptr(), M4_PTY_O_CLOEXEC);
    if pipe_result < 0 {
        return m4_openpty_error(m, s, (-pipe_result) as c_int);
    }
    ...
    if received != 0 {
        let mut ignored = 0;
        let _ = waitpid(pid, &mut ignored, 0);
        return m4_openpty_error(m, -1, if received == core::mem::size_of::<c_int>() { error } else { EIO_VAL });
    }
    *master = m;
    pid
}
...
#[no_mangle]
pub unsafe extern "C" fn cfmakeraw(tio: *mut m4_termios) {
