// M4 system and legacy utility exports.
//
...
pub unsafe extern "C" fn l64a(value: c_long) -> *mut c_char {
    ...
    while i < 6 && remaining != 0 {
        M64A_BUFFER[i] = M4_L64A_DIGITS[(remaining & 63) as usize];
        remaining >>= 6;
        i += 1;
    }
    M64A_BUFFER[i] = 0;
...
const M4_SYS_GETPRIORITY: i64 = 140;
#[cfg(target_arch = "x86_64")]
const M4_SYS_SETPRIORITY: i64 = 141;

#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_GETPRIORITY: i64 = 141;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_SETPRIORITY: i64 = 140;

const M4_PRIO_PROCESS: c_int = 0;
const M4_NZERO: c_int = 20;
const M4_EPERM: c_int = 1;
const M4_EACCES: c_int = 13;

#[inline]
unsafe fn m4_priority_errno(result: i64) -> c_int {
    if result < 0 {
...
pub unsafe extern "C" fn getpriority(which: c_int, who: c_uint) -> c_int {
    let result = <Arch as Syscalls>::syscall2(M4_SYS_GETPRIORITY, which as i64, who as i64);
    if result < 0 {
        ERRNO = (-result) as c_int;
        return -1;
    }
...
pub unsafe extern "C" fn nice(inc: c_int) -> c_int {
    ...
    if inc > -2 * M4_NZERO && inc < 2 * M4_NZERO {
        priority += getpriority(M4_PRIO_PROCESS, 0);
    }
    if priority > M4_NZERO - 1 {
        priority = M4_NZERO - 1;
    }
    if priority < -M4_NZERO {
        priority = -M4_NZERO;
    }

    if setpriority(M4_PRIO_PROCESS, 0, priority) != 0 {
        if ERRNO == M4_EACCES {
            ERRNO = M4_EPERM;
        }
        -1
