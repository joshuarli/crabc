// M4 program and utility interfaces.
//
...
const M4_PATH_MAX: usize = 4096;
const M4_SYMLOOP_MAX: usize = 40;
const M4_INT_MAX: u64 = 0x7fff_ffff;

const M4_EEXIST: c_int = 17;
const M4_EINVAL: c_int = 22;
const M4_PROGRAM_ENOENT: c_int = 2;
const M4_ENAMETOOLONG: c_int = 36;
const M4_ELOOP: c_int = 40;

const M4_AT_SECURE: c_ulong = 23;

const M4_O_NOCTTY: i32 = 0x100;
const M4_O_CLOEXEC: i32 = 0x80000;
const M4_O_RDWR: i32 = 2;
const M4_O_CREAT: i32 = 64;
const M4_O_EXCL: i32 = 128;
const M4_O_ACCMODE: i32 = 3;
const M4_F_GETFD: i32 = 1;
...
pub unsafe extern "C" fn get_current_dir_name() -> *mut c_char {
    ...
    loop {
        ...
        if error != ENAMETOOLONG_VAL || size >= M4_PATH_MAX {
            ERRNO = error;
            return core::ptr::null_mut();
        }
        size = match size.checked_mul(2) {
            Some(next) if next <= M4_PATH_MAX => next,
            _ => M4_PATH_MAX,
        };
    }
}
...
pub unsafe extern "C" fn realpath(
    ...
    let mut stack = [0u8; M4_PATH_MAX + 1];
    let mut output = [0u8; M4_PATH_MAX];
    let mut p: usize;
    let mut q = 0usize;
    let mut l: usize;
    let mut l0: usize;
    ...
    if l == 0 {
        ERRNO = M4_PROGRAM_ENOENT;
        return core::ptr::null_mut();
    }
    if l >= M4_PATH_MAX {
        ERRNO = M4_ENAMETOOLONG;
        return core::ptr::null_mut();
    }
    p = stack.len() - l - 1;
...
static mut M4_TEMP_COUNTER: usize = 0;

unsafe fn m4_temp_name(template: *mut c_char, suffix_len: usize, flags: c_int) -> c_int {
    if template.is_null() {
        ERRNO = M4_EINVAL;
        return -1;
    }
    ...
    let mut seed = (if clock >= 0 { ts.tv_nsec as usize } else { 0 })
        ^ (sys_getpid() as usize)
        ^ M4_TEMP_COUNTER;
    M4_TEMP_COUNTER = M4_TEMP_COUNTER.wrapping_add(1);
    let alphabet = b"0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-_";

    let open_flags = (flags & !M4_O_ACCMODE) | M4_O_RDWR | M4_O_CREAT | M4_O_EXCL;
    for _ in 0..100 {
...
pub unsafe extern "C" fn mkostemps(
    ...
    m4_temp_name(template, suffix_len as usize, flags)
}
...
pub unsafe extern "C" fn mkstemps(template: *mut c_char, suffix_len: c_int) -> c_int {
    ...
    m4_temp_name(template, suffix_len as usize, 0)
}
...
pub unsafe extern "C" fn tempnam(dir: *const c_char, pfx: *const c_char) -> *mut c_char {
    ...
    let directory_len = strnlen(directory as *const u8, M4_PATH_MAX);
    if directory_len == M4_PATH_MAX {
        ERRNO = M4_ENAMETOOLONG;
        return core::ptr::null_mut();
    }
    ...
    {
        Some(value) if value <= M4_PATH_MAX => value,
        _ => {
            ERRNO = M4_ENAMETOOLONG;
            return core::ptr::null_mut();
        }
    ...
    *result.add(offset + 6) = 0;
    ...
    let fd = m4_temp_name(result, 0, M4_O_CLOEXEC);
    if fd < 0 {
...
// matches the public termios header already used by m4_terminal_exports.rs.
#[repr(C)]
#[derive(Clone, Copy)]
struct M4PassTermios {
    c_iflag: c_uint,
    c_oflag: c_uint,
    c_cflag: c_uint,
    c_lflag: c_uint,
...
const M4_PROGRAM_TCGETS: u32 = 0x5401;
const M4_PROGRAM_TCSETSF: u32 = 0x5404;
const M4_TCSAFLUSH: c_int = 2;
const M4_ECHO: c_uint = 0o10;
const M4_ISIG: c_uint = 0o1;
const M4_ICRNL: c_uint = 0o400;
const M4_INLCR: c_uint = 0o100;
const M4_IGNCR: c_uint = 0o200;
const M4_ICANON: c_uint = 0o2;

static mut M4_PASSWORD: [u8; 128] = [0; 128];
const M4_PASSWORD_CAPACITY: usize = 128;

#[no_mangle]
pub unsafe extern "C" fn getpass(prompt: *const c_char) -> *mut c_char {
    let fd = sys_open(
        b"/dev/tty\0".as_ptr(),
        (M4_O_RDWR | M4_O_NOCTTY | M4_O_CLOEXEC) as i64,
        0,
    );
    if fd < 0 {
        ERRNO = (-fd) as c_int;
    ...
    let mut saved: M4PassTermios = core::mem::zeroed();
    let mut modified: M4PassTermios;
    let got = sys_ioctl(fd, M4_PROGRAM_TCGETS, &mut saved as *mut M4PassTermios as *mut u8);
    if got < 0 {
    ...
    modified = saved;
    modified.c_lflag &= !(M4_ECHO | M4_ISIG);
    modified.c_lflag |= M4_ICANON;
    modified.c_iflag &= !(M4_INLCR | M4_IGNCR);
    modified.c_iflag |= M4_ICRNL;
    let changed = sys_ioctl(fd, M4_PROGRAM_TCSETSF, &mut modified as *mut M4PassTermios as *mut u8);
    if changed < 0 {
    ...
    let read_len = sys_read(
        fd as i64,
        core::ptr::addr_of_mut!(M4_PASSWORD).cast::<u8>(),
        M4_PASSWORD_CAPACITY,
    );
    let restore = sys_ioctl(fd, M4_PROGRAM_TCSETSF, &mut saved as *mut M4PassTermios as *mut u8);
    if restore < 0 {
    ...
    if (length > 0 && M4_PASSWORD[length - 1] == b'\n') || length == M4_PASSWORD_CAPACITY {
        length -= 1;
    }
    M4_PASSWORD[length] = 0;
    core::ptr::addr_of_mut!(M4_PASSWORD).cast::<c_char>()
}
...
const M4_EXEC_PATH_MAX: usize = 4096;
const M4_EXEC_NAME_MAX: usize = 255;
const M4_EXEC_ARGV_MAX: usize = 256;
const M4_EXEC_ENOENT: c_int = 2;
const M4_EXEC_E2BIG: c_int = 7;
const M4_EXEC_ENOEXEC: c_int = 8;
const M4_EXEC_EBADF: c_int = 9;
const M4_EXEC_EACCES: c_int = 13;
const M4_EXEC_EFAULT: c_int = 14;
const M4_EXEC_ENOTDIR: c_int = 20;
const M4_EXEC_ENAMETOOLONG: c_int = 36;
const M4_EXEC_ENOSYS: c_int = 38;
const M4_EXEC_EINVAL: c_int = 22;
const M4_EXEC_AT_EMPTY_PATH: i64 = 0x1000;

#[cfg(target_arch = "x86_64")]
const M4_EXEC_SYS_EXECVEAT: i64 = 322;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_EXEC_SYS_EXECVEAT: i64 = 281;

#[inline]
unsafe fn m4_exec_attempt(
    path: *const c_char,
...
unsafe fn m4_exec_shell(
    ...
    let mut shell_argv: [*const c_char; M4_EXEC_ARGV_MAX + 4] =
        [core::ptr::null(); M4_EXEC_ARGV_MAX + 4];
    ...
    if !argv.is_null() {
        ...
        if source == M4_EXEC_ARGV_MAX + 1 && !(*argv.add(source)).is_null() {
            ERRNO = M4_EXEC_E2BIG;
            return -1;
        }
    }
    ...
    m4_exec_attempt(shell.as_ptr() as *const c_char, shell_argv.as_ptr(), envp);
    -1
}

unsafe fn m4_execvpe_impl(
    file: *const c_char,
    argv: *const *const c_char,
    envp: *const *const c_char,
) -> c_int {
    if file.is_null() {
        ERRNO = M4_EXEC_EFAULT;
        return -1;
    }

    if *file as u8 == 0 {
        ERRNO = M4_EXEC_ENOENT;
        return -1;
    }
    ...
    if has_slash {
        let error = m4_exec_attempt(file, argv, envp);
        if error == M4_EXEC_ENOEXEC {
            return m4_exec_shell(file, argv, envp);
        }
        return -1;
    }

    let file_len = strnlen(file as *const u8, M4_EXEC_NAME_MAX + 1);
    if file_len > M4_EXEC_NAME_MAX {
        ERRNO = M4_EXEC_ENAMETOOLONG;
        return -1;
    }
    ...
    let path_len = strnlen(path, M4_EXEC_PATH_MAX - 1);
    let mut candidate = [0u8; M4_EXEC_PATH_MAX];
    ...
    loop {
        ...
        if candidate_len < M4_EXEC_PATH_MAX {
            if directory_len == 0 {
            ...
            let error = m4_exec_attempt(
                candidate.as_ptr() as *const c_char,
                argv,
                envp,
            );
            match error {
                M4_EXEC_EACCES => seen_eacces = true,
                M4_EXEC_ENOENT | M4_EXEC_ENOTDIR => {}
                M4_EXEC_ENOEXEC => {
                    return m4_exec_shell(candidate.as_ptr() as *const c_char, argv, envp);
                }
                _ => return -1,
            }
        }
...
pub unsafe extern "C" fn execv(
    ...
    m4_exec_attempt(path, argv, __environ as *const *const c_char);
    -1
}
...
pub unsafe extern "C" fn execvp(
    ...
    m4_execvpe_impl(file, argv, __environ as *const *const c_char)
}
...
pub unsafe extern "C" fn execvpe(
    ...
    m4_execvpe_impl(file, argv, envp)
}
...
pub unsafe extern "C" fn execlp(
    ...
    let mut argv: [*const c_char; M4_EXEC_ARGV_MAX + 1] =
        [core::ptr::null(); M4_EXEC_ARGV_MAX + 1];
    argv[0] = arg;
    ...
    m4_execvpe_impl(file, argv.as_ptr(), __environ as *const *const c_char)
}

unsafe fn m4_execveat(
    fd: c_int,
    argv: *const *const c_char,
    envp: *const *const c_char,
) -> i64 {
    <Arch as Syscalls>::syscall5(
        M4_EXEC_SYS_EXECVEAT,
        fd as i64,
        b"\0".as_ptr() as i64,
        argv as i64,
        envp as i64,
        M4_EXEC_AT_EMPTY_PATH,
    )
}

unsafe fn m4_exec_procfd(fd: c_int, out: &mut [u8; 32]) -> *const c_char {
...
pub unsafe extern "C" fn fexecve(
    ...
    let result = m4_execveat(fd, argv, envp);
    if result != -M4_EXEC_ENOSYS as i64 {
        if result < 0 {
            ERRNO = (-result) as c_int;
        } else {
            ERRNO = M4_EXEC_EINVAL;
        }
        return -1;
    }

    let mut procfd = [0u8; 32];
    let path = m4_exec_procfd(fd, &mut procfd);
    let error = m4_exec_attempt(path, argv, envp);
    if error == M4_EXEC_ENOENT {
        ERRNO = M4_EXEC_EBADF;
    }
    -1
}
