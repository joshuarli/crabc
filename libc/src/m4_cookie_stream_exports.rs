...
type M4CookieReadFn = Option<unsafe extern "C" fn(*mut c_void, *mut c_char, usize) -> isize>;
type M4CookieWriteFn = Option<unsafe extern "C" fn(*mut c_void, *const c_char, usize) -> isize>;
type M4CookieSeekFn = Option<unsafe extern "C" fn(*mut c_void, *mut i64, c_int) -> c_int>;
type M4CookieCloseFn = Option<unsafe extern "C" fn(*mut c_void) -> c_int>;

#[repr(C)]
pub struct M4CookieIoFunctions {
    read: M4CookieReadFn,
    write: M4CookieWriteFn,
    seek: M4CookieSeekFn,
    close: M4CookieCloseFn,
}

#[repr(C)]
struct M4CookieStream {
    user_cookie: *mut c_void,
    io: M4CookieIoFunctions,
}

unsafe fn m4_cookie_fail(file: *mut FILE, is_error: bool) -> usize {
    if is_error {
        (*file).flags |= F_ERR;
        (*file)._err = 1;
    } else {
...
unsafe extern "C" fn m4_cookie_read(file: *mut FILE, output: *mut u8, length: usize) -> usize {
    let stream = (*file).cookie as *mut M4CookieStream;
    let read = match (*stream).io.read {
        Some(callback) => callback,
        None => return m4_cookie_fail(file, true),
    };
    ...
    if first <= 0 {
        return m4_cookie_fail(file, first < 0);
    }
    ...
    if first > direct_length {
        ...
        ERRNO = EIO_VAL;
        return m4_cookie_fail(file, true);
    }
...
unsafe extern "C" fn m4_cookie_write(file: *mut FILE, input: *const u8, length: usize) -> usize {
    let stream = (*file).cookie as *mut M4CookieStream;
    ...
    if buffered != 0 {
        ...
        if m4_cookie_write(file, (*file).wbase, buffered) < buffered {
            return 0;
        }
    }
...
unsafe extern "C" fn m4_cookie_seek(file: *mut FILE, offset: i64, whence: c_int) -> i64 {
...
unsafe extern "C" fn m4_cookie_close(file: *mut FILE) -> c_int {
    let stream = (*file).cookie as *mut M4CookieStream;
...
pub unsafe extern "C" fn fopencookie(
    ...
    let allocation = match file_size.checked_add(core::mem::size_of::<M4CookieStream>()) {
        Some(size) => size,
        None => {
            ERRNO = ENOMEM;
            return core::ptr::null_mut();
    ...
    let stream = (file as *mut u8).add(file_size) as *mut M4CookieStream;
    core::ptr::write(stream, M4CookieStream { user_cookie, io });

    init_file(
        file,
        -1,
        mode,
        Some(m4_cookie_close),
        buf_ptr(file),
        BUFSIZ,
    );
    (*file).cookie = stream as *mut c_void;
    (*file).read_fn = Some(m4_cookie_read);
    (*file).write_fn = Some(m4_cookie_write);
    (*file).seek_fn = Some(m4_cookie_seek);
    (*file).lbf = -1;
    file
}
