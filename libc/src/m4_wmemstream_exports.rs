// M4 wide-memory streams.  FILE internally transports encoded bytes, so this
...
unsafe fn m4_wmemstream_append(cookie: *mut M4WmemstreamCookie, value: wchar_t) -> bool {
...
unsafe extern "C" fn m4_wmemstream_write(
    file: *mut FILE,
    bytes: *const u8,
    length: usize,
) -> usize {
    let cookie = (*file).cookie as *mut M4WmemstreamCookie;
    if cookie.is_null() {
        ERRNO = EINVAL;
        (*file).flags |= F_ERR;
        (*file)._err = 1;
    ...
    if buffered != 0 {
        ...
        if m4_wmemstream_write(file, (*file).wbase, buffered) != buffered {
            return 0;
        }
    }
    if length == 0 {
    ...
    while offset < length {
        ...
        if converted == !0usize || !m4_wmemstream_append(cookie, wide) {
            (*file).flags |= F_ERR;
            (*file)._err = 1;
            (*file).wpos = core::ptr::null_mut();
            return 0;
...
unsafe extern "C" fn m4_wmemstream_close(file: *mut FILE) -> c_int {
    let cookie = (*file).cookie as *mut M4WmemstreamCookie;
    if cookie.is_null() {
        return 0;
    }
...
pub unsafe extern "C" fn open_wmemstream(
    ...
    let cookie = calloc(1, core::mem::size_of::<M4WmemstreamCookie>()) as *mut M4WmemstreamCookie;
    if cookie.is_null() {
    ...
    (*file).read_fn = None;
    (*file).seek_fn = None;
    (*file).write_fn = Some(m4_wmemstream_write);
