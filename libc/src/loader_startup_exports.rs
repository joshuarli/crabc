...
const CABI_DLS_STARTUP_MAX_ARGC: usize = 4096;
const CABI_DLS_STARTUP_MAX_ENVC: usize = 4096;
const CABI_DLS_STARTUP_MAX_AUX_ENTRIES: usize = 128;
...
unsafe fn cabi_dls_install_startup_state(sp: *mut usize, auxv: *mut usize) -> bool {
    if sp.is_null() || auxv.is_null() {
        return false;
    }

    let argc = *sp;
    if argc > CABI_DLS_STARTUP_MAX_ARGC {
        return false;
    }
    ...
    let mut envc = 0usize;
    while envc < CABI_DLS_STARTUP_MAX_ENVC && !(*envp.add(envc)).is_null() {
        envc += 1;
    }
    if envc == CABI_DLS_STARTUP_MAX_ENVC {
        return false;
    }

    let mut aux = auxv;
    let mut terminated = false;
    for _ in 0..CABI_DLS_STARTUP_MAX_AUX_ENTRIES {
        if *aux == 0 {
            terminated = true;
            break;
        }
    ...
    if argc != 0 {
        let argv0 = *argv;
        if !argv0.is_null() {
            cabi_set_program_names(argv0 as *const c_char);
        }
    }
    true
}

/// The libc-visible `__dls3` startup entry point for crabc's split loader.
///
...
pub unsafe extern "C" fn __dls3(sp: *mut usize, auxv: *mut usize) {
    let _ = cabi_dls_install_startup_state(sp, auxv);
}

/// `__dls2b` dispatches to `__dls3` after the early relocation/TLS barrier.
///
