...
const M4_SYS_ACCEPT4: i64 = 288;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_ACCEPT4: i64 = 242;

#[cfg(target_arch = "x86_64")]
const M4_SYS_GETPEERNAME: i64 = 52;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_GETPEERNAME: i64 = 205;

#[cfg(target_arch = "x86_64")]
const M4_SYS_GETSOCKOPT: i64 = 55;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_GETSOCKOPT: i64 = 209;

#[cfg(target_arch = "x86_64")]
const M4_SYS_SENDMSG: i64 = 46;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_SENDMSG: i64 = 211;

#[cfg(target_arch = "x86_64")]
const M4_SYS_RECVMSG: i64 = 47;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_RECVMSG: i64 = 212;

#[cfg(target_arch = "x86_64")]
const M4_SYS_RECVMMSG: i64 = 299;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_RECVMMSG: i64 = 243;

#[cfg(target_arch = "x86_64")]
const M4_SYS_IOCTL: i64 = 16;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_IOCTL: i64 = 29;

// Linux's IOV_MAX, and the bound used by musl's 64-bit sendmmsg path.
const M4_IOV_MAX: c_uint = 1024;
const M4_CMSG_ALIGN: usize = 8;
const M4_CMSG_HDR_SIZE: usize = 16;
// CMSG_SPACE(255 * sizeof(int)) + one header, rounded to a usize word.
const M4_SEND_CONTROL_WORDS: usize = 132;
...
#[inline]
fn m4_cmsg_align(length: usize) -> usize {
    length.wrapping_add(M4_CMSG_ALIGN - 1) & !(M4_CMSG_ALIGN - 1)
}
...
unsafe fn m4_zero_cmsg_padding(control: *mut u8, length: usize) {
    let mut offset = 0usize;
    while offset + M4_CMSG_HDR_SIZE <= length {
        ...
        if cmsg_len < M4_CMSG_HDR_SIZE {
            break;
        }
        let next = m4_cmsg_align(cmsg_len);
...
unsafe fn m4_sendmsg_raw(fd: c_int, msg: *const c_void, flags: c_int) -> i64 {
    if msg.is_null() {
        return <Arch as Syscalls>::syscall3(M4_SYS_SENDMSG, fd as i64, 0, flags as i64);
    }

    let mut header: M4Msghdr;
    let mut control_words = [0usize; M4_SEND_CONTROL_WORDS];

    header = core::ptr::read_unaligned(msg as *const M4Msghdr);
    ...
    if header.msg_controllen != 0 {
        ...
        m4_zero_cmsg_padding(control, length);
    ...
    <Arch as Syscalls>::syscall3(
        M4_SYS_SENDMSG,
        fd as i64,
        &header as *const M4Msghdr as i64,
        flags as i64,
    )
}

unsafe fn m4_recvmsg_raw(fd: c_int, msg: *mut c_void, flags: c_int) -> i64 {
    if msg.is_null() {
        return <Arch as Syscalls>::syscall3(M4_SYS_RECVMSG, fd as i64, 0, flags as i64);
    }

    let mut header = core::ptr::read_unaligned(msg as *const M4Msghdr);
    header.msg_iovlen_pad = 0;
    header.msg_controllen_pad = 0;
    let result = <Arch as Syscalls>::syscall3(
        M4_SYS_RECVMSG,
        fd as i64,
        &mut header as *mut M4Msghdr as i64,
        flags as i64,
    );
    ...
    // paths.
    core::ptr::write_unaligned(msg as *mut M4Msghdr, header);
    result
}
...
pub unsafe extern "C" fn sendmsg(
    ...
    syscall_result(m4_sendmsg_raw(fd, msg, flags)) as SSizeT
}
...
pub unsafe extern "C" fn recvmsg(
    ...
    syscall_result(m4_recvmsg_raw(fd, msg, flags)) as SSizeT
}
...
pub unsafe extern "C" fn sendmmsg(
    ...
    let count = if vlen > M4_IOV_MAX { M4_IOV_MAX } else { vlen };
    let messages = msgvec as *mut M4Mmsghdr;
    let mut sent = 0u32;
    while sent < count {
        let message = messages.add(sent as usize);
        let result = m4_sendmsg_raw(
            fd,
            &(*message).msg_hdr as *const M4Msghdr as *const c_void,
            flags as c_int,
        );
        if result < 0 {
