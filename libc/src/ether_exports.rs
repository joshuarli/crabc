// Ethernet address conversion and legacy /etc/ethers mappings.
//
...
#[repr(C)]
pub struct CabiEtherAddr {
    pub ether_addr_octet: [u8; 6],
}

const CABI_ETHER_TEXT_LEN: usize = 18;
const CABI_ETHER_LINE_LEN: usize = 4096;
const CABI_ETHER_ENOENT: c_int = 2;

static mut CABI_ETHER_ATON_RESULT: CabiEtherAddr = CabiEtherAddr {
    ether_addr_octet: [0; 6],
};
static mut CABI_ETHER_NTOA_RESULT: [c_char; CABI_ETHER_TEXT_LEN] = [0; CABI_ETHER_TEXT_LEN];

#[inline]
unsafe fn cabi_ether_hex(c: u8) -> Option<u8> {
    match c {
        b'0'..=b'9' => Some(c - b'0'),
...
#[inline]
unsafe fn cabi_ether_space(c: u8) -> bool {
    matches!(c, b' ' | b'\t' | b'\n' | b'\r' | b'\x0b' | b'\x0c')
}

#[inline]
unsafe fn cabi_ether_ascii_lower(c: u8) -> u8 {
    if c >= b'A' && c <= b'Z' {
        c + (b'a' - b'A')
    } else {
        c
...
pub unsafe extern "C" fn ether_aton_r(
    asc: *const c_char,
    addr: *mut CabiEtherAddr,
) -> *mut CabiEtherAddr {
    if asc.is_null() || addr.is_null() {
        return core::ptr::null_mut();
    }

    let mut parsed = CabiEtherAddr {
        ether_addr_octet: [0; 6],
    };
    let mut cursor = asc;
    for index in 0..6 {
...
#[no_mangle]
pub unsafe extern "C" fn ether_aton(asc: *const c_char) -> *mut CabiEtherAddr {
    ether_aton_r(asc, core::ptr::addr_of_mut!(CABI_ETHER_ATON_RESULT))
}

#[no_mangle]
pub unsafe extern "C" fn ether_ntoa_r(
    addr: *const CabiEtherAddr,
    buffer: *mut c_char,
) -> *mut c_char {
...
#[no_mangle]
pub unsafe extern "C" fn ether_ntoa(addr: *const CabiEtherAddr) -> *mut c_char {
    ether_ntoa_r(addr, core::ptr::addr_of_mut!(CABI_ETHER_NTOA_RESULT).cast())
}
...
pub unsafe extern "C" fn ether_line(
    ...
    for index in 0..6 {
        let first = match cabi_ether_hex(*cursor) {
            Some(value) => value,
            None => return -1,
        };
        cursor = cursor.add(1);
        ...
        if index < 5 {
            if next != b':' {
                let second = match cabi_ether_hex(next) {
                    Some(value) => value,
                    None => return -1,
                };
            ...
            cursor = cursor.add(1);
        } else if next != 0 && !cabi_ether_space(next) {
            let second = match cabi_ether_hex(next) {
                Some(value) => value,
                None => return -1,
            };
    ...
    if *cursor != 0 && !cabi_ether_space(*cursor) {
        return -1;
    }
    while cabi_ether_space(*cursor) {
        cursor = cursor.add(1);
    }
    ...
    while *cursor != 0 && *cursor != b'#' && !cabi_ether_space(*cursor) {
        *host_cursor = *cursor;
...
#[inline]
unsafe fn cabi_ether_name_equal(left: *const c_char, right: *const c_char) -> bool {
    let mut index = 0usize;
    loop {
        let left_byte = cabi_ether_ascii_lower(*left.add(index) as u8);
        let right_byte = cabi_ether_ascii_lower(*right.add(index) as u8);
...
unsafe fn cabi_ether_lookup(
    wanted_name: *const c_char,
    wanted_addr: *const CabiEtherAddr,
    output_name: *mut c_char,
    output_addr: *mut CabiEtherAddr,
) -> c_int {
    ...
    let mut line = [0u8; CABI_ETHER_LINE_LEN];
    ...
    let _ = sys_close(fd);
    ERRNO = CABI_ETHER_ENOENT;
    -1
}

unsafe fn cabi_ether_lookup_line(
    line: *const c_char,
    wanted_name: *const c_char,
    wanted_addr: *const CabiEtherAddr,
    output_name: *mut c_char,
    ...
    let mut parsed_addr = CabiEtherAddr {
        ether_addr_octet: [0; 6],
    };
    let mut parsed_name = [0 as c_char; CABI_ETHER_LINE_LEN];
    if ether_line(
        line,
        &mut parsed_addr,
    ...
    let matches = if !wanted_name.is_null() {
        cabi_ether_name_equal(wanted_name, parsed_name.as_ptr())
    } else {
...
pub unsafe extern "C" fn ether_hostton(
    ...
    cabi_ether_lookup(hostname, core::ptr::null(), core::ptr::null_mut(), addr)
}
...
pub unsafe extern "C" fn ether_ntohost(
    ...
    cabi_ether_lookup(core::ptr::null(), addr, hostname, core::ptr::null_mut())
}
