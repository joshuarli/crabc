// M4 getopt state and program-name exports.
//
...
unsafe fn m4_getopt_message(argv0: *const c_char, prefix: *const u8, option: *const c_char, len: usize) {
...
#[inline]
unsafe fn m4_getopt_codepoint(argument: *const c_char, position: c_int) -> (c_int, c_int) {
    let mut codepoint = 0;
...
pub unsafe extern "C" fn getopt(
    ...
    let (option, width) = m4_getopt_codepoint(current, __optpos);
    __optpos += width;
...
const M4_GETOPT_NO_ARGUMENT: c_int = 0;
const M4_GETOPT_REQUIRED_ARGUMENT: c_int = 1;

#[inline]
unsafe fn m4_getopt_long_colon(optstring: *const c_char) -> bool {
...
unsafe fn m4_getopt_long_permute(
    argv: *const *mut c_char,
    dest: c_int,
    src: c_int,
) {
...
unsafe fn m4_getopt_long_core(
    argc: c_int,
    argv: *const *mut c_char,
    optstring: *const c_char,
    longopts: *const M4GetoptOption,
    idx: *mut c_int,
    longonly: bool,
) -> c_int {
    ...
    {
        let colon = m4_getopt_long_colon(optstring);
        let mut count = 0;
        ...
        if count == 1 {
            ...
            if *argument as u8 == b'=' {
                if option.has_arg == M4_GETOPT_NO_ARGUMENT {
                    optopt = option.val;
                    if colon || opterr == 0 {
                        return b'?' as c_int;
                    }
            ...
            } else if option.has_arg == M4_GETOPT_REQUIRED_ARGUMENT {
        ...
        if *current.add(1) as u8 == b'-' {
            ...
            if !colon && opterr != 0 {
                ...
                m4_getopt_message(*argv, prefix, option_name, strlen(option_name));
            }
            optind += 1;
            return b'?' as c_int;
        }
...
unsafe fn m4_getopt_long_impl(
    ...
    let resumed = optind;
    let result = m4_getopt_long_core(argc, argv, optstring, longopts, idx, longonly);
    if resumed > skipped {
        let count = optind - resumed;
        let mut i = 0;
        while i < count {
            m4_getopt_long_permute(argv, skipped, optind - 1);
            i += 1;
        }
...
pub unsafe extern "C" fn getopt_long(
    ...
    m4_getopt_long_impl(argc, argv, optstring, longopts, idx, false)
}
...
pub unsafe extern "C" fn getopt_long_only(
    ...
    m4_getopt_long_impl(argc, argv, optstring, longopts, idx, true)
}
...
pub unsafe fn m4_set_program_names(argv0: *const c_char) {
    if argv0.is_null() {
        return;
    }
    let mut short = argv0;
