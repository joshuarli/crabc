// M4 non-portable pthread extensions.
//
...
const M4_PTHREAD_DEFAULT_STACK_MAX: usize = 8 * 1024 * 1024;
const M4_PTHREAD_DEFAULT_GUARD_MAX: usize = 1024 * 1024;
const M4_PTHREAD_DEFAULT_STACK_SIZE: usize = STACK_SIZE;
const M4_PTHREAD_DEFAULT_GUARD_SIZE: usize = 4096;
const M4_PTHREAD_EIO: c_int = 5;
const M4_PTHREAD_ERANGE: c_int = 34;
const M4_PTHREAD_ENAMETOOLONG: c_int = 36;
const M4_PTHREAD_ESRCH: c_int = 3;
...
const M4_ATTR_STACK_SIZE: usize = 0;
const M4_ATTR_GUARD_SIZE: usize = 2;
const M4_ATTR_STACK_ADDR: usize = 4;
const M4_ATTR_DETACH: usize = 6;
const M4_ATTR_INHERIT_SCHED: usize = 7;
const M4_ATTR_POLICY: usize = 8;
const M4_ATTR_PRIORITY: usize = 9;

static M4_DEFAULT_STACK_SIZE: AtomicUsize = AtomicUsize::new(M4_PTHREAD_DEFAULT_STACK_SIZE);
static M4_DEFAULT_GUARD_SIZE: AtomicUsize = AtomicUsize::new(M4_PTHREAD_DEFAULT_GUARD_SIZE);

// pthread_attr_init in the pre-M4 implementation currently has its defaults
...
#[inline]
fn m4_pthread_default_stack_size() -> usize {
    M4_DEFAULT_STACK_SIZE.load(Ordering::Acquire)
}

#[inline]
fn m4_pthread_default_guard_size() -> usize {
    M4_DEFAULT_GUARD_SIZE.load(Ordering::Acquire)
}

#[inline]
unsafe fn m4_pthread_attr_zero_except_sizes(attr: *const pthread_attr_t) -> bool {
    let fields = &(*attr).__i;
    for i in 0..fields.len() {
        if i != M4_ATTR_STACK_SIZE && i != M4_ATTR_GUARD_SIZE && fields[i] != 0 {
            return false;
        }
    }
    true
...
#[inline]
unsafe fn m4_pthread_attr_set_sizes(attr: *mut pthread_attr_t, stack_size: usize, guard_size: usize) {
    *((*attr).__i.as_mut_ptr() as *mut usize) = stack_size;
    *((*attr).__i.as_mut_ptr().add(M4_ATTR_GUARD_SIZE) as *mut usize) = guard_size;
}

#[inline]
unsafe fn m4_pthread_attr_stack_size(attr: *const pthread_attr_t) -> usize {
    *((*attr).__i.as_ptr() as *const usize)
}

#[inline]
unsafe fn m4_pthread_attr_guard_size(attr: *const pthread_attr_t) -> usize {
    *((*attr).__i.as_ptr().add(M4_ATTR_GUARD_SIZE) as *const usize)
}
...
pub unsafe extern "C" fn pthread_getattr_default_np(attr: *mut pthread_attr_t) -> c_int {
    ...
    m4_pthread_attr_set_sizes(
        attr,
        m4_pthread_default_stack_size(),
        m4_pthread_default_guard_size(),
    );
    0
}
...
pub unsafe extern "C" fn pthread_setattr_default_np(attr: *const pthread_attr_t) -> c_int {
    if attr.is_null() || !m4_pthread_attr_zero_except_sizes(attr) {
        return EINVAL;
    }

    // musl permits callers to request a larger default, caps the request, and
    // never lowers an already-selected process default.
    let stack_size = core::cmp::min(m4_pthread_attr_stack_size(attr), M4_PTHREAD_DEFAULT_STACK_MAX);
    let guard_size = core::cmp::min(m4_pthread_attr_guard_size(attr), M4_PTHREAD_DEFAULT_GUARD_MAX);
    loop {
        let old = M4_DEFAULT_STACK_SIZE.load(Ordering::Acquire);
        let new = core::cmp::max(old, stack_size);
        if M4_DEFAULT_STACK_SIZE
    ...
    loop {
        let old = M4_DEFAULT_GUARD_SIZE.load(Ordering::Acquire);
        let new = core::cmp::max(old, guard_size);
        if M4_DEFAULT_GUARD_SIZE
            .compare_exchange(old, new, Ordering::AcqRel, Ordering::Acquire)
            .is_ok()
        {
            break;
...
#[inline]
unsafe fn m4_pthread_slot_in_range(thread: PthreadT) -> Option<*mut Thread> {
    if thread == 0 {
        return None;
    }
...
#[inline]
unsafe fn m4_pthread_live_tid(thread: PthreadT) -> Result<( *mut Thread, c_int), c_int> {
    let Some(slot) = m4_pthread_slot_in_range(thread) else {
        return Err(M4_PTHREAD_ESRCH);
    };
    let tid = core::ptr::read_volatile(core::ptr::addr_of!((*slot).tid));
    if tid <= 0 {
        Err(M4_PTHREAD_ESRCH)
    } else {
        Ok((slot, tid))
    }
}
...
#[inline]
unsafe fn m4_pthread_join_slot(thread: PthreadT) -> Result<*mut Thread, c_int> {
    let Some(slot) = m4_pthread_slot_in_range(thread) else {
        return Err(EINVAL);
    };
    let tid = core::ptr::read_volatile(core::ptr::addr_of!((*slot).tid));
...
#[inline]
unsafe fn m4_pthread_append_decimal(path: &mut [u8], at: &mut usize, value: c_int) -> bool {
    if value < 0 {
        return false;
    }
...
#[inline]
unsafe fn m4_pthread_comm_path(tid: c_int, path: &mut [u8; 64]) -> bool {
    ...
    if !m4_pthread_append_decimal(path, &mut at, tid) {
        return false;
    }
    for byte in suffix {
        if at >= path.len() {
...
#[inline]
unsafe fn m4_pthread_name_length(name: *const c_char) -> Result<usize, c_int> {
    if name.is_null() {
        return Err(EINVAL);
    }
    ...
    Err(M4_PTHREAD_ERANGE)
}

#[cfg(target_arch = "x86_64")]
const M4_PTHREAD_SYS_PRCTL: i64 = 157;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_PTHREAD_SYS_PRCTL: i64 = 167;

const M4_PTHREAD_PR_SET_NAME: i64 = 15;
const M4_PTHREAD_PR_GET_NAME: i64 = 16;
const M4_PTHREAD_O_RDONLY: c_int = 0;
const M4_PTHREAD_O_WRONLY: c_int = 1;
const M4_PTHREAD_O_CLOEXEC: c_int = 0x80000;

#[inline]
unsafe fn m4_pthread_prctl_name(option: i64, name: *mut c_char) -> c_int {
    let result = <Arch as Syscalls>::syscall5(
        M4_PTHREAD_SYS_PRCTL,
        option,
        name as i64,
        0,
        0,
...
pub unsafe extern "C" fn pthread_setname_np(thread: PthreadT, name: *const c_char) -> c_int {
    let length = match m4_pthread_name_length(name) {
        Ok(length) if length <= 15 => length,
        Ok(_) | Err(M4_PTHREAD_ERANGE) => return M4_PTHREAD_ERANGE,
        Err(error) => return error,
    };
    let (_, tid) = match m4_pthread_live_tid(thread) {
        Ok(value) => value,
        Err(error) => return error,
    };

    if thread == pthread_self() {
        return m4_pthread_prctl_name(M4_PTHREAD_PR_SET_NAME, name as *mut c_char);
    }

    let mut path = [0u8; 64];
    if !m4_pthread_comm_path(tid, &mut path) {
        return M4_PTHREAD_ENAMETOOLONG;
    }
    let fd = sys_open(path.as_ptr(), (M4_PTHREAD_O_WRONLY | M4_PTHREAD_O_CLOEXEC) as i64, 0);
    if fd < 0 {
        return (-fd) as c_int;
    }
...
pub unsafe extern "C" fn pthread_getname_np(
    ...
    if thread == pthread_self() {
        return m4_pthread_prctl_name(M4_PTHREAD_PR_GET_NAME, name);
    }
    ...
    let fd = sys_open(path.as_ptr(), (M4_PTHREAD_O_RDONLY | M4_PTHREAD_O_CLOEXEC) as i64, 0);
    if fd < 0 {
        return (-fd) as c_int;
    }
...
unsafe fn m4_pthread_main_stack(attr: *mut pthread_attr_t) -> c_int {
    let path = b"/proc/self/maps\0";
    let fd = sys_open(path.as_ptr(), M4_PTHREAD_O_RDONLY as i64, 0);
    if fd < 0 {
        return (-fd) as c_int;
    }
    ...
    let _ = sys_close(fd);
    if count <= 0 {
        return if count < 0 { (-count) as c_int } else { M4_PTHREAD_EIO };
    }
        ...
        if end >= marker.len() {
            ...
            if bytes[marker_start..end] == *marker {
                ...
                if split > line && split + 1 < end {
                    ...
                    if finish > start {
                        m4_pthread_attr_set_sizes(attr, finish - start, m4_pthread_default_guard_size());
                        *((*attr).__i.as_mut_ptr().add(M4_ATTR_STACK_ADDR) as *mut usize) = start;
                        (*attr).__i[M4_ATTR_DETACH] = PTHREAD_CREATE_JOINABLE;
                        return 0;
                    }
                }
            }
...
pub unsafe extern "C" fn pthread_getattr_np(
    ...
    if (*slot).stack.is_null() {
        return m4_pthread_main_stack(attr);
    }
...
#[cfg(target_arch = "x86_64")]
const M4_PTHREAD_SYS_SCHED_SETPARAM: i64 = 142;
#[cfg(target_arch = "x86_64")]
const M4_PTHREAD_SYS_SCHED_GETPARAM: i64 = 143;
#[cfg(target_arch = "x86_64")]
const M4_PTHREAD_SYS_SCHED_SETSCHEDULER: i64 = 144;
#[cfg(target_arch = "x86_64")]
const M4_PTHREAD_SYS_SCHED_GETSCHEDULER: i64 = 145;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_PTHREAD_SYS_SCHED_SETPARAM: i64 = 142;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_PTHREAD_SYS_SCHED_GETPARAM: i64 = 143;
...
#[inline]
fn m4_pthread_syscall_errno(result: i64) -> c_int {
    if result < 0 && result >= -4095 {
        (-result) as c_int
    } else if result < 0 {
        EINVAL
...
pub unsafe extern "C" fn pthread_getschedparam(
    ...
    let result = <Arch as Syscalls>::syscall2(M4_PTHREAD_SYS_SCHED_GETPARAM, tid as i64, param as i64);
    let error = m4_pthread_syscall_errno(result);
    if error != 0 {
        return error;
    }
    let scheduler = <Arch as Syscalls>::syscall1(M4_PTHREAD_SYS_SCHED_GETSCHEDULER, tid as i64);
    let error = m4_pthread_syscall_errno(scheduler);
    if error != 0 {
        return error;
    }
...
pub unsafe extern "C" fn pthread_setschedparam(
    ...
    let result = <Arch as Syscalls>::syscall3(
        M4_PTHREAD_SYS_SCHED_SETSCHEDULER,
        tid as i64,
        policy as i64,
        param as i64,
    );
    m4_pthread_syscall_errno(result)
}
...
pub unsafe extern "C" fn pthread_tryjoin_np(thread: PthreadT, retval: *mut *mut c_void) -> c_int {
    let slot = match m4_pthread_join_slot(thread) {
        Ok(slot) => slot,
        Err(error) => return error,
    };
