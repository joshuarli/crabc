...
const M4_CLONE_VM: c_int = 0x00000100;
const M4_CLONE_PIDFD: c_int = 0x00001000;
const M4_CLONE_THREAD: c_int = 0x00010000;
const M4_CLONE_SETTLS: c_int = 0x00080000;
const M4_CLONE_PARENT_SETTID: c_int = 0x00100000;
const M4_CLONE_CHILD_CLEARTID: c_int = 0x00200000;
const M4_CLONE_CHILD_SETTID: c_int = 0x01000000;
...
// copy in m4_clone_post_fork(0), and the parent releases its copy on every
// return from __m4_clone.
static M4_CLONE_ABORT_LOCK: AtomicI32 = AtomicI32::new(0);

#[inline]
unsafe fn m4_clone_lock_abort() {
    while M4_CLONE_ABORT_LOCK
...
#[inline]
unsafe fn m4_clone_unlock_abort() {
    M4_CLONE_ABORT_LOCK.store(0, Ordering::Release);
}

#[repr(C)]
struct M4CloneStartArgs {
...
unsafe fn m4_clone_block_all_sigs(old: *mut SigSetT) {
    let mut all: SigSetT = 0;
    sigfillset(&mut all);
    let _ = sigprocmask(SIG_BLOCK, &all, old);
}

unsafe fn m4_clone_restore_sigs(old: *const SigSetT) {
...
// child exit status.
unsafe extern "C" fn m4_clone_start(arg: *mut c_void) -> c_int {
    let csa = &*(arg as *const M4CloneStartArgs);
    m4_clone_post_fork(0);
    m4_clone_restore_sigs(&csa.sigmask);
    (csa.func)(csa.arg)
}

// __m4_clone(func, stack, flags, arg, ptid, tls, ctid)
//
...
pub unsafe extern "C" fn clone(
    ...
    const BAD_FLAGS: c_int = M4_CLONE_THREAD | M4_CLONE_SETTLS | M4_CLONE_CHILD_CLEARTID;
    ...
    if (flags & (M4_CLONE_PIDFD | M4_CLONE_PARENT_SETTID | M4_CLONE_CHILD_SETTID)) != 0 {
        ptid = args.next_arg();
    }
    if (flags & M4_CLONE_CHILD_SETTID) != 0 {
        tls = args.next_arg();
        ctid = args.next_arg();
    }
    ...
    // m4_clone_start in the child.
    if (flags & M4_CLONE_VM) != 0 {
        return syscall_result(__m4_clone(
            func as usize,
            stack as *mut u8,
            flags,
            arg,
    ...
    let mut csa = M4CloneStartArgs {
        func,
        arg,
        sigmask: 0,
    };
    m4_clone_block_all_sigs(&mut csa.sigmask);
    m4_clone_lock_abort();
    let ret = __m4_clone(
        m4_clone_start as *const () as usize,
        stack as *mut u8,
        flags,
        &mut csa as *mut M4CloneStartArgs as *mut c_void,
        ptid,
        tls,
        ctid,
    );
    // __m4_clone returns only in the parent.  On failure this also releases
    // the lock before errno conversion; the child releases its private copy
    // in m4_clone_start before the callback runs.
    m4_clone_post_fork(ret);
    m4_clone_restore_sigs(&csa.sigmask);
