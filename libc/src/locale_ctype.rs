...
#[inline]
const fn cabi_ctype_b_x(value: u16) -> u16 {
...
const fn cabi_ctype_b_value(c: i32) -> u16 {
    if c < 0 || c > 127 {
        return 0;
    }
...
const fn cabi_ctype_b_table() -> [u16; 384] {
    let mut table = [0u16; 384];
    let mut i = 0usize;
    while i < table.len() {
        table[i] = cabi_ctype_b_value(i as i32 - 128);
        i += 1;
    }
    table
}

const fn cabi_ctype_tolower_value(c: i32) -> i32 {
    if c >= b'A' as i32 && c <= b'Z' as i32 { c + 32 }
    else if c >= 0 && c <= 127 { c }
    else { 0 }
}

const fn cabi_ctype_toupper_value(c: i32) -> i32 {
    if c >= b'a' as i32 && c <= b'z' as i32 { c - 32 }
    else if c >= 0 && c <= 127 { c }
    else { 0 }
}

const fn cabi_ctype_tolower_table() -> [i32; 384] {
    let mut table = [0i32; 384];
    let mut i = 0usize;
    while i < table.len() {
        table[i] = cabi_ctype_tolower_value(i as i32 - 128);
        i += 1;
    }
    table
}

const fn cabi_ctype_toupper_table() -> [i32; 384] {
    let mut table = [0i32; 384];
    let mut i = 0usize;
    while i < table.len() {
        table[i] = cabi_ctype_toupper_value(i as i32 - 128);
        i += 1;
    }
    table
}

static CABI_CTYPE_B_TABLE: [u16; 384] = cabi_ctype_b_table();
static CABI_CTYPE_TOLOWER_TABLE: [i32; 384] = cabi_ctype_tolower_table();
static CABI_CTYPE_TOUPPER_TABLE: [i32; 384] = cabi_ctype_toupper_table();
...
static mut CABI_CTYPE_B_PTR: *const u16 =
    core::ptr::addr_of!(CABI_CTYPE_B_TABLE).cast::<u16>().wrapping_add(128);
static mut CABI_CTYPE_TOLOWER_PTR: *const i32 =
    core::ptr::addr_of!(CABI_CTYPE_TOLOWER_TABLE).cast::<i32>().wrapping_add(128);
static mut CABI_CTYPE_TOUPPER_PTR: *const i32 =
    core::ptr::addr_of!(CABI_CTYPE_TOUPPER_TABLE).cast::<i32>().wrapping_add(128);

#[no_mangle]
pub unsafe extern "C" fn __ctype_b_loc() -> *const *const u16 {
    core::ptr::addr_of!(CABI_CTYPE_B_PTR)
}

#[no_mangle]
pub unsafe extern "C" fn __ctype_tolower_loc() -> *const *const i32 {
    core::ptr::addr_of!(CABI_CTYPE_TOLOWER_PTR)
}

#[no_mangle]
pub unsafe extern "C" fn __ctype_toupper_loc() -> *const *const i32 {
    core::ptr::addr_of!(CABI_CTYPE_TOUPPER_PTR)
}
