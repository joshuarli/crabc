...
pub type CabiDlIterateCallback = unsafe extern "C" fn(
    *mut dl_phdr_info,
    usize,
    *mut c_void,
) -> c_int;
pub type CabiLdsoIteratePhdr = unsafe extern "C" fn(
    CabiDlIterateCallback,
    *mut c_void,
) -> c_int;
pub type CabiLdsoDladdr = unsafe extern "C" fn(
    *const c_void,
    *mut CabiDladdrResult,
) -> c_int;
pub type CabiLdsoDlinfo = unsafe extern "C" fn(
    *mut c_void,
    c_int,
    *mut c_void,
) -> c_int;
...
#[repr(C)]
#[derive(Clone, Copy)]
pub struct CabiLoaderSnapshotText {
    len: u16,
    flags: u16,
    bytes: [u8; 256],
}
...
pub struct CabiLoaderImageV1 {
    ...
    tls_module: usize,
    tls_data: *mut c_void,
    image_name: CabiLoaderSnapshotText,
}

#[repr(C)]
pub struct CabiLoaderInformationV1 {
    image_base: *mut c_void,
    dynamic_address: *mut c_void,
    image_name: CabiLoaderSnapshotText,
}

pub type CabiLdsoLoaderSnapshot = unsafe extern "C" fn(
    *mut CabiLoaderImageV1,
    usize,
    *mut usize,
    *mut u64,
    *mut CabiLoaderSnapshotText,
) -> c_int;
pub type CabiLdsoLoaderInformation = unsafe extern "C" fn(
    *mut c_void,
    *mut CabiLoaderInformationV1,
    *mut CabiLoaderSnapshotText,
) -> c_int;
...
const CABI_LDSO_PRIVATE_HANDLE: *mut c_void = 2usize as *mut c_void;

unsafe fn cabi_ldso_private_symbol(name: *const c_char) -> *mut c_void {
    let Some(resolve) = LDSO_DLSYM else {
        return core::ptr::null_mut();
    };
    resolve(CABI_LDSO_PRIVATE_HANDLE, name)
}

unsafe fn cabi_ldso_iterate_phdr() -> Option<CabiLdsoIteratePhdr> {
    let address = cabi_ldso_private_symbol(b"__crabc_ldso_iterate_phdr\0".as_ptr().cast());
    (!address.is_null()).then(|| core::mem::transmute(address))
}

unsafe fn cabi_ldso_dladdr() -> Option<CabiLdsoDladdr> {
    let address = cabi_ldso_private_symbol(b"__crabc_ldso_dladdr\0".as_ptr().cast());
    (!address.is_null()).then(|| core::mem::transmute(address))
}

unsafe fn cabi_ldso_dlinfo() -> Option<CabiLdsoDlinfo> {
    let address = cabi_ldso_private_symbol(b"__crabc_ldso_dlinfo\0".as_ptr().cast());
    (!address.is_null()).then(|| core::mem::transmute(address))
}

unsafe fn cabi_ldso_tls_get_addr() -> *mut c_void {
    cabi_ldso_private_symbol(b"__crabc_ldso_tls_get_addr\0".as_ptr().cast())
}

unsafe fn cabi_ldso_loader_snapshot() -> Option<CabiLdsoLoaderSnapshot> {
    let address = cabi_ldso_private_symbol(b"__crabc_ldso_loader_snapshot\0".as_ptr().cast());
    (!address.is_null()).then(|| core::mem::transmute(address))
}

unsafe fn cabi_ldso_loader_information() -> Option<CabiLdsoLoaderInformation> {
    let address = cabi_ldso_private_symbol(b"__crabc_ldso_loader_information\0".as_ptr().cast());
    (!address.is_null()).then(|| core::mem::transmute(address))
}
...
pub unsafe extern "C" fn dl_iterate_phdr(
    callback: Option<CabiDlIterateCallback>,
    data: *mut c_void,
) -> c_int {
    let Some(callback) = callback else {
        return 0;
    };
    match cabi_ldso_iterate_phdr() {
...
pub unsafe extern "C" fn dladdr(
    ...
    let Some(dladdr_impl) = cabi_ldso_dladdr() else {
        return 0;
    };

    let mut result = CabiDladdrResult {
        fname: core::ptr::null(),
        fbase: 0,
        sname: core::ptr::null(),
        saddr: 0,
...
pub unsafe extern "C" fn dlinfo(
    ...
    let Some(dlinfo_impl) = cabi_ldso_dlinfo() else {
        return -1;
    };
