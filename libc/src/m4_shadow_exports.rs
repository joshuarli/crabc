...
const M4_SHADOW_PATH: &[u8] = b"/etc/shadow\0";
const M4_SHADOW_MODE: &[u8] = b"rbe\0";
...
const M4_SHADOW_LOCK_PATH: &[u8] = b"/etc/.pwd.lock\0";
...
static M4_SHADOW_LOCK_FD: AtomicI32 = AtomicI32::new(-1);

static mut M4_SHADOW_FILE: *mut FILE = core::ptr::null_mut();
static mut M4_SHADOW_LINE: *mut c_char = core::ptr::null_mut();
static mut M4_SHADOW_LINE_SIZE: usize = 0;
static mut M4_SHADOW_RESULT: M4Shadow = M4Shadow {
...
static mut M4_SHADOW_LOOKUP_LINE: *mut c_char = core::ptr::null_mut();
static mut M4_SHADOW_LOOKUP_LINE_SIZE: usize = 0;
static mut M4_SHADOW_LOOKUP_RESULT: M4Shadow = M4Shadow {
    sp_namp: core::ptr::null_mut(),
    sp_pwdp: core::ptr::null_mut(),
    sp_lstchg: 0,
    sp_min: 0,
...
static mut M4_FGETSPENT_LINE: *mut c_char = core::ptr::null_mut();
static mut M4_FGETSPENT_LINE_SIZE: usize = 0;
static mut M4_FGETSPENT_RESULT: M4Shadow = M4Shadow {
    sp_namp: core::ptr::null_mut(),
    sp_pwdp: core::ptr::null_mut(),
    sp_lstchg: 0,
    sp_min: 0,
...
#[inline]
unsafe fn m4_shadow_empty() -> M4Shadow {
    M4Shadow {
        sp_namp: core::ptr::null_mut(),
        sp_pwdp: core::ptr::null_mut(),
        sp_lstchg: 0,
...
unsafe fn m4_shadow_parse_signed(field: *const c_char, value: *mut c_long) -> bool {
...
unsafe fn m4_shadow_parse_flag(field: *const c_char, value: *mut c_ulong) -> bool {
    if field.is_null() || value.is_null() {
        return false;
    }
    if *field == 0 {
    ...
    if *field as u8 == b'-' {
        ...
        if !m4_shadow_parse_signed(field, &mut signed) {
            return false;
        }
...
unsafe fn m4_shadow_parse_line(line: *mut c_char, length: usize, shadow: *mut M4Shadow) -> bool {
    ...
    while date_index < dates.len() {
        if !m4_shadow_parse_signed(fields[date_index + 2], &mut dates[date_index]) {
            return false;
        }
        date_index += 1;
    }
    let mut flag = 0 as c_ulong;
    if !m4_shadow_parse_flag(fields[8], &mut flag) {
        return false;
    }
...
unsafe fn m4_shadow_next(
    ...
    loop {
        ...
        if m4_shadow_parse_line(*line, length as usize, shadow) {
            return 1;
        }
    }
}

unsafe fn m4_shadow_open() -> *mut FILE {
    fopen(
        M4_SHADOW_PATH.as_ptr() as *const c_char,
        M4_SHADOW_MODE.as_ptr() as *const c_char,
    )
}

unsafe fn m4_shadow_name_matches(name: *const c_char, record: *const c_char) -> bool {
    !name.is_null() && !record.is_null() && strcmp(name as *const u8, record as *const u8) == 0
}

unsafe fn m4_shadow_scan_name(
    name: *const c_char,
    file: *mut FILE,
    line: *mut *mut c_char,
    line_size: *mut usize,
    ...
    loop {
        let status = m4_shadow_next(file, line, line_size, shadow);
        if status <= 0 {
            return status;
        }
        if m4_shadow_name_matches(name, (*shadow).sp_namp) {
            return 1;
        }
    }
}

unsafe fn m4_shadow_copy_result(
    source: *const M4Shadow,
    destination: *mut M4Shadow,
    buffer: *mut c_char,
    buffer_size: usize,
    result: *mut *mut M4Shadow,
) -> c_int {
...
unsafe fn m4_shadow_reentrant(
    ...
    *result = core::ptr::null_mut();
    ...
    let mut parsed = m4_shadow_empty();
    let status = m4_shadow_scan_name(name, file, &mut line, &mut line_size, &mut parsed);
    let scan_errno = ERRNO;
    let result_code = if status > 0 {
        m4_shadow_copy_result(&parsed, shadow, buffer, buffer_size, result)
    } else if status < 0 {
...
pub unsafe extern "C" fn getspnam_r(
    ...
    m4_shadow_reentrant(name, shadow, buffer, buffer_size, result)
}

unsafe fn m4_shadow_global_lookup(name: *const c_char) -> *mut M4Shadow {
    if name.is_null() {
        ERRNO = EINVAL;
    ...
    ERRNO = 0;
    let status = m4_shadow_scan_name(
        name,
        file,
        core::ptr::addr_of_mut!(M4_SHADOW_LOOKUP_LINE),
        core::ptr::addr_of_mut!(M4_SHADOW_LOOKUP_LINE_SIZE),
        core::ptr::addr_of_mut!(M4_SHADOW_LOOKUP_RESULT),
    );
    let scan_errno = ERRNO;
    let _ = fclose(file);
    if status > 0 {
...
#[no_mangle]
pub unsafe extern "C" fn getspnam(name: *const c_char) -> *mut M4Shadow {
    m4_shadow_global_lookup(name)
}

#[no_mangle]
pub unsafe extern "C" fn setspent() {
    let file = core::ptr::addr_of_mut!(M4_SHADOW_FILE);
    if !(*file).is_null() {
        let _ = fclose(*file);
        *file = core::ptr::null_mut();
    }
    free(M4_SHADOW_LINE as *mut c_void);
    M4_SHADOW_LINE = core::ptr::null_mut();
    M4_SHADOW_LINE_SIZE = 0;
}
...
#[no_mangle]
pub unsafe extern "C" fn getspent() -> *mut M4Shadow {
    let file = core::ptr::addr_of_mut!(M4_SHADOW_FILE);
    if (*file).is_null() {
        *file = m4_shadow_open();
        if (*file).is_null() {
    ...
    let status = m4_shadow_next(
        *file,
        core::ptr::addr_of_mut!(M4_SHADOW_LINE),
        core::ptr::addr_of_mut!(M4_SHADOW_LINE_SIZE),
        core::ptr::addr_of_mut!(M4_SHADOW_RESULT),
    );
    if status > 0 {
        core::ptr::addr_of_mut!(M4_SHADOW_RESULT)
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn fgetspent(file: *mut FILE) -> *mut M4Shadow {
    if file.is_null() {
        ERRNO = EINVAL;
        return core::ptr::null_mut();
    }
    let status = m4_shadow_next(
        file,
        core::ptr::addr_of_mut!(M4_FGETSPENT_LINE),
        core::ptr::addr_of_mut!(M4_FGETSPENT_LINE_SIZE),
        core::ptr::addr_of_mut!(M4_FGETSPENT_RESULT),
    );
    if status > 0 {
        core::ptr::addr_of_mut!(M4_FGETSPENT_RESULT)
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn putspent(shadow: *const M4Shadow, file: *mut FILE) -> c_int {
...
pub unsafe extern "C" fn lckpwdf() -> c_int {
    ...
    let fd = open(
        M4_SHADOW_LOCK_PATH.as_ptr() as *const c_char,
        O_WRONLY | O_CREAT | O_CLOEXEC,
        0o600,
    );
    if fd < 0 {
        M4_SHADOW_LOCK_FD.store(-1, Ordering::Release);
        return -1;
    }
    ...
    if m4_lockf(fd, M4_F_TLOCK, 0) < 0 {
        let _ = close(fd);
        M4_SHADOW_LOCK_FD.store(-1, Ordering::Release);
        return -1;
    }

    M4_SHADOW_LOCK_FD.store(fd, Ordering::Release);
    0
}

#[no_mangle]
pub unsafe extern "C" fn ulckpwdf() -> c_int {
    let fd = M4_SHADOW_LOCK_FD.load(Ordering::Acquire);
    if fd < 0
        || M4_SHADOW_LOCK_FD
    ...
    let unlock_result = m4_lockf(fd, M4_F_ULOCK, 0);
