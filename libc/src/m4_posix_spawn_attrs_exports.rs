// M4 POSIX spawn attributes and file-action extensions.
//
...
// __actions pointer as an ordered heap list; m4_spawn_apply_linked_actions and
// m4_spawn_destroy_linked_actions are kept private to this crate so the
// existing spawn child path can apply and release that list.

const M4_POSIX_SPAWN_VALID_FLAGS: c_int =
    1 | 2 | 4 | 8 | 16 | 32 | 64;

const M4_SPAWN_ACTION_OPEN: c_int = 0;
const M4_SPAWN_ACTION_CHDIR: c_int = 1;
const M4_SPAWN_ACTION_FCHDIR: c_int = 2;

#[cfg(target_arch = "x86_64")]
const M4_SPAWN_SYS_FCHDIR: i64 = 81;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SPAWN_SYS_FCHDIR: i64 = 50;

#[repr(C)]
struct M4SpawnLinkedAction {
    kind: c_int,
    fd: c_int,
    oflag: c_int,
    mode: c_uint,
    path: *mut c_char,
    next: *mut M4SpawnLinkedAction,
}

#[inline]
unsafe fn m4_spawn_attr_valid(attr: *const posix_spawnattr_t) -> bool {
    !attr.is_null()
}

#[inline]
unsafe fn m4_spawn_copy_sigset(dst: *mut c_ulong, src: *const c_ulong) {
...
pub unsafe extern "C" fn posix_spawnattr_setflags(
    ...
    if attr.is_null() || (flags as c_int & !M4_POSIX_SPAWN_VALID_FLAGS) != 0 {
        return EINVAL;
    }
...
pub unsafe extern "C" fn posix_spawnattr_getflags(
    ...
    if !m4_spawn_attr_valid(attr) || flags.is_null() {
        return EINVAL;
    }
...
pub unsafe extern "C" fn posix_spawnattr_getpgroup(
    ...
    if !m4_spawn_attr_valid(attr) || pgroup.is_null() {
        return EINVAL;
    }
    *pgroup = (*attr).__pgrp;
    0
...
pub unsafe extern "C" fn posix_spawnattr_setsigmask(
    ...
    m4_spawn_copy_sigset((*attr).__mask.as_mut_ptr(), mask);
    0
}
...
pub unsafe extern "C" fn posix_spawnattr_getsigmask(
    ...
    if !m4_spawn_attr_valid(attr) || mask.is_null() {
        return EINVAL;
    }
    m4_spawn_copy_sigset(mask, (*attr).__mask.as_ptr());
    0
}
...
pub unsafe extern "C" fn posix_spawnattr_setsigdefault(
    ...
    m4_spawn_copy_sigset((*attr).__def.as_mut_ptr(), default);
    0
}
...
pub unsafe extern "C" fn posix_spawnattr_getsigdefault(
    ...
    if !m4_spawn_attr_valid(attr) || default.is_null() {
        return EINVAL;
    }
    m4_spawn_copy_sigset(default, (*attr).__def.as_ptr());
    0
}
...
pub unsafe extern "C" fn posix_spawnattr_getschedparam(
    ...
    if !m4_spawn_attr_valid(attr) || param.is_null() {
        return EINVAL;
    }
...
pub unsafe extern "C" fn posix_spawnattr_getschedpolicy(
    ...
    if !m4_spawn_attr_valid(attr) || policy.is_null() {
        return EINVAL;
    }
    *policy = (*attr).__pol;
    0
...
#[inline]
unsafe fn m4_spawn_action_alloc(
    kind: c_int,
    fd: c_int,
    oflag: c_int,
    mode: c_uint,
    path: *const c_char,
) -> Result<*mut M4SpawnLinkedAction, c_int> {
    let path_len = if path.is_null() { 0 } else { strlen(path) + 1 };
    if (kind == M4_SPAWN_ACTION_OPEN || kind == M4_SPAWN_ACTION_CHDIR)
        && path.is_null()
    {
        return Err(EINVAL);
    }

    let bytes = core::mem::size_of::<M4SpawnLinkedAction>()
        .checked_add(path_len)
        .ok_or(ENOMEM)?;
    let action = malloc(bytes) as *mut M4SpawnLinkedAction;
    if action.is_null() {
        return Err(ENOMEM);
    }
...
#[inline]
unsafe fn m4_spawn_action_append(
    fa: *mut posix_spawn_file_actions_t,
    action: *mut M4SpawnLinkedAction,
) -> c_int {
    if fa.is_null() || action.is_null() {
        return EINVAL;
    }
    let mut tail = (*fa).__actions as *mut M4SpawnLinkedAction;
    if tail.is_null() {
...
unsafe fn m4_spawn_action_add(
    ...
    let action = match m4_spawn_action_alloc(kind, fd, oflag, mode, path) {
        Ok(action) => action,
        Err(error) => return error,
    };
    let result = m4_spawn_action_append(fa, action);
    if result != 0 {
...
pub unsafe extern "C" fn posix_spawn_file_actions_addopen(
    ...
    m4_spawn_action_add(fa, M4_SPAWN_ACTION_OPEN, fd, oflag, mode, path)
}
...
pub unsafe extern "C" fn posix_spawn_file_actions_addchdir_np(
    ...
    m4_spawn_action_add(fa, M4_SPAWN_ACTION_CHDIR, -1, 0, 0, path)
}
...
unsafe fn m4_spawn_apply_linked_actions(
    fa: *const posix_spawn_file_actions_t,
) -> c_int {
    if fa.is_null() {
        return 0;
    }
    let mut action = (*fa).__actions as *mut M4SpawnLinkedAction;
    while !action.is_null() {
        let result = match (*action).kind {
            M4_SPAWN_ACTION_OPEN => {
                let opened = sys_open(
            ...
            M4_SPAWN_ACTION_CHDIR => {
                let result = sys_chdir((*action).path as *const u8);
                if result < 0 { (-result) as c_int } else { 0 }
            }
            M4_SPAWN_ACTION_FCHDIR => {
                let result = <Arch as Syscalls>::syscall1(M4_SPAWN_SYS_FCHDIR, (*action).fd as i64);
                if result < 0 { (-result) as c_int } else { 0 }
            }
            _ => EINVAL,
        };
...
unsafe fn m4_spawn_destroy_linked_actions(fa: *mut posix_spawn_file_actions_t) {
    if fa.is_null() {
        return;
    }
    let mut action = (*fa).__actions as *mut M4SpawnLinkedAction;
