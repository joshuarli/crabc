...
static mut M4_CUSERID_RESULT: [c_char; 20] = [0; 20];
...
pub unsafe extern "C" fn cuserid(buffer: *mut c_char) -> *mut c_char {
    ...
    let mut password: M4Passwd = core::mem::zeroed();
    let mut password_result: *mut M4Passwd = core::ptr::null_mut();
    ...
    let destination = if buffer.is_null() {
        core::ptr::addr_of_mut!(M4_CUSERID_RESULT).cast::<c_char>()
    } else {
        buffer
    };
