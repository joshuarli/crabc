...
//! The adjacent GNU `__freading` observation also reads only the
//! process-lifetime `stdin` pointer. Musl's fixed stdin record retains
...
//! access mode, write cursor, or output transition.
//! The disjoint GNU `__fbufsize` observation reads only the process-lifetime
...
/// Observe musl's fixed permanent-stdin read-direction predicate.
///
/// Musl 1.2.6 `src/stdio/ext.c` implements `__freading` as
/// `(f->flags & F_NOWR) || f->rend`, while `src/stdio/stdin.c` fixes the
/// permanent stdin record's `F_PERM | F_NOWR` flags. This owned stdin object
/// therefore has the exact process-lifetime result `1` through the source
/// expression's first term.
/// It deliberately does not inspect a read cursor or import musl's general
/// FILE direction state. It rejects every pointer other than stdin; the result
/// does not select another permanent stream, pathname state, input
/// transitions, or any other stdio-extension helper.
///
/// # Safety
///
...
/// Observe musl's fixed permanent-stderr buffer capacity.
///
/// Musl 1.2.6 `src/stdio/ext.c` implements `__fbufsize` as
/// `return f->buf_size`, while its permanent `src/stdio/stderr.c` record fixes
/// that member at zero. The owned stderr record likewise retains capacity zero
/// before and after this module's unrelated static-stream initialization. This
/// private adapter therefore has the exact process-lifetime `size_t` result
/// `0` without importing a general FILE buffer-size query, configuration,
/// cursor, or output behavior. It intentionally rejects every pointer other
/// than stderr; the result does not select input, another permanent stream,
/// pathname state, or any other stdio-extension helper.
///
/// # Safety
///
/// `stream` must be the exported process-lifetime `stderr` pointer. Its
/// observation is externally serialized with the selected permanent stream.
#[no_mangle]
pub unsafe extern "C" fn __fbufsize(stream: *mut StandardStream) -> usize {
    // The fixed stderr capacity is complete before first use; this query must
    // not trigger shared permanent-stream buffer initialization.
    if stream != ptr::addr_of_mut!(STDERR_STREAM) {
        // SAFETY: no caller stream was dereferenced on this closed boundary.
        unsafe { reject_stream() };
        return 0;
