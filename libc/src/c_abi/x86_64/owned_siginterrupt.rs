...
//! The two action calls retain the source's public relocation. Without an
//! application interposer, replacing SA_RESTART reaches the owned outer
//! action transaction, updates sticky EINTR bookkeeping, and serializes
//! SIGABRT actions. The frozen private raw-syscall adapter remains separately
//! selected.
...
unsafe extern "C" {
    // siginterrupt.c uses public sigaction in both source calls. Do not turn
    // that ordinary public composition into a hidden implementation call.
    #[link_name = "sigaction"]
    fn public_sigaction(
        signal: c_int,
        action: *const c_void,
