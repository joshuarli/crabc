...
//!
//! This leaf owns exactly C `msync`. It composes the Linux syscall register
//! boundary, C `errno`, and the owned runtime's cancellation window. It is
...
//! cancellation assembly is `src/thread/x86_64/syscall_cp.s`. The owned
//! runtime preserves the source cancellation point before kernel validation;
//! standalone archive selections retain the raw `msync=26` syscall.
//! Linux retains ownership of flag,
...
/// kernel request. The owned runtime supplies pthread cancellation. This leaf
/// does not supply VM-wide synchronization or a completion/durability policy.
#[no_mangle]
