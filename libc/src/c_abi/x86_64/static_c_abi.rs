...
//! binary32/binary64 truncation, remainder, and cube root, selected
...
//! signal action/mask, one pure GNU signal-set predicate, one paired GNU
...
//! termios-control, two direct terminal-descriptor observations, one historical
...
//! and `clock_nanosleep`, direct clock-observation artifacts, one
//! caller-buffered fixed-UTC `gmtime_r` conversion artifact, and one fixed-UTC
