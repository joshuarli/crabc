...
//! maps `src/process/posix_spawnattr_sched.c::posix_spawnattr_getschedparam`
//! directly to [`posix_spawnattr_getschedparam`]. Its complete body is
...
//! pointer, writing a scheduler parameter, or issuing a scheduler syscall.
//!
//! The System V AMD64 ABI passes the declared read-only `posix_spawnattr_t *`
//! and mutable `struct sched_param *` values in `rdi` and `rsi`, then returns
//! the signed `int` result in `eax`. Musl intentionally ignores both pointer
//! values, including null, so this target-private composition does not import
//! either record layout, pointer validation, attribute initialization,
