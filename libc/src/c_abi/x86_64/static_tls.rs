...
//! executable's initial TLS template.  A freestanding entry shim passes the
//! untouched Linux initial stack to [`__crabc_x86_static_tls_bootstrap`]
//! before any C ABI entry can access `#[thread_local]` storage.  The bootstrap
//! validates the final executable's live `AT_PHDR` program-header table,
...
//! loader handoff, or general TCB contract.  The existing private x86
//! `rcrt1.o` static-PIE bootstrap remains a separately qualified prerequisite
//! until a later CRT composition profile calls this owner rather than keeping
//! equivalent first-thread arithmetic.
