...
//!
//! Linux 5.10 x86-64 provides raw syscall `sched_getparam=143`, but that
//! query targets a thread and writes a kernel scheduler record. Musl does not
//! expose it through the POSIX process-facing spelling. Preserve musl's
...
//! issuing syscall 143.
//!
//! This one-symbol leaf selects neither scheduler mutation, parameter records
//! or policy, priority bounds, `sched_yield`, affinity, pthread scheduling
//! attributes, thread/process lifecycle, dynamic or loader TLS, CRT, sysroot,
//! promotion, nor public x86 support.
...
/// dereferencing the otherwise-required scheduler record.
#[no_mangle]
