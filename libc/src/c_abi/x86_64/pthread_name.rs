...
//! list, or `/proc`/cancellation path. This leaf admits a selected main or
//! worker task through that task's own `pthread_self()` handle. It verifies
//! the existing `%fs:0` plus selected-task/Linux-TID identity before
...
//! the current self handle. Null, foreign, completed, and non-self handles
//! fail closed with candidate-only `ESRCH` before the name input or output is
//! observed. It does not select cross-thread names, musl's procfs task-name path,
