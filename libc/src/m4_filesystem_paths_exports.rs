// M4 filesystem path and descriptor operations with directly testable
...
const M4_SYS_FACCESSAT2: i64 = 439;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_FACCESSAT2: i64 = 439;

#[cfg(target_arch = "x86_64")]
const M4_SYS_FCHDIR: i64 = 81;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_FCHDIR: i64 = 50;

#[cfg(target_arch = "x86_64")]
const M4_SYS_MKNODAT: i64 = 259;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_MKNODAT: i64 = 33;

#[cfg(target_arch = "x86_64")]
const M4_SYS_FLOCK: i64 = 73;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_FLOCK: i64 = 32;

const M4_S_IFIFO: c_uint = 0o010000;

const M4_F_ULOCK: c_int = 0;
const M4_F_LOCK: c_int = 1;
const M4_F_TLOCK: c_int = 2;
const M4_F_TEST: c_int = 3;
const M4_AT_EACCESS: c_int = 0x200;

const M4_F_WRLCK: i16 = 1;
const M4_F_UNLCK: i16 = 2;
...
#[inline]
unsafe fn m4_fchdir(fd: c_int) -> i64 {
    <Arch as Syscalls>::syscall1(M4_SYS_FCHDIR, fd as i64)
}

#[inline]
unsafe fn m4_mknodat(
    dirfd: c_int,
    path: *const c_char,
    mode: c_uint,
    dev: c_ulong,
...
#[inline]
unsafe fn m4_lockf(fd: c_int, command: c_int, len: c_long) -> i64 {
    if command != M4_F_ULOCK
        && command != M4_F_LOCK
        && command != M4_F_TLOCK
        && command != M4_F_TEST
    {
    ...
    let lock_type = if command == M4_F_ULOCK {
        M4_F_UNLCK
    } else {
        M4_F_WRLCK
    };
    let mut lock = M4Flock {
        l_type: lock_type,
    ...
    let fcntl_command = match command {
        M4_F_LOCK => F_SETLKW,
        M4_F_TLOCK | M4_F_ULOCK => F_SETLK,
        M4_F_TEST => F_GETLK,
        _ => F_SETLK,
    };
    let result = sys_fcntl(fd, fcntl_command, &mut lock as *mut M4Flock as i64);
    if result < 0 {
        return syscall_result(result);
    }
    if command == M4_F_TEST && lock.l_type != M4_F_UNLCK {
        ERRNO = EACCES_VAL;
        return -1;
    }
    0
...
pub unsafe extern "C" fn faccessat(
    ...
    syscall_result(m4_faccessat(dirfd, path, mode, flags)) as c_int
}
...
pub unsafe extern "C" fn euidaccess(path: *const c_char, mode: c_int) -> c_int {
    syscall_result(m4_faccessat(AT_FDCWD, path, mode, M4_AT_EACCESS)) as c_int
}
...
pub unsafe extern "C" fn mkfifoat(
    ...
    syscall_result(m4_mknodat(dirfd, path, mode | M4_S_IFIFO, 0)) as c_int
}
...
pub unsafe extern "C" fn mknod(
    ...
    syscall_result(m4_mknodat(AT_FDCWD, path, mode, dev)) as c_int
}
...
pub unsafe extern "C" fn mknodat(
    ...
    syscall_result(m4_mknodat(dirfd, path, mode, dev)) as c_int
}

#[no_mangle]
pub unsafe extern "C" fn flock(fd: c_int, operation: c_int) -> c_int {
    syscall_result(m4_flock(fd, operation)) as c_int
}

#[no_mangle]
pub unsafe extern "C" fn lockf(fd: c_int, command: c_int, len: c_long) -> c_int {
    m4_lockf(fd, command, len) as c_int
}
...
pub unsafe extern "C" fn futimes(
    ...
    let times = match m4_timespecs(times, &mut converted) {
        Ok(times) => times,
        Err(error) => {
            ERRNO = error;
            return -1;
