...
const CABI_ADJ_OFFSET_SINGLESHOT: c_uint = 0x8001;
...
pub unsafe extern "C" fn clock_adjtime(clock_id: c_int, tx: *mut CabiTimex) -> c_int {
    syscall_result(cabi_clock_adjtime(clock_id, tx)) as c_int
}

#[no_mangle]
pub unsafe extern "C" fn adjtimex(tx: *mut CabiTimex) -> c_int {
    ...
    syscall_result(cabi_adjtimex(tx)) as c_int
}
...
pub unsafe extern "C" fn adjtime(increment: *const timeval, remaining: *mut timeval) -> c_int {
    ...
    let mut tx: CabiTimex = core::mem::zeroed();
    if !increment.is_null() {
        ...
        tx.modes = CABI_ADJ_OFFSET_SINGLESHOT;
    }
...
pub unsafe extern "C" fn settimeofday(tv: *const timeval, tz: *const c_void) -> c_int {
    ...
    syscall_result(cabi_settimeofday(tv, tz)) as c_int
}
