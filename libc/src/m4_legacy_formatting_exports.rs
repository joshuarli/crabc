// M4 legacy formatting interfaces.
//
...
const M4_MM_PRINT: c_int = 256;
const M4_MM_CONSOLE: c_int = 512;
const M4_MM_NOTOK: c_int = -1;
const M4_MM_NOMSG: c_int = 1;
const M4_MM_NOCON: c_int = 4;

const M4_MM_HALT: c_int = 1;
const M4_MM_ERROR: c_int = 2;
const M4_MM_WARNING: c_int = 3;
const M4_MM_INFO: c_int = 4;

unsafe fn m4_fmtmsg_component_mismatch(
    wanted: *const u8,
    actual: *const u8,
) -> bool {
    let mut i = 0usize;
...
unsafe fn m4_fmtmsg_verb_mask() -> c_int {
...
pub unsafe extern "C" fn fmtmsg(
    ...
    let severity_text = match severity {
        M4_MM_HALT => b"HALT: \0".as_ptr() as *const c_char,
        M4_MM_ERROR => b"ERROR: \0".as_ptr() as *const c_char,
        M4_MM_WARNING => b"WARNING: \0".as_ptr() as *const c_char,
        M4_MM_INFO => b"INFO: \0".as_ptr() as *const c_char,
        _ => b"\0".as_ptr() as *const c_char,
    } as *const c_char;

    if classification & M4_MM_CONSOLE as c_long != 0 {
        let console_fd = open(b"/dev/console\0".as_ptr() as *const c_char, O_WRONLY, 0);
        if console_fd < 0 {
            result = M4_MM_NOCON;
        } else {
            let written = m4_fmtmsg_write(
                console_fd,
                [
    ...
    if classification & M4_MM_PRINT as c_long != 0 {
        let verb = m4_fmtmsg_verb_mask();
        let written = m4_fmtmsg_write(
            2,
            [
    ...
    if result == (M4_MM_NOCON | M4_MM_NOMSG) {
        M4_MM_NOTOK
    } else {
        result
    }
...
#[inline]
fn m4_strfmon_digit(value: u8) -> bool {
    value >= b'0' && value <= b'9'
}

unsafe fn m4_vstrfmon_l(
    output: *mut c_char,
    mut remaining: usize,
    format: *const c_char,
    mut args: VaList,
...
pub unsafe extern "C" fn strfmon_l(
    ...
    m4_vstrfmon_l(output, remaining, format, args)
}
