// M4 scheduling and CPU-affinity exports.
//
...
const M4_ESRCH: c_int = 3;

#[cfg(target_arch = "x86_64")]
const M4_SYS_SCHED_YIELD: i64 = 24;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_SCHED_YIELD: i64 = 124;

#[cfg(target_arch = "x86_64")]
const M4_SYS_SCHED_GET_PRIORITY_MAX: i64 = 146;
#[cfg(target_arch = "x86_64")]
const M4_SYS_SCHED_GET_PRIORITY_MIN: i64 = 147;
#[cfg(target_arch = "x86_64")]
const M4_SYS_SCHED_RR_GET_INTERVAL: i64 = 148;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_SCHED_GET_PRIORITY_MAX: i64 = 125;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_SCHED_GET_PRIORITY_MIN: i64 = 126;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_SCHED_RR_GET_INTERVAL: i64 = 127;

#[cfg(target_arch = "x86_64")]
const M4_SYS_SCHED_SETAFFINITY: i64 = 203;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_SCHED_SETAFFINITY: i64 = 122;

#[cfg(target_arch = "x86_64")]
const M4_SYS_SCHED_GETAFFINITY: i64 = 204;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_SCHED_GETAFFINITY: i64 = 123;

#[cfg(target_arch = "x86_64")]
const M4_SYS_GETCPU: i64 = 309;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_SYS_GETCPU: i64 = 168;

#[inline]
unsafe fn m4_sched_rr_get_interval(pid: c_int, interval: *mut timespec) -> i64 {
    <Arch as Syscalls>::syscall2(
        M4_SYS_SCHED_RR_GET_INTERVAL,
        pid as i64,
        interval as i64,
    )
}
...
#[inline]
unsafe fn m4_sched_setaffinity(pid: c_int, cpusetsize: SizeT, mask: *const c_void) -> i64 {
    <Arch as Syscalls>::syscall3(
        M4_SYS_SCHED_SETAFFINITY,
        pid as i64,
        cpusetsize as i64,
        mask as i64,
    )
...
#[inline]
unsafe fn m4_sched_getaffinity(pid: c_int, cpusetsize: SizeT, mask: *mut c_void) -> i64 {
    let result = <Arch as Syscalls>::syscall3(
        M4_SYS_SCHED_GETAFFINITY,
        pid as i64,
        cpusetsize as i64,
        mask as i64,
    );
...
#[inline]
unsafe fn m4_getcpu(cpu: *mut c_uint, node: *mut c_uint) -> i64 {
    <Arch as Syscalls>::syscall3(M4_SYS_GETCPU, cpu as i64, node as i64, 0)
}

#[inline]
fn m4_pthread_errno(result: i64) -> c_int {
...
#[inline]
unsafe fn m4_thread_tid(thread: PthreadT) -> Result<c_int, c_int> {
    if thread == 0 {
        return Err(M4_ESRCH);
    }
...
pub unsafe extern "C" fn sched_getcpu() -> c_int {
    let mut cpu = 0u32;
    let mut node = 0u32;
    if syscall_result(m4_getcpu(&mut cpu, &mut node)) < 0 {
        -1
    } else {
        cpu as c_int
    }
...
pub unsafe extern "C" fn sched_rr_get_interval(pid: c_int, interval: *mut timespec) -> c_int {
    syscall_result(m4_sched_rr_get_interval(pid, interval)) as c_int
}
...
pub unsafe extern "C" fn sched_getaffinity(
    ...
    let result = m4_sched_getaffinity(pid, cpusetsize, mask);
    if result < 0 {
...
pub unsafe extern "C" fn sched_setaffinity(
    ...
    syscall_result(m4_sched_setaffinity(pid, cpusetsize, mask)) as c_int
}
...
pub unsafe extern "C" fn pthread_getaffinity_np(
    ...
    let tid = match m4_thread_tid(thread) {
        Ok(tid) => tid,
        Err(error) => return error,
    };
    m4_pthread_errno(m4_sched_getaffinity(tid, cpusetsize, mask))
}
...
pub unsafe extern "C" fn pthread_setaffinity_np(
    ...
    m4_pthread_errno(m4_sched_setaffinity(tid, cpusetsize, mask))
}
