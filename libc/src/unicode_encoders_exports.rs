...
static mut CABI_C16RTOMB_INTERNAL_STATE: c_uint = 0;
static mut CABI_MBRTOC16_INTERNAL_STATE: c_uint = 0;
static mut CABI_MBRTOC32_INTERNAL_STATE: c_uint = 0;
...
pub unsafe extern "C" fn c16rtomb(
    ...
    if ps.is_null() {
        ps = core::ptr::addr_of_mut!(CABI_C16RTOMB_INTERNAL_STATE);
    }
    let state = &mut *ps;
    let c16 = c16 as u32;
...
pub unsafe extern "C" fn mbrtoc16(
    ...
    if ps.is_null() {
        ps = core::ptr::addr_of_mut!(CABI_MBRTOC16_INTERNAL_STATE);
    }
    let pending = &mut *ps;
...
pub unsafe extern "C" fn mbrtoc32(
    ...
    if ps.is_null() {
        ps = core::ptr::addr_of_mut!(CABI_MBRTOC32_INTERNAL_STATE);
    }
    if s.is_null() {
