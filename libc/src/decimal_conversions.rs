// legacy decimal-conversion interfaces.
//
...
static mut CABI_ECVT_RESULT: [c_char; 17] = [0; 17];

unsafe fn cabi_ecvt_result_ptr() -> *mut c_char {
    core::ptr::addr_of_mut!(CABI_ECVT_RESULT).cast::<c_char>()
}

unsafe fn cabi_ecvt_special(x: f64, decpt: *mut c_int, sign: *mut c_int) -> *mut c_char {
    let word = if x.is_nan() { b"nan\0" } else { b"inf\0" };
    let result = cabi_ecvt_result_ptr();
...
pub unsafe extern "C" fn ecvt(
    ...
    if !x.is_finite() {
        return cabi_ecvt_special(x, decpt, sign);
    }
...
unsafe fn cabi_gcvt_large(x: f64, n: c_int, buf: *mut c_char) -> *mut c_char {
