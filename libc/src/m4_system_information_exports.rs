...
const M4_INFO_SYS_SYSINFO: i64 = 99;
#[cfg(target_arch = "x86_64")]
const M4_INFO_SYS_SCHED_GETAFFINITY: i64 = 204;

#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_INFO_SYS_SYSINFO: i64 = 179;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const M4_INFO_SYS_SCHED_GETAFFINITY: i64 = 123;

const M4_INFO_AT_PAGESZ: c_ulong = 6;
const M4_INFO_LOAD_SCALE: f64 = 65536.0;
const M4_INFO_CPUSET_BYTES: usize = 128;
const M4_INFO_CLOCK_PROCESS_CPUTIME_ID: c_int = 2;
const M4_INFO_ESRCH: c_int = 3;
const M4_INFO_ENOSYS: c_int = 38;
...
#[inline]
unsafe fn m4_info_page_size() -> u64 {
    ...
    getauxval(M4_INFO_AT_PAGESZ) as u64
}

#[inline]
unsafe fn m4_info_page_count(info: &M4Sysinfo, available: bool) -> c_long {
    let page_size = m4_info_page_size();
    if page_size == 0 {
        ERRNO = M4_INFO_ENOSYS;
        return -1;
    }
...
#[inline]
unsafe fn m4_info_nprocs() -> c_int {
    ...
    let mut mask = [0u8; M4_INFO_CPUSET_BYTES];
    mask[0] = 1;
    let _ = <Arch as Syscalls>::syscall3(
        M4_INFO_SYS_SCHED_GETAFFINITY,
        0,
        M4_INFO_CPUSET_BYTES as i64,
...
#[no_mangle]
#[linkage = "weak"]
pub unsafe extern "C" fn sysinfo(info: *mut M4Sysinfo) -> c_int {
    syscall_result(m4_info_sysinfo_raw(info)) as c_int
}
...
pub unsafe extern "C" fn getloadavg(loads: *mut f64, n: c_int) -> c_int {
    ...
    let mut info: M4Sysinfo = core::mem::zeroed();
    if syscall_result(m4_info_sysinfo_raw(&mut info)) < 0 {
        return -1;
    }
    ...
    let mut i = 0usize;
    while i < count {
        *loads.add(i) = info.loads[i] as f64 / M4_INFO_LOAD_SCALE;
        i += 1;
    }
    count as c_int
}
