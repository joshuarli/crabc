...
const CABI_SYS_MQ_OPEN: i64 = 240;
#[cfg(target_arch = "x86_64")]
const CABI_SYS_MQ_UNLINK: i64 = 241;
#[cfg(target_arch = "x86_64")]
const CABI_SYS_MQ_TIMEDSEND: i64 = 242;
#[cfg(target_arch = "x86_64")]
const CABI_SYS_MQ_TIMEDRECEIVE: i64 = 243;
#[cfg(target_arch = "x86_64")]
const CABI_SYS_MQ_NOTIFY: i64 = 244;
#[cfg(target_arch = "x86_64")]
const CABI_SYS_MQ_GETSETATTR: i64 = 245;

#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const CABI_SYS_MQ_OPEN: i64 = 180;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const CABI_SYS_MQ_UNLINK: i64 = 181;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const CABI_SYS_MQ_TIMEDSEND: i64 = 182;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const CABI_SYS_MQ_TIMEDRECEIVE: i64 = 183;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const CABI_SYS_MQ_NOTIFY: i64 = 184;
#[cfg(any(target_arch = "aarch64", target_arch = "riscv64"))]
const CABI_SYS_MQ_GETSETATTR: i64 = 185;
...
unsafe fn cabi_mq_kernel_name(name: *const c_char) -> *const c_char {
    if name.is_null() {
        return name;
    }
...
unsafe fn cabi_mq_unlink(name: *const c_char) -> i64 {
    <Arch as Syscalls>::syscall1(CABI_SYS_MQ_UNLINK, name as i64)
}

#[inline]
unsafe fn cabi_mq_timedsend(
    mqdes: c_int,
...
#[inline]
unsafe fn cabi_mq_timedreceive(
    mqdes: c_int,
    msg_ptr: *mut c_char,
    msg_len: SizeT,
    msg_prio: *mut c_uint,
...
unsafe fn cabi_mq_notify(mqdes: c_int, notification: *const c_void) -> i64 {
    <Arch as Syscalls>::syscall2(CABI_SYS_MQ_NOTIFY, mqdes as i64, notification as i64)
}

#[inline]
unsafe fn cabi_mq_getsetattr(
    mqdes: c_int,
...
pub unsafe extern "C" fn mq_open(
    ...
    let (mode, attr) = if oflag & O_CREAT != 0 {
        (
            ...
            args.next_arg::<*const CabiMqAttr>(),
        )
    } else {
        (0, core::ptr::null())
    };
    let kernel_name = cabi_mq_kernel_name(name);
    if !name.is_null() && kernel_name.is_null() {
        return -1;
    }
    syscall_result(cabi_mq_open(kernel_name, oflag, mode, attr)) as c_int
}
...
pub unsafe extern "C" fn mq_unlink(name: *const c_char) -> c_int {
    ...
    syscall_result(cabi_mq_unlink(kernel_name)) as c_int
}
...
pub unsafe extern "C" fn mq_send(
    ...
    syscall_result(cabi_mq_timedsend(
        mqdes,
        msg_ptr,
        msg_len,
        msg_prio,
...
pub unsafe extern "C" fn mq_receive(
    ...
    syscall_result(cabi_mq_timedreceive(
        mqdes,
        msg_ptr,
        msg_len,
        msg_prio,
...
#[no_mangle]
pub unsafe extern "C" fn mq_getattr(mqdes: c_int, attr: *mut CabiMqAttr) -> c_int {
    syscall_result(cabi_mq_getsetattr(
        mqdes,
        core::ptr::null(),
        attr,
    )) as c_int
...
pub unsafe extern "C" fn mq_setattr(
    ...
    syscall_result(cabi_mq_getsetattr(mqdes, new_attr, old_attr)) as c_int
}

#[no_mangle]
pub unsafe extern "C" fn mq_notify(mqdes: c_int, notification: *const c_void) -> c_int {
    syscall_result(cabi_mq_notify(mqdes, notification)) as c_int
}
