// M4 filesystem-stat and legacy timestamp exports.
//
...
#[inline]
unsafe fn m4_statfs(path: *const c_char, buf: *mut M4Statfs) -> i64 {
    <Arch as Syscalls>::syscall2(SYS_STATFS, path as i64, buf as i64)
}

#[inline]
unsafe fn m4_fstatfs(fd: c_int, buf: *mut M4Statfs) -> i64 {
    <Arch as Syscalls>::syscall2(SYS_FSTATFS, fd as i64, buf as i64)
}

#[no_mangle]
#[linkage = "weak"]
pub unsafe extern "C" fn statfs(path: *const c_char, buf: *mut M4Statfs) -> c_int {
    syscall_result(m4_statfs(path, buf)) as c_int
}

#[no_mangle]
#[linkage = "weak"]
pub unsafe extern "C" fn fstatfs(fd: c_int, buf: *mut M4Statfs) -> c_int {
    syscall_result(m4_fstatfs(fd, buf)) as c_int
}

#[inline]
unsafe fn m4_legacy_timeval_pair(
    times: *const timeval,
...
#[no_mangle]
pub unsafe extern "C" fn utime(path: *const c_char, times: *const M4Utimbuf) -> c_int {
...
pub unsafe extern "C" fn lutimes(
    ...
    let mut converted: [timespec; 2] = core::mem::zeroed();
    let times_ptr = match m4_legacy_timeval_pair(times, &mut converted) {
        Ok(value) => value,
        Err(error) => {
            ERRNO = error;
            return -1;
...
#[no_mangle]
pub unsafe extern "C" fn ftime(buf: *mut M4Timeb) -> c_int {
