...
const CABI_NSIG: usize = 65;
const CABI_SIGSET_WORDS: usize = CABI_NSIG / 8 / core::mem::size_of::<c_ulong>();

#[inline]
unsafe fn cabi_sigset_words(set: *const SigSetT) -> *const c_ulong {
    set as *const c_ulong
}

#[inline]
unsafe fn cabi_sigset_words_mut(set: *mut SigSetT) -> *mut c_ulong {
    set as *mut c_ulong
}
...
pub unsafe extern "C" fn sigandset(
    ...
    let d = cabi_sigset_words_mut(dest);
    let l = cabi_sigset_words(left);
    let r = cabi_sigset_words(right);
    let mut i = 0usize;
    while i < CABI_SIGSET_WORDS {
...
pub unsafe extern "C" fn sigisemptyset(set: *const SigSetT) -> c_int {
    let words = cabi_sigset_words(set);
    let mut i = 0usize;
    while i < CABI_SIGSET_WORDS {
        if *words.add(i) != 0 {
            return 0;
...
#[repr(C)]
pub union CabiSigval {
    pub sival_int: c_int,
    pub sival_ptr: *mut c_void,
}

#[inline]
unsafe fn cabi_rt_sigqueueinfo(
    pid: c_int,
    sig: c_int,
...
#[inline]
unsafe fn cabi_siginfo_write_i32(info: &mut [u8; 128], offset: usize, value: c_int) {
    core::ptr::write_unaligned(info.as_mut_ptr().add(offset) as *mut c_int, value);
}

#[no_mangle]
pub unsafe extern "C" fn sigqueue(pid: c_int, sig: c_int, value: CabiSigval) -> c_int {
    ...
    cabi_siginfo_write_i32(&mut info.bytes, 0, sig);
    cabi_siginfo_write_i32(&mut info.bytes, 8, -1); // SI_QUEUE
    cabi_siginfo_write_i32(&mut info.bytes, 16, getpid());
    core::ptr::write_unaligned(
        info.bytes.as_mut_ptr().add(20) as *mut c_uint,
        getuid(),
    );
    core::ptr::copy_nonoverlapping(
        &value as *const CabiSigval as *const u8,
        info.bytes.as_mut_ptr().add(24),
        core::mem::size_of::<CabiSigval>(),
    );
    syscall_result(cabi_rt_sigqueueinfo(pid, sig, &info)) as c_int
}

static CABI_SIGNAL_NAMES: [&[u8]; 65] = [
    b"Unknown signal\0",
    b"Hangup\0",
    b"Interrupt\0",
    b"Quit\0",
...
pub unsafe extern "C" fn strsignal(signum: c_int) -> *mut c_char {
    let index = if signum > 0 && signum < CABI_SIGNAL_NAMES.len() as c_int {
        signum as usize
    } else {
        0
    };
    CABI_SIGNAL_NAMES[index].as_ptr() as *mut c_char
}
