...
# These cases are not meaningful on Docker's root overlay. Each exception must
# be reproduced against the pinned musl oracle before it is admitted here.
ORACLE_ENVIRONMENT_SKIPS = {
    ("regression", "statvfs"): (
        "pinned musl also reports zero inode capacity for Docker's root overlay"
    ),
}
    ...
    for directory in dirs:
        ...
        for source in sorted(source_dir.glob("*.c")):
            ...
            if base.endswith("_dso"):
                continue
            counters["TOTAL"] += 1
            skip_reason = ORACLE_ENVIRONMENT_SKIPS.get((directory, base))
            if skip_reason is not None:
                append_text(raw_report, f"SKIP {directory}/{base}: {skip_reason}\n")
                record_event(events_report, directory, base, "SKIP", "preflight", "oracle_environment", Path("-"))
                counters["SKIP"] += 1
                continue
            ...
            compile_status = execute(
                [compiler, *strict_api_flags, *flags, *extra_flags, "-c", "-o", str(object_file), str(source)],
