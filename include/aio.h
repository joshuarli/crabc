...
/*
 * Pinned musl leaves aio.h's embedded struct sigevent incomplete in the
 * macro-free strict profiles. The candidate keeps those two source-only
 * closure rows consumable with an opaque 64-byte record; no strict-profile
 * signal or AIO runtime contract is implied.
 */
...
struct sigevent {
	char __crabc_strict_aio_sigevent[64];
};
#endif
